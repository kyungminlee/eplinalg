      SUBROUTINE YHESCAL( UPLO, M, N, IOFFD, ALPHA, A, LDA )
      IMPLICIT NONE
*
...
*  =======
*
*  YHESCAL  scales  a two-dimensional  array A by the real scalar alpha.
...
*
*  A       (input/output) COMPLEX(KIND=10) array
...
*
*     .. Parameters ..
      REAL(KIND=10)   RONE, RZERO
      PARAMETER          ( RONE = 1.0E0_10, RZERO = 0.0E0_10 )
...
*
     ...
     $       LSAME( UPLO, 'D' ) ) THEN
            DO 10 J = MAX( 0, -IOFFD ) + 1, MIN( M - IOFFD, N )
               JTMP = J + IOFFD
               A( JTMP, J ) = CMPLX( REAL( A( JTMP, J ) , KIND=10), 
     +RZERO , KIND=10)
   10       CONTINUE
         END IF
         RETURN
      ELSE IF( ALPHA.EQ.RZERO ) THEN
         CALL YTZPAD( UPLO, 'N', M, N, IOFFD, ZERO, ZERO, A, LDA )
         RETURN
      END IF
*
...
*
         MN = MAX( 0, -IOFFD )
         DO 20 J = 1, MIN( MN, N )
            CALL YESCAL( M, ALPHA, A( 1, J ), 1 )
   20    CONTINUE
         DO 30 J = MN + 1, MIN( M - IOFFD, N )
            JTMP = J + IOFFD
            A( JTMP, J ) = CMPLX( ALPHA * REAL( A( JTMP, J ) , KIND=10),
     + RZERO , KIND=10)
            IF( M.GT.JTMP )          CALL YESCAL( M-JTMP, ALPHA, A( 
     +JTMP + 1, J ), 1 )
   30    CONTINUE
*
...
*
         ...
         DO 40 J = MAX( 0, -IOFFD ) + 1, MN
            JTMP = J + IOFFD
            CALL YESCAL( JTMP - 1, ALPHA, A( 1, J ), 1 )
            A( JTMP, J ) = CMPLX( ALPHA * REAL( A( JTMP, J ) , KIND=10),
     + RZERO , KIND=10)
   40    CONTINUE
