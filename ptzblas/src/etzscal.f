      SUBROUTINE ETZSCAL( UPLO, M, N, IOFFD, ALPHA, A, LDA )
      IMPLICIT NONE
*
...
*  =======
*
*  ETZSCAL scales a two-dimensional array A by the scalar alpha.
*
*  Arguments
*  =========
*
...
*
*  A       (input/output) REAL(KIND=10) array
...
*
      IF( ALPHA.EQ.ZERO ) THEN
         CALL ETZPAD( UPLO, 'N', M, N, IOFFD, ZERO, ZERO, A, LDA )
         RETURN
      END IF
*
...
*
   ...
   10    CONTINUE
         DO 20 J = MN + 1, MIN( M - IOFFD, N )
            JTMP = J + IOFFD
            IF( M.GE.JTMP )          CALL ESCAL( M-JTMP+1, ALPHA, A( 
     +JTMP, J ), 1 )
   20    CONTINUE
*
...
*
         ...
         DO 30 J = MAX( 0, -IOFFD ) + 1, MN
            CALL ESCAL( J + IOFFD, ALPHA, A( 1, J ), 1 )
   30    CONTINUE
