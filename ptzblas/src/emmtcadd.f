      SUBROUTINE EMMTCADD( M, N, ALPHA, A, LDA, BETA, B, LDB )
      IMPLICIT NONE
*
...
*  =======
*
*  EMMTCADD performs the following operation:
*
*     B := alpha * A' + beta * B,
*
...
*          not be set on input.
*
*  A       (local input) REAL(KIND=10) array
...
*          be set on input.
*
*  B       (local input/local output) REAL(KIND=10) array
      ...
      IF( M.GE.N ) THEN
         IF( ALPHA.EQ.ONE ) THEN
            IF( BETA.EQ.ZERO ) THEN
               DO 20 J = 1, N
                  CALL ECOPY( M, A( 1, J ), 1, B( J, 1 ), LDB )
*                 DO 10 I = 1, M
*                    B( J, I ) = A( I, J )
*  10             CONTINUE
   20          CONTINUE
   ...
   40          CONTINUE
            ELSE
               DO 60 J = 1, N
                  CALL EAXPY( M, ONE, A( 1, J ), 1, B( J, 1 ), LDB )
*                 DO 50 I = 1, M
*                    B( J, I ) = A( I, J ) + B( J, I )
*  50             CONTINUE
   60          CONTINUE
  ...
  100          CONTINUE
            ELSE
               DO 120 J = 1, N
                  CALL EAXPY( M, ALPHA, A( 1, J ), 1, B( J, 1 ), LDB )
*                 DO 110 I = 1, M
  ...
  160          CONTINUE
      ...
      ELSE
         IF( ALPHA.EQ.ONE ) THEN
            IF( BETA.EQ.ZERO ) THEN
               DO 180 J = 1, M
                  CALL ECOPY( N, A( J, 1 ), LDA, B( 1, J ), 1 )
*                 DO 170 I = 1, N
*                    B( I, J ) = A( J, I )
* 170             CONTINUE
  180          CONTINUE
  ...
  200          CONTINUE
            ELSE
               DO 220 J = 1, M
                  CALL EAXPY( N, ONE, A( J, 1 ), LDA, B( 1, J ), 1 )
*                 DO 210 I = 1, N
*                    B( I, J ) = A( J, I ) + B( I, J )
* 210             CONTINUE
  220          CONTINUE
  ...
  260          CONTINUE
            ELSE
               DO 280 J = 1, M
                  CALL EAXPY( N, ALPHA, A( J, 1 ), LDA, B( 1, J ), 1 )
*                 DO 270 I = 1, N
