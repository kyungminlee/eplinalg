/*
 * yhpr — kind10 port of OpenBLAS zhpr.  Hermitian packed rank-1 update.
 *
 *   A := alpha * x * x^H + A   (alpha REAL; A Hermitian, packed)
 *
 * Threaded path mirrors driver/level2/spr_thread.c (HER variant): sqrt-
 ...
 * forward-mapped.
 *
 * Fortran ABI:  subroutine yhpr(uplo, n, alpha, x, incx, ap)
 */
...
void yhpr_(const char *UPLO, const int *N, const R *ALPHA,
