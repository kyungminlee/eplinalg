 ...
 * Reference ZGEMMTR algorithm, kind16. Conjugation under 'C' applied
 * at element access — one sign flip vs. ~100 cycles per libquadmath
 * complex mul, so the branch cost is noise.
 */
#include <stddef.h>
#include <ctype.h>
#include <quadmath.h>
              ...
              size_t uplo_len, size_t ta_len, size_t tb_len)
{
    ...
    if (alpha == zero || K == 0) {
        if (beta == one) return;
#ifdef _OPENMP
        const int use_omp0 = (N >= XGEMMTR_OMP_MIN && blas_omp_max_threads() > 1);
...
#ifdef _OPENMP
    const int use_omp = (N >= XGEMMTR_OMP_MIN && blas_omp_max_threads() > 1);
