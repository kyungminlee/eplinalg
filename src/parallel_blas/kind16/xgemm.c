 ...
 *
 *   xgemm_         — top-level entry. Opens its own `#pragma omp parallel
 *                    for collapse(2)` over the (M, N) tile grid. Use this
 *                    from any non-OMP caller (LAPACK, user code, etc.).
 *
 *   xgemm_serial_  — bare serial entry. No OpenMP pragma anywhere on the
 ...
 *                    refactored xtrsv_blocked) hand each thread a slice
 *                    of the trailing matrix and call this on the slice.
 *
 * Both go through `xgemm_tile_compute` for numerics so the parallel and
 * serial paths produce bitwise-identical results.
 *
 * As a defensive guard, xgemm_ also checks `omp_in_parallel()` and skips
 ...
 *
 * TRANSA / TRANSB independently in {N, T, C}. Conjugation under 'C' is
 * applied at element access time.
 */
 ...
 */
static void xgemm_tile_compute(
