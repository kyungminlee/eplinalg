...
void pyrot_( Int *n, ECOMPLEX X[], Int *ix, Int *jx, Int desc_X[], Int *incx, ECOMPLEX Y[], Int *iy, Int *jy, Int desc_Y[], Int *incy, EREAL *c, ECOMPLEX *s )
/*
*  Mark Fahey
*  June 22, 2000
*/
...
*  =======
*
*  PYROT applies a plane rotation, where the cos (C) is real and the
...
*/
   ...
   void        ygerv2d_( Int *ictxt, Int *m, Int *n, ECOMPLEX *A, Int *lda, Int *rsrc, Int *csrc );
   void        ygesd2d_( Int *ictxt, Int *m, Int *n, ECOMPLEX *A, Int *lda, Int *rdest, Int *cdest );
   ...
   F_INTG_FCT  pbytrnv_( Int *ictxt, char *scope, char *trans, Int *n, Int *nb, Int *nz, ECOMPLEX *A, Int *lda, ECOMPLEX *beta, ECOMPLEX *work, Int *ldwork, Int *prow, Int *pcol, Int *qrow, Int *qcol, ECOMPLEX *buf );
   F_INTG_FCT  yrot_( Int *n, ECOMPLEX *cx, Int *incx, ECOMPLEX *cy, Int *incy, EREAL *c, ECOMPLEX *s );
...
*/
   ...
   if( info ) { PB_Cabort( ictxt, "PYROT", info ); return; }
/*
   if( info )
   {
      pberror_( &ictxt, "PYROT", &info );
      return;
   }
*/
...
*/
   if( *n == 1 )
   {
      if( ( myrow == ixrow ) && ( mycol == ixcol ) )
      {
         ...
         {
            ygesd2d_( &ictxt, n, n, buff, n, &iyrow, &iycol );
            ygerv2d_( &ictxt, n, n, ywork, n, &iyrow, &iycol );
         }
         else
            *ywork = Y[iiy-1+(jjy-1)*desc_Y[LLD_]];
         yrot_( n, buff, n, ywork, n, c, s );
      ...
      {
         ygesd2d_( &ictxt, n, n, &Y[iiy-1+(jjy-1)*desc_Y[LLD_]], n,
                   &ixrow, &ixcol );
         ygerv2d_( &ictxt, n, n, xwork, n, &ixrow, &ixcol );
         yrot_( n, xwork, n, &Y[iiy-1+(jjy-1)*desc_Y[LLD_]], n, c, s );
      }
      return;
   }
   ...
   {               /* X and Y are both distributed over a process row */
      ...
      if( ixrow == iyrow )
      {
         if( myrow == ixrow )
         {
            yrot_( &nq, &X[iix-1+(jjx-1)*desc_X[LLD_]], &desc_X[LLD_],
      ...
      else
      {
         if( myrow == ixrow )
         {
            ygesd2d_( &ictxt, &ione, &nq,
                      &X[iix-1+(jjx-1)*desc_X[LLD_]], &desc_X[LLD_],
                      &iyrow, &mycol );
            buff = (ECOMPLEX *)getpbbuf( "PYROT", nq*sizeof(ECOMPLEX) );
            ygerv2d_( &ictxt, &nq, &ione, buff, &nq, &iyrow, &mycol );
            yrot_( &nq, &X[iix-1+(jjx-1)*desc_X[LLD_]], &desc_X[LLD_],
                        buff, &ione, c, s );
         }
         else if( myrow == iyrow )
         {
            ygesd2d_( &ictxt, &ione, &nq,
                      ...
                      &ixrow, &mycol );
            buff = (ECOMPLEX *)getpbbuf( "PYROT", nq*sizeof(ECOMPLEX) );
            ygerv2d_( &ictxt, &nq, &ione, buff, &nq, &ixrow, &mycol );
            yrot_( &nq, buff, &ione,
                        &Y[iiy-1+(jjy-1)*desc_Y[LLD_]], &desc_Y[LLD_], c, s );
         }
      }
   ...
   {            /* X and Y are both distributed over a process column */
      ...
      if( ixcol == iycol )
      {
         if( mycol == ixcol )
         {
            yrot_( &np, &X[iix-1+(jjx-1)*desc_X[LLD_]], incx,
      ...
      else
      {
         if( mycol == ixcol )
         {
            ygesd2d_( &ictxt, &np, &ione,
                      &X[iix-1+(jjx-1)*desc_X[LLD_]], &desc_X[LLD_],
                      &myrow, &iycol );
            buff = (ECOMPLEX *)getpbbuf( "PYROT", np*sizeof(ECOMPLEX) );
            ygerv2d_( &ictxt, &np, &ione, buff, &np, &myrow, &iycol );
            yrot_( &np, &X[iix-1+(jjx-1)*desc_X[LLD_]], incx,
                        buff, &ione, c, s );
         }
         else if( mycol == iycol )
         {
            ygesd2d_( &ictxt, &np, &ione,
                      ...
                      &myrow, &ixcol );
            buff = (ECOMPLEX *)getpbbuf( "PYROT", np*sizeof(ECOMPLEX) );
            ygerv2d_( &ictxt, &np, &ione, buff, &np, &myrow, &ixcol );
            yrot_( &np, buff, &ione,
                        &Y[iiy-1+(jjy-1)*desc_Y[LLD_]], incy, c, s );
         }
      }
   ...
   else       /* X and Y are not distributed along the same direction */
   {
      ...
      {                     /* X is distributed over a process column */
         ...
         buff = (ECOMPLEX *)getpbbuf( "PYROT", wksz*sizeof(ECOMPLEX) );

         if( mycol == iycol )
            jjy -= nz;
         if( myrow == ixrow )
            np -= nz;
         pbytrnv_( &ictxt, C2F_CHAR( "R" ), C2F_CHAR( "T" ), n,
                   &desc_Y[NB_], &nz, &Y[iiy-1+(jjy-1)*desc_Y[LLD_]],
                   &desc_Y[LLD_], &zero, buff, &ione, &iyrow, &iycol,
                   &ixrow, &ixcol, buff+np );
         if( mycol == ixcol )
         {
            yrot_( &np, &X[iix-1+(jjx-1)*desc_X[LLD_]],
                     incx, buff, &ione, c, s );
         }
         pbytrnv_( &ictxt, C2F_CHAR( "R" ), C2F_CHAR( "T" ), n,
