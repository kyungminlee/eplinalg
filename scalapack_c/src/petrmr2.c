 ...
 * 
 * some functions used by the petrmr2d routine see file pdtrmr.c for more
 * documentation.
 * 
...
void
setmemory(EREAL **adpointer, Int blocksize)
{
  assert(blocksize >= 0);
  if (blocksize == 0) {
    *adpointer = NULL;
  ...
  *adpointer = (EREAL *) mr2d_malloc(
				      (size_t)blocksize * sizeof(EREAL));
}
/******************************************************************/
/* Free the memory space after the malloc */
void
freememory(EREAL *ptrtobefreed)
{
  if (ptrtobefreed == NULL)
    return;
  free((char *) ptrtobefreed);
}
/* extern functions for intersect() extern ecopy_(); */
/**************************************************************/
...
intersect(char *uplo, char *diag, Int j, Int start,
           Int end, Int action, Int *ptrsizebuff, EREAL **pptrbuff,
           EREAL *ptrblock, Int m, Int n, MDESC *ma,
           Int ia, Int ja, Int templateheight0, Int templatewidth0,
           MDESC *mb, Int ib, Int jb, Int templateheight1,
           Int templatewidth1){
  /* int       un = 1; only when we use ecopy instead of memcpy */
  EREAL *ptrstart;
  Int   offset, nbline;
  Int   intervalsize;
  assert(start < end);
  ...
  case SENDBUFF:	/* fill buff with local elements to be sent */
    ...
    memcpy((char *) (*pptrbuff), (char *) ptrstart,
	   intervalsize * sizeof(EREAL));
    /* ecopy_(&intervalsize, (char *) (ptrstart), &un, (char *) (*pptrbuff),
     * &un); */
  ...
  case RECVBUFF:	/* fill local memory with the values received */
    ...
    memcpy((char *) ptrstart, (char *) (*pptrbuff),
	   intervalsize * sizeof(EREAL));
    /* ecopy_(&intervalsize, (char *) (*pptrbuff), &un, (char *) (ptrstart),
     * &un); */
    (*pptrbuff) += intervalsize;
    break;
...
scanD0(char *uplo, char *diag,
       ...
       EREAL *ptrbuff,		/* address of the communication ptrbuffer (a suffisant
