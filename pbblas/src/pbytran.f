      SUBROUTINE PBYTRAN( ICONTXT, ADIST, TRANS, M, N, NB, A, LDA, BETA,
...
*     .. Scalar Arguments ..
     ...
     $                   M, N, NB
      COMPLEX(KIND=10)            BETA
*     ..
*     .. Array Arguments ..
      COMPLEX(KIND=10)            A( LDA, * ), C( LDC, * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PBYTRAN  transposes  a column block to row block, or a row block to
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGEBR2D, YGEBS2D, YGERV2D,     
     +               YGESD2D, PBYMATADD, PBYTR2AF, PBYTR2AT,            
     +        PBYTR2BT, PBYTRGET, PBYTRSRT, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
   10 CONTINUE
      IF( INFO .NE. 0 ) THEN
         CALL PXERBLA( ICONTXT, 'PBYTRAN ', INFO )
         RETURN
      END IF
*
...
*
              ...
              IF( MYROW.EQ.JCROW .AND. MYCOL.EQ.MCCOL ) THEN
                CALL PBYTR2AT( ICONTXT, 'Col', TRANS, MP-IDEX, N, NB,   
...
*
              ELSE
                CALL PBYTR2BT( ICONTXT, 'Col', TRANS, MP-IDEX, N, NB,   
     +                       A(IDEX+1,1), LDA, ZERO, WORK, N,           
     +               LCMP*NB )
                CALL YGESD2D( ICONTXT, N, MQ0, WORK, N, JCROW, MCCOL )
              END IF
*
...
*
            ELSE IF( MYROW.EQ.JCROW .AND. MYCOL.EQ.MCCOL ) THEN
              IF( LCMQ.EQ.1 .AND. TBETA.EQ.ZERO ) THEN
                CALL YGERV2D( ICONTXT, N, MQ0, C, LDC, MCROW, IACOL )
              ELSE
                CALL YGERV2D( ICONTXT, N, MQ0, WORK, N, MCROW, IACOL )
                CALL PBYTR2AF( ICONTXT, 'Row', N, MQ-JDEX, NB, WORK, N, 
...
*
          IF( ICROW.EQ.-1 ) THEN
            IF( MYROW.EQ.JCROW ) THEN
              CALL YGEBS2D( ICONTXT, 'Col', '1-tree', N, MQ, C, LDC )
            ELSE
              CALL YGEBR2D( ICONTXT, 'Col', '1-tree', N, MQ, C, LDC,    
     +                   JCROW, MYCOL )
            END IF
          END IF
*
          ...
          DO 30 I = 0, LCMP-1
            IF( MRCOL.EQ.MOD( NPROW*I+MRROW, NPCOL ) ) THEN
              IF( LCMQ.EQ.1.AND.(ICROW.EQ.-1.OR.ICROW.EQ.MYROW) ) THEN
                 CALL PBYTR2BT( ICONTXT, 'Col', TRANS, MP-I*NB, N, NB,  
...
*
          ...
          IF( LCMQ.GT.1 ) THEN
            ...
            CALL PBYTRGET( ICONTXT, 'Row', N, MQ0, ICEIL(M,NB), WORK, N,
...
*
          IF( ICROW.EQ.-1 ) THEN
            IF( MYROW.EQ.MCROW ) THEN
              IF( LCMQ.GT.1 )           CALL PBYTRSRT( ICONTXT, 'Row', 
...
*
          ELSE
            IF( LCMQ.EQ.1 ) THEN
              IF( MYROW.EQ.MCROW ) THEN
                IF( MYROW.NE.ICROW )             CALL YGESD2D( ICONTXT, 
     +N, MQ, WORK, N, ICROW, MYCOL )
              ELSE IF( MYROW.EQ.ICROW ) THEN
                IF( BETA.EQ.ZERO ) THEN
                  CALL YGERV2D( ICONTXT, N, MQ, C, LDC, MCROW, MYCOL )
                ELSE
                  CALL YGERV2D( ICONTXT, N, MQ, WORK, N, MCROW, MYCOL )
                  CALL PBYMATADD( ICONTXT, 'G', N, MQ, ONE, WORK, N,    
     +                         BETA, C, LDC )
                END IF
              END IF
*
     ...
     +N, ML, WORK, N, ICROW, MYCOL )
              ELSE IF( MYROW.EQ.ICROW ) THEN
                CALL YGERV2D( ICONTXT, N, ML, WORK, N, MCROW, MYCOL )
              END IF
*
              IF( MYROW.EQ.ICROW )           CALL PBYTRSRT( ICONTXT, 
...
*
               ...
               IF( MYROW.EQ.MCROW .AND. MYCOL.EQ.JCCOL ) THEN
                 CALL PBYTR2AT( ICONTXT, 'Row', TRANS, M, NQ-JDEX, NB,  
...
*
               ELSE
                 CALL PBYTR2BT( ICONTXT, 'Row', TRANS, M, NQ-JDEX, NB,  
     +                         A(1,JDEX+1), LDA, ZERO, WORK, NP0,       
     +                    LCMQ*NB )
                 CALL YGESD2D( ICONTXT, NP0, M, WORK, NP0,              
     +            MCROW, JCCOL )
               END IF
*
*           A destination node receives the copied blocks
*
            ELSE IF( MYROW.EQ.MCROW .AND. MYCOL.EQ.JCCOL ) THEN
              IF( LCMP.EQ.1 .AND. TBETA.EQ.ZERO ) THEN
                CALL YGERV2D( ICONTXT, NP0, M, C, LDC, IAROW, MCCOL )
              ELSE
                CALL YGERV2D( ICONTXT, NP0, M, WORK, NP0, IAROW, MCCOL )
                CALL PBYTR2AF( ICONTXT, 'Col', NP-IDEX, M, NB, WORK,    
...
*
          IF( ICCOL.EQ.-1 ) THEN
            IF( MYCOL.EQ.JCCOL ) THEN
              CALL YGEBS2D( ICONTXT, 'Row', '1-tree', NP, M, C, LDC )
            ELSE
              CALL YGEBR2D( ICONTXT, 'Row', '1-tree', NP, M, C, LDC,    
     +                    MYROW, JCCOL )
            END IF
          END IF
*
          ...
          DO 50 I = 0, LCMQ-1
            IF( MRROW.EQ.MOD(NPCOL*I+MRCOL, NPROW) ) THEN
              IF( LCMP.EQ.1.AND.(ICCOL.EQ.-1.OR.ICCOL.EQ.MYCOL) ) THEN
                CALL PBYTR2BT( ICONTXT, 'Row', TRANS, M, NQ-I*NB, NB,   
...
*
          ...
          IF( LCMP.GT.1 ) THEN
            ...
            CALL PBYTRGET( ICONTXT, 'Col', NP0, M, ICEIL(N,NB), WORK,   
...
*
          IF( ICCOL.EQ.-1 ) THEN
            IF( MYCOL.EQ.MCCOL ) THEN
              IF( LCMP.GT.1 )           CALL PBYTRSRT( ICONTXT, 'Col', 
...
*
          ELSE
            IF( LCMP.EQ.1 ) THEN
              IF( MYCOL.EQ.MCCOL ) THEN
                IF( MYCOL.NE.ICCOL )             CALL YGESD2D( ICONTXT, 
     +NP, M, WORK, NP, MYROW, ICCOL )
              ELSE IF( MYCOL.EQ.ICCOL ) THEN
                IF( BETA.EQ.ZERO ) THEN
                  CALL YGERV2D( ICONTXT, NP, M, C, LDC, MYROW, MCCOL )
                ELSE
                  CALL YGERV2D( ICONTXT, NP, M, WORK, NP, MYROW, MCCOL )
                  CALL PBYMATADD( ICONTXT, 'G', NP, M, ONE, WORK, NP,   
     +                          BETA, C, LDC )
                END IF
              END IF
*
     ...
     +NP0, ML, WORK, NP0,                           MYROW, ICCOL )
              ELSE IF( MYCOL.EQ.ICCOL ) THEN
                CALL YGERV2D( ICONTXT, NP0, ML, WORK, NP0,              
     +           MYROW, MCCOL )
              END IF
*
              IF( MYCOL.EQ.ICCOL )           CALL PBYTRSRT( ICONTXT, 
...
*=======================================================================
*
      SUBROUTINE PBYTR2AT( ICONTXT, ADIST, TRANS, M, N, NB, A, LDA,     
...
*     ..
*     .. Array Arguments ..
      COMPLEX(KIND=10)            A( LDA, * ), B( LDB, * )
*     ..
*
*  Purpose
*  =======
*
*  PBYTR2AT forms   B <== A^T + beta*B, or A^C + beta*B
...
*
      IF( LCMP.EQ.LCMQ ) THEN
         CALL PBYMATADD( ICONTXT, TRANS, N, M, ONE, A, LDA, BETA, B,    
     +                LDB )
*
      ELSE
*
...
*
         IF( LSAME( ADIST, 'C' ) ) THEN
            ...
            DO 10 K = 1, ICEIL( M, INTV )
               CALL PBYMATADD( ICONTXT, TRANS, N, MIN( M-IA+1, NB ),    
...
*
         ELSE
            ...
            DO 20 K = 1, ICEIL( N, JNTV )
               CALL PBYMATADD( ICONTXT, TRANS, MIN( N-IA+1, NB ), M,    
...
*=======================================================================
*
      SUBROUTINE PBYTR2BT( ICONTXT, ADIST, TRANS, M, N, NB, A, LDA,     
     +                 BETA, B, LDB, INTV )
      IMPLICIT NONE
*
*  -- PB-BLAS routine (version 2.1) --
...
*  =======
*
*  PBYTR2BT forms T <== A^T + beta*T or A^C + beta*T, where T is a
...
*=======================================================================
*
      SUBROUTINE PBYTR2AF( ICONTXT, ADIST, M, N, NB, A, LDA, BETA, B,   
     +                   LDB, LCMP, LCMQ, NINT )
      IMPLICIT NONE
*
*  -- PB-BLAS routine (version 2.1) --
...
*  =======
*
*  PBYTR2AF forms  T <== A + BETA*T, where T is a scattered block
...
*
      IF( LSAME( ADIST, 'R' ) ) THEN
         ...
         DO 10 K = 1, ICEIL( NINT, NB )
            CALL PBYMATADD( ICONTXT, 'G', M, MIN( N-JB+1, NB ), ONE,    
...
*
      ELSE
         ...
         DO 20 K = 1, ICEIL( NINT, NB )
            CALL PBYMATADD( ICONTXT, 'G', MIN( M-JB+1, NB ), N, ONE,    
