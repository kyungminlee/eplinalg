      SUBROUTINE PBETRNV( ICONTXT, XDIST, TRANS, N, NB, NZ, X, INCX,    
...
*     ..
*     .. Array Arguments ..
      REAL(KIND=10)   WORK( * ), X( * ), Y( * )
*     ..
*
*  Purpose
*  =======
*
*  PBETRNV transposes a column vector to row vector, or a row vector to
...
*
*  Y       (input/output) REAL(KIND=10) array of dimension at least
...
*     .. External Subroutines ..
     ...
     +               EGESD2D, PBETR2A1, PBETR2B1, PBETRGET,             
     +       PBETRST1, PBEVECADD, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
   10 CONTINUE
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICONTXT, 'PBETRNV ', INFO )
         RETURN
      END IF
*
...
*
              ...
              IF( MYROW.EQ.JYROW .AND. MYCOL.EQ.MCCOL ) THEN
                CALL PBETR2B1( ICONTXT, TRANS, NP-IDEX, NB, KZ,         
...
*
     ...
     +             LCMP, 1 )
                CALL EGESD2D( ICONTXT, 1, NQ0-KZ, WORK, 1,              
     +           JYROW, MCCOL )
              END IF
*
...
*
            ELSE IF( MYROW.EQ.JYROW .AND. MYCOL.EQ.MCCOL ) THEN
              IF( LCMQ.EQ.1 .AND. TBETA.EQ.ZERO ) THEN
                CALL EGERV2D( ICONTXT, 1, NQ0-KZ, Y, INCY,              
     +           MCROW, IXCOL )
              ELSE
                CALL EGERV2D( ICONTXT, 1, NQ0-KZ, WORK, 1,              
     +           MCROW, IXCOL )
                CALL PBETR2A1( ICONTXT, NQ-JDEX, NB, KZ, WORK, 1, TBETA,
...
*
          IF( IYROW.EQ.-1 ) THEN
            IF( MYROW.EQ.JYROW ) THEN
              CALL EGEBS2D( ICONTXT, 'Col', '1-tree', 1, NQ, Y, INCY )
            ELSE
              CALL EGEBR2D( ICONTXT, 'Col', '1-tree', 1, NQ, Y, INCY,   
     +                   JYROW, MYCOL )
             END IF
          END IF
*
          ...
          DO 30 I = 0, LCMP - 1
            IF( MRCOL.EQ.MOD(NPROW*I+MRROW, NPCOL) ) THEN
              ...
              IF( LCMQ.EQ.1 .AND. (IYROW.EQ.-1.OR.IYROW.EQ.MYROW) ) THEN
                 CALL PBETR2B1( ICONTXT, TRANS, NP-IDEX, NB, JZ,        
...
*
          ...
          IF( LCMQ.GT.1 ) THEN
            ...
            CALL PBETRGET( ICONTXT, 'Row', 1, NQ0, ICEIL( NN, NB ),     
          ...
          IF( IYROW.EQ.-1 ) THEN
            IF( MYROW.EQ.MCROW ) THEN
              IF( LCMQ.GT.1 ) THEN
                ...
                CALL PBETRST1( ICONTXT, 'Row', NQ, NB, KZ, WORK, 1,     
...
*
          ELSE
            IF( LCMQ.EQ.1 ) THEN
              IF( MYROW.EQ.MCROW ) THEN
                IF( MYROW.NE.IYROW )             CALL EGESD2D( ICONTXT, 
     +1, NQ0, WORK, 1, IYROW, MYCOL )
              ELSE IF( MYROW.EQ.IYROW ) THEN
                IF( BETA.EQ.ZERO ) THEN
                  CALL EGERV2D( ICONTXT, 1, NQ0, Y, INCY, MCROW, MYCOL )
                ELSE
                  CALL EGERV2D( ICONTXT, 1, NQ0, WORK, 1, MCROW, MYCOL )
                  CALL PBEVECADD( ICONTXT, 'G', NQ0, ONE, WORK, 1,      
     +                       BETA, Y, INCY )
                END IF
              END IF
*
            ELSE
              ...
              IF( MYROW.EQ.MCROW ) THEN
                IF( MYROW.NE.IYROW )             CALL EGESD2D( ICONTXT, 
     +1, NQ1, WORK, 1, IYROW, MYCOL )
              ELSE IF( MYROW.EQ.IYROW ) THEN
                CALL EGERV2D( ICONTXT, 1, NQ1, WORK, 1, MCROW, MYCOL )
              END IF
*
...
*
              ...
              IF( MYROW.EQ.MCROW .AND. MYCOL.EQ.JYCOL ) THEN
                CALL PBETR2B1( ICONTXT, TRANS, NQ-IDEX, NB, KZ,         
     +                 X(IDEX*INCX+1), INCX, TBETA,                     
...
*
     ...
     +                 X(IDEX*INCX+1), INCX, ZERO, WORK, 1,             
     +             LCMQ, 1 )
                CALL EGESD2D( ICONTXT, 1, NP0-KZ, WORK, 1,              
     +           MCROW, JYCOL )
              END IF
*
...
*
            ELSE IF( MYROW.EQ.MCROW .AND. MYCOL.EQ.JYCOL ) THEN
              IF( LCMP.EQ.1 .AND. TBETA.EQ.ZERO ) THEN
                CALL EGERV2D( ICONTXT, 1, NP0-KZ, Y, INCY,              
     +           IXROW, MCCOL )
              ELSE
                CALL EGERV2D( ICONTXT, 1, NP0-KZ, WORK, 1,              
     +           IXROW, MCCOL )
                CALL PBETR2A1( ICONTXT, NP-JDEX, NB, KZ, WORK, 1, TBETA,
...
*
          IF( IYCOL.EQ.-1 ) THEN
            IF( MYCOL.EQ.JYCOL ) THEN
              CALL EGEBS2D( ICONTXT, 'Row', '1-tree', 1, NP, Y, INCY )
            ELSE
              CALL EGEBR2D( ICONTXT, 'Row', '1-tree', 1, NP, Y, INCY,   
     +                    MYROW, JYCOL )
            END IF
          END IF
*
          ...
          DO 50 I = 0, LCMQ-1
            IF( MRROW.EQ.MOD(NPCOL*I+MRCOL, NPROW) ) THEN
              ...
              IF( LCMP.EQ.1 .AND. (IYCOL.EQ.-1.OR.IYCOL.EQ.MYCOL) ) THEN
                CALL PBETR2B1( ICONTXT, TRANS, NQ-IDEX, NB, JZ,         
     +                  X(IDEX*INCX+1), INCX, BETA, Y, INCY,            
     +               LCMQ, 1 )
              ELSE
                CALL PBETR2B1( ICONTXT, TRANS, NQ-IDEX, NB, JZ,         
...
*
          ...
          IF( LCMP.GT.1 ) THEN
            ...
            CALL PBETRGET( ICONTXT, 'Col', 1, NP0, ICEIL( NN, NB ),     
     +                 WORK, 1, MCROW, MCCOL, IGD, MYROW, MYCOL,        
     +              NPROW, NPCOL )
          END IF
*
          ...
          IF( IYCOL.EQ.-1 ) THEN
            IF( MYCOL.EQ.MCCOL ) THEN
              IF( LCMP.GT.1 ) THEN
                ...
                CALL PBETRST1( ICONTXT, 'Col', NP, NB, KZ, WORK, 1,     
...
*
          ELSE
            IF( LCMP.EQ.1 ) THEN
              IF( MYCOL.EQ.MCCOL ) THEN
                IF( MYCOL.NE.IYCOL )             CALL EGESD2D( ICONTXT, 
     +1, NP, WORK, 1, MYROW, IYCOL )
              ELSE IF( MYCOL.EQ.IYCOL ) THEN
                IF( BETA.EQ.ZERO ) THEN
                  CALL EGERV2D( ICONTXT, 1, NP, Y, INCY, MYROW, MCCOL )
                ELSE
                  CALL EGERV2D( ICONTXT, 1, NP, WORK, 1, MYROW, MCCOL )
                  CALL PBEVECADD( ICONTXT, 'G', NP, ONE, WORK, 1, BETA, 
     +                            Y, INCY )
                END IF
              END IF
*
            ELSE
              ...
              IF( MYCOL.EQ.MCCOL ) THEN
                IF( MYCOL.NE.IYCOL )             CALL EGESD2D( ICONTXT, 
     +1, NP1, WORK, 1, MYROW, IYCOL )
              ELSE IF( MYCOL.EQ.IYCOL ) THEN
                CALL EGERV2D( ICONTXT, 1, NP1, WORK, 1, MYROW, MCCOL )
              END IF
*
...
*=======================================================================
*
      SUBROUTINE PBETR2A1( ICONTXT, N, NB, NZ, X, INCX, BETA, Y, INCY,  
     +                    INTV )
      IMPLICIT NONE
*
*  -- PB-BLAS routine (version 2.1) --
...
*
      IF( ITER.GT.1 ) THEN
         CALL PBEVECADD( ICONTXT, 'G', NB-JZ, ONE, X(IX*INCX+1), INCX,  
...
*
         DO 10 K = 2, ITER-1
            CALL PBEVECADD( ICONTXT, 'G', NB, ONE, X(IX*INCX+1), INCX,  
     +                     BETA, Y(IY*INCY+1), INCY )
            IX = IX + NB
            IY = IY + INTV
   10    CONTINUE
      END IF
*
      CALL PBEVECADD( ICONTXT, 'G', MIN( N-IY, NB-JZ ), ONE,            
...
*=======================================================================
*
      SUBROUTINE PBETR2B1( ICONTXT, TRANS, N, NB, NZ, X, INCX, BETA, Y, 
     +                     INCY, JINX, JINY )
      IMPLICIT NONE
*
*  -- PB-BLAS routine (version 2.1) --
...
*
      IF( JINX.EQ.1 .AND. JINY.EQ.1 ) THEN
         CALL PBEVECADD( ICONTXT, TRANS, N, ONE, X, INCX, BETA,         
     +           Y, INCY )
*
      ELSE
         IX   = 0
...
*
         IF( ITER.GT.1 ) THEN
            CALL PBEVECADD( ICONTXT, TRANS, NB-JZ, ONE, X(IX*INCX+1),   
...
*
            DO 10 K = 2, ITER-1
               CALL PBEVECADD( ICONTXT, TRANS, NB, ONE, X(IX*INCX+1),   
     +                       INCX, BETA, Y(IY*INCY+1), INCY )
               IX = IX + LENX
               IY = IY + LENY
   10       CONTINUE
         END IF
*
         CALL PBEVECADD( ICONTXT, TRANS, MIN( N-IX, NB-JZ ), ONE,       
     +             X(IX*INCX+1), INCX, BETA, Y(IY*INCY+1), INCY )
      END IF
*
      RETURN
