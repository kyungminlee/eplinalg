      SUBROUTINE PBETRAN( ICONTXT, ADIST, TRANS, M, N, NB, A, LDA, BETA,
...
*     ..
*     .. Array Arguments ..
      REAL(KIND=10)   A( LDA, * ), C( LDC, * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PBETRAN  transposes  a column block to row block, or a row block to
...
*
*  A       (input) REAL(KIND=10) array of DIMENSION ( LDA, Lx ),
...
*
*  BETA    (input) REAL(KIND=10)
*          BETA specifies scaler beta.
*
*  C       (input/output) REAL(KIND=10) array of DIMENSION
*          ( LDC, Lx ),
...
*          set ICCOL = -1.
*
*  WORK    (workspace) REAL(KIND=10) array of dimension Size(WORK).
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGEBR2D, EGEBS2D, EGERV2D,     
     +               EGESD2D, PBEMATADD, PBETR2AF, PBETR2AT,            
     +        PBETR2BT, PBETRGET, PBETRSRT, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
   10 CONTINUE
      IF( INFO .NE. 0 ) THEN
         CALL PXERBLA( ICONTXT, 'PBETRAN ', INFO )
         RETURN
      END IF
*
...
*
              ...
              IF( MYROW.EQ.JCROW .AND. MYCOL.EQ.MCCOL ) THEN
                CALL PBETR2AT( ICONTXT, 'Col', TRANS, MP-IDEX, N, NB,   
...
*
              ELSE
                CALL PBETR2BT( ICONTXT, 'Col', TRANS, MP-IDEX, N, NB,   
     +                       A(IDEX+1,1), LDA, ZERO, WORK, N,           
     +               LCMP*NB )
                CALL EGESD2D( ICONTXT, N, MQ0, WORK, N, JCROW, MCCOL )
              END IF
*
...
*
            ELSE IF( MYROW.EQ.JCROW .AND. MYCOL.EQ.MCCOL ) THEN
              IF( LCMQ.EQ.1 .AND. TBETA.EQ.ZERO ) THEN
                CALL EGERV2D( ICONTXT, N, MQ0, C, LDC, MCROW, IACOL )
              ELSE
                CALL EGERV2D( ICONTXT, N, MQ0, WORK, N, MCROW, IACOL )
                CALL PBETR2AF( ICONTXT, 'Row', N, MQ-JDEX, NB, WORK, N, 
...
*
          IF( ICROW.EQ.-1 ) THEN
            IF( MYROW.EQ.JCROW ) THEN
              CALL EGEBS2D( ICONTXT, 'Col', '1-tree', N, MQ, C, LDC )
            ELSE
              CALL EGEBR2D( ICONTXT, 'Col', '1-tree', N, MQ, C, LDC,    
     +                   JCROW, MYCOL )
            END IF
          END IF
*
          ...
          DO 30 I = 0, LCMP-1
            IF( MRCOL.EQ.MOD( NPROW*I+MRROW, NPCOL ) ) THEN
              IF( LCMQ.EQ.1.AND.(ICROW.EQ.-1.OR.ICROW.EQ.MYROW) ) THEN
                 CALL PBETR2BT( ICONTXT, 'Col', TRANS, MP-I*NB, N, NB,  
...
*
          ...
          IF( LCMQ.GT.1 ) THEN
            ...
            CALL PBETRGET( ICONTXT, 'Row', N, MQ0, ICEIL(M,NB), WORK, N,
...
*
          IF( ICROW.EQ.-1 ) THEN
            IF( MYROW.EQ.MCROW ) THEN
              IF( LCMQ.GT.1 )           CALL PBETRSRT( ICONTXT, 'Row', 
     +N, MQ, NB, WORK, N, BETA,                          C, LDC, LCMP, 
     +LCMQ, MQ0 )
              CALL EGEBS2D( ICONTXT, 'Col', '1-tree', N, MQ, C, LDC )
            ELSE
              CALL EGEBR2D( ICONTXT, 'Col', '1-tree', N, MQ, C, LDC,    
...
*
          ELSE
            IF( LCMQ.EQ.1 ) THEN
              IF( MYROW.EQ.MCROW ) THEN
                IF( MYROW.NE.ICROW )             CALL EGESD2D( ICONTXT, 
     +N, MQ, WORK, N, ICROW, MYCOL )
              ELSE IF( MYROW.EQ.ICROW ) THEN
                IF( BETA.EQ.ZERO ) THEN
                  CALL EGERV2D( ICONTXT, N, MQ, C, LDC, MCROW, MYCOL )
                ELSE
                  CALL EGERV2D( ICONTXT, N, MQ, WORK, N, MCROW, MYCOL )
                  CALL PBEMATADD( ICONTXT, 'G', N, MQ, ONE, WORK, N,    
     +                         BETA, C, LDC )
                END IF
              END IF
*
            ELSE
              ...
              IF( MYROW.EQ.MCROW ) THEN
                IF( MYROW.NE.ICROW )             CALL EGESD2D( ICONTXT, 
     +N, ML, WORK, N, ICROW, MYCOL )
              ELSE IF( MYROW.EQ.ICROW ) THEN
                CALL EGERV2D( ICONTXT, N, ML, WORK, N, MCROW, MYCOL )
              END IF
*
              IF( MYROW.EQ.ICROW )           CALL PBETRSRT( ICONTXT, 
     +'Row', N, MQ, NB, WORK, N, BETA,                          C, LDC, 
     +LCMP, LCMQ, MQ0 )
            END IF
          END IF
*
...
*
               ...
               IF( MYROW.EQ.MCROW .AND. MYCOL.EQ.JCCOL ) THEN
                 CALL PBETR2AT( ICONTXT, 'Row', TRANS, M, NQ-JDEX, NB,  
...
*
               ELSE
                 CALL PBETR2BT( ICONTXT, 'Row', TRANS, M, NQ-JDEX, NB,  
     +                         A(1,JDEX+1), LDA, ZERO, WORK, NP0,       
     +                    LCMQ*NB )
                 CALL EGESD2D( ICONTXT, NP0, M, WORK, NP0,              
     +            MCROW, JCCOL )
               END IF
*
*           A destination node receives the copied blocks
*
            ELSE IF( MYROW.EQ.MCROW .AND. MYCOL.EQ.JCCOL ) THEN
              IF( LCMP.EQ.1 .AND. TBETA.EQ.ZERO ) THEN
                CALL EGERV2D( ICONTXT, NP0, M, C, LDC, IAROW, MCCOL )
              ELSE
                CALL EGERV2D( ICONTXT, NP0, M, WORK, NP0, IAROW, MCCOL )
                CALL PBETR2AF( ICONTXT, 'Col', NP-IDEX, M, NB, WORK,    
...
*
          IF( ICCOL.EQ.-1 ) THEN
            IF( MYCOL.EQ.JCCOL ) THEN
              CALL EGEBS2D( ICONTXT, 'Row', '1-tree', NP, M, C, LDC )
            ELSE
              CALL EGEBR2D( ICONTXT, 'Row', '1-tree', NP, M, C, LDC,    
     +                    MYROW, JCCOL )
            END IF
          END IF
*
          ...
          DO 50 I = 0, LCMQ-1
            IF( MRROW.EQ.MOD(NPCOL*I+MRCOL, NPROW) ) THEN
              IF( LCMP.EQ.1.AND.(ICCOL.EQ.-1.OR.ICCOL.EQ.MYCOL) ) THEN
                CALL PBETR2BT( ICONTXT, 'Row', TRANS, M, NQ-I*NB, NB,   
...
*
          ...
          IF( LCMP.GT.1 ) THEN
            ...
            CALL PBETRGET( ICONTXT, 'Col', NP0, M, ICEIL(N,NB), WORK,   
...
*
          IF( ICCOL.EQ.-1 ) THEN
            IF( MYCOL.EQ.MCCOL ) THEN
              IF( LCMP.GT.1 )           CALL PBETRSRT( ICONTXT, 'Col', 
     +NP, M, NB, WORK, NP0,                          BETA, C, LDC, LCMP,
     + LCMQ, NP0 )
              CALL EGEBS2D( ICONTXT, 'Row', '1-tree', NP, M, C, LDC )
            ELSE
              CALL EGEBR2D( ICONTXT, 'Row', '1-tree', NP, M, C, LDC,    
...
*
          ELSE
            IF( LCMP.EQ.1 ) THEN
              IF( MYCOL.EQ.MCCOL ) THEN
                IF( MYCOL.NE.ICCOL )             CALL EGESD2D( ICONTXT, 
     +NP, M, WORK, NP, MYROW, ICCOL )
              ELSE IF( MYCOL.EQ.ICCOL ) THEN
                IF( BETA.EQ.ZERO ) THEN
                  CALL EGERV2D( ICONTXT, NP, M, C, LDC, MYROW, MCCOL )
                ELSE
                  CALL EGERV2D( ICONTXT, NP, M, WORK, NP, MYROW, MCCOL )
                  CALL PBEMATADD( ICONTXT, 'G', NP, M, ONE, WORK, NP,   
     +                          BETA, C, LDC )
                END IF
              END IF
*
            ELSE
              ...
              IF( MYCOL.EQ.MCCOL ) THEN
                IF( MYCOL.NE.ICCOL )             CALL EGESD2D( ICONTXT, 
     +NP0, ML, WORK, NP0,                           MYROW, ICCOL )
              ELSE IF( MYCOL.EQ.ICCOL ) THEN
                CALL EGERV2D( ICONTXT, NP0, ML, WORK, NP0,              
     +           MYROW, MCCOL )
              END IF
*
              IF( MYCOL.EQ.ICCOL )           CALL PBETRSRT( ICONTXT, 
     +'Col', NP, M, NB, WORK, NP0,                          BETA, C, 
     +LDC, LCMP, LCMQ, NP0 )
            END IF
          END IF
*
...
*=======================================================================
*
      SUBROUTINE PBETR2AT( ICONTXT, ADIST, TRANS, M, N, NB, A, LDA,     
...
*  =======
*
*  PBETR2AT forms   B <== A^T + beta*B, or A^C + beta*B
...
*
      IF( LCMP.EQ.LCMQ ) THEN
         CALL PBEMATADD( ICONTXT, TRANS, N, M, ONE, A, LDA, BETA, B,    
     +                LDB )
*
      ELSE
*
...
*
         IF( LSAME( ADIST, 'C' ) ) THEN
            ...
            DO 10 K = 1, ICEIL( M, INTV )
               CALL PBEMATADD( ICONTXT, TRANS, N, MIN( M-IA+1, NB ),    
...
*
         ELSE
            ...
            DO 20 K = 1, ICEIL( N, JNTV )
               CALL PBEMATADD( ICONTXT, TRANS, MIN( N-IA+1, NB ), M,    
...
*=======================================================================
*
      SUBROUTINE PBETR2BT( ICONTXT, ADIST, TRANS, M, N, NB, A, LDA,     
     +                 BETA, B, LDB, INTV )
      IMPLICIT NONE
*
*  -- PB-BLAS routine (version 2.1) --
...
*  =======
*
*  PBETR2BT forms T <== A^T + beta*T or A^C + beta*T, where T is a
...
*=======================================================================
*
      SUBROUTINE PBETR2AF( ICONTXT, ADIST, M, N, NB, A, LDA, BETA, B,   
     +                   LDB, LCMP, LCMQ, NINT )
      IMPLICIT NONE
*
*  -- PB-BLAS routine (version 2.1) --
...
*  =======
*
*  PBETR2AF forms  T <== A + BETA*T, where T is a scattered block
...
*
      IF( LSAME( ADIST, 'R' ) ) THEN
         ...
         DO 10 K = 1, ICEIL( NINT, NB )
            CALL PBEMATADD( ICONTXT, 'G', M, MIN( N-JB+1, NB ), ONE,    
...
*
      ELSE
         ...
         DO 20 K = 1, ICEIL( NINT, NB )
            CALL PBEMATADD( ICONTXT, 'G', MIN( M-JB+1, NB ), N, ONE,    
