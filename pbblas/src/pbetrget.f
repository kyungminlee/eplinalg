      SUBROUTINE PBETRGET( ICONTXT, ADIST, M, N, MNB, A, LDA, MCROW,    
...
*  =======
*
*  PBETRGET forms a row block of A from scattered row subblocks if
...
*
               IF( KMOD.EQ.0 ) THEN
                  IF( KPPOS+KINT.LT.NNUM ) THEN
                     ...
                     CALL EGERV2D( ICONTXT, M, KLEN, A(1,NLEN+1), LDA,  
     +                            MOD(MYROW+KINT, NPROW), MYCOL )
                     NLEN = NLEN + KLEN
                  END IF
               ELSE
                  CALL EGESD2D( ICONTXT, M, NLEN, A, LDA,               
...
*
     ...
     +                            MYROW, MOD(MYCOL+KINT, NPCOL) )
                  ...
                  END IF
               ELSE
                  CALL EGESD2D( ICONTXT, M, NLEN, A, LDA, MYROW,        
