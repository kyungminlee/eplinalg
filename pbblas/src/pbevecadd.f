      SUBROUTINE PBEVECADD( ICONTXT, MODE, N, ALPHA, X, INCX, BETA, Y,  
     +                     INCY )
      IMPLICIT NONE
*
...
*  =======
*
*  PBEVECADD performs a vector X to be added to Y
*    Y := alpha*op(X) + beta*Y,
...
*
            ELSE IF( BETA.EQ.ONE ) THEN
               IF( LSAME( MODE, 'V' ) ) THEN
                  CALL EAXPY( N, ALPHA, X, INCX, Y, INCY )
               ELSE IF( INCX.EQ.1 .AND. INCY.EQ.1 ) THEN
                  DO 130 I = 1, N
                     Y( I ) = ALPHA * X( I ) + Y( I )
  130             CONTINUE
