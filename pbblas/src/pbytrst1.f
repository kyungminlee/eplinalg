      SUBROUTINE PBYTRST1( ICONTXT, XDIST, N, NB, NZ, X, INCX, BETA, Y, 
     +                     INCY, LCMP, LCMQ, NINT )
      IMPLICIT NONE
*
...
*  =======
*
*  PBYTRST1 forms  y <== x + beta * y, where y is a sorted
...
*
            IF( ITER.GT.1 ) THEN
               CALL PBYVECADD( ICONTXT, 'G', NB-KZ, ONE, X(IX*INCX+1),  
...
*
               DO 10 K = 2, ITER-1
                  CALL PBYVECADD( ICONTXT, 'G', NB, ONE, X(IX*INCX+1),  
     +                           INCX, BETA, Y(IY*INCY+1), INCY )
                  IX = IX + NB
                  IY = IY + NJUMP
   10          CONTINUE
            END IF
*
            CALL PBYVECADD( ICONTXT, 'G', MIN(NB-KZ,N-IY), ONE,         
