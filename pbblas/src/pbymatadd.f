      SUBROUTINE PBYMATADD( ICONTXT, MODE, M, N, ALPHA, A, LDA, BETA, B,
     +                       LDB )
      IMPLICIT NONE
*
...
*  =======
*
*  PBYMATADD performs the matrix add operation B := alpha*A + beta*B,
         ...
         ELSE IF( ALPHA.EQ.ONE ) THEN
            IF( BETA.EQ.ZERO ) THEN
               IF( M.EQ.LDA .AND. M.EQ.LDB ) THEN
                  CALL YCOPY( M*N, A( 1, 1 ), 1, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 700 J = 1, N
...
*
            ELSE IF( BETA.EQ.ONE ) THEN
               IF( M.EQ.LDA .AND. M.EQ.LDB ) THEN
                  CALL YAXPY( M*N, ALPHA, A( 1, 1 ), 1, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 790 J = 1, N
