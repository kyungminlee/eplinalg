      SUBROUTINE PBEMATADD( ICONTXT, MODE, M, N, ALPHA, A, LDA, BETA, B,
     +                       LDB )
      IMPLICIT NONE
*
...
*  =======
*
*  PBEMATADD performs the matrix add operation B := alpha*A + beta*B,
...
*
*  ALPHA   (input) REAL(KIND=10)
*          ALPHA specifies the scalar alpha.
*
*  A       (input) REAL(KIND=10) array, dimension (LDA,N)
...
*
*  B       (input) REAL(KIND=10) array, dimension (LDB,N)
...
*
            ELSE
               IF( M.EQ.LDB ) THEN
                  CALL ESCAL( M*N, BETA, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 510 J = 1, N
                     CALL ESCAL( M, BETA, B( 1, J ), 1 )
  510             CONTINUE
               ELSE
                  DO 530 J = 1, N
                     DO 520 I = 1, M
         ...
         ELSE IF( ALPHA.EQ.ONE ) THEN
            IF( BETA.EQ.ZERO ) THEN
               IF( M.EQ.LDA .AND. M.EQ.LDB ) THEN
                  CALL ECOPY( M*N, A( 1, 1 ), 1, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 540 J = 1, N
                     CALL ECOPY( M, A( 1, J ), 1, B( 1, J ), 1 )
  540             CONTINUE
               ELSE
                  DO 560 J = 1, N
                     DO 550 I = 1, M
...
*
            ELSE IF( BETA.EQ.ONE ) THEN
               IF( M.EQ.LDA .AND. M.EQ.LDB ) THEN
                  CALL EAXPY( M*N, ALPHA, A( 1, 1 ), 1, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 630 J = 1, N
                     CALL EAXPY( M, ALPHA, A( 1, J ), 1, B( 1, J ), 1 )
  630             CONTINUE
               ELSE
                  DO 650 J = 1, N
                     DO 640 I = 1, M
