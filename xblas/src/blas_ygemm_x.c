 ...
 */
{
  static const char routine_name[] = "BLAS_ygemm_x";
  switch (prec) {
  ...
  case blas_prec_indigenous:{
	...
	for (i = 0; i < m; i++, ci += incci) {
	  ...
	  for (j = 0; j < n; j++, cij += inccij) {
	    ...
	    {
	      tmp1[0] =
		(EREAL) c_elem[0] * beta_i[0] -
		(EREAL) c_elem[1] * beta_i[1];
	      tmp1[1] =
		(EREAL) c_elem[0] * beta_i[1] +
		(EREAL) c_elem[1] * beta_i[0];
	    }
	    c_i[cij] = tmp1[0];
	    c_i[cij + 1] = tmp1[1];
	  }
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0; i < m; i++, ci += incci, ai += incai) {
		...
		{
		  prod[0] =
		    (EREAL) a_elem[0] * b_elem[0] -
		    (EREAL) a_elem[1] * b_elem[1];
		  prod[1] =
		    (EREAL) a_elem[0] * b_elem[1] +
		    (EREAL) a_elem[1] * b_elem[0];
		}
		sum[0] = sum[0] + prod[0];
		sum[1] = sum[1] + prod[1];
	      }
