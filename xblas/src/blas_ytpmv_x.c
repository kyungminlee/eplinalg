 ...
 */
{
  static const char routine_name[] = "BLAS_ytpmv_x";
...

	{
	  if ((uplo == blas_upper &&
		...
		if ((diag == blas_unit_diag) && (col_index == matrix_row)) {
		  {
		    rowtmp[0] =
		      (EREAL) vecval[0] * one[0] -
		      (EREAL) vecval[1] * one[1];
		    rowtmp[1] =
		      (EREAL) vecval[0] * one[1] +
		      (EREAL) vecval[1] * one[0];
		  }
		} else {
		  ...
		  {
		    rowtmp[0] =
		      (EREAL) matval[0] * vecval[0] -
		      (EREAL) matval[1] * vecval[1];
		    rowtmp[1] =
		      (EREAL) matval[0] * vecval[1] +
		      (EREAL) matval[1] * vecval[0];
		  }
		}
		...
		col_index++;
	      ...
	      {
		result[0] =
		  (EREAL) rowsum[0] * alpha_i[0] -
		  (EREAL) rowsum[1] * alpha_i[1];
		result[1] =
		  (EREAL) rowsum[0] * alpha_i[1] +
		  (EREAL) rowsum[1] * alpha_i[0];
	      }
  ...
  case blas_prec_extra:{
	...
	const EREAL *tp_i = (EREAL *) tp;
	EREAL *x_i = (EREAL *) x;
	EREAL head_rowsum[2], tail_rowsum[2];
	EREAL head_rowtmp[2], tail_rowtmp[2];
	EREAL head_result[2], tail_result[2];
	EREAL matval[2];
	EREAL vecval[2];
	EREAL one[2];
