...
{
  /* Routine name */
  static const char routine_name[] = "BLAS_yge_sum_mv";
  switch (prec) {
  ...
  case blas_prec_double:
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    for (j = 0, xi = x_starti; j < n; j++, xi += incxi) {
	      ...
	      {
		prod[0] =
		  (EREAL) b_elem[0] * x_elem[0] -
		  (EREAL) b_elem[1] * x_elem[1];
		prod[1] =
		  (EREAL) b_elem[0] * x_elem[1] +
		  (EREAL) b_elem[1] * x_elem[0];
	      }
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    {
	      tmp1[0] =
		(EREAL) sumB[0] * beta_i[0] - (EREAL) sumB[1] * beta_i[1];
	      tmp1[1] =
		(EREAL) sumB[0] * beta_i[1] + (EREAL) sumB[1] * beta_i[0];
	    }
	    y_i[yi] = tmp1[0];
	    y_i[yi + 1] = tmp1[1];
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    for (j = 0, xi = x_starti; j < n; j++, xi += incxi) {
	      ...
	      {
		prod[0] =
		  (EREAL) a_elem[0] * x_elem[0] -
		  (EREAL) a_elem[1] * x_elem[1];
		prod[1] =
		  (EREAL) a_elem[0] * x_elem[1] +
		  (EREAL) a_elem[1] * x_elem[0];
	      }
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    {
	      tmp1[0] =
		(EREAL) sumA[0] * alpha_i[0] - (EREAL) sumA[1] * alpha_i[1];
	      tmp1[1] =
		(EREAL) sumA[0] * alpha_i[1] + (EREAL) sumA[1] * alpha_i[0];
	    }
	    y_i[yi] = tmp1[0];
	    y_i[yi + 1] = tmp1[1];
	    ai += incai;
  ...
  case blas_prec_extra:
    {
      ...
      EREAL head_sumA[2], tail_sumA[2];
      EREAL head_sumB[2], tail_sumB[2];
