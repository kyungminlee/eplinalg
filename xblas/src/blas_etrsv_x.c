 ...
 */
{
  char *routine_name = "BLAS_etrsv";

  int i, j;			/* used to idx matrix */
  int ix, jx;			/* used to idx vector x */
  int start_x;			/* used as the starting idx to vector x */
  const EREAL *T_i = T;	/* internal matrix T */
  EREAL *x_i = x;		/* internal x */
  EREAL alpha_i = alpha;	/* internal alpha */
  EREAL T_element;		/* temporary variable for an element of matrix A */
  ...
  case blas_prec_indigenous:
    ...
    {
      EREAL temp1;		/* temporary variable for calculations */
      EREAL temp2;		/* temporary variable for calculations */
      EREAL temp3;		/* temporary variable for calculations */
  ...
  case blas_prec_extra:
      ...
      {
	{
	  ...
	  EREAL head_temp1, tail_temp1;	/* temporary variable for calculations */
	  EREAL head_temp2, tail_temp2;	/* temporary variable for calculations */
	  EREAL head_temp3, tail_temp3;	/* temporary variable for calculations */
	  EREAL *head_intx, *tail_intx;	/* copy of x used for calculations */

	  /* allocate space for intx */
	  head_intx = (EREAL *) blas_malloc(n * sizeof(EREAL));
	  tail_intx = (EREAL *) blas_malloc(n * sizeof(EREAL));
