 ...
 */
{
  static const char routine_name[] = "BLAS_ygemv2_x";
  switch (prec) {
  ...
  case blas_prec_indigenous:{
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0; i < leny; i++) {
	    ...
	    {
	      tmp1[0] =
		(EREAL) y_elem[0] * beta_i[0] -
		(EREAL) y_elem[1] * beta_i[1];
	      tmp1[1] =
		(EREAL) y_elem[0] * beta_i[1] +
		(EREAL) y_elem[1] * beta_i[0];
	    }
	    y_i[iy] = tmp1[0];
	    y_i[iy + 1] = tmp1[1];
	    iy += incy;
	...
	if (trans == blas_conj_trans) {
	  ...
	  if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
		...
		{
		  tmp1[0] =
		    (EREAL) sum[0] * alpha_i[0] -
		    (EREAL) sum[1] * alpha_i[1];
		  tmp1[1] =
		    (EREAL) sum[0] * alpha_i[1] +
		    (EREAL) sum[1] * alpha_i[0];
		}
		{
		  tmp2[0] =
		    (EREAL) sum2[0] * alpha_i[0] -
		    (EREAL) sum2[1] * alpha_i[1];
		  tmp2[1] =
		    (EREAL) sum2[0] * alpha_i[1] +
		    (EREAL) sum2[1] * alpha_i[0];
		}
