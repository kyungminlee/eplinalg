 ...
 */
{
  char *routine_name = "BLAS_ytrsv";

  int i, j;			/* used to idx matrix */
  int ix, jx;			/* used to idx vector x */
  int start_x;			/* used as the starting idx to vector x */
  const EREAL *T_i = (EREAL *) T;	/* internal matrix T */
  EREAL *x_i = (EREAL *) x;	/* internal x */
  EREAL *alpha_i = (EREAL *) alpha;	/* internal alpha */
  EREAL T_element[2];		/* temporary variable for an element of matrix A */
  ...
  case blas_prec_indigenous:
    ...
    {
      EREAL temp1[2];		/* temporary variable for calculations */
      EREAL temp2[2];		/* temporary variable for calculations */
      EREAL temp3[2];		/* temporary variable for calculations */
	...
	if (trans == blas_conj_trans) {
	  ...
	  for (j = n - 1; j >= 0; j--) {
	    ...
	    {
	      temp1[0] =
		(EREAL) temp3[0] * alpha_i[0] -
		(EREAL) temp3[1] * alpha_i[1];
	      temp1[1] =
		(EREAL) temp3[0] * alpha_i[1] +
		(EREAL) temp3[1] * alpha_i[0];
	    }
	    ...
	    for (i = n - 1; i >= j + 1; i--) {
	      ...
	      {
		temp2[0] =
		  (EREAL) temp3[0] * T_element[0] -
		  (EREAL) temp3[1] * T_element[1];
		temp2[1] =
		  (EREAL) temp3[0] * T_element[1] +
		  (EREAL) temp3[1] * T_element[0];
	      }
	    ...
	    if (diag == blas_non_unit_diag) {
	      ...
	      {
		EREAL S = 1.0, eps, ov, un;
		EREAL abs_a, abs_b, abs_c, abs_d, ab, cd;
		EREAL r;
		EREAL t;
		EREAL q[2];
		...
		abs_a = fabs(temp1[0]);
		abs_b = fabs(temp1[1]);
		abs_c = fabs((EREAL) T_element[0]);
		abs_d = fabs((EREAL) T_element[1]);
		ab = MAX(abs_a, abs_b);
		cd = MAX(abs_c, abs_d);
  ...
  case blas_prec_extra:
      ...
      {
	{
	  ...
	  EREAL head_temp1[2], tail_temp1[2];	/* temporary variable for calculations */
	  EREAL head_temp2[2], tail_temp2[2];	/* temporary variable for calculations */
	  EREAL head_temp3[2], tail_temp3[2];	/* temporary variable for calculations */
	  EREAL *head_intx, *tail_intx;
	  /* copy of x used for calculations */

	  /* allocate space for intx */
	  head_intx = (EREAL *) blas_malloc(n * sizeof(EREAL) * 2);
	  tail_intx = (EREAL *) blas_malloc(n * sizeof(EREAL) * 2);
	  ...
	  if ((order == blas_rowmajor &&
		...
		for (i = n - 1; i >= j + 1; i--) {
		  ...
		  {
		    EREAL head_at, tail_at;
		    EREAL head_bt, tail_bt;
		    EREAL head_ct, tail_ct;
		...
		if (diag == blas_non_unit_diag) {
		  ...
		  {
		    EREAL S = 1.0, eps, ov, un, eps1, ov1, un1;
		    EREAL abs_a, abs_b, abs_c, abs_d, ab, cd;
		    EREAL s;
		    EREAL r;
		    EREAL head_t, tail_t;
		    EREAL head_t1, tail_t1;
		    EREAL head_t2, tail_t2;
		    EREAL head_q[2], tail_q[2];

		    eps = pow(2.0, -53.0);	/* EREAL precision */
		    un = pow(2.0, -1022.0);
			...
			tail_a = tail_temp1[0];
			{
			  ...
			  EREAL b1, b2, con, e, t1, t2, t11, t21, t12, t22;
