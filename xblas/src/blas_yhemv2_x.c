...
{
  /* Routine name */
  const char routine_name[] = "BLAS_yhemv2_x";
  switch (prec) {
  ...
  case blas_prec_single:{
      ...
      const EREAL *x_head_i = (EREAL *) x_head;
      const EREAL *x_tail_i = (EREAL *) x_tail;
      EREAL *y_i = (EREAL *) y;
	...
	for (i = 0, yi = yi0, ai = 0; i < n; i++, yi += incy, ai += incai) {
	  ...
	  {
	    tmp2[0] =
	      (EREAL) y_elem[0] * beta_i[0] - (EREAL) y_elem[1] * beta_i[1];
	    tmp2[1] =
	      (EREAL) y_elem[0] * beta_i[1] + (EREAL) y_elem[1] * beta_i[0];
	  }
  ...
  case blas_prec_extra:{
      ...
      EREAL y_elem[2];
      EREAL diag_elem;
      EREAL head_prod1[2], tail_prod1[2];
      EREAL head_prod2[2], tail_prod2[2];
	...
	for (i = 0, yi = yi0, ai = 0; i < n; i++, yi += incy, ai += incai) {
	  ...
	  x_elem[1] = x_head_i[xi + 1];
	  {
	    /* Compute complex-extra = complex-EREAL * real. */
	    EREAL head_t, tail_t;
	    {
	      /* Compute double_double = EREAL * EREAL. */
	      EREAL a1, a2, b1, b2, con;
