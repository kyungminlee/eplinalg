...
{
  /* Routine name */
  static const char routine_name[] = "BLAS_ytbsv_x";
...

	{
	  ...
	  if ((trans == blas_conj) || (trans == blas_conj_trans)) {
	    ...
	    for (j = 0; j < k; j++) {
	      ...
	      {
		temp1[0] =
		  (EREAL) x_elem[0] * alpha_i[0] -
		  (EREAL) x_elem[1] * alpha_i[1];
		temp1[1] =
		  (EREAL) x_elem[0] * alpha_i[1] +
		  (EREAL) x_elem[1] * alpha_i[0];
	      }
		...
		{
		  temp2[0] =
		    (EREAL) x_elem[0] * T_element[0] -
		    (EREAL) x_elem[1] * T_element[1];
		  temp2[1] =
		    (EREAL) x_elem[0] * T_element[1] +
		    (EREAL) x_elem[1] * T_element[0];
		}
  ...
  case blas_prec_extra:{
...

	{
	  ...
	  EREAL x_elem[2];
	  EREAL T_element[2];	/* temporary variable for an element of matrix T */

	  int x_inti = 0, inc_x_inti = 1;
	  int k_compare = k;	/*used for comparisons with x_inti */
	  EREAL *head_x_internal, *tail_x_internal;

	  FPU_FIX_DECL;

	  k_compare *= 2;
	  inc_x_inti *= 2;
	  head_x_internal = (EREAL *) blas_malloc(k * sizeof(EREAL) * 2);
	  tail_x_internal = (EREAL *) blas_malloc(k * sizeof(EREAL) * 2);
