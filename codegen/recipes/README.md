...
XBLAS, …), consumed by the migrator (`codegen/migrator/`) to retarget the
double-precision sources under `extern/` to the extended-precision
targets defined in `codegen/targets/*.yaml`. The recipe schema is documented in
`doc/dev/recipes.md`.
