...
*  =======
*
*  PDPBTRSV solves a banded triangular system of linear equations
*
...
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*BW ) ) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT, 'PDPBTRSV, D&C alg.: NB too small',
     $                 -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -15
            CALL PXERBLA( ICTXT, 'PDPBTRSV: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDPBTRSV', -INFO )
         RETURN
      END IF
*
...
*
            CALL DTBTRS( UPLO, 'N', 'N', ODD_SIZE, BW, NRHS,
...
*
     ...
     $                      WORK( 1 ), BW )
*
               CALL DTRMM( 'L', 'U', 'N', 'N', BW, NRHS, -ONE,
     $                     A( ( OFST+( BW+1 )+( ODD_SIZE-BW )*LLDA ) ),
     $                     LLDA-1, WORK( 1 ), BW )
*
               CALL DMATADD( BW, NRHS, ONE, WORK( 1 ), BW, ONE,
...
*
               CALL DGEMM( 'T', 'N', BW, NRHS, ODD_SIZE, -ONE, AF( 1 ),
...
*
            IF( MYCOL.GT.0 ) THEN
*
               CALL DGESD2D( ICTXT, BW, NRHS, WORK( 1 ), BW, 0,
     $                       MYCOL-1 )
*
            END IF
*
...
*
               CALL DGERV2D( ICTXT, BW, NRHS, WORK( 1 ), BW, 0,
     $                       MYCOL+1 )
*
...
*
               CALL DGEMM( 'N', 'N', ODD_SIZE, NRHS, BW, -ONE, AF( 1 ),
...
*
               CALL DMATADD( BW, NRHS, ONE, WORK( 1+BW-BW ), BW, ONE,
     $                       B( PART_OFFSET+ODD_SIZE-BW+1 ), LLDB )
*
            END IF
*
*       Use main partition in each processor to solve locally
*
            CALL DTBTRS( UPLO, 'T', 'N', ODD_SIZE, BW, NRHS,
     $                   A( OFST+1 ), LLDA, B( PART_OFFSET+1 ), LLDB,
     $                   INFO )
*
         END IF
...
*
     ...
     $                      WORK( 1 ), BW )
*
               CALL DTRMM( 'L', 'L', 'T', 'N', BW, NRHS, -ONE,
