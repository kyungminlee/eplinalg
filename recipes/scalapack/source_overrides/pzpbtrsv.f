...
*  =======
*
*  PZPBTRSV solves a banded triangular system of linear equations
*
...
*  Cholesky factorization code PZPBTRF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPBTRSV', -INFO )
         RETURN
      END IF
*
...
*
     ...
     $                WORK( 1 ), BW )
*
            CALL ZTRMM( 'L', 'U', 'N', 'N', BW, NRHS, -CONE,
     $                  A(( OFST+(BW+1)+(ODD_SIZE-BW)*LLDA )), LLDA-1,
     $                  WORK( 1 ), BW )
*
            CALL ZMATADD( BW, NRHS, CONE, WORK( 1 ), BW,
...
*
            CALL ZGEMM( 'C', 'N', BW, NRHS, ODD_SIZE, -CONE, AF( 1 ),
...
*
        CALL ZTRTRS( 'L', 'N', 'N', BW, NRHS, AF( ODD_SIZE*BW+MBW2+1 ),
...
*
          CALL ZGEMM( 'C', 'N', BW, NRHS, BW, -CONE,
     $                     AF( (ODD_SIZE)*BW+1 ),
     $                     BW,
...
*
          CALL ZGEMM( 'N', 'N', BW, NRHS, BW, -CONE,
     $                     AF( ODD_SIZE*BW+2*MBW2+1 ),
     $                     BW,
     $                     B( PART_OFFSET+ODD_SIZE+1 ),
     $                     LLDB, CZERO,
     $                     WORK( 1 ),
...
*       Solve with diagonal block
*
        CALL ZTRTRS( 'L', 'C', 'N', BW, NRHS, AF( ODD_SIZE*BW+MBW2+1 ),
     $               BW, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
          GO TO 1000
...
*
          CALL ZGEMM( 'N', 'N', ODD_SIZE, NRHS, BW, -CONE, AF( 1 ),
...
*
     ...
     $                WORK( 1+BW-BW ), BW )
*
          CALL ZMATADD( BW, NRHS, CONE, WORK( 1+BW-BW ), BW, CONE,
     $                  B( PART_OFFSET+ODD_SIZE-BW+1 ), LLDB )
*
        ENDIF
*
...
*
     ...
     $                WORK( 1 ), BW )
*
            CALL ZTRMM( 'L', 'L', 'C', 'N', BW, NRHS, -CONE,
