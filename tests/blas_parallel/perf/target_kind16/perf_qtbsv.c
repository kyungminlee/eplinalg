 ...
 *
 * Kernel-isolated C perf harness for qtbsv (overlay vs migrated).
...
BLAS_EXTERN void qtbsv_(const char *, const char *, const char *, const int *, const int *,
    const Q16 *, const int *, Q16 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void qtbsv_migrated_(const char *, const char *, const char *, const int *, const int *,
    const Q16 *, const int *, Q16 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int K, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        qtbsv_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(Q16));
        qtbsv_migrated_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(Q16));
    }
    ...
    perf_emit("qtbsv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("qtbsv", key, N, iters, flops, t_subject, t_mg);
