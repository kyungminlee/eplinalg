 ...
 *
 * Kernel-isolated C perf harness for qtrsv (overlay vs migrated).
...
BLAS_EXTERN void qtrsv_(const char *, const char *, const char *, const int *,
    const Q16 *, const int *, Q16 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void qtrsv_migrated_(const char *, const char *, const char *, const int *,
    const Q16 *, const int *, Q16 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        qtrsv_(&uplo, &trans, &diag, &N, A, &N, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(Q16));
        qtrsv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(Q16));
    }
    ...
    perf_emit("qtrsv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("qtrsv", key, N, iters, flops, t_subject, t_mg);
