...
static void run_one(char uplo, char trans, char diag, int N, int K, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        xtbsv_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(X16));
        xtbsv_migrated_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(X16));
    }
    ...
    perf_emit("xtbsv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("xtbsv", key, N, iters, flops, t_ov, t_mg);
