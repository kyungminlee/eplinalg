...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        xtrsv_(&uplo, &trans, &diag, &N, A, &N, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(X16));
        xtrsv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(X16));
    }
    ...
    perf_emit("xtrsv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("xtrsv", key, N, iters, flops, t_subject, t_mg);
