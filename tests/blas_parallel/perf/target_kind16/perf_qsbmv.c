 ...
 *
 * Kernel-isolated C perf harness for qsbmv (overlay vs migrated).
...
BLAS_EXTERN void qsbmv_(const char *, const int *, const int *,
    const Q16 *, const Q16 *, const int *, const Q16 *, const int *,
    const Q16 *, Q16 *, const int *, size_t);
BLAS_EXTERN void qsbmv_migrated_(const char *, const int *, const int *,
    const Q16 *, const Q16 *, const int *, const Q16 *, const int *,
    const Q16 *, Q16 *, const int *, size_t);
...
static void run_one(char uplo, int N, int K, int incx, int incy,
    ...
    for (int r = 0; r < warmup; ++r) {
        qsbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(Q16));
        qsbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(Q16));
    }
    memcpy(Y, Yi, leny * sizeof(Q16));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) qsbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
    double t1 = perf_now_s();
    double t_subject = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, leny * sizeof(Q16));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) qsbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
    t1 = perf_now_s();
    ...
    perf_emit("qsbmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("qsbmv", key, N, iters, flops, t_subject, t_mg);
