...
static void run_one(char uplo, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        qsbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(Q16));
        qsbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(Q16));
    }
    memcpy(Y, Yi, (size_t)N * sizeof(Q16));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) qsbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)N * sizeof(Q16));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) qsbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
