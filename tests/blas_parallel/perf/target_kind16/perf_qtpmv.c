 ...
 *
 * Kernel-isolated C perf harness for qtpmv (overlay vs migrated).
...
BLAS_EXTERN void qtpmv_(const char *, const char *, const char *, const int *,
    const Q16 *, Q16 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void qtpmv_migrated_(const char *, const char *, const char *, const int *,
    const Q16 *, Q16 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        qtpmv_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(Q16));
        qtpmv_migrated_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(Q16));
    }
    ...
    perf_emit("qtpmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("qtpmv", key, N, iters, flops, t_subject, t_mg);
