 ...
 *
 * Kernel-isolated C perf harness for qtrsm (overlay vs migrated).
...
BLAS_EXTERN void qtrsm_(const char *, const char *, const char *, const char *,
    const int *, const int *, const Q16 *,
    const Q16 *, const int *, Q16 *, const int *,
    size_t, size_t, size_t, size_t);
BLAS_EXTERN void qtrsm_migrated_(const char *, const char *, const char *, const char *,
    const int *, const int *, const Q16 *,
    const Q16 *, const int *, Q16 *, const int *,
    size_t, size_t, size_t, size_t);
...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    for (int r = 0; r < warmup; ++r) {
        qtrsm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(Q16));
        qtrsm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(Q16));
    }
    ...
    perf_emit("qtrsm", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("qtrsm", key, N, iters, flops, t_subject, t_mg);
