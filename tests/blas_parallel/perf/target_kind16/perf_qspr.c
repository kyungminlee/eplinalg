...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        qspr_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(Q16));
        qspr_migrated_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(Q16));
    }
    ...
    char key[2] = {uplo, 0};
    perf_emit("qspr", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("qspr", key, N, iters, flops, t_ov, t_mg);
