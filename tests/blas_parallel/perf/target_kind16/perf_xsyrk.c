...
static void run_one(char uplo, char trans, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xsyrk_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(X16));
        xsyrk_migrated_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(X16));
    }
    memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(X16));
    ...
    perf_emit("xsyrk", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("xsyrk", key, N, iters, flops, t_ov, t_mg);
