 ...
 *
 * Kernel-isolated C perf harness for xdotu (overlay vs migrated).
...
BLAS_EXTERN X16 xdotu_(const int *, const X16 *, const int *, const X16 *, const int *);
BLAS_EXTERN X16 xdotu_migrated_(const int *, const X16 *, const int *, const X16 *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    perf_emit("xdotu", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("xdotu", "-", N, iters, flops, t_ov, t_mg);
    free(X); free(Y);
}
