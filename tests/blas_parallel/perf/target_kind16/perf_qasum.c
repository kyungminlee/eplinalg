 ...
 *
 * Kernel-isolated C perf harness for qasum (overlay vs migrated).
...
BLAS_EXTERN Q16 qasum_(const int *, const Q16 *, const int *);
BLAS_EXTERN Q16 qasum_migrated_(const int *, const Q16 *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    perf_emit("qasum", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("qasum", "-", N, iters, flops, t_ov, t_mg);
