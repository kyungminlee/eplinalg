 ...
 *
 * Kernel-isolated C perf harness for qcopy (overlay vs migrated).
...
BLAS_EXTERN void qcopy_(const int *, const Q16 *, const int *, Q16 *, const int *);
BLAS_EXTERN void qcopy_migrated_(const int *, const Q16 *, const int *, Q16 *, const int *);
...
static void run_qcopy(int N, int iters, int warmup) {
    ...
    perf_emit("qcopy", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("qcopy", "-", N, iters, flops, t_ov, t_mg);
...
int main(void) {
    ...
    perf_print_header();
    for (int i = 0; i < n; ++i) run_qcopy(sizes[i], iters, warmup);
    return 0;
}
