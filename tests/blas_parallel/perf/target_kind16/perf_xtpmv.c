...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        xtpmv_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(X16));
        xtpmv_migrated_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(X16));
    }
    ...
    perf_emit("xtpmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("xtpmv", key, N, iters, flops, t_subject, t_mg);
