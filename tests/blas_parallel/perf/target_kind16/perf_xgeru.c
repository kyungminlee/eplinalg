 ...
 *
 * Kernel-isolated C perf harness for xgeru (overlay vs migrated).
...
BLAS_EXTERN void xgeru_(const int *, const int *, const X16 *,
    const X16 *, const int *, const X16 *, const int *, X16 *, const int *);
BLAS_EXTERN void xgeru_migrated_(const int *, const int *, const X16 *,
    const X16 *, const int *, const X16 *, const int *, X16 *, const int *);
...
static void run_one(int M, int N, int incx, int incy, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xgeru_(&M, &N, &alpha, X, &incx, Y, &incy, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(X16));
        xgeru_migrated_(&M, &N, &alpha, X, &incx, Y, &incy, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(X16));
    }
    ...
    perf_emit("xgeru", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("xgeru", key, N, iters, flops, t_subject, t_mg);
