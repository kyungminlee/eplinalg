...
static void run_one(int M, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xgeru_(&M, &N, &alpha, X, &one, Y, &one, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(X16));
        xgeru_migrated_(&M, &N, &alpha, X, &one, Y, &one, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(X16));
    }
    ...
    perf_emit("xgeru", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("xgeru", "-", N, iters, flops, t_ov, t_mg);
