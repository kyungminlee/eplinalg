 ...
 *
 * Kernel-isolated C perf harness for xtbmv (overlay vs migrated).
...
BLAS_EXTERN void xtbmv_(const char *, const char *, const char *, const int *, const int *,
    const X16 *, const int *, X16 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void xtbmv_migrated_(const char *, const char *, const char *, const int *, const int *,
    const X16 *, const int *, X16 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xtbmv_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(X16));
        xtbmv_migrated_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(X16));
    }
    ...
    perf_emit("xtbmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("xtbmv", key, N, iters, flops, t_ov, t_mg);
