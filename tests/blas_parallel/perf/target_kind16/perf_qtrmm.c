 ...
 *
 * Kernel-isolated C perf harness for qtrmm (overlay vs migrated).
...
BLAS_EXTERN void qtrmm_(const char *, const char *, const char *, const char *,
    const int *, const int *, const Q16 *,
    const Q16 *, const int *, Q16 *, const int *,
    size_t, size_t, size_t, size_t);
BLAS_EXTERN void qtrmm_migrated_(const char *, const char *, const char *, const char *,
    const int *, const int *, const Q16 *,
    const Q16 *, const int *, Q16 *, const int *,
    size_t, size_t, size_t, size_t);
...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    Q16 *Bi = (Q16 *)perf_aligned_alloc(64, (size_t)M * (size_t)N * sizeof(Q16));
    ...
    for (size_t i = 0; i < (size_t)M*N; ++i) { int s = 4; Bi[i] = Q16_FROM(perf_fill_double(i, s)); }
    memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(Q16));
    for (int r = 0; r < warmup; ++r) {
        qtrmm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(Q16));
        qtrmm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(Q16));
    }
    ...
    perf_emit("qtrmm", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("qtrmm", key, N, iters, flops, t_subject, t_mg);
