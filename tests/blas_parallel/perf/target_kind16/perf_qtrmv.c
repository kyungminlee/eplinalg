 ...
 *
 * Kernel-isolated C perf harness for qtrmv (overlay vs migrated).
...
BLAS_EXTERN void qtrmv_(const char *, const char *, const char *, const int *,
    const Q16 *, const int *, Q16 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void qtrmv_migrated_(const char *, const char *, const char *, const int *,
    const Q16 *, const int *, Q16 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        qtrmv_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(Q16));
        qtrmv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(Q16));
    }
    ...
    perf_emit("qtrmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("qtrmv", key, N, iters, flops, t_ov, t_mg);
