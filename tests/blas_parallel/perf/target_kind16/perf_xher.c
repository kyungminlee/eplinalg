...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xher_(&uplo, &N, &alpha, X, &one, A, &N, 1);
        memcpy(A, Ai, (size_t)N * (size_t)N * sizeof(X16));
        xher_migrated_(&uplo, &N, &alpha, X, &one, A, &N, 1);
        memcpy(A, Ai, (size_t)N * (size_t)N * sizeof(X16));
    }
    ...
    char key[2] = {uplo, 0};
    perf_emit("xher", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("xher", key, N, iters, flops, t_ov, t_mg);
