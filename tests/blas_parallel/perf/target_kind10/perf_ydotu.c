 ...
 *
 * Kernel-isolated C perf harness for ydotu (overlay vs migrated).
...
BLAS_EXTERN C10 ydotu_(const int *, const C10 *, const int *, const C10 *, const int *);
BLAS_EXTERN C10 ydotu_migrated_(const int *, const C10 *, const int *, const C10 *, const int *);

static volatile unsigned long perf_sink = 0;

static inline void sink_T(const C10 *p) {
...
static void run_one(int N, int iters, int warmup) {
    ...
    for (int i = 0; i < N; ++i) { int s = 1; Y[i] = C10_FROM(perf_fill_double(i, s), perf_fill_double(i, s + 131)); }
    for (int r2 = 0; r2 < warmup; ++r2) {
        r = ydotu_(&N, X, &one, Y, &one); sink_T(&r);
        r = ydotu_migrated_(&N, X, &one, Y, &one); sink_T(&r);
    }
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) {
        r = ydotu_(&N, X, &one, Y, &one);
        sink_T(&r);
    }
    ...
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) {
        r = ydotu_migrated_(&N, X, &one, Y, &one);
        sink_T(&r);
    }
    t1 = perf_now_s();
    ...
    perf_emit("ydotu", "-", N, iters, flops, t_subject, t_mg);
    perf_emit_json("ydotu", "-", N, iters, flops, t_subject, t_mg);
    free(X); free(Y);
}
