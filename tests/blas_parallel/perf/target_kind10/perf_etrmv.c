 ...
 *
 * Kernel-isolated C perf harness for etrmv (overlay vs migrated).
...
BLAS_EXTERN void etrmv_(const char *, const char *, const char *, const int *,
    const R10 *, const int *, R10 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void etrmv_migrated_(const char *, const char *, const char *, const int *,
    const R10 *, const int *, R10 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        etrmv_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(R10));
        etrmv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(R10));
    }
    ...
    perf_emit("etrmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("etrmv", key, N, iters, flops, t_ov, t_mg);
