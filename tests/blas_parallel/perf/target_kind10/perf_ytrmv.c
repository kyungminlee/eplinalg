 ...
 *
 * Kernel-isolated C perf harness for ytrmv (overlay vs migrated).
...
BLAS_EXTERN void ytrmv_(const char *, const char *, const char *, const int *,
    const C10 *, const int *, C10 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void ytrmv_migrated_(const char *, const char *, const char *, const int *,
    const C10 *, const int *, C10 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        ytrmv_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(C10));
        ytrmv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(C10));
    }
    ...
    perf_emit("ytrmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("ytrmv", key, N, iters, flops, t_ov, t_mg);
