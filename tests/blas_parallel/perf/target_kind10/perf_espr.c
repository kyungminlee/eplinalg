 ...
 *
 * Kernel-isolated C perf harness for espr (overlay vs migrated).
...
BLAS_EXTERN void espr_(const char *, const int *, const R10 *,
    const R10 *, const int *, R10 *, size_t);
BLAS_EXTERN void espr_migrated_(const char *, const int *, const R10 *,
    const R10 *, const int *, R10 *, size_t);
...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        espr_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(R10));
        espr_migrated_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(R10));
    }
    ...
    char key[2] = {uplo, 0};
    perf_emit("espr", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("espr", key, N, iters, flops, t_ov, t_mg);
