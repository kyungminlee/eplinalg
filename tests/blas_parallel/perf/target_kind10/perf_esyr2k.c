 ...
 *
 * Kernel-isolated C perf harness for esyr2k (overlay vs migrated).
...
BLAS_EXTERN void esyr2k_(const char *, const char *, const int *, const int *,
    const R10 *, const R10 *, const int *,
    const R10 *, const int *,
    const R10 *, R10 *, const int *, size_t, size_t);
BLAS_EXTERN void esyr2k_migrated_(const char *, const char *, const int *, const int *,
    const R10 *, const R10 *, const int *,
    const R10 *, const int *,
    const R10 *, R10 *, const int *, size_t, size_t);
...
static void run_one(char uplo, char trans, int N, int K, int iters, int warmup) {
    ...
    R10 *A  = (R10 *)perf_aligned_alloc(64, (size_t)A_rows * (size_t)A_cols * sizeof(R10));
    R10 *B  = (R10 *)perf_aligned_alloc(64, (size_t)A_rows * (size_t)A_cols * sizeof(R10));
    ...
    for (size_t i = 0; i < (size_t)A_rows*A_cols; ++i) { int s = 2; A[i] = R10_FROM(perf_fill_double(i, s)); }
    for (size_t i = 0; i < (size_t)A_rows*A_cols; ++i) { int s = 3; B[i] = R10_FROM(perf_fill_double(i, s)); }
    ...
    for (int r = 0; r < warmup; ++r) {
        ...
        memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(R10));
        esyr2k_migrated_(&uplo, &trans, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(R10));
    }
    memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(R10));
