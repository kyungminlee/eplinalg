 ...
 *
 * Kernel-isolated C perf harness for yherk (overlay vs migrated).
...
BLAS_EXTERN void yherk_(const char *, const char *, const int *, const int *,
    const R10 *, const C10 *, const int *,
    const R10 *, C10 *, const int *, size_t, size_t);
BLAS_EXTERN void yherk_migrated_(const char *, const char *, const int *, const int *,
    const R10 *, const C10 *, const int *,
    const R10 *, C10 *, const int *, size_t, size_t);
...
static void run_one(char uplo, char trans, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        yherk_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(C10));
        yherk_migrated_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(C10));
    }
    memcpy(C, Ci, (size_t)N * (size_t)N * sizeof(C10));
    ...
    perf_emit("yherk", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("yherk", key, N, iters, flops, t_subject, t_mg);
