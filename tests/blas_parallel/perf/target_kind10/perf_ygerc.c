...
static void run_one(int M, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        ygerc_(&M, &N, &alpha, X, &one, Y, &one, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(C10));
        ygerc_migrated_(&M, &N, &alpha, X, &one, Y, &one, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(C10));
    }
    ...
    perf_emit("ygerc", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("ygerc", "-", N, iters, flops, t_ov, t_mg);
