...
static void run_one(char uplo, int N, int incx, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        yhpr_(&uplo, &N, &alpha, X, &incx, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(C10));
        yhpr_migrated_(&uplo, &N, &alpha, X, &incx, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(C10));
    }
    ...
    perf_emit("yhpr", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("yhpr", key, N, iters, flops, t_ov, t_mg);
