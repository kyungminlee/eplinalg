...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        etpmv_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(R10));
        etpmv_migrated_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(R10));
    }
    ...
    perf_emit("etpmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("etpmv", key, N, iters, flops, t_subject, t_mg);
