 ...
 *
 * Kernel-isolated C perf harness for yhbmv (overlay vs migrated).
...
BLAS_EXTERN void yhbmv_(const char *, const int *, const int *,
    const C10 *, const C10 *, const int *, const C10 *, const int *,
    const C10 *, C10 *, const int *, size_t);
BLAS_EXTERN void yhbmv_migrated_(const char *, const int *, const int *,
    const C10 *, const C10 *, const int *, const C10 *, const int *,
    const C10 *, C10 *, const int *, size_t);
...
static void run_one(char uplo, int N, int K, int incx, int incy,
    ...
    for (int r = 0; r < warmup; ++r) {
        yhbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(C10));
        yhbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(C10));
    }
    memcpy(Y, Yi, leny * sizeof(C10));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) yhbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
    double t1 = perf_now_s();
    double t_subject = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, leny * sizeof(C10));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) yhbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
    t1 = perf_now_s();
    ...
    perf_emit("yhbmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("yhbmv", key, N, iters, flops, t_subject, t_mg);
