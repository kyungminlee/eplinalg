...
static void run_one(char trans, int M, int N, int KL, int KU,
    ...
    for (int r = 0; r < warmup; ++r) {
        egbmv_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(R10));
        egbmv_migrated_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(R10));
    }
    memcpy(Y, Yi, leny * sizeof(R10));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) egbmv_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
    double t1 = perf_now_s();
    double t_subject = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, leny * sizeof(R10));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) egbmv_migrated_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);
    t1 = perf_now_s();
    ...
    perf_emit("egbmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("egbmv", key, N, iters, flops, t_subject, t_mg);
