...
static void run_one(char trans, int M, int N, int KL, int KU, int iters, int warmup) {
    ...
    int XL = (trans == 'N') ? N : M;
    int YL = (trans == 'N') ? M : N;
    R10 *X  = (R10 *)perf_aligned_alloc(64, (size_t)XL * sizeof(R10));
    R10 *Y  = (R10 *)perf_aligned_alloc(64, (size_t)YL * sizeof(R10));
    R10 *Yi = (R10 *)perf_aligned_alloc(64, (size_t)YL * sizeof(R10));
    for (size_t i = 0; i < (size_t)LDA*N; ++i) { int s = 2; A[i] = R10_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < XL; ++i) { int s = 3; X[i] = R10_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < YL; ++i) { int s = 4; Yi[i] = R10_FROM(perf_fill_double(i, s)); }
    memcpy(Y, Yi, (size_t)YL * sizeof(R10));
    for (int r = 0; r < warmup; ++r) {
        egbmv_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)YL * sizeof(R10));
        egbmv_migrated_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)YL * sizeof(R10));
    }
    memcpy(Y, Yi, (size_t)YL * sizeof(R10));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) egbmv_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)YL * sizeof(R10));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) egbmv_migrated_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    double flops = 2.0 * (double)(KL+KU+1) * (double)N;
    char key[2] = {trans, 0};
    perf_emit("egbmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("egbmv", key, N, iters, flops, t_ov, t_mg);
