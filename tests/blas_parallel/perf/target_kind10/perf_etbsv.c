 ...
 *
 * Kernel-isolated C perf harness for etbsv (overlay vs migrated).
...
BLAS_EXTERN void etbsv_(const char *, const char *, const char *, const int *, const int *,
    const R10 *, const int *, R10 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void etbsv_migrated_(const char *, const char *, const char *, const int *, const int *,
    const R10 *, const int *, R10 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int K, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        etbsv_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(R10));
        etbsv_migrated_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(R10));
    }
    ...
    perf_emit("etbsv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("etbsv", key, N, iters, flops, t_subject, t_mg);
