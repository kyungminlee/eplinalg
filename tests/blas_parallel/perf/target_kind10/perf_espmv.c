...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        espmv_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(R10));
        espmv_migrated_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(R10));
    }
    memcpy(Y, Yi, (size_t)N * sizeof(R10));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) espmv_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)N * sizeof(R10));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) espmv_migrated_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
