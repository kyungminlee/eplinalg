 ...
 *
 * Kernel-isolated C perf harness for ytpsv (overlay vs migrated).
...
BLAS_EXTERN void ytpsv_(const char *, const char *, const char *, const int *,
    const C10 *, C10 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void ytpsv_migrated_(const char *, const char *, const char *, const int *,
    const C10 *, C10 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        ytpsv_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(C10));
        ytpsv_migrated_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(C10));
    }
    ...
    perf_emit("ytpsv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("ytpsv", key, N, iters, flops, t_subject, t_mg);
