...
static void run_one(char uplo, int N, int incx, int incy, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        yhemv_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(C10));
        yhemv_migrated_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(C10));
    }
    memcpy(Y, Yi, leny * sizeof(C10));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) yhemv_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1);
    double t1 = perf_now_s();
    double t_subject = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, leny * sizeof(C10));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) yhemv_migrated_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1);
    t1 = perf_now_s();
    ...
    perf_emit("yhemv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("yhemv", key, N, iters, flops, t_subject, t_mg);
