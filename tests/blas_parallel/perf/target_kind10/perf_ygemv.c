...
static void run_one(char trans, int M, int N, int incx, int incy,
    ...
    for (int r = 0; r < warmup; ++r) {
        ygemv_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(C10));
        ygemv_migrated_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(C10));
    }
    ...
    perf_emit("ygemv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("ygemv", key, N, iters, flops, t_subject, t_mg);
