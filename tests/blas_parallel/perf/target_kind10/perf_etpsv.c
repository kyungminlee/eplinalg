 ...
 *
 * Kernel-isolated C perf harness for etpsv (overlay vs migrated).
...
BLAS_EXTERN void etpsv_(const char *, const char *, const char *, const int *,
    const R10 *, R10 *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void etpsv_migrated_(const char *, const char *, const char *, const int *,
    const R10 *, R10 *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        etpsv_(&uplo, &trans, &diag, &N, AP, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(R10));
        etpsv_migrated_(&uplo, &trans, &diag, &N, AP, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(R10));
    }
    ...
    perf_emit("etpsv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("etpsv", key, N, iters, flops, t_ov, t_mg);
