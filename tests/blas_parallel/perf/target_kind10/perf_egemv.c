 ...
 *
 * Kernel-isolated C perf harness for egemv (overlay vs migrated).
...
BLAS_EXTERN void egemv_(const char *, const int *, const int *, const R10 *,
    const R10 *, const int *, const R10 *, const int *,
    const R10 *, R10 *, const int *, size_t);
BLAS_EXTERN void egemv_migrated_(const char *, const int *, const int *, const R10 *,
    const R10 *, const int *, const R10 *, const int *,
    const R10 *, R10 *, const int *, size_t);
...
static void run_one(char trans, int M, int N, int iters, int warmup) {
    ...
    R10 *Y  = (R10 *)perf_aligned_alloc(64, (size_t)M * sizeof(R10));
    R10 *Yi = (R10 *)perf_aligned_alloc(64, (size_t)M * sizeof(R10));
    for (size_t i = 0; i < (size_t)M*N; ++i) { int s = 2; A[i] = R10_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < N; ++i)           { int s = 3; X[i] = R10_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < M; ++i)           { int s = 4; Yi[i] = R10_FROM(perf_fill_double(i, s)); }

    memcpy(Y, Yi, (size_t)M * sizeof(R10));
    ...
    char key[2] = {trans, 0};
    perf_emit("egemv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("egemv", key, N, iters, flops, t_ov, t_mg);
