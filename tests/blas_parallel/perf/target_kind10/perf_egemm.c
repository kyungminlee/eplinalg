...
static void run_one(char ta, char tb, int M, int N, int K, int iters, int warmup) {
    ...
    R10 *A  = (R10 *)perf_aligned_alloc(64, (size_t)M * (size_t)K * sizeof(R10));
    R10 *B  = (R10 *)perf_aligned_alloc(64, (size_t)K * (size_t)N * sizeof(R10));
    ...
    for (size_t i = 0; i < (size_t)M*K; ++i) { int s = 2; A[i] = R10_FROM(perf_fill_double(i, s)); }
    for (size_t i = 0; i < (size_t)K*N; ++i) { int s = 3; B[i] = R10_FROM(perf_fill_double(i, s)); }
    for (size_t i = 0; i < (size_t)M*N; ++i) { int s = 4; Ci[i] = R10_FROM(perf_fill_double(i, s)); }
    memcpy(C, Ci, (size_t)M * (size_t)N * sizeof(R10));

    for (int r = 0; r < warmup; ++r) {
        egemm_(&ta, &tb, &M, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)M * (size_t)N * sizeof(R10));
        egemm_migrated_(&ta, &tb, &M, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)M * (size_t)N * sizeof(R10));
    }
    memcpy(C, Ci, (size_t)M * (size_t)N * sizeof(R10));
    ...
    char key[3] = {ta, tb, 0};
    perf_emit("egemm", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("egemm", key, N, iters, flops, t_ov, t_mg);
