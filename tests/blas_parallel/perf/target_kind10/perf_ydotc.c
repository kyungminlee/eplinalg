 ...
 *
 * Kernel-isolated C perf harness for ydotc (overlay vs migrated).
...
BLAS_EXTERN C10 ydotc_(const int *, const C10 *, const int *, const C10 *, const int *);
BLAS_EXTERN C10 ydotc_migrated_(const int *, const C10 *, const int *, const C10 *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    for (int r2 = 0; r2 < warmup; ++r2) {
        r = ydotc_(&N, X, &one, Y, &one); sink_T(&r);
        r = ydotc_migrated_(&N, X, &one, Y, &one); sink_T(&r);
    }
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) {
        r = ydotc_(&N, X, &one, Y, &one);
        sink_T(&r);
    }
    ...
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) {
        r = ydotc_migrated_(&N, X, &one, Y, &one);
        sink_T(&r);
    }
    t1 = perf_now_s();
    ...
    perf_emit("ydotc", "-", N, iters, flops, t_subject, t_mg);
    perf_emit_json("ydotc", "-", N, iters, flops, t_subject, t_mg);
    free(X); free(Y);
}
