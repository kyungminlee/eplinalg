...
static void run_one(char uplo, char trans, char diag, int N, int K, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        etbmv_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(R10));
        etbmv_migrated_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(R10));
    }
    ...
    perf_emit("etbmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("etbmv", key, N, iters, flops, t_subject, t_mg);
