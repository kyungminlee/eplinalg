 ...
 * -ffunction-sections -Wl,--gc-sections so the overlay's symbol and the
 * migrated's symbol end up close in the binary (avoids iTLB churn —
 * see findings doc Addendum 14). The helpers below stay header-only so
 * each translation unit picks up only what it actually uses; --gc-sections
 * drops the rest.
 *
 * Env knobs:
...
static inline void perf_print_header(void) {
    ...
    printf("# routine            key      size    iters   overlay_GFs   migrated_GFs   mig/ov\n");
}

static inline void perf_emit(const char *routine, const char *key, int size,
                             int iters, double flops, double t_ov, double t_mg)
{
    ...
    printf("%-18s  %-7s  %6d  %6d  %12.4f  %13.4f  %8.3fx\n",
           routine, key, size, iters, g_ov, g_mg, ratio);
}
                                  ...
                                  double t_ov, double t_mg)
{
    ...
    fprintf(f, "{\"routine\":\"%s\",\"key\":\"%s\",\"size\":%d,\"iters\":%d,"
               "\"t_ov\":%.6e,\"t_mg\":%.6e,\"gflops_ov\":%.4f,"
               "\"gflops_mg\":%.4f,\"ratio\":%.4f}\n",
            routine, key, size, iters, t_ov, t_mg, g_ov, g_mg, ratio);
    fclose(f);
}
