 ...
 *
 * Kernel-isolated C perf harness for wtbsv (overlay vs migrated).
...
BLAS_EXTERN void wtbsv_(const char *, const char *, const char *, const int *, const int *,
    const MFC *, const int *, MFC *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void wtbsv_migrated_(const char *, const char *, const char *, const int *, const int *,
    const MFC *, const int *, MFC *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int K, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        wtbsv_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(MFC));
        wtbsv_migrated_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(MFC));
    }
    ...
    perf_emit("wtbsv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("wtbsv", key, N, iters, flops, t_subject, t_mg);
