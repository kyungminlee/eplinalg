 ...
 *
 * Kernel-isolated C perf harness for msyr (overlay vs migrated).
...
BLAS_EXTERN void msyr_(const char *, const int *, const MFR *,
    const MFR *, const int *, MFR *, const int *, size_t);
BLAS_EXTERN void msyr_migrated_(const char *, const int *, const MFR *,
    const MFR *, const int *, MFR *, const int *, size_t);
...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    MFR *Ai = (MFR *)perf_aligned_alloc(64, (size_t)N * (size_t)N * sizeof(MFR));
    MFR *X  = (MFR *)perf_aligned_alloc(64, (size_t)N * sizeof(MFR));
    for (size_t i = 0; i < (size_t)N*N; ++i) { int s = 2; Ai[i] = MFR_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < N; ++i)            { int s = 3; X[i] = MFR_FROM(perf_fill_double(i, s)); }
    memcpy(A, Ai, (size_t)N * (size_t)N * sizeof(MFR));
    for (int r = 0; r < warmup; ++r) {
        msyr_(&uplo, &N, &alpha, X, &one, A, &N, 1);
        memcpy(A, Ai, (size_t)N * (size_t)N * sizeof(MFR));
        msyr_migrated_(&uplo, &N, &alpha, X, &one, A, &N, 1);
        memcpy(A, Ai, (size_t)N * (size_t)N * sizeof(MFR));
    }
