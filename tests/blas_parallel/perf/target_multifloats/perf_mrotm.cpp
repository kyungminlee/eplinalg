 ...
 *
 * Kernel-isolated C perf harness for mrotm (overlay vs migrated).
...
BLAS_EXTERN void mrotm_(const int *, MFR *, const int *, MFR *, const int *,
    const MFR *);
BLAS_EXTERN void mrotm_migrated_(const int *, MFR *, const int *, MFR *, const int *,
    const MFR *);

static void run_one(int N, int iters, int warmup) {
    int one = 1;
    MFR PARAM[5] = { MFR_FROM(0.7), MFR_FROM(0.7), MFR_FROM(0.7), MFR_FROM(0.7), MFR_FROM(0.7) };
