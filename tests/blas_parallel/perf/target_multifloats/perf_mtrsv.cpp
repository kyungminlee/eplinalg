 ...
 *
 * Kernel-isolated C perf harness for mtrsv (overlay vs migrated).
...
BLAS_EXTERN void mtrsv_(const char *, const char *, const char *, const int *,
    const MFR *, const int *, MFR *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void mtrsv_migrated_(const char *, const char *, const char *, const int *,
    const MFR *, const int *, MFR *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        mtrsv_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
        mtrsv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
    }
    ...
    perf_emit("mtrsv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("mtrsv", key, N, iters, flops, t_ov, t_mg);
