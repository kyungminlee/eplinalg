...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        wtrmv_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
        wtrmv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
    }
    ...
    perf_emit("wtrmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("wtrmv", key, N, iters, flops, t_ov, t_mg);
