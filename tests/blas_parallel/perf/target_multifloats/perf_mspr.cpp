 ...
 *
 * Kernel-isolated C perf harness for mspr (overlay vs migrated).
...
BLAS_EXTERN void mspr_(const char *, const int *, const MFR *,
    const MFR *, const int *, MFR *, size_t);
BLAS_EXTERN void mspr_migrated_(const char *, const int *, const MFR *,
    const MFR *, const int *, MFR *, size_t);
...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    MFR *APi = (MFR *)perf_aligned_alloc(64, AP_LEN * sizeof(MFR));
    MFR *X   = (MFR *)perf_aligned_alloc(64, (size_t)N * sizeof(MFR));
    for (size_t i = 0; i < AP_LEN; ++i) { int s = 2; APi[i] = MFR_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < N; ++i)         { int s = 3; X[i]   = MFR_FROM(perf_fill_double(i, s)); }
    memcpy(AP, APi, AP_LEN * sizeof(MFR));
    for (int r = 0; r < warmup; ++r) {
        mspr_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(MFR));
        mspr_migrated_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(MFR));
    }
    ...
    char key[2] = {uplo, 0};
    perf_emit("mspr", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("mspr", key, N, iters, flops, t_ov, t_mg);
