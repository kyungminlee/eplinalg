...
static void run_one(char uplo, int N, int incx, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        mspr_(&uplo, &N, &alpha, X, &incx, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(MFR));
        mspr_migrated_(&uplo, &N, &alpha, X, &incx, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(MFR));
    }
    ...
    perf_emit("mspr", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("mspr", key, N, iters, flops, t_subject, t_mg);
