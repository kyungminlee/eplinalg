...
static void run_one(char uplo, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        msbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFR));
        msbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    }
    memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) msbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) msbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    double flops = 2.0 * (double)(2*K+1) * (double)N;
    char key[2] = {uplo, 0};
    perf_emit("msbmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("msbmv", key, N, iters, flops, t_ov, t_mg);
