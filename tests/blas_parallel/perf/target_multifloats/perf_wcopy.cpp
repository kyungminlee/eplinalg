 ...
 *
 * Kernel-isolated C perf harness for wcopy (overlay vs migrated).
...
BLAS_EXTERN void wcopy_(const int *, const MFC *, const int *, MFC *, const int *);
BLAS_EXTERN void wcopy_migrated_(const int *, const MFC *, const int *, MFC *, const int *);

static void run_wcopy(int N, int iters, int warmup) {
    int one = 1;
    ...
    for (int r = 0; r < warmup; ++r) {
        wcopy_(&N, X, &one, Y, &one);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
        wcopy_migrated_(&N, X, &one, Y, &one);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    }
    ...
    double flops = 2.0 * (double)N * (double)sizeof(MFC);
    perf_emit("wcopy", "-", N, iters, flops, t_subject, t_mg);
    perf_emit_json("wcopy", "-", N, iters, flops, t_subject, t_mg);
...
int main(void) {
    ...
    perf_print_header();
    for (int i = 0; i < n; ++i) run_wcopy(sizes[i], iters, warmup);
    return 0;
}
