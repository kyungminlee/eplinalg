 ...
 *
 * Kernel-isolated C perf harness for iwamax (overlay vs migrated).
...
BLAS_EXTERN int iwamax_(const int *, const MFC *, const int *);
BLAS_EXTERN int iwamax_migrated_(const int *, const MFC *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    for (int r2 = 0; r2 < warmup; ++r2) {
        r ^= iwamax_(&N, X, &one);
        r ^= iwamax_migrated_(&N, X, &one);
    }
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) r ^= iwamax_(&N, X, &one);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) r ^= iwamax_migrated_(&N, X, &one);
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    double flops = 2.0 * (double)N;
    perf_emit("iwamax", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("iwamax", "-", N, iters, flops, t_ov, t_mg);
