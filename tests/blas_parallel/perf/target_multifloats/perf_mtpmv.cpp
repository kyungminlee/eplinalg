 ...
 *
 * Kernel-isolated C perf harness for mtpmv (overlay vs migrated).
...
BLAS_EXTERN void mtpmv_(const char *, const char *, const char *, const int *,
    const MFR *, MFR *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void mtpmv_migrated_(const char *, const char *, const char *, const int *,
    const MFR *, MFR *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        mtpmv_(&uplo, &trans, &diag, &N, AP, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
        mtpmv_migrated_(&uplo, &trans, &diag, &N, AP, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
    }
