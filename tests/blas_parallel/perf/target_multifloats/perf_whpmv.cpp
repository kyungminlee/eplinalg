...
static void run_one(char uplo, int N, int incx, int incy, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        whpmv_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(MFC));
        whpmv_migrated_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1);
        memcpy(Y, Yi, leny * sizeof(MFC));
    }
    memcpy(Y, Yi, leny * sizeof(MFC));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whpmv_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1);
    double t1 = perf_now_s();
    double t_subject = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, leny * sizeof(MFC));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whpmv_migrated_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1);
    t1 = perf_now_s();
    ...
    perf_emit("whpmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("whpmv", key, N, iters, flops, t_subject, t_mg);
