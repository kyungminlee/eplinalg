 ...
 *
 * Kernel-isolated C perf harness for whpmv (overlay vs migrated).
...
BLAS_EXTERN void whpmv_(const char *, const int *, const MFC *, const MFC *,
    const MFC *, const int *, const MFC *, MFC *, const int *, size_t);
BLAS_EXTERN void whpmv_migrated_(const char *, const int *, const MFC *, const MFC *,
    const MFC *, const int *, const MFC *, MFC *, const int *, size_t);
...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    for (int i = 0; i < N; ++i)         { int s = 4; Yi[i] = MFC_FROM(perf_fill_double(i, s), perf_fill_double(i, s + 131)); }
    memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    for (int r = 0; r < warmup; ++r) {
        whpmv_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
        whpmv_migrated_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    }
    memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whpmv_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whpmv_migrated_(&uplo, &N, &alpha, AP, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    double flops = 8.0 * (double)N * (double)N;
    char key[2] = {uplo, 0};
    perf_emit("whpmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("whpmv", key, N, iters, flops, t_ov, t_mg);
