...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        mtrmv_(&uplo, &trans, &diag, &N, A, &N, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(MFR));
        mtrmv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(MFR));
    }
    ...
    perf_emit("mtrmv", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("mtrmv", key, N, iters, flops, t_subject, t_mg);
