 ...
 *
 * Kernel-isolated C perf harness for mtrmv (overlay vs migrated).
...
BLAS_EXTERN void mtrmv_(const char *, const char *, const char *, const int *,
    const MFR *, const int *, MFR *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void mtrmv_migrated_(const char *, const char *, const char *, const int *,
    const MFR *, const int *, MFR *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        mtrmv_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
        mtrmv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
    }
    ...
    perf_emit("mtrmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("mtrmv", key, N, iters, flops, t_ov, t_mg);
