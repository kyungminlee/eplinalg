 ...
 *
 * Kernel-isolated C perf harness for whbmv (overlay vs migrated).
...
BLAS_EXTERN void whbmv_(const char *, const int *, const int *,
    const MFC *, const MFC *, const int *, const MFC *, const int *,
    const MFC *, MFC *, const int *, size_t);
BLAS_EXTERN void whbmv_migrated_(const char *, const int *, const int *,
    const MFC *, const MFC *, const int *, const MFC *, const int *,
    const MFC *, MFC *, const int *, size_t);
...
static void run_one(char uplo, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        whbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
        whbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    }
    memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
