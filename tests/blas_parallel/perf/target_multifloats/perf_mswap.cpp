 ...
 *
 * Kernel-isolated C perf harness for mswap (overlay vs migrated).
...
BLAS_EXTERN void mswap_(const int *, MFR *, const int *, MFR *, const int *);
BLAS_EXTERN void mswap_migrated_(const int *, MFR *, const int *, MFR *, const int *);

static void run_mswap(int N, int iters, int warmup) {
    int one = 1;
    ...
    for (int r = 0; r < warmup; ++r) {
        mswap_(&N, X, &one, Y, &one);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
        memcpy(Y, Yi, (size_t)N * sizeof(MFR));
        mswap_migrated_(&N, X, &one, Y, &one);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
        memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    }
    ...
    double flops = 4.0 * (double)N * (double)sizeof(MFR);
    perf_emit("mswap", "-", N, iters, flops, t_subject, t_mg);
    perf_emit_json("mswap", "-", N, iters, flops, t_subject, t_mg);
...
int main(void) {
    ...
    perf_print_header();
    for (int i = 0; i < n; ++i) run_mswap(sizes[i], iters, warmup);
    return 0;
}
