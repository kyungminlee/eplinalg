 ...
 *
 * Kernel-isolated C perf harness for mrotg (overlay vs migrated).
...
BLAS_EXTERN void mrotg_(MFR *, MFR *, MFR *, MFR *);
BLAS_EXTERN void mrotg_migrated_(MFR *, MFR *, MFR *, MFR *);

static void run_one(int iters, int warmup) {
    MFR A = MFR_FROM(0.7), B = MFR_FROM(0.7);
    MFR C = Tr_from_d(0.0);
    MFR S = Tr_from_d(0.0);
    /* per call: regenerate fresh A, B inputs */
    for (int r = 0; r < warmup; ++r) {
        MFR a = A, b = B; mrotg_(&a, &b, &C, &S);
        a = A; b = B; mrotg_migrated_(&a, &b, &C, &S);
    }
    ...
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) {
        MFR a = A, b = B; mrotg_migrated_(&a, &b, &C, &S);
    }
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    /* report time per call as "flops" abuse: per-call flop count ~10. */
    double flops = 10.0;
    perf_emit("mrotg", "-", iters, iters, flops, t_subject, t_mg);
    perf_emit_json("mrotg", "-", iters, iters, flops, t_subject, t_mg);
}
