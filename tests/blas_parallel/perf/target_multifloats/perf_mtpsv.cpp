 ...
 *
 * Kernel-isolated C perf harness for mtpsv (overlay vs migrated).
...
BLAS_EXTERN void mtpsv_(const char *, const char *, const char *, const int *,
    const MFR *, MFR *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void mtpsv_migrated_(const char *, const char *, const char *, const int *,
    const MFR *, MFR *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int i = 0; i < N; ++i) { int s = 3; Xi[i] = MFR_FROM(perf_fill_double(i, s)); }
    memcpy(X, Xi, (size_t)N * sizeof(MFR));
    for (int r = 0; r < warmup; ++r) {
        mtpsv_(&uplo, &trans, &diag, &N, AP, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
        mtpsv_migrated_(&uplo, &trans, &diag, &N, AP, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
    }
    ...
    perf_emit("mtpsv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("mtpsv", key, N, iters, flops, t_ov, t_mg);
