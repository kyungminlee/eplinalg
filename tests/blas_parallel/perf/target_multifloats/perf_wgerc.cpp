...
static void run_one(int M, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        wgerc_(&M, &N, &alpha, X, &one, Y, &one, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(MFC));
        wgerc_migrated_(&M, &N, &alpha, X, &one, Y, &one, A, &M);
        memcpy(A, Ai, (size_t)M * (size_t)N * sizeof(MFC));
    }
    ...
    perf_emit("wgerc", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("wgerc", "-", N, iters, flops, t_ov, t_mg);
