 ...
 *
 * Kernel-isolated C perf harness for wtrmm (overlay vs migrated).
...
BLAS_EXTERN void wtrmm_(const char *, const char *, const char *, const char *,
    const int *, const int *, const MFC *,
    const MFC *, const int *, MFC *, const int *,
    size_t, size_t, size_t, size_t);
BLAS_EXTERN void wtrmm_migrated_(const char *, const char *, const char *, const char *,
    const int *, const int *, const MFC *,
    const MFC *, const int *, MFC *, const int *,
    size_t, size_t, size_t, size_t);
...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    for (int r = 0; r < warmup; ++r) {
        wtrmm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(MFC));
        wtrmm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(MFC));
    }
    ...
    perf_emit("wtrmm", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("wtrmm", key, N, iters, flops, t_subject, t_mg);
