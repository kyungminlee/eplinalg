 ...
 *
 * Kernel-isolated C perf harness for mwasum (overlay vs migrated).
...
BLAS_EXTERN MFR mwasum_(const int *, const MFC *, const int *);
BLAS_EXTERN MFR mwasum_migrated_(const int *, const MFC *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    perf_emit("mwasum", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("mwasum", "-", N, iters, flops, t_ov, t_mg);
