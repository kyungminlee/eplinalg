 ...
 *
 * Kernel-isolated C perf harness for msymv (overlay vs migrated).
...
BLAS_EXTERN void msymv_(const char *, const int *, const MFR *, const MFR *, const int *,
    const MFR *, const int *, const MFR *, MFR *, const int *, size_t);
BLAS_EXTERN void msymv_migrated_(const char *, const int *, const MFR *, const MFR *, const int *,
    const MFR *, const int *, const MFR *, MFR *, const int *, size_t);

static void run_one(char uplo, int N, int iters, int warmup) {
    int one = 1;
    MFR alpha = MFR_FROM(0.7), beta = MFR_FROM(0.3);
    MFR *A  = (MFR *)perf_aligned_alloc(64, (size_t)N * (size_t)N * sizeof(MFR));
    ...
    for (size_t i = 0; i < (size_t)N*N; ++i) { int s = 2; A[i] = MFR_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < N; ++i)            { int s = 3; X[i] = MFR_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < N; ++i)            { int s = 4; Yi[i] = MFR_FROM(perf_fill_double(i, s)); }
    memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    for (int r = 0; r < warmup; ++r) {
        msymv_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFR));
        msymv_migrated_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    }
    memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) msymv_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)N * sizeof(MFR));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) msymv_migrated_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    double flops = 2.0 * (double)N * (double)N;
    char key[2] = {uplo, 0};
    perf_emit("msymv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("msymv", key, N, iters, flops, t_ov, t_mg);
