 ...
 *
 * Kernel-isolated C perf harness for wdotc (overlay vs migrated).
...
BLAS_EXTERN MFC wdotc_(const int *, const MFC *, const int *, const MFC *, const int *);
BLAS_EXTERN MFC wdotc_migrated_(const int *, const MFC *, const int *, const MFC *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    for (int r2 = 0; r2 < warmup; ++r2) {
        r = wdotc_(&N, X, &one, Y, &one); sink_T(&r);
        r = wdotc_migrated_(&N, X, &one, Y, &one); sink_T(&r);
    }
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) {
        r = wdotc_(&N, X, &one, Y, &one);
        sink_T(&r);
    }
    ...
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) {
        r = wdotc_migrated_(&N, X, &one, Y, &one);
        sink_T(&r);
    }
    t1 = perf_now_s();
    ...
    perf_emit("wdotc", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("wdotc", "-", N, iters, flops, t_ov, t_mg);
    free(X); free(Y);
}
