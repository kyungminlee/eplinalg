 ...
 *
 * Kernel-isolated C perf harness for imamax (overlay vs migrated).
...
BLAS_EXTERN int imamax_(const int *, const MFR *, const int *);
BLAS_EXTERN int imamax_migrated_(const int *, const MFR *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    perf_emit("imamax", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("imamax", "-", N, iters, flops, t_ov, t_mg);
