 ...
 *
 * Kernel-isolated C perf harness for wtrsm (overlay vs migrated).
...
BLAS_EXTERN void wtrsm_(const char *, const char *, const char *, const char *,
    const int *, const int *, const MFC *,
    const MFC *, const int *, MFC *, const int *,
    size_t, size_t, size_t, size_t);
BLAS_EXTERN void wtrsm_migrated_(const char *, const char *, const char *, const char *,
    const int *, const int *, const MFC *,
    const MFC *, const int *, MFC *, const int *,
    size_t, size_t, size_t, size_t);
...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    MFC *Bi = (MFC *)perf_aligned_alloc(64, (size_t)M * (size_t)N * sizeof(MFC));
    ...
    for (size_t i = 0; i < (size_t)M*N; ++i) { int s = 4; Bi[i] = MFC_FROM(perf_fill_double(i, s), perf_fill_double(i, s + 131)); }
    memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(MFC));
    for (int r = 0; r < warmup; ++r) {
        wtrsm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(MFC));
        wtrsm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(MFC));
    }
    ...
    perf_emit("wtrsm", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("wtrsm", key, N, iters, flops, t_subject, t_mg);
