...
static void run_one(char trans, int M, int N, int iters, int warmup) {
    ...
    MFR *Y  = (MFR *)perf_aligned_alloc(64, (size_t)M * sizeof(MFR));
    MFR *Yi = (MFR *)perf_aligned_alloc(64, (size_t)M * sizeof(MFR));
    ...
    for (int i = 0; i < M; ++i)           { int s = 4; Yi[i] = MFR_FROM(perf_fill_double(i, s)); }

    memcpy(Y, Yi, (size_t)M * sizeof(MFR));
    for (int r = 0; r < warmup; ++r) {
        mgemv_(&trans, &M, &N, &alpha, A, &M, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)M * sizeof(MFR));
        mgemv_migrated_(&trans, &M, &N, &alpha, A, &M, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)M * sizeof(MFR));
    }
    ...
    char key[2] = {trans, 0};
    perf_emit("mgemv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("mgemv", key, N, iters, flops, t_ov, t_mg);
