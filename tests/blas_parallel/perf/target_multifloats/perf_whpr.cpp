 ...
 *
 * Kernel-isolated C perf harness for whpr (overlay vs migrated).
...
BLAS_EXTERN void whpr_(const char *, const int *, const MFR *,
    const MFC *, const int *, MFC *, size_t);
BLAS_EXTERN void whpr_migrated_(const char *, const int *, const MFR *,
    const MFC *, const int *, MFC *, size_t);
...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        whpr_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(MFC));
        whpr_migrated_(&uplo, &N, &alpha, X, &one, AP, 1);
        memcpy(AP, APi, AP_LEN * sizeof(MFC));
    }
