 ...
 *
 * Kernel-isolated C perf harness for waxpy (overlay vs migrated).
...
BLAS_EXTERN void waxpy_(const int *, const MFC *, const MFC *, const int *, MFC *, const int *);
BLAS_EXTERN void waxpy_migrated_(const int *, const MFC *, const MFC *, const int *, MFC *, const int *);
...
static void run_waxpy(int N, int iters, int warmup) {
    ...
    perf_emit("waxpy", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("waxpy", "-", N, iters, flops, t_ov, t_mg);
...
int main(void) {
    ...
    perf_print_header();
    for (int i = 0; i < n; ++i) run_waxpy(sizes[i], iters, warmup);
    return 0;
}
