...
static void run_one(char side, char uplo, int M, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        whemm_(&side, &uplo, &M, &N, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)M * (size_t)N * sizeof(MFC));
        whemm_migrated_(&side, &uplo, &M, &N, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1);
        memcpy(C, Ci, (size_t)M * (size_t)N * sizeof(MFC));
    }
    memcpy(C, Ci, (size_t)M * (size_t)N * sizeof(MFC));
    ...
    perf_emit("whemm", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("whemm", key, N, iters, flops, t_ov, t_mg);
