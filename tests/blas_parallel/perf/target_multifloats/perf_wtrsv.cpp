 ...
 *
 * Kernel-isolated C perf harness for wtrsv (overlay vs migrated).
...
BLAS_EXTERN void wtrsv_(const char *, const char *, const char *, const int *,
    const MFC *, const int *, MFC *, const int *, size_t, size_t, size_t);
BLAS_EXTERN void wtrsv_migrated_(const char *, const char *, const char *, const int *,
    const MFC *, const int *, MFC *, const int *, size_t, size_t, size_t);
...
static void run_one(char uplo, char trans, char diag, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        wtrsv_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
        wtrsv_migrated_(&uplo, &trans, &diag, &N, A, &N, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
    }
