 ...
 *
 * Kernel-isolated C perf harness for mtrmm (overlay vs migrated).
...
BLAS_EXTERN void mtrmm_(const char *, const char *, const char *, const char *,
    const int *, const int *, const MFR *,
    const MFR *, const int *, MFR *, const int *,
    size_t, size_t, size_t, size_t);
BLAS_EXTERN void mtrmm_migrated_(const char *, const char *, const char *, const char *,
    const int *, const int *, const MFR *,
    const MFR *, const int *, MFR *, const int *,
    size_t, size_t, size_t, size_t);
...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    for (int r = 0; r < warmup; ++r) {
        mtrmm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(MFR));
        mtrmm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);
        memcpy(B, Bi, (size_t)M * (size_t)N * sizeof(MFR));
    }
    ...
    perf_emit("mtrmm", key, N, iters, flops, t_subject, t_mg);
    perf_emit_json("mtrmm", key, N, iters, flops, t_subject, t_mg);
