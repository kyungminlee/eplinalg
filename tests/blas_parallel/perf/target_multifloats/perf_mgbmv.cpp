...
static void run_one(char trans, int M, int N, int KL, int KU, int iters, int warmup) {
    ...
    int XL = (trans == 'N') ? N : M;
    int YL = (trans == 'N') ? M : N;
    MFR *X  = (MFR *)perf_aligned_alloc(64, (size_t)XL * sizeof(MFR));
    MFR *Y  = (MFR *)perf_aligned_alloc(64, (size_t)YL * sizeof(MFR));
    MFR *Yi = (MFR *)perf_aligned_alloc(64, (size_t)YL * sizeof(MFR));
    for (size_t i = 0; i < (size_t)LDA*N; ++i) { int s = 2; A[i] = MFR_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < XL; ++i) { int s = 3; X[i] = MFR_FROM(perf_fill_double(i, s)); }
    for (int i = 0; i < YL; ++i) { int s = 4; Yi[i] = MFR_FROM(perf_fill_double(i, s)); }
    memcpy(Y, Yi, (size_t)YL * sizeof(MFR));
    for (int r = 0; r < warmup; ++r) {
        mgbmv_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)YL * sizeof(MFR));
        mgbmv_migrated_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)YL * sizeof(MFR));
    }
    memcpy(Y, Yi, (size_t)YL * sizeof(MFR));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) mgbmv_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)YL * sizeof(MFR));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) mgbmv_migrated_(&trans, &M, &N, &KL, &KU, &alpha, A, &LDA, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    double flops = 2.0 * (double)(KL+KU+1) * (double)N;
    char key[2] = {trans, 0};
    perf_emit("mgbmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("mgbmv", key, N, iters, flops, t_ov, t_mg);
