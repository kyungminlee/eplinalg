...
static void run_one(char uplo, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        whemv_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
        whemv_migrated_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
        memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    }
    memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    double t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whemv_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
    double t1 = perf_now_s();
    double t_ov = (t1 - t0) / (iters ? iters : 1);
    memcpy(Y, Yi, (size_t)N * sizeof(MFC));
    t0 = perf_now_s();
    for (int it = 0; it < iters; ++it) whemv_migrated_(&uplo, &N, &alpha, A, &N, X, &one, &beta, Y, &one, 1);
    t1 = perf_now_s();
    double t_mg = (t1 - t0) / (iters ? iters : 1);
    double flops = 8.0 * (double)N * (double)N;
    char key[2] = {uplo, 0};
    perf_emit("whemv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("whemv", key, N, iters, flops, t_ov, t_mg);
