 ...
 *
 * Kernel-isolated C perf harness for wmscal (overlay vs migrated).
...
BLAS_EXTERN void wmscal_(const int *, const MFR *, MFC *, const int *);
BLAS_EXTERN void wmscal_migrated_(const int *, const MFR *, MFC *, const int *);

static void run_wmscal(int N, int iters, int warmup) {
    int one = 1;
    ...
    for (int r = 0; r < warmup; ++r) {
        wmscal_(&N, &alpha, X, &one);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
        wmscal_migrated_(&N, &alpha, X, &one);
        memcpy(X, Xi, (size_t)N * sizeof(MFC));
    }
    ...
    perf_emit("wmscal", "-", N, iters, flops, t_ov, t_mg);
    perf_emit_json("wmscal", "-", N, iters, flops, t_ov, t_mg);
    free(X); free(Xi);
}
...
int main(void) {
    ...
    perf_print_header();
    for (int i = 0; i < n; ++i) run_wmscal(sizes[i], iters, warmup);
    return 0;
}
