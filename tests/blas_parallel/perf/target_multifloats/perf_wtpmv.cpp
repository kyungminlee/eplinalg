...
static void run_one(char uplo, char trans, char diag, int N, int incx,
    ...
    for (int r = 0; r < warmup; ++r) {
        wtpmv_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(MFC));
        wtpmv_migrated_(&uplo, &trans, &diag, &N, AP, X, &incx, 1, 1, 1);
        memcpy(X, Xi, lenx * sizeof(MFC));
    }
    ...
    perf_emit("wtpmv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("wtpmv", key, N, iters, flops, t_ov, t_mg);
