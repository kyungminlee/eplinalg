...
static void run_one(char uplo, char trans, char diag, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        mtbsv_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
        mtbsv_migrated_(&uplo, &trans, &diag, &N, &K, A, &LDA, X, &one, 1, 1, 1);
        memcpy(X, Xi, (size_t)N * sizeof(MFR));
    }
    ...
    perf_emit("mtbsv", key, N, iters, flops, t_ov, t_mg);
    perf_emit_json("mtbsv", key, N, iters, flops, t_ov, t_mg);
