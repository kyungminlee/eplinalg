/*
 * Self-contained MUMPS C header for the migrated wmumps archive
 * (double-double multifloats; complex arithmetic; prefix
 * w/W, maps to upstream ``z``).
 *
 * Declares WMUMPS_STRUC_C and wmumps_c() directly instead of
 ...
 * widths use this target's WMUMPS_REAL / WMUMPS_COMPLEX (mumps_float64x2 (scalar) / mumps_complex64x2 (complex)),
 * defined below. This makes the struct ABI-identical to the wmumps
 * bridge object (upstream ``mumps_c.c`` compiled with
 * -DZMUMPS_STRUC_C=WMUMPS_STRUC_C -Dzmumps_c=wmumps_c and force-including
...
#endif
#define WMUMPS_REAL    mumps_float64x2
#define WMUMPS_COMPLEX mumps_complex64x2
 ...
 * NB: WMUMPS_COMPLEX are REAL types in s and d arithmetics.
 */
typedef struct {
    ...
    MUMPS_INT      *irn_loc;
    MUMPS_INT      *jcn_loc;
    WMUMPS_COMPLEX *a_loc;

    /* Element entry */
    MUMPS_INT      nelt;
    MUMPS_INT      *eltptr;
    MUMPS_INT      *eltvar;
    WMUMPS_COMPLEX *a_elt;
    ...
    WMUMPS_REAL    *colsca_loc;
    WMUMPS_REAL    *rowsca_loc;

    /* Info after facto */
    MUMPS_INT      *rowind;
    MUMPS_INT      *colind;
    WMUMPS_COMPLEX *pivots;

    /* RHS, solution, ouptput data and statistics */
    WMUMPS_COMPLEX *rhs, *redrhs, *rhs_sparse, *sol_loc, *rhs_loc, *rhsintr;
    ...
    WMUMPS_REAL    rinfo[40], rinfog[40];

    /* Null space */
    MUMPS_INT      deficiency;
    MUMPS_INT      *pivnul_list;
    MUMPS_INT      *mapping;
    WMUMPS_REAL    *singular_values;

    /* Schur */
    MUMPS_INT      size_schur;
    MUMPS_INT      *listvar_schur;
    WMUMPS_COMPLEX *schur;

    /* user workspace */
    WMUMPS_COMPLEX *wk_user;
...
void MUMPS_CALL
wmumps_c( WMUMPS_STRUC_C * wmumps_par );
