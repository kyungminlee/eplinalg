...
def run_bench_one(bdir: Path, routine: str, size: int, iters: int,
    ...
    (overlay GF/s, migrated GF/s) rows, or None on failure."""
...
def summarize_bench(rows) -> tuple[float, float] | None:
    """Return (median overlay GF/s, median speedup overlay/migrated)."""
    if not rows:
        return None
...
def main():
    ...
    out_lines = [
        ...
        "",
        f"Auto-generated by `scripts/blas_overlay_report.py`. Size = {args.size}, "
        f"iters = {args.iters}, OMP = 1 / 4. Speedup = "
        "`overlay_gflops / migrated_gflops`. Error digits are "
