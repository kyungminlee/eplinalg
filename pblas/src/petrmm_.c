...
#ifdef __STDC__
void petrmm_( F_CHAR_T SIDE, F_CHAR_T UPLO, F_CHAR_T TRANS, F_CHAR_T DIAG,
              Int * M, Int * N, EREAL * ALPHA,
              EREAL * A, Int * IA, Int * JA, Int * DESCA,
              EREAL * B, Int * IB, Int * JB, Int * DESCB )
#else
void petrmm_( SIDE, UPLO, TRANS, DIAG, M, N, ALPHA,
...
*  =======
*
*  PETRMM  performs one of the matrix-matrix operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 1101 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PETRMM", "Illegal SIDE = %c\n", SideOp );
         info = -1;
      }
      else if( ( !upper ) && ( UploA != CLOWER ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PETRMM", "Illegal UPLO = %c\n", UploA );
         info = -2;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PETRMM", "Illegal TRANS = %c\n", TranOp );
         info = -3;
      }
      if( ( !nounit ) && ( DiagA != CUNIT ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PETRMM",
                   "Illegal DIAG = %c\n", DiagA );
         info = -4;
      }
      if( lside )
         PB_Cchkmat( ctxt, "PETRMM", "A", *M, 5, *M, 5, Ai, Aj, Ad, 11,
                     &info );
      else
         PB_Cchkmat( ctxt, "PETRMM", "A", *N, 6, *N, 6, Ai, Aj, Ad, 11,
                     &info );
      PB_Cchkmat(    ctxt, "PETRMM", "B", *M, 5, *N, 6, Bi, Bj, Bd, 15,
                     &info );
   }
   if( info ) { PB_Cabort( ctxt, "PETRMM", info ); return; }
#endif
/*
