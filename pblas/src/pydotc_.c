...
#else
void pydotc_( N, DOT, X, IX, JX, DESCX, INCX, Y, IY, JY, DESCY, INCY )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PYDOTC  forms the dot product of two subvectors,
*
*     DOT := sub( X )**H * sub( Y ),
*
*  where
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PYDOTC", "X", *N, 1, Xi, Xj, Xd, *INCX,  6, &info );
      PB_Cchkvec( ctxt, "PYDOTC", "Y", *N, 1, Yi, Yj, Yd, *INCY, 11, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYDOTC", info ); return; }
#endif
   DOT[REAL_PART] = ZERO;
   DOT[IMAG_PART] = ZERO;
/*
...
*/
      type = PB_Cytypeset(); size = type->size; dot = type->Fvvdotc;
/*
...
*/
         if( ( XisR && YisR ) || ( XmyprocR == XprocR ) )
         {
            ...
            Cygsum2d( ctxt, &scope, top, 1, 1, ((char *) DOT), 1, -1, 0 );
         }
...
*/
            ...
            if( XnpD > 0 )
            {
               ...
               if( ( myrow != rsrc ) || ( mycol != csrc ) )
               {
                  ...
                  if( YisRow )
                     Cygerv2d( ctxt, 1, XnpD, buf,    1, rsrc, csrc );
                  else
                     Cygerv2d( ctxt, XnpD, 1, buf, XnpD, rsrc, csrc );
                  dot( &XnpD, ((char *) DOT), Mptr( ((char *) X), Xii, Xjj, Xld,
                       size ), &Xlinc, buf, &ione );
                  if( buf ) free( buf );
               }
            ...
            if( XisRow )
            {
               ...
               Cygsum2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1, -1, 0 );
            }
            else
            {
               top = PB_Ctop( &ctxt, COMBINE, COLUMN, TOP_GET );
               Cygsum2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1, -1, 0 );
            }
         }
      }
      else
...
*/
         if( XisRow )
         {
           ...
           if( XmyprocR == XprocR )
              Cygebs2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1 );
           else
              Cygebr2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1, XprocR,
                        XmyprocD );
         }
         else
         {
           ...
           if( XmyprocR == XprocR )
              Cygebs2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1 );
           else
              Cygebr2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1, XmyprocD,
                        XprocR );
         }
      }
      else
...
*/
            if( XprocR != YprocR )
            {
               ...
               {
                  if( XisRow )
                     Cygesd2d( ctxt, 1, 1, ((char *) DOT), 1, YprocR,
                               YmyprocD );
                  else
                     Cygesd2d( ctxt, 1, 1, ((char *) DOT), 1, YmyprocD,
                               YprocR );
               }
               else if( YmyprocR == YprocR )
               {
                  if( XisRow )
                     Cygerv2d( ctxt, 1, 1, ((char *) DOT), 1, XprocR,
                               XmyprocD );
                  else
                     Cygerv2d( ctxt, 1, 1, ((char *) DOT), 1, XmyprocD,
                               XprocR );
               }
            }
         }
            ...
            if( YmyprocR == YprocR )
            {
               if( YisRow )
               {
                  ...
                  if( YmyprocD == XprocR )
                     ...
                     Cygebr2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1,
                               YprocR, XprocR );
               }
               else
               {
                  ...
                  if( YmyprocD == XprocR )
                     ...
                     Cygebr2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1,
                               XprocR, YprocR );
               }
            }
         }
