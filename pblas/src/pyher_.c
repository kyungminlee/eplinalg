...
#ifdef __STDC__
void pyher_( F_CHAR_T UPLO, Int * N, EREAL * ALPHA,
             EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX,
             EREAL * A, Int * IA, Int * JA, Int * DESCA )
#else
void pyher_( UPLO, N, ALPHA, X, IX, JX, DESCX, INCX, A, IA, JA, DESCA )
/*
*  .. Scalar Arguments ..
*/
   F_CHAR_T       UPLO;
...
*  =======
*
*  PYHER  performs the Hermitian rank 1 operation
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYHER", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      PB_Cchkvec( ctxt, "PYHER", "X", *N, 2, Xi, Xj, Xd, *INCX,  7, &info );
      PB_Cchkmat( ctxt, "PYHER", "A", *N, 2, *N, 2, Ai, Aj, Ad, 12, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYHER", info ); return; }
#endif
/*
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
               ygerc_( &Akp, &Anq0, ((char *) Calpha), XC, &ione,
      ...
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            Anq0 = PB_Cnumroc( kb,   k, Ainb1, Anb, mycol, Acol, npcol );
            if( Amp0 > 0 && Anq0 > 0 )
               ygerc_( &Amp0, &Anq0, ((char *) Calpha), Mptr( XC, Akp,
