...
#ifdef __STDC__
void pyamax_( Int * N, EREAL * AMAX, Int * INDX,
              EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX )
#else
void pyamax_( N, AMAX, INDX, X, IX, JX, DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PYAMAX  computes the global index of the maximum element in  absolute
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PYAMAX", "X", *N, 1, Xi, Xj, Xd, *INCX, 7, &info );
   if( info ) { PB_Cabort( ctxt, "PYAMAX", info ); return; }
#endif
/*
...
*/
          ...
          ( ( mycol == Xcol ) || ( Xcol < 0 ) ) )
      {
         *INDX = *JX;
         ...
         AMAX[REAL_PART] = ((EREAL*)(Xptr))[REAL_PART];
         AMAX[IMAG_PART] = ((EREAL*)(Xptr))[IMAG_PART];
      }
      return;
   }
   else if( *INCX == Xd[M_] )
...
*/
            if( Xnq > 0 )
            {
               ...
               Xlindx = Xjj - 1 +
                        iyamax_( &Xnq, Mptr( ((char *) X), Xii, Xjj, Xld,
                                 size ), &Xld );
               Mindxl2g( Xgindx, Xlindx, Xinb, Xnb, mycol, Xsrc, npcol );
               Xptr = Mptr( ((char *) X), Xii, Xlindx, Xld, size );
               work[0][REAL_PART] = ((EREAL*)(Xptr))[REAL_PART];
               work[0][IMAG_PART] = ((EREAL*)(Xptr))[IMAG_PART];
               work[1][REAL_PART] = ((EREAL)( Xgindx+1 ));
...
l_10:
               if( mydist & 1 )
               {
                  ...
                  Cygesd2d( ctxt, 2, 1, ((char*)work), 2, myrow, dst );
                  goto l_20;
               }
               else
               {
                  ...
                  if( mycol < src )
                  {
                     Cygerv2d( ctxt, 2, 1, ((char*) work[2]), 2, myrow,
                               src );
...
*/
               ...
               if( mycol == 0 )
               {
                  Cygebs2d( ctxt, ROW, &rbtop, 2, 1, ((char*)work), 2 );
               }
               else
               {
                  Cygebr2d( ctxt, ROW, &rbtop, 2, 1, ((char*)work), 2,
                            myrow, 0 );
               }
            }
/*
...
*  distributed
*/
               Cygamx2d( ctxt, ROW, &rctop, 1, 1, ((char*)AMAX), 1,
...
*/
            if( Xnp > 0 )
            {
               ...
               type = PB_Cytypeset(); size = type->size;
               Xlindx = Xii - 1 +
                        iyamax_( &Xnp, Mptr( ((char *)X), Xii, Xjj, Xld,
                                 size ), INCX );
...
l_30:
               if( mydist & 1 )
               {
                  ...
                  Cygesd2d( ctxt, 2, 1, ((char*)work), 2, dst, mycol );
                  goto l_40;
               }
               else
               {
                  ...
                  if( myrow < src )
                  {
                     Cygerv2d( ctxt, 2, 1, ((char*) work[2]), 2,
                               src, mycol );
                     if( ( ABS( work[0][REAL_PART] ) +
                           ABS( work[0][IMAG_PART] ) ) <
                         ( ABS( work[2][REAL_PART] ) +
...
*/
               ...
               if( myrow == 0 )
               {
                  Cygebs2d( ctxt, COLUMN, &cbtop, 2, 1, ((char*)work), 2 );
               }
               else
               {
                  Cygebr2d( ctxt, COLUMN, &cbtop, 2, 1, ((char*)work), 2,
                            0, mycol );
               }
            }
/*
...
*  distributed.
*/
               Cygamx2d( ctxt, COLUMN, &cctop, 1, 1, ((char*)AMAX), 1,
