...
*  =======
*
*  PYHER2  performs the Hermitian rank 2 operation
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYHER2", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      PB_Cchkvec( ctxt, "PYHER2", "X", *N, 2, Xi, Xj, Xd, *INCX,  7, &info );
      PB_Cchkvec( ctxt, "PYHER2", "Y", *N, 2, Yi, Yj, Yd, *INCY, 12, &info );
      PB_Cchkmat( ctxt, "PYHER2", "A", *N, 2, *N, 2, Ai, Aj, Ad, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYHER2", info ); return; }
#endif
/*
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               ygerc_( &Akp, &Anq0, ((char *) ALPHA), XC, &ione,
                       Mptr( YR,   0, Akq, YRld, size ), &YRld,
                       Mptr( Aptr, 0, Akq,  Ald, size ), &Ald );
               ygerc_( &Akp, &Anq0, ((char *) Calpha), YC, &ione,
