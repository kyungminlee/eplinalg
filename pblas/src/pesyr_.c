...
#ifdef __STDC__
void pesyr_( F_CHAR_T UPLO, Int * N, EREAL * ALPHA,
             EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX,
             EREAL * A, Int * IA, Int * JA, Int * DESCA )
#else
void pesyr_( UPLO, N, ALPHA, X, IX, JX, DESCX, INCX, A, IA, JA, DESCA )
/*
*  .. Scalar Arguments ..
*/
   F_CHAR_T       UPLO;
...
*  =======
*
*  PESYR  performs the symmetric rank 1 operation
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PESYR", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      PB_Cchkvec( ctxt, "PESYR", "X", *N, 2, Xi, Xj, Xd, *INCX,  7, &info );
      PB_Cchkmat( ctxt, "PESYR", "A", *N, 2, *N, 2, Ai, Aj, Ad, 12, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PESYR", info ); return; }
#endif
/*
...
*/
      ...
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            Anq0 = PB_Cnumroc( kb,   k, Ainb1, Anb, mycol, Acol, npcol );
            if( Amp0 > 0 && Anq0 > 0 )
               eger_( &Amp0, &Anq0, ((char *) ALPHA), Mptr( XC, Akp,
