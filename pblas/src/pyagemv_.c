...
*  =======
*
*  PYAGEMV  performs one of the matrix-vector operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYAGEMV", "Illegal TRANS=%c\n", TrA );
         info = -1;
      }
      PB_Cchkmat(    ctxt, "PYAGEMV", "A", *M, 2, *N, 3, Ai, Aj, Ad,  8,
                     &info );
      if( nota )
      {
         PB_Cchkvec( ctxt, "PYAGEMV", "X", *N, 3, Xi, Xj, Xd, *INCX, 12,
                     &info );
         PB_Cchkvec( ctxt, "PYAGEMV", "Y", *M, 2, Yi, Yj, Yd, *INCY, 18,
                     &info );
      }
      else
      {
         PB_Cchkvec( ctxt, "PYAGEMV", "X", *M, 2, Xi, Xj, Xd, *INCX, 12,
                     &info );
         PB_Cchkvec( ctxt, "PYAGEMV", "Y", *N, 3, Yi, Yj, Yd, *INCY, 18,
                     &info );
      }
   }
   if( info ) { PB_Cabort( ctxt, "PYAGEMV", info ); return; }
#endif
/*
...
*/
      ...
      if( ( Amp > 0 ) && ( Anq > 0 ) )
      {
         yagemv_( TRANS, &Amp, &Anq, ((char *) ALPHA), Mptr( ((char *) A),
