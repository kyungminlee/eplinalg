...
#ifdef __STDC__
void pysyrk_( F_CHAR_T UPLO, F_CHAR_T TRANS, Int * N, Int * K,
              EREAL * ALPHA,
              EREAL * A, Int * IA, Int * JA, Int * DESCA,
              EREAL * BETA,
              EREAL * C, Int * IC, Int * JC, Int * DESCC )
#else
void pysyrk_( UPLO, TRANS, N, K, ALPHA, A, IA, JA, DESCA, BETA,
              C, IC, JC, DESCC )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PYSYRK  performs one of the symmetric rank k operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 901 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYSYRK", "Illegal UPLO = %c\n", UploC );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PYSYRK", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      if( notran )
         PB_Cchkmat( ctxt, "PYSYRK", "A", *N, 3, *K, 4, Ai, Aj, Ad,  9,
                     &info );
      else
         PB_Cchkmat( ctxt, "PYSYRK", "A", *K, 4, *N, 3, Ai, Aj, Ad,  9,
                     &info );
      PB_Cchkmat(    ctxt, "PYSYRK", "C", *N, 3, *N, 3, Ci, Cj, Cd, 14,
                     &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYSYRK", info ); return; }
#endif
/*
