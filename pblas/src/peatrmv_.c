...
#ifdef __STDC__
void peatrmv_( F_CHAR_T UPLO, F_CHAR_T TRANS, F_CHAR_T DIAG, Int * N,
               EREAL * ALPHA,
               ...
               Int * INCY )
#else
void peatrmv_( UPLO, TRANS, DIAG, N, ALPHA, A, IA, JA, DESCA, X, IX,
...
*  =======
*
*  PEATRMV  performs one of the matrix-vector operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PEATRMV", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PEATRMV", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      else if( ( !nounit ) && ( DiagA != CUNIT ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PEATRMV", "Illegal DIAG = %c\n", DiagA );
         info = -3;
      }
      PB_Cchkmat( ctxt, "PEATRMV", "A", *N, 4, *N, 4, Ai, Aj, Ad,  9, &info );
      PB_Cchkvec( ctxt, "PEATRMV", "X", *N, 4, Xi, Xj, Xd, *INCX, 13, &info );
      PB_Cchkvec( ctxt, "PEATRMV", "Y", *N, 4, Yi, Yj, Yd, *INCY, 19, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PEATRMV", info ); return; }
#endif
/*
      ...
      if( upper )
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Akp > 0 && Anq0 > 0 )
               {
                  eagemv_( TRANS, &Akp, &Anq0, ((char *) ALPHA),
      ...
      else
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Amp0 > 0 && Anq0 > 0 )
               {
                  eagemv_( TRANS, &Amp0, &Anq0, ((char *) ALPHA),
