...
#ifdef __STDC__
void peyasum_( Int * N, EREAL * ASUM,
               EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX )
#else
void peyasum_( N, ASUM, X, IX, JX, DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
   Int            * INCX, * IX, * JX, * N;
...
*  =======
*
*  PEYASUM  returns  the sum of absolute values of the entries of a sub-
*  vector sub( X ),
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PEYASUM", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PEYASUM", info ); return; }
#endif
/*
*  Initialize ASUM
*/
...
*/
          ...
          ( ( mycol == Xcol ) || ( Xcol < 0 ) ) )
      {
         ...
         *ASUM = ABS( ((EREAL *) Xptr)[REAL_PART] ) +
                 ABS( ((EREAL *) Xptr)[IMAG_PART] );
      }
      return;
   }
   else if( *INCX == Xd[M_] )
...
*/
         ...
         if( Xnq > 0 )
         {
            Xld = Xd[LLD_];
            type = PB_Cytypeset();
            eyvasum_( &Xnq, ((char *) ASUM), Mptr( ((char *) X), Xii,
                      Xjj, Xld, type->size ), &Xld );
         }
/*
...
*/
            ...
            Cegsum2d( ctxt, ROW, &top, 1, 1, ((char *)ASUM), 1, -1,
                      mycol );
         }
      }
      return;
...
*/
         ...
         {
            type = PB_Cytypeset();
            eyvasum_( &Xnp, ((char *) ASUM), Mptr( ((char *) X), Xii,
                      Xjj, Xd[LLD_], type->size ), INCX );
         }
/*
...
*/
            ...
            Cegsum2d( ctxt, COLUMN, &top, 1, 1, ((char *)ASUM), 1, -1,
                      mycol );
         }
      }
      return;
