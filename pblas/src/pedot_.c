...
#else
void pedot_( N, DOT, X, IX, JX, DESCX, INCX, Y, IY, JY, DESCY, INCY )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PEDOT  forms the dot product of two subvectors,
*
*     DOT := sub( X )**T * sub( Y ),
*
*  where
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PEDOT", "X", *N, 1, Xi, Xj, Xd, *INCX,  6, &info );
      PB_Cchkvec( ctxt, "PEDOT", "Y", *N, 1, Yi, Yj, Yd, *INCY, 11, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PEDOT", info ); return; }
#endif
   DOT[REAL_PART] = ZERO;
/*
...
*/
      type = PB_Cetypeset(); size = type->size; dot = type->Fvvdotu;
/*
...
*/
         if( ( XisR && YisR ) || ( XmyprocR == XprocR ) )
         {
            ...
            Cegsum2d( ctxt, &scope, top, 1, 1, ((char *) DOT), 1, -1, 0 );
         }
...
*/
            ...
            if( XnpD > 0 )
            {
               ...
               if( ( myrow != rsrc ) || ( mycol != csrc ) )
               {
                  ...
                  if( YisRow )
                     Cegerv2d( ctxt, 1, XnpD, buf,    1, rsrc, csrc );
                  else
                     Cegerv2d( ctxt, XnpD, 1, buf, XnpD, rsrc, csrc );
                  dot( &XnpD, ((char *) DOT), Mptr( ((char *) X), Xii, Xjj, Xld,
                       size ), &Xlinc, buf, &ione );
                  if( buf ) free( buf );
               }
            ...
            if( XisRow )
            {
               ...
               Cegsum2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1, -1, 0 );
            }
            else
            {
               top = PB_Ctop( &ctxt, COMBINE, COLUMN, TOP_GET );
               Cegsum2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1, -1, 0 );
            }
         }
      }
      else
...
*/
         if( XisRow )
         {
           ...
           if( XmyprocR == XprocR )
              Cegebs2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1 );
           else
              Cegebr2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1, XprocR,
                        XmyprocD );
         }
         else
         {
           ...
           if( XmyprocR == XprocR )
              Cegebs2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1 );
           else
              Cegebr2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1, XmyprocD,
                        XprocR );
         }
      }
      else
...
*/
            if( XprocR != YprocR )
            {
               ...
               {
                  if( XisRow )
                     Cegesd2d( ctxt, 1, 1, ((char *) DOT), 1, YprocR,
                               YmyprocD );
                  else
                     Cegesd2d( ctxt, 1, 1, ((char *) DOT), 1, YmyprocD,
                               YprocR );
               }
               else if( YmyprocR == YprocR )
               {
                  if( XisRow )
                     Cegerv2d( ctxt, 1, 1, ((char *) DOT), 1, XprocR,
                               XmyprocD );
                  else
                     Cegerv2d( ctxt, 1, 1, ((char *) DOT), 1, XmyprocD,
                               XprocR );
               }
            }
         }
            ...
            if( YmyprocR == YprocR )
            {
               if( YisRow )
               {
                  ...
                  if( YmyprocD == XprocR )
                     ...
                     Cegebr2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1,
                               YprocR, XprocR );
               }
               else
               {
                  ...
                  if( YmyprocD == XprocR )
                     ...
                     Cegebr2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1,
                               XprocR, YprocR );
               }
            }
         }
