...
#ifdef __STDC__
void pyscal_( Int * N, EREAL * ALPHA,
              EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX )
#else
void pyscal_( N, ALPHA, X, IX, JX, DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
   Int            * INCX, * IX, * JX, * N;
...
*  =======
*
*  PYSCAL  multiplies  an  n  element  subvector  sub( X ) by the scalar
*  alpha,
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PYSCAL", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PYSCAL", info ); return; }
#endif
/*
...
*/
         ...
         if( Xnq > 0 )
         {
            ...
            {
               yset_( &Xnq, ((char *) ALPHA), Mptr( ((char *) X), Xii, Xjj,
                      Xld, type->size ), &Xld );
            }
            else
            {
               yscal_( &Xnq, ((char *) ALPHA), Mptr( ((char *) X), Xii,
...
*/
         ...
         if( Xnp > 0 )
         {
            ...
            if( ( ALPHA[REAL_PART] == ZERO ) && ( ALPHA[IMAG_PART] == ZERO ) )
            {
               yset_( &Xnp, ((char *) ALPHA), Mptr( ((char *) X), Xii,
                      Xjj, Xd[LLD_], type->size ), INCX );
            }
            else
            {
               yscal_( &Xnp, ((char *) ALPHA), Mptr( ((char *) X), Xii,
                       Xjj, Xd[LLD_], type->size ), INCX );
            }
         }
      }
