...
#ifdef __STDC__
void pescal_( Int * N, EREAL * ALPHA,
              EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX )
#else
void pescal_( N, ALPHA, X, IX, JX, DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
   Int            * INCX, * IX, * JX, * N;
...
*  =======
*
*  PESCAL  multiplies  an  n  element  subvector  sub( X ) by the scalar
*  alpha,
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PESCAL", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PESCAL", info ); return; }
#endif
/*
...
*/
         ...
         if( Xnq > 0 )
         {
            ...
            {
               eset_( &Xnq, ((char *) ALPHA), ((char *)Mptr(X,Xii,Xjj,Xld,1)),
                      &Xld );
            }
            else
            {
               escal_( &Xnq, ((char *) ALPHA), ((char *)Mptr(X,Xii,Xjj,Xld,1)),
                       &Xld );
            }
         }
      }
...
*/
         ...
         if( Xnp > 0 )
         {
            if( ALPHA[REAL_PART] == ZERO )
            {
               eset_( &Xnp, ((char *) ALPHA),
                      ((char *)Mptr( X,Xii,Xjj,Xd[LLD_],1) ), INCX );
            }
            else
            {
               escal_( &Xnp, ((char *) ALPHA),
                       ((char *)Mptr( X,Xii,Xjj,Xd[LLD_],1) ), INCX );
            }
         }
      }
