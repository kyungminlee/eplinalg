...
#ifdef __STDC__
void pyatrmv_( F_CHAR_T UPLO, F_CHAR_T TRANS, F_CHAR_T DIAG, Int * N,
               EREAL * ALPHA,
               EREAL * A, Int * IA, Int * JA, Int * DESCA,
               EREAL * X, Int * IX, Int * JX, Int * DESCX,
               Int * INCX, EREAL * BETA,
               EREAL * Y, Int * IY, Int * JY, Int * DESCY,
               Int * INCY )
#else
void pyatrmv_( UPLO, TRANS, DIAG, N, ALPHA, A, IA, JA, DESCA, X, IX,
...
*  =======
*
*  PYATRMV  performs one of the matrix-vector operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYATRMV", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PYATRMV", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      else if( ( !nounit ) && ( DiagA != CUNIT ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PYATRMV", "Illegal DIAG = %c\n", DiagA );
         info = -3;
      }
      PB_Cchkmat( ctxt, "PYATRMV", "A", *N, 4, *N, 4, Ai, Aj, Ad,  9, &info );
      PB_Cchkvec( ctxt, "PYATRMV", "X", *N, 4, Xi, Xj, Xd, *INCX, 13, &info );
      PB_Cchkvec( ctxt, "PYATRMV", "Y", *N, 4, Yi, Yj, Yd, *INCY, 19, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYATRMV", info ); return; }
#endif
/*
...
*/
            if( Amp > 0 )
               eascal_( &Amp, ((char *) tbeta), YA, &ione );
         }
      }
...
*/
            if( Anq > 0 )
               eascal_( &Anq, ((char *) tbeta), YA, &YAld );
         }
      }
/*
      ...
      if( upper )
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Akp > 0 && Anq0 > 0 )
               {
                  yagemv_( TRANS, &Akp, &Anq0, ((char *) ALPHA),
      ...
      else
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Amp0 > 0 && Anq0 > 0 )
               {
                  yagemv_( TRANS, &Amp0, &Anq0, ((char *) ALPHA),
         ...
         else
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Amp0 > 0 && Anq0 > 0 )
               {
                   ...
                   * scaling. Same bug as peatrmv_.c L,T; see the
