...
#ifdef __STDC__
void pesyrk_( F_CHAR_T UPLO, F_CHAR_T TRANS, Int * N, Int * K,
              EREAL * ALPHA,
              EREAL * A, Int * IA, Int * JA, Int * DESCA,
              EREAL * BETA,
              EREAL * C, Int * IC, Int * JC, Int * DESCC )
#else
void pesyrk_( UPLO, TRANS, N, K, ALPHA, A, IA, JA, DESCA, BETA,
              C, IC, JC, DESCC )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PESYRK  performs one of the symmetric rank k operations
*
...
*/
   ...
   EREAL         Aest, ACest, tmp1, tmp2, tmp3, tmp4;
   PBTYP_T        * type;
/*
*  .. Local Arrays ..
*/
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 901 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PESYRK", "Illegal UPLO = %c\n", UploC );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PESYRK", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      if( notran )
         PB_Cchkmat( ctxt, "PESYRK", "A", *N, 3, *K, 4, Ai, Aj, Ad,  9,
                     &info );
      else
         PB_Cchkmat( ctxt, "PESYRK", "A", *K, 4, *N, 3, Ai, Aj, Ad,  9,
                     &info );
      PB_Cchkmat(    ctxt, "PESYRK", "C", *N, 3, *N, 3, Ci, Cj, Cd, 14,
                     &info );
   }
   if( info ) { PB_Cabort( ctxt, "PESYRK", info ); return; }
#endif
/*
