...
#ifdef __STDC__
void pyher2k_( F_CHAR_T UPLO, F_CHAR_T TRANS, Int * N, Int * K,
              EREAL * ALPHA,
...
#else
void pyher2k_( UPLO, TRANS, N, K, ALPHA, A, IA, JA, DESCA,
...
*  =======
*
*  PYHER2K  performs one of the Hermitian rank 2k operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 901 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYHER2K", "Illegal UPLO = %c\n", UploC );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PYHER2K", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      if( notran )
      {
         PB_Cchkmat( ctxt, "PYHER2K", "A", *N, 3, *K, 4, Ai, Aj, Ad,  9,
                     &info );
         PB_Cchkmat( ctxt, "PYHER2K", "B", *N, 3, *K, 4, Bi, Bj, Bd, 13,
                     &info );
      }
      else
      {
         PB_Cchkmat( ctxt, "PYHER2K", "A", *K, 4, *N, 3, Ai, Aj, Ad,  9,
                     &info );
         PB_Cchkmat( ctxt, "PYHER2K", "B", *K, 4, *N, 3, Bi, Bj, Bd, 13,
                     &info );
      }
      PB_Cchkmat(    ctxt, "PYHER2K", "C", *N, 3, *N, 3, Ci, Cj, Cd, 18,
                     &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYHER2K", info ); return; }
#endif
/*
