...
*  =======
*
*  PYGERU  performs the rank 1 operation
*
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PYGERU", "X", *M, 1, Xi, Xj, Xd, *INCX,  7, &info );
      PB_Cchkvec( ctxt, "PYGERU", "Y", *N, 2, Yi, Yj, Yd, *INCY, 12, &info );
      PB_Cchkmat( ctxt, "PYGERU", "A", *M, 1, *N, 2, Ai, Aj, Ad, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYGERU", info ); return; }
#endif
/*
...
*/
   ...
   if( ( Amp > 0 ) && ( Anq > 0 ) )
   {
      ygeru_( &Amp, &Anq, ((char *) ALPHA), XA, &ione, YA, &YAd[LLD_],
