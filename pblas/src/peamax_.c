...
#ifdef __STDC__
void peamax_( Int * N, EREAL * AMAX, Int * INDX,
              EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX )
#else
void peamax_( N, AMAX, INDX, X, IX, JX, DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PEAMAX  computes the global index of the maximum element in  absolute
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PEAMAX", "X", *N, 1, Xi, Xj, Xd, *INCX, 7, &info );
   if( info ) { PB_Cabort( ctxt, "PEAMAX", info ); return; }
#endif
/*
...
l_10:
               if( mydist & 1 )
               {
                  ...
                  Cegesd2d( ctxt, 2, 1, ((char*)work), 2, myrow, dst );
                  goto l_20;
               }
               else
               {
                  ...
                  if( mycol < src )
                  {
                     Cegerv2d( ctxt, 2, 1, ((char*) &work[2]), 2, myrow,
                               src );
...
*/
               ...
               if( mycol == 0 )
               {
                  Cegebs2d( ctxt, ROW, &rbtop, 2, 1, ((char*)work), 2 );
               }
               else
               {
                  Cegebr2d( ctxt, ROW, &rbtop, 2, 1, ((char*)work), 2,
                            myrow, 0 );
               }
            }
/*
...
*  distributed
*/
               Cegamx2d( ctxt, ROW, &rctop, 1, 1, ((char*)AMAX), 1,
...
*/
            if( Xnp > 0 )
            {
               Xld     = Xd[LLD_];
               Xlindx  = Xii - 1 +
                         ieamax_( &Xnp, ((char*)Mptr(X,Xii,Xjj,Xld,1)), INCX );
...
l_30:
               if( mydist & 1 )
               {
                  ...
                  Cegesd2d( ctxt, 2, 1, ((char*)work), 2, dst, mycol );
                  goto l_40;
               }
               else
               {
                  ...
                  if( myrow < src )
                  {
                     Cegerv2d( ctxt, 2, 1, ((char*) &work[2]), 2,
                               src, mycol );
                     if( ABS( work[0] ) < ABS( work[2] ) )
                     { work[0] = work[2]; work[1] = work[3]; }
                  }
...
*/
               ...
               if( myrow == 0 )
               {
                  Cegebs2d( ctxt, COLUMN, &cbtop, 2, 1, ((char*)work), 2 );
               }
               else
               {
                  Cegebr2d( ctxt, COLUMN, &cbtop, 2, 1, ((char*)work), 2,
                            0, mycol );
               }
            }
/*
...
*  distributed.
*/
               Cegamx2d( ctxt, COLUMN, &cctop, 1, 1, ((char*)AMAX), 1,
