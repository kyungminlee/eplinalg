...
#else
void pyescal_( N, ALPHA, X, IX, JX, DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
   Int            * INCX, * IX, * JX, * N;
...
*  =======
*
*  PYESCAL multiplies an n element subvector sub( X ) by the real scalar
*  alpha,
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PYESCAL", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PYESCAL", info ); return; }
#endif
/*
...
*/
         ...
         if( Xnq > 0 )
         {
            ...
            {
               yset_( &Xnq, type->zero, Mptr( ((char *) X), Xii, Xjj, Xld,
                      type->size ), &Xld );
            }
            else
            {
               yescal_( &Xnq, ((char *) ALPHA), Mptr( ((char *) X),
...
*/
         ...
         if( Xnp > 0 )
         {
            ...
            if( ALPHA[REAL_PART] == ZERO )
            {
               yset_( &Xnp, type->zero, Mptr( ((char *) X), Xii, Xjj,
                      Xd[LLD_], type->size ), INCX );
            }
            else
            {
               yescal_( &Xnp, ((char *) ALPHA), Mptr( ((char *) X),
