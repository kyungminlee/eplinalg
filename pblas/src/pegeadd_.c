...
#ifdef __STDC__
void pegeadd_( F_CHAR_T TRANS, Int * M, Int * N,
               EREAL * ALPHA,
               EREAL * A, Int * IA, Int * JA, Int * DESCA,
               EREAL * BETA,
               EREAL * C, Int * IC, Int * JC, Int * DESCC )
#else
void pegeadd_( TRANS, M, N, ALPHA, A, IA, JA, DESCA, BETA, C, IC, JC, DESCC )
/*
*  .. Scalar Arguments ..
*/
   F_CHAR_T       TRANS;
...
*  =======
*
*  PEGEADD  adds a matrix to another
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PEGEADD", "Illegal TRANS = %c\n", TrA );
         info = -1;
      }
      if( notran )
         PB_Cchkmat( ctxt, "PEGEADD", "A", *M, 2, *N, 3, Ai, Aj, Ad,  8,
                     &info );
      else
         PB_Cchkmat( ctxt, "PEGEADD", "A", *N, 3, *M, 2, Ai, Aj, Ad,  8,
                     &info );
      PB_Cchkmat( ctxt, "PEGEADD", "C", *M, 2, *N, 3, Ci, Cj, Cd, 13, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PEGEADD", info ); return; }
#endif
/*
...
*/
   if( ALPHA[REAL_PART] == ZERO )
   {
      ...
      {
         PB_Cplapad( PB_Cetypeset(), ALL, NOCONJG, *M, *N, ((char *)BETA),
                     ((char *)BETA), ((char *) C), Ci, Cj, Cd );
      }
      else
      {
         PB_Cplascal( PB_Cetypeset(), ALL, NOCONJG, *M, *N, ((char *)BETA),
                      ((char * )C), Ci, Cj, Cd );
      }
      return;
   }
...
*/
   if( notran )
   {
      ...
      PB_Cpgeadd( PB_Cetypeset(), &DirA, &DirC, NOCONJG, *M, *N,
                  ((char *) ALPHA), ((char *) A), Ai, Aj, Ad,
                  ((char *)  BETA), ((char *) C), Ci, Cj, Cd );
   }
   else
   {
      PB_Cptran( PB_Cetypeset(), NOCONJG, *M, *N, ((char *) ALPHA),
