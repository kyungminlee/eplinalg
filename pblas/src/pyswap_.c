...
#ifdef __STDC__
void pyswap_( Int * N,
              EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX,
              EREAL * Y, Int * IY, Int * JY, Int * DESCY, Int * INCY )
#else
void pyswap_( N, X, IX, JX, DESCX, INCX, Y, IY, JY, DESCY, INCY )
/*
*  .. Scalar Arguments ..
*/
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PYSWAP", "X", *N, 1, Xi, Xj, Xd, *INCX,  5, &info );
      PB_Cchkvec( ctxt, "PYSWAP", "Y", *N, 1, Yi, Yj, Yd, *INCY, 10, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYSWAP", info ); return; }
#endif
/*
...
*/
      type = PB_Cytypeset(); size = type->size;
/*
...
*/
            if( XmyprocR == XprocR )
            {
               ...
               if( XnpD > 0 )
               {
                  ...
                  if( XisRow )
                  {
                     Cygesd2d( ctxt, 1, XnpD, Mptr( ((char*) X), Xii, Xjj,
                               Xld, size ), Xld, YprocR, dst );
                     Cygerv2d( ctxt, 1, XnpD, Mptr( ((char*) X), Xii, Xjj,
                               Xld, size ), Xld, YprocR, dst );
                  }
                  else
                  {

                     Cygesd2d( ctxt, XnpD, 1, Mptr( ((char*) X), Xii, Xjj,
                               Xld, size ), Xld, dst, YprocR );
                     Cygerv2d( ctxt, XnpD, 1, Mptr( ((char*) X), Xii, Xjj,
                               Xld, size ), Xld, dst, YprocR );
                  }
               }
            }
            if( YmyprocR == YprocR )
            {
               ...
               if( YnpD > 0 )
               {
                  ...
                  if( YisRow )
                  {
                     Cygesd2d( ctxt, 1, YnpD, Mptr( ((char*) Y), Yii, Yjj,
                               Yld, size ), Yld, XprocR, dst );
                     Cygerv2d( ctxt, 1, YnpD, Mptr( ((char*) Y), Yii, Yjj,
                               Yld, size ), Yld, XprocR, dst );
                  }
                  else
                  {
                     Cygesd2d( ctxt, YnpD, 1, Mptr( ((char*) Y), Yii, Yjj,
                               Yld, size ), Yld, dst, XprocR );
                     Cygerv2d( ctxt, YnpD, 1, Mptr( ((char*) Y), Yii, Yjj,
                               Yld, size ), Yld, dst, XprocR );
                  }
               }
            }
...
*/
               ...
               if( XnpD > 0 )
               {
                  ...
                  {
                     yswap_( &XnpD,  Mptr( ((char *) X), Xii, Xjj, Xld,
                  ...
                  {
                     if( XisRow )
                        Cygesd2d( ctxt, 1, XnpD, Mptr( ((char *) X), Xii,
                                  Xjj, Xld, size ), Xld, rdst, cdst );
                     else
                        Cygesd2d( ctxt, XnpD, 1, Mptr( ((char *) X), Xii,
                                  Xjj, Xld, size ), Xld, rdst, cdst );
                  }
               }
            }
...
*/
               ...
               if( YnpD > 0 )
               {
                  ...
                  if( ( myrow != rsrc ) || ( mycol != csrc ) )
                  {
                     ...
                     if( XisRow )
                        Cygerv2d( ctxt, 1, YnpD, buf,    1, rsrc, csrc );
                     else
                        Cygerv2d( ctxt, YnpD, 1, buf, YnpD, rsrc, csrc );
                     if( YisRow )
                        Cygesd2d( ctxt, 1, YnpD, Mptr( ((char *) Y), Yii,
                                  Yjj, Yld, size ), Yld, rsrc, csrc );
                     else
                        Cygesd2d( ctxt, YnpD, 1, Mptr( ((char *) Y), Yii,
                                  Yjj, Yld, size ), Yld, rsrc, csrc );
                     ycopy_( &YnpD, buf, &ione, Mptr( ((char *) Y), Yii,
                             Yjj, Yld, size ), &Ylinc );
                     if( buf ) free( buf );
                  }
               }
...
*/
               if( XnpD > 0 )
               {
                  if( ( myrow != rdst ) || ( mycol != cdst ) )
                  {
                     ...
                     if( YisRow )
                        Cygerv2d( ctxt, 1, XnpD, buf,    1, rdst, cdst );
                     else
                        Cygerv2d( ctxt, XnpD, 1, buf, XnpD, rdst, cdst );
                     ycopy_( &XnpD, buf, &ione, Mptr( ((char *) X), Xii,
                             Xjj, Xld, size ), &Xlinc );
                     if( buf ) free( buf );
                  }
               }
...
*/
                        if( ( XmyprocR == XprocR ) && ( XmyprocD == Xroc ) )
                        {
                           ...
                           Cygesd2d( ctxt, Xm, Xn, buf, Xm, rdst, cdst );
                           Cygerv2d( ctxt, Ym, Yn, buf, Ym, rdst, cdst );
                        ...
                        if( ( YmyprocR == YprocR ) && ( YmyprocD == Yroc  ) )
                        {
                           ...
                           PB_CVMpack( type, &VM, COLUMN, &Yscope, PACKING,
                                       ...
                                       Ym );
                           Cygesd2d( ctxt, Ym, Yn, buf, Ym, rsrc, csrc );
                           Cygerv2d( ctxt, Xm, Xn, buf, Xm, rsrc, csrc );
...
*/
         ...
         if( XnpD > 0 )
         {
            if( XisRow )
            {
               ...
               if( XmyprocR == XprocR )
                  Cygebs2d( ctxt, COLUMN, top, 1, XnpD, Mptr( ((char *) X),
                            Xii, Xjj, Xld, size ), Xld );
               else
                  Cygebr2d( ctxt, COLUMN, top, 1, XnpD, Mptr( ((char *) X),
            ...
            else
            {
               ...
               if( XmyprocR == XprocR )
                  Cygebs2d( ctxt, ROW,    top, XnpD, 1, Mptr( ((char *) X),
                            Xii, Xjj, Xld, size ), Xld );
               else
                  Cygebr2d( ctxt, ROW,    top, XnpD, 1, Mptr( ((char *) X),
...
*/
         ...
         if( YnpD > 0 )
         {
            if( YisRow )
            {
               top = PB_Ctop( &ctxt, BCAST, COLUMN, TOP_GET );
               if( YmyprocR == YprocR )
                  Cygebs2d( ctxt, COLUMN, top, 1, YnpD, Mptr( ((char *) Y),
                            Yii, Yjj, Yld, size ), Yld );
               else
                  Cygebr2d( ctxt, COLUMN, top, 1, YnpD, Mptr( ((char *) Y),
            ...
            else
            {
               top = PB_Ctop( &ctxt, BCAST, ROW,    TOP_GET );
               if( YmyprocR == YprocR )
                  Cygebs2d( ctxt, ROW,    top, YnpD, 1, Mptr( ((char *) Y),
                            Yii, Yjj, Yld, size ), Yld );
               else
                  Cygebr2d( ctxt, ROW,    top, YnpD, 1, Mptr( ((char *) Y),
...
*/
      PB_CpswapND( PB_Cytypeset(), *N, ((char *) X), Xi, Xj, Xd, *INCX,
...
*/
      PB_CpswapND( PB_Cytypeset(), *N, ((char *) Y), Yi, Yj, Yd, *INCY,
...
*/
      PB_CpswapNN( PB_Cytypeset(), *N, ((char *) X), Xi, Xj, Xd, *INCX,
                   ((char *) Y), Yi, Yj, Yd, *INCY );
   }
/*
*  End of PYSWAP
