...
*  =======
*
*  PB_Ctzsymv  performs the matrix-vector  operation
*
*     y := alpha * A * x + y,
*
...
*/
   ...
   if( Mupcase( UPLO[0] ) == CLOWER )
   {
      ...
      if( ( n1 = MIN( mn, N ) ) > 0 )
      {
         gemv( C2F_CHAR( NOTRAN ), &M, &n1, ALPHA, A, &LDA, XR, &LDXR, one, YC,
               &ione );
         gemv( C2F_CHAR( TRAN   ), &M, &n1, ALPHA, A, &LDA, XC, &ione, one, YR,
               &LDYR );
      }
      n1 = M - IOFFD;
      if( ( n1 = MIN( n1, N ) - mn ) > 0 )
      {
         i1 = ( j1 = mn ) + IOFFD;
         TYPE->Fsymv( C2F_CHAR( UPLO ), &n1, ALPHA, Mptr( A, i1, j1, LDA,
         ...
         if( ( m1 = M - mn - n1 - IOFFD ) > 0 )
         {
            ...
            gemv( C2F_CHAR( NOTRAN ), &m1, &n1, ALPHA, Mptr( A, i1, j1, LDA,
                  ...
                  Mptr( YC, i1, 0, LDYC, size ), &ione );
            gemv( C2F_CHAR( TRAN   ), &m1, &n1, ALPHA, Mptr( A, i1, j1, LDA,
                  size ), &LDA, Mptr( XC, i1, 0, LDXC, size ), &ione, one,
   ...
   else if( Mupcase( UPLO[0] ) == CUPPER )
   {
      ...
      if( ( n1 = mn - MAX( 0, -IOFFD ) ) > 0 )
      {
         ...
         if( ( m1 = MAX( 0, IOFFD ) ) > 0 )
         {
            gemv( C2F_CHAR( NOTRAN ), &m1, &n1, ALPHA, A, &LDA, XR, &LDXR, one,
                  YC, &ione );
            gemv( C2F_CHAR( TRAN   ), &m1, &n1, ALPHA, A, &LDA, XC, &ione, one,
                  YR, &LDYR );
         }
         TYPE->Fsymv( C2F_CHAR( UPLO ), &n1, ALPHA, Mptr( A, m1, j1, LDA,
      ...
      if( ( n1 = N - MAX( 0, mn ) ) > 0 )
      {
         ...
         gemv( C2F_CHAR( TRAN   ), &M, &n1, ALPHA, Mptr( A, 0, j1, LDA, size ),
   ...
   else
   {
      ...
      gemv( C2F_CHAR( NOTRAN ), &M, &N, ALPHA, A, &LDA, XR, &LDXR, one, YC,
            &ione );
      gemv( C2F_CHAR( TRAN   ), &M, &N, ALPHA, A, &LDA, XC, &ione, one, YR,
            &LDYR );
   }
/*
*  End of PB_Ctzsymv
