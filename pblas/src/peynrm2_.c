...
#ifdef __STDC__
void peynrm2_( Int * N, EREAL * NORM2,
               EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX )
#else
void peynrm2_( N, NORM2, X, IX, JX, DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
   Int            * INCX, * IX, * JX, * N;
...
*  =======
*
*  PEYNRM2  computes the 2-norm of a subvector sub( X ),
*
*  where
*
...
*/
   ...
   EREAL         Xtmp, scale, ssq, temp1, temp2;
   PBTYP_T        * type;
/*
*  .. Local Arrays ..
*/
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PEYNRM2", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PEYNRM2", info ); return; }
#endif
/*
*  Initialize NORM2
*/
...
l_10:
            ...
            else
            {
               ...
               if( mycol < src )
               {
                  Cegerv2d( ctxt, 2, 1, ((char*)&work[2]), 2, myrow, src );
                  if( work[0] >= work[2] )
                  {
                     if( work[0] != ZERO )
                     {
...
*/
            ...
            if( mycol == 0 )
            {
               Cegebs2d( ctxt, ROW, &top, 2, 1, ((char*)work), 2 );
            }
            else
            {
               Cegebr2d( ctxt, ROW, &top, 2, 1, ((char*)work), 2,
                         myrow, 0 );
            }
/*
*  Compute NORM2 redundantly NORM2  = WORK( 1 ) * SQRT( WORK( 2 ) )
*/
            easqrtb_( &work[0], &work[1], NORM2 );
         }
         else
         {
/*
...
l_30:
            ...
            else
            {
               ...
               if( myrow < src )
               {
                  Cegerv2d( ctxt, 2, 1, ((char*)&work[2]), 2, src, mycol );
                  if( work[0] >= work[2] )
                  {
                     if( work[0] != ZERO )
                     {
...
*/
            ...
            if( myrow == 0 )
            {
               Cegebs2d( ctxt, COLUMN, &top, 2, 1, ((char*)work), 2 );
            }
            else
            {
               Cegebr2d( ctxt, COLUMN, &top, 2, 1, ((char*)work), 2,
                         0, mycol );
            }
/*
