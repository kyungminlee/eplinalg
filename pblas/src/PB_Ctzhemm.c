...
*  =======
*
*  PB_Ctzhemm  performs the matrix-matrix  operation
*
*     C := alpha * A * B + C,
*
*     or
...
*/
   ...
   if( Mupcase( SIDE[0] ) == CLEFT )
   {
      if( Mupcase( UPLO[0] ) == CLOWER )
      {
         ...
         if( ( n1 = MIN( mn, N ) ) > 0 )
         {
            ...
            gemm( C2F_CHAR( COTRAN ), C2F_CHAR( NOTRAN ), &K, &n1, &M, Calph,
         ...
         {
            i1 = ( j1 = mn ) + IOFFD;
            TYPE->Fhemm( C2F_CHAR( SIDE ), C2F_CHAR( UPLO ), &n1, &K, ALPHA,
            ...
            if( ( m1 = M - mn - n1 - IOFFD ) > 0 )
            {
               ...
               gemm( C2F_CHAR( COTRAN ), C2F_CHAR( NOTRAN ), &K, &n1, &m1,
                     Calph, Mptr( BC, i1, 0, LDBC, size ), &LDBC, Mptr( A, i1,
                     j1, LDA, size ), &LDA, one, Mptr( CR, 0, j1, LDCR, size ),
                     &LDCR );
            }
         }
      }
         ...
         if( ( n1 = mn - MAX( 0, -IOFFD ) ) > 0 )
         {
            ...
            if( ( m1 = MAX( 0, IOFFD ) ) > 0 )
            {
               gemm( C2F_CHAR( NOTRAN ), C2F_CHAR( TRAN ), &m1, &K, &n1, ALPHA,
                     A, &LDA, BR, &LDBR, one, CC, &LDCC );
               gemm( C2F_CHAR( COTRAN ), C2F_CHAR( NOTRAN ), &K, &n1, &m1,
                     Calph, BC, &LDBC, A, &LDA, one, CR, &LDCR );
            }
            TYPE->Fhemm( C2F_CHAR( SIDE ), C2F_CHAR( UPLO ), &n1, &K, ALPHA,
                         Mptr( A, m1, j1, LDA, size ), &LDA, Mptr( BC, m1, 0,
                         LDBC, size ), &LDBC, one, Mptr( CC, m1, 0, LDCC,
                         size ), &LDCC );
         }
         if( ( n1 = N - MAX( 0, mn ) ) > 0 )
         {
            ...
            gemm( C2F_CHAR( COTRAN ), C2F_CHAR( NOTRAN ), &K, &n1, &M, Calph,
                  BC, &LDBC, Mptr( A, 0, j1, LDA, size ), &LDA, one, Mptr( CR,
                  0, j1, LDCR, size ), &LDCR );
         }
      }
      else
      {
         ...
         gemm( C2F_CHAR( NOTRAN ), C2F_CHAR( TRAN   ), &M, &K, &N, ALPHA, A,
               &LDA, BR, &LDBR, one, CC, &LDCC );
         gemm( C2F_CHAR( COTRAN ), C2F_CHAR( NOTRAN ), &K, &N, &M, Calph, BC,
   ...
   else
   {
      if( Mupcase( UPLO[0] ) == CLOWER )
      {
         ...
         if( ( n1 = MIN( mn, N ) ) > 0 )
         {
            gemm( C2F_CHAR( NOTRAN ), C2F_CHAR( COTRAN ), &M, &K, &n1, Calph,
                  A, &LDA, BR, &LDBR, one, CC, &LDCC );
            gemm( C2F_CHAR( TRAN   ), C2F_CHAR( NOTRAN ), &K, &n1, &M, ALPHA,
                  BC, &LDBC, A, &LDA, one, CR, &LDCR );
         }
         n1 = M - IOFFD;
         if( ( n1 = MIN( n1, N ) - mn ) > 0 )
         {
            i1 = ( j1 = mn ) + IOFFD;
            TYPE->Fhemm( C2F_CHAR( SIDE ), C2F_CHAR( UPLO ), &K, &n1, ALPHA,
                         Mptr( A, i1, j1, LDA, size ), &LDA, Mptr( BR, 0, j1,
                         LDBR, size ), &LDBR, one, Mptr( CR, 0, j1, LDCR,
                         size ), &LDCR );
            if( ( m1 = M - mn - n1 - IOFFD ) > 0 )
            {
               i1 += n1;
               gemm( C2F_CHAR( NOTRAN ), C2F_CHAR( COTRAN ), &m1, &K, &n1,
                     Calph, Mptr( A, i1, j1, LDA, size ), &LDA, Mptr( BR, 0, j1,
                     LDBR, size ), &LDBR, one, Mptr( CC, i1, 0, LDCC, size ),
                     &LDCC );
      ...
      else if( Mupcase( UPLO[0] ) == CUPPER )
      {
         ...
         if( ( n1 = mn - MAX( 0, -IOFFD ) ) > 0 )
         {
            ...
            {
               gemm( C2F_CHAR( NOTRAN ), C2F_CHAR( COTRAN ), &m1, &K, &n1,
                     Calph, A, &LDA, BR, &LDBR, one, CC, &LDCC );
      ...
      {
         one = TYPE->one; gemm = TYPE->Fgemm;
         gemm( C2F_CHAR( NOTRAN ), C2F_CHAR( COTRAN ), &M, &K, &N, Calph, A,
               &LDA, BR, &LDBR, one, CC, &LDCC );
