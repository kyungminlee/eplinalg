...
#ifdef __STDC__
void petrsv_( F_CHAR_T UPLO, F_CHAR_T TRANS, F_CHAR_T DIAG, Int * N,
              EREAL * A, Int * IA, Int * JA, Int * DESCA,
              EREAL * X, Int * IX, Int * JX, Int * DESCX,
              Int * INCX )
#else
void petrsv_( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA, X, IX, JX,
              DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PETRSV  solves one of the systems of equations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PETRSV", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PETRSV", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      else if( ( !nounit ) && ( DiagA != CUNIT ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PETRSV", "Illegal DIAG = %c\n", DiagA );
         info = -3;
      }
      PB_Cchkmat( ctxt, "PETRSV", "A", *N, 4, *N, 4, Ai, Aj, Ad,  8, &info );
      PB_Cchkvec( ctxt, "PETRSV", "X", *N, 4, Xi, Xj, Xd, *INCX, 12, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PETRSV", info ); return; }
#endif
/*
            ...
            if( Akp > 0 )
            {
               ...
               if( XACsum )
               {
                  ...
                  if( ktmp > 0 )
                  {
                     ...
                     Cegsum2d( ctxt, ROW, &ctop, ktmp, 1, Mptr( XAC, Akp,
                               0, XACld, size ), XACld, myrow, Asrc );
                     if( mycol != Asrc )
                        eset_( &ktmp, zero, Mptr( XAC, Akp, 0, XACld,
                               size ), &ione );
                  }
                  if( Akp > 0 && Anq0 > 0 )
                     egemv_( TRANS, &Akp, &Anq0, negone,
...
*/
         if( XACsum && ( Anp > 0 ) )
         {
            Cegsum2d( ctxt, ROW, &ctop, Anp, 1, XAC, XACld, myrow,
                      XACd[CSRC_] );
         }
/*
...
*/
         if( XARsum && ( Anq > 0 ) )
         {
            Cegsum2d( ctxt, COLUMN, &ctop, 1, Anq, XAR, XARld, XARd[RSRC_],
                      mycol );
         }
/*
