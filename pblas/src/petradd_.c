...
#ifdef __STDC__
void petradd_( F_CHAR_T UPLO, F_CHAR_T TRANS, Int * M, Int * N,
               EREAL * ALPHA,
               EREAL * A, Int * IA, Int * JA, Int * DESCA,
               EREAL * BETA,
               EREAL * C, Int * IC, Int * JC, Int * DESCC )
#else
void petradd_( UPLO, TRANS, M, N, ALPHA, A, IA, JA, DESCA, BETA,
               C, IC, JC, DESCC )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PETRADD  adds a trapezoidal matrix to another
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 901 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PETRADD", "Illegal UPLO = %c\n", UploC );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PETRADD", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      if( notran )
         PB_Cchkmat( ctxt, "PETRADD", "A", *M, 3, *N, 4, Ai, Aj, Ad,  9,
                     &info );
      else
         PB_Cchkmat( ctxt, "PETRADD", "A", *N, 4, *M, 3, Ai, Aj, Ad,  9,
                     &info );
      PB_Cchkmat(    ctxt, "PETRADD", "C", *M, 3, *N, 4, Ci, Cj, Cd, 14,
                     &info );
   }
   if( info ) { PB_Cabort( ctxt, "PETRADD", info ); return; }
#endif
/*
...
*/
   if( ALPHA[REAL_PART] == ZERO )
   {
      ...
      {
         PB_Cplapad( PB_Cetypeset(), &UploC, NOCONJG, *M, *N,
                     ((char *)BETA), ((char *)BETA), ((char *) C), Ci, Cj, Cd );
      }
      else
      {
         PB_Cplascal( PB_Cetypeset(), &UploC, NOCONJG, *M, *N,
...
*/
   ...
   PB_Cptradd( PB_Cetypeset(), &DirAC, &UploC, ( notran ? NOTRAN :
