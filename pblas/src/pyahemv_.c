...
#ifdef __STDC__
void pyahemv_( F_CHAR_T UPLO, Int * N, EREAL * ALPHA,
               EREAL * A, Int * IA, Int * JA, Int * DESCA,
               EREAL * X, Int * IX, Int * JX, Int * DESCX, Int * INCX,
               EREAL * BETA,
               EREAL * Y, Int * IY, Int * JY, Int * DESCY, Int * INCY )
#else
void pyahemv_( UPLO, N, ALPHA, A, IA, JA, DESCA, X, IX, JX, DESCX,
...
*  =======
*
*  PYAHEMV  performs the matrix-vector operation
*
...
*
*  Alpha and beta are real scalars, sub( Y ) is a n element real subvec-
*  tor,  sub( X )  is  an n element subvector  and sub( A ) is an n by n
*  Hermitian submatrix.
*
*  Notes
*  =====
...
*          Hermitian  submatrix  sub( A ), and  the local entries of the
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      PB_Cchkmat( ctxt, "PYAHEMV", "A", *N, 2, *N, 2, Ai, Aj, Ad,  7, &info );
      PB_Cchkvec( ctxt, "PYAHEMV", "X", *N, 2, Xi, Xj, Xd, *INCX, 11, &info );
      PB_Cchkvec( ctxt, "PYAHEMV", "Y", *N, 2, Yi, Yj, Yd, *INCY, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYAHEMV", info ); return; }
#endif
/*
...
*/
   ...
   utyp = PB_Cetypeset(); usiz = type->usiz;
/*
...
*/
            ...
            {
               Yld = Yd[LLD_];
               eascal_( &Ynq, ((char *) BETA), Mptr( ((char *) Y), Yii,
                        Yjj, Yld, usiz ), &Yld );
            }
         }
      }
...
*/
            ...
            if( Ynp > 0 )
            {
               eascal_( &Ynp, ((char *) BETA), Mptr( ((char *) Y), Yii,
...
*/
   if( ( YisRow = ( *INCY == Yd[M_] ) ) != 0 )
   {
      PB_CInOutV( utyp, ROW,    *N, *N, Ad0, 1, ((char *)BETA), ((char *) Y),
                  ...
                  &YRsum, &YRpbY );
      PB_COutV( utyp, COLUMN, INIT, *N, *N, Ad0, 1, &YC, YCd, &YCfr, &YCsum );
   }
   else
   {
      PB_CInOutV( utyp, COLUMN, *N, *N, Ad0, 1, ((char *)BETA), ((char *) Y),
                  ...
                  &YCsum, &YCpbY );
      PB_COutV( utyp, ROW,    INIT, *N, *N, Ad0, 1, &YR, YRd, &YRfr, &YRsum );
   }
/*
...
*/
               if( Anq > 0 )
                  eascal_( &Anq, ((char *) tbeta), YR, &YRld );
            }
         }
      }
      else
...
*/
               if( Amp > 0 )
                  eascal_( &Amp, ((char *) tbeta), YC, &ione );
            }
         }
      }
/*
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               yagemv_( C2F_CHAR( NOTRAN ), &Akp, &Anq0, ((char *)ALPHA),
                        Mptr( Aptr, 0, Akq, Ald, size ), &Ald, Mptr( XR, 0, Akq,
                        XRld, size ), &XRld, one, YC, &ione );
               yagemv_( C2F_CHAR( COTRAN ), &Akp, &Anq0, ((char *)ALPHA),
            ...
            PB_Cpsym( type, utyp, LEFT, UPPER, kb, 1, ((char *) ALPHA), Aptr, k,
                      ...
                      Akq, XRld, size ), XRld, Mptr( YC, Akp, 0, YCld, usiz ),
                      YCld, Mptr( YR, 0, Akq, YRld, usiz ), YRld, PB_Ctzahemv );
         }
      }
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            Akp = PB_Cnumroc( k, 0, Aimb1, Amb, myrow, Arow, nprow );
            Akq = PB_Cnumroc( k, 0, Ainb1, Anb, mycol, Acol, npcol );
            PB_Cpsym( type, utyp, LEFT, LOWER, kb, 1, ((char *) ALPHA), Aptr, k,
                      k, Ad0, Mptr( XC, Akp, 0, XCld, size ), XCld, Mptr( XR, 0,
                      Akq, XRld, size ), XRld, Mptr( YC, Akp, 0, YCld, usiz ),
                      YCld, Mptr( YR, 0, Akq, YRld, usiz ), YRld, PB_Ctzahemv );
            ...
            if( Amp0 > 0 && Anq0 > 0 )
            {
               yagemv_( C2F_CHAR( NOTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
                        Mptr( Aptr, Akp, Akq, Ald, size ), &Ald, Mptr( XR, 0,
                        Akq, XRld, size ), &XRld, one, Mptr( YC, Akp, 0, YCld,
                        usiz ), &ione );
               yagemv_( C2F_CHAR( COTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
...
*/
      if( YCsum )
      {
         ...
         if( Amp > 0 )
         {
            ...
            Cegsum2d( ctxt, ROW, &top, Amp, 1, YC, YCd[LLD_], myrow, 0 );
         }
      }
/*
...
*/
      if( YRsum && ( Anq > 0 ) )
      {
         ...
         Cegsum2d( ctxt, COLUMN, &top, 1, Anq, YR, YRd[LLD_], YRd[RSRC_],
                   mycol );
      }
/*
*  YR := YR + YC
*/
      PB_Cpaxpby( utyp, NOCONJG, *N, 1, one, YC, 0, 0, YCd, COLUMN, one,
                  YR, 0, 0, YRd, ROW );
/*
...
*/
         ...
         PB_Cpaxpby( utyp, NOCONJG, 1, *N, one, YR, 0, 0, YRd, ROW, one,
                     ((char *) Y), Yi, Yj, Yd, ROW );
      }
   }
   else
...
*/
      if( YRsum )
      {
         ...
         {
            top = *PB_Ctop( &ctxt, COMBINE, COLUMN, TOP_GET );
            Cegsum2d( ctxt, COLUMN, &top, 1, Anq, YR, YRd[LLD_], 0,
                      mycol );
         }
      }
/*
...
*/
      ...
      {
         top = *PB_Ctop( &ctxt, COMBINE, ROW, TOP_GET );
         Cegsum2d( ctxt, ROW, &top, Amp, 1, YC, YCd[LLD_], myrow,
                   YCd[CSRC_] );
      }
/*
*  YC := YR + YC
