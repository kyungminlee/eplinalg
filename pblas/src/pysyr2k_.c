...
#ifdef __STDC__
void pysyr2k_( F_CHAR_T UPLO, F_CHAR_T TRANS, Int * N, Int * K,
              EREAL * ALPHA,
              EREAL * A, Int * IA, Int * JA, Int * DESCA,
              EREAL * B, Int * IB, Int * JB, Int * DESCB,
              EREAL * BETA,
              EREAL * C, Int * IC, Int * JC, Int * DESCC )
#else
void pysyr2k_( UPLO, TRANS, N, K, ALPHA, A, IA, JA, DESCA,
...
*  =======
*
*  PYSYR2K  performs one of the symmetric rank 2k operations
*
...
*          entry with TRANS = 'T' or 't', K specifies the number of rows
...
*/
   ...
   EREAL         ABCest, ABest, tmp1, tmp2, tmp3, tmp4;
   PBTYP_T        * type;
/*
*  .. Local Arrays ..
*/
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 901 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYSYR2K", "Illegal UPLO = %c\n", UploC );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PYSYR2K", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      if( notran )
      {
         PB_Cchkmat( ctxt, "PYSYR2K", "A", *N, 3, *K, 4, Ai, Aj, Ad,  9,
                     &info );
         PB_Cchkmat( ctxt, "PYSYR2K", "B", *N, 3, *K, 4, Bi, Bj, Bd, 13,
                     &info );
      }
      else
      {
         PB_Cchkmat( ctxt, "PYSYR2K", "A", *K, 4, *N, 3, Ai, Aj, Ad,  9,
                     &info );
         PB_Cchkmat( ctxt, "PYSYR2K", "B", *K, 4, *N, 3, Bi, Bj, Bd, 13,
                     &info );
      }
      PB_Cchkmat(    ctxt, "PYSYR2K", "C", *N, 3, *N, 3, Ci, Cj, Cd, 18,
                     &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYSYR2K", info ); return; }
#endif
/*
...
*/
      ...
      PB_Cpsyr2kAC( type, &DirA, NOCONJG, &UploC, ( notran ? NOTRAN : TRAN ),
...
*/
      ...
      PB_Cpsyr2kA( type, &DirA, NOCONJG, &UploC, ( notran ? NOTRAN : TRAN ), *N,
                   *K, ((char *)ALPHA), ((char *)A), Ai, Aj, Ad, ((char *)B),
                   Bi, Bj, Bd, ((char *)BETA), ((char *)C), Ci, Cj, Cd );
   }
/*
