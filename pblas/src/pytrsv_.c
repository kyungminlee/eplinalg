...
#ifdef __STDC__
void pytrsv_( F_CHAR_T UPLO, F_CHAR_T TRANS, F_CHAR_T DIAG, Int * N,
              EREAL * A, Int * IA, Int * JA, Int * DESCA,
              EREAL * X, Int * IX, Int * JX, Int * DESCX,
              Int * INCX )
#else
void pytrsv_( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA, X, IX, JX,
              DESCX, INCX )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PYTRSV  solves one of the systems of equations
*
*    sub( A )*sub( X ) = b,   or   sub( A )'*sub( X ) = b,   or
*
*    conjg( sub( A )' )*sub( X ) = b,
*
*  where
...
*
*             TRANS = 'C' or 'c'   conjg( sub( A )' ) * sub( X ) = b.
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PYTRSV", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PYTRSV", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      else if( ( !nounit ) && ( DiagA != CUNIT ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PYTRSV", "Illegal DIAG = %c\n", DiagA );
         info = -3;
      }
      PB_Cchkmat( ctxt, "PYTRSV", "A", *N, 4, *N, 4, Ai, Aj, Ad,  8, &info );
      PB_Cchkvec( ctxt, "PYTRSV", "X", *N, 4, Xi, Xj, Xd, *INCX, 12, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYTRSV", info ); return; }
#endif
/*
            ...
            if( Akp > 0 )
            {
               ...
               if( XACsum )
               {
                  ...
                  if( ktmp > 0 )
                  {
                     ...
                     Cygsum2d( ctxt, ROW, &ctop, ktmp, 1, Mptr( XAC, Akp,
                               0, XACld, size ), XACld, myrow, Asrc );
                     if( mycol != Asrc )
                        yset_( &ktmp, zero, Mptr( XAC, Akp, 0, XACld,
                               size ), &ione );
                  }
                  if( Akp > 0 && Anq0 > 0 )
                     ygemv_( TRANS, &Akp, &Anq0, negone,
...
*/
         if( XACsum && ( Anp > 0 ) )
         {
            Cygsum2d( ctxt, ROW, &ctop, Anp, 1, XAC, XACld, myrow,
                      XACd[CSRC_] );
         }
/*
...
*/
         if( XARsum && ( Anq > 0 ) )
         {
            Cygsum2d( ctxt, COLUMN, &ctop, 1, Anq, XAR, XARld, XARd[RSRC_],
                      mycol );
         }
/*
