...
#else
void pydotu_( N, DOT, X, IX, JX, DESCX, INCX, Y, IY, JY, DESCY, INCY )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PYDOTU  forms the dot product of two subvectors,
*
*     DOT := sub( X )**T * sub( Y ),
*
*  where
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PYDOTU", "X", *N, 1, Xi, Xj, Xd, *INCX,  6, &info );
      PB_Cchkvec( ctxt, "PYDOTU", "Y", *N, 1, Yi, Yj, Yd, *INCY, 11, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYDOTU", info ); return; }
#endif
   DOT[REAL_PART] = ZERO;
   DOT[IMAG_PART] = ZERO;
/*
...
*/
      type = PB_Cytypeset(); size = type->size; dot = type->Fvvdotu;
/*
