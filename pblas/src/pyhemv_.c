...
*  =======
*
*  PYHEMV  performs the matrix-vector operation
*
...
*  vectors and sub( A ) is an n by n Hermitian submatrix.
*
*  Notes
*  =====
*
...
*/
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      PB_Cchkmat( ctxt, "PYHEMV", "A", *N, 2, *N, 2, Ai, Aj, Ad,  7, &info );
      PB_Cchkvec( ctxt, "PYHEMV", "X", *N, 2, Xi, Xj, Xd, *INCX, 11, &info );
      PB_Cchkvec( ctxt, "PYHEMV", "Y", *N, 2, Yi, Yj, Yd, *INCY, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYHEMV", info ); return; }
#endif
/*
...
*/
            ...
            if( Ynq > 0 )
            {
               ...
               {
                  yset_( &Ynq, ((char *) BETA), Mptr( ((char *) Y), Yii,
                         Yjj, Yld, type->size ), &Yld );
               }
               else
               {
                  yscal_( &Ynq, ((char *) BETA), Mptr( ((char *) Y), Yii,
                          Yjj, Yld, type->size ), &Yld );
               }
            }
         }
...
*/
            ...
            if( Ynp > 0 )
            {
               if( ( BETA[REAL_PART] == ZERO ) && ( BETA[IMAG_PART] == ZERO ) )
               {
                  yset_( &Ynp, ((char *) BETA), Mptr( ((char *) Y), Yii,
                         Yjj, Yd[LLD_], type->size ), INCY );
               }
               else
               {
                  yscal_( &Ynp, ((char *) BETA), Mptr( ((char *) Y), Yii,
                          Yjj, Yd[LLD_], type->size ), INCY );
               }
            }
         }
...
*/
               if( Anq > 0 )
               {
                  if( ( tbeta[REAL_PART] == ZERO ) &&
                      ( tbeta[IMAG_PART] == ZERO ) )
                  {
                     yset_( &Anq, ((char *) tbeta), YR, &YRld );
                  }
                  else
                  {
                     yscal_( &Anq, ((char *) tbeta), YR, &YRld );
                  }
               }
            }
         }
...
*/
               if( Amp > 0 )
               {
                  if( ( tbeta[REAL_PART] == ZERO ) &&
                      ( tbeta[IMAG_PART] == ZERO ) )
                  {
                     yset_( &Amp, ((char *) tbeta), YC, &ione );
                  }
                  else
                  {
                     yscal_( &Amp, ((char *) tbeta), YC, &ione );
                  }
               }
            }
         }
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               ygemv_( C2F_CHAR( NOTRAN ), &Akp, &Anq0, ((char *)ALPHA),
                       Mptr( Aptr, 0, Akq, Ald, size ), &Ald, Mptr( XR, 0, Akq,
                       XRld, size ), &XRld, one, YC, &ione );
               ygemv_( C2F_CHAR( COTRAN ), &Akp, &Anq0, ((char *)ALPHA),
      ...
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Amp0 > 0 && Anq0 > 0 )
            {
               ygemv_( C2F_CHAR( NOTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
                       Mptr( Aptr, Akp, Akq, Ald, size ), &Ald, Mptr( XR, 0,
                       Akq, XRld, size ), &XRld, one, Mptr( YC, Akp, 0, YCld,
                       size ), &ione );
               ygemv_( C2F_CHAR( COTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
...
*/
      if( YCsum )
      {
         ...
         if( Amp > 0 )
         {
            ...
            Cygsum2d( ctxt, ROW, &top, Amp, 1, YC, YCd[LLD_], myrow, 0 );
         }
      }
/*
...
*/
      if( YRsum && ( Anq > 0 ) )
      {
         ...
         Cygsum2d( ctxt, COLUMN, &top, 1, Anq, YR, YRd[LLD_], YRd[RSRC_],
                   mycol );
      }
...
*/
      if( YRsum )
      {
         ...
         {
            top = *PB_Ctop( &ctxt, COMBINE, COLUMN, TOP_GET );
            Cygsum2d( ctxt, COLUMN, &top, 1, Anq, YR, YRd[LLD_], 0,
                      mycol );
         }
      }
/*
...
*/
      ...
      {
         top = *PB_Ctop( &ctxt, COMBINE, ROW, TOP_GET );
         Cygsum2d( ctxt, ROW, &top, Amp, 1, YC, YCd[LLD_], myrow,
                   YCd[CSRC_] );
      }
/*
*  YC := YR + YC
