...
*  =======
*
*  PESYMV  performs the matrix-vector operation
*
...
*/
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      PB_Cchkmat( ctxt, "PESYMV", "A", *N, 2, *N, 2, Ai, Aj, Ad,  7, &info );
      PB_Cchkvec( ctxt, "PESYMV", "X", *N, 2, Xi, Xj, Xd, *INCX, 11, &info );
      PB_Cchkvec( ctxt, "PESYMV", "Y", *N, 2, Yi, Yj, Yd, *INCY, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PESYMV", info ); return; }
#endif
/*
...
*/
               if( Anq > 0 )
               {
                  ...
                  {
                     eset_( &Anq, ((char *) tbeta), YR, &YRld );
                  }
                  else
                  {
                     escal_( &Anq, ((char *) tbeta), YR, &YRld );
                  }
               }
            }
         }
...
*/
               if( Amp > 0 )
               {
                  if( tbeta[REAL_PART] == ZERO )
                  {
                     eset_( &Amp, ((char *) tbeta), YC, &ione );
                  }
                  else
                  {
                     escal_( &Amp, ((char *) tbeta), YC, &ione );
                  }
               }
            }
         }
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               egemv_( C2F_CHAR( NOTRAN ), &Akp, &Anq0, ((char *)ALPHA),
                       Mptr( Aptr, 0, Akq, Ald, size ), &Ald, Mptr( XR, 0, Akq,
                       XRld, size ), &XRld, one, YC, &ione );
               egemv_( C2F_CHAR( TRAN   ), &Akp, &Anq0, ((char *)ALPHA),
      ...
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Amp0 > 0 && Anq0 > 0 )
            {
               egemv_( C2F_CHAR( NOTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
                       Mptr( Aptr, Akp, Akq, Ald, size ), &Ald, Mptr( XR, 0,
                       Akq, XRld, size ), &XRld, one, Mptr( YC, Akp, 0, YCld,
                       size ), &ione );
               egemv_( C2F_CHAR( TRAN   ), &Amp0, &Anq0, ((char *) ALPHA),
