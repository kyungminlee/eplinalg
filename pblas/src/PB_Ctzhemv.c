...
*  =======
*
*  PB_Ctzhemv  performs the matrix-vector  operation
*
*     y := alpha * A * x + y,
*
*  where alpha is a scalar,  x and y are n element vectors and A is an m
*  by n trapezoidal symmetric or Hermitian matrix.
*
*  Arguments
*  =========
*
...
*/
   ...
   {
      size = TYPE->size; one = TYPE->one; gemv = TYPE->Fgemv;
      mn   = MAX( 0, -IOFFD );
      if( ( n1 = MIN( mn, N ) ) > 0 )
      {
         gemv( C2F_CHAR( NOTRAN ), &M, &n1, ALPHA, A, &LDA, XR, &LDXR, one, YC,
               &ione );
         gemv( C2F_CHAR( COTRAN ), &M, &n1, ALPHA, A, &LDA, XC, &ione, one, YR,
               &LDYR );
      }
      n1 = M - IOFFD;
      if( ( n1 = MIN( n1, N ) - mn ) > 0 )
      {
         i1 = ( j1 = mn ) + IOFFD;
         TYPE->Fhemv( C2F_CHAR( UPLO ), &n1, ALPHA, Mptr( A, i1, j1, LDA,
                      size ), &LDA, Mptr( XC, i1, 0, LDXC, size ), &ione, one,
                      Mptr( YC, i1, 0, LDYC, size ), &ione );
         if( ( m1 = M - mn - n1 - IOFFD ) > 0 )
         {
            i1 += n1;
            gemv( C2F_CHAR( NOTRAN ), &m1, &n1, ALPHA, Mptr( A, i1, j1, LDA,
                  size ), &LDA, Mptr( XR, 0, j1, LDXR, size ), &LDXR, one,
                  Mptr( YC, i1, 0, LDYC, size ), &ione );
            gemv( C2F_CHAR( COTRAN ), &m1, &n1, ALPHA, Mptr( A, i1, j1, LDA,
                  size ), &LDA, Mptr( XC, i1, 0, LDXC, size ), &ione, one,
   ...
   else if( Mupcase( UPLO[0] ) == CUPPER )
   {
      ...
      if( ( n1 = mn - MAX( 0, -IOFFD ) ) > 0 )
      {
         ...
         if( ( m1 = MAX( 0, IOFFD ) ) > 0 )
         {
            gemv( C2F_CHAR( NOTRAN ), &m1, &n1, ALPHA, A, &LDA, XR, &LDXR, one,
                  YC, &ione );
            gemv( C2F_CHAR( COTRAN ), &m1, &n1, ALPHA, A, &LDA, XC, &ione, one,
                  YR, &LDYR );
         }
         TYPE->Fhemv( C2F_CHAR( UPLO ), &n1, ALPHA, Mptr( A, m1, j1, LDA,
                      size ), &LDA, Mptr( XC, m1, 0, LDXC, size ), &ione, one,
                      Mptr( YC, m1, 0, LDYC, size ), &ione );
      }
      if( ( n1 = N - MAX( 0, mn ) ) > 0 )
      {
         j1 = N - n1;
         gemv( C2F_CHAR( NOTRAN ), &M, &n1, ALPHA, Mptr( A, 0, j1, LDA, size ),
               &LDA, Mptr( XR, 0, j1, LDXR, size ), &LDXR, one, YC, &ione );
         gemv( C2F_CHAR( COTRAN ), &M, &n1, ALPHA, Mptr( A, 0, j1, LDA, size ),
               &LDA, XC, &ione, one, Mptr( YR, 0, j1, LDYR, size ), &LDYR );
      }
   }
   else
   {
      one = TYPE->one; gemv = TYPE->Fgemv;
      gemv( C2F_CHAR( NOTRAN ), &M, &N, ALPHA, A, &LDA, XR, &LDXR, one, YC,
            &ione );
      gemv( C2F_CHAR( COTRAN ), &M, &N, ALPHA, A, &LDA, XC, &ione, one, YR,
            &LDYR );
   }
/*
*  End of PB_Ctzhemv
