...
*  =======
*
*  PESYR2  performs the symmetric rank 2 operation
*
*     sub( A ) := alpha*sub( X )*sub( Y )' +
*                 alpha*sub( Y )*sub( X )' + sub( A ) ,
*
*  where
*
...
*  sub( A ) is an n by n symmetric submatrix.
*
*  Notes
*  =====
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PESYR2", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      PB_Cchkvec( ctxt, "PESYR2", "X", *N, 2, Xi, Xj, Xd, *INCX,  7, &info );
      PB_Cchkvec( ctxt, "PESYR2", "Y", *N, 2, Yi, Yj, Yd, *INCY, 12, &info );
      PB_Cchkmat( ctxt, "PESYR2", "A", *N, 2, *N, 2, Ai, Aj, Ad, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PESYR2", info ); return; }
#endif
/*
*  Quick return if possible
*/
   if( (*N == 0) || ( ALPHA[REAL_PART] == ZERO ) ) return;
/*
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               eger_( &Akp, &Anq0, ((char *) ALPHA), XC, &ione,
                      Mptr( YR,   0, Akq, YRld, size ), &YRld,
                      Mptr( Aptr, 0, Akq,  Ald, size ), &Ald );
               eger_( &Akp, &Anq0, ((char *) ALPHA), YC, &ione,
