...
*  =======
*
*  PEASYMV  performs the matrix-vector operation
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      PB_Cchkmat( ctxt, "PEASYMV", "A", *N, 2, *N, 2, Ai, Aj, Ad,  7, &info );
      PB_Cchkvec( ctxt, "PEASYMV", "X", *N, 2, Xi, Xj, Xd, *INCX, 11, &info );
      PB_Cchkvec( ctxt, "PEASYMV", "Y", *N, 2, Yi, Yj, Yd, *INCY, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PEASYMV", info ); return; }
#endif
/*
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               eagemv_( C2F_CHAR( NOTRAN ), &Akp, &Anq0, ((char *)ALPHA),
                        Mptr( Aptr, 0, Akq, Ald, size ), &Ald, Mptr( XR, 0, Akq,
                        XRld, size ), &XRld, one, YC, &ione );
               eagemv_( C2F_CHAR( TRAN   ), &Akp, &Anq0, ((char *)ALPHA),
            ...
            PB_Cpsym( type, utyp, LEFT, UPPER, kb, 1, ((char *) ALPHA), Aptr, k,
                      ...
                      YCld, Mptr( YR, 0, Akq, YRld, usiz ), YRld, PB_Ctzasymv );
         }
      }
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Amp0 > 0 && Anq0 > 0 )
            {
               eagemv_( C2F_CHAR( NOTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
                        Mptr( Aptr, Akp, Akq, Ald, size ), &Ald, Mptr( XR, 0,
                        Akq, XRld, size ), &XRld, one, Mptr( YC, Akp, 0, YCld,
                        usiz ), &ione );
               eagemv_( C2F_CHAR( TRAN   ), &Amp0, &Anq0, ((char *) ALPHA),
