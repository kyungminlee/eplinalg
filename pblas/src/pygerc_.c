...
*  =======
*
*  PYGERC  performs the rank 1 operation
*
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PYGERC", "X", *M, 1, Xi, Xj, Xd, *INCX,  7, &info );
      PB_Cchkvec( ctxt, "PYGERC", "Y", *N, 2, Yi, Yj, Yd, *INCY, 12, &info );
      PB_Cchkmat( ctxt, "PYGERC", "A", *M, 1, *N, 2, Ai, Aj, Ad, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PYGERC", info ); return; }
#endif
/*
...
*/
   ...
   if( ( Amp > 0 ) && ( Anq > 0 ) )
   {
      ygerc_( &Amp, &Anq, ((char *) ALPHA), XA, &ione, YA, &YAd[LLD_],
