...
#if (INTFACE == C_CALL)
void Cegamn2d(Int ConTxt, char *scope, char *top, Int m, Int n, EREAL *A,
              Int lda, Int *rA, Int *cA, Int ldia, Int rdest, Int cdest)
#else
F_VOID_FUNC egamn2d_(Int *ConTxt, F_CHAR scope, F_CHAR top, Int *m, Int *n,
 ...
 *  Purpose
 *  =======
 *  Combine amn operation for EREAL precision rectangular matrices.
 *
 *  Arguments
 *  =========
 *
 ...
 */
{
   ...
   void BI_eMPI_amn(void *, void *, MpiInt *, MPI_Datatype *);
   void BI_eMPI_amn2(void *, void *, MpiInt *, MPI_Datatype *);
/*
 ...
 */
      j = 8;
      if (sizeof(EREAL) > j) j = sizeof(EREAL);
      i = length % j;
      if (i) length += j - i;
      i = 2 * length;
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=MPI_Op_create(BI_eMPI_amn2, i, &BlacComb);
      }
      else
      {
         ierr=MPI_Op_create(BI_eMPI_amn, i, &BlacComb);
 ...
 */
      if (bp != &BI_AuxBuff) BI_evmcopy(Mpval(m), Mpval(n), A, tlda, (EREAL*)bp->Buff);
   }
}
