...
#if (INTFACE == C_CALL)
void Cetrsd2d(Int ConTxt, char *uplo, char *diag, Int m, Int n, EREAL *A,
              Int lda, Int rdest, Int cdest)
#else
F_VOID_FUNC etrsd2d_(Int *ConTxt, F_CHAR uplo, F_CHAR diag, Int *m, Int *n,
 ...
 *  Purpose
 *  =======
 *  Locally-blocking point-to-point trapezoidal EREAL precision send.
 *
 *  Arguments
 *  =========
 *
 ...
 *
 *  A       (input) Ptr to EREAL precision two dimensional array
...
#if (BlacsDebugLvl > 0)
   BI_ArgCheck(Mpval(ConTxt), RT_SD, "ETRSD2D", 'a', tuplo, tdiag, Mpval(m),
