/*
 * Migrator-side override of upstream BLACS ``egsum2d_.c``.
 *
 ...
 * combine op via ``MPI_Op_create(BI_eMPI_sum, ...)`` that calls
 * ``BI_evvsum`` (the same vector-sum kernel used by every other
 ...
 * generated correctly by the BLACS C migrator from ``BI_evvsum.c`` and
 ...
 * ``zgsum2d_`` path. The companion ``BI_eMPI_sum.c`` ships alongside
 * this file via the same ``patches:`` recipe list.
 *
...
#if (INTFACE == C_CALL)
void Cegsum2d(Int ConTxt, char *scope, char *top, Int m, Int n, EREAL *A,
              Int lda, Int rdest, Int cdest)
#else
F_VOID_FUNC egsum2d_(Int *ConTxt, F_CHAR scope, F_CHAR top, Int *m, Int *n,
                     EREAL *A, Int *lda, Int *rdest, Int *cdest)
#endif
/*
 *  -- V1.1 BLACS routine --
 ...
 *  Purpose
 *  =======
 *  Combine sum operation for EREAL precision rectangular matrices.
 *
 *  Arguments
 *  =========
 *
 ...
 *
 *  A       (output) Ptr to EREAL precision two dimensional array
 ...
 */
{
   ...
   void BI_evvsum(Int, char *, char *);
   void BI_eMPI_sum(void *, void *, MpiInt *, MPI_Datatype *);
/*
 ...
 */
   else
   {
      ...
      BI_emvcopy(Mpval(m), Mpval(n), A, tlda, (EREAL*)bp->Buff);
   }
   bp->dtype = bp2->dtype = MPI_LONG_DOUBLE;
   bp->N = bp2->N = N;
   ...
   case ' ':         /* use MPI's reduction by default */
      length = 1;
      ierr=MPI_Op_create(BI_eMPI_sum, length, &BlacComb);
      if (dest != -1)
      {
         ierr=MPI_Reduce(bp->Buff, bp2->Buff, bp->N, bp->dtype, BlacComb,
                       dest, ctxt->scp->comm);
         if (ctxt->scp->Iam == dest)
	    BI_evmcopy(Mpval(m), Mpval(n), A, tlda, (EREAL*)bp2->Buff);
      }
      else
      {
	 ...
	 BI_evmcopy(Mpval(m), Mpval(n), A, tlda, (EREAL*)bp2->Buff);
      ...
      break;
   case 'i':
      BI_MringComb(ctxt, bp, bp2, N, BI_evvsum, dest, 1);
      break;
   case 'd':
      BI_MringComb(ctxt, bp, bp2, N, BI_evvsum, dest, -1);
      break;
   case 's':
      BI_MringComb(ctxt, bp, bp2, N, BI_evvsum, dest, 2);
      break;
   case 'm':
      BI_MringComb(ctxt, bp, bp2, N, BI_evvsum, dest, ctxt->Nr_co);
      break;
   case '1':
   case '2':
   case '3':
   ...
   case '8':
   case '9':
      BI_TreeComb(ctxt, bp, bp2, N, BI_evvsum, dest, ttop-47);
      break;
   case 'f':
      BI_TreeComb(ctxt, bp, bp2, N, BI_evvsum, dest, FULLCON);
      break;
   case 't':
      BI_TreeComb(ctxt, bp, bp2, N, BI_evvsum, dest, ctxt->Nb_co);
      break;
   case 'h':
/*
 ...
 */
      if ( (trdest == -1) && !(ctxt->TopsCohrnt) )
         BI_BeComb(ctxt, bp, bp2, N, BI_evvsum);
      else
         BI_TreeComb(ctxt, bp, bp2, N, BI_evvsum, dest, 2);
      break;
   default :
 ...
 */
   if (bp != &BI_AuxBuff)
   {
      if ( (ctxt->scp->Iam == dest) || (dest == -1) )
         BI_evmcopy(Mpval(m), Mpval(n), A, tlda, (EREAL*)bp->Buff);
      BI_UpdateBuffs(bp);
   }
   else
   {
