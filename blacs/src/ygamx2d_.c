...
#if (INTFACE == C_CALL)
void Cygamx2d(Int ConTxt, char *scope, char *top, Int m, Int n, EREAL *A,
              Int lda, Int *rA, Int *cA, Int ldia, Int rdest, Int cdest)
#else
F_VOID_FUNC ygamx2d_(Int *ConTxt, F_CHAR scope, F_CHAR top, Int *m, Int *n,
                     EREAL *A, Int *lda, Int *rA, Int *cA, Int *ldia,
                     Int *rdest, Int *cdest)
#endif
/*
 *  -- V1.1 BLACS routine --
 ...
 *  Purpose
 *  =======
 *  Combine amx operation for EREAL complex rectangular matrices.
 *
 *  Arguments
 *  =========
 *
 ...
 */
{
   ...
   void BI_yMPI_amx(void *, void *, MpiInt *, MPI_Datatype *);
   void BI_yMPI_amx2(void *, void *, MpiInt *, MPI_Datatype *);
/*
 ...
 */
      j = 8;
      if (sizeof(ECOMPLEX) > j) j = sizeof(ECOMPLEX);
      i = length % j;
      if (i) length += j - i;
      i = 2 * length;
 ...
 */
      ...
      disp[0] = 0;
      disp[1] = idist;
      dtypes[0] = MPI_C_LONG_DOUBLE_COMPLEX;
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=MPI_Op_create(BI_yMPI_amx2, i, &BlacComb);
      }
      else
      {
         ierr=MPI_Op_create(BI_yMPI_amx, i, &BlacComb);
 ...
 */
      if (bp != &BI_AuxBuff) BI_yvmcopy(Mpval(m), Mpval(n), A, tlda, bp->Buff);
   }
}
