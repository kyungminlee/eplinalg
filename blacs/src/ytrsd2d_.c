...
#if (INTFACE == C_CALL)
void Cytrsd2d(Int ConTxt, char *uplo, char *diag, Int m, Int n, EREAL *A,
              Int lda, Int rdest, Int cdest)
#else
F_VOID_FUNC ytrsd2d_(Int *ConTxt, F_CHAR uplo, F_CHAR diag, Int *m, Int *n,
 ...
 *  Purpose
 *  =======
 *  Locally-blocking point-to-point trapezoidal EREAL complex send.
 *
 *  Arguments
 *  =========
 *
 ...
 *
 *  A       (input) Ptr to EREAL complex two dimensional array
 ...
 */
{
...
#if (BlacsDebugLvl > 0)
   BI_ArgCheck(Mpval(ConTxt), RT_SD, "YTRSD2D", 'a', tuplo, tdiag, Mpval(m),
   ...
   MatTyp = BI_GetMpiTrType(ctxt, tuplo, tdiag, Mpval(m), Mpval(n), tlda,
                            MPI_C_LONG_DOUBLE_COMPLEX, &BI_AuxBuff.N);
#ifdef SndIsLocBlk
