...
#if (INTFACE == C_CALL)
void Cygsum2d(Int ConTxt, char *scope, char *top, Int m, Int n, EREAL *A,
              Int lda, Int rdest, Int cdest)
#else
F_VOID_FUNC ygsum2d_(Int *ConTxt, F_CHAR scope, F_CHAR top, Int *m, Int *n,
 ...
 *  Purpose
 *  =======
 *  Combine sum operation for EREAL complex rectangular matrices.
 *
 *  Arguments
 *  =========
 *
 ...
 *
 *  A       (output) Ptr to EREAL complex two dimensional array
 ...
 */
{
   ...
   void BI_yMPI_sum(void *, void *, MpiInt *, MPI_Datatype *);
/*
 ...
 */
   else
   {
      ...
      BI_ymvcopy(Mpval(m), Mpval(n), A, tlda, bp->Buff);
   }
   bp->dtype = bp2->dtype = MPI_C_LONG_DOUBLE_COMPLEX;
   bp->N = bp2->N = N;
   ...
   case ' ':         /* use MPI's reduction by default */
      length = 1;
      ierr=MPI_Op_create(BI_yMPI_sum, length, &BlacComb);
      if (dest != -1)
      {
         ierr=MPI_Reduce(bp->Buff, bp2->Buff, bp->N, bp->dtype, BlacComb,
                       dest, ctxt->scp->comm);
         if (ctxt->scp->Iam == dest)
	    BI_yvmcopy(Mpval(m), Mpval(n), A, tlda, bp2->Buff);
      }
      else
      {
	 ...
	 BI_yvmcopy(Mpval(m), Mpval(n), A, tlda, bp2->Buff);
      ...
      break;
   case 'i':
      BI_MringComb(ctxt, bp, bp2, N, BI_yvvsum, dest, 1);
      break;
   case 'd':
      BI_MringComb(ctxt, bp, bp2, N, BI_yvvsum, dest, -1);
      break;
   case 's':
      BI_MringComb(ctxt, bp, bp2, N, BI_yvvsum, dest, 2);
      break;
   case 'm':
      BI_MringComb(ctxt, bp, bp2, N, BI_yvvsum, dest, ctxt->Nr_co);
      break;
   case '1':
   case '2':
   case '3':
   ...
   case '8':
   case '9':
      BI_TreeComb(ctxt, bp, bp2, N, BI_yvvsum, dest, ttop-47);
      break;
   case 'f':
      BI_TreeComb(ctxt, bp, bp2, N, BI_yvvsum, dest, FULLCON);
      break;
   case 't':
      BI_TreeComb(ctxt, bp, bp2, N, BI_yvvsum, dest, ctxt->Nb_co);
      break;
   case 'h':
/*
 ...
 */
      if ( (trdest == -1) && !(ctxt->TopsCohrnt) )
         BI_BeComb(ctxt, bp, bp2, N, BI_yvvsum);
      else
         BI_TreeComb(ctxt, bp, bp2, N, BI_yvvsum, dest, 2);
      break;
   default :
 ...
 */
   if (bp != &BI_AuxBuff)
   {
      if ( (ctxt->scp->Iam == dest) || (dest == -1) )
         BI_yvmcopy(Mpval(m), Mpval(n), A, tlda, bp->Buff);
      BI_UpdateBuffs(bp);
   }
   else
   {
