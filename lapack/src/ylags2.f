...
*
*> \htmlonly
*> Download YLAGS2 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ylags2.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ylags2.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ylags2.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YLAGS2( UPPER, A1, A2, A3, B1, B2, B3, CSU, SNU, CSV,
*                          SNV, CSQ, SNQ )
*
*       .. Scalar Arguments ..
*       LOGICAL            UPPER
*       REAL(KIND=10)   A1, A3, B1, B3, CSQ, CSU, CSV
...
*
*  =====================================================================
      SUBROUTINE YLAGS2( UPPER, A1, A2, A3, B1, B2, B3, CSU, SNU,       
     +             CSV,                    SNV, CSQ, SNQ )
*
...
*     ..
*     .. Local Scalars ..
      REAL(KIND=10)   A, AUA11, AUA12, AUA21, AUA22, AVB12, AVB11,      
     +              AVB21, AVB22, CSL, CSR, D, FB, FC, S1, S2,          
     +          SNL, SNR, UA11R, UA22R, VB11R, VB22R
      COMPLEX(KIND=10)         B, C, D1, R, T, UA11, UA12, UA21, UA22, 
     +VB11,                    VB12, VB21, VB22
*     ..
*     .. External Subroutines ..
      EXTERNAL           ELASV2, YLARTG
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS
*     ..
...
*     .. Statement Function definitions ..
      ABS1( T ) = ABS( REAL( T , KIND=10) ) + ABS( AIMAG( T ) )
*     ..
...
*
         CALL ELASV2( A, FB, D, S1, S2, SNR, CSR, SNL, CSL )
*
...
*
            IF( ( ABS( UA11R )+ABS1( UA12 ) ).EQ.ZERO ) THEN
               CALL YLARTG( -CMPLX( VB11R , KIND=10), CONJG( VB12 ), 
     +CSQ,                       SNQ,                       R )
            ELSE IF( ( ABS( VB11R )+ABS1( VB12 ) ).EQ.ZERO ) THEN
               CALL YLARTG( -CMPLX( UA11R , KIND=10), CONJG( UA12 ), 
     +CSQ,                       SNQ,                       R )
...
*
            IF( ( ABS1( UA21 )+ABS1( UA22 ) ).EQ.ZERO ) THEN
               CALL YLARTG( -CONJG( VB21 ), CONJG( VB22 ), CSQ,         
     +              SNQ,                       R )
            ELSE IF( ( ABS1( VB21 )+ABS( VB22 ) ).EQ.ZERO ) THEN
               CALL YLARTG( -CONJG( UA21 ), CONJG( UA22 ), CSQ,         
     +              SNQ,                       R )
...
*
         CALL ELASV2( A, FC, D, S1, S2, SNR, CSR, SNL, CSL )
*
...
*
            IF( ( ABS1( UA21 )+ABS( UA22R ) ).EQ.ZERO ) THEN
               CALL YLARTG( CMPLX( VB22R , KIND=10), VB21, CSQ, SNQ, R )
            ELSE IF( ( ABS1( VB21 )+ABS( VB22R ) ).EQ.ZERO ) THEN
               CALL YLARTG( CMPLX( UA22R , KIND=10), UA21, CSQ, SNQ, R )
...
*
            IF( ( ABS1( UA11 )+ABS1( UA12 ) ).EQ.ZERO ) THEN
               CALL YLARTG( VB12, VB11, CSQ, SNQ, R )
            ELSE IF( ( ABS1( VB11 )+ABS1( VB12 ) ).EQ.ZERO ) THEN
               CALL YLARTG( UA12, UA11, CSQ, SNQ, R )
