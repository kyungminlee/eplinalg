...
*
*> \htmlonly
*> Download YGETSQRHRT + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ygetsqrhrt.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ygetsqrhrt.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ygetsqrhrt.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YGETSQRHRT( M, N, MB1, NB1, NB2, A, LDA, T, LDT, WORK,
*      $                       LWORK, INFO )
*       IMPLICIT NONE
*
*       .. Scalar Arguments ..
...
*       ..
*       .. Array Arguments ..
*       COMPLEX(KIND=10)        A( LDA, * ), T( LDT, * ), WORK( * )
*       ..
*
*
*> \par Purpose:
...
*> \verbatim
*>
*> YGETSQRHRT computes a NB2-sized column blocked QR-factorization
...
*>    A = Q_tsqr * R_tsqr = Q * R.
*>
*> The output Q and R factors are stored in the same format as in YGEQRT
*> (Q is in blocked compact WY-representation). See the documentation
*> of YGEQRT for more details on the format.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*>
*  =====================================================================
      SUBROUTINE YGETSQRHRT( M, N, MB1, NB1, NB2, A, LDA, T, LDT,       
     +                 WORK,                        LWORK, INFO )
      IMPLICIT NONE
*
...
*     .. External Subroutines ..
      EXTERNAL           YCOPY, YLATSQR, YUNGTSQR_ROW,                  
     +  YUNHR_COL,                    XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*        a) Matrix T and WORK for YLATSQR;
*        b) N-by-N upper-triangular factor R_tsqr;
*        c) Matrix T and array WORK for YUNGTSQR_ROW;
*        d) Diagonal D for YUNHR_COL.
*
...
*
            NB1LOCAL = MIN( NB1, N )
*
            NUM_ALL_ROW_BLOCKS = MAX( 1,                    CEILING( 
     +REAL( M - N , KIND=10) / REAL( MB1 - N , KIND=10) ) )
*
...
*
            LW1 = NB1LOCAL * N
*
*           Length of YUNGTSQR_ROW work array.
*
            LW2 = NB1LOCAL * MAX( NB1LOCAL, ( N - NB1LOCAL ) )
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'YGETSQRHRT', -INFO )
         RETURN
      ELSE IF ( LQUERY ) THEN
         WORK( 1 ) = CMPLX( LWORKOPT , KIND=10)
         RETURN
      END IF
*
...
*
      CALL YLATSQR( M, N, MB1, NB1LOCAL, A, LDA, WORK, LDWT,            
...
*
      DO J = 1, N
         CALL YCOPY( J, A( 1, J ), 1, WORK( LWT + N*(J-1)+1 ), 1 )
      END DO
*
...
*     DIAG is stored in WORK( LWT+N*N+1 ) from the YUNHR_COL output.
*
...
*
      DO I = 1, N
         IF( WORK( LWT+N*N+I ).EQ.-CONE ) THEN
            ...
            END DO
         ELSE
            CALL YCOPY( N-I+1, WORK(LWT+N*(I-1)+I), N, A( I, I ),       
     +            LDA )
         END IF
      END DO
*
