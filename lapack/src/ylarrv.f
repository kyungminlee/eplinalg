*> \brief \b YLARRV computes the eigenvectors of the tridiagonal matrix T = L D LT given L, D and the eigenvalues of L D LT.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download YLARRV + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ylarrv.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ylarrv.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ylarrv.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YLARRV( N, VL, VU, D, L, PIVMIN,
...
*>          = 0:  successful exit
*>
*>          > 0:  A problem occurred in YLARRV.
...
*     .. External Subroutines ..
      EXTERNAL           ECOPY, ELARRB, ELARRF, YESCAL,                 
     +   YLAR1V,                    YLASET
*     ..
*     .. Intrinsic Functions ..
...
*     The width of the part of Z that is used
      ...
      CALL YLASET( 'Full', N, ZUSEDW, CZERO, CZERO,                     
     +Z(1,ZUSEDL), LDZ )
...
*        This is for a 1x1 block
         IF( IBEGIN.EQ.IEND ) THEN
            DONE = DONE+1
            Z( IBEGIN, WBEGIN ) = CMPLX( ONE, ZERO , KIND=10)
...
*                       Get representation from the right end of Z array
     ...
     +                     J+1 ) , KIND=10)
   45             CONTINUE
                  D( IEND ) = REAL( Z( IEND, J ) , KIND=10)
                  SIGMA = REAL( Z( IEND, J+1 ) , KIND=10)

*                 Set the corresponding entries in Z to zero
                  CALL YLASET( 'Full', IN, 2, CZERO, CZERO,             
     +             Z( IBEGIN, J), LDZ )
               END IF
...
*                    ELARRF needs LWORK = 2*N
     ...
     + RGAP, PIVMIN, TAU,                          WORK( INDIN1 ), 
     +WORK( INDIN2 ),                          WORK( INDWRK ), IINFO )
*                    In the complex case, ELARRF cannot write
*                    the new RRR directly into Z and needs an intermediate
*                    workspace
                     DO 56 K = 1, IN-1
                        Z( IBEGIN+K-1, NEWFTT ) =                      
     +CMPLX( WORK( INDIN1+K-1 ), ZERO , KIND=10)
                        Z( IBEGIN+K-1, NEWFTT+1 ) =                     
     + CMPLX( WORK( INDIN2+K-1 ), ZERO , KIND=10)
   56                CONTINUE
                     Z( IEND, NEWFTT ) =                   CMPLX( WORK( 
     +INDIN1+IN-1 ), ZERO , KIND=10)
                     IF( IINFO.EQ.0 ) THEN
...
*                    Given LAMBDA, compute the eigenvector.
                     CALL YLAR1V( IN, 1, IN, LAMBDA, D( IBEGIN ),       
...
*                    Ensure vector is ok if support in the RQI has changed
 ...
 123                    CONTINUE
                     ENDIF
                     CALL YESCAL( ZTO-ZFROM+1, NRMINV,                  
     +      Z( ZFROM, WINDEX ), 1 )
 125                 CONTINUE
*                    Update W
