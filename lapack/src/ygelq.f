...
*  ===========
*
*       SUBROUTINE YGELQ( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*>  YLASWLQ (if the matrix is short-and-wide) or YGELQT to compute
...
*     .. External Subroutines ..
      EXTERNAL           YGELQT, YLASWLQ, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN( M, N ).GT.0 ) THEN
        MB = ILAENV_EP( 1, 'YGELQ ', ' ', M, N, 1, -1 )
        NB = ILAENV_EP( 1, 'YGELQ ', ' ', M, N, 2, -1 )
      ELSE
        MB = 1
        NB = N
      END IF
...
*
      IF( ( N.LE.M ) .OR. ( NB.LE.M ) .OR. ( NB.GE.N ) ) THEN
        CALL YGELQT( M, N, MB, A, LDA, T( 6 ), MB, WORK, INFO )
      ELSE
        CALL YLASWLQ( M, N, MB, NB, A, LDA, T( 6 ), MB, WORK,           
     +      LWORK, INFO )
      END IF
*
      WORK( 1 ) = LWREQ
