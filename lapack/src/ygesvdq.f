*> \brief <b> YGESVDQ computes the singular value decomposition (SVD) with a QR-Preconditioned QR SVD Method for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download YGESVDQ + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ygesvdq.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ygesvdq.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ygesvdq.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*      SUBROUTINE YGESVDQ( JOBA, JOBP, JOBR, JOBU, JOBV, M, N, A, LDA,
...
*     ..
*     .. Array Arguments ..
*      COMPLEX(KIND=10)       A( LDA, * ), U( LDU, * ), V( LDV, * ), CWORK( * )
...
*>        where EPS = ELAMCH('Epsilon'). This authorises YGESVDQ to
...
*> \verbatim
*>          JOBR is CHARACTER*1
*>          = 'T' After the initial pivoted QR factorization, YGESVD is applied to
*>          the adjoint R**H of the computed triangular factor R. This involves
*>          some extra data movement (matrix transpositions). Useful for
*>          experiments, research and development.
*>          = 'N' The triangular factor R is given as input to YGESVD. This may be
...
*> \param[in,out] A
*> \verbatim
*>          A is COMPLEX(KIND=10) array of dimensions LDA x N
*>          On entry, the input matrix A.
*>          On exit, if JOBU .NE. 'N' or JOBV .NE. 'N', the lower triangle of A contains
*>          the Householder vectors as stored by YGEQP3. If JOBU = 'F', these Householder
...
*> \param[out] S
*> \verbatim
*>          S is REAL(KIND=10) array of dimension N.
*>          The singular values of A, ordered so that S(i) >= S(i+1).
*> \endverbatim
*>
*> \param[out] U
*> \verbatim
*>          U is COMPLEX(KIND=10) array, dimension
...
*>          of YGESVD. The final value of NUMRANK might be further reduced if
...
*>          exact zeros in YGESVD applied to the upper triangular or trapezoidal
*>          R (from the initial QR factorization). In case of early exit (no call to
*>          YGESVD, such as in the case of zero matrix) RWORK(2) = -1.
*>
*>          If LIWORK, LCWORK, or LRWORK = -1, then on exit, if INFO = 0,
...
*>          > 0:  if YBDSQR did not converge, INFO specifies how many superdiagonals
*>          of an intermediate bidiagonal form B (computed in YGESVD) did not
*>          converge to zero.
*> \endverbatim
*
*> \par Further Details:
...
*>   SVD routine (YGESVD) to do its own preemptive scaling if potential over-
*>   or underflows are detected. To avoid repeated scanning of the array A,
*>   an optimal implementation would do all necessary scaling before calling
*>   YGESVD and the scaling in YGESVD can be switched off.
...
*     .. Local Scalars ..
     ...
     $            WNTUF,  WNTUR, WNTUS, WNTVA,   WNTVR
      REAL(KIND=10) BIG, EPSLN, RTMP, SCONDA, SFMIN
      COMPLEX(KIND=10)       CTMP
*     ..
*     .. Local Arrays
...
*     .. External Subroutines (BLAS, LAPACK)
      EXTERNAL    YGELQF, YGEQP3, YGEQRF, YGESVD, YLACPY,               
     +     YLAPMT,             YLASCL, YLASET, YLASWP, YESCAL, ELASET, 
     +ELASCL,             YPOCON, YUNMLQ, YUNMQR, XERBLA
*     ..
*     .. External Functions (BLAS, LAPACK)
      LOGICAL     LSAME
      INTEGER                     IEAMAX
      REAL(KIND=10)   YLANGE,          EYNRM2, ELAMCH
      EXTERNAL    LSAME, YLANGE,  IEAMAX, EYNRM2, ELAMCH
*     ..
*     .. Intrinsic Functions ..
...
*        .. minimal workspace length for YGEQP3 of an M x N matrix
         LWQP3 = N+1
*        .. minimal workspace length for YUNMQR to build left singular vectors
...
*        .. YGESVD of an N x N matrix
     ...
     +            LDU, CDUMMY, -1, IERR )
             ...
             ELSE IF ( WNTUA ) THEN
                 CALL YUNMQR( 'L', 'N', M, M, N, A, LDA, CDUMMY, U,     
     +            LDU, CDUMMY, -1, IERR )
                 LWRK_ZUNMQR = INT( CDUMMY(1) )
             ELSE
                 LWRK_ZUNMQR = 0
...
*            only the singular values are requested
             ...
             END IF
             IF ( LQUERY ) THEN
                 CALL YGESVD( 'N', 'N', N, N, A, LDA, S, U, LDU,        
...
*            singular values and the left singular vectors are requested
             ...
             IF ( LQUERY ) THEN
                IF ( RTRANS ) THEN
                   CALL YGESVD( 'N', 'O', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, CDUMMY, -1, RDUMMY, IERR )
                ELSE
                   CALL YGESVD( 'O', 'N', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, CDUMMY, -1, RDUMMY, IERR )
                END IF
                LWRK_ZGESVD = INT( CDUMMY(1) )
                IF ( CONDA ) THEN
...
*            full SVD is requested
             IF ( RTRANS ) THEN
                 ...
                 MINWRK = MINWRK + N
                 IF ( WNTVA ) THEN
*                   .. minimal workspace length for N x N/2 YGEQRF
                    LWQRF  = MAX( N/2, 1 )
*                   .. minimal workspace length for N/2 x N/2 YGESVD
                    LWSVD2 = MAX( 3 * (N/2), 1 )
                    LWUNQ2 = MAX( N, 1 )
     ...
     $                        N/2+LWUNQ2, LWUNQ )
             ...
             ELSE
                 ...
                 MINWRK = MINWRK + N
                 IF ( WNTVA ) THEN
*                   .. minimal workspace length for N/2 x N YGELQF
                    LWLQF  = MAX( N/2, 1 )
                    LWSVD2 = MAX( 3 * (N/2), 1 )
                    LWUNLQ = MAX( N , 1 )
     ...
     $                        N/2+LWUNLQ, LWUNQ )
             ...
             IF ( LQUERY ) THEN
                IF ( RTRANS ) THEN
                   CALL YGESVD( 'O', 'A', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, CDUMMY, -1, RDUMMY, IERR )
                   LWRK_ZGESVD = INT( CDUMMY(1) )
                   ...
                   OPTWRK = N + OPTWRK
                   IF ( WNTVA ) THEN
                       CALL YGEQRF(N,N/2,U,LDU,CDUMMY,CDUMMY,-1,IERR)
                       LWRK_ZGEQRF = INT( CDUMMY(1) )
                       CALL YGESVD( 'S', 'O', N/2,N/2, V,LDV, S, U,     
     +                          LDU,                       V, LDV, 
     +CDUMMY, -1, RDUMMY, IERR )
                       LWRK_ZGESVD2 = INT( CDUMMY(1) )
                       CALL YUNMQR( 'R', 'C', N, N, N/2, U, LDU,        
     +                       CDUMMY,                       V, LDV, 
     +CDUMMY, -1, IERR )
     ...
     $                           N/2+LWRK_ZGESVD2, N/2+LWRK_ZUNMQR2 )
                   ...
                   END IF
                ELSE
                   CALL YGESVD( 'S', 'O', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, CDUMMY, -1, RDUMMY, IERR )
                   LWRK_ZGESVD = INT( CDUMMY(1) )
                   OPTWRK = MAX(LWRK_ZGEQP3,LWRK_ZGESVD,LWRK_ZUNMQR)
                   IF ( CONDA ) OPTWRK = MAX( OPTWRK, LWCON )
                   OPTWRK = N + OPTWRK
                   IF ( WNTVA ) THEN
                      CALL YGELQF(N/2,N,U,LDU,CDUMMY,CDUMMY,-1,IERR)
                      LWRK_ZGELQF = INT( CDUMMY(1) )
                      CALL YGESVD( 'S','O', N/2,N/2, V, LDV, S, U,      
     +                        LDU,                      V, LDV, CDUMMY, 
     +-1, RDUMMY, IERR )
                      LWRK_ZGESVD2 = INT( CDUMMY(1) )
                      CALL YUNMLQ( 'R', 'N', N, N, N/2, U, LDU,         
     +                     CDUMMY,                      V, LDV, CDUMMY,
     +-1,IERR )
...
*           the case of differently scaled rows.
            DO 1904 p = 1, M
*               RWORK(p) = ABS( A(p,IYAMAX(N,A(p,1),LDA)) )
*               [[YLANGE will return NaN if an entry of the p-th row is Nan]]
                RWORK(p) = YLANGE( 'M', 1, N, A(p,1), LDA, RDUMMY )
...
*              Quick return: A is the M x N zero matrix.
               NUMRANK = 0
               CALL ELASET( 'G', N, 1, ZERO, ZERO, S, N )
               IF ( WNTUS ) CALL YLASET('G', M, N, CZERO, CONE, U,      
     +         LDU)
               IF ( WNTUA ) CALL YLASET('G', M, M, CZERO, CONE, U,      
     +         LDU)
               IF ( WNTVA ) CALL YLASET('G', N, N, CZERO, CONE, V,      
     +         LDV)
               IF ( WNTUF ) THEN
                   CALL YLASET( 'G', N, 1, CZERO, CZERO, CWORK, N )
                   CALL YLASET( 'G', M, N, CZERO, CONE, U, LDU )
               END IF
               DO 5001 p = 1, N
                   IWORK(p) = p
 5001          CONTINUE
 ...
 5002              CONTINUE
            ...
            END IF
*
            IF ( RWORK(1) .GT. BIG / SQRT(REAL(M, KIND=10)) ) THEN
*               .. to prevent overflow in the QR factorization, scale the
*               matrix by 1/sqrt(M) if too large entry detected
                CALL YLASCL('G',0,0,SQRT(REAL(M, KIND=10)),ONE, M,N, A,
     +LDA,                        IERR)
                ASCALED = .TRUE.
            END IF
            CALL YLASWP( N, A, LDA, 1, M-1, IWORK(N+1), 1 )
      END IF
*
...
*
     ...
     $         ( (RTMP*ZERO) .NE. ZERO ) ) THEN
               ...
               RETURN
          END IF
          IF ( RTMP .GT. BIG / SQRT(REAL(M, KIND=10)) ) THEN
*             .. to prevent overflow in the QR factorization, scale the
*             matrix by 1/sqrt(M) if too large entry detected
              CALL YLASCL('G',0,0, SQRT(REAL(M, KIND=10)),ONE, M,N, A,
     +LDA,                      IERR)
...
*        .. all columns are free columns
         IWORK(p) = 0
 1963 CONTINUE
      CALL YGEQP3( M, N, A, LDA, IWORK, CWORK, CWORK(N+1), LCWORK-N,    
     +  RWORK, IERR )
*
...
*        backward error of the order of N*EPS*||A||_F.
         NR = 1
         RTMP = SQRT(REAL(N, KIND=10))*EPSLN
         DO 3001 p = 2, N
...
*        close-to-rank-deficient. The threshold is set to EPSLN=ELAMCH('E').
...
*              perturbation theory.
               DO 3053 p = 1, NR
                  RTMP = EYNRM2( p, V(1,p), 1 )
                  CALL YESCAL( p, ONE/RTMP, V(1,p), 1 )
 3053          CONTINUE
               IF ( .NOT. ( LSVEC .OR. RSVEC ) ) THEN
                   CALL YPOCON( 'U', NR, V, LDV, ONE, RTMP,             
     +      CWORK, RWORK, IERR )
               ELSE
                   CALL YPOCON( 'U', NR, V, LDV, ONE, RTMP,             
...
*           the upper triangle of [A] to zero.
 ...
 1146       CONTINUE
*
            CALL YGESVD( 'N', 'N', N, NR, A, LDA, S, U, LDU,            
...
*
            IF ( NR .GT. 1 )           CALL YLASET( 'L', NR-1,NR-1, 
     +CZERO,CZERO, A(2,1),                        LDA )
            CALL YGESVD( 'N', 'N', NR, N, A, LDA, S, U, LDU,            
     +V, LDV, CWORK, LCWORK, RWORK, INFO )
*
         END IF
*
...
*            vectors of R
 ...
 1193          CONTINUE
 1192       CONTINUE
            IF ( NR .GT. 1 )           CALL YLASET( 'U', NR-1,NR-1, 
     +CZERO,CZERO, U(1,2),                        LDU )
*           .. the left singular vectors not computed, the NR right singular
*           vectors overwrite [U](1:NR,1:NR) as conjugate transposed. These
*           will be pre-multiplied by Q to build the left singular vectors of A.
               CALL YGESVD( 'N', 'O', N, NR, U, LDU, S, U, LDU,         
...
*            .. copy R into [U] and overwrite [U] with the left singular vectors
             CALL YLACPY( 'U', NR, N, A, LDA, U, LDU )
             IF ( NR .GT. 1 )          CALL YLASET( 'L', NR-1, NR-1, 
     +CZERO, CZERO, U(2,1),                       LDU )
*            .. the right singular vectors not computed, the NR left singular
*            vectors overwrite [U](1:NR,1:NR)
                CALL YGESVD( 'O', 'N', NR, N, U, LDU, S, U, LDU,        
...
*              (M x NR) or (M x N) or (M x M).
         IF ( ( NR .LT. M ) .AND. ( .NOT.WNTUF ) ) THEN
             CALL YLASET('A', M-NR, NR, CZERO, CZERO, U(NR+1,1), LDU)
             IF ( NR .LT. N1 ) THEN
...
*           vectors matrix U.
*
         IF ( .NOT.WNTUF )        CALL YUNMQR( 'L', 'N', M, N1, N, A, 
     +LDA, CWORK, U,             LDU, CWORK(N+1), LCWORK-N, IERR )
         IF ( ROWPRM .AND. .NOT.WNTUF )           CALL YLASWP( N1, U, 
     +LDU, 1, M-1, IWORK(N+1), -1 )
*
      ELSE IF ( RSVEC .AND. ( .NOT. LSVEC ) ) THEN
*.......................................................................
...
*
 ...
 1103              CONTINUE
               END IF
               CALL YLAPMT( .FALSE., NR, N, V, LDV, IWORK )
            ELSE
...
*               how to implement this, see the " FULL SVD " branch.
                CALL YLASET('G', N, N-NR, CZERO, CZERO, V(1,NR+1),      
     +                  LDV)
                CALL YGESVD( 'O', 'N', N, N, V, LDV, S, U, LDU,         
     +       U, LDU, CWORK(N+1), LCWORK-N, RWORK, INFO )
*
                DO 1123 p = 1, N
                   V(p,p) = CONJG(V(p,p))
 ...
 1124              CONTINUE
 1123           CONTINUE
                CALL YLAPMT( .FALSE., N, N, V, LDV, IWORK )
            END IF
*
          ELSE
*            .. aply YGESVD to R
*            .. copy R into V and overwrite V with the right singular vectors
             CALL YLACPY( 'U', NR, N, A, LDA, V, LDV )
             IF ( NR .GT. 1 )          CALL YLASET( 'L', NR-1, NR-1, 
     +CZERO, CZERO, V(2,1),                       LDV )
*            .. the right singular vectors overwrite V, the NR left singular
*            vectors stored in U(1:NR,1:NR)
             IF ( WNTVR .OR. ( NR .EQ. N ) ) THEN
                CALL YGESVD( 'N', 'O', NR, N, V, LDV, S, U, LDU,        
     +        V, LDV, CWORK(N+1), LCWORK-N, RWORK, INFO )
                CALL YLAPMT( .FALSE., NR, N, V, LDV, IWORK )
...
*               how to implement this, see the " FULL SVD " branch.
                 CALL YLASET('G', N-NR, N, CZERO,CZERO, V(NR+1,1),      
     +                   LDV)
                 CALL YGESVD( 'N', 'O', N, N, V, LDV, S, U, LDU,        
     +         V, LDV, CWORK(N+1), LCWORK-N, RWORK, INFO )
                 CALL YLAPMT( .FALSE., N, N, V, LDV, IWORK )
             END IF
...
*.......................................................................
         IF ( RTRANS ) THEN
*
*            .. apply YGESVD to R**H [[this option is left for R&D&T]]
*
            IF ( WNTVR .OR. ( NR .EQ. N ) ) THEN
...
*           transposed
               CALL YGESVD( 'O', 'A', N, NR, V, LDV, S, V, LDV,         
     +      U, LDU, CWORK(N+1), LCWORK-N, RWORK, INFO )
*              .. assemble V
               DO 1115 p = 1, NR
                  V(p,p) = CONJG(V(p,p))
...
*               OPTRATIO = ILAENV_EP(6, 'YGESVD', 'S' // 'O', NR,N,0,0)
*               OPTRATIO = MAX( OPTRATIO, 2 )
 ...
 1199                 CONTINUE
 1198              CONTINUE
                   IF ( NR .GT. 1 )              CALL YLASET('U',NR-1,
     +NR-1, CZERO,CZERO, V(1,2),                           LDV)
*
                   CALL YLASET('A',N,N-NR,CZERO,CZERO,V(1,NR+1),LDV)
                   CALL YGESVD( 'O', 'A', N, N, V, LDV, S, V, LDV,      
     +             U, LDU, CWORK(N+1), LCWORK-N, RWORK, INFO )
*
                   DO 1113 p = 1, N
                      V(p,p) = CONJG(V(p,p))
...
*
                   IF ( ( N .LT. M ) .AND. .NOT.(WNTUF)) THEN
                      CALL YLASET('A',M-N,N,CZERO,CZERO,U(N+1,1),LDU)
                      IF ( N .LT. N1 ) THEN
                        CALL YLASET('A',N,N1-N,CZERO,CZERO,U(1,N+1),    
     +                            LDU)
                        CALL YLASET('A',M-N,N1-N,CZERO,CONE,            
     +            U(N+1,N+1), LDU )
                      END IF
                   END IF
                ELSE
...
*                  singular vectors of R
 ...
 1196              CONTINUE
                   IF ( NR .GT. 1 )              CALL YLASET('U',NR-1,
     +NR-1,CZERO,CZERO,U(1,NR+2),                           LDU)
                   CALL YGEQRF( N, NR, U(1,NR+1), LDU, CWORK(N+1),      
 ...
 1143              CONTINUE
                  ...
                  CALL YGESVD( 'S', 'O', NR, NR, V, LDV, S, U, LDU,     
     ...
     +                      LDV)
                  CALL YUNMQR('R','C', N, N, NR, U(1,NR+1), LDU,        
...
*
         ELSE
*
*            .. apply YGESVD to R [[this is the recommended option]]
*
             IF ( WNTVR .OR. ( NR .EQ. N ) ) THEN
*                .. copy R into [V] and overwrite V with the right singular vectors
                 CALL YLACPY( 'U', NR, N, A, LDA, V, LDV )
...
*               singular vectors of R stored in [U](1:NR,1:NR)
                CALL YGESVD( 'S', 'O', NR, N, V, LDV, S, U, LDU,        
     +        V, LDV, CWORK(N+1), LCWORK-N, RWORK, INFO )
                CALL YLAPMT( .FALSE., NR, N, V, LDV, IWORK )
*               .. now [V](1:NR,1:N) contains V(1:N,1:NR)**H
*               .. assemble the left singular vector matrix U of dimensions
...
*               OPTRATIO = MAX( OPTRATIO, 2 )
               ...
               IF ( OPTRATIO * NR .GT. N ) THEN
                  CALL YLACPY( 'U', NR, N, A, LDA, V, LDV )
                  IF ( NR .GT. 1 )             CALL YLASET('L', NR-1,
     +NR-1, CZERO,CZERO, V(2,1),                          LDV)
*              .. the right singular vectors of R overwrite [V], the NR left
*                 singular vectors of R stored in [U](1:NR,1:NR)
                  CALL YLASET('A', N-NR,N, CZERO,CZERO, V(NR+1,1),      
     +                    LDV)
                  CALL YGESVD( 'S', 'O', N, N, V, LDV, S, U, LDU,       
     +           V, LDV, CWORK(N+1), LCWORK-N, RWORK, INFO )
                  CALL YLAPMT( .FALSE., N, N, V, LDV, IWORK )
...
*                 (M x N1), i.e. (M x N) or (M x M).
     ...
     +             U(N+1,N+1), LDU )
                  ...
                  END IF
               ELSE
                  CALL YLACPY( 'U', NR, N, A, LDA, U(NR+1,1), LDU )
                  IF ( NR .GT. 1 )             CALL YLASET('L',NR-1,
     +NR-1,CZERO,CZERO,U(NR+2,1),                          LDU)
                  CALL YGELQF( NR, N, U(NR+1,1), LDU, CWORK(N+1),       
     +           CWORK(N+NR+1), LCWORK-N-NR, IERR )
                  CALL YLACPY('L',NR,NR,U(NR+1,1),LDU,V,LDV)
                  IF ( NR .GT. 1 )             CALL YLASET('U',NR-1,
     +NR-1,CZERO,CZERO,V(1,2),LDV)
                  CALL YGESVD( 'S', 'O', NR, NR, V, LDV, S, U, LDU,     
     +             V, LDV, CWORK(N+NR+1), LCWORK-N-NR, RWORK, INFO )
                  ...
                  CALL YLASET('A',N-NR,N-NR,CZERO,CONE,V(NR+1,NR+1),    
     +                      LDV)
                  CALL YUNMLQ('R','N',N,N,NR,U(NR+1,1),LDU,             
     +             CWORK(N+1),                  V, LDV, CWORK(N+NR+1),
     +LCWORK-N-NR,IERR)
                  CALL YLAPMT( .FALSE., N, N, V, LDV, IWORK )
*               .. assemble the left singular vector matrix U of dimensions
*              (M x NR) or (M x N) or (M x M).
...
*     singular values are set to zero.
      IF ( NR .LT. N ) CALL ELASET( 'G', N-NR,1, ZERO,ZERO, S(NR+1),    
     +  N )
*     .. undo scaling; this may cause overflow in the largest singular
*     values.
      IF ( ASCALED )    CALL ELASCL( 'G',0,0, ONE,SQRT(REAL(M, 
     +KIND=10)), NR,1, S, N, IERR )
      IF ( CONDA ) RWORK(1) = SCONDA
      RWORK(2) = p - NR
*     .. p-NR is the number of singular values that are computed as
*     exact zeros in YGESVD() applied to the (possibly truncated)
