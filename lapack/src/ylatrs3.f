*> \brief \b YLATRS3 solves a triangular system of equations with the scale factors set to prevent overflow.
*
*  Definition:
*  ===========
*
*      SUBROUTINE YLATRS3( UPLO, TRANS, DIAG, NORMIN, N, NRHS, A, LDA,
...
*       ..
*       .. Array Arguments ..
*       REAL(KIND=10)   CNORM( * ), SCALE( * ), WORK( * )
*       COMPLEX(KIND=10)         A( LDA, * ), X( LDX, * )
*       ..
*
*
*> \par Purpose:
...
*> \verbatim
*>
*> YLATRS3 solves one of the triangular systems
*>
...
*> \param[out] SCALE
*> \verbatim
*>          SCALE is REAL(KIND=10) array, dimension (NRHS)
...
*     ..
*     .. Local Arrays ..
      REAL(KIND=10)   W( NBMAX ), XNRM( NBRHS )
*     ..
*     .. Local Scalars ..
     ...
     $                   LANRM, LDS, LSCALE, NB, NBA, NBX, RHS, LWMIN
      REAL(KIND=10)   ANRM, BIGNUM, BNRM, RSCAL, SCAL, SCALOC,          
     +          SCAMIN, SMLNUM, TMAX
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ILAENV_EP
      REAL(KIND=10)   ELAMCH, YLANGE, ELARMM
      EXTERNAL           ILAENV_EP, LSAME, ELAMCH, YLANGE,              
     +      ELARMM
*     ..
*     .. External Subroutines ..
      EXTERNAL           YGEMM, YLATRS, YESCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      NB = MAX( NBMIN, ILAENV_EP( 1, 'YLATRS', '', N, N, -1, -1 ) )
      NB = MIN( NBMAX, NB )
      NBA = MAX( 1, (N + NB - 1) / NB )
...
*
      BIGNUM = ELAMCH( 'Overflow' )
      SMLNUM = ELAMCH( 'Safe Minimum' )
*
*     Use unblocked code for small problems
*
      IF( NRHS.LT.NRHSMIN ) THEN
         CALL YLATRS( UPLO, TRANS, DIAG, NORMIN, N, A, LDA, X( 1, 1),   
     +              SCALE( 1 ), CNORM, INFO )
         DO K = 2, NRHS
            CALL YLATRS( UPLO, TRANS, DIAG, 'Y', N, A, LDA, X( 1,       
     +             K ),                    SCALE( K ), CNORM, INFO )
         END DO
         RETURN
      END IF
*
...
*
            IF( NOTRAN ) THEN
               ANRM = YLANGE( 'I', I2-I1, J2-J1, A( I1, J1 ), LDA,      
     +                   W )
               WORK( AWRK + I+(J-1)*NBA ) = ANRM
            ELSE
               ANRM = YLANGE( '1', I2-I1, J2-J1, A( I1, J1 ), LDA,      
     +                   W )
      ...
      END DO
*
      IF( .NOT. TMAX.LE.ELAMCH('Overflow') ) THEN
*
...
*
         DO K = 1, NRHS
            CALL YLATRS( UPLO, TRANS, DIAG, 'N', N, A, LDA, X( 1,       
     +             K ),                    SCALE( K ), CNORM, INFO )
         END DO
         RETURN
      END IF
...
*
            DO KK = 1, K2 - K1
               RHS = K1 + KK - 1
               IF( KK.EQ.1 ) THEN
                  CALL YLATRS( UPLO, TRANS, DIAG, 'N', J2-J1,           
     +               A( J1, J1 ), LDA, X( J1, RHS ),                    
     +      SCALOC, CNORM, INFO )
               ELSE
                  CALL YLATRS( UPLO, TRANS, DIAG, 'Y', J2-J1,           
     +               A( J1, J1 ), LDA, X( J1, RHS ),                    
     +      SCALOC, CNORM, INFO )
               END IF
...
*              growth in the linear updates.
               XNRM( KK ) = YLANGE( 'I', J2-J1, 1, X( J1, RHS ),        
     +                       LDX, W )
*
...
*                 factor WORK( J, KK ) > 0.
                  ...
                  IF( XNRM( KK )*RSCAL .LE. BIGNUM ) THEN
                     ...
                     CALL YESCAL( J2-J1, RSCAL, X( J1, RHS ), 1 )
                     SCALOC = ONE
                  ELSE
...
*
                  ...
                  IF( SCAL.NE.ONE ) THEN
                     CALL YESCAL( I2-I1, SCAL, X( I1, RHS ), 1 )
                     WORK( I+KK*LDS ) = SCAMIN*SCALOC
                  END IF
*
                  SCAL = ( SCAMIN / WORK( J+KK*LDS ) )*SCALOC
                  IF( SCAL.NE.ONE ) THEN
                     CALL YESCAL( J2-J1, SCAL, X( J1, RHS ), 1 )
...
*
                  CALL YGEMM( 'N', 'N', I2-I1, K2-K1, J2-J1, -CONE,     
...
*
                  CALL YGEMM( 'T', 'N', I2-I1, K2-K1, J2-J1, -CONE,     
...
*
                  CALL YGEMM( 'C', 'N', I2-I1, K2-K1, J2-J1, -CONE,     
     +                    A( J1, I1 ), LDA, X( J1, K1 ), LDX,           
     +              CONE, X( I1, K1 ), LDX )
               END IF
            END DO
         ...
         DO KK = 1, K2 - K1
            ...
            IF( SCALE( RHS ).NE.ONE .AND. SCALE( RHS ).NE. ZERO ) THEN
               DO I = 1, NBA
                  ...
                  IF( SCAL.NE.ONE )                CALL YESCAL( I2-I1, 
     +SCAL, X( I1, RHS ), 1 )
               END DO
            END IF
         END DO
