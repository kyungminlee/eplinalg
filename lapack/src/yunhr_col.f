...
*
*> \htmlonly
*> Download YUNHR_COL + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/yunhr_col.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/yunhr_col.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/yunhr_col.f">
*> [TXT]</a
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YUNHR_COL( M, N, NB, A, LDA, T, LDT, D, INFO )
*
*       .. Scalar Arguments ..
*       INTEGER           INFO, LDA, LDT, M, N, NB
*       ..
*       .. Array Arguments ..
*       COMPLEX(KIND=10)        A( LDA, * ), D( * ), T( LDT, * )
*       ..
*
*> \par Purpose:
*  =============
...
*> \verbatim
*>
*>  YUNHR_COL takes an M-by-N complex matrix Q_in with orthonormal columns
...
*>             YGEQRT). The matrix T and the matrix V stored on output
...
*>          blocks (same format as the output T in YGEQRT).
...
*     .. External Subroutines ..
      EXTERNAL           YCOPY, YLAUNHR_COL_GETRFNP, YSCAL,             
     +       YTRSM,                    XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*     (1-1) Factor V1 and U.
      ...
      CALL YLAUNHR_COL_GETRFNP( N, N, A, LDA, D, IINFO )
*
*     (1-2) Solve for V2.
*
      IF( M.GT.N ) THEN
         CALL YTRSM( 'R', 'U', 'N', 'N', M-N, N, CONE, A, LDA,          
     +      A( N+1, 1 ), LDA )
      END IF
*
...
*
         JBTEMP1 = JB - 1
         DO J = JB, JB+JNB-1
            CALL YCOPY( J-JBTEMP1, A( JB, J ), 1, T( 1, J ), 1 )
         END DO
*
...
*
         DO J = JB, JB+JNB-1
            IF( D( J ).EQ.CONE ) THEN
               CALL YSCAL( J-JBTEMP1, -CONE, T( 1, J ), 1 )
            END IF
         END DO
*
...
*             triangular, the routine YTRSM will access all JNB**2
...
*
         CALL YTRSM( 'R', 'L', 'C', 'U', JNB, JNB, CONE,                
