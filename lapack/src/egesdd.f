...
*
*> \htmlonly
*> Download EGESDD + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/egesdd.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/egesdd.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/egesdd.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE EGESDD( JOBZ, M, N, A, LDA, S, U, LDU, VT, LDVT,
...
*> \verbatim
*>
*> EGESDD computes the singular value decomposition (SVD) of a real
...
*>          >  0:  EBDSDC did not converge, updating process failed.
*>          =  0:  successful exit.
*> \endverbatim
*
*  Authors:
...
*     .. External Subroutines ..
      EXTERNAL           EBDSDC, EGEBRD, EGELQF, EGEMM, EGEQRF,         
...
*     .. External Functions ..
      ...
      EXTERNAL           ELAMCH, ELANGE, LSAME, EISNAN
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         MAXWRK = 1
         BDSPAC = 0
         MNTHR  = INT( MINMN*11.0E0_10 / 6.0E0_10 )
         IF( M.GE.N .AND. MINMN.GT.0 ) THEN
*
*           Compute space needed for EBDSDC
*
            IF( WNTQN ) THEN
*              ebdsdc needs only 4*N (or 6*N for uplo=L for LAPACK <= 3.6)
...
*           Compute space preferred for each routine
            CALL EGEBRD( M, N, DUM(1), M, DUM(1), DUM(1), DUM(1),       
     +             DUM(1), DUM(1), -1, IERR )
            LWORK_DGEBRD_MN = INT( DUM(1) )
*
            CALL EGEBRD( N, N, DUM(1), N, DUM(1), DUM(1), DUM(1),       
     +             DUM(1), DUM(1), -1, IERR )
            LWORK_DGEBRD_NN = INT( DUM(1) )
*
            CALL EGEQRF( M, N, DUM(1), M, DUM(1), DUM(1), -1, IERR )
            LWORK_DGEQRF_MN = INT( DUM(1) )
*
            CALL EORGBR( 'Q', N, N, N, DUM(1), N, DUM(1), DUM(1), -1,   
     +                 IERR )
            LWORK_DORGBR_Q_NN = INT( DUM(1) )
*
            CALL EORGQR( M, M, N, DUM(1), M, DUM(1), DUM(1), -1,        
     +            IERR )
            LWORK_DORGQR_MM = INT( DUM(1) )
*
            CALL EORGQR( M, N, N, DUM(1), M, DUM(1), DUM(1), -1,        
     +            IERR )
            LWORK_DORGQR_MN = INT( DUM(1) )
*
            CALL EORMBR( 'P', 'R', 'T', N, N, N, DUM(1), N,             
     +       DUM(1), DUM(1), N, DUM(1), -1, IERR )
            LWORK_DORMBR_PRT_NN = INT( DUM(1) )
*
            CALL EORMBR( 'Q', 'L', 'N', N, N, N, DUM(1), N,             
     +       DUM(1), DUM(1), N, DUM(1), -1, IERR )
            LWORK_DORMBR_QLN_NN = INT( DUM(1) )
*
            CALL EORMBR( 'Q', 'L', 'N', M, N, N, DUM(1), M,             
     +       DUM(1), DUM(1), M, DUM(1), -1, IERR )
            LWORK_DORMBR_QLN_MN = INT( DUM(1) )
*
            CALL EORMBR( 'Q', 'L', 'N', M, M, N, DUM(1), M,             
     +       DUM(1), DUM(1), M, DUM(1), -1, IERR )
...
*           Compute space preferred for each routine
     ...
     +             DUM(1), DUM(1), -1, IERR )
            LWORK_DGEBRD_MN = INT( DUM(1) )
*
            CALL EGEBRD( M, M, A, M, S, DUM(1), DUM(1),                 
     +   DUM(1), DUM(1), -1, IERR )
            LWORK_DGEBRD_MM = INT( DUM(1) )
*
            CALL EGELQF( M, N, A, M, DUM(1), DUM(1), -1, IERR )
...
*
            CALL EORGLQ( M, N, M, A, M, DUM(1), DUM(1), -1, IERR )
            LWORK_DORGLQ_MN = INT( DUM(1) )
*
            CALL EORGBR( 'P', M, M, M, A, N, DUM(1), DUM(1), -1,        
     +            IERR )
            LWORK_DORGBR_P_MM = INT( DUM(1) )
*
            CALL EORMBR( 'P', 'R', 'T', M, M, M, DUM(1), M,             
     +       DUM(1), DUM(1), M, DUM(1), -1, IERR )
            LWORK_DORMBR_PRT_MM = INT( DUM(1) )
*
            CALL EORMBR( 'P', 'R', 'T', M, N, M, DUM(1), M,             
     +       DUM(1), DUM(1), M, DUM(1), -1, IERR )
            LWORK_DORMBR_PRT_MN = INT( DUM(1) )
*
            CALL EORMBR( 'P', 'R', 'T', N, N, M, DUM(1), N,             
     +       DUM(1), DUM(1), N, DUM(1), -1, IERR )
            LWORK_DORMBR_PRT_NN = INT( DUM(1) )
*
            CALL EORMBR( 'Q', 'L', 'N', M, M, M, DUM(1), M,             
     +       DUM(1), DUM(1), M, DUM(1), -1, IERR )
            LWORK_DORMBR_QLN_MM = INT( DUM(1) )
*
            IF( N.GE.MNTHR ) THEN
...
*
               CALL EBDSDC( 'U', 'N', N, S, WORK( IE ), DUM, 1, DUM,    
     +                   1,                       DUM, IDUM, WORK( 
     +NWORK ), IWORK, INFO )
*
            ELSE IF( WNTQO ) THEN
...
*
               CALL ELACPY( 'U', N, N, A, LDA, WORK( IR ), LDWRKR )
...
*
               CALL EGEBRD( N, N, WORK( IR ), LDWRKR, S, WORK( IE ),    
...
*
               CALL EBDSDC( 'U', 'I', N, S, WORK( IE ), WORK( IU ),     
     +                  N,                       VT, LDVT, DUM, IDUM, 
...
*
               CALL EORMBR( 'Q', 'L', 'N', N, N, N, WORK( IR ),         
     +              LDWRKR,                       WORK( ITAUQ ), WORK( 
     +IU ), N, WORK( NWORK ),                       LWORK - NWORK + 1, 
     +IERR )
               CALL EORMBR( 'P', 'R', 'T', N, N, N, WORK( IR ),         
     +              LDWRKR,                       WORK( ITAUP ), VT, 
     +LDVT, WORK( NWORK ),                       LWORK - NWORK + 1, 
     +IERR )
*
...
*
               DO 10 I = 1, M, LDWRKR
                  ...
                  CALL EGEMM( 'N', 'N', CHUNK, N, N, ONE, A( I, 1 ),    
     +                     LDA, WORK( IU ), N, ZERO, WORK( IR ),        
     +                 LDWRKR )
                  CALL ELACPY( 'F', CHUNK, N, WORK( IR ), LDWRKR,       
     +                   A( I, 1 ), LDA )
   10          CONTINUE
*
            ELSE IF( WNTQS ) THEN
...
*
               CALL EBDSDC( 'U', 'I', N, S, WORK( IE ), U, LDU, VT,     
...
*
               CALL ELACPY( 'F', N, N, U, LDU, WORK( IR ), LDWRKR )
...
*
               CALL EORMBR( 'Q', 'L', 'N', N, N, N, A, LDA,             
     +          WORK( ITAUQ ), WORK( IU ), LDWRKU,                      
     + WORK( NWORK ), LWORK - NWORK + 1, IERR )
               CALL EORMBR( 'P', 'R', 'T', N, N, N, A, LDA,             
...
*
                  LDWRKU = M
                  NWORK = IU + LDWRKU*N
                  CALL ELASET( 'F', M, N, ZERO, ZERO, WORK( IU ),       
     +                   LDWRKU )
...
*
                  CALL EORMBR( 'Q', 'L', 'N', M, N, N, A, LDA,          
     +                WORK( ITAUQ ), WORK( IU ), LDWRKU,                
     +          WORK( NWORK ), LWORK - NWORK + 1, IERR )
*
*                 Copy left singular vectors of A from WORK(IU) to A
*
                  CALL ELACPY( 'F', M, N, WORK( IU ), LDWRKU, A,        
     +                  LDA )
               ELSE
*
*                 Path 5o-slow
...
*              Workspace: need   3*N [e, tauq, taup] + BDSPAC
*
               CALL ELASET( 'F', M, N, ZERO, ZERO, U, LDU )
               CALL EBDSDC( 'U', 'I', N, S, WORK( IE ), U, LDU, VT,     
     +                  LDVT, DUM, IDUM, WORK( NWORK ), IWORK,          
     +             INFO )
*
...
*              Workspace: need   3*N [e, tauq, taup] + BDSPAC
*
               CALL ELASET( 'F', M, M, ZERO, ZERO, U, LDU )
               CALL EBDSDC( 'U', 'I', N, S, WORK( IE ), U, LDU, VT,     
     +                  LDVT, DUM, IDUM, WORK( NWORK ), IWORK,          
     +             INFO )
*
...
*
               IF( M.GT.N ) THEN
                  CALL ELASET( 'F', M - N, M - N, ZERO, ONE, U(N+1,     
     +                     N+1),                          LDU )
               END IF
*
*              Overwrite U by left singular vectors of A and VT
...
*
               CALL EORMBR( 'Q', 'L', 'N', M, M, N, A, LDA,             
     +          WORK( ITAUQ ), U, LDU, WORK( NWORK ),                   
     +    LWORK - NWORK + 1, IERR )
               CALL EORMBR( 'P', 'R', 'T', N, N, M, A, LDA,             
     +          WORK( ITAUP ), VT, LDVT, WORK( NWORK ),                 
     +      LWORK - NWORK + 1, IERR )
            END IF
*
...
*
               CALL EBDSDC( 'U', 'N', M, S, WORK( IE ), DUM, 1, DUM,    
     +                   1,                       DUM, IDUM, WORK( 
     +NWORK ), IWORK, INFO )
*
            ELSE IF( WNTQO ) THEN
...
*
               CALL ELACPY( 'L', M, M, A, LDA, WORK( IL ), LDWRKL )
...
*
               CALL EGEBRD( M, M, WORK( IL ), LDWRKL, S, WORK( IE ),    
     +                   WORK( ITAUQ ), WORK( ITAUP ), WORK( NWORK ),   
     +                    LWORK - NWORK + 1, IERR )
*
*              Perform bidiagonal SVD, computing left singular vectors
...
*
               CALL EBDSDC( 'U', 'I', M, S, WORK( IE ), U, LDU,         
...
*
               CALL EORMBR( 'Q', 'L', 'N', M, M, M, WORK( IL ),         
     +              LDWRKL,                       WORK( ITAUQ ), U, LDU,
     + WORK( NWORK ),                       LWORK - NWORK + 1, IERR )
               CALL EORMBR( 'P', 'R', 'T', M, M, M, WORK( IL ),         
     +              LDWRKL,                       WORK( ITAUP ), WORK( 
     +IVT ), M,                       WORK( NWORK ), LWORK - NWORK + 1, 
     +IERR )
*
...
*
               DO 30 I = 1, N, CHUNK
                  BLK = MIN( N - I + 1, CHUNK )
                  CALL EGEMM( 'N', 'N', M, BLK, M, ONE, WORK( IVT ),    
     +                     M,                         A( 1, I ), LDA, 
     +ZERO, WORK( IL ), LDWRKL )
                  CALL ELACPY( 'F', M, BLK, WORK( IL ), LDWRKL,         
     +                 A( 1, I ), LDA )
   30          CONTINUE
*
            ELSE IF( WNTQS ) THEN
...
*
               CALL EBDSDC( 'U', 'I', M, S, WORK( IE ), U, LDU, VT,     
     +                  LDVT, DUM, IDUM, WORK( NWORK ), IWORK,          
     +             INFO )
*
...
*
               CALL ELACPY( 'F', M, M, VT, LDVT, WORK( IL ), LDWRKL )
               CALL EGEMM( 'N', 'N', M, N, M, ONE, WORK( IL ),          
...
*
               CALL EORMBR( 'Q', 'L', 'N', M, M, M, A, LDA,             
     +          WORK( ITAUQ ), U, LDU, WORK( NWORK ),                   
     +    LWORK - NWORK + 1, IERR )
               CALL EORMBR( 'P', 'R', 'T', M, M, M, A, LDA,             
...
*
               CALL EGEMM( 'N', 'N', M, N, M, ONE, WORK( IVT ),         
...
*
               CALL EBDSDC( 'L', 'N', M, S, WORK( IE ), DUM, 1, DUM,    
     +                   1,                       DUM, IDUM, WORK( 
     +NWORK ), IWORK, INFO )
            ELSE IF( WNTQO ) THEN
*              Path 5to (N > M, JOBZ='O')
...
*                 WORK( IVT ) is M by N
*
                  CALL ELASET( 'F', M, N, ZERO, ZERO, WORK( IVT ),      
     +                    LDWKVT )
                  NWORK = IVT + LDWKVT*N
...
*
               CALL EBDSDC( 'L', 'I', M, S, WORK( IE ), U, LDU,         
     +              WORK( IVT ), LDWKVT, DUM, IDUM,                     
     +  WORK( NWORK ), IWORK, INFO )
*
...
*
                  CALL EORMBR( 'P', 'R', 'T', M, N, M, A, LDA,          
     +                WORK( ITAUP ), WORK( IVT ), LDWKVT,               
     +           WORK( NWORK ), LWORK - NWORK + 1, IERR )
*
*                 Copy right singular vectors of A from WORK(IVT) to A
*
                  CALL ELACPY( 'F', M, N, WORK( IVT ), LDWKVT, A,       
     +                   LDA )
               ELSE
*
*                 Path 5to-slow
...
*
     ...
     +              WORK( IVT ),                            LDWKVT, A( 
     +1, I ), LDA, ZERO,                            WORK( IL ), M )
                     CALL ELACPY( 'F', M, BLK, WORK( IL ), M, A( 1,     
     +                        I ),                             LDA )
   40             CONTINUE
               END IF
            ELSE IF( WNTQS ) THEN
...
*              Workspace: need   3*M [e, tauq, taup] + BDSPAC
*
               CALL ELASET( 'F', M, N, ZERO, ZERO, VT, LDVT )
               CALL EBDSDC( 'L', 'I', M, S, WORK( IE ), U, LDU, VT,     
     +                  LDVT, DUM, IDUM, WORK( NWORK ), IWORK,          
     +             INFO )
*
*              Overwrite U by left singular vectors of A and VT
...
*              Workspace: need   3*M [e, tauq, taup] + BDSPAC
*
               CALL ELASET( 'F', N, N, ZERO, ZERO, VT, LDVT )
               CALL EBDSDC( 'L', 'I', M, S, WORK( IE ), U, LDU, VT,     
     +                  LDVT, DUM, IDUM, WORK( NWORK ), IWORK,          
     +             INFO )
*
...
*
               IF( N.GT.M ) THEN
                  CALL ELASET( 'F', N-M, N-M, ZERO, ONE, VT(M+1,M+1),   
     +                       LDVT )
               END IF
*
*              Overwrite U by left singular vectors of A and VT
