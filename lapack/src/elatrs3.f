*> \brief \b ELATRS3 solves a triangular system of equations with the scale factors set to prevent overflow.
*
*  Definition:
*  ===========
*
*      SUBROUTINE ELATRS3( UPLO, TRANS, DIAG, NORMIN, N, NRHS, A, LDA,
...
*       ..
*       .. Array Arguments ..
*       REAL(KIND=10)   A( LDA, * ), CNORM( * ), SCALE( * ), 
*                          WORK( * ), X( LDX, * )
*       ..
*
*
...
*> \verbatim
*>
*> ELATRS3 solves one of the triangular systems
*>
...
*     ..
*     .. Array Arguments ..
      REAL(KIND=10)   A( LDA, * ), CNORM( * ), X( LDX, * ),             
     +       SCALE( * ), WORK( * )
*     ..
*
*  =====================================================================
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ILAENV_EP
      REAL(KIND=10)   ELAMCH, ELANGE, ELARMM
      EXTERNAL           ELAMCH, ELANGE, ELARMM, ILAENV_EP,             
     +       LSAME
*     ..
*     .. External Subroutines ..
      EXTERNAL           EGEMM, ELATRS, ESCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      NB = MAX( 8, ILAENV_EP( 1, 'ELATRS', '', N, N, -1, -1 ) )
      NB = MIN( NBMAX, NB )
      NBA = MAX( 1, (N + NB - 1) / NB )
...
*
      IF( NRHS.LT.NRHSMIN ) THEN
         CALL ELATRS( UPLO, TRANS, DIAG, NORMIN, N, A, LDA, X( 1, 1),   
     +              SCALE( 1 ), CNORM, INFO )
         DO K = 2, NRHS
            CALL ELATRS( UPLO, TRANS, DIAG, 'Y', N, A, LDA, X( 1,       
...
*
            IF( NOTRAN ) THEN
               ANRM = ELANGE( 'I', I2-I1, J2-J1, A( I1, J1 ), LDA,      
     +                   W )
               WORK( AWRK + I+(J-1)*NBA ) = ANRM
            ELSE
               ANRM = ELANGE( '1', I2-I1, J2-J1, A( I1, J1 ), LDA,      
     +                   W )
...
*
         DO K = 1, NRHS
            CALL ELATRS( UPLO, TRANS, DIAG, 'N', N, A, LDA, X( 1,       
     +             K ),                    SCALE( K ), CNORM, INFO )
         END DO
         RETURN
      END IF
...
*
            DO KK = 1, K2-K1
               RHS = K1 + KK - 1
               IF( KK.EQ.1 ) THEN
                  CALL ELATRS( UPLO, TRANS, DIAG, 'N', J2-J1,           
     +               A( J1, J1 ), LDA, X( J1, RHS ),                    
     +      SCALOC, CNORM, INFO )
               ELSE
                  CALL ELATRS( UPLO, TRANS, DIAG, 'Y', J2-J1,           
     +               A( J1, J1 ), LDA, X( J1, RHS ),                    
     +      SCALOC, CNORM, INFO )
               END IF
...
*              growth in the linear updates.
               XNRM( KK ) = ELANGE( 'I', J2-J1, 1, X( J1, RHS ),        
     +                       LDX, W )
*
...
*                 factor WORK( J, KK ) > 0.
                  ...
                  IF( XNRM( KK ) * RSCAL .LE. BIGNUM ) THEN
                     ...
                     CALL ESCAL( J2-J1, RSCAL, X( J1, RHS ), 1 )
                     SCALOC = ONE
                  ELSE
...
*
                  BNRM = ELANGE( 'I', I2-I1, 1, X( I1, RHS ), LDX,      
     +                      W )
...
*
                  ...
                  IF( SCAL.NE.ONE ) THEN
                     CALL ESCAL( I2-I1, SCAL, X( I1, RHS ), 1 )
                     WORK( I+KK*LDS ) = SCAMIN*SCALOC
                  END IF
*
                  SCAL = ( SCAMIN / WORK( J+KK*LDS ) )*SCALOC
                  IF( SCAL.NE.ONE ) THEN
                     CALL ESCAL( J2-J1, SCAL, X( J1, RHS ), 1 )
...
*
                  CALL EGEMM( 'N', 'N', I2-I1, K2-K1, J2-J1, -ONE,      
...
*
                  CALL EGEMM( 'T', 'N', I2-I1, K2-K1, J2-J1, -ONE,      
         ...
         DO KK = 1, K2-K1
            ...
            IF( SCALE( RHS ).NE.ONE .AND. SCALE( RHS ).NE. ZERO ) THEN
               DO I = 1, NBA
                  ...
                  IF( SCAL.NE.ONE )                CALL ESCAL( I2-I1, 
     +SCAL, X( I1, RHS ), 1 )
               END DO
            END IF
         END DO
