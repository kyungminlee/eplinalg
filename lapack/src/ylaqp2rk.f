*> \brief \b YLAQP2RK computes truncated QR factorization with column pivoting of a complex matrix block using Level 2 BLAS and overwrites a complex m-by-nrhs matrix B with Q**H * B.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download YLAQP2RK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ylaqp2rk.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ylaqp2rk.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ylaqp2rk.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*      SUBROUTINE YLAQP2RK( M, N, NRHS, IOFFSET, KMAX, ABSTOL, RELTOL,
...
*> \verbatim
*>
*> YLAQP2RK computes a truncated (rank K) or full rank Householder QR
...
*>          main routine YGEQP3RK. 1 <= KP1 <= N_orig_mat.
*> \endverbatim
*>
*> \param[in] MAXC2NRM
*> \verbatim
...
*> \param[out] WORK
*> \verbatim
*>          WORK is COMPLEX(KIND=10) array, dimension (N-1)
*>          Used in YLARF1F subroutine to apply an elementary
...
*           submatrix A(I:M,K:N).
*
            KP = ( KK-1 ) + IEAMAX( N-KK+1, VN1( KK ), 1 )
*
...
*
         IF( KP.NE.KK ) THEN
            CALL YSWAP( M, A( 1, KP ), 1, A( 1, KK ), 1 )
            VN1( KP ) = VN1( KK )
            VN2( KP ) = VN2( KK )
            ITEMP = JPIV( KP )
            JPIV( KP ) = JPIV( KK )
...
*
         IF( I.LT.M ) THEN
            CALL YLARFG( M-I+1, A( I, KK ), A( I+1, KK ), 1,            
     +        TAU( KK ) )
         ELSE
            TAU( KK ) = CZERO
         END IF
...
*        by YLARFG is covered by checking TAU(KK) for NaN.
*
         IF( EISNAN( REAL( TAU(KK) , KIND=10) ) ) THEN
            TAUNAN = REAL( TAU(KK) , KIND=10)
         ELSE IF( EISNAN( AIMAG( TAU(KK) ) ) ) THEN
            TAUNAN = AIMAG( TAU(KK) )
         ELSE
            TAUNAN = ZERO
         END IF
...
*
         IF( KK.LT.MINMNUPDT ) THEN
            CALL YLARF1F( 'Left', M-I+1, N+NRHS-KK, A( I, KK ), 1,      
...
*
         JMAXC2NRM = K + IEAMAX( N-K, VN1( K+1 ), 1 )
