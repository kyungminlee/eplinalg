*> \brief \b ELAQP2RK computes truncated QR factorization with column pivoting of a real matrix block using Level 2 BLAS and overwrites a real m-by-nrhs matrix B with Q**T * B.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download ELAQP2RK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/elaqp2rk.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/elaqp2rk.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/elaqp2rk.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*      SUBROUTINE ELAQP2RK( M, N, NRHS, IOFFSET, KMAX, ABSTOL, RELTOL,
...
*     .. Array Arguments ..
*      INTEGER            JPIV( * )
*      REAL(KIND=10)   A( LDA, * ), TAU( * ), VN1( * ), VN2( * ),
*     $                   WORK( * )
*       ..
*
*
...
*> \verbatim
*>
*> ELAQP2RK computes a truncated (rank K) or full rank Householder QR
...
*>          main routine EGEQP3RK. 1 <= KP1 <= N_orig_mat.
*> \endverbatim
*>
*> \param[in] MAXC2NRM
*> \verbatim
...
*> \param[out] WORK
*> \verbatim
*>          WORK is REAL(KIND=10) array, dimension (N-1)
*>          Used in ELARF1F subroutine to apply an elementary
...
*     .. Local Scalars ..
     ...
     $                   MINMNUPDT
      REAL(KIND=10)   HUGEVAL, TEMP, TEMP2, TOL3Z
*     ..
*     .. External Subroutines ..
      EXTERNAL           ELARF1F, ELARFG, ESWAP
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( KP.NE.KK ) THEN
            CALL ESWAP( M, A( 1, KP ), 1, A( 1, KK ), 1 )
            VN1( KP ) = VN1( KK )
            VN2( KP ) = VN2( KK )
            ITEMP = JPIV( KP )
            JPIV( KP ) = JPIV( KK )
...
*
         IF( I.LT.M ) THEN
            CALL ELARFG( M-I+1, A( I, KK ), A( I+1, KK ), 1,            
     +        TAU( KK ) )
         ELSE
            TAU( KK ) = ZERO
         END IF
...
*        since ELARFG cannot produce TAU(KK) or Householder vector
...
*        by ELARFG is covered by checking TAU(KK) for NaN.
*
         IF( EISNAN( TAU(KK) ) ) THEN
            K = KK - 1
            INFO = KK
*
...
*
         IF( KK.LT.MINMNUPDT ) THEN
            CALL ELARF1F( 'Left', M-I+1, N+NRHS-KK, A( I, KK ), 1,      
