...
*  ===========
*
*       SUBROUTINE ETRSYL3( TRANA, TRANB, ISGN, M, N, A, LDA, B, LDB,
*                           C,
...
*> \verbatim
*>
*>  ETRSYL3 solves the real Sylvester matrix equation:
*>
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ILAENV_EP
      REAL(KIND=10)   ELANGE, ELAMCH, ELARMM
      EXTERNAL           ELANGE, ELAMCH, ELARMM, ILAENV_EP,             
     +       LSAME
*     ..
*     .. External Subroutines ..
      EXTERNAL           EGEMM, ELASCL, ESCAL, ETRSYL,                  
     +  XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      NB = MAX(8, ILAENV_EP( 1, 'ETRSYL', '', M, N, -1, -1) )
*
...
*
     ...
     $    LIWORK.LT.IWORK(1) ) THEN
        CALL ETRSYL( TRANA, TRANB, ISGN, M, N, A, LDA, B, LDB,          
     +      C, LDC, SCALE, INFO )
        RETURN
      END IF
*
...
*
      ...
      DO K = 1, NBA
         ...
         DO L = K, NBA
            ...
            L2 = IWORK( L + 1 )
            IF( NOTRNA ) THEN
               SWORK( K, AWRK + L ) = ELANGE( 'I', K2-K1, L2-L1,        
     +                                 A( K1, L1 ), LDA, WNRM )
            ELSE
               SWORK( L, AWRK + K ) = ELANGE( '1', K2-K1, L2-L1,        
     +                                 A( K1, L1 ), LDA, WNRM )
            END IF
         END DO
      END DO
      ...
      DO K = 1, NBB
         ...
         DO L = K, NBB
            ...
            L2 = IWORK( PC + L + 1 )
            IF( NOTRNB ) THEN
               SWORK( K, BWRK + L ) = ELANGE( 'I', K2-K1, L2-L1,        
     +                                 B( K1, L1 ), LDB, WNRM )
            ELSE
               SWORK( L, BWRK + K ) = ELANGE( '1', K2-K1, L2-L1,        
     +                                 B( K1, L1 ), LDB, WNRM )
            END IF
         END DO
      END DO
...
*
               ...
               L2 = IWORK( PC + L + 1 )
*
               CALL ETRSYL( TRANA, TRANB, ISGN, K2-K1, L2-L1,           
     +            A( K1, K1 ), LDA,                       B( L1, L1 ), 
...
*                       solution will be flushed in consistency scaling.
     ...
     +    SWORK( LL, JJ ) / 2.E0_10**EXPONENT( SCALOC ) )
               ...
               XNRM = ELANGE( 'I', K2-K1, L2-L1, C( K1, L1 ), LDC,      
     +                   WNRM )
*
               DO I = K - 1, 1, -1
*
...
*
                  CNRM = ELANGE( 'I', I2-I1, L2-L1, C( I1, L1 ),        
     +                    LDC, WNRM )
...
*
                  ...
                  IF (SCAL .NE. ONE) THEN
                      DO JJ = L1, L2-1
                         CALL ESCAL( K2-K1, SCAL, C( K1, JJ ), 1)
                      END DO
                  ENDIF
*
                  ...
                  IF (SCAL .NE. ONE) THEN
                      DO LL = L1, L2-1
                         CALL ESCAL( I2-I1, SCAL, C( I1, LL ), 1)
                      END DO
                  ENDIF
*
...
*
                  CALL EGEMM( 'N', 'N', I2-I1, L2-L1, K2-K1, -ONE,      
...
*                 simulating consistent scaling.
*
                  CNRM = ELANGE( 'I', K2-K1, J2-J1, C( K1, J1 ),        
     +                    LDC, WNRM )
...
*
                  ...
                  IF( SCAL .NE. ONE ) THEN
                     DO LL = L1, L2-1
                        CALL ESCAL( K2-K1, SCAL, C( K1, LL ), 1 )
                     END DO
                  ENDIF
*
...
*
                  CALL EGEMM( 'N', 'N', K2-K1, J2-J1, L2-L1, -SGN,      
...
*
                  ...
                  SWORK( I, L ) = SCAMIN * SCALOC
*
                  CALL EGEMM( 'T', 'N', I2-I1, L2-L1, K2-K1, -ONE,      
...
*
                  ...
                  SWORK( K, J ) = SCAMIN * SCALOC
*
                  CALL EGEMM( 'N', 'T', K2-K1, J2-J1, L2-L1, -SGN,      
...
*
         ...
         BUF = BUF * SCALOC
         CALL ELASCL( 'G', -1, -1, ONE, SCALOC, M, N, C, LDC,           
     +     IWORK(1) )
      END IF
*
