*> \brief \b YGEQP3RK computes a truncated Householder QR factorization with column pivoting of a complex m-by-n matrix A by using Level 3 BLAS and overwrites m-by-nrhs matrix B with Q**H * B.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download YGEQP3RK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ygeqp3rk.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ygeqp3rk.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ygeqp3rk.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YGEQP3RK( M, N, NRHS, KMAX, ABSTOL, RELTOL, A, LDA,
...
*       REAL(KIND=10)   ABSTOL, MAXC2NRMK, RELMAXC2NRMK, RELTOL
*      ..
*      .. Array Arguments ..
*       INTEGER            IWORK( * ), JPIV( * )
*       REAL(KIND=10)   RWORK( * )
*       COMPLEX(KIND=10)         A( LDA, * ), TAU( * ), WORK( * )
*      ..
*
*
*> \par Purpose:
...
*> \verbatim
*>
*> YGEQP3RK performs two tasks simultaneously:
*>
...
*>          is less than or equal to ABSTOL. Let SAFMIN = ELAMCH('S').
*>
...
*>          than or equal to RELTOL. Let EPS = ELAMCH('E').
*>
...
*> \param[in,out] A
*> \verbatim
*>          A is COMPLEX(KIND=10) array, dimension (LDA,N+NRHS)
*>
*>          On entry:
*>
...
*> \param[out] TAU
*> \verbatim
*>          TAU is COMPLEX(KIND=10) array, dimension (min(M,N))
...
*>          where NB is the optimal block size for YGEQP3RK returned
*>          by ILAENV_EP. Minimal block size MINNB=2.
*>
...
*>          by ILAENV_EP. (For N less than NX, unblocked code should be
*>          used.)
*>
...
*>          matrix in the blocked step auxiliary subroutine YLAQP3RK ).
*> \endverbatim
*>
*> \param[out] INFO
*> \verbatim
...
*
*> \verbatim
*> YGEQP3RK is based on the same BLAS3 Householder QR factorization
*> algorithm with column pivoting as in YGEQP3 routine which uses
*> YLARFG routine to generate Householder reflectors
...
*     .. Local Scalars ..
     ...
     $                   NBMIN, NX
      REAL(KIND=10)   EPS, HUGEVAL, MAXC2NRM, SAFMIN
*     ..
*     .. External Subroutines ..
      EXTERNAL           YLAQP2RK, YLAQP3RK, XERBLA
*     ..
*     .. External Functions ..
      LOGICAL            EISNAN
      INTEGER            IEAMAX, ILAENV_EP
      REAL(KIND=10)   ELAMCH, EYNRM2
      EXTERNAL           EISNAN, ELAMCH, EYNRM2, IEAMAX, ILAENV_EP
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      ...
      ELSE IF( KMAX.LT.0 ) THEN
         INFO = -4
      ELSE IF( EISNAN( ABSTOL ) ) THEN
         INFO = -5
      ELSE IF( EISNAN( RELTOL ) ) THEN
         INFO = -6
...
*           BLAS 2 code in YLAQP2RK.
*           1) YLAQP2RK: N+NRHS-1 to use in WORK array that is used
*              in YLARF1F subroutine inside YLAQP2RK to apply an
...
*
            NB = ILAENV_EP( INB, 'YGEQP3RK', ' ', M, N, -1, -1 )
*
*           A formula for the optimal workspace size in case of using
*           both unblocked BLAS 2 in YLAQP2RK and blocked BLAS 3 code
*           in YLAQP3RK.
*           1) YGEQP3RK, YLAQP2RK, YLAQP3RK: 2*N to store full and
*              partial column 2-norms.
*           2) YLAQP2RK: N+NRHS-1 to use in WORK array that is used
*              in YLARF1F subroutine to apply an elementary reflector
*              from the left.
*           3) YLAQP3RK: NB*(N+NRHS) to use in the work array F that
*              is used to apply a block reflector from
*              the left.
*           4) YLAQP3RK: NB to use in the auxilixary array AUX.
...
*
      KP1 = IEAMAX( N, RWORK( 1 ), 1 )
*
*     ==================================================================.
*
      IF( EISNAN( MAXC2NRM ) ) THEN
*
...
*
         NX = MAX( 0, ILAENV_EP( IXOVER, 'YGEQP3RK', ' ', M, N, -1,     
     +                    -1 ) )
*
         IF( NX.LT.MINMN ) THEN
*
...
*
               NB = ( LWORK-2*N ) / ( N+1 )
               NBMIN = MAX( 2, ILAENV_EP( INBMIN, 'YGEQP3RK', ' ', M, N,
     +                  -1, -1 ) )
*
            END IF
         END IF
...
*
            CALL YLAQP3RK( M, N_SUB, NRHS, IOFFSET, JB, ABSTOL,         
     +             RELTOL, KP1, MAXC2NRM, A( 1, J ), LDA,               
     +       DONE, JBF, MAXC2NRMK, RELMAXC2NRMK,                      
     +JPIV( J ), TAU( J ),                      RWORK( J ), RWORK( N+J 
     +),                      WORK( 1 ), WORK( JB+1 ),                  
...
*
         ...
         IOFFSET = J-1
*
         CALL YLAQP2RK( M, N_SUB, NRHS, IOFFSET, JMAX-J+1,              
...
*
         IF( K.LT.MINMN ) THEN
            JMAXC2NRM = K + IEAMAX( N-K, RWORK( K+1 ), 1 )
