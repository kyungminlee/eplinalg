...
*
*> \htmlonly
*> Download YTGEVC + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ytgevc.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ytgevc.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ytgevc.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YTGEVC( SIDE, HOWMNY, SELECT, N, S, LDS, P, LDP, VL,
...
*       LOGICAL            SELECT( * )
*       REAL(KIND=10)   RWORK( * )
*       COMPLEX(KIND=10)         P( LDP, * ), S( LDS, * ), VL( LDVL, * ),
*      $                   VR( LDVR, * ), WORK( * )
*       ..
*
*
...
*> \param[in] S
*> \verbatim
*>          S is COMPLEX(KIND=10) array, dimension (LDS,N)
*>          The upper triangular matrix S from a generalized Schur
*>          factorization, as computed by YHGEQZ.
*> \endverbatim
*>
*> \param[in] LDS
*> \verbatim
...
*> \param[in] P
*> \verbatim
*>          P is COMPLEX(KIND=10) array, dimension (LDP,N)
*>          The upper triangular matrix P from a generalized Schur
*>          factorization, as computed by YHGEQZ.  P must have real
*>          diagonal elements.
*> \endverbatim
*>
*> \param[in] LDP
...
*>          of left Schur vectors returned by YHGEQZ).
...
*>          of right Schur vectors returned by YHGEQZ).
...
*     .. Local Scalars ..
     ...
     $                   J, JE, JR
      REAL(KIND=10)   ACOEFA, ACOEFF, ANORM, ASCALE, BCOEFA, BIG,       
     +             BIGNUM, BNORM, BSCALE, DMIN, SAFMIN, SBETA,          
     +          SCALE, SMALL, TEMP, ULP, XMAX
      COMPLEX(KIND=10)         BCOEFF, CA, CB, D, SALPHA, SUM, SUMA, 
     +SUMB, X
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      ...
      EXTERNAL           LSAME, ELAMCH, YLADIV
*     ..
*     .. External Subroutines ..
      EXTERNAL           XERBLA, YGEMV
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     +ABS( REAL( P( JE, JE ) , KIND=10) ).LE.SAFMIN ) THEN
*
...
*
     ...
     +     ABS( REAL( P( JE, JE ) , KIND=10) )*BSCALE, SAFMIN )
               SALPHA = ( TEMP*S( JE, JE ) )*ASCALE
               SBETA = ( TEMP*REAL( P( JE, JE ) , KIND=10) )*BSCALE
               ACOEFF = SBETA*ASCALE
...
*
               IF( ILBACK ) THEN
                  CALL YGEMV( 'N', N, N+1-JE, CONE, VL( 1, JE ),        
     +                 LDVL,                         WORK( JE ), 1, 
     +CZERO, WORK( N+1 ), 1 )
                  ISRC = 2
                  IBEG = 1
...
*
               IF( ILBACK ) THEN
                  CALL YGEMV( 'N', N, JE, CONE, VR, LDVR, WORK, 1,      
     +                   CZERO, WORK( N+1 ), 1 )
                  ISRC = 2
                  IEND = N
               ELSE
