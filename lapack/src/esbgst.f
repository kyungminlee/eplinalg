...
*
*> \htmlonly
*> Download ESBGST + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/esbgst.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/esbgst.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/esbgst.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE ESBGST( VECT, UPLO, N, KA, KB, AB, LDAB, BB, LDBB, X,
*                          LDX, WORK, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO, VECT
...
*       ..
*       .. Array Arguments ..
*       REAL(KIND=10)   AB( LDAB, * ), BB( LDBB, * ), WORK( * ),
*      $                   X( LDX, * )
*       ..
*
*
...
*> \verbatim
*>
*> ESBGST reduces a real symmetric-definite banded generalized
*> eigenproblem  A*x = lambda*B*x  to standard form  C*y = lambda*y,
*> such that C has the same bandwidth as A.
*>
*> B must have been previously factorized as S**T*S by EPBSTF, using a
...
*> \param[in] BB
*> \verbatim
*>          BB is REAL(KIND=10) array, dimension (LDBB,N)
*>          The banded factor S from the split Cholesky factorization of
*>          B, as returned by EPBSTF, stored in the first KB+1 rows of
*>          the array.
*> \endverbatim
*>
*> \param[in] LDBB
...
*> \param[out] X
*> \verbatim
*>          X is REAL(KIND=10) array, dimension (LDX,N)
...
*> \param[out] WORK
*> \verbatim
*>          WORK is REAL(KIND=10) array, dimension (2*N)
*> \endverbatim
*>
*> \param[out] INFO
*> \verbatim
...
*
*  =====================================================================
      SUBROUTINE ESBGST( VECT, UPLO, N, KA, KB, AB, LDAB, BB, LDBB,     
     +               X,                    LDX, WORK, INFO )
*
...
*     .. External Subroutines ..
      EXTERNAL           EGER, ELAR2V, ELARGV, ELARTG, ELARTV,          
     +          ELASET,                    EROT, ESCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      IF( WANTX )    CALL ELASET( 'Full', N, N, ZERO, ONE, X, LDX )
*
*     Set M to the splitting point m. It must be the same value as is
*     used in EPBSTF. The chosen value allows the arrays WORK and RWORK
...
*
               CALL ESCAL( N-M, ONE / BII, X( M+1, I ), 1 )
               IF( KBT.GT.0 )             CALL EGER( N-M, KBT, -ONE, X( 
     +M+1, I ), 1,                        BB( KB1-KBT, I ), 1, X( M+1, 
     +I-KBT ), LDX )
            END IF
*
...
*
            IF( NRT.GT.0 )          CALL ELARGV( NRT, AB( 1, J2T ), 
     +INCA, WORK( J2T-M ),                       KA1,                   
     +    WORK( N+J2T-M ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 100 L = 1, KA - 1
                  CALL ELARTV( NR, AB( KA1-L, J2 ), INCA,               
...
*              blocks
*
               CALL ELAR2V( NR, AB( KA1, J2 ), AB( KA1, J2+1 ),         
...
*
            DO 110 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )             CALL ELARTV( NRT, AB( L, 
...
*
               DO 120 J = J2, J1, KA1
                  CALL EROT( N-M, X( M+1, J ), 1, X( M+1, J+1 ), 1,     
     +                   WORK( N+J-M ), WORK( J-M ) )
  120          CONTINUE
            END IF
  130    CONTINUE
...
*
               CALL ELARGV( NR, AB( 1, J2 ), INCA, WORK( J2 ), KA1,     
     +                  WORK( N+J2 ), KA1 )
*
...
*           have been created outside the band
*
            IF( NRT.GT.0 )          CALL ELARGV( NRT, AB( KA1, J2T-KA ),
     + INCA,                       WORK( J2T-M ),                       
...
*
               DO 330 L = 1, KA - 1
                  CALL ELARTV( NR, AB( L+1, J2-L ), INCA,               
...
*              blocks
*
               CALL ELAR2V( NR, AB( 1, J2 ), AB( 1, J2+1 ), AB( 2,      
     +                 J2 ),                       INCA, WORK( N+J2-M ),
     + WORK( J2-M ), KA1 )
*
            END IF
...
*
            DO 340 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )             CALL ELARTV( NRT, AB( KA1-L+1,
...
*              which have been created outside the band
*
               CALL ELARGV( NR, AB( KA1, J2-KA ), INCA, WORK( J2 ),     
     +                  KA1,                       WORK( N+J2 ), KA1 )
*
...
*              post-multiply X by inv(S(i))
*
               CALL ESCAL( NX, ONE / BII, X( 1, I ), 1 )
               IF( KBT.GT.0 )             CALL EGER( NX, KBT, -ONE, X( 
     +1, I ), 1, BB( KB,                        I+1 ),                  
     +      LDBB-1, X( 1, I+1 ), LDX )
            END IF
*
...
*                 generate rotation to annihilate a(i+k-ka-1,i)
*
                  CALL ELARTG( AB( K+1, I ), RA1, WORK( N+I+K-KA ),     
     +                     WORK( I+K-KA ), RA )
*
*                 create nonzero element a(i+k-ka-1,i+k) outside the
...
*           have been created outside the band
*
            IF( NRT.GT.0 )          CALL ELARGV( NRT, AB( 1, J1+KA ), 
     +INCA, WORK( J1 ),                       KA1,                      
     + WORK( N+J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 580 L = 1, KA - 1
                  CALL ELARTV( NR, AB( KA1-L, J1+L ), INCA,             
...
*              blocks
*
               CALL ELAR2V( NR, AB( KA1, J1 ), AB( KA1, J1-1 ),         
...
*
            DO 590 L = KA - 1, KB - K + 1, -1
               NRT = ( J2+L-1 ) / KA1
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )             CALL ELARTV( NRT, AB( L, J1T 
     +), INCA,                          AB( L+1, J1T-1 ), INCA, WORK( 
     +N+J1T ),                          WORK( J1T ), KA1 )
  590       CONTINUE
*
            IF( WANTX ) THEN
...
*
               DO 600 J = J1, J2, KA1
                  CALL EROT( NX, X( 1, J ), 1, X( 1, J-1 ), 1,          
     +              WORK( N+J ), WORK( J ) )
  600          CONTINUE
            END IF
  610    CONTINUE
...
*              which have been created outside the band
*
               CALL ELARGV( NR, AB( 1, J1+KA ), INCA,                   
     +    WORK( M-KB+J1 ),                       KA1, WORK( N+M-KB+J1 ),
     + KA1 )
*
*              apply rotations in 2nd set from the left
*
...
*                 generate rotation to annihilate a(i,i+k-ka-1)
*
                  CALL ELARTG( AB( KA1-K, I+K-KA ), RA1,                
...
*           have been created outside the band
*
            IF( NRT.GT.0 )          CALL ELARGV( NRT, AB( KA1, J1 ), 
     +INCA, WORK( J1 ),                       KA1,                      
     + WORK( N+J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 810 L = 1, KA - 1
                  CALL ELARTV( NR, AB( L+1, J1 ), INCA, AB( L+2,        
     +                  J1-1 ),                          INCA, WORK( 
     +N+J1 ), WORK( J1 ), KA1 )
  810          CONTINUE
*
...
*              blocks
*
               CALL ELAR2V( NR, AB( 1, J1 ), AB( 1, J1-1 ),             
...
*              which have been created outside the band
*
               CALL ELARGV( NR, AB( KA1, J1 ), INCA, WORK( M-KB+J1 ),   
