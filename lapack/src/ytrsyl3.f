...
*  ===========
*
*       SUBROUTINE YTRSYL3( TRANA, TRANB, ISGN, M, N, A, LDA, B, LDB,
...
*> \verbatim
*>
*>  YTRSYL3 solves the complex Sylvester matrix equation:
*>
...
*> \param[out] SWORK
*> \verbatim
*>          SWORK is REAL(KIND=10) array, dimension (MAX(2, ROWS),
*>          MAX(1,COLS)).
...
*     .. Local Scalars ..
     ...
     $                   K, K1, K2, L, L1, L2, LL, NBA, NB, NBB
      REAL(KIND=10)   ANRM, BIGNUM, BNRM, CNRM, SCAL, SCALOC,           
     +         SCAMIN, SGN, XNRM, BUF, SMLNUM
      COMPLEX(KIND=10)         CSGN
*     ..
*     .. Local Arrays ..
      REAL(KIND=10)   WNRM( MAX( M, N ) )
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ILAENV_EP
      REAL(KIND=10)   ELAMCH, ELARMM, YLANGE
      EXTERNAL           ELAMCH, ELARMM, ILAENV_EP, LSAME,              
     +      YLANGE
*     ..
*     .. External Subroutines ..
      EXTERNAL           XERBLA, YESCAL, YGEMM, YLASCL,                 
     +   YTRSYL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, EXPONENT, MAX, MIN
*     ..
...
*
      NB = MAX( 8, ILAENV_EP( 1, 'YTRSYL', '', M, N, -1, -1) )
*
...
*
      IF( MIN( NBA, NBB ).EQ.1 .OR. LDSWORK.LT.MAX( NBA, NBB ) ) THEN
        CALL YTRSYL( TRANA, TRANB, ISGN, M, N, A, LDA, B, LDB,          
     +      C, LDC, SCALE, INFO )
        RETURN
      END IF
*
...
*
      ...
      DO K = 1, NBA
         ...
         DO L = K, NBA
            ...
            L2 = MIN( L * NB, M ) + 1
            IF( NOTRNA ) THEN
               SWORK( K, AWRK + L ) = YLANGE( 'I', K2-K1, L2-L1,        
     +                                 A( K1, L1 ), LDA, WNRM )
            ELSE
               SWORK( L, AWRK + K ) = YLANGE( '1', K2-K1, L2-L1,        
     +                                 A( K1, L1 ), LDA, WNRM )
            END IF
         END DO
      END DO
      ...
      DO K = 1, NBB
         ...
         DO L = K, NBB
            ...
            L2 = MIN( L * NB, N ) + 1
            IF( NOTRNB ) THEN
               SWORK( K, BWRK + L ) = YLANGE( 'I', K2-K1, L2-L1,        
     +                                 B( K1, L1 ), LDB, WNRM )
            ELSE
               SWORK( L, BWRK + K ) = YLANGE( '1', K2-K1, L2-L1,        
     +                                 B( K1, L1 ), LDB, WNRM )
            END IF
         END DO
      END DO
*
      SGN = REAL( ISGN , KIND=10)
      CSGN = CMPLX( SGN, ZERO , KIND=10)
*
...
*
               ...
               L2 = MIN( L * NB, N ) + 1
*
               CALL YTRSYL( TRANA, TRANB, ISGN, K2-K1, L2-L1,           
     +            A( K1, K1 ), LDA,                       B( L1, L1 ), 
...
*                    Mark the computation as pointless.
                     BUF = ZERO
                  ELSE
                     BUF = BUF*2.E0_10**EXPONENT( SCALOC )
                  END IF
                  DO JJ = 1, NBB
                     DO LL = 1, NBA
...
*                       solution will be flushed in consistency scaling.
     ...
     +    SWORK( LL, JJ ) / 2.E0_10**EXPONENT( SCALOC ) )
                     END DO
                  END DO
               END IF
               SWORK( K, L ) = SCALOC * SWORK( K, L )
               XNRM = YLANGE( 'I', K2-K1, L2-L1, C( K1, L1 ), LDC,      
     +                   WNRM )
*
               DO I = K - 1, 1, -1
*
...
*                    Use second scaling factor to prevent flushing to zero.
     ...
     +    SWORK( LL, JJ ) / 2.E0_10**EXPONENT( SCALOC ) )
                        END DO
                     END DO
                     SCAMIN = SCAMIN / 2.E0_10**EXPONENT( SCALOC )
                     SCALOC = SCALOC / 2.E0_10**EXPONENT( SCALOC )
                  END IF
                  CNRM = CNRM * SCALOC
                  XNRM = XNRM * SCALOC
*
...
*
                  ...
                  IF( SCAL .NE. ONE ) THEN
                      DO JJ = L1, L2-1
                         CALL YESCAL( K2-K1, SCAL, C( K1, JJ ), 1)
                      END DO
                  ENDIF
*
                  ...
                  IF( SCAL .NE. ONE ) THEN
                      DO LL = L1, L2-1
                         CALL YESCAL( I2-I1, SCAL, C( I1, LL ), 1)
                      END DO
                  ENDIF
*
...
*
                  CALL YGEMM( 'N', 'N', I2-I1, L2-L1, K2-K1, -CONE,     
...
*                 simulating consistent scaling.
*
                  CNRM = YLANGE( 'I', K2-K1, J2-J1, C( K1, J1 ),        
     +                    LDC, WNRM )
                  SCAMIN = MIN( SWORK( K, J ), SWORK( K, L ) )
                  CNRM = CNRM * ( SCAMIN / SWORK( K, J ) )
                  XNRM = XNRM * ( SCAMIN / SWORK( K, L ) )
                  BNRM = SWORK(L, BWRK + J)
                  SCALOC = ELARMM( BNRM, XNRM, CNRM )
                  IF( SCALOC * SCAMIN .EQ. ZERO ) THEN
*                    Use second scaling factor to prevent flushing to zero.
                     BUF = BUF*2.E0_10**EXPONENT( SCALOC )
                     DO JJ = 1, NBB
...
*
                  ...
                  IF( SCAL .NE. ONE ) THEN
                     DO LL = L1, L2-1
                        CALL YESCAL( K2-K1, SCAL, C( K1, LL ), 1 )
                     END DO
                  ENDIF
*
...
*
                  CALL YGEMM( 'N', 'N', K2-K1, J2-J1, L2-L1, -CSGN,     
...
*
                  ...
                  SWORK( I, L ) = SCAMIN * SCALOC
*
                  CALL YGEMM( 'C', 'N', I2-I1, L2-L1, K2-K1, -CONE,     
...
*
                  ...
                  SWORK( K, J ) = SCAMIN * SCALOC
*
                  CALL YGEMM( 'N', 'C', K2-K1, J2-J1, L2-L1, -CSGN,     
...
*        form (1/SCALE)*X if SCALE is REAL(KIND=10). Set SCALE to
*        zero and give up.
*
         SWORK(1,1) = MAX( NBA, NBB )
         SWORK(2,1) = 2 * NBB + NBA
...
*
         SCAL = MAX( ABS( REAL( C( 1, 1 ) , KIND=10) ),                
     +ABS( AIMAG( C ( 1, 1 ) ) ) )
         DO K = 1, M
            DO L = 1, N
               SCAL = MAX( SCAL, ABS( REAL( C( K, L ) , KIND=10) ),     
     +                 ABS( AIMAG ( C( K, L ) ) ) )
            END DO
         END DO
*
...
*
         ...
         BUF = BUF * SCALOC
         CALL YLASCL( 'G', -1, -1, ONE, SCALOC, M, N, C, LDC, IINFO )
      END IF
*
