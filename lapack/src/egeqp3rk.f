*> \brief \b EGEQP3RK computes a truncated Householder QR factorization with column pivoting of a real m-by-n matrix A by using Level 3 BLAS and overwrites a real m-by-nrhs matrix B with Q**T * B.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download EGEQP3RK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/egeqp3rk.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/egeqp3rk.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/egeqp3rk.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE EGEQP3RK( M, N, NRHS, KMAX, ABSTOL, RELTOL, A, LDA,
...
*> \verbatim
*>
*> EGEQP3RK performs two tasks simultaneously:
*>
...
*> \param[in,out] A
*> \verbatim
*>          A is REAL(KIND=10) array, dimension (LDA,N+NRHS)
*>
*>          On entry:
*>
...
*> \param[out] TAU
*> \verbatim
*>          TAU is REAL(KIND=10) array, dimension (min(M,N))
...
*>          where NB is the optimal block size for EGEQP3RK returned
...
*>          matrix in the blocked step auxiliary subroutine ELAQP3RK ).
*> \endverbatim
*>
*> \param[out] INFO
*> \verbatim
...
*
*> \verbatim
*> EGEQP3RK is based on the same BLAS3 Householder QR factorization
*> algorithm with column pivoting as in EGEQP3 routine which uses
*> ELARFG routine to generate Householder reflectors
...
*     .. External Subroutines ..
      EXTERNAL           ELAQP2RK, ELAQP3RK, XERBLA
*     ..
*     .. External Functions ..
      LOGICAL            EISNAN
      INTEGER            IEAMAX, ILAENV_EP
      REAL(KIND=10)   ELAMCH, ENRM2
      EXTERNAL           EISNAN, ELAMCH, ENRM2, IEAMAX, ILAENV_EP
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*           BLAS 2 code in ELAQP2RK.
*           1) EGEQP3RK and ELAQP2RK: 2*N to store full and partial
*              column 2-norms.
*           2) ELAQP2RK: N+NRHS-1 to use in WORK array that is used
*              in ELARF1F subroutine inside ELAQP2RK to apply an
...
*
            NB = ILAENV_EP( INB, 'EGEQP3RK', ' ', M, N, -1, -1 )
*
*           A formula for the optimal workspace size in case of using
*           both unblocked BLAS 2 in ELAQP2RK and blocked BLAS 3 code
*           in ELAQP3RK.
*           1) EGEQP3RK, ELAQP2RK, ELAQP3RK: 2*N to store full and
*              partial column 2-norms.
*           2) ELAQP2RK: N+NRHS-1 to use in WORK array that is used
*              in ELARF1F subroutine to apply an elementary reflector
*              from the left.
*           3) ELAQP3RK: NB*(N+NRHS) to use in the work array F that
*              is used to apply a block reflector from
*              the left.
*           4) ELAQP3RK: NB to use in the auxilixary array AUX.
...
*
         NX = MAX( 0, ILAENV_EP( IXOVER, 'EGEQP3RK', ' ', M, N, -1,     
     +                    -1 ))
*
         IF( NX.LT.MINMN ) THEN
*
...
*
               NB = ( LWORK-2*N ) / ( N+1 )
               NBMIN = MAX( 2, ILAENV_EP( INBMIN, 'EGEQP3RK', ' ', M, N,
     +                  -1, -1 ) )
*
            END IF
         END IF
...
*
            CALL ELAQP3RK( M, N_SUB, NRHS, IOFFSET, JB, ABSTOL,         
     +             RELTOL, KP1, MAXC2NRM, A( 1, J ), LDA,               
     +       DONE, JBF, MAXC2NRMK, RELMAXC2NRMK,                      
     +JPIV( J ), TAU( J ),                      WORK( J ), WORK( N+J ), 
...
*
         ...
         IOFFSET = J-1
*
         CALL ELAQP2RK( M, N_SUB, NRHS, IOFFSET, JMAX-J+1,              
...
*
         IF( K.LT.MINMN ) THEN
            JMAXC2NRM = K + IEAMAX( N-K, WORK( K+1 ), 1 )
