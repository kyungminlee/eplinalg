*> \brief \b YLARFT forms the triangular factor T of a block reflector H = I - vtvH
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download YLARFT + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ylarft.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ylarft.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ylarft.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       RECURSIVE SUBROUTINE YLARFT( DIRECT, STOREV, N, K, V, LDV, TAU, T, LDT )
*
*       .. Scalar Arguments ..
...
*       ..
*       .. Array Arguments ..
*       COMPLEX(KIND=10)         T( LDT, * ), TAU( * ), V( LDV, * )
*       ..
*
*
*> \par Purpose:
...
*> \param[in] TAU
*> \verbatim
*>          TAU is COMPLEX(KIND=10) array, dimension (K)
...
*>
*  =====================================================================
      RECURSIVE SUBROUTINE YLARFT( DIRECT, STOREV, N, K, V, LDV,        
     +                      TAU, T, LDT )
*
...
*     .. Parameters ..
*
      COMPLEX(KIND=10)        ONE, NEG_ONE, ZERO
...
*
         CALL YLARFT(DIRECT, STOREV, N, L, V, LDV, TAU, T, LDT)
*
*        Compute T_{2,2} recursively
*
         CALL YLARFT(DIRECT, STOREV, N-L, K-L, V(L+1, L+1), LDV,        
...
*
         CALL YTRMM('Right', 'Lower', 'No transpose', 'Unit', L,        
...
*
         CALL YGEMM('Conjugate', 'No transpose', L, K-L, N-K, ONE,      
...
*
         CALL YTRMM('Left', 'Upper', 'No transpose', 'Non-unit', L,     
     +           K-L, NEG_ONE, T, LDT, T(1, L+1), LDT)
*
*        T_{1,2} = T_{1,2}*T_{2,2}
*
         CALL YTRMM('Right', 'Upper', 'No transpose', 'Non-unit', L,    
...
*        T_{1,2} = V_{1,2}
*
         CALL YLACPY('All', L, K-L, V(1, L+1), LDV, T(1, L+1), LDT)
*
*        T_{1,2} = T_{1,2}*V_{2,2}'
*
         CALL YTRMM('Right', 'Upper', 'Conjugate', 'Unit', L, K-L,      
...
*        Note: We assume K <= N, and GEMM will do nothing if N=K
*
         CALL YGEMM('No transpose', 'Conjugate', L, K-L, N-K, ONE,      
...
*        Compute T_{1,1} recursively
*
         CALL YLARFT(DIRECT, STOREV, N-L, K-L, V, LDV, TAU, T, LDT)
*
*        Compute T_{2,2} recursively
*
         CALL YLARFT(DIRECT, STOREV, N, L, V(1, K-L+1), LDV,            
...
*
         CALL YTRMM('Right', 'Upper', 'No transpose', 'Unit', L,        
...
*
         CALL YTRMM('Left', 'Lower', 'No transpose', 'Non-unit', L,     
     +           K-L, NEG_ONE, T(K-L+1, K-L+1), LDT,                
     +T(K-L+1, 1), LDT)
*
*        T_{2,1} = T_{2,1}*T_{1,1}
*
         CALL YTRMM('Right', 'Lower', 'No transpose', 'Non-unit', L,    
...
*        Compute T_{2,2} recursively
*
         CALL YLARFT(DIRECT, STOREV, N, L, V(K-L+1, 1), LDV,            
     +    TAU(K-L+1), T(K-L+1, K-L+1), LDT)
*
*        Compute T_{2,1}
*        T_{2,1} = V_{2,2}
*
         CALL YLACPY('All', L, K-L, V(K-L+1, N-K+1), LDV,               
     + T(K-L+1, 1), LDT)

*
*        T_{2,1} = T_{2,1}*V_{1,2}'
*
         CALL YTRMM('Right', 'Lower', 'Conjugate', 'Unit', L, K-L,      
...
*        T_{2,1} = -T_{2,2}*T_{2,1}
*
         CALL YTRMM('Left', 'Lower', 'No tranpose', 'Non-unit', L,      
     +          K-L, NEG_ONE, T(K-L+1, K-L+1), LDT,                 
     +T(K-L+1, 1), LDT)
...
*        T_{2,1} = T_{2,1}*T_{1,1}
*
         CALL YTRMM('Right', 'Lower', 'No tranpose', 'Non-unit', L,     
     +           K-L, ONE, T, LDT, T(K-L+1, 1), LDT)
      END IF
      END SUBROUTINE
