...
*
*> \htmlonly
*> Download YLAQZ3 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/YLAQZ3.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/YLAQZ3.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/YLAQZ3.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*      SUBROUTINE YLAQZ3( ILSCHUR, ILQ, ILZ, N, ILO, IHI, NSHIFTS,
...
*> \verbatim
*>
*> YLAQZ3 Executes a single multishift QZ sweep
*> \endverbatim
*
*  Arguments:
*  ==========
...
*> \param[in] ALPHA
*> \verbatim
*>          ALPHA is COMPLEX(KIND=10) array. SR contains
*>          the alpha parts of the shifts to use.
*> \endverbatim
*>
*> \param[in] BETA
*> \verbatim
*>          BETA is COMPLEX(KIND=10) array. SS contains
...
*> \param[in,out] QC
*> \verbatim
*>          QC is COMPLEX(KIND=10) array, dimension (LDQC, NBLOCK_DESIRED)
*> \endverbatim
*>
*> \param[in] LDQC
*> \verbatim
...
*> \param[in,out] ZC
*> \verbatim
*>          ZC is COMPLEX(KIND=10) array, dimension (LDZC, NBLOCK_DESIRED)
*> \endverbatim
*>
*> \param[in] LDZC
*> \verbatim
...
*     Function arguments
     ...
     +LDQ,    * ), Z( LDZ, * ), QC( LDQC, * ), ZC( LDZC, * ), WORK( * ),
     +    ALPHA( * ), BETA( * )
...
*     Local scalars
     ...
     $           ISTARTB, ISTOPB, ISHIFT, NBLOCK, NPOS
      REAL(KIND=10) :: SAFMIN, SAFMAX, C, SCALE
      COMPLEX(KIND=10) :: TEMP, TEMP2, TEMP3, S

*     External Functions
      EXTERNAL :: XERBLA, YLASET, YLARTG, YROT, YLAQZ1, YGEMM,          
     +   YLACPY
...
*     of size (ns+1) x ns.
      ...
      CALL YLASET( 'FULL', NS+1, NS+1, CZERO, CONE, QC, LDQC )
      CALL YLASET( 'FULL', NS, NS, CZERO, CONE, ZC, LDZC )
...
*        Introduce the shift
     ...
     $      ABS( TEMP3 ) .GT. SAFMAX ) THEN
         ...
         CALL YLARTG( TEMP2, TEMP3, C, S, TEMP )
         CALL YROT( NS, A( ILO, ILO ), LDA, A( ILO+1, ILO ), LDA, C,    
     +           S )
         CALL YROT( NS, B( ILO, ILO ), LDB, B( ILO+1, ILO ), LDB, C,    
     +           S )
         CALL YROT( NS+1, QC( 1, 1 ), 1, QC( 1, 2 ), 1, C,              
     + CONJG( S ) )
        
*        Chase the shift down
         DO J = 1, NS-I

            CALL YLAQZ1( .TRUE., .TRUE., J, 1, NS, IHI-ILO+1, A( ILO,   
...
*     from the left with Qc(1:ns+1,1:ns+1)'
      ...
      IF ( SWIDTH > 0 ) THEN
         CALL YGEMM( 'C', 'N', SHEIGHT, SWIDTH, SHEIGHT, CONE, QC,      
     +          LDQC,                A( ILO, ILO+NS ), LDA, CZERO, WORK,
     + SHEIGHT )
         CALL YLACPY( 'ALL', SHEIGHT, SWIDTH, WORK, SHEIGHT, A( ILO,    
     +             ILO+NS ), LDA )
         CALL YGEMM( 'C', 'N', SHEIGHT, SWIDTH, SHEIGHT, CONE, QC,      
     +          LDQC,                B( ILO, ILO+NS ), LDB, CZERO, WORK,
     + SHEIGHT )
         CALL YLACPY( 'ALL', SHEIGHT, SWIDTH, WORK, SHEIGHT, B( ILO,    
     +             ILO+NS ), LDB )
      END IF
      IF ( ILQ ) THEN
         CALL YGEMM( 'N', 'N', N, SHEIGHT, SHEIGHT, CONE, Q( 1,         
     +       ILO ),                LDQ, QC, LDQC, CZERO, WORK, N )
         CALL YLACPY( 'ALL', N, SHEIGHT, WORK, N, Q( 1, ILO ), LDQ )
      END IF
...
*     from the right with Zc(1:ns,1:ns)
      ...
      SWIDTH = NS
      IF ( SHEIGHT > 0 ) THEN
         CALL YGEMM( 'N', 'N', SHEIGHT, SWIDTH, SWIDTH, CONE,           
     +     A( ISTARTM, ILO ), LDA, ZC, LDZC, CZERO, WORK,               
     + SHEIGHT )
         CALL YLACPY( 'ALL', SHEIGHT, SWIDTH, WORK, SHEIGHT,            
     +     A( ISTARTM,                 ILO ), LDA )
         CALL YGEMM( 'N', 'N', SHEIGHT, SWIDTH, SWIDTH, CONE,           
     ...
     +     B( ISTARTM,                 ILO ), LDB )
      END IF
      IF ( ILZ ) THEN
         CALL YGEMM( 'N', 'N', N, SWIDTH, SWIDTH, CONE, Z( 1, ILO ),    
     +            LDZ, ZC, LDZC, CZERO, WORK, N )
         CALL YLACPY( 'ALL', N, SWIDTH, WORK, N, Z( 1, ILO ), LDZ )
      END IF
...
*        istopb points to the last column we will be updating
         ...
         CALL YLASET( 'FULL', NS+NP, NS+NP, CZERO, CONE, QC, LDQC )
         CALL YLASET( 'FULL', NS+NP, NS+NP, CZERO, CONE, ZC, LDZC )
...
*              (k:k+ns+np,k:k+ns+np-1)
               CALL YLAQZ1( .TRUE., .TRUE., K+I+J, ISTARTB, ISTOPB,     
     +                  IHI,                       A, LDA, B, LDB, 
...
*        from the left with Qc(1:ns+np,1:ns+np)'
     ...
     +           B( K+1,                    K+NS+NP ), LDB )
         END IF
         IF ( ILQ ) THEN
            CALL YGEMM( 'N', 'N', N, NBLOCK, NBLOCK, CONE, Q( 1,        
     +           K+1 ),                   LDQ, QC, LDQC, CZERO, WORK, N 
     +)
            CALL YLACPY( 'ALL', N, NBLOCK, WORK, N, Q( 1, K+1 ),        
     +            LDQ )
         END IF
...
*        from the right with Zc(1:ns+np,1:ns+np)
     ...
     +           B( ISTARTM, K ), LDB )
         END IF
         IF ( ILZ ) THEN
            CALL YGEMM( 'N', 'N', N, NBLOCK, NBLOCK, CONE, Z( 1, K ),   
     +                LDZ, ZC, LDZC, CZERO, WORK, N )
            CALL YLACPY( 'ALL', N, NBLOCK, WORK, N, Z( 1, K ), LDZ )
         END IF
...
*     one by one. Updates are initially applied to A(ihi-ns+1:ihi,ihi-ns:ihi).
      ...
      CALL YLASET( 'FULL', NS, NS, CZERO, CONE, QC, LDQC )
      CALL YLASET( 'FULL', NS+1, NS+1, CZERO, CONE, ZC, LDZC )
...
*        Chase the shift down to the bottom right corner
         DO ISHIFT = IHI-I, IHI-1
            CALL YLAQZ1( .TRUE., .TRUE., ISHIFT, ISTARTB, ISTOPB,       
...
*     from the left with Qc(1:ns,1:ns)'
      ...
      IF ( SWIDTH > 0 ) THEN
         CALL YGEMM( 'C', 'N', SHEIGHT, SWIDTH, SHEIGHT, CONE, QC,      
     +          LDQC,                A( IHI-NS+1, IHI+1 ), LDA, CZERO, 
     +WORK, SHEIGHT )
         CALL YLACPY( 'ALL', SHEIGHT, SWIDTH, WORK, SHEIGHT,            
     +     A( IHI-NS+1, IHI+1 ), LDA )
         CALL YGEMM( 'C', 'N', SHEIGHT, SWIDTH, SHEIGHT, CONE, QC,      
     +          LDQC,                B( IHI-NS+1, IHI+1 ), LDB, CZERO, 
     +WORK, SHEIGHT )
         CALL YLACPY( 'ALL', SHEIGHT, SWIDTH, WORK, SHEIGHT,            
     +     B( IHI-NS+1, IHI+1 ), LDB )
      END IF
      IF ( ILQ ) THEN
         CALL YGEMM( 'N', 'N', N, NS, NS, CONE, Q( 1, IHI-NS+1 ),       
     +         LDQ,                QC, LDQC, CZERO, WORK, N )
         CALL YLACPY( 'ALL', N, NS, WORK, N, Q( 1, IHI-NS+1 ), LDQ )
      END IF
...
*     from the right with Zc(1:ns+1,1:ns+1)
     ...
     +     B( ISTARTM,                 IHI-NS ), LDB )
      END IF
      IF ( ILZ ) THEN
         CALL YGEMM( 'N', 'N', N, NS+1, NS+1, CONE, Z( 1, IHI-NS ),     
     +           LDZ,                ZC, LDZC, CZERO, WORK, N )
         CALL YLACPY( 'ALL', N, NS+1, WORK, N, Z( 1, IHI-NS ), LDZ )
      END IF
