...
*
*> \htmlonly
*> Download EGETSQRHRT + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/egetsqrhrt.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/egetsqrhrt.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/egetsqrhrt.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE EGETSQRHRT( M, N, MB1, NB1, NB2, A, LDA, T, LDT, WORK,
*      $                       LWORK, INFO )
*       IMPLICIT NONE
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> EGETSQRHRT computes a NB2-sized column blocked QR-factorization
...
*>    A = Q_tsqr * R_tsqr = Q * R.
*>
*> The output Q and R factors are stored in the same format as in EGEQRT
*> (Q is in blocked compact WY-representation). See the documentation
*> of EGEQRT for more details on the format.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*>
*  =====================================================================
      SUBROUTINE EGETSQRHRT( M, N, MB1, NB1, NB2, A, LDA, T, LDT,       
     +                 WORK,                        LWORK, INFO )
      IMPLICIT NONE
*
...
*     .. External Subroutines ..
      EXTERNAL           ECOPY, ELATSQR, EORGTSQR_ROW,                  
     +  EORHR_COL,                    XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*        a) Matrix T and WORK for ELATSQR;
*        b) N-by-N upper-triangular factor R_tsqr;
*        c) Matrix T and array WORK for EORGTSQR_ROW;
*        d) Diagonal D for EORHR_COL.
*
...
*
            LW1 = NB1LOCAL * N
*
*           Length of EORGTSQR_ROW work array.
*
            LW2 = NB1LOCAL * MAX( NB1LOCAL, ( N - NB1LOCAL ) )
*
...
*
      CALL ELATSQR( M, N, MB1, NB1LOCAL, A, LDA, WORK, LDWT,            
...
*
      DO J = 1, N
         CALL ECOPY( J, A( 1, J ), 1, WORK( LWT + N*(J-1)+1 ), 1 )
      END DO
*
...
*     DIAG is stored in WORK( LWT+N*N+1 ) from the EORHR_COL output.
*
...
*
      DO I = 1, N
         IF( WORK( LWT+N*N+I ).EQ.-ONE ) THEN
            ...
            END DO
         ELSE
            CALL ECOPY( N-I+1, WORK(LWT+N*(I-1)+I), N, A( I, I ),       
     +            LDA )
         END IF
      END DO
*
