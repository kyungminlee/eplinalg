...
*
*> \htmlonly
*> Download YBBCSD + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ybbcsd.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ybbcsd.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ybbcsd.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YBBCSD( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS, M, P, Q,
...
*      $                   B21D( * ), B21E( * ), B22D( * ), B22E( * ),
*      $                   PHI( * ), THETA( * ), RWORK( * )
*       COMPLEX(KIND=10)         U1( LDU1, * ), U2( LDU2, * ), V1T( LDV1T, * ),
*      $                   V2T( LDV2T, * )
*       ..
*
*
...
*> \param[in,out] U1
*> \verbatim
*>          U1 is COMPLEX(KIND=10) array, dimension (LDU1,P)
...
*> \param[in,out] U2
*> \verbatim
*>          U2 is COMPLEX(KIND=10) array, dimension (LDU2,M-P)
...
*> \param[in,out] V1T
*> \verbatim
*>          V1T is COMPLEX(KIND=10) array, dimension (LDV1T,Q)
...
*> \param[in,out] V2T
*> \verbatim
*>          V2T is COMPLEX(KIND=10) array, dimension (LDV2T,M-Q)
...
*>          When YBBCSD converges, B11D contains the cosines of THETA(1),
*>          ..., THETA(Q). If YBBCSD fails to converge, then B11D
...
*>          When YBBCSD converges, B11E contains zeros. If YBBCSD fails
...
*>          When YBBCSD converges, B12D contains the negative sines of
*>          THETA(1), ..., THETA(Q). If YBBCSD fails to converge, then
...
*>          When YBBCSD converges, B12E contains zeros. If YBBCSD fails
...
*>          When YBBCSD converges, B21D contains the negative sines of
*>          THETA(1), ..., THETA(Q). If YBBCSD fails to converge, then
...
*>          When YBBCSD converges, B21E contains zeros. If YBBCSD fails
...
*>          When YBBCSD converges, B22D contains the negative sines of
*>          THETA(1), ..., THETA(Q). If YBBCSD fails to converge, then
...
*>          When YBBCSD converges, B22E contains zeros. If YBBCSD fails
...
*>          > 0:  if YBBCSD did not converge, INFO specifies the number
...
*
*  =====================================================================
      SUBROUTINE YBBCSD( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS, M, P,     
     +               Q,                    THETA, PHI, U1, LDU1, U2, 
     +LDU2, V1T, LDV1T,                    V2T, LDV2T, B11D, B11E, B12D,
     + B12E, B21D, B21E,                    B22D, B22E, RWORK, LRWORK, 
     +INFO )
*
...
*     .. Parameters ..
     ...
     +0.0E0_10 )
      COMPLEX(KIND=10)         NEGONECOMPLEX
      PARAMETER          ( NEGONECOMPLEX = (-1.0E0_10,0.0E0_10) )
...
*     .. Local Scalars ..
     ...
     +   UNFL, X1, X2, Y1, Y2
*
      EXTERNAL           ELARTGP, ELARTGS, ELAS2, XERBLA, YLASR,        
     +            YSCAL,                    YSWAP
*     ..
*     .. External Functions ..
      REAL(KIND=10)   ELAMCH
...
*
         IF( B11D(IMIN)**2+B11BULGE**2 .GT. THRESH**2 ) THEN
            CALL ELARTGP( B11BULGE, B11D(IMIN), RWORK(IU1SN+IMIN-1),    
     ...
     +      RWORK(IU1CS+IMIN-1), RWORK(IU1SN+IMIN-1) )
         ...
         IF( B21D(IMIN)**2+B21BULGE**2 .GT. THRESH**2 ) THEN
            CALL ELARTGP( B21BULGE, B21D(IMIN), RWORK(IU2SN+IMIN-1),    
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL ELARTGP( X2, X1, RWORK(IV1TSN+I-1),                 
     +       RWORK(IV1TCS+I-1), R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART21 ) THEN
               CALL ELARTGP( B11BULGE, B11E(I-1), RWORK(IV1TSN+I-1),    
     +                    RWORK(IV1TCS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL ELARTGP( B21BULGE, B21E(I-1), RWORK(IV1TSN+I-1),    
     +                    RWORK(IV1TCS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL ELARTGS( B11D(I), B11E(I), MU, RWORK(IV1TCS+I-1),   
     +                     RWORK(IV1TSN+I-1) )
            ELSE
               CALL ELARTGS( B21D(I), B21E(I), NU, RWORK(IV1TCS+I-1),   
     +                     RWORK(IV1TSN+I-1) )
            END IF
            RWORK(IV1TCS+I-1) = -RWORK(IV1TCS+I-1)
            RWORK(IV1TSN+I-1) = -RWORK(IV1TSN+I-1)
            IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( Y2, Y1, RWORK(IV2TSN+I-1-1),               
     +         RWORK(IV2TCS+I-1-1), R )
            ELSE IF( .NOT. RESTART12 .AND. RESTART22 ) THEN
               CALL ELARTGP( B12BULGE, B12D(I-1),                       
     + RWORK(IV2TSN+I-1-1),                        RWORK(IV2TCS+I-1-1), 
     +R )
            ELSE IF( RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( B22BULGE, B22D(I-1),                       
     + RWORK(IV2TSN+I-1-1),                        RWORK(IV2TCS+I-1-1), 
     +R )
            ELSE IF( NU .LT. MU ) THEN
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL ELARTGP( X2, X1, RWORK(IU1SN+I-1),                  
     +      RWORK(IU1CS+I-1),                        R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART12 ) THEN
               CALL ELARTGP( B11BULGE, B11D(I), RWORK(IU1SN+I-1),       
     +                 RWORK(IU1CS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL ELARTGP( B12BULGE, B12E(I-1), RWORK(IU1SN+I-1),     
     +                   RWORK(IU1CS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL ELARTGS( B11E(I), B11D(I+1), MU,                    
     +    RWORK(IU1CS+I-1),                        RWORK(IU1SN+I-1) )
            ELSE
               CALL ELARTGS( B12D(I), B12E(I), NU, RWORK(IU1CS+I-1),    
     +                    RWORK(IU1SN+I-1) )
            END IF
            IF( .NOT. RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( Y2, Y1, RWORK(IU2SN+I-1),                  
     +      RWORK(IU2CS+I-1),                        R )
            ELSE IF( .NOT. RESTART21 .AND. RESTART22 ) THEN
               CALL ELARTGP( B21BULGE, B21D(I), RWORK(IU2SN+I-1),       
     +                 RWORK(IU2CS+I-1), R )
            ELSE IF( RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( B22BULGE, B22E(I-1), RWORK(IU2SN+I-1),     
     +                   RWORK(IU2CS+I-1), R )
            ELSE IF( NU .LT. MU ) THEN
               CALL ELARTGS( B21E(I), B21E(I+1), NU,                    
     +    RWORK(IU2CS+I-1),                        RWORK(IU2SN+I-1) )
            ELSE
               CALL ELARTGS( B22D(I), B22E(I), MU, RWORK(IU2CS+I-1),    
     +                    RWORK(IU2SN+I-1) )
            END IF
...
*
         IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
            CALL ELARTGP( Y2, Y1, RWORK(IV2TSN+IMAX-1-1),               
...
*
         IF( WANTU1 ) THEN
            IF( COLMAJOR ) THEN
               CALL YLASR( 'R', 'V', 'F', P, IMAX-IMIN+1,               
     +       RWORK(IU1CS+IMIN-1), RWORK(IU1SN+IMIN-1),                  
     +    U1(1,IMIN), LDU1 )
            ELSE
               CALL YLASR( 'L', 'V', 'F', IMAX-IMIN+1, P,               
     +       RWORK(IU1CS+IMIN-1), RWORK(IU1SN+IMIN-1),                  
     +    U1(IMIN,1), LDU1 )
            END IF
         END IF
         IF( WANTU2 ) THEN
            IF( COLMAJOR ) THEN
               CALL YLASR( 'R', 'V', 'F', M-P, IMAX-IMIN+1,             
     +         RWORK(IU2CS+IMIN-1), RWORK(IU2SN+IMIN-1),                
     +      U2(1,IMIN), LDU2 )
            ELSE
               CALL YLASR( 'L', 'V', 'F', IMAX-IMIN+1, M-P,             
     +         RWORK(IU2CS+IMIN-1), RWORK(IU2SN+IMIN-1),                
     +      U2(IMIN,1), LDU2 )
            END IF
         END IF
         IF( WANTV1T ) THEN
            IF( COLMAJOR ) THEN
               CALL YLASR( 'L', 'V', 'F', IMAX-IMIN+1, Q,               
     +       RWORK(IV1TCS+IMIN-1), RWORK(IV1TSN+IMIN-1),                
     +      V1T(IMIN,1), LDV1T )
            ELSE
               CALL YLASR( 'R', 'V', 'F', Q, IMAX-IMIN+1,               
     +       RWORK(IV1TCS+IMIN-1), RWORK(IV1TSN+IMIN-1),                
     +      V1T(1,IMIN), LDV1T )
            END IF
         END IF
         IF( WANTV2T ) THEN
            IF( COLMAJOR ) THEN
               CALL YLASR( 'L', 'V', 'F', IMAX-IMIN+1, M-Q,             
     +         RWORK(IV2TCS+IMIN-1), RWORK(IV2TSN+IMIN-1),              
     +        V2T(IMIN,1), LDV2T )
            ELSE
               CALL YLASR( 'R', 'V', 'F', M-Q, IMAX-IMIN+1,             
     +         RWORK(IV2TCS+IMIN-1), RWORK(IV2TSN+IMIN-1),              
     +        V2T(1,IMIN), LDV2T )
            END IF
         END IF
...
*
         IF( B11E(IMAX-1)+B21E(IMAX-1) .GT. 0 ) THEN
            ...
            IF( WANTV1T ) THEN
               IF( COLMAJOR ) THEN
                  CALL YSCAL( Q, NEGONECOMPLEX, V1T(IMAX,1), LDV1T )
               ELSE
                  CALL YSCAL( Q, NEGONECOMPLEX, V1T(1,IMAX), 1 )
               END IF
            END IF
         END IF
*
...
*
         IF( B11D(IMAX)+B12E(IMAX-1) .LT. 0 ) THEN
            ...
            IF( WANTU1 ) THEN
               IF( COLMAJOR ) THEN
                  CALL YSCAL( P, NEGONECOMPLEX, U1(1,IMAX), 1 )
               ELSE
                  CALL YSCAL( P, NEGONECOMPLEX, U1(IMAX,1), LDU1 )
               END IF
            END IF
         END IF
         IF( B21D(IMAX)+B22E(IMAX-1) .GT. 0 ) THEN
            ...
            IF( WANTU2 ) THEN
               IF( COLMAJOR ) THEN
                  CALL YSCAL( M-P, NEGONECOMPLEX, U2(1,IMAX), 1 )
               ELSE
                  CALL YSCAL( M-P, NEGONECOMPLEX, U2(IMAX,1), LDU2 )
               END IF
            END IF
         END IF
*
...
*
         IF( B12D(IMAX)+B22D(IMAX) .LT. 0 ) THEN
            IF( WANTV2T ) THEN
               IF( COLMAJOR ) THEN
                  CALL YSCAL( M-Q, NEGONECOMPLEX, V2T(IMAX,1),          
     +               LDV2T )
               ELSE
                  CALL YSCAL( M-Q, NEGONECOMPLEX, V2T(1,IMAX), 1 )
               END IF
            END IF
         END IF
*
...
*
         IF( MINI .NE. I ) THEN
            ...
            THETA(I) = THETAMIN
            IF( COLMAJOR ) THEN
               IF( WANTU1 )             CALL YSWAP( P, U1(1,I), 1, U1(1,
     +MINI), 1 )
               IF( WANTU2 )             CALL YSWAP( M-P, U2(1,I), 1, 
     +U2(1,MINI), 1 )
               IF( WANTV1T )             CALL YSWAP( Q, V1T(I,1), LDV1T,
     + V1T(MINI,1),                         LDV1T )
               IF( WANTV2T )             CALL YSWAP( M-Q, V2T(I,1), 
     +LDV2T, V2T(MINI,1),                LDV2T )
            ELSE
               IF( WANTU1 )             CALL YSWAP( P, U1(I,1), LDU1, 
     +U1(MINI,1), LDU1 )
               IF( WANTU2 )             CALL YSWAP( M-P, U2(I,1), LDU2, 
     +U2(MINI,1), LDU2 )
               IF( WANTV1T )             CALL YSWAP( Q, V1T(1,I), 1, 
     +V1T(1,MINI), 1 )
               IF( WANTV2T )             CALL YSWAP( M-Q, V2T(1,I), 1, 
     +V2T(1,MINI), 1 )
            END IF
         END IF
*
