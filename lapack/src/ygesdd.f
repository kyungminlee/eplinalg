...
*
*> \htmlonly
*> Download YGESDD + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ygesdd.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ygesdd.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ygesdd.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YGESDD( JOBZ, M, N, A, LDA, S, U, LDU, VT, LDVT,
...
*> \verbatim
*>
*> YGESDD computes the singular value decomposition (SVD) of a complex
...
*>          >  0:  The updating process of EBDSDC did not converge.
*>          =  0:  successful exit.
*> \endverbatim
*
*  Authors:
...
*     .. Local Scalars ..
     ...
     $                   LWORK_ZUNMBR_PRC_NN, LWORK_ZUNMBR_QLN_NN
      REAL(KIND=10)   ANRM, BIGNUM, EPS, SMLNUM
*     ..
*     .. Local Arrays ..
      INTEGER            IDUM( 1 )
      REAL(KIND=10)   DUM( 1 )
      COMPLEX(KIND=10)         CDUM( 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           EBDSDC, ELASCL, XERBLA, YGEBRD, YGELQF,        
     +            YGEMM,                    YGEQRF, YLACP2, YLACPY, 
     +YLACRM, YLARCM, YLASCL,                    YLASET, YUNGBR, YUNGLQ,
     + YUNGQR, YUNMBR
*     ..
*     .. External Functions ..
      LOGICAL            LSAME, EISNAN
      REAL(KIND=10)   ELAMCH, YLANGE
      EXTERNAL           LSAME, ELAMCH, YLANGE, EISNAN
*     ..
*     .. Intrinsic Functions ..
...
*
      INFO   = 0
      MINMN  = MIN( M, N )
      MNTHR1 = INT( MINMN*17.0E0_10 / 9.0E0_10 )
      MNTHR2 = INT( MINMN*5.0E0_10 / 3.0E0_10 )
      WNTQA  = LSAME( JOBZ, 'A' )
      WNTQS  = LSAME( JOBZ, 'S' )
...
*       immediately following subroutine, as returned by ILAENV_EP.)
*
      IF( INFO.EQ.0 ) THEN
         MINWRK = 1
         MAXWRK = 1
...
*           The real work space needed for bidiagonal SVD (ebdsdc) is
...
*           Compute space preferred for each routine
            CALL YGEBRD( M, N, CDUM(1), M, DUM(1), DUM(1), CDUM(1),     
     +               CDUM(1), CDUM(1), -1, IERR )
            LWORK_ZGEBRD_MN = INT( CDUM(1) )
*
            CALL YGEBRD( N, N, CDUM(1), N, DUM(1), DUM(1), CDUM(1),     
     +               CDUM(1), CDUM(1), -1, IERR )
            LWORK_ZGEBRD_NN = INT( CDUM(1) )
*
            CALL YGEQRF( M, N, CDUM(1), M, CDUM(1), CDUM(1), -1,        
     +            IERR )
            LWORK_ZGEQRF_MN = INT( CDUM(1) )
*
            CALL YUNGBR( 'P', N, N, N, CDUM(1), N, CDUM(1), CDUM(1),    
     +                -1, IERR )
            LWORK_ZUNGBR_P_NN = INT( CDUM(1) )
*
            CALL YUNGBR( 'Q', M, M, N, CDUM(1), M, CDUM(1), CDUM(1),    
     +                -1, IERR )
            LWORK_ZUNGBR_Q_MM = INT( CDUM(1) )
*
            CALL YUNGBR( 'Q', M, N, N, CDUM(1), M, CDUM(1), CDUM(1),    
     +                -1, IERR )
            LWORK_ZUNGBR_Q_MN = INT( CDUM(1) )
*
            CALL YUNGQR( M, M, N, CDUM(1), M, CDUM(1), CDUM(1),         
     +           -1, IERR )
            LWORK_ZUNGQR_MM = INT( CDUM(1) )
*
            CALL YUNGQR( M, N, N, CDUM(1), M, CDUM(1), CDUM(1),         
     +           -1, IERR )
            LWORK_ZUNGQR_MN = INT( CDUM(1) )
*
            CALL YUNMBR( 'P', 'R', 'C', N, N, N, CDUM(1), N, CDUM(1),   
     +                 CDUM(1), N, CDUM(1), -1, IERR )
            LWORK_ZUNMBR_PRC_NN = INT( CDUM(1) )
*
            CALL YUNMBR( 'Q', 'L', 'N', M, M, N, CDUM(1), M, CDUM(1),   
     +                 CDUM(1), M, CDUM(1), -1, IERR )
            LWORK_ZUNMBR_QLN_MM = INT( CDUM(1) )
*
            CALL YUNMBR( 'Q', 'L', 'N', M, N, N, CDUM(1), M, CDUM(1),   
     +                 CDUM(1), M, CDUM(1), -1, IERR )
            LWORK_ZUNMBR_QLN_MN = INT( CDUM(1) )
*
            CALL YUNMBR( 'Q', 'L', 'N', N, N, N, CDUM(1), N, CDUM(1),   
     +                 CDUM(1), N, CDUM(1), -1, IERR )
...
*           Compute space preferred for each routine
     ...
     +               CDUM(1), CDUM(1), -1, IERR )
            LWORK_ZGEBRD_MN = INT( CDUM(1) )
*
            CALL YGEBRD( M, M, CDUM(1), M, DUM(1), DUM(1), CDUM(1),     
     +               CDUM(1), CDUM(1), -1, IERR )
            LWORK_ZGEBRD_MM = INT( CDUM(1) )
*
            CALL YGELQF( M, N, CDUM(1), M, CDUM(1), CDUM(1), -1,        
     +            IERR )
            LWORK_ZGELQF_MN = INT( CDUM(1) )
*
            CALL YUNGBR( 'P', M, N, M, CDUM(1), M, CDUM(1), CDUM(1),    
     +                -1, IERR )
            LWORK_ZUNGBR_P_MN = INT( CDUM(1) )
*
            CALL YUNGBR( 'P', N, N, M, CDUM(1), N, CDUM(1), CDUM(1),    
     +                -1, IERR )
            LWORK_ZUNGBR_P_NN = INT( CDUM(1) )
*
            CALL YUNGBR( 'Q', M, M, N, CDUM(1), M, CDUM(1), CDUM(1),    
     +                -1, IERR )
            LWORK_ZUNGBR_Q_MM = INT( CDUM(1) )
*
            CALL YUNGLQ( M, N, M, CDUM(1), M, CDUM(1), CDUM(1),         
     +           -1, IERR )
            LWORK_ZUNGLQ_MN = INT( CDUM(1) )
*
            CALL YUNGLQ( N, N, M, CDUM(1), N, CDUM(1), CDUM(1),         
     +           -1, IERR )
            LWORK_ZUNGLQ_NN = INT( CDUM(1) )
*
            CALL YUNMBR( 'P', 'R', 'C', M, M, M, CDUM(1), M, CDUM(1),   
     +                 CDUM(1), M, CDUM(1), -1, IERR )
            LWORK_ZUNMBR_PRC_MM = INT( CDUM(1) )
*
            CALL YUNMBR( 'P', 'R', 'C', M, N, M, CDUM(1), M, CDUM(1),   
     +                 CDUM(1), M, CDUM(1), -1, IERR )
            LWORK_ZUNMBR_PRC_MN = INT( CDUM(1) )
*
            CALL YUNMBR( 'P', 'R', 'C', N, N, M, CDUM(1), N, CDUM(1),   
     +                 CDUM(1), N, CDUM(1), -1, IERR )
            LWORK_ZUNMBR_PRC_NN = INT( CDUM(1) )
*
            CALL YUNMBR( 'Q', 'L', 'N', M, M, M, CDUM(1), M, CDUM(1),   
     +                 CDUM(1), M, CDUM(1), -1, IERR )
            LWORK_ZUNMBR_QLN_MM = INT( CDUM(1) )
*
            IF( N.GE.MNTHR1 ) THEN
...
*
      ...
      IF( ANRM.GT.ZERO .AND. ANRM.LT.SMLNUM ) THEN
         ISCL = 1
         CALL YLASCL( 'G', 0, 0, ANRM, SMLNUM, M, N, A, LDA, IERR )
      ELSE IF( ANRM.GT.BIGNUM ) THEN
         ISCL = 1
         CALL YLASCL( 'G', 0, 0, ANRM, BIGNUM, M, N, A, LDA, IERR )
      END IF
*
      IF( M.GE.N ) THEN
*
...
*              RWorkspace: need   0
*
               CALL YGEQRF( M, N, A, LDA, WORK( ITAU ),                 
     +      WORK( NWORK ),                       LWORK-NWORK+1, IERR )
*
*              Zero out below R
*
               CALL YLASET( 'L', N-1, N-1, CZERO, CZERO, A( 2, 1 ),     
     +                  LDA )
               IE = 1
               ITAUQ = 1
               ITAUP = ITAUQ + N
...
*              RWorkspace: need   N [e]
*
               CALL YGEBRD( N, N, A, LDA, S, RWORK( IE ),               
     +        WORK( ITAUQ ),                       WORK( ITAUP ), WORK( 
     +NWORK ), LWORK-NWORK+1,                       IERR )
               NRWORK = IE + N
*
...
*
               CALL EBDSDC( 'U', 'N', N, S, RWORK( IE ), DUM,1,DUM,1,   
...
*
               CALL YLACPY( 'U', N, N, A, LDA, WORK( IR ), LDWRKR )
...
*              RWorkspace: need   0
*
               CALL YUNGQR( M, N, N, A, LDA, WORK( ITAU ),              
     +         WORK( NWORK ), LWORK-NWORK+1, IERR )
               IE = 1
               ITAUQ = ITAU
               ITAUP = ITAUQ + N
...
*              RWorkspace: need   N [e]
*
               CALL YGEBRD( N, N, WORK( IR ), LDWRKR, S, RWORK( IE ),   
...
*
               ...
               IRVT = IRU + N*N
               NRWORK = IRVT + N*N
               CALL EBDSDC( 'U', 'I', N, S, RWORK( IE ),                
     +       RWORK( IRU ),                       N, RWORK( IRVT ), N, 
     +DUM, IDUM,                       RWORK( NRWORK ), IWORK, INFO )
*
...
*              RWorkspace: need   0
*
               CALL YLACP2( 'F', N, N, RWORK( IRU ), N, WORK( IU ),     
     +                  LDWRKU )
               CALL YUNMBR( 'Q', 'L', 'N', N, N, N, WORK( IR ),         
     +              LDWRKR,                       WORK( ITAUQ ), WORK( 
     +IU ), LDWRKU,                       WORK( NWORK ), LWORK-NWORK+1, 
     +IERR )
*
...
*              RWorkspace: need   0
*
               CALL YLACP2( 'F', N, N, RWORK( IRVT ), N, VT, LDVT )
               CALL YUNMBR( 'P', 'R', 'C', N, N, N, WORK( IR ),         
     +              LDWRKR,                       WORK( ITAUP ), VT, 
     +LDVT, WORK( NWORK ),                       LWORK-NWORK+1, IERR )
*
...
*
               DO 10 I = 1, M, LDWRKR
                  ...
                  CALL YGEMM( 'N', 'N', CHUNK, N, N, CONE, A( I, 1 ),   
     +                      LDA, WORK( IU ), LDWRKU, CZERO,             
     +            WORK( IR ), LDWRKR )
                  CALL YLACPY( 'F', CHUNK, N, WORK( IR ), LDWRKR,       
     +                   A( I, 1 ), LDA )
   10          CONTINUE
*
            ELSE IF( WNTQS ) THEN
...
*              RWorkspace: need   0
*
               CALL YLACP2( 'F', N, N, RWORK( IRU ), N, U, LDU )
               CALL YUNMBR( 'Q', 'L', 'N', N, N, N, WORK( IR ),         
     +              LDWRKR,                       WORK( ITAUQ ), U, LDU,
     + WORK( NWORK ),                       LWORK-NWORK+1, IERR )
*
*              Copy real matrix RWORK(IRVT) to complex matrix VT
...
*              RWorkspace: need   0
*
               CALL YLACPY( 'F', N, N, U, LDU, WORK( IR ), LDWRKR )
               CALL YGEMM( 'N', 'N', M, N, N, CONE, A, LDA,             
...
*              RWorkspace: need   0
*
               CALL YUNGQR( M, M, N, U, LDU, WORK( ITAU ),              
     +         WORK( NWORK ), LWORK-NWORK+1, IERR )
*
...
*              RWorkspace: need   0
*
               CALL YGEMM( 'N', 'N', M, N, N, CONE, U, LDU,             
     +         WORK( IU ),                      LDWRKU, CZERO, A, LDA )
*
*              Copy left singular vectors of A from A to U
*
               CALL YLACPY( 'F', M, N, A, LDA, U, LDU )
*
            END IF
*
         ELSE IF( M.GE.MNTHR2 ) THEN
...
*           YUNGBR and matrix multiplication to compute singular vectors
*
            IE = 1
            NRWORK = IE + N
            ITAUQ = 1
...
*           RWorkspace: need   N [e]
*
            CALL YGEBRD( M, N, A, LDA, S, RWORK( IE ), WORK( ITAUQ ),   
...
*              RWorkspace: need   N [e] + BDSPAC
*
               CALL EBDSDC( 'U', 'N', N, S, RWORK( IE ), DUM, 1,DUM,    
     +                   1,                       DUM, IDUM, RWORK( 
     +NRWORK ), IWORK, INFO )
            ELSE IF( WNTQO ) THEN
               IU = NWORK
...
*              RWorkspace: need   0
*
               CALL YLACPY( 'U', N, N, A, LDA, VT, LDVT )
               CALL YUNGBR( 'P', N, N, N, VT, LDVT, WORK( ITAUP ),      
     +                 WORK( NWORK ), LWORK-NWORK+1, IERR )
*
*              Generate Q in A
*              CWorkspace: need   2*N [tauq, taup] + N    [work]
...
*              RWorkspace: need   0
*
               CALL YUNGBR( 'Q', M, N, N, A, LDA, WORK( ITAUQ ),        
     +               WORK( NWORK ), LWORK-NWORK+1, IERR )
*
...
*
               CALL YLARCM( N, N, RWORK( IRVT ), N, VT, LDVT,           
     +            WORK( IU ), LDWRKU, RWORK( NRWORK ) )
               CALL YLACPY( 'F', N, N, WORK( IU ), LDWRKU, VT, LDVT )
*
...
*
               ...
               DO 20 I = 1, M, LDWRKU
                  ...
                  CALL YLACRM( CHUNK, N, A( I, 1 ), LDA,                
     +          RWORK( IRU ),                          N, WORK( IU ), 
     +LDWRKU, RWORK( NRWORK ) )
                  CALL YLACPY( 'F', CHUNK, N, WORK( IU ), LDWRKU,       
     +                   A( I, 1 ), LDA )
   20          CONTINUE
*
            ELSE IF( WNTQS ) THEN
...
*
               CALL YLACPY( 'L', M, N, A, LDA, U, LDU )
               CALL YUNGBR( 'Q', M, N, N, U, LDU, WORK( ITAUQ ),        
     +               WORK( NWORK ), LWORK-NWORK+1, IERR )
*
*              Perform bidiagonal SVD, computing left singular vectors
*              of bidiagonal matrix in RWORK(IRU) and computing right
...
*              RWorkspace: need   N [e] + N*N [RU] + N*N [RVT] + 2*N*N [rwork]
*
               CALL YLARCM( N, N, RWORK( IRVT ), N, VT, LDVT, A, LDA,   
     +                    RWORK( NRWORK ) )
               CALL YLACPY( 'F', N, N, A, LDA, VT, LDVT )
*
...
*
               NRWORK = IRVT
               CALL YLACRM( M, N, U, LDU, RWORK( IRU ), N, A, LDA,      
     +                 RWORK( NRWORK ) )
               CALL YLACPY( 'F', M, N, A, LDA, U, LDU )
            ELSE
*
...
*
               CALL YLACPY( 'L', M, N, A, LDA, U, LDU )
               CALL YUNGBR( 'Q', M, M, N, U, LDU, WORK( ITAUQ ),        
     +               WORK( NWORK ), LWORK-NWORK+1, IERR )
*
*              Perform bidiagonal SVD, computing left singular vectors
*              of bidiagonal matrix in RWORK(IRU) and computing right
...
*           Use YUNMBR to compute singular vectors
*
            IE = 1
            NRWORK = IE + N
            ITAUQ = 1
...
*
                  CALL YLASET( 'F', M, N, CZERO, CZERO, WORK( IU ),     
     +                     LDWRKU )
                  CALL YLACP2( 'F', N, N, RWORK( IRU ), N,              
     +            WORK( IU ),                          LDWRKU )
                  CALL YUNMBR( 'Q', 'L', 'N', M, N, N, A, LDA,          
     +                WORK( ITAUQ ), WORK( IU ), LDWRKU,                
     +          WORK( NWORK ), LWORK-NWORK+1, IERR )
                  CALL YLACPY( 'F', M, N, WORK( IU ), LDWRKU, A,        
     +                  LDA )
               ELSE
*
*                 Path 6o-slow
...
*              RWorkspace: need   N [e] + N*N [RU] + N*N [RVT]
*
               CALL YLASET( 'F', M, N, CZERO, CZERO, U, LDU )
               CALL YLACP2( 'F', N, N, RWORK( IRU ), N, U, LDU )
               CALL YUNMBR( 'Q', 'L', 'N', M, N, N, A, LDA,             
...
*
               CALL YLASET( 'F', M, M, CZERO, CZERO, U, LDU )
               IF( M.GT.N ) THEN
                  CALL YLASET( 'F', M-N, M-N, CZERO, CONE,              
     +            U( N+1, N+1 ), LDU )
               END IF
*
*              Copy real matrix RWORK(IRU) to complex matrix U
...
*              RWorkspace: need   0
*
               CALL YGELQF( M, N, A, LDA, WORK( ITAU ),                 
     +      WORK( NWORK ),                       LWORK-NWORK+1, IERR )
*
*              Zero out above L
*
               CALL YLASET( 'U', M-1, M-1, CZERO, CZERO, A( 1, 2 ),     
     +                  LDA )
               IE = 1
               ITAUQ = 1
               ITAUP = ITAUQ + M
...
*              RWorkspace: need   M [e]
*
               CALL YGEBRD( M, M, A, LDA, S, RWORK( IE ),               
     +        WORK( ITAUQ ),                       WORK( ITAUP ), WORK( 
     +NWORK ), LWORK-NWORK+1,                       IERR )
               NRWORK = IE + M
*
...
*
               CALL EBDSDC( 'U', 'N', M, S, RWORK( IE ), DUM,1,DUM,1,   
     +                    DUM, IDUM, RWORK( NRWORK ), IWORK, INFO )
*
            ELSE IF( WNTQO ) THEN
*
...
*
               CALL YLACPY( 'L', M, M, A, LDA, WORK( IL ), LDWRKL )
...
*              RWorkspace: need   0
*
               CALL YUNGLQ( M, N, M, A, LDA, WORK( ITAU ),              
     +         WORK( NWORK ), LWORK-NWORK+1, IERR )
               IE = 1
               ITAUQ = ITAU
               ITAUP = ITAUQ + M
...
*              RWorkspace: need   M [e]
*
               CALL YGEBRD( M, M, WORK( IL ), LDWRKL, S, RWORK( IE ),   
     +                    WORK( ITAUQ ), WORK( ITAUP ), WORK( NWORK ),  
     +                     LWORK-NWORK+1, IERR )
*
*              Perform bidiagonal SVD, computing left singular vectors
...
*
               ...
               IRVT = IRU + M*M
               NRWORK = IRVT + M*M
               CALL EBDSDC( 'U', 'I', M, S, RWORK( IE ),                
     +       RWORK( IRU ),                       M, RWORK( IRVT ), M, 
     +DUM, IDUM,                       RWORK( NRWORK ), IWORK, INFO )
*
*              Copy real matrix RWORK(IRU) to complex matrix WORK(IU)
...
*              RWorkspace: need   0
*
               CALL YLACP2( 'F', M, M, RWORK( IRU ), M, U, LDU )
               CALL YUNMBR( 'Q', 'L', 'N', M, M, M, WORK( IL ),         
     +              LDWRKL,                       WORK( ITAUQ ), U, LDU,
     + WORK( NWORK ),                       LWORK-NWORK+1, IERR )
*
...
*              RWorkspace: need   0
*
               CALL YLACP2( 'F', M, M, RWORK( IRVT ), M, WORK( IVT ),   
     +                    LDWKVT )
               CALL YUNMBR( 'P', 'R', 'C', M, M, M, WORK( IL ),         
     +              LDWRKL,                       WORK( ITAUP ), WORK( 
     +IVT ), LDWKVT,                       WORK( NWORK ), LWORK-NWORK+1,
     + IERR )
*
...
*
               DO 40 I = 1, N, CHUNK
                  BLK = MIN( N-I+1, CHUNK )
                  CALL YGEMM( 'N', 'N', M, BLK, M, CONE, WORK( IVT ),   
     +                      M,                         A( 1, I ), LDA, 
     +CZERO, WORK( IL ),                         LDWRKL )
                  CALL YLACPY( 'F', M, BLK, WORK( IL ), LDWRKL,         
     +                 A( 1, I ), LDA )
   40          CONTINUE
*
            ELSE IF( WNTQS ) THEN
...
*              RWorkspace: need   0
*
               CALL YLACP2( 'F', M, M, RWORK( IRVT ), M, VT, LDVT )
               CALL YUNMBR( 'P', 'R', 'C', M, M, M, WORK( IL ),         
     +              LDWRKL,                       WORK( ITAUP ), VT, 
     +LDVT, WORK( NWORK ),                       LWORK-NWORK+1, IERR )
*
...
*              RWorkspace: need   0
*
               CALL YLACPY( 'F', M, M, VT, LDVT, WORK( IL ), LDWRKL )
               CALL YGEMM( 'N', 'N', M, N, M, CONE, WORK( IL ),         
     +             LDWRKL,                      A, LDA, CZERO, VT, LDVT 
     +)
*
            ELSE IF( WNTQA ) THEN
*
...
*
               CALL YGELQF( M, N, A, LDA, WORK( ITAU ),                 
     +      WORK( NWORK ),                       LWORK-NWORK+1, IERR )
               CALL YLACPY( 'U', M, N, A, LDA, VT, LDVT )
*
*              Generate Q in VT
...
*              RWorkspace: need   0
*
               CALL YUNGLQ( N, N, M, VT, LDVT, WORK( ITAU ),            
     +           WORK( NWORK ), LWORK-NWORK+1, IERR )
*
...
*              RWorkspace: need   0
*
               CALL YGEMM( 'N', 'N', M, N, M, CONE, WORK( IVT ),        
     +              LDWKVT,                      VT, LDVT, CZERO, A, 
     +LDA )
*
*              Copy right singular vectors of A from A to VT
*
               CALL YLACPY( 'F', M, N, A, LDA, VT, LDVT )
*
            END IF
*
         ELSE IF( N.GE.MNTHR2 ) THEN
...
*              RWorkspace: need   M [e] + BDSPAC
*
               CALL EBDSDC( 'L', 'N', M, S, RWORK( IE ), DUM,1,DUM,1,   
     +                    DUM, IDUM, RWORK( NRWORK ), IWORK, INFO )
            ELSE IF( WNTQO ) THEN
               IRVT = NRWORK
               IRU = IRVT + M*M
...
*              RWorkspace: need   0
*
               CALL YUNGBR( 'P', M, N, M, A, LDA, WORK( ITAUP ),        
     +               WORK( NWORK ), LWORK-NWORK+1, IERR )
*
               LDWKVT = M
...
*
               CALL YLACRM( M, M, U, LDU, RWORK( IRU ), M,              
     +         WORK( IVT ),                       LDWKVT, RWORK( NRWORK 
     +) )
               CALL YLACPY( 'F', M, M, WORK( IVT ), LDWKVT, U, LDU )
*
...
*
               ...
               DO 50 I = 1, N, CHUNK
                  BLK = MIN( N-I+1, CHUNK )
                  CALL YLARCM( M, BLK, RWORK( IRVT ), M, A( 1, I ),     
     +                     LDA,                          WORK( IVT ), 
     +LDWKVT, RWORK( NRWORK ) )
                  CALL YLACPY( 'F', M, BLK, WORK( IVT ), LDWKVT,        
     +                  A( 1, I ), LDA )
   50          CONTINUE
            ELSE IF( WNTQS ) THEN
*
...
*
               CALL YLACPY( 'U', M, N, A, LDA, VT, LDVT )
               CALL YUNGBR( 'P', M, N, M, VT, LDVT, WORK( ITAUP ),      
     +                 WORK( NWORK ), LWORK-NWORK+1, IERR )
*
*              Perform bidiagonal SVD, computing left singular vectors
*              of bidiagonal matrix in RWORK(IRU) and computing right
...
*              RWorkspace: need   M [e] + M*M [RVT] + M*M [RU] + 2*M*M [rwork]
*
               CALL YLACRM( M, M, U, LDU, RWORK( IRU ), M, A, LDA,      
     +                 RWORK( NRWORK ) )
               CALL YLACPY( 'F', M, M, A, LDA, U, LDU )
*
*              Multiply real matrix RWORK(IRVT) by P**H in VT,
*              storing the result in A, copying to VT
*              CWorkspace: need   0
...
*
               NRWORK = IRU
               CALL YLARCM( M, N, RWORK( IRVT ), M, VT, LDVT, A, LDA,   
     +                    RWORK( NRWORK ) )
               CALL YLACPY( 'F', M, N, A, LDA, VT, LDVT )
            ELSE
*
...
*
               CALL YLACPY( 'U', M, N, A, LDA, VT, LDVT )
               CALL YUNGBR( 'P', N, N, M, VT, LDVT, WORK( ITAUP ),      
     +                 WORK( NWORK ), LWORK-NWORK+1, IERR )
*
*              Perform bidiagonal SVD, computing left singular vectors
*              of bidiagonal matrix in RWORK(IRU) and computing right
...
*                 WORK( IVT ) is M by N
*
                  CALL YLASET( 'F', M, N, CZERO, CZERO, WORK( IVT ),    
     +                      LDWKVT )
                  NWORK = IVT + LDWKVT*N
               ELSE
*
...
*
                  CALL YLACP2( 'F', M, M, RWORK( IRVT ), M,             
     +             WORK( IVT ),                          LDWKVT )
                  CALL YUNMBR( 'P', 'R', 'C', M, N, M, A, LDA,          
     +                WORK( ITAUP ), WORK( IVT ), LDWKVT,               
     +           WORK( NWORK ), LWORK-NWORK+1, IERR )
                  CALL YLACPY( 'F', M, N, WORK( IVT ), LDWKVT, A,       
     +                   LDA )
               ELSE
*
*                 Path 6to-slow
...
*
                  ...
                  DO 60 I = 1, N, CHUNK
                     BLK = MIN( N-I+1, CHUNK )
                     CALL YLARCM( M, BLK, RWORK( IRVT ), M, A( 1,       
     +                      I ),                             LDA, WORK( 
     +IVT ), LDWKVT,                             RWORK( NRWORK ) )
                     CALL YLACPY( 'F', M, BLK, WORK( IVT ), LDWKVT,     
     +                        A( 1, I ), LDA )
   60             CONTINUE
               END IF
...
*              RWorkspace: need   M [e] + M*M [RVT]
*
               CALL YLASET( 'F', M, N, CZERO, CZERO, VT, LDVT )
               CALL YLACP2( 'F', M, M, RWORK( IRVT ), M, VT, LDVT )
               CALL YUNMBR( 'P', 'R', 'C', M, N, M, A, LDA,             
     +          WORK( ITAUP ), VT, LDVT, WORK( NWORK ),                 
     +      LWORK-NWORK+1, IERR )
...
*
               CALL YLASET( 'F', N, N, CZERO, CONE, VT, LDVT )
*
*              Copy real matrix RWORK(IRVT) to complex matrix VT
*              Overwrite VT by right singular vectors of A
*              CWorkspace: need   2*M [tauq, taup] + N    [work]
...
*
     ...
     +ANRM, MINMN, 1, S, MINMN,                    IERR )
         IF( INFO.NE.0 .AND. ANRM.GT.BIGNUM )       CALL ELASCL( 'G', 0,
     + 0, BIGNUM, ANRM, MINMN-1, 1,                    RWORK( IE ), 
     +MINMN, IERR )
         IF( ANRM.LT.SMLNUM )       CALL ELASCL( 'G', 0, 0, SMLNUM, 
     +ANRM, MINMN, 1, S, MINMN,                    IERR )
         IF( INFO.NE.0 .AND. ANRM.LT.SMLNUM )       CALL ELASCL( 'G', 0,
     + 0, SMLNUM, ANRM, MINMN-1, 1,                    RWORK( IE ), 
     +MINMN, IERR )
      END IF
*
