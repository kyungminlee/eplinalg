...
*  ===========
*
*       SUBROUTINE EGEQR( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> EGEQR computes a QR factorization of a real M-by-N matrix A:
*>
*>    A = Q * ( R ),
*>            ( 0 )
*>
...
*     .. External Subroutines ..
      EXTERNAL           ELATSQR, EGEQRT, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN( M, N ).GT.0 ) THEN
        MB = ILAENV_EP( 1, 'EGEQR ', ' ', M, N, 1, -1 )
        NB = ILAENV_EP( 1, 'EGEQR ', ' ', M, N, 2, -1 )
      ELSE
        MB = M
        NB = 1
      END IF
...
*
      IF( ( M.LE.N ) .OR. ( MB.LE.N ) .OR. ( MB.GE.M ) ) THEN
        CALL EGEQRT( M, N, NB, A, LDA, T( 6 ), NB, WORK, INFO )
      ELSE
        CALL ELATSQR( M, N, MB, NB, A, LDA, T( 6 ), NB, WORK,           
     +      LWORK, INFO )
      END IF
*
      WORK( 1 ) = LWREQ
