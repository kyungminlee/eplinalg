*> \brief \b ELAQP3RK computes a step of truncated QR factorization with column pivoting of a real m-by-n matrix A using Level 3 BLAS and overwrites a real m-by-nrhs matrix B with Q**T * B.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download ELAQP3RK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/elaqp3rk.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/elaqp3rk.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/elaqp3rk.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*      SUBROUTINE ELAQP3RK( M, N, NRHS, IOFFSET, NB, ABSTOL,
...
*     .. Array Arguments ..
*      INTEGER            IWORK( * ), JPIV( * )
*      REAL(KIND=10)   A( LDA, * ), AUXV( * ), F( LDF, * ), TAU( * ),
*     $                   VN1( * ), VN2( * )
*     ..
*
*
...
*> \verbatim
*>
*> ELAQP3RK computes a step of truncated QR factorization with column
...
*>          main routine EGEQP3RK. 1 <= KP1 <= N_orig.
*> \endverbatim
*>
*> \param[in] MAXC2NRM
*> \verbatim
...
*>          matrix A_orig computed in the main routine EGEQP3RK.
*>          MAXC2NRM >= 0.
*> \endverbatim
*>
*> \param[in,out] A
...
*> \param[out] TAU
*> \verbatim
*>          TAU is REAL(KIND=10) array, dimension (min(M-IOFFSET,N))
...
*> \param[out] AUXV
*> \verbatim
*>          AUXV is REAL(KIND=10) array, dimension (NB)
*>          Auxiliary vector.
*> \endverbatim
*>
*> \param[out] F
*> \verbatim
*>          F is REAL(KIND=10) array, dimension (LDF,NB)
*>          Matrix F**T = L*(Y**T)*A.
*> \endverbatim
*>
*> \param[in] LDF
...
*     .. Local Scalars ..
     ...
     $                   LSTICC, KP, I, IF
      REAL(KIND=10)   AIK, HUGEVAL, TEMP, TEMP2, TOL3Z
*     ..
*     .. External Subroutines ..
      EXTERNAL           EGEMM, EGEMV, ELARFG, ESWAP
*     ..
*     .. Intrinsic Functions ..
...
*     .. External Functions ..
      ...
      EXTERNAL           EISNAN, ELAMCH, IEAMAX, ENRM2
*     ..
...
*                               A(I+1:M,1:KB) * F(N+1:N+NRHS,1:KB)**T.
               ...
               IF( NRHS.GT.0 .AND. KB.LT.(M-IOFFSET) ) THEN
                  CALL EGEMM( 'No transpose', 'Transpose',              
...
*
         IF( KP.NE.K ) THEN
            CALL ESWAP( M, A( 1, KP ), 1, A( 1, K ), 1 )
            CALL ESWAP( K-1, F( KP, 1 ), LDF, F( K, 1 ), LDF )
            VN1( KP ) = VN1( K )
            VN2( KP ) = VN2( K )
            ITEMP = JPIV( KP )
            JPIV( KP ) = JPIV( K )
...
*
         IF( K.GT.1 ) THEN
            CALL EGEMV( 'No transpose', M-I+1, K-1, -ONE, A( I, 1 ),    
...
*
         IF( I.LT.M ) THEN
            CALL ELARFG( M-I+1, A( I, K ), A( I+1, K ), 1, TAU( K ) )
         ELSE
            TAU( K ) = ZERO
         END IF
*
...
*        since ELARFG cannot produce TAU(K) or Householder vector
*        below the diagonal containing Inf. Only BETA on the diagonal,
*        returned by ELARFG can contain Inf, which requires
*        TAU(K) to contain NaN. Therefore, this case of generating Inf
*        by ELARFG is covered by checking TAU(K) for NaN.
*
         IF( EISNAN( TAU(K) ) ) THEN
*
            DONE = .TRUE.
*
*           Set KB, the number of factorized partial columns
...
*
            IF( NRHS.GT.0 .AND. KB.LT.(M-IOFFSET) ) THEN
               CALL EGEMM( 'No transpose', 'Transpose',                
     +M-IF, NRHS, KB, -ONE, A( IF+1, 1 ), LDA,                F( N+1, 1 
...
*
         IF( K.LT.N+NRHS ) THEN
            CALL EGEMV( 'Transpose', M-I+1, N+NRHS-K,                   
     +TAU( K ), A( I, K+1 ), LDA, A( I, K ), 1,                   ZERO, 
     +F( K+1, K ), 1 )
         END IF
*
...
*
         IF( K.GT.1 ) THEN
            CALL EGEMV( 'Transpose', M-I+1, K-1, -TAU( K ),             
     +      A( I, 1 ), LDA, A( I, K ), 1, ZERO,                   AUXV( 
     +1 ), 1 )
*
            CALL EGEMV( 'No transpose', N+NRHS, K-1, ONE,               
     +    F( 1, 1 ), LDF, AUXV( 1 ), 1, ONE,                   F( 1, K 
     +), 1 )
         END IF
*
*        ===============================================================
...
*
         IF( K.LT.N+NRHS ) THEN
            CALL EGEMV( 'No transpose', N+NRHS-K, K, -ONE,              
     +     F( K+1, 1 ), LDF, A( I, 1 ), LDA, ONE,                   A( 
     +I, K+1 ), LDA )
         END IF
*
         A( I, K ) = AIK
...
*
      IF( KB.LT.MINMNUPDT ) THEN
         CALL EGEMM( 'No transpose', 'Transpose',          M-IF, 
     +N+NRHS-KB, KB, -ONE, A( IF+1, 1 ), LDA,          F( KB+1, 1 ), 
     +LDF, ONE, A( IF+1, KB+1 ), LDA )
      END IF
*
...
*        ENRM2 does not fail on vectors with norm below the value of
*        SQRT(ELAMCH('S'))
*
         VN1( LSTICC ) = ENRM2( M-IF, A( IF+1, LSTICC ), 1 )
