...
*
*> \htmlonly
*> Download EBDSVDX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ebdsvdx.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ebdsvdx.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ebdsvdx.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*     SUBROUTINE EBDSVDX( UPLO, JOBZ, RANGE, N, D, E, VL, VU, IL, IU,
...
*> \verbatim
*>
*>  EBDSVDX computes the singular value decomposition (SVD) of a real
...
*>  and superdiagonal E = [ e_1 e_2 ... e_N-1 ], EBDSVDX computes the
...
*>  descending order (as in EGESVD/EGESDD) or b) compute s,v and reorder
*>  the values (and corresponding vectors). EBDSVDX implements a) by
*>  calling ESTEVX (bisection plus inverse iteration, to be replaced
...
*> \param[in] E
*> \verbatim
*>          E is REAL(KIND=10) array, dimension (max(1,N-1))
...
*> \param[out] Z
*> \verbatim
*>          Z is REAL(KIND=10) array, dimension (2*N,K)
...
*> \param[out] WORK
*> \verbatim
*>          WORK is REAL(KIND=10) array, dimension (14*N)
*> \endverbatim
*>
*> \param[out] IWORK
*> \verbatim
...
*>          of the eigenvectors that failed to converge in ESTEVX.
*> \endverbatim
*>
*> \param[out] INFO
*> \verbatim
...
*>                   in ESTEVX. The indices of the eigenvectors
*>                   (as returned by ESTEVX) are stored in the
*>                   array IWORK.
...
*
*     .. Parameters ..
      REAL(KIND=10)   ZERO, ONE, TEN, HNDRD, MEIGTH
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10, TEN = 
     +10.0E0_10,                      HNDRD = 100.0E0_10, MEIGTH = 
     +-0.1250E0_10 )
      REAL(KIND=10)   FUDGE
      PARAMETER          ( FUDGE = 2.0E0_10 )
*     ..
*     .. Local Scalars ..
      CHARACTER          RNGVX
     ...
     $                   NTGK, NRU, NRV, NSL
      REAL(KIND=10)   ABSTOL, EPS, EMIN, MU, NRMU, NRMV, ORTOL, SMAX,   
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            IEAMAX
      REAL(KIND=10)   EDOT, ELAMCH, ENRM2
      EXTERNAL           IEAMAX, LSAME, EAXPY, EDOT, ELAMCH,            
     +        ENRM2
*     ..
*     .. External Subroutines ..
      EXTERNAL           ESTEVX, ECOPY, ELASET, ESCAL, ESWAP,           
     +         XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      END IF
*
      ABSTOL = 2*ELAMCH( 'Safe Minimum' )
      ULP = ELAMCH( 'Precision' )
      EPS = ELAMCH( 'Epsilon' )
      SQRT2 = SQRT( 2.0E0_10 )
      ORTOL = SQRT( ULP )
*
*     Criterion for splitting is taken from EBDSQR when singular
...
*
*     Pointers for arrays used by ESTEVX.
*
      IDTGK = 1
      IETGK = IDTGK + N*2
      ITEMP = IETGK + N*2
      ...
      IIWORK = IIFAIL + N*2
*
*     Set RNGVX, which corresponds to RANGE for ESTEVX in TGK mode.
...
*
         RNGVX = 'I'
         IF( WANTZ ) CALL ELASET( 'F', N*2, N+1, ZERO, ZERO, Z, LDZ )
      ELSE IF( VALSV ) THEN
*
...
*
         ...
         CALL ECOPY( N, D, 1, WORK( IETGK ), 2 )
         CALL ECOPY( N-1, E, 1, WORK( IETGK+1 ), 2 )
         CALL ESTEVX( 'N', 'V', N*2, WORK( IDTGK ), WORK( IETGK ),      
     ...
     +IWORK( IIFAIL ), INFO )
         IF( NS.EQ.0 ) THEN
            RETURN
         ELSE
            IF( WANTZ ) CALL ELASET( 'F', N*2, NS, ZERO, ZERO, Z,       
     +    LDZ )
         END IF
      ELSE IF( INDSV ) THEN
*
...
*        values, therefore mimicking ESTEBZ, where
*
...
*
         ...
         CALL ECOPY( N, D, 1, WORK( IETGK ), 2 )
         CALL ECOPY( N-1, E, 1, WORK( IETGK+1 ), 2 )
         CALL ESTEVX( 'N', 'I', N*2, WORK( IDTGK ), WORK( IETGK ),      
...
*
*        If VLTGK=VUTGK, ESTEVX returns an error message,
*        so if needed we change VUTGK slightly.
*
         IF( VLTGK.EQ.VUTGK ) VLTGK = VLTGK - TOL
*
         IF( WANTZ ) CALL ELASET( 'F', N*2, IU-IL+1, ZERO, ZERO, Z,     
     +   LDZ)
      END IF
*
...
*                        small, additional eigenvector is needed.
                  ...
                  END IF
*
*                 Workspace needed by ESTEVX:
*                 WORK( ITEMP: ): 2*5*NTGK
*                 IWORK( 1: ): 2*6*NTGK
*
                  CALL ESTEVX( JOBZ, RNGVX, NTGK,                       
     +   WORK( IDTGK+ISPLT-1 ),                          WORK( 
     +IETGK+ISPLT-1 ), VLTGK, VUTGK,                          ILTGK, 
     +IUTGK, ABSTOL, NSL, S( ISBEG ),                          Z( IROWZ,
     +ICOLZ ), LDZ, WORK( ITEMP ),                          IWORK( 
     +IIWORK ), IWORK( IIFAIL ),                          INFO )
                  IF( INFO.NE.0 ) THEN
*                    Exit with the error code from ESTEVX.
                     RETURN
                  END IF
...
*
                     DO I = 0, MIN( NSL-1, NRU-1 )
                        NRMU = ENRM2( NRU, Z( IROWU, ICOLZ+I ), 2 )
                        IF( NRMU.EQ.ZERO ) THEN
                           INFO = N*2 + 1
                           RETURN
                        END IF
                        CALL ESCAL( NRU, ONE/NRMU,                      
     +         Z( IROWU,ICOLZ+I ), 2 )
                        IF( NRMU.NE.ONE .AND.
     $                      ABS( NRMU-ORTOL )*SQRT2.GT.ONE )
     $                      THEN
                           DO J = 0, I-1
                              ZJTJI = -EDOT( NRU, Z( IROWU,             
     +                           ICOLZ+J ),                             
     +           2, Z( IROWU, ICOLZ+I ), 2 )
                              CALL EAXPY( NRU, ZJTJI,                   
     +                  Z( IROWU, ICOLZ+J ), 2,                         
     +            Z( IROWU, ICOLZ+I ), 2 )
                           END DO
                           NRMU = ENRM2( NRU, Z( IROWU, ICOLZ+I ),      
     +                              2 )
                           CALL ESCAL( NRU, ONE/NRMU,                   
     +               Z( IROWU,ICOLZ+I ), 2 )
                        END IF
                     ...
                     DO I = 0, MIN( NSL-1, NRV-1 )
                        NRMV = ENRM2( NRV, Z( IROWV, ICOLZ+I ), 2 )
                        IF( NRMV.EQ.ZERO ) THEN
                           INFO = N*2 + 1
                           RETURN
                        END IF
                        CALL ESCAL( NRV, -ONE/NRMV,                     
     +          Z( IROWV,ICOLZ+I ), 2 )
                        IF( NRMV.NE.ONE .AND.
     $                      ABS( NRMV-ORTOL )*SQRT2.GT.ONE )
     $                      THEN
                           DO J = 0, I-1
                              ZJTJI = -EDOT( NRV, Z( IROWV,             
     +                           ICOLZ+J ),                             
     +           2, Z( IROWV, ICOLZ+I ), 2 )
                              CALL EAXPY( NRU, ZJTJI,                   
     +                  Z( IROWV, ICOLZ+J ), 2,                         
     +            Z( IROWV, ICOLZ+I ), 2 )
                           END DO
                           NRMV = ENRM2( NRV, Z( IROWV, ICOLZ+I ),      
     +                              2 )
                           CALL ESCAL( NRV, ONE/NRMV,                   
     +               Z( IROWV,ICOLZ+I ), 2 )
                        END IF
...
*
      DO I = 1, NS-1
         ...
         IF( K.NE.NS+1-I ) THEN
            S( K ) = S( NS+1-I )
            S( NS+1-I ) = SMIN
            IF( WANTZ ) CALL ESWAP( N*2, Z( 1,K ), 1, Z( 1,NS+1-I ),    
     +       1 )
         END IF
      END DO
*
...
*
      IF( WANTZ ) THEN
      DO I = 1, NS
         CALL ECOPY( N*2, Z( 1,I ), 1, WORK, 1 )
         IF( LOWER ) THEN
            CALL ECOPY( N, WORK( 2 ), 2, Z( N+1,I ), 1 )
            CALL ECOPY( N, WORK( 1 ), 2, Z( 1  ,I ), 1 )
         ELSE
            CALL ECOPY( N, WORK( 2 ), 2, Z( 1  ,I ), 1 )
            CALL ECOPY( N, WORK( 1 ), 2, Z( N+1,I ), 1 )
         END IF
      END DO
      END IF
*
