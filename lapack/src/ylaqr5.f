*> \brief \b YLAQR5 performs a single small-bulge multi-shift QR sweep.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download YLAQR5 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ylaqr5.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ylaqr5.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ylaqr5.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YLAQR5( WANTT, WANTZ, KACC22, N, KTOP, KBOT, NSHFTS, S,
...
*       ..
*       .. Array Arguments ..
*       COMPLEX(KIND=10)         H( LDH, * ), S( * ), U( LDU, * ), V( LDV, * ),
...
*> \verbatim
*>
*>    YLAQR5, called by YLAQR0, performs a
...
*>             orthogonal updates.
*>        = 0: YLAQR5 does not accumulate reflections and does not
*>             use matrix-matrix multiply to update far-from-diagonal
*>             matrix entries.
*>        = 1: YLAQR5 accumulates reflections and uses matrix-matrix
...
*> \param[in,out] S
*> \verbatim
*>          S is COMPLEX(KIND=10) array, dimension (NSHFTS)
...
*> \param[in,out] Z
*> \verbatim
*>          Z is COMPLEX(KIND=10) array, dimension (LDZ,IHIZ)
...
*> \param[out] V
*> \verbatim
*>          V is COMPLEX(KIND=10) array, dimension (LDV,NSHFTS/2)
*> \endverbatim
*>
*> \param[in] LDV
*> \verbatim
...
*> \param[out] U
*> \verbatim
*>          U is COMPLEX(KIND=10) array, dimension (LDU,2*NSHFTS)
*> \endverbatim
*>
*> \param[in] LDU
*> \verbatim
...
*> \param[out] WV
*> \verbatim
*>          WV is COMPLEX(KIND=10) array, dimension (LDWV,2*NSHFTS)
*> \endverbatim
*>
*> \param[in] LDWV
*> \verbatim
...
*> \param[out] WH
*> \verbatim
*>          WH is COMPLEX(KIND=10) array, dimension (LDWH,NH)
*> \endverbatim
*>
*> \param[in] LDWH
*> \verbatim
...
*>
*  =====================================================================
      SUBROUTINE YLAQR5( WANTT, WANTZ, KACC22, N, KTOP, KBOT, NSHFTS,   
     +                 S,                    H, LDH, ILOZ, IHIZ, Z, LDZ,
...
*     ..
*     .. Local Scalars ..
      COMPLEX(KIND=10)         ALPHA, BETA, CDUM, REFSUM, T1, T2, T3
      REAL(KIND=10)   H11, H12, H21, H22, SAFMAX, SAFMIN, SCL,          
...
*     ..
*     .. Local Arrays ..
      COMPLEX(KIND=10)         VT( 3 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           YGEMM, YLACPY, YLAQR1, YLARFG, YLASET,         
     +           YTRMM
*     ..
*     .. Statement Functions ..
      REAL(KIND=10)   CABS1
...
*
         NDCOL = INCOL + KDU
         IF( ACCUM )       CALL YLASET( 'ALL', KDU, KDU, ZERO, ONE, U, 
     +LDU )
*
...
*
               K = KRCOL + 2*( M22-1 )
               IF( K.EQ.KTOP-1 ) THEN
                  CALL YLAQR1( 2, H( K+1, K+1 ), LDH, S( 2*M22-1 ),     
     +                     S( 2*M22 ), V( 1, M22 ) )
                  BETA = V( 1, M22 )
                  CALL YLARFG( 2, BETA, V( 2, M22 ), 1, V( 1, M22 ) )
               ELSE
                  BETA = H( K+1, K )
                  V( 2, M22 ) = H( K+2, K )
                  CALL YLARFG( 2, BETA, V( 2, M22 ), 1, V( 1, M22 ) )
...
*
            DO 80 M = MBOT, MTOP, -1
               K = KRCOL + 2*( M-1 )
               IF( K.EQ.KTOP-1 ) THEN
                  CALL YLAQR1( 3, H( KTOP, KTOP ), LDH, S( 2*M-1 ),     
     +                     S( 2*M ), V( 1, M ) )
                  ALPHA = V( 1, M )
                  CALL YLARFG( 3, ALPHA, V( 2, M ), 1, V( 1, M ) )
               ELSE
*
...
*
                  ...
                  V( 2, M ) = H( K+2, K )
                  V( 3, M ) = H( K+3, K )
                  CALL YLARFG( 3, BETA, V( 2, M ), 1, V( 1, M ) )
*
...
*
                     CALL YLAQR1( 3, H( K+1, K+1 ), LDH, S( 2*M-1 ),    
     +                         S( 2*M ), VT )
                     ALPHA = VT( 1 )
                     CALL YLARFG( 3, ALPHA, VT( 2 ), 1, VT( 1 ) )
                     T1 = CONJG( VT( 1 ) )
                     T2 = T1*VT( 2 )
                     T3 = T1*VT( 3 )
...
*
               ...
               T2 = T1*V( 2, M )
               T3 = T1*V( 3, M )
               REFSUM = H( K+1, K+1 )                   + CONJG( V( 2, 
     +M ) )*H( K+2, K+1 )                   + CONJG( V( 3, M ) )*H( K+3,
     + K+1 )
...
*
            DO 150 JCOL = MIN( NDCOL, KBOT ) + 1, JBOT, NH
               ...
               CALL YGEMM( 'C', 'N', NU, JLEN, NU, ONE, U( K1, K1 ),    
     +                  LDU, H( INCOL+K1, JCOL ), LDH, ZERO, WH,        
     +              LDWH )
               CALL YLACPY( 'ALL', NU, JLEN, WH, LDWH,                  
     +     H( INCOL+K1, JCOL ), LDH )
  150       CONTINUE
*
*           ==== Vertical multiply ====
*
            DO 160 JROW = JTOP, MAX( KTOP, INCOL ) - 1, NV
               ...
               CALL YGEMM( 'N', 'N', JLEN, NU, NU, ONE,                 
     +     H( JROW, INCOL+K1 ), LDH, U( K1, K1 ),                      
     +LDU, ZERO, WV, LDWV )
               CALL YLACPY( 'ALL', JLEN, NU, WV, LDWV,                  
     +     H( JROW, INCOL+K1 ), LDH )
  160       CONTINUE
*
