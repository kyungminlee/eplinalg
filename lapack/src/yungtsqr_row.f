...
*
*> \htmlonly
*> Download YUNGTSQR_ROW + dependencies
...
*  ===========
*
*       SUBROUTINE YUNGTSQR_ROW( M, N, MB, NB, A, LDA, T, LDT, WORK,
*      $                         LWORK, INFO )
*       IMPLICIT NONE
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> YUNGTSQR_ROW generates an M-by-N complex matrix Q_out with
*> orthonormal columns from the output of YLATSQR. These N orthonormal
*> columns are the first N columns of a product of complex unitary
*> matrices Q(k)_in of order M, which are returned by YLATSQR in
*> a special format.
*>
*>      Q_out = first_N_columns_of( Q(1)_in * Q(2)_in * ... * Q(k)_in ).
*>
*> The input matrices Q(k)_in are stored in row and column blocks in A.
*> See the documentation of YLATSQR for more details on the format of
*> Q(k)_in, where each Q(k)_in is represented by block Householder
*> transformations. This routine calls an auxiliary routine YLARFB_GETT,
*> where the computation is performed on each individual block. The
*> algorithm first sweeps NB-sized column blocks from the right to left
*> starting in the bottom row block and continues to the top row block
*> (hence _ROW in the routine name). This sweep is in reverse order of
*> the order in which YLATSQR generates the output blocks.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*>             diagonal are not stored). See YLATSQR for more details.
*>
*>          On exit:
*>
...
*>          column blocks. See YLATSQR for more details on the format
*>          of T.
*> \endverbatim
*>
*> \param[in] LDT
...
*     ..
*     .. Local Arrays ..
      COMPLEX(KIND=10)         DUMMY( 1, 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           YLARFB_GETT, YLASET, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      CALL YLASET('U', M, N, CZERO, CONE, A, LDA )
*
...
*
               CALL YLARFB_GETT( 'I', IMB, N-KB+1, KNB,                 
...
*
            CALL YLARFB_GETT( 'N', 0, N-KB+1, KNB,                      
     +   T( 1, KB ), LDT, A( KB, KB ), LDA,                         
     +DUMMY( 1, 1 ), 1, WORK, KNB )
         ELSE
            CALL YLARFB_GETT( 'N', MB1-KB-KNB+1, N-KB+1, KNB,           
     +              T( 1, KB ), LDT, A( KB, KB ), LDA,                  
