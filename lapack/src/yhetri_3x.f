...
*
*> \htmlonly
*> Download YHETRI_3X + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/yhetri_3x.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/yhetri_3x.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/yhetri_3x.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YHETRI_3X( UPLO, N, A, LDA, E, IPIV, WORK, NB, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO
...
*       .. Array Arguments ..
*       INTEGER            IPIV( * )
*       COMPLEX(KIND=10)         A( LDA, * ),  E( * ), WORK( N+NB+1, * )
*       ..
*
*
*> \par Purpose:
...
*>
*> \verbatim
*> YHETRI_3X computes the inverse of a complex Hermitian indefinite
*> matrix A using the factorization computed by YHETRF_RK or ZHETRF_BK:
*>
...
*>          factors U or L as computed by YHETRF_RK and ZHETRF_BK:
...
*>          as determined by YHETRF_RK or ZHETRF_BK.
*> \endverbatim
*>
*> \param[out] WORK
*> \verbatim
*>          WORK is COMPLEX(KIND=10) array, dimension (N+NB+1,NB+3).
*> \endverbatim
*>
*> \param[in] NB
*> \verbatim
...
*
*  =====================================================================
      SUBROUTINE YHETRI_3X( UPLO, N, A, LDA, E, IPIV, WORK, NB,         
     +              INFO )
*
...
*     .. Local Scalars ..
      ...
      REAL(KIND=10)   AK, AKP1, T
      COMPLEX(KIND=10)         AKKP1, D, U01_I_J, U01_IP1_J, U11_I_J,   
     +                 U11_IP1_J
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*     .. External Subroutines ..
      EXTERNAL           YGEMM, YHESWAPR, YTRTRI, YTRMM,                
     +    XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
...
*
         CALL YTRTRI( UPLO, 'U', N, A, LDA, INFO )
*
...
*
         ...
         DO WHILE( K.LE.N )
            IF( IPIV( K ).GT.0 ) THEN
*              1 x 1 diagonal NNB
               WORK( K, INVD ) = ONE / REAL( A( K, K ) , KIND=10)
               WORK( K, INVD+1 ) = CZERO
            ELSE
*              2 x 2 diagonal NNB
               T = ABS( WORK( K+1, 1 ) )
...
*
            CALL YGEMM( 'C', 'N', NNB, NNB, CUT, CONE, A( 1, CUT+1 ),   
...
*
         DO I = 1, N
             IP = ABS( IPIV( I ) )
             IF( IP.NE.I ) THEN
                IF (I .LT. IP) CALL YHESWAPR( UPLO, N, A, LDA, I ,      
     +          IP )
                IF (I .GT. IP) CALL YHESWAPR( UPLO, N, A, LDA, IP ,     
     +           I )
             END IF
         END DO
*
...
*           L11**H * invD1 * L11 -> L11
*
            CALL YTRMM( 'L', UPLO, 'C', 'U', NNB, NNB, CONE,            
...
*
               CALL YGEMM( 'C', 'N', NNB, NNB, N-NNB-CUT, CONE,         
...
*              L01 =  L22**H * invD2 * L21
*
               CALL YTRMM( 'L', UPLO, 'C', 'U', N-NNB-CUT, NNB, CONE,   
