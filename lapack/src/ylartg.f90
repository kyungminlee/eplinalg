!> \brief \b YLARTG generates a plane rotation with real cosine and complex sine.
!
!  =========== DOCUMENTATION ===========
!
...
!  ===========
!
!       SUBROUTINE YLARTG( F, G, C, S, R )
!
!       .. Scalar Arguments ..
!       REAL(KIND=10)              C
!       COMPLEX(KIND=10)           F, G, R, S
!       ..
!
!> \par Purpose:
!  =============
...
!> identical to those returned by ELARTG.
!>
!> The algorithm used to compute these quantities incorporates scaling
!> to avoid overflow or underflow in computing the square root of the
!> sum of squares.
!>
!> This is the same routine YROTG fom BLAS1, except that
!> F and G are unchanged on return.
!>
!> Below, wp=>dp stands for REAL(KIND=10) from LA_CONSTANTS module.
!> \endverbatim
!
!  Arguments:
!  ==========
...
!> \param[in] F
!> \verbatim
!>          F is COMPLEX(KIND=10)
!>          The first component of vector to be rotated.
!> \endverbatim
!>
!> \param[in] G
!> \verbatim
!>          G is COMPLEX(KIND=10)
...
subroutine YLARTG( f, g, c, s, r )
   use LA_CONSTANTS_EP, &
   only: wp=>ep, zero=>ezero, one=>eone, two=>etwo, yzero=>yzero, &
         safmin=>esafmin, safmax=>esafmax
!
...
!  .. Statement Function definitions ..
   ABSSQ( t ) = REAL( t , KIND=10)**2 + aimag( t )**2
!  ..
!  .. Constants ..
   rtmin = sqrt( safmin )
!  ..
...
!
   if( g == czero ) then
      ...
      c = zero
      if( REAL(g, KIND=10) == zero ) then
         r = abs(aimag(g))
         s = conjg( g ) / r
      elseif( aimag(g) == zero ) then
         r = abs(REAL(g, KIND=10))
         s = conjg( g ) / r
      else
         g1 = max( abs(REAL(g, KIND=10)), abs(aimag(g)) )
         rtmax = sqrt( safmax/2 )
...
!           This algorithm do not use the intrinsic complex abs.
      ...
      f1 = max( abs(REAL(f, KIND=10)), abs(aimag(f)) )
      g1 = max( abs(REAL(g, KIND=10)), abs(aimag(g)) )
      rtmax = sqrt( safmax/4 )
