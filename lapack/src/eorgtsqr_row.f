...
*
*> \htmlonly
*> Download EORGTSQR_ROW + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/eorgtsqr_row.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/eorgtsqr_row.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/eorgtsqr_row.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE EORGTSQR_ROW( M, N, MB, NB, A, LDA, T, LDT, WORK,
*      $                         LWORK, INFO )
*       IMPLICIT NONE
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> EORGTSQR_ROW generates an M-by-N real matrix Q_out with
*> orthonormal columns from the output of ELATSQR. These N orthonormal
*> columns are the first N columns of a product of complex unitary
*> matrices Q(k)_in of order M, which are returned by ELATSQR in
*> a special format.
*>
*>      Q_out = first_N_columns_of( Q(1)_in * Q(2)_in * ... * Q(k)_in ).
*>
*> The input matrices Q(k)_in are stored in row and column blocks in A.
*> See the documentation of ELATSQR for more details on the format of
*> Q(k)_in, where each Q(k)_in is represented by block Householder
*> transformations. This routine calls an auxiliary routine ELARFB_GETT,
*> where the computation is performed on each individual block. The
*> algorithm first sweeps NB-sized column blocks from the right to left
*> starting in the bottom row block and continues to the top row block
*> (hence _ROW in the routine name). This sweep is in reverse order of
*> the order in which ELATSQR generates the output blocks.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*> \verbatim
*>          MB is INTEGER
*>          The row block size used by ELATSQR to return
*>          arrays A and T. MB > N.
...
*> \verbatim
*>          NB is INTEGER
*>          The column block size used by ELATSQR to return
*>          arrays A and T. NB >= 1.
...
*>             lower-trapezoidal blocked matrix V computed by ELATSQR
*>             that defines the input matrices Q_in(k) (ones on the
*>             diagonal are not stored). See ELATSQR for more details.
*>
*>          On exit:
*>
...
*>          column blocks. See ELATSQR for more details on the format
*>          of T.
*> \endverbatim
*>
*> \param[in] LDT
...
*     ..
*     .. Local Arrays ..
      REAL(KIND=10)   DUMMY( 1, 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           ELARFB_GETT, ELASET, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'EORGTSQR_ROW', -INFO )
         RETURN
      ELSE IF ( LQUERY ) THEN
         WORK( 1 ) = REAL( LWORKOPT , KIND=10)
         RETURN
      END IF
*
...
*
      CALL ELASET('U', M, N, ZERO, ONE, A, LDA )
*
...
*
               CALL ELARFB_GETT( 'I', IMB, N-KB+1, KNB,                 
