*> \brief \b YLAQP3RK computes a step of truncated QR factorization with column pivoting of a complex m-by-n matrix A using Level 3 BLAS and overwrites a complex m-by-nrhs matrix B with Q**H * B.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download YLAQP3RK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ylaqp3rk.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ylaqp3rk.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ylaqp3rk.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*      SUBROUTINE YLAQP3RK( M, N, NRHS, IOFFSET, NB, ABSTOL,
...
*     $                   NB, NRHS
*      REAL(KIND=10)   ABSTOL, MAXC2NRM, MAXC2NRMK, RELMAXC2NRMK,
*     $                   RELTOL
*     ..
*     .. Array Arguments ..
*      INTEGER            IWORK( * ), JPIV( * )
*      REAL(KIND=10)   VN1( * ), VN2( * )
*      COMPLEX(KIND=10)         A( LDA, * ), AUXV( * ), F( LDF, * ), TAU( * )
*     ..
*
*
*> \par Purpose:
...
*> \verbatim
*>
*> YLAQP3RK computes a step of truncated QR factorization with column
...
*> \param[in] ABSTOL
*> \verbatim
*>          ABSTOL is REAL(KIND=10), cannot be NaN.
*>
...
*> \param[in] RELTOL
*> \verbatim
*>          RELTOL is REAL(KIND=10), cannot be NaN.
*>
...
*>          main routine YGEQP3RK. 1 <= KP1 <= N_orig.
*> \endverbatim
*>
*> \param[in] MAXC2NRM
*> \verbatim
*>          MAXC2NRM is REAL(KIND=10)
*>          The maximum column 2-norm of the whole original
*>          matrix A_orig computed in the main routine YGEQP3RK.
*>          MAXC2NRM >= 0.
*> \endverbatim
*>
*> \param[in,out] A
...
*> \param[out] TAU
*> \verbatim
*>          TAU is COMPLEX(KIND=10) array, dimension (min(M-IOFFSET,N))
*>          The scalar factors of the elementary reflectors.
*> \endverbatim
*>
*> \param[in,out] VN1
*> \verbatim
*>          VN1 is REAL(KIND=10) array, dimension (N)
*>          The vector with the partial column norms.
*> \endverbatim
*>
*> \param[in,out] VN2
*> \verbatim
*>          VN2 is REAL(KIND=10) array, dimension (N)
*>          The vector with the exact column norms.
*> \endverbatim
*>
*> \param[out] AUXV
*> \verbatim
*>          AUXV is COMPLEX(KIND=10) array, dimension (NB)
*>          Auxiliary vector.
*> \endverbatim
*>
*> \param[out] F
*> \verbatim
*>          F is COMPLEX(KIND=10) array, dimension (LDF,NB)
*>          Matrix F**H = L*(Y**H)*A.
*> \endverbatim
*>
*> \param[in] LDF
...
*  =====================================================================
      SUBROUTINE YLAQP3RK( M, N, NRHS, IOFFSET, NB, ABSTOL,             
     +         RELTOL, KP1, MAXC2NRM, A, LDA, DONE, KB,                 
     +     MAXC2NRMK, RELMAXC2NRMK, JPIV, TAU,                      VN1,
     + VN2, AUXV, F, LDF, IWORK, INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            IWORK( * ), JPIV( * )
      REAL(KIND=10)   VN1( * ), VN2( * )
      COMPLEX(KIND=10)         A( LDA, * ), AUXV( * ), F( LDF, * ), 
     +TAU( * )
*     ..
*
*  =====================================================================
...
*     .. Local Scalars ..
     ...
     $                   LSTICC, KP, I, IF
      REAL(KIND=10)   HUGEVAL, TAUNAN, TEMP, TEMP2, TOL3Z
      COMPLEX(KIND=10)         AIK
*     ..
*     .. External Subroutines ..
      EXTERNAL           YGEMM, YGEMV, YLARFG, YSWAP
*     ..
*     .. Intrinsic Functions ..
...
*     .. External Functions ..
      ...
      EXTERNAL           EISNAN, ELAMCH, IEAMAX, EYNRM2
*     ..
...
*                               A(I+1:M,1:KB) * F(N+1:N+NRHS,1:KB)**H.
               ...
               IF( NRHS.GT.0 .AND. KB.LT.(M-IOFFSET) ) THEN
                  CALL YGEMM( 'No transpose', 'Conjugate transpose',    
...
*
         IF( KP.NE.K ) THEN
            CALL YSWAP( M, A( 1, KP ), 1, A( 1, K ), 1 )
            CALL YSWAP( K-1, F( KP, 1 ), LDF, F( K, 1 ), LDF )
            VN1( KP ) = VN1( K )
            VN2( KP ) = VN2( K )
            ITEMP = JPIV( KP )
            JPIV( KP ) = JPIV( K )
...
*
         IF( K.GT.1 ) THEN
            DO J = 1, K - 1
               F( K, J ) = CONJG( F( K, J ) )
            END DO
            CALL YGEMV( 'No transpose', M-I+1, K-1, -CONE, A( I, 1 ),   
...
*
         IF( I.LT.M ) THEN
            CALL YLARFG( M-I+1, A( I, K ), A( I+1, K ), 1, TAU( K ) )
         ELSE
            TAU( K ) = CZERO
         END IF
*
...
*        since YLARFG cannot produce TAU(KK) or Householder vector
*        below the diagonal containing Inf. Only BETA on the diagonal,
*        returned by YLARFG can contain Inf, which requires
*        TAU(K) to contain NaN. Therefore, this case of generating Inf
*        by YLARFG is covered by checking TAU(K) for NaN.
*
         IF( EISNAN( REAL( TAU(K) , KIND=10) ) ) THEN
            TAUNAN = REAL( TAU(K) , KIND=10)
         ELSE IF( EISNAN( AIMAG( TAU(K) ) ) ) THEN
            TAUNAN = AIMAG( TAU(K) )
         ELSE
            TAUNAN = ZERO
         END IF
*
         IF( EISNAN( TAUNAN ) ) THEN
*
            DONE = .TRUE.
*
*           Set KB, the number of factorized partial columns
...
*
         IF( K.LT.N+NRHS ) THEN
            CALL YGEMV( 'Conjugate transpose', M-I+1, N+NRHS-K,         
...
*
         IF( K.GT.1 ) THEN
            CALL YGEMV( 'Conjugate Transpose', M-I+1, K-1, -TAU( K ),   
     +                 A( I, 1 ), LDA, A( I, K ), 1, CZERO,             
     +       AUXV( 1 ), 1 )
*
            CALL YGEMV( 'No transpose', N+NRHS, K-1, CONE,              
     +     F( 1, 1 ), LDF, AUXV( 1 ), 1, CONE,                   F( 1, 
     +K ), 1 )
         END IF
*
*        ===============================================================
...
*
         IF( K.LT.N+NRHS ) THEN
            CALL YGEMM( 'No transpose', 'Conjugate transpose',          
     +   1, N+NRHS-K, K, -CONE, A( I, 1 ), LDA,             F( K+1, 1 ),
     + LDF, CONE, A( I, K+1 ), LDA )
         END IF
*
         A( I, K ) = AIK
*
...
*
      IF( KB.LT.MINMNUPDT ) THEN
         CALL YGEMM( 'No transpose', 'Conjugate transpose',          
     +M-IF, N+NRHS-KB, KB, -CONE, A( IF+1, 1 ), LDA,          F( KB+1, 
     +1 ), LDF, CONE, A( IF+1, KB+1 ), LDA )
      END IF
*
...
*        EYNRM2 does not fail on vectors with norm below the value of
*        SQRT(ELAMCH('S'))
*
         VN1( LSTICC ) = EYNRM2( M-IF, A( IF+1, LSTICC ), 1 )
