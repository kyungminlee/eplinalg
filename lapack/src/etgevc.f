...
*
*> \htmlonly
*> Download ETGEVC + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/etgevc.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/etgevc.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/etgevc.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE ETGEVC( SIDE, HOWMNY, SELECT, N, S, LDS, P, LDP, VL,
...
*       .. Array Arguments ..
*       LOGICAL            SELECT( * )
*       REAL(KIND=10)   P( LDP, * ), S( LDS, * ), VL( LDVL, * ),
*      $                   VR( LDVR, * ), WORK( * )
*       ..
*
*
...
*> \verbatim
*>
*> ETGEVC computes some or all of the right and/or left eigenvectors of
...
*> \param[in] S
*> \verbatim
*>          S is REAL(KIND=10) array, dimension (LDS,N)
*>          The upper quasi-triangular matrix S from a generalized Schur
*>          factorization, as computed by EHGEQZ.
*> \endverbatim
*>
*> \param[in] LDS
*> \verbatim
...
*> \param[in] P
*> \verbatim
*>          P is REAL(KIND=10) array, dimension (LDP,N)
...
*>          of left Schur vectors returned by EHGEQZ).
...
*>          of right Schur vectors returned by EHGEQZ).
*>
...
*     .. Local Scalars ..
     ...
     $                   J, JA, JC, JE, JR, JW, NA, NW
      REAL(KIND=10)   ACOEF, ACOEFA, ANORM, ASCALE, BCOEFA, BCOEFI,     
...
*     ..
*     .. Local Arrays ..
      REAL(KIND=10)   BDIAG( 2 ), SUM( 2, 2 ), SUMS( 2, 2 ),            
     +        SUMP( 2, 2 )
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*     .. External Subroutines ..
      EXTERNAL           EGEMV, ELACPY, ELAG2, ELALN2,                  
     +  XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      BIG = ONE / SAFMIN
      ULP = ELAMCH( 'Epsilon' )*ELAMCH( 'Base' )
      SMALL = SAFMIN*N / ULP
      BIG = ONE / SMALL
      BIGNUM = ONE / ( SAFMIN*N )
*
...
*              Complex eigenvalue
*
               CALL ELAG2( S( JE, JE ), LDS, P( JE, JE ), LDP,          
...
*
               CALL ELALN2( .TRUE., NA, NW, DMIN, ACOEF, S( J, J ),     
     +                  LDS,                       BDIAG( 1 ), BDIAG( 2 
     +), SUM, 2, BCOEFR,                       BCOEFI, WORK( 2*N+J ), N,
     + SCALE, TEMP,                       IINFO )
...
*
            ...
            IF( ILBACK ) THEN
               DO 170 JW = 0, NW - 1
                  CALL EGEMV( 'N', N, N+1-JE, ONE, VL( 1, JE ), LDVL,   
     +                      WORK( ( JW+2 )*N+JE ), 1, ZERO,             
     +            WORK( ( JW+4 )*N+1 ), 1 )
  170          CONTINUE
               CALL ELACPY( ' ', N, NW, WORK( 4*N+1 ), N, VL( 1,        
     +               JE ),                       LDVL )
               IBEG = 1
            ELSE
               CALL ELACPY( ' ', N, NW, WORK( 2*N+1 ), N, VL( 1,        
     +               IEIG ),                       LDVL )
               IBEG = JE
            END IF
*
...
*              Complex eigenvalue
*
               CALL ELAG2( S( JE-1, JE-1 ), LDS, P( JE-1, JE-1 ),       
     +               LDP,                      SAFMIN*SAFETY, ACOEF, 
...
*
               CALL ELALN2( .FALSE., NA, NW, DMIN, ACOEF, S( J, J ),    
