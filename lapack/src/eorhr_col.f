...
*
*> \htmlonly
*> Download EORHR_COL + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/eorhr_col.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/eorhr_col.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/eorhr_col.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE EORHR_COL( M, N, NB, A, LDA, T, LDT, D, INFO )
*
*       .. Scalar Arguments ..
*       INTEGER           INFO, LDA, LDT, M, N, NB
*       ..
*       .. Array Arguments ..
*       REAL(KIND=10)  A( LDA, * ), D( * ), T( LDT, * )
*       ..
*
*> \par Purpose:
*  =============
...
*> \verbatim
*>
*>  EORHR_COL takes an M-by-N real matrix Q_in with orthonormal columns
...
*>             EGEQRT). The matrix T and the matrix V stored on output
...
*>          blocks (same format as the output T in EGEQRT).
...
*     .. External Subroutines ..
      EXTERNAL           ECOPY, ELAORHR_COL_GETRFNP, ESCAL,             
     +       ETRSM,                    XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*     (1-1) Factor V1 and U.
      ...
      CALL ELAORHR_COL_GETRFNP( N, N, A, LDA, D, IINFO )
*
*     (1-2) Solve for V2.
*
      IF( M.GT.N ) THEN
         CALL ETRSM( 'R', 'U', 'N', 'N', M-N, N, ONE, A, LDA,           
     +     A( N+1, 1 ), LDA )
      END IF
*
...
*
         JBTEMP1 = JB - 1
         DO J = JB, JB+JNB-1
            CALL ECOPY( J-JBTEMP1, A( JB, J ), 1, T( 1, J ), 1 )
         END DO
*
...
*
         DO J = JB, JB+JNB-1
            IF( D( J ).EQ.ONE ) THEN
               CALL ESCAL( J-JBTEMP1, -ONE, T( 1, J ), 1 )
            END IF
         END DO
*
...
*             triangular, the routine ETRSM will access all JNB**2
...
*
         CALL ETRSM( 'R', 'L', 'T', 'U', JNB, JNB, ONE,                
