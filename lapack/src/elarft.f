*> \brief \b ELARFT forms the triangular factor T of a block reflector H = I - vtvH
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download ELARFT + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/elarft.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/elarft.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/elarft.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       RECURSIVE SUBROUTINE ELARFT( DIRECT, STOREV, N, K, V, LDV, TAU, T, LDT )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> ELARFT forms the triangular factor T of a real block reflector H
...
*> \param[in] TAU
*> \verbatim
*>          TAU is REAL(KIND=10) array, dimension (K)
...
*>
*  =====================================================================
      RECURSIVE SUBROUTINE ELARFT( DIRECT, STOREV, N, K, V, LDV,        
     +                      TAU, T, LDT )
*
...
*     .. Parameters ..
*
      REAL(KIND=10) ONE, NEG_ONE, ZERO
      PARAMETER(ONE=1.0E0_10, ZERO = 0.0E0_10, NEG_ONE=-1.0E0_10)
*
*     .. Local Scalars ..
*
      INTEGER           I,J,L
...
*
      ...
      EXTERNAL          LSAME
*     
*     The general scheme used is inspired by the approach inside EGEQRT3
...
*
         CALL ELARFT(DIRECT, STOREV, N, L, V, LDV, TAU, T, LDT)
*
*        Compute T_{2,2} recursively
*
         CALL ELARFT(DIRECT, STOREV, N-L, K-L, V(L+1, L+1), LDV,        
...
*
         CALL ETRMM('Right', 'Lower', 'No transpose', 'Unit', L,        
...
*
         CALL EGEMM('Transpose', 'No transpose', L, K-L, N-K, ONE,      
...
*
         CALL ETRMM('Left', 'Upper', 'No transpose', 'Non-unit', L,     
     +           K-L, NEG_ONE, T, LDT, T(1, L+1), LDT)
*
*        T_{1,2} = T_{1,2}*T_{2,2}
*
         CALL ETRMM('Right', 'Upper', 'No transpose', 'Non-unit', L,    
...
*        T_{1,2} = V_{1,2}
*
         CALL ELACPY('All', L, K-L, V(1, L+1), LDV, T(1, L+1), LDT)
*
*        T_{1,2} = T_{1,2}*V_{2,2}'
*
         CALL ETRMM('Right', 'Upper', 'Transpose', 'Unit', L, K-L,      
...
*        Note: We assume K <= N, and GEMM will do nothing if N=K
*
         CALL EGEMM('No transpose', 'Transpose', L, K-L, N-K, ONE,      
...
*        Compute T_{1,1} recursively
*
         CALL ELARFT(DIRECT, STOREV, N-L, K-L, V, LDV, TAU, T, LDT)
*
*        Compute T_{2,2} recursively
*
         CALL ELARFT(DIRECT, STOREV, N, L, V(1, K-L+1), LDV,            
...
*
         CALL ETRMM('Right', 'Upper', 'No transpose', 'Unit', L,        
...
*
         CALL ETRMM('Left', 'Lower', 'No transpose', 'Non-unit', L,     
     +           K-L, NEG_ONE, T(K-L+1, K-L+1), LDT,                
     +T(K-L+1, 1), LDT)
*
*        T_{2,1} = T_{2,1}*T_{1,1}
*
         CALL ETRMM('Right', 'Lower', 'No transpose', 'Non-unit', L,    
...
*        Compute T_{2,2} recursively
*
         CALL ELARFT(DIRECT, STOREV, N, L, V(K-L+1, 1), LDV,            
     +    TAU(K-L+1), T(K-L+1, K-L+1), LDT)
*
*        Compute T_{2,1}
*        T_{2,1} = V_{2,2}
*
         CALL ELACPY('All', L, K-L, V(K-L+1, N-K+1), LDV,               
     + T(K-L+1, 1), LDT)

*
*        T_{2,1} = T_{2,1}*V_{1,2}'
*
         CALL ETRMM('Right', 'Lower', 'Transpose', 'Unit', L, K-L,      
...
*        T_{2,1} = -T_{2,2}*T_{2,1}
*
         CALL ETRMM('Left', 'Lower', 'No tranpose', 'Non-unit', L,      
     +          K-L, NEG_ONE, T(K-L+1, K-L+1), LDT,                
     +T(K-L+1, 1), LDT)
...
*        T_{2,1} = T_{2,1}*T_{1,1}
*
         CALL ETRMM('Right', 'Lower', 'No tranpose', 'Non-unit', L,     
     +           K-L, ONE, T, LDT, T(K-L+1, 1), LDT)
      END IF
      END SUBROUTINE
