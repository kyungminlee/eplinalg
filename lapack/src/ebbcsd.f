...
*
*> \htmlonly
*> Download EBBCSD + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ebbcsd.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ebbcsd.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ebbcsd.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE EBBCSD( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS, M, P, Q,
...
*       ..
*       .. Array Arguments ..
*       REAL(KIND=10)   B11D( * ), B11E( * ), B12D( * ), B12E( * ),
*      $                   B21D( * ), B21E( * ), B22D( * ), B22E( * ),
*      $                   PHI( * ), THETA( * ), WORK( * )
*       REAL(KIND=10)   U1( LDU1, * ), U2( LDU2, * ), V1T( LDV1T, * ),
*      $                   V2T( LDV2T, * )
*       ..
*
*
...
*> \verbatim
*>
*> EBBCSD computes the CS decomposition of an orthogonal matrix in
...
*> the TRANS and SIGNS options. See EORCSD for details.)
*>
...
*> \param[in,out] THETA
*> \verbatim
*>          THETA is REAL(KIND=10) array, dimension (Q)
...
*> \param[in,out] PHI
*> \verbatim
*>          PHI is REAL(KIND=10) array, dimension (Q-1)
...
*> \param[in,out] U1
*> \verbatim
*>          U1 is REAL(KIND=10) array, dimension (LDU1,P)
...
*> \param[in,out] U2
*> \verbatim
*>          U2 is REAL(KIND=10) array, dimension (LDU2,M-P)
...
*> \param[in,out] V1T
*> \verbatim
*>          V1T is REAL(KIND=10) array, dimension (LDV1T,Q)
...
*> \param[in,out] V2T
*> \verbatim
*>          V2T is REAL(KIND=10) array, dimension (LDV2T,M-Q)
...
*> \param[out] B11D
*> \verbatim
*>          B11D is REAL(KIND=10) array, dimension (Q)
*>          When EBBCSD converges, B11D contains the cosines of THETA(1),
*>          ..., THETA(Q). If EBBCSD fails to converge, then B11D
...
*> \param[out] B11E
*> \verbatim
*>          B11E is REAL(KIND=10) array, dimension (Q-1)
*>          When EBBCSD converges, B11E contains zeros. If EBBCSD fails
...
*> \param[out] B12D
*> \verbatim
*>          B12D is REAL(KIND=10) array, dimension (Q)
*>          When EBBCSD converges, B12D contains the negative sines of
*>          THETA(1), ..., THETA(Q). If EBBCSD fails to converge, then
...
*> \param[out] B12E
*> \verbatim
*>          B12E is REAL(KIND=10) array, dimension (Q-1)
*>          When EBBCSD converges, B12E contains zeros. If EBBCSD fails
...
*> \param[out] B21D
*> \verbatim
*>          B21D is REAL(KIND=10)  array, dimension (Q)
*>          When EBBCSD converges, B21D contains the negative sines of
*>          THETA(1), ..., THETA(Q). If EBBCSD fails to converge, then
...
*> \param[out] B21E
*> \verbatim
*>          B21E is REAL(KIND=10)  array, dimension (Q-1)
*>          When EBBCSD converges, B21E contains zeros. If EBBCSD fails
...
*> \param[out] B22D
*> \verbatim
*>          B22D is REAL(KIND=10)  array, dimension (Q)
*>          When EBBCSD converges, B22D contains the negative sines of
*>          THETA(1), ..., THETA(Q). If EBBCSD fails to converge, then
...
*> \param[out] B22E
*> \verbatim
*>          B22E is REAL(KIND=10)  array, dimension (Q-1)
*>          When EBBCSD converges, B22E contains zeros. If EBBCSD fails
...
*>          > 0:  if EBBCSD did not converge, INFO specifies the number
...
*>
*> \verbatim
*>  TOLMUL  REAL(KIND=10), default = MAX(10,MIN(100,EPS**(-1/8)))
...
*
*  =====================================================================
      SUBROUTINE EBBCSD( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS, M, P,     
     +               Q,                    THETA, PHI, U1, LDU1, U2, 
     +LDU2, V1T, LDV1T,                    V2T, LDV2T, B11D, B11E, B12D,
     + B12E, B21D, B21E,                    B22D, B22E, WORK, LWORK, 
     +INFO )
*
...
*     .. Parameters ..
      INTEGER            MAXITR
      PARAMETER          ( MAXITR = 6 )
      REAL(KIND=10)   HUNDRED, MEIGHTH, ONE, TEN, ZERO
      PARAMETER          ( HUNDRED = 100.0E0_10, MEIGHTH = -0.125E0_10, 
     +                     ONE = 1.0E0_10, TEN = 10.0E0_10, ZERO = 
     +0.0E0_10 )
      REAL(KIND=10)   NEGONE
      PARAMETER          ( NEGONE = -1.0E0_10 )
      REAL(KIND=10)   PIOVER2
      PARAMETER ( PIOVER2 = 1.57079632679489661923132169163975144210E0_
     +10 )
*     ..
*     .. Local Scalars ..
     ...
     $                   LWORKMIN, LWORKOPT, MAXIT, MINI
      REAL(KIND=10)   B11BULGE, B12BULGE, B21BULGE, B22BULGE, DUMMY,    
     +                EPS, MU, NU, R, SIGMA11, SIGMA21,                 
     +   TEMP, THETAMAX, THETAMIN, THRESH, TOL, TOLMUL,                 
     +   UNFL, X1, X2, Y1, Y2
*
*     .. External Subroutines ..
      EXTERNAL           ELASR, ESCAL, ESWAP, ELARTGP, ELARTGS,         
     +           ELAS2,                    XERBLA
*     ..
*     .. External Functions ..
      REAL(KIND=10)   ELAMCH
...
*
            CALL ELAS2( B11D(IMAX-1), B11E(IMAX-1), B11D(IMAX),         
     +          SIGMA11,                   DUMMY )
            CALL ELAS2( B21D(IMAX-1), B21E(IMAX-1), B21D(IMAX),         
     +          SIGMA21,                   DUMMY )
*
...
*
         IF( MU .LE. NU ) THEN
            CALL ELARTGS( B11D(IMIN), B11E(IMIN), MU,                   
     +  WORK(IV1TCS+IMIN-1), WORK(IV1TSN+IMIN-1) )
         ELSE
            CALL ELARTGS( B21D(IMIN), B21E(IMIN), NU,                   
     +  WORK(IV1TCS+IMIN-1), WORK(IV1TSN+IMIN-1) )
         END IF
*
...
*
         IF( B11D(IMIN)**2+B11BULGE**2 .GT. THRESH**2 ) THEN
            CALL ELARTGP( B11BULGE, B11D(IMIN), WORK(IU1SN+IMIN-1),     
     +                WORK(IU1CS+IMIN-1), R )
         ELSE IF( MU .LE. NU ) THEN
            CALL ELARTGS( B11E( IMIN ), B11D( IMIN + 1 ), MU,           
     +          WORK(IU1CS+IMIN-1), WORK(IU1SN+IMIN-1) )
         ELSE
            CALL ELARTGS( B12D( IMIN ), B12E( IMIN ), NU,               
     +      WORK(IU1CS+IMIN-1), WORK(IU1SN+IMIN-1) )
         END IF
         IF( B21D(IMIN)**2+B21BULGE**2 .GT. THRESH**2 ) THEN
            CALL ELARTGP( B21BULGE, B21D(IMIN), WORK(IU2SN+IMIN-1),     
     +                WORK(IU2CS+IMIN-1), R )
         ELSE IF( NU .LT. MU ) THEN
            CALL ELARTGS( B21E( IMIN ), B21D( IMIN + 1 ), NU,           
     +          WORK(IU2CS+IMIN-1), WORK(IU2SN+IMIN-1) )
         ELSE
            CALL ELARTGS( B22D(IMIN), B22E(IMIN), MU,                   
     +  WORK(IU2CS+IMIN-1), WORK(IU2SN+IMIN-1) )
         END IF
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL ELARTGP( X2, X1, WORK(IV1TSN+I-1),                  
     +      WORK(IV1TCS+I-1),                        R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART21 ) THEN
               CALL ELARTGP( B11BULGE, B11E(I-1), WORK(IV1TSN+I-1),     
     +                   WORK(IV1TCS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL ELARTGP( B21BULGE, B21E(I-1), WORK(IV1TSN+I-1),     
     +                   WORK(IV1TCS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL ELARTGS( B11D(I), B11E(I), MU, WORK(IV1TCS+I-1),    
     +                    WORK(IV1TSN+I-1) )
            ELSE
               CALL ELARTGS( B21D(I), B21E(I), NU, WORK(IV1TCS+I-1),    
     +                    WORK(IV1TSN+I-1) )
            END IF
            WORK(IV1TCS+I-1) = -WORK(IV1TCS+I-1)
            WORK(IV1TSN+I-1) = -WORK(IV1TSN+I-1)
            IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( Y2, Y1, WORK(IV2TSN+I-1-1),                
     +        WORK(IV2TCS+I-1-1), R )
            ELSE IF( .NOT. RESTART12 .AND. RESTART22 ) THEN
               CALL ELARTGP( B12BULGE, B12D(I-1), WORK(IV2TSN+I-1-1),   
     +                     WORK(IV2TCS+I-1-1), R )
            ELSE IF( RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( B22BULGE, B22D(I-1), WORK(IV2TSN+I-1-1),   
     +                     WORK(IV2TCS+I-1-1), R )
            ELSE IF( NU .LT. MU ) THEN
               CALL ELARTGS( B12E(I-1), B12D(I), NU,                    
     +    WORK(IV2TCS+I-1-1),                        WORK(IV2TSN+I-1-1) 
     +)
            ELSE
               CALL ELARTGS( B22E(I-1), B22D(I), MU,                    
     +    WORK(IV2TCS+I-1-1),                        WORK(IV2TSN+I-1-1) 
     +)
            END IF
*
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL ELARTGP( X2, X1, WORK(IU1SN+I-1),                   
     +     WORK(IU1CS+I-1),                        R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART12 ) THEN
               CALL ELARTGP( B11BULGE, B11D(I), WORK(IU1SN+I-1),        
     +                WORK(IU1CS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL ELARTGP( B12BULGE, B12E(I-1), WORK(IU1SN+I-1),      
     +                  WORK(IU1CS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL ELARTGS( B11E(I), B11D(I+1), MU, WORK(IU1CS+I-1),   
     +                     WORK(IU1SN+I-1) )
            ELSE
               CALL ELARTGS( B12D(I), B12E(I), NU, WORK(IU1CS+I-1),     
     +                   WORK(IU1SN+I-1) )
            END IF
            IF( .NOT. RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( Y2, Y1, WORK(IU2SN+I-1),                   
     +     WORK(IU2CS+I-1),                        R )
            ELSE IF( .NOT. RESTART21 .AND. RESTART22 ) THEN
               CALL ELARTGP( B21BULGE, B21D(I), WORK(IU2SN+I-1),        
     +                WORK(IU2CS+I-1), R )
            ELSE IF( RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL ELARTGP( B22BULGE, B22E(I-1), WORK(IU2SN+I-1),      
     +                  WORK(IU2CS+I-1), R )
            ELSE IF( NU .LT. MU ) THEN
               CALL ELARTGS( B21E(I), B21E(I+1), NU, WORK(IU2CS+I-1),   
     +                     WORK(IU2SN+I-1) )
            ELSE
               CALL ELARTGS( B22D(I), B22E(I), MU, WORK(IU2CS+I-1),     
     +                   WORK(IU2SN+I-1) )
            END IF
...
*
         IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
            CALL ELARTGP( Y2, Y1, WORK(IV2TSN+IMAX-1-1),                
     +     WORK(IV2TCS+IMAX-1-1), R )
         ELSE IF( .NOT. RESTART12 .AND. RESTART22 ) THEN
            CALL ELARTGP( B12BULGE, B12D(IMAX-1),                     
     +WORK(IV2TSN+IMAX-1-1),                     WORK(IV2TCS+IMAX-1-1), 
     +R )
         ELSE IF( RESTART12 .AND. .NOT. RESTART22 ) THEN
            CALL ELARTGP( B22BULGE, B22D(IMAX-1),                     
     +WORK(IV2TSN+IMAX-1-1),                     WORK(IV2TCS+IMAX-1-1), 
     +R )
         ELSE IF( NU .LT. MU ) THEN
            CALL ELARTGS( B12E(IMAX-1), B12D(IMAX), NU,                 
     +    WORK(IV2TCS+IMAX-1-1), WORK(IV2TSN+IMAX-1-1) )
         ELSE
            CALL ELARTGS( B22E(IMAX-1), B22D(IMAX), MU,                 
     +    WORK(IV2TCS+IMAX-1-1), WORK(IV2TSN+IMAX-1-1) )
         END IF
*
...
*
         IF( WANTU1 ) THEN
            IF( COLMAJOR ) THEN
               CALL ELASR( 'R', 'V', 'F', P, IMAX-IMIN+1,               
     +       WORK(IU1CS+IMIN-1), WORK(IU1SN+IMIN-1),                    
     +  U1(1,IMIN), LDU1 )
            ELSE
               CALL ELASR( 'L', 'V', 'F', IMAX-IMIN+1, P,               
     +       WORK(IU1CS+IMIN-1), WORK(IU1SN+IMIN-1),                    
     +  U1(IMIN,1), LDU1 )
            END IF
         END IF
         IF( WANTU2 ) THEN
            IF( COLMAJOR ) THEN
               CALL ELASR( 'R', 'V', 'F', M-P, IMAX-IMIN+1,             
     +         WORK(IU2CS+IMIN-1), WORK(IU2SN+IMIN-1),                  
     +    U2(1,IMIN), LDU2 )
            ELSE
               CALL ELASR( 'L', 'V', 'F', IMAX-IMIN+1, M-P,             
     +         WORK(IU2CS+IMIN-1), WORK(IU2SN+IMIN-1),                  
     +    U2(IMIN,1), LDU2 )
            END IF
         END IF
         IF( WANTV1T ) THEN
            IF( COLMAJOR ) THEN
               CALL ELASR( 'L', 'V', 'F', IMAX-IMIN+1, Q,               
     +       WORK(IV1TCS+IMIN-1), WORK(IV1TSN+IMIN-1),                  
     +    V1T(IMIN,1), LDV1T )
            ELSE
               CALL ELASR( 'R', 'V', 'F', Q, IMAX-IMIN+1,               
     +       WORK(IV1TCS+IMIN-1), WORK(IV1TSN+IMIN-1),                  
     +    V1T(1,IMIN), LDV1T )
            END IF
         END IF
         IF( WANTV2T ) THEN
            IF( COLMAJOR ) THEN
               CALL ELASR( 'L', 'V', 'F', IMAX-IMIN+1, M-Q,             
     +         WORK(IV2TCS+IMIN-1), WORK(IV2TSN+IMIN-1),                
     +      V2T(IMIN,1), LDV2T )
            ELSE
               CALL ELASR( 'R', 'V', 'F', M-Q, IMAX-IMIN+1,             
     +         WORK(IV2TCS+IMIN-1), WORK(IV2TSN+IMIN-1),                
     +      V2T(1,IMIN), LDV2T )
            END IF
         END IF
...
*
         IF( B11E(IMAX-1)+B21E(IMAX-1) .GT. 0 ) THEN
            ...
            IF( WANTV1T ) THEN
               IF( COLMAJOR ) THEN
                  CALL ESCAL( Q, NEGONE, V1T(IMAX,1), LDV1T )
               ELSE
                  CALL ESCAL( Q, NEGONE, V1T(1,IMAX), 1 )
               END IF
            END IF
         END IF
*
...
*
         IF( B11D(IMAX)+B12E(IMAX-1) .LT. 0 ) THEN
            ...
            IF( WANTU1 ) THEN
               IF( COLMAJOR ) THEN
                  CALL ESCAL( P, NEGONE, U1(1,IMAX), 1 )
               ELSE
                  CALL ESCAL( P, NEGONE, U1(IMAX,1), LDU1 )
               END IF
            END IF
         END IF
         IF( B21D(IMAX)+B22E(IMAX-1) .GT. 0 ) THEN
            ...
            IF( WANTU2 ) THEN
               IF( COLMAJOR ) THEN
                  CALL ESCAL( M-P, NEGONE, U2(1,IMAX), 1 )
               ELSE
                  CALL ESCAL( M-P, NEGONE, U2(IMAX,1), LDU2 )
               END IF
            END IF
         END IF
*
...
*
         IF( B12D(IMAX)+B22D(IMAX) .LT. 0 ) THEN
            IF( WANTV2T ) THEN
               IF( COLMAJOR ) THEN
                  CALL ESCAL( M-Q, NEGONE, V2T(IMAX,1), LDV2T )
               ELSE
                  CALL ESCAL( M-Q, NEGONE, V2T(1,IMAX), 1 )
               END IF
            END IF
         END IF
*
...
*
         IF( MINI .NE. I ) THEN
            ...
            THETA(I) = THETAMIN
            IF( COLMAJOR ) THEN
               IF( WANTU1 )             CALL ESWAP( P, U1(1,I), 1, U1(1,
     +MINI), 1 )
               IF( WANTU2 )             CALL ESWAP( M-P, U2(1,I), 1, 
     +U2(1,MINI), 1 )
               IF( WANTV1T )             CALL ESWAP( Q, V1T(I,1), LDV1T,
     + V1T(MINI,1),                         LDV1T )
               IF( WANTV2T )             CALL ESWAP( M-Q, V2T(I,1), 
     +LDV2T, V2T(MINI,1),                LDV2T )
            ELSE
               IF( WANTU1 )             CALL ESWAP( P, U1(I,1), LDU1, 
     +U1(MINI,1), LDU1 )
               IF( WANTU2 )             CALL ESWAP( M-P, U2(I,1), LDU2, 
     +U2(MINI,1), LDU2 )
               IF( WANTV1T )             CALL ESWAP( Q, V1T(1,I), 1, 
     +V1T(1,MINI), 1 )
               IF( WANTV2T )             CALL ESWAP( M-Q, V2T(1,I), 1, 
     +V2T(1,MINI), 1 )
            END IF
         END IF
*
