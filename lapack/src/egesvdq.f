*> \brief <b> EGESVDQ computes the singular value decomposition (SVD) with a QR-Preconditioned QR SVD Method for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download EGESVDQ + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/egesvdq.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/egesvdq.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/egesvdq.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*      SUBROUTINE EGESVDQ( JOBA, JOBP, JOBR, JOBU, JOBV, M, N, A, LDA,
...
*     ..
*     .. Array Arguments ..
*      REAL(KIND=10)  A( LDA, * ), U( LDU, * ), V( LDV, * ), WORK( * )
...
*> \verbatim
*>
*> EGESVDQ computes the singular value decomposition (SVD) of a real
...
*>        where EPS = ELAMCH('Epsilon'). This authorises EGESVDQ to
...
*> \verbatim
*>          JOBR is CHARACTER*1
*>          = 'T' After the initial pivoted QR factorization, EGESVD is applied to
*>          the transposed R**T of the computed triangular factor R. This involves
*>          some extra data movement (matrix transpositions). Useful for
*>          experiments, research and development.
*>          = 'N' The triangular factor R is given as input to EGESVD. This may be
...
*> \param[in,out] A
*> \verbatim
*>          A is REAL(KIND=10) array of dimensions LDA x N
*>          On entry, the input matrix A.
*>          On exit, if JOBU .NE. 'N' or JOBV .NE. 'N', the lower triangle of A contains
*>          the Householder vectors as stored by EGEQP3. If JOBU = 'F', these Householder
...
*> \param[out] U
*> \verbatim
*>          U is REAL(KIND=10) array, dimension
...
*>          of EGESVD. The final value of NUMRANK might be further reduced if
...
*> \param[out] WORK
*> \verbatim
*>          WORK is REAL(KIND=10) array, dimension (max(2, LWORK)), used as a workspace.
...
*>          exact zeros in EGESVD applied to the upper triangular or trapezoidal
*>          R (from the initial QR factorization). In case of early exit (no call to
*>          EGESVD, such as in the case of zero matrix) RWORK(2) = -1.
*>
*>          If LIWORK, LWORK, or LRWORK = -1, then on exit, if INFO = 0,
...
*>          > 0:  if EBDSQR did not converge, INFO specifies how many superdiagonals
*>          of an intermediate bidiagonal form B (computed in EGESVD) did not
*>          converge to zero.
*> \endverbatim
*
*> \par Further Details:
...
*     .. External Subroutines (BLAS, LAPACK)
      EXTERNAL    EGELQF, EGEQP3, EGEQRF, EGESVD, ELACPY,               
     +     ELAPMT,             ELASCL, ELASET, ELASWP, ESCAL,  EPOCON, 
     +EORMLQ,             EORMQR, XERBLA
*     ..
*     .. External Functions (BLAS, LAPACK)
      LOGICAL    LSAME
      INTEGER    IEAMAX
      REAL(KIND=10)  ELANGE, ENRM2, ELAMCH
      EXTERNAL    ELANGE, LSAME, IEAMAX, ENRM2, ELAMCH
*     ..
*     .. Intrinsic Functions ..
*
...
*
*        .. minimal workspace length for EGEQP3 of an M x N matrix
         LWQP3 = 3 * N + 1
*        .. minimal workspace length for EORMQR to build left singular vectors
         IF ( WNTUS .OR. WNTUR ) THEN
             LWORQ  = MAX( N  , 1 )
         ELSE IF ( WNTUA ) THEN
             LWORQ = MAX( M , 1 )
         END IF
*        .. minimal workspace length for EPOCON of an N x N matrix
         LWCON = 3 * N
*        .. EGESVD of an N x N matrix
         LWSVD = MAX( 5 * N, 1 )
         IF ( LQUERY ) THEN
             CALL EGEQP3( M, N, A, LDA, IWORK, RDUMMY, RDUMMY, -1,      
     +      IERR )
             LWRK_DGEQP3 = INT( RDUMMY(1) )
             IF ( WNTUS .OR. WNTUR ) THEN
                 CALL EORMQR( 'L', 'N', M, N, N, A, LDA, RDUMMY, U,     
     +            LDU, RDUMMY, -1, IERR )
                 LWRK_DORMQR = INT( RDUMMY(1) )
             ELSE IF ( WNTUA ) THEN
                 CALL EORMQR( 'L', 'N', M, M, N, A, LDA, RDUMMY, U,     
     +            LDU, RDUMMY, -1, IERR )
                 LWRK_DORMQR = INT( RDUMMY(1) )
             ELSE
                 LWRK_DORMQR = 0
...
*            only the singular values are requested
             ...
             END IF
             IF ( LQUERY ) THEN
                 CALL EGESVD( 'N', 'N', N, N, A, LDA, S, U, LDU,        
     +         V, LDV, RDUMMY, -1, IERR )
...
*            singular values and the left singular vectors are requested
             ...
             IF ( LQUERY ) THEN
                IF ( RTRANS ) THEN
                   CALL EGESVD( 'N', 'O', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, RDUMMY, -1, IERR )
                ELSE
                   CALL EGESVD( 'O', 'N', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, RDUMMY, -1, IERR )
                END IF
                LWRK_DGESVD = INT( RDUMMY(1) )
                IF ( CONDA ) THEN
...
*            full SVD is requested
             IF ( RTRANS ) THEN
                 ...
                 MINWRK = MINWRK + N
                 IF ( WNTVA ) THEN
*                   .. minimal workspace length for N x N/2 EGEQRF
                    LWQRF  = MAX( N/2, 1 )
*                   .. minimal workspace length for N/2 x N/2 EGESVD
                    LWSVD2 = MAX( 5 * (N/2), 1 )
                    LWORQ2 = MAX( N, 1 )
     ...
     $                        N/2+LWORQ2, LWORQ )
             ...
             ELSE
                 ...
                 MINWRK = MINWRK + N
                 IF ( WNTVA ) THEN
*                   .. minimal workspace length for N/2 x N EGELQF
                    LWLQF  = MAX( N/2, 1 )
                    LWSVD2 = MAX( 5 * (N/2), 1 )
                    LWORLQ = MAX( N , 1 )
     ...
     $                        N/2+LWORLQ, LWORQ )
             ...
             IF ( LQUERY ) THEN
                IF ( RTRANS ) THEN
                   CALL EGESVD( 'O', 'A', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, RDUMMY, -1, IERR )
                   LWRK_DGESVD = INT( RDUMMY(1) )
                   ...
                   OPTWRK = N + OPTWRK
                   IF ( WNTVA ) THEN
                       CALL EGEQRF(N,N/2,U,LDU,RDUMMY,RDUMMY,-1,IERR)
                       LWRK_DGEQRF = INT( RDUMMY(1) )
                       CALL EGESVD( 'S', 'O', N/2,N/2, V,LDV, S, U,     
     +                          LDU,                       V, LDV, 
     +RDUMMY, -1, IERR )
                       LWRK_DGESVD2 = INT( RDUMMY(1) )
                       CALL EORMQR( 'R', 'C', N, N, N/2, U, LDU,        
     +                       RDUMMY,                       V, LDV, 
     +RDUMMY, -1, IERR )
     ...
     $                           N/2+LWRK_DGESVD2, N/2+LWRK_DORMQR2 )
                   ...
                   END IF
                ELSE
                   CALL EGESVD( 'S', 'O', N, N, A, LDA, S, U, LDU,      
     +             V, LDV, RDUMMY, -1, IERR )
                   LWRK_DGESVD = INT( RDUMMY(1) )
                   OPTWRK = MAX(LWRK_DGEQP3,LWRK_DGESVD,LWRK_DORMQR)
                   IF ( CONDA ) OPTWRK = MAX( OPTWRK, LWCON )
                   OPTWRK = N + OPTWRK
                   IF ( WNTVA ) THEN
                      CALL EGELQF(N/2,N,U,LDU,RDUMMY,RDUMMY,-1,IERR)
                      LWRK_DGELQF = INT( RDUMMY(1) )
                      CALL EGESVD( 'S','O', N/2,N/2, V, LDV, S, U,      
     +                        LDU,                      V, LDV, RDUMMY, 
     +-1, IERR )
                      LWRK_DGESVD2 = INT( RDUMMY(1) )
                      CALL EORMLQ( 'R', 'N', N, N, N/2, U, LDU,         
     +                     RDUMMY,                      V, LDV, RDUMMY,
     +-1,IERR )
...
*               [[ELANGE will return NaN if an entry of the p-th row is Nan]]
                RWORK(p) = ELANGE( 'M', 1, N, A(p,1), LDA, RDUMMY )
...
*              Quick return: A is the M x N zero matrix.
               ...
               IF ( WNTUS ) CALL ELASET('G', M, N, ZERO, ONE, U, LDU)
               IF ( WNTUA ) CALL ELASET('G', M, M, ZERO, ONE, U, LDU)
               IF ( WNTVA ) CALL ELASET('G', N, N, ZERO, ONE, V, LDV)
               IF ( WNTUF ) THEN
                   CALL ELASET( 'G', N, 1, ZERO, ZERO, WORK, N )
                   CALL ELASET( 'G', M, N, ZERO,  ONE, U, LDU )
               END IF
               DO 5001 p = 1, N
                   IWORK(p) = p
 5001          CONTINUE
...
*               matrix by 1/sqrt(M) if too large entry detected
                CALL ELASCL('G',0,0,SQRT(REAL(M, KIND=10)),ONE, M,N, A,
     +LDA,                        IERR)
                ASCALED = .TRUE.
            END IF
            CALL ELASWP( N, A, LDA, 1, M-1, IWORK(N+1), 1 )
      END IF
*
...
*
      IF ( .NOT.ROWPRM ) THEN
          RTMP = ELANGE( 'M', M, N, A, LDA, RDUMMY )
          IF ( ( RTMP .NE. RTMP ) .OR.
...
*        .. all columns are free columns
         IWORK(p) = 0
 1963 CONTINUE
      CALL EGEQP3( M, N, A, LDA, IWORK, WORK, WORK(N+1), LWORK-N,       
     +IERR )
*
...
*              perturbation theory.
               DO 3053 p = 1, NR
                  RTMP = ENRM2( p, V(1,p), 1 )
                  CALL ESCAL( p, ONE/RTMP, V(1,p), 1 )
 3053          CONTINUE
               IF ( .NOT. ( LSVEC .OR. RSVEC ) ) THEN
                   CALL EPOCON( 'U', NR, V, LDV, ONE, RTMP,             
...
*           the upper triangle of [A] to zero.
 ...
 1146       CONTINUE
*
            CALL EGESVD( 'N', 'N', N, NR, A, LDA, S, U, LDU,            
...
*
            IF ( NR .GT. 1 )           CALL ELASET( 'L', NR-1,NR-1, 
     +ZERO,ZERO, A(2,1), LDA )
            CALL EGESVD( 'N', 'N', NR, N, A, LDA, S, U, LDU,            
     +V, LDV, WORK, LWORK, INFO )
*
         END IF
*
...
*            .. copy R into [U] and overwrite [U] with the left singular vectors
             CALL ELACPY( 'U', NR, N, A, LDA, U, LDU )
             IF ( NR .GT. 1 )          CALL ELASET( 'L', NR-1, NR-1, 
     +ZERO, ZERO, U(2,1),                       LDU )
*            .. the right singular vectors not computed, the NR left singular
*            vectors overwrite [U](1:NR,1:NR)
                CALL EGESVD( 'O', 'N', NR, N, U, LDU, S, U, LDU,        
...
*              (M x NR) or (M x N) or (M x M).
         IF ( ( NR .LT. M ) .AND. ( .NOT.WNTUF ) ) THEN
             ...
             IF ( NR .LT. N1 ) THEN
                ...
                CALL ELASET( 'A',M-NR,N1-NR,ZERO,ONE,                
     +U(NR+1,NR+1), LDU )
             END IF
         END IF
*
...
*           vectors matrix U.
*
         IF ( .NOT.WNTUF )        CALL EORMQR( 'L', 'N', M, N1, N, A, 
     +LDA, WORK, U,             LDU, WORK(N+1), LWORK-N, IERR )
         IF ( ROWPRM .AND. .NOT.WNTUF )           CALL ELASWP( N1, U, 
...
*           vectors not computed
            IF ( WNTVR .OR. ( NR .EQ. N ) ) THEN
               CALL EGESVD( 'O', 'N', N, NR, V, LDV, S, U, LDU,         
     +      U, LDU, WORK(N+1), LWORK-N, INFO )
*
               DO 1121 p = 1, NR
                   DO 1122 q = p + 1, NR
...
*
 ...
 1103              CONTINUE
               END IF
               CALL ELAPMT( .FALSE., NR, N, V, LDV, IWORK )
            ELSE
...
*               how to implement this, see the " FULL SVD " branch.
                CALL ELASET('G', N, N-NR, ZERO, ZERO, V(1,NR+1), LDV)
                CALL EGESVD( 'O', 'N', N, N, V, LDV, S, U, LDU,         
     +       U, LDU, WORK(N+1), LWORK-N, INFO )
*
                DO 1123 p = 1, N
                   DO 1124 q = p + 1, N
 ...
 1124              CONTINUE
 1123           CONTINUE
                CALL ELAPMT( .FALSE., N, N, V, LDV, IWORK )
            END IF
*
          ELSE
*            .. aply EGESVD to R
*            .. copy R into V and overwrite V with the right singular vectors
             CALL ELACPY( 'U', NR, N, A, LDA, V, LDV )
             IF ( NR .GT. 1 )          CALL ELASET( 'L', NR-1, NR-1, 
     +ZERO, ZERO, V(2,1),                       LDV )
*            .. the right singular vectors overwrite V, the NR left singular
*            vectors stored in U(1:NR,1:NR)
             IF ( WNTVR .OR. ( NR .EQ. N ) ) THEN
                CALL EGESVD( 'N', 'O', NR, N, V, LDV, S, U, LDU,        
     +        V, LDV, WORK(N+1), LWORK-N, INFO )
                CALL ELAPMT( .FALSE., NR, N, V, LDV, IWORK )
...
*               how to implement this, see the " FULL SVD " branch.
                 CALL ELASET('G', N-NR, N, ZERO,ZERO, V(NR+1,1), LDV)
                 CALL EGESVD( 'N', 'O', N, N, V, LDV, S, U, LDU,        
     +         V, LDV, WORK(N+1), LWORK-N, INFO )
                 CALL ELAPMT( .FALSE., N, N, V, LDV, IWORK )
             END IF
...
*.......................................................................
         IF ( RTRANS ) THEN
*
*            .. apply EGESVD to R**T [[this option is left for R&D&T]]
*
            IF ( WNTVR .OR. ( NR .EQ. N ) ) THEN
...
*           singular vectors of R**T stored in [U](1:NR,1:NR) as transposed
               CALL EGESVD( 'O', 'A', N, NR, V, LDV, S, V, LDV,         
     +      U, LDU, WORK(N+1), LWORK-N, INFO )
*              .. assemble V
               DO 1115 p = 1, NR
                  DO 1116 q = p + 1, NR
...
*               OPTRATIO = ILAENV_EP(6, 'EGESVD', 'S' // 'O', NR,N,0,0)
*               OPTRATIO = MAX( OPTRATIO, 2 )
 ...
 1199                 CONTINUE
 1198              CONTINUE
                   IF ( NR .GT. 1 )              CALL ELASET('U',NR-1,
     +NR-1, ZERO,ZERO, V(1,2),LDV)
*
                   CALL ELASET('A',N,N-NR,ZERO,ZERO,V(1,NR+1),LDV)
                   CALL EGESVD( 'O', 'A', N, N, V, LDV, S, V, LDV,      
     +             U, LDU, WORK(N+1), LWORK-N, INFO )
*
                   DO 1113 p = 1, N
                      DO 1114 q = p + 1, N
...
*
     ...
     +                          LDU)
                        CALL ELASET('A',M-N,N1-N,ZERO,ONE,              
     +          U(N+1,N+1), LDU )
                      END IF
                   END IF
                ELSE
...
*                  singular vectors of R
 ...
 1196              CONTINUE
                   IF ( NR .GT. 1 )              CALL ELASET('U',NR-1,
     +NR-1,ZERO,ZERO,U(1,NR+2),LDU)
                   CALL EGEQRF( N, NR, U(1,NR+1), LDU, WORK(N+1),       
 ...
 1143              CONTINUE
                  ...
                  CALL EGESVD( 'S', 'O', NR, NR, V, LDV, S, U, LDU,     
     ...
     +                    LDV)
                  CALL EORMQR('R','C', N, N, NR, U(1,NR+1), LDU,        
...
*
         ELSE
*
*            .. apply EGESVD to R [[this is the recommended option]]
*
             IF ( WNTVR .OR. ( NR .EQ. N ) ) THEN
*                .. copy R into [V] and overwrite V with the right singular vectors
                 CALL ELACPY( 'U', NR, N, A, LDA, V, LDV )
...
*               singular vectors of R stored in [U](1:NR,1:NR)
                CALL EGESVD( 'S', 'O', NR, N, V, LDV, S, U, LDU,        
     +        V, LDV, WORK(N+1), LWORK-N, INFO )
                CALL ELAPMT( .FALSE., NR, N, V, LDV, IWORK )
*               .. now [V](1:NR,1:N) contains V(1:N,1:NR)**T
*               .. assemble the left singular vector matrix U of dimensions
...
*               OPTRATIO = MAX( OPTRATIO, 2 )
               ...
               IF ( OPTRATIO * NR .GT. N ) THEN
                  CALL ELACPY( 'U', NR, N, A, LDA, V, LDV )
                  IF ( NR .GT. 1 )             CALL ELASET('L', NR-1,
     +NR-1, ZERO,ZERO, V(2,1),LDV)
*              .. the right singular vectors of R overwrite [V], the NR left
*                 singular vectors of R stored in [U](1:NR,1:NR)
                  CALL ELASET('A', N-NR,N, ZERO,ZERO, V(NR+1,1),LDV)
                  CALL EGESVD( 'S', 'O', N, N, V, LDV, S, U, LDU,       
     +           V, LDV, WORK(N+1), LWORK-N, INFO )
                  CALL ELAPMT( .FALSE., N, N, V, LDV, IWORK )
...
*                 (M x N1), i.e. (M x N) or (M x M).
     ...
     +           U(N+1,N+1), LDU )
                  ...
                  END IF
               ELSE
                  CALL ELACPY( 'U', NR, N, A, LDA, U(NR+1,1), LDU )
                  IF ( NR .GT. 1 )             CALL ELASET('L',NR-1,
     +NR-1,ZERO,ZERO,U(NR+2,1),LDU)
                  CALL EGELQF( NR, N, U(NR+1,1), LDU, WORK(N+1),        
     +          WORK(N+NR+1), LWORK-N-NR, IERR )
                  CALL ELACPY('L',NR,NR,U(NR+1,1),LDU,V,LDV)
                  IF ( NR .GT. 1 )             CALL ELASET('U',NR-1,
     +NR-1,ZERO,ZERO,V(1,2),LDV)
                  CALL EGESVD( 'S', 'O', NR, NR, V, LDV, S, U, LDU,     
     +             V, LDV, WORK(N+NR+1), LWORK-N-NR, INFO )
                  ...
                  CALL ELASET('A',N-NR,N-NR,ZERO,ONE,V(NR+1,NR+1),      
     +                    LDV)
                  CALL EORMLQ('R','N',N,N,NR,U(NR+1,1),LDU,WORK(N+1),   
...
*     exact zeros in EGESVD() applied to the (possibly truncated)
