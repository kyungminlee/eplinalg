...
*  ===========
*
*       SUBROUTINE YGEQR( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> YGEQR computes a QR factorization of a complex M-by-N matrix A:
*>
*>    A = Q * ( R ),
*>            ( 0 )
*>
...
*     .. External Subroutines ..
      EXTERNAL           YLATSQR, YGEQRT, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN ( M, N ).GT.0 ) THEN
        MB = ILAENV_EP( 1, 'YGEQR ', ' ', M, N, 1, -1 )
        NB = ILAENV_EP( 1, 'YGEQR ', ' ', M, N, 2, -1 )
      ELSE
        MB = M
        NB = 1
      END IF
...
*
      IF( ( M.LE.N ) .OR. ( MB.LE.N ) .OR. ( MB.GE.M ) ) THEN
        CALL YGEQRT( M, N, NB, A, LDA, T( 6 ), NB, WORK, INFO )
      ELSE
        CALL YLATSQR( M, N, MB, NB, A, LDA, T( 6 ), NB, WORK,           
     +      LWORK, INFO )
      END IF
*
      WORK( 1 ) = LWREQ
