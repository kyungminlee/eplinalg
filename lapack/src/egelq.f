...
*  ===========
*
*       SUBROUTINE EGELQ( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> EGELQ computes an LQ factorization of a real M-by-N matrix A:
*>
*>    A = ( L 0 ) *  Q
*>
*> where:
...
*>  ELASWLQ (if the matrix is short-and-wide) or EGELQT to compute
...
*     .. External Subroutines ..
      EXTERNAL           EGELQT, ELASWLQ, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN( M, N ).GT.0 ) THEN
        MB = ILAENV_EP( 1, 'EGELQ ', ' ', M, N, 1, -1 )
        NB = ILAENV_EP( 1, 'EGELQ ', ' ', M, N, 2, -1 )
      ELSE
        MB = 1
        NB = N
      END IF
...
*
      IF( ( N.LE.M ) .OR. ( NB.LE.M ) .OR. ( NB.GE.N ) ) THEN
        CALL EGELQT( M, N, MB, A, LDA, T( 6 ), MB, WORK, INFO )
      ELSE
        CALL ELASWLQ( M, N, MB, NB, A, LDA, T( 6 ), MB, WORK,           
     +      LWORK, INFO )
      END IF
*
      WORK( 1 ) = LWREQ
