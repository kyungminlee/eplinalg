...
*
*> \htmlonly
*> Download YHBGST + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/yhbgst.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/yhbgst.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/yhbgst.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE YHBGST( VECT, UPLO, N, KA, KB, AB, LDAB, BB, LDBB, X,
*                          LDX, WORK, RWORK, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO, VECT
...
*       .. Array Arguments ..
*       REAL(KIND=10)   RWORK( * )
*       COMPLEX(KIND=10)         AB( LDAB, * ), BB( LDBB, * ), WORK( * ),
*      $                   X( LDX, * )
*       ..
*
*
...
*> \param[in] BB
*> \verbatim
*>          BB is COMPLEX(KIND=10) array, dimension (LDBB,N)
*>          The banded factor S from the split Cholesky factorization of
*>          B, as returned by YPBSTF, stored in the first kb+1 rows of
*>          the array.
*> \endverbatim
*>
*> \param[in] LDBB
...
*> \param[out] X
*> \verbatim
*>          X is COMPLEX(KIND=10) array, dimension (LDX,N)
...
*
*  =====================================================================
      SUBROUTINE YHBGST( VECT, UPLO, N, KA, KB, AB, LDAB, BB, LDBB,     
...
*     .. Parameters ..
     ...
     +      CONE = ( 1.0E0_10, 0.0E0_10 ), ONE = 1.0E0_10 )
*     ..
*     .. Local Scalars ..
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, YESCAL, YGERC, YGERU, YLACGV,          
     +          YLAR2V,                    YLARGV, YLARTG, YLARTV, 
     +YLASET, YROT
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
      IF( WANTX )    CALL YLASET( 'Full', N, N, CZERO, CONE, X, LDX )
*
*     Set M to the splitting point m. It must be the same value as is
*     used in YPBSTF. The chosen value allows the arrays WORK and RWORK
...
*
            BII = REAL( BB( KB1, I ) , KIND=10)
            AB( KA1, I ) = ( REAL( AB( KA1, I ) , KIND=10) / BII ) / BII
            DO 20 J = I + 1, I1
   ...
   30       CONTINUE
     ...
     +               BB( J-I+KB1, I )*                                
     +CONJG( AB( K-I+KA1, I ) ) -                                CONJG( 
     +BB( K-I+KB1, I ) )*                                AB( J-I+KA1, I 
     +) +                                REAL( AB( KA1, I ) , KIND=10)* 
     +                               BB( J-I+KB1, I )*                  
     +              CONJG( BB( K-I+KB1, I ) )
   40          CONTINUE
...
*
               CALL YESCAL( N-M, ONE / BII, X( M+1, I ), 1 )
               IF( KBT.GT.0 )             CALL YGERC( N-M, KBT, -CONE, 
     +X( M+1, I ), 1,                         BB( KB1-KBT, I ), 1, X( 
     +M+1, I-KBT ),                         LDX )
            END IF
*
...
*
            IF( NRT.GT.0 )          CALL YLARGV( NRT, AB( 1, J2T ), 
     +INCA, WORK( J2T-M ),                       KA1,                   
     +    RWORK( J2T-M ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 100 L = 1, KA - 1
                  CALL YLARTV( NR, AB( KA1-L, J2 ), INCA,               
...
*              blocks
*
               CALL YLAR2V( NR, AB( KA1, J2 ), AB( KA1, J2+1 ),         
     +              AB( KA, J2+1 ), INCA, RWORK( J2-M ),                
     +       WORK( J2-M ), KA1 )
*
               CALL YLACGV( NR, WORK( J2-M ), KA1 )
            END IF
*
...
*
            DO 110 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )             CALL YLARTV( NRT, AB( L, 
     +J2+KA1-L ), INCA,                          AB( L+1, J2+KA1-L ), 
...
*
               DO 120 J = J2, J1, KA1
                  CALL YROT( N-M, X( M+1, J ), 1, X( M+1, J+1 ), 1,     
...
*
            DO 140 L = KB - K, 1, -1
               NRT = ( N-J2+KA+L ) / KA1
               IF( NRT.GT.0 )             CALL YLARTV( NRT, AB( L, 
     +J2-L+1 ), INCA,                          AB( L+1, J2-L+1 ), INCA, 
...
*
               CALL YLARGV( NR, AB( 1, J2 ), INCA, WORK( J2 ), KA1,     
     +                  RWORK( J2 ), KA1 )
*
...
*           Form  inv(S(i))**H * A * inv(S(i))
*
            BII = REAL( BB( 1, I ) , KIND=10)
            AB( 1, I ) = ( REAL( AB( 1, I ) , KIND=10) / BII ) / BII
            DO 250 J = I + 1, I1
  ...
  260       CONTINUE
     ...
     +     K ) ) - CONJG( BB( I-K+1, K ) )*                             
     + AB( I-J+1, J ) + REAL( AB( 1, I ) , KIND=10)*                    
...
*
               CALL YESCAL( N-M, ONE / BII, X( M+1, I ), 1 )
               IF( KBT.GT.0 )             CALL YGERU( N-M, KBT, -CONE, 
     +X( M+1, I ), 1,                         BB( KBT+1, I-KBT ), 
...
*           have been created outside the band
*
            IF( NRT.GT.0 )          CALL YLARGV( NRT, AB( KA1, J2T-KA ),
     + INCA,                       WORK( J2T-M ),                       
     +KA1, RWORK( J2T-M ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 330 L = 1, KA - 1
                  CALL YLARTV( NR, AB( L+1, J2-L ), INCA,               
...
*              blocks
*
               CALL YLAR2V( NR, AB( 1, J2 ), AB( 1, J2+1 ), AB( 2,      
     +                 J2 ),                       INCA, RWORK( J2-M ), 
     +WORK( J2-M ), KA1 )
*
               CALL YLACGV( NR, WORK( J2-M ), KA1 )
...
*
            DO 340 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )             CALL YLARTV( NRT, AB( KA1-L+1,
     + J2 ), INCA,                          AB( KA1-L, J2+1 ), INCA, 
...
*              which have been created outside the band
*
               CALL YLARGV( NR, AB( KA1, J2-KA ), INCA, WORK( J2 ),     
     +                  KA1,                       RWORK( J2 ), KA1 )
*
...
*              post-multiply X by inv(S(i))
*
               CALL YESCAL( NX, ONE / BII, X( 1, I ), 1 )
               IF( KBT.GT.0 )             CALL YGERU( NX, KBT, -CONE, 
     +X( 1, I ), 1,                         BB( KB, I+1 ), LDBB-1, X( 1,
     + I+1 ), LDX )
            END IF
*
*           store a(i1,i) in RA1 for use in next loop over K
...
*                 generate rotation to annihilate a(i+k-ka-1,i)
*
                  CALL YLARTG( AB( K+1, I ), RA1, RWORK( I+K-KA ),      
     +                    WORK( I+K-KA ), RA )
*
*                 create nonzero element a(i+k-ka-1,i+k) outside the
...
*           have been created outside the band
*
            IF( NRT.GT.0 )          CALL YLARGV( NRT, AB( 1, J1+KA ), 
     +INCA, WORK( J1 ),                       KA1,                      
     + RWORK( J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 580 L = 1, KA - 1
                  CALL YLARTV( NR, AB( KA1-L, J1+L ), INCA,             
...
*              blocks
*
               CALL YLAR2V( NR, AB( KA1, J1 ), AB( KA1, J1-1 ),         
     +              AB( KA, J1 ), INCA, RWORK( J1 ), WORK( J1 ),        
     +               KA1 )
*
               CALL YLACGV( NR, WORK( J1 ), KA1 )
            END IF
*
*           start applying rotations in 1st set from the right
*
            DO 590 L = KA - 1, KB - K + 1, -1
               NRT = ( J2+L-1 ) / KA1
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )             CALL YLARTV( NRT, AB( L, J1T 
     +), INCA,                          AB( L+1, J1T-1 ), INCA, RWORK( 
     +J1T ),                          WORK( J1T ), KA1 )
  590       CONTINUE
*
            IF( WANTX ) THEN
...
*
               DO 600 J = J1, J2, KA1
                  CALL YROT( NX, X( 1, J ), 1, X( 1, J-1 ), 1,          
     +              RWORK( J ), WORK( J ) )
  600          CONTINUE
            END IF
  610    CONTINUE
...
*
            DO 620 L = KB - K, 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )             CALL YLARTV( NRT, AB( L, 
     +J1T+KA ), INCA,                          AB( L+1, J1T+KA-1 ), 
     +INCA,                          RWORK( M-KB+J1T+KA ),              
...
*              which have been created outside the band
*
               CALL YLARGV( NR, AB( 1, J1+KA ), INCA,                   
     +    WORK( M-KB+J1 ),                       KA1, RWORK( M-KB+J1 ), 
     +KA1 )
*
*              apply rotations in 2nd set from the left
*
...
*
     ...
     +        WORK( M-KB+J1 ), KA1 )
*
               CALL YLACGV( NR, WORK( M-KB+J1 ), KA1 )
            END IF
*
*           start applying rotations in 2nd set from the right
*
...
*
  ...
  740       CONTINUE
     ...
     +     I ) ) - CONJG( BB( K-I+1, I ) )*                             
     + AB( J-I+1, I ) + REAL( AB( 1, I ) , KIND=10)*                    
...
*
               CALL YESCAL( NX, ONE / BII, X( 1, I ), 1 )
               IF( KBT.GT.0 )             CALL YGERC( NX, KBT, -CONE, 
     +X( 1, I ), 1, BB( 2,                         I ),                 
     +        1, X( 1, I+1 ), LDX )
            END IF
*
...
*                 generate rotation to annihilate a(i,i+k-ka-1)
*
                  CALL YLARTG( AB( KA1-K, I+K-KA ), RA1,                
...
*           have been created outside the band
*
            IF( NRT.GT.0 )          CALL YLARGV( NRT, AB( KA1, J1 ), 
     +INCA, WORK( J1 ),                       KA1,                      
     + RWORK( J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 810 L = 1, KA - 1
                  CALL YLARTV( NR, AB( L+1, J1 ), INCA, AB( L+2,        
     +                  J1-1 ),                          INCA, RWORK( 
     +J1 ), WORK( J1 ), KA1 )
  810          CONTINUE
*
...
*              blocks
*
               CALL YLAR2V( NR, AB( 1, J1 ), AB( 1, J1-1 ),             
...
*
            DO 820 L = KA - 1, KB - K + 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )             CALL YLARTV( NRT, AB( KA1-L+1,
     + J1T-KA1+L ), INCA,                          AB( KA1-L, J1T-KA1+L 
...
*              which have been created outside the band
*
               CALL YLARGV( NR, AB( KA1, J1 ), INCA, WORK( M-KB+J1 ),   
