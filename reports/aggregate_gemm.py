...
TRANSA, TRANSB, size) row with overlay/migrated GFLOPS and speedup."""
import re
...
out = ["# Comprehensive GEMM bench — OMP=1, all (TA, TB, size) combos",
       "",
...
for prec in PREC_ORDER:
    for (p, prefix), rs in sorted(by_routine.items()):
        ...
        out.append("")
        out.append("| TA | TB | size | overlay GF/s | migrated GF/s | speedup |")
        out.append("|----|----|------|--------------|---------------|---------|")
