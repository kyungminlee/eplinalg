      SUBROUTINE ETRMVT( UPLO, N, T, LDT, X, INCX, Y, INCY, W, INCW, Z, 
     +                   INCZ )
      IMPLICIT NONE
*
...
*     ..
*     .. Array Arguments ..
      REAL(KIND=10)   T( LDT, * ), W( * ), X( * ), Y( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  ETRMVT  performs the matrix-vector operations
*
*     x := T' *y, and w := T *z,
*
...
*           Unchanged on exit.
*
*  T      - REAL(KIND=10) array of DIMENSION ( LDT, n ).
...
*           Unchanged on exit.
*
*  X      - REAL(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCX ) ).
*           On exit, X = T' * y
*
*  INCX   - INTEGER.
...
*           Unchanged on exit.
*
*  Y      - REAL(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCY ) ).
...
*           Unchanged on exit.
*
*  W      - REAL(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCW ) ).
*           On exit, W = T * z
*
*  INCW   - INTEGER.
...
*           Unchanged on exit.
*
*  Z      - REAL(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCZ ) ).
...
*
     ...
     $    .TRUE. ) THEN
         CALL ECOPY( N, Y, INCY, X, INCX )
         CALL ETRMV( UPLO, 'C', 'N', N, T, LDT, X, INCX )
         CALL ECOPY( N, Z, INCZ, W, INCW )
         CALL ETRMV( UPLO, 'N', 'N', N, T, LDT, W, INCW )
         RETURN
      END IF
*
      RETURN
