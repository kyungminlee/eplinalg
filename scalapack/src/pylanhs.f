      REAL(KIND=10)   FUNCTION PYLANHS( NORM, N, A, IA, JA, DESCA,      
     +                                WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLANHS returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PYLANHS as described
*          above.
*
*  N       (global input) INTEGER
*          The number of rows and columns to be operated on i.e the
*          number of rows and columns of the distributed submatrix
*          sub( A ). When N = 0, PYLANHS is set to zero. N >= 0.
*
*  A       (local input) COMPLEX(KIND=10) pointer into the local memory
...
*     .. External Subroutines ..
     ...
     +       EGEBS2D, EGAMX2D, EGSUM2D,                    INFOG2L, 
     +PETREECOMB, YLASSQ
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
   ...
   20          CONTINUE
            ...
            END IF
*
*           Bug fix (vs upstream ScaLAPACK 2.2.3): see pelanhs.f.
            II = II + JB
...
*        process grid
*
         CALL EGSUM2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK, MAX( 1, NP ),
     +                  MYROW, 0 )
*
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 440 LL = JJ, JJ+JB-1
                  CALL YLASSQ( MIN( II+LL-JJ+1, IIA+NP-1 )-IIA+1,       
  ...
  470             CONTINUE
               ELSE
                  DO 480 LL = JJ, JJ + JB -1
                     CALL YLASSQ( MIN( II-1, IIA+NP-1 )-IIA+1,          
     +                   A( IIA+IOFFA ), 1, SCALE, SUM )
                     IOFFA = IOFFA + LDA
  480             CONTINUE
                  IF( MYROW.EQ.INXTROW .AND. II.LE.IIA+NP-1 )           
     +     CALL YLASSQ( 1, A( II+(JJ+JB-2)*LDA ), 1,                    
     +         SCALE, SUM )
               END IF
               JJ = JJ + JB
            END IF
...
*
         RWORK( 1 ) = SCALE
         RWORK( 2 ) = SUM
         CALL PETREECOMB( ICTXT, 'All', 2, RWORK, 0, 0, ECOMBSSQ )
