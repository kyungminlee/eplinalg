      SUBROUTINE YLAREF( TYPE, A, LDA, WANTZ, Z, LDZ, BLOCK, IROW1,     
...
*     .. Scalar Arguments ..
     ...
     $                   LDZ, LIHIZ, LILOZ
      COMPLEX(KIND=10)         T1, T2, T3, V2, V3
*     ..
*     .. Array Arguments ..
      COMPLEX(KIND=10)         A( LDA, * ), VECS( * ), Z( LDZ, * )
*     ..
*
*  Purpose
*  =======
*
*  YLAREF applies one or several Householder reflectors of size 3
...
*          Unchanged on exit.
*
*  A       (global input/output) COMPLEX(KIND=10) array, (LDA,*)
...
*
*  Z       (global input/output) COMPLEX(KIND=10) array, (LDZ,*)
...
*              when WANTZ is set.
*
*  VECS    (global input) COMPLEX(KIND=10) array of size 3*N (matrix size)
...
*  T1
*  T2
*  T3      (global input/output) COMPLEX(KIND=10)
...
*     .. Local Scalars ..
      INTEGER            J, K
      COMPLEX(KIND=10)         A1, A11, A2, A22, A3, A4, A5, B1, B2, B3,
     + B4,                    B5, H11, H22, SUM, SUM1, SUM2, SUM3, T12, 
     +T13,                    T22, T23, T32, T33, TMP1, TMP2, TMP3, V22,
     + V23,                    V32, V33
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
   ...
   10          CONTINUE
     ...
     +CONJG( T2 )*A( IROW1+1, J ) +                   CONJG( T3 )*A( 
     +IROW1+2, J )
   ...
   50       CONTINUE
         ELSE
            DO 60 J = ITMP1, ITMP2
               SUM = CONJG( T1 )*A( IROW1, J ) +                CONJG( 
     +T2 )*A( IROW1+1, J ) +                CONJG( T3 )*A( IROW1+2, J )
               A( IROW1, J ) = A( IROW1, J ) - SUM
               A( IROW1+1, J ) = A( IROW1+1, J ) - SUM*V2
               A( IROW1+2, J ) = A( IROW1+2, J ) - SUM*V3
   60       CONTINUE
