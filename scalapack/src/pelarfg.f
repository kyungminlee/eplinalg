      SUBROUTINE PELARFG( N, ALPHA, IAX, JAX, X, IX, JX, DESCX, INCX,   
     +                  TAU )
      IMPLICIT NONE
*
...
*  =======
*
*  PELARFG generates a real elementary reflector H of order n, such
*  that
*
...
*
*  ALPHA   (local output) REAL(KIND=10)
...
*
*  TAU     (local output) REAL(KIND=10) array, dimension  LOCc(JX)
...
*     .. Local Scalars ..
     ...
     $                   KNT, MYCOL, MYROW, NPCOL, NPROW
      REAL(KIND=10)   BETA, RSAFMN, SAFMIN, XNORM
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGEBR2D, EGEBS2D, PESCAL,      
     +              INFOG2L, PENRM2
*     ..
*     .. External Functions ..
...
*
         IF( MYCOL.EQ.IXCOL ) THEN
            ...
            CALL EGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, X( J ), 1 )
            ALPHA = X( J )
         ELSE
            CALL EGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA, 1,        
     +             MYROW, IXCOL )
         END IF
*
         INDXTAU = IIAX
...
*
         IF( MYROW.EQ.IXROW ) THEN
            J = IIAX+(JJAX-1)*DESCX( LLD_ )
            CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, X( J ), 1 )
            ALPHA = X( J )
         ELSE
            CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, ALPHA, 1,     
     +                IXROW, MYCOL )
         END IF
*
         INDXTAU = JJAX
...
*
      ...
      END IF
*
      CALL PENRM2( N-1, XNORM, X, IX, JX, DESCX, INCX )
*
...
*
   ...
   10       CONTINUE
            KNT = KNT + 1
            CALL PESCAL( N-1, RSAFMN, X, IX, JX, DESCX, INCX )
            BETA = BETA*RSAFMN
            ALPHA = ALPHA*RSAFMN
...
*
            ...
            CALL PESCAL( N-1, ONE/(ALPHA-BETA), X, IX, JX, DESCX, INCX )
*
