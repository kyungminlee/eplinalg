      SUBROUTINE PEORG2L( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEORG2L generates an M-by-N real distributed matrix Q denoting
...
*          as returned by PEGEQLF in the K columns of its distributed
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, PEELSET, 
     +                   PELARF, PELASET, PESCAL, PB_TOPGET,            
     +        PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORG2L', -INFO )
...
*
      CALL PELASET( 'All', M-N, N-K, ZERO, ZERO, A, IA, JA, DESCA )
      CALL PELASET( 'All', N, N-K, ZERO, ONE, A, IA+M-N, JA, DESCA )
*
      TAUJ = ZERO
...
*
         CALL PEELSET( A, IA+M-N+J-JA, J, DESCA, ONE )
         CALL PELARF( 'Left', M-N+J-JA+1, J-JA, A, IA, J, DESCA, 1, TAU,
     +                 A, IA, JA, DESCA, WORK )
*
     ...
     $      TAUJ = TAU( MIN( JJ, NQA0 ) )
         CALL PESCAL( M-N+J-JA, -TAUJ, A, IA, J, DESCA, 1 )
         CALL PEELSET( A, IA+M-N+J-JA, J, DESCA, ONE-TAUJ )
*
*        Set A(ia+m-n+j-ja+1:ia+m-1,j) to zero
*
         CALL PELASET( 'All', JA+N-1-J, 1, ZERO, ZERO, A, IA+M-N+J-JA+1,
     +                  J, DESCA )
*
   10 CONTINUE
*
