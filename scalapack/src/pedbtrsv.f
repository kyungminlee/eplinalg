      SUBROUTINE PEDBTRSV( UPLO, TRANS, N, BWL, BWU, NRHS, A, JA, DESCA,
...
*  =======
*
*  PEDBTRSV solves a banded triangular system of linear equations
*
...
*
*  Routine PEDBTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*     .. External Subroutines ..
     ...
     +                  EGEMM, EGERV2D, EGESD2D, ELAMOV, EMATADD,       
     +             ETBTRS, ETRMM, GLOBCHK, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 3 )
         CALL PXERBLA( ICTXT,                  
     +'PEDBTRSV, D&C alg.: only 1 block per proc',                  
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*MAX( BWL, BWU ) ) ) THEN
         INFO = -( 9*100+4 )
         CALL PXERBLA( ICTXT, 'PEDBTRSV, D&C alg.: NB too small',       
     +           -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -16
            CALL PXERBLA( ICTXT, 'PEDBTRSV: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEDBTRSV', -INFO )
         RETURN
      END IF
*
...
*
            CALL ETBTRS( UPLO, 'N', 'U', ODD_SIZE, BWL, NRHS,           
...
*             then use it on RHS
*
               CALL ELAMOV( 'N', BWL, NRHS,                       B( 
     +PART_OFFSET+ODD_SIZE-BWL+1 ), LLDB,                       WORK( 1 
     +), MAX_BW )
*
               CALL ETRMM( 'L', 'U', 'N', 'N', BWL, NRHS, -ONE,         
     +             A( ( OFST+( BWL+BWU+1 )+( ODD_SIZE-BWL )*            
     +          LLDA ) ), LLDA-1, WORK( 1 ), MAX_BW )
*
               CALL EMATADD( BWL, NRHS, ONE, WORK( 1 ), MAX_BW, ONE,    
...
*
               CALL EGEMM( 'N', 'N', BWU, NRHS, ODD_SIZE, -ONE, AF( 1 ),
...
*
            IF( MYCOL.GT.0 ) THEN
*
               CALL EGESD2D( ICTXT, MAX_BW, NRHS, WORK( 1 ), MAX_BW, 0, 
     +                       MYCOL-1 )
*
            END IF
*
...
*
               CALL EGERV2D( ICTXT, MAX_BW, NRHS, WORK( 1 ), MAX_BW, 0, 
     +                       MYCOL+1 )
*
*         Combine contribution to locally stored right hand sides
*
               CALL EMATADD( MAX_BW, NRHS, ONE, WORK( 1 ), MAX_BW, ONE, 
     +                       B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
            END IF
*
...
*
               CALL EGEMM( 'T', 'N', MAX_BW, NRHS, MAX_BW, -ONE,        
...
*
               CALL EGEMM( 'N', 'N', MAX_BW, NRHS, MAX_BW, -ONE,        
...
*       Solve with diagonal block
*
            CALL ETBTRS( 'L', 'T', 'U', MAX_BW, MIN( BWL, MAX_BW-1 ),   
     +                 NRHS, AF( ODD_SIZE*BWU+MBW2+1 ), MAX_BW+1,       
     +             B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
            IF( INFO.NE.0 ) THEN
...
*
            IF( MYCOL+LEVEL_DIST.LT.NPCOL-1 ) THEN
*
               CALL EGESD2D( ICTXT, MAX_BW, NRHS,                       
...
*
               CALL EGEMM( 'T', 'N', ODD_SIZE, NRHS, BWU, -ONE, AF( 1 ),
...
*           then use it on RHS
*
               CALL ELAMOV( 'N', BWL, NRHS, B( PART_OFFSET+ODD_SIZE+1 ),
     +                       LLDB, WORK( 1+MAX_BW-BWL ), MAX_BW )
*
               CALL ETRMM( 'L', 'U', 'T', 'N', BWL, NRHS, -ONE,         
     +             A( ( OFST+( BWL+BWU+1 )+( ODD_SIZE-BWL )*            
     +          LLDA ) ), LLDA-1, WORK( 1+MAX_BW-BWL ),                 
     +     MAX_BW )
*
               CALL EMATADD( BWL, NRHS, ONE, WORK( 1+MAX_BW-BWL ),      
...
*       Use main partition in each processor to solve locally
*
            CALL ETBTRS( UPLO, 'T', 'U', ODD_SIZE, BWL, NRHS,           
     +         A( OFST+1+BWU ), LLDA, B( PART_OFFSET+1 ),               
     +     LLDB, INFO )
*
         END IF
...
*       Use main partition in each processor to solve locally
*
            CALL ETBTRS( UPLO, 'T', 'N', ODD_SIZE, BWU, NRHS,           
...
*             then use it on RHS
*
               CALL ELAMOV( 'N', BWU, NRHS,                       B( 
     +PART_OFFSET+ODD_SIZE-BWU+1 ), LLDB,                       WORK( 1 
     +), MAX_BW )
*
               CALL ETRMM( 'L', 'L', 'T', 'N', BWU, NRHS, -ONE,         
     +             A( ( OFST+1+ODD_SIZE*LLDA ) ), LLDA-1,               
     +       WORK( 1 ), MAX_BW )
*
               CALL EMATADD( BWU, NRHS, ONE, WORK( 1 ), MAX_BW, ONE,    
     +                    B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
            END IF
*
...
*           processor's righthand-side.
*
               CALL EGEMM( 'N', 'N', BWL, NRHS, ODD_SIZE, -ONE,         
...
*
     ...
     +          MAX_BW-1 ) ), MAX_BW+1,                    B( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
            IF( INFO.NE.0 ) THEN
               GO TO 190
            END IF
...
*       Solve with diagonal block
*
            CALL ETBTRS( 'U', 'N', 'N', MAX_BW, MIN( BWU, MAX_BW-1 ),   
     +                 NRHS, AF( ODD_SIZE*BWU+MBW2+1-MIN( BWU,          
     +          MAX_BW-1 ) ), MAX_BW+1,                    B( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
...
*           processor's solution.
*
               CALL EGEMM( 'T', 'N', ODD_SIZE, NRHS, BWL, -ONE,         
...
*           then use it on RHS
*
               CALL ELAMOV( 'N', BWU, NRHS, B( PART_OFFSET+ODD_SIZE+1 ),
     +                       LLDB, WORK( 1+MAX_BW-BWU ), MAX_BW+BWL )
*
               CALL ETRMM( 'L', 'L', 'N', 'N', BWU, NRHS, -ONE,         
     +             A( ( OFST+1+ODD_SIZE*LLDA ) ), LLDA-1,               
     +       WORK( 1+MAX_BW-BWU ), MAX_BW+BWL )
*
               CALL EMATADD( BWU, NRHS, ONE, WORK( 1+MAX_BW-BWU ),      
...
*       Use main partition in each processor to solve locally
*
            CALL ETBTRS( UPLO, 'N', 'N', ODD_SIZE, BWU, NRHS,           
     +         A( OFST+1 ), LLDA, B( PART_OFFSET+1 ), LLDB,             
     +       INFO )
*
         END IF
