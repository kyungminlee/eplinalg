      SUBROUTINE PEGERQ2( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGERQ2 computes a RQ factorization of a real distributed M-by-N
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGERQ2', -INFO )
...
*        A(i+m-k,ja:j+n-k-1)
*
         CALL PELARFG( N-K+J-JA+1, AII, I+M-K, J+N-K, A, I+M-K, JA,     
     +             DESCA, DESCA( M_ ), TAU )
*
*        Apply H(i) to A(ia:i+m-k-1,ja:j+n-k) from the right
*
         CALL PEELSET( A, I+M-K, J+N-K, DESCA, ONE )
         CALL PELARF( 'Right', M-K+I-IA, N-K+J-JA+1, A, M-K+I, JA,      
     +           DESCA, DESCA( M_ ), TAU, A, IA, JA, DESCA, WORK )
         CALL PEELSET( A, I+M-K, J+N-K, DESCA, AII )
*
   10 CONTINUE
*
