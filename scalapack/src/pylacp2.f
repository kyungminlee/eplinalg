      SUBROUTINE PYLACP2( UPLO, M, N, A, IA, JA, DESCA, B, IB, JB,      
     +                DESCB )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLACP2 copies all or part of a distributed matrix A to another
*  distributed matrix B.  No communication is performed, PYLACP2
*  performs a local copy sub( A ) := sub( B ), where sub( A ) denotes
*  A(IA:IA+M-1,JA:JA+N-1) and sub( B ) denotes B(IB:IB+M-1,JB:JB+N-1).
*  PYLACP2 requires that only dimension of the matrix operands is
*  distributed.
*
*  Notes
*  =====
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YLAMOV, INFOG2L
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
   10          CONTINUE
               IF( ( N-ITOP ).GT.0 ) THEN
                  CALL YLAMOV( UPLO, IINXTA-IIBEGA+1, N-ITOP,           
...
*
   20          CONTINUE
               IF( JJAA.LE.( JJA+N-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL YLAMOV( 'All', MPAA, ITOP-JJAA+JJA,              
     +            A( IIAA+(JJAA-1)*LDA ), LDA,                          
     +B( IIBB+(JJBB-1)*LDB ), LDB )
                  CALL YLAMOV( UPLO, MPAA, HEIGHT,                      
...
*
            ELSE
*
               CALL YLAMOV( 'All', MP, N, A( IIA+(JJA-1)*LDA ),         
...
*
   30          CONTINUE
               IF( ( M-ILEFT ).GT.0 ) THEN
                  CALL YLAMOV( UPLO, M-ILEFT, JJNXTA-JJBEGA+1,          
...
*
   40          CONTINUE
               IF( IIAA.LE.( IIA+M-1 ) ) THEN
                  WIDE = IRIGHT - ILEFT
                  CALL YLAMOV( 'All', ILEFT-IIAA+IIA, NQAA,             
     +             A( IIAA+(JJAA-1)*LDA ), LDA,                         
     + B( IIBB+(JJBB-1)*LDB ), LDB )
                  CALL YLAMOV( UPLO, WIDE, NQAA,                        
...
*
            ELSE
*
               CALL YLAMOV( 'All', M, NQ, A( IIA+(JJA-1)*LDA ),         
     +              LDA, B( IIB+(JJB-1)*LDB ), LDB )
*
            END IF
*
