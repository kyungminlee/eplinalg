      SUBROUTINE PELABRD( M, N, NB, A, IA, JA, DESCA, D, E, TAUQ, TAUP, 
...
*  =======
*
*  PELABRD reduces the first NB rows and columns of a real general
...
*
*  This is an auxiliary routine called by PEGEBRD.
*
*  Notes
*  =====
*
...
*
*  D       (local output) REAL(KIND=10) array, dimension
...
*          matrix A.
*
*  E       (local output) REAL(KIND=10) array, dimension
...
*
*  TAUQ    (local output) REAL(KIND=10) array dimension
...
*
*  Y       (local output) REAL(KIND=10) pointer into the local memory
...
*     .. Local Scalars ..
     ...
     $                   JWY, K, MYCOL, MYROW, NPCOL, NPROW
      REAL(KIND=10)   ALPHA, TAU
      INTEGER            DESCD( DLEN_ ), DESCE( DLEN_ ),
     $                   DESCTP( DLEN_ ), DESCTQ( DLEN_ ),
     $                   DESCW( DLEN_ ), DESCWY( DLEN_ )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DESCSET, INFOG2L, PECOPY,      
     +              PEELGET, PEELSET, PEGEMV, PELARFG,                  
     +  PESCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MIN, MOD
...
*
            IF( K.GT.1 ) THEN
               CALL PEGEMV( 'No transpose', M-K+1, K-1, -ONE, A, I, JA, 
     +                      DESCA, Y, IY, JY+K-1, DESCY, 1, ONE, A, I,  
     +                     J, DESCA, 1 )
               CALL PEGEMV( 'No transpose', M-K+1, K-1, -ONE, X, IX+K-1,
     +                       JX, DESCX, A, IA, J, DESCA, 1, ONE, A, I, 
     +J,                       DESCA, 1 )
               CALL PEELSET( A, I-1, J, DESCA, ALPHA )
            END IF
*
*           Generate reflection Q(i) to annihilate A(i+1:ia+m-1,j)
*
            CALL PELARFG( M-K+1, ALPHA, I, J, A, I+1, J, DESCA, 1,      
     +               TAUQ )
            CALL PEELSET( D, 1, J, DESCD, ALPHA )
            CALL PEELSET( A, I, J, DESCA, ONE )
*
*           Compute Y(IA+I:IA+N-1,J)
*
            CALL PEGEMV( 'Transpose', M-K+1, N-K, ONE, A, I, J+1, DESCA,
     +                    A, I, J, DESCA, 1, ZERO, WORK( IPY ), 1, JWY, 
     +                   DESCWY, DESCWY( M_ ) )
            CALL PEGEMV( 'Transpose', M-K+1, K-1, ONE, A, I, JA, DESCA, 
     +                   A, I, J, DESCA, 1, ZERO, WORK, IW, 1, DESCW,   
     +                 1 )
            CALL PEGEMV( 'Transpose', K-1, N-K, -ONE, Y, IY, JY+K,      
     +              DESCY, WORK, IW, 1, DESCW, 1, ONE, WORK( IPY ),     
     +               1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PEGEMV( 'Transpose', M-K+1, K-1, ONE, X, IX+K-1, JX,   
     +                 DESCX, A, I, J, DESCA, 1, ZERO, WORK, IW, 1,     
     +               DESCW, 1 )
            CALL PEGEMV( 'Transpose', K-1, N-K, -ONE, A, IA, J+1, DESCA,
     +                    WORK, IW, 1, DESCW, 1, ONE, WORK( IPY ), 1,   
     +                 JWY, DESCWY, DESCWY( M_ ) )
*
            CALL PEELGET( 'Rowwise', ' ', TAU, TAUQ, 1, J, DESCTQ )
            CALL PESCAL( N-K, TAU, WORK( IPY ), 1, JWY, DESCWY,         
     +           DESCWY( M_ ) )
            CALL PECOPY( N-K, WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ),
     +                    Y, IY+K-1, JY+K, DESCY, DESCY( M_ ) )
*
*           Update A(i,j+1:ja+n-1)
*
            CALL PEGEMV( 'Transpose', K, N-K, -ONE, Y, IY, JY+K, DESCY, 
     +                   A, I, JA, DESCA, DESCA( M_ ), ONE, A, I, J+1,  
     +                  DESCA, DESCA( M_ ) )
            CALL PEGEMV( 'Transpose', K-1, N-K, -ONE, A, IA, J+1, DESCA,
     +                    X, IX+K-1, JX, DESCX, DESCX( M_ ), ONE, A, I, 
     +                   J+1, DESCA, DESCA( M_ ) )
            CALL PEELSET( A, I, J, DESCA, ALPHA )
*
*           Generate reflection P(i) to annihilate A(i,j+2:ja+n-1)
*
            CALL PELARFG( N-K, ALPHA, I, J+1, A, I,                     
     +MIN( J+2, N+JA-1 ), DESCA, DESCA( M_ ), TAUP )
            CALL PEELSET( E, I, 1, DESCE, ALPHA )
            CALL PEELSET( A, I, J+1, DESCA, ONE )
*
*           Compute X(I+1:IA+M-1,J)
*
            CALL PEGEMV( 'No transpose', M-K, N-K, ONE, A, I+1, J+1,    
     +                DESCA, A, I, J+1, DESCA, DESCA( M_ ), ZERO, X,    
     +                IX+K, JX+K-1, DESCX, 1 )
            CALL PEGEMV( 'No transpose', K, N-K, ONE, Y, IY, JY+K,      
     +              DESCY, A, I, J+1, DESCA, DESCA( M_ ), ZERO,         
     +           WORK, IW, 1, DESCW, 1 )
            CALL PEGEMV( 'No transpose', M-K, K, -ONE, A, I+1, JA,      
     +              DESCA, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,         
     +           JX+K-1, DESCX, 1 )
            CALL PEGEMV( 'No transpose', K-1, N-K, ONE, A, IA, J+1,     
     +               DESCA, A, I, J+1, DESCA, DESCA( M_ ), ZERO,        
     +            WORK, IW, 1, DESCW, 1 )
            CALL PEGEMV( 'No transpose', M-K, K-1, -ONE, X, IX+K, JX,   
     +                 DESCX, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,      
     +              JX+K-1, DESCX, 1 )
*
            CALL PEELGET( 'Columnwise', ' ', TAU, TAUP, I, 1, DESCTP )
            CALL PESCAL( M-K, TAU, X, IX+K, JX+K-1, DESCX, 1 )
   10    CONTINUE
*
      ELSE
*
...
*
            IF( K.GT.1 ) THEN
               CALL PEGEMV( 'Transpose', K-1, N-K+1, -ONE, Y, IY,       
     +                JY+K-1, DESCY, A, I, JA, DESCA, DESCA( M_ ),      
     +                 ONE, A, I, J, DESCA, DESCA( M_ ) )
               CALL PEGEMV( 'Transpose', K-1, N-K+1, -ONE, A, IA, J,    
     +                   DESCA, X, IX+K-1, JX, DESCX, DESCX( M_ ),      
     +                 ONE, A, I, J, DESCA, DESCA( M_ ) )
               CALL PEELSET( A, I, J-1, DESCA, ALPHA )
            END IF
*
*           Generate reflection P(i) to annihilate A(i,j+1:ja+n-1)
*
            CALL PELARFG( N-K+1, ALPHA, I, J, A, I, J+1, DESCA,         
     +            DESCA( M_ ), TAUP )
            CALL PEELSET( D, I, 1, DESCD, ALPHA )
            CALL PEELSET( A, I, J, DESCA, ONE )
*
*           Compute X(i+1:ia+m-1,j)
*
            CALL PEGEMV( 'No transpose', M-K, N-K+1, ONE, A, I+1, J,    
     +                DESCA, A, I, J, DESCA, DESCA( M_ ), ZERO, X,      
     +              IX+K, JX+K-1, DESCX, 1 )
            CALL PEGEMV( 'No transpose', K-1, N-K+1, ONE, Y, IY, JY+K-1,
     +                    DESCY, A, I, J, DESCA, DESCA( M_ ), ZERO,     
     +               WORK, IW, 1, DESCW, 1 )
            CALL PEGEMV( 'No transpose', M-K, K-1, -ONE, A, I+1, JA,    
     +                DESCA, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,       
     +             JX+K-1, DESCX, 1 )
            CALL PEGEMV( 'No transpose', K-1, N-K+1, ONE, A, IA, J,     
...
*
     ...
     +                 DESCA, 1 )
            CALL PEGEMV( 'No transpose', M-K, K, -ONE, X, IX+K, JX,     
...
*
            CALL PELARFG( M-K, ALPHA, I+1, J, A, MIN( I+2, M+IA-1 ),    
     +                 J, DESCA, 1, TAUQ )
            CALL PEELSET( E, 1, J, DESCE, ALPHA )
            CALL PEELSET( A, I+1, J, DESCA, ONE )
*
*           Compute Y(ia+i:ia+n-1,j)
*
            CALL PEGEMV( 'Transpose', M-K, N-K, ONE, A, I+1, J+1, DESCA,
     +                    A, I+1, J, DESCA, 1, ZERO, WORK( IPY ), 1,    
     +                JWY, DESCWY, DESCWY( M_ ) )
            CALL PEGEMV( 'Transpose', M-K, K-1, ONE, A, I+1, JA, DESCA, 
     +                   A, I+1, J, DESCA, 1, ZERO, WORK, IW, 1, DESCW, 
     +                   1 )
            CALL PEGEMV( 'Transpose', K-1, N-K, -ONE, Y, IY, JY+K,      
     +              DESCY, WORK, IW, 1, DESCW, 1, ONE, WORK( IPY ),     
     +               1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PEGEMV( 'Transpose', M-K, K, ONE, X, IX+K, JX, DESCX,  
     +                  A, I+1, J, DESCA, 1, ZERO, WORK, IW, 1, DESCW,  
     +                  1 )
            CALL PEGEMV( 'Transpose', K, N-K, -ONE, A, IA, J+1, DESCA,  
     +                  WORK, IW, 1, DESCW, 1, ONE, WORK( IPY ), 1,     
     +               JWY, DESCWY, DESCWY( M_ ) )
*
            CALL PEELGET( 'Rowwise', ' ', TAU, TAUQ, 1, J, DESCTQ )
