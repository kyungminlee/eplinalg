      SUBROUTINE PEPBTRF( UPLO, N, BW, A, JA, DESCA, AF, LAF, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEPBTRF computes a Cholesky factorization
...
*  subsequent calls to PEPBTRS to solve linear systems.
*
...
*          PEPBTRF and this is stored in AF. If a linear system
*          is to be solved using PEPBTRS after the factorization
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO, EAXPY,         
     +           DESC_CONVERT, EGEMM, EGERV2D, EGESD2D, ELAMOV,         
     +           ELATCPY, EPBTRF, EPOTRF, ESYRK, ETBTRS, ETRMM,         
     +           ETRRV2D, ETRSD2D, ETRSM, ETRTRS, GLOBCHK,              
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     + 'PEPBTRF, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*BW ) ) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT, 'PEPBTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PEPBTRF: auxiliary storage error ',      
     +            -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -10
            CALL PXERBLA( ICTXT, 'PEPBTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPBTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL EPBTRF( UPLO, ODD_SIZE, BW, A( OFST+1 ), LLDA, INFO )
*
         IF( INFO.NE.0 ) THEN
            INFO = MYCOL + 1
            GO TO 30
...
*           in main storage.
*
            CALL ELATCPY( 'L', BW, BW, AF( ODD_SIZE*BW+2*MBW2+1+BW-BW ),
...
*
*
            CALL ESYRK( UPLO, 'T', BW, BW, -ONE,                   AF( 
...
*
               CALL EGESD2D( ICTXT, BW, BW, AF( ODD_SIZE*BW+2*MBW2+1 ), 
     +                       BW, 0, MYCOL-1 )
*
*
               IF( MYCOL.LT.NP-1 ) THEN
...
*             H_i^T to H_i.
*
                  CALL ELATCPY( 'N', BW, BW, AF( ODD_SIZE-BW+1 ),       
     +                    ODD_SIZE, AF( ( ODD_SIZE )*BW+1 ), BW )
*
                  CALL ETRMM( 'R', 'U', 'T', 'N', BW, BW, -ONE,         
...
*
            CALL EGESD2D( ICTXT, BW, BW, AF( ODD_SIZE*BW+1 ), BW, 0,    
     +                 MYCOL-1 )
*
         END IF
*
...
*         operations.
*
         CALL ELAMOV( 'N', BW, BW, A( OFST+ODD_SIZE*LLDA+1 ), LLDA-1,   
...
*
         IF( MYCOL.LT.NPCOL-1 ) THEN
*
            CALL EGERV2D( ICTXT, BW, BW, AF( ODD_SIZE*BW+2*MBW2+1 ), BW,
     +                     0, MYCOL+1 )
*
*          Add contribution to diagonal block
*
            CALL EAXPY( MBW2, ONE, AF( ODD_SIZE*BW+2*MBW2+1 ), 1,       
...
*
         IF( MYCOL-LEVEL_DIST.GE.0 ) THEN
            CALL EGERV2D( ICTXT, BW, BW, WORK( 1 ), BW, 0,              
     +       MYCOL-LEVEL_DIST )
*
            CALL EAXPY( MBW2, ONE, WORK( 1 ), 1,                   AF( 
...
*
         CALL EPOTRF( 'L', BW, AF( ODD_SIZE*BW+MBW2+1 ), BW, INFO )
*
         IF( INFO.NE.0 ) THEN
            INFO = NPCOL + MYCOL
         END IF
...
*             calcs.
*
            CALL ELAMOV( 'N', BW, BW, AF( ODD_SIZE*BW+1 ), BW,          
...
*
            CALL EGERV2D( ICTXT, BW, BW, AF( ODD_SIZE*BW+1 ), BW, 0,    
     +                 COMM_PROC )
*
            IF( INFO.EQ.0 ) THEN
*
...
*
            CALL ESYRK( 'L', 'T', BW, BW, -ONE, AF( ( ODD_SIZE )*BW+1 ),
     +                   BW, ZERO, WORK( 1 ), BW )
*
*         Send contribution to diagonal block's owning processor.
*
            CALL EGESD2D( ICTXT, BW, BW, WORK( 1 ), BW, 0,              
     +       MYCOL+LEVEL_DIST )
*
         END IF
...
*           left
*
               CALL EGERV2D( ICTXT, BW, BW, AF( ODD_SIZE*BW+2*MBW2+1 ), 
...
*
            CALL ELATCPY( 'L', PREV_TRI_SIZE_N, PREV_TRI_SIZE_M,        
     +             A( OFST+1 ), LLDA-1, AF( 1 ), ODD_SIZE )
*
            IF( INFO.EQ.0 ) THEN
*
               CALL ETBTRS( 'U', 'T', 'N', ODD_SIZE, BW, BW,            
     +           A( OFST+1 ), LLDA, AF( 1 ), ODD_SIZE, INFO )
*
*
*         Calculate the update block for previous proc, E_i = G_i{G_i}^T
...
*         operations.
*
         CALL ELATCPY( 'U', BW, BW, A( OFST+ODD_SIZE*LLDA+1+BW ),       
