      SUBROUTINE PEORMRQ( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCC( * )
      REAL(KIND=10)   A( * ), C( * ), TAU( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PEORMRQ overwrites the general real M-by-N distributed matrix
...
*        Q = H(1) H(2) . . . H(k)
*
*  as returned by PEGERQF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          H(i), IA <= i <= IA+K-1, as returned by PEGERQF in the
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension LOCc(IA+K-1).
*          This array contains the scalar factors TAU(i) of the
*          elementary reflectors H(i) as returned by PEGERQF.
...
*     .. Local Scalars ..
     ...
     $                   LCMP, LWMIN, MI, MPC0, MQA0, MYCOL, MYROW, NI,
     $                   NPCOL, NPROW, NQ, NQC0
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PELARFB,    
     +                PELARFT, PEORMR2, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
         ...
         IDUM2( 4 ) = 16
         IF( LEFT ) THEN
            CALL PCHK2MAT( K, 5, M, 3, IA, JA, DESCA, 9, M, 3, N, 4,
     $                     IC, JC, DESCC, 14, 4, IDUM1, IDUM2, INFO )
         ELSE
            CALL PCHK2MAT( K, 5, N, 4, IA, JA, DESCA, 9, M, 3, N, 4,
     $                     IC, JC, DESCC, 14, 4, IDUM1, IDUM2, INFO )
         END IF
      END IF
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORMRQ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N - K + IB
         END IF
         CALL PEORMR2( SIDE, TRANS, MI, NI, IB, A, IA, JA, DESCA, TAU,  
...
*
         CALL PELARFT( 'Backward', 'Rowwise', NQ-K+I+IB-IA, IB,         
     +         A, I, JA, DESCA, TAU, WORK, WORK( IPW ) )
         IF( LEFT ) THEN
*
*           H or H' is applied to C(ic:ic+m-k+i+ib-ia-1,jc:jc+n-1)
*
            MI = M - K + I + IB - IA
         ELSE
*
*           H or H' is applied to C(ic:ic+m-1,jc:jc+n-k+i+ib-ia-1)
*
            NI = N - K + I + IB - IA
         END IF
*
*        Apply H or H'
*
         CALL PELARFB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI,     
     +            IB, A, I, JA, DESCA, WORK, C, IC, JC, DESCC,          
     +       WORK( IPW ) )
   10 CONTINUE
*
