      SUBROUTINE PYPOCON( UPLO, N, A, IA, JA, DESCA, ANORM, RCOND, WORK,
...
*  =======
*
*  PYPOCON estimates the reciprocal of the condition number (in the
...
*     .. External Subroutines ..
     ...
     +               DESCSET, INFOG2L, PYAMAX, PCHK1MAT, PYLATRS,       
     +             PYLACON, PYERSCL, PB_TOPGET, PB_TOPSET,              
     +      PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPOCON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
   10 CONTINUE
      CALL PYLACON( N, WORK( IPV ), IV, JV, DESCV, WORK( IPX ), IX, JX, 
     +              DESCX, AINVNM, KASE )
      IF( KASE.NE.0 ) THEN
         IF( UPPER ) THEN
*
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PYLATRS( 'Upper', 'Conjugate transpose', 'Non-unit',   
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PYLATRS( 'Upper', 'No transpose', 'Non-unit', NORMIN,  
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PYLATRS( 'Lower', 'No transpose', 'Non-unit', NORMIN,  
     +                   N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,      
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PYLATRS( 'Lower', 'Conjugate transpose', 'Non-unit',   
     +                  NORMIN, N, A, IA, JA, DESCA, WORK( IPX ),       
...
*
         ...
         IF( SCALE.NE.ONE ) THEN
            CALL PYAMAX( N, WMAX, IXX, WORK( IPX ), IX, JX, DESCX, 1 )
            IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
               CALL PB_TOPGET( ICTXT, 'Broadcast', 'Columnwise', CBTOP )
               IF( MYROW.EQ.IAROW ) THEN
                  CALL YGEBS2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX, 1 )
               ELSE
                  CALL YGEBR2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX, 1,  
     +                         IAROW, MYCOL )
               END IF
            END IF
            IF( SCALE.LT.CABS1( WMAX )*SMLNUM .OR. SCALE.EQ.ZERO )
     $         GO TO 20
            CALL PYERSCL( N, SCALE, WORK( IPX ), IX, JX, DESCX, 1 )
         END IF
         GO TO 10
      END IF
*
