      SUBROUTINE PYLAUUM( UPLO, N, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLAUUM computes the product U * U' or L' * L, where the triangular
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      ...
      PARAMETER          ( CONE = 1.0E0_10 )
*     ..
*     .. Local Scalars ..
      INTEGER            I, J, JB, JN
*     ..
*     .. External Subroutines ..
      EXTERNAL           PYGEMM, PYHERK, PYLAUU2, PYTRMM
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL
...
*
         JB = JN-JA+1
         CALL PYLAUU2( 'Upper', JB, A, IA, JA, DESCA )
         IF( JB.LE.N-1 ) THEN
            CALL PYHERK( 'Upper', 'No transpose', JB, N-JB, ONE, A, IA, 
...
*
         DO 10 J = JN+1, JA+N-1, DESCA( NB_ )
            ...
            I = IA + J - JA
            CALL PYTRMM( 'Right', 'Upper', 'Conjugate transpose',       
     +             'Non-unit', J-JA, JB, CONE, A, I, J, DESCA,          
     +          A, IA, J, DESCA )
            CALL PYLAUU2( 'Upper', JB, A, I, J, DESCA )
            IF( J+JB.LE.JA+N-1 ) THEN
               CALL PYGEMM( 'No transpose', 'Conjugate transpose',      
     +                 J-JA, JB, N-J-JB+JA, CONE, A, IA, J+JB,          
     +             DESCA, A, I, J+JB, DESCA, CONE, A, IA,               
     +        J, DESCA )
               CALL PYHERK( 'Upper', 'No transpose', JB, N-J-JB+JA, ONE,
...
*
         JB = JN-JA+1
         CALL PYLAUU2( 'Lower', JB, A, IA, JA, DESCA )
         IF( JB.LE.N-1 ) THEN
            CALL PYHERK( 'Lower', 'Conjugate transpose', JB, N-JB, ONE, 
...
*
         DO 20 J = JN+1, JA+N-1, DESCA( NB_ )
            JB = MIN( N-J+JA, DESCA( NB_ ) )
            I = IA + J - JA
            CALL PYTRMM( 'Left', 'Lower', 'Conjugate Transpose',        
     +            'Non-unit', JB, J-JA, CONE, A, I, J, DESCA, A,        
     +            I, JA, DESCA )
            CALL PYLAUU2( 'Lower', JB, A, I, J, DESCA )
            IF( J+JB.LE.JA+N-1 ) THEN
               CALL PYGEMM( 'Conjugate transpose', 'No transpose', JB,  
     +                     J-JA, N-J-JB+JA, CONE, A, I+JB, J, DESCA,    
     +                   A, I+JB, JA, DESCA, CONE, A, I, JA, DESCA )
               CALL PYHERK( 'Lower', 'Conjugate transpose', JB,         
