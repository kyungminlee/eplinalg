      SUBROUTINE PYGEQPF( M, N, A, IA, JA, DESCA, IPIV, TAU, WORK,      
...
*  =======
*
*  PYGEQPF computes a QR factorization with column pivoting of a
...
*     .. External Subroutines ..
     ...
     +             YGERV2D, YGESD2D, CHK1MAT, YLARFG,                   
     + YSWAP, DESCSET, IGERV2D, IGESD2D, INFOG1L,                    
     +INFOG2L, PYELSET, PCHK1MAT, PYLARFC,                    PYLARFG, 
     +PEAMAX, PEYNRM2, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEQPF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      ...
      IF( MYCOL.EQ.IACOL ) THEN
         DO 60 KK = 0, JB-1
            CALL PEYNRM2( M, RWORK( JJ+KK ), A, IA, JA+KK, DESCA, 1 )
...
*
         IF( MYCOL.EQ.ICURCOL ) THEN
            DO 70 KK = 0, JB-1
               CALL PEYNRM2( M, RWORK( JJ+KK ), A, IA, J+KK, DESCA, 1 )
               RWORK( NQ+JJ+KK ) = RWORK( JJ+KK )
   70       CONTINUE
            JJ = JJ + JB
         END IF
...
*
     ...
     $                 JJ, ICURCOL )
         K = JA + N - J
         IF( K.GT.1 ) THEN
            CALL PEAMAX( K, TEMP, PVT, RWORK, 1, J, DESCN,              
     +      DESCN( M_ ) )
         ELSE
            PVT = J
         END IF
     ...
     $                    DESCA( CSRC_ ), JJPVT, IPCOL )
            IF( ICURCOL.EQ.IPCOL ) THEN
               IF( MYCOL.EQ.ICURCOL ) THEN
                  CALL YSWAP( MP, A( IIA+(JJ-1)*LDA ), 1,               
     +          A( IIA+(JJPVT-1)*LDA ), 1 )
                  ITEMP = IPIV( JJPVT )
                  IPIV( JJPVT ) = IPIV( JJ )
                  IPIV( JJ ) = ITEMP
            ...
            ELSE
               IF( MYCOL.EQ.ICURCOL ) THEN
*
                  CALL YGESD2D( ICTXT, MP, 1, A( IIA+(JJ-1)*LDA ), LDA, 
     +                          MYROW, IPCOL )
                  WORK( 1 ) = CMPLX( REAL( IPIV( JJ ) , KIND=10) , 
     +KIND=10)
                  WORK( 2 ) = CMPLX( RWORK( JJ ) , KIND=10)
                  WORK( 3 ) = CMPLX( RWORK( JJ + NQ ) , KIND=10)
                  CALL YGESD2D( ICTXT, 3, 1, WORK, 3, MYROW, IPCOL )
*
                  CALL YGERV2D( ICTXT, MP, 1, A( IIA+(JJ-1)*LDA ), LDA, 
     +                          MYROW, IPCOL )
...
*
                  CALL YGESD2D( ICTXT, MP, 1, A( IIA+(JJPVT-1)*LDA ),   
     +                        LDA, MYROW, ICURCOL )
                  CALL IGESD2D( ICTXT, 1, 1, IPIV( JJPVT ), 1, MYROW,
     $                          ICURCOL )
*
                  CALL YGERV2D( ICTXT, MP, 1, A( IIA+(JJPVT-1)*LDA ),   
     +                        LDA, MYROW, ICURCOL )
                  CALL YGERV2D( ICTXT, 3, 1, WORK, 3, MYROW, ICURCOL )
                  IPIV( JJPVT ) = INT( REAL( WORK( 1 ) , KIND=10) )
                  RWORK( JJPVT ) = REAL( WORK( 2 ) , KIND=10)
                  RWORK( JJPVT+NQ ) = REAL( WORK( 3 ) , KIND=10)
*
               END IF
*
            END IF
...
*
         ...
         IF( DESCA( M_ ).EQ.1 ) THEN
            IF( MYROW.EQ.ICURROW ) THEN
               IF( MYCOL.EQ.ICURCOL ) THEN
                  ...
                  AJJ = A( IOFFA )
                  CALL YLARFG( 1, AJJ, A( IOFFA ), 1, TAU( JJ ) )
                  IF( N.GT.1 ) THEN
                     ALPHA = CMPLX( ONE , KIND=10) - CONJG( TAU( JJ ) )
                     CALL YGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA,  
     +                                 1 )
                     CALL YSCAL( NQ-JJ, ALPHA, A( IOFFA+DESCA( LLD_ ) ),
     +                            DESCA( LLD_ ) )
                  END IF
     ...
     +                  TAU( JJ ), 1 )
               ...
               ELSE
                  IF( N.GT.1 ) THEN
                     CALL YGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA,  
     +                            1, ICURROW, ICURCOL )
                     CALL YSCAL( NQ-JJ+1, ALPHA, A( I ), DESCA( LLD_ ) )
                  END IF
               END IF
            ELSE IF( MYCOL.EQ.ICURCOL ) THEN
               CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAU( JJ ), 
     +                       1, ICURROW, ICURCOL )
            END IF
*
         ELSE
*
            CALL PYLARFG( M-J+JA, AJJ, I, J, A, MIN( I+1, IA+M-1 ), J,  
     +                   DESCA, 1, TAU )
            IF( J.LT.JA+N-1 ) THEN
*
*              Apply H(i) to A(ia+j-ja:ia+m-1,j+1:ja+n-1) from the left
*
               CALL PYELSET( A, I, J, DESCA, CMPLX( ONE , KIND=10) )
               CALL PYLARFC( 'Left', M-J+JA, JA+N-1-J, A, I, J, DESCA,  
     +                      1, TAU, A, I, J+1, DESCA, WORK )
            END IF
            CALL PYELSET( A, I, J, DESCA, AJJ )
*
         END IF
*
...
*
     ...
     $      ICURCOL = MOD( ICURCOL+1, NPCOL )
         IF( (JJA+NQ-JJ).GT.0 ) THEN
            IF( MYROW.EQ.ICURROW ) THEN
               CALL YGEBS2D( ICTXT, 'Columnwise', ' ', 1, JJA+NQ-JJ,    
     +                    A( II+( MIN( JJA+NQ-1, JJ )-1 )*LDA ),        
     +                LDA )
               CALL YCOPY( JJA+NQ-JJ, A( II+( MIN( JJA+NQ-1, JJ )       
     +               -1)*LDA ), LDA, WORK( MIN( JJA+NQ-1, JJ ) ),       
     +               1 )
            ELSE
               CALL YGEBR2D( ICTXT, 'Columnwise', ' ', JJA+NQ-JJ, 1,    
            ...
            DO 90 LL = JJ, JJ + JN - J - 1
               IF( RWORK( LL ).NE.ZERO ) THEN
                  ...
                  IF( TEMP2.LE.TOL3Z ) THEN
                     IF( IA+M-1.GT.I ) THEN
                        CALL PEYNRM2( IA+M-I-1, RWORK( LL ), A,         
