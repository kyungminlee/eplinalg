      SUBROUTINE PETRSEN( JOB, COMPQ, SELECT, PARA, N, T, IT, JT,      
...
*  =======
*
*  PETRSEN reorders the real Schur factorization of a real matrix
...
*
*  SEP     (global output) REAL(KIND=10)
...
*     .. Local Scalars ..
     ...
     $                   WRK1, IWRK1, WRK2, IWRK2, WRK3, IWRK3
      REAL(KIND=10)   ELEM, EST, SCALE, RNORM
*     .. Local Arrays ..
      INTEGER            DESCT12( DLEN_ ), MBNB2( 2 ), MMAX( 1 ),
     $                   MMIN( 1 )
      REAL(KIND=10)   DPDUM1( 1 )
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
      REAL(KIND=10)   PELANGE
      EXTERNAL           LSAME, NUMROC, PELANGE
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCINIT,             
     +       IGAMX2D, INFOG2L, PELACPY, PETRORD, PXERBLA,               
     +     PCHK1MAT, PCHK2MAT
*     $                   , PGESYCTD, PSYCTCON
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 .AND. .NOT.LQUERY ) THEN
         ...
         S = ONE
         SEP = ZERO
         CALL PXERBLA( ICTXT, 'PETRSEN', -INFO )
         RETURN
      ELSEIF( LQUERY ) THEN
...
*
      IF( M.EQ.N .OR. M.EQ.0 ) THEN
         IF( WANTS )
     $        S = ONE
         IF( WANTSP )         SEP = PELANGE( '1', N, N, T, IT, JT, 
     +DESCT, WORK )
         GO TO 50
      END IF
*
...
*
      CALL PETRORD( COMPQ, IWORK, PARA, N, T, IT, JT,      DESCT, Q, IQ,
     + JQ, DESCQ, WR, WI, M, WORK, LWORK,      IWORK(N+1), LIWORK-N, 
     +INFO )
*
      IF( WANTS ) THEN
*
...
*
     ...
     $        TCSRC, ICTXT, MAX(1,T12ROWS), IERR )
         CALL PELACPY( 'All', N1, N2, T, 1, N1+1, DESCT, WORK,         
...
*        of eigenvalues.
*
         RNORM = PELANGE( 'Frobenius', N1, N2, WORK, 1, 1+ICOFFT12,     
     +    DESCT12, DPDUM1 )
...
c     $        LWORK, IWORK(N+1), LIWORK-N, EST, ITER, IERR )
         EST = EST * SQRT(REAL(N1*N2, KIND=10))
         SEP = ONE / EST
         IF( IERR.LT.0 ) THEN
            INFO = N+4
         ELSE
