      SUBROUTINE PYTRCON( NORM, UPLO, DIAG, N, A, IA, JA, DESCA, RCOND, 
...
*  =======
*
*  PYTRCON estimates the reciprocal of the condition number of a
...
*     .. External Functions ..
      ...
      REAL(KIND=10)               PYLANTR, PELAMCH
      EXTERNAL           ICEIL, INDXG2P, LSAME, NUMROC, PYLANTR,        
     +            PELAMCH
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYTRCON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      ANORM = PYLANTR( NORM, UPLO, DIAG, N, N, A, IA, JA, DESCA, RWORK )
*
...
*
         ...
         KASE = 0
   10    CONTINUE
         CALL PYLACON( N, WORK( IPV ), IV, JV, DESCV, WORK( IPX ),      
...
*
               DESCX( CSRC_ ) = IACOL
               CALL PYLATRS( UPLO, 'No transpose', DIAG, NORMIN,        
...
*
               DESCX( CSRC_ ) = IACOL
               CALL PYLATRS( UPLO, 'Conjugate transpose', DIAG, NORMIN, 
     +                       N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,  
     +                      DESCX, SCALE, RWORK( IPN ), WORK( IPW ) )
               DESCX( CSRC_ ) = MYCOL
            END IF
...
*
            IF( SCALE.NE.ONE ) THEN
               CALL PYAMAX( N, WMAX, IXX, WORK( IPX ), IX, JX,          
     +          DESCX, 1 )
               IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
                  CALL PB_TOPGET( ICTXT, 'Broadcast', 'Columnwise',
     $                          CBTOP )
                  IF( MYROW.EQ.IAROW ) THEN
                     CALL YGEBS2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX,  
     +                            1 )
                  ELSE
                     CALL YGEBR2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX,  
     +                            1, IAROW, MYCOL )
                  END IF
               END IF
