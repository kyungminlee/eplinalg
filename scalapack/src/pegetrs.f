      SUBROUTINE PEGETRS( TRANS, N, NRHS, A, IA, JA, DESCA, IPIV, B,    
     +                 IB, JB, DESCB, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGETRS solves a system of distributed linear equations
*
*                   op( sub( A ) ) * X = sub( B )
*
*  with a general N-by-N distributed matrix sub( A ) using the LU
*  factorization computed by PEGETRF.
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGETRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PELAPIV( 'Forward', 'Row', 'Col', N, NRHS, B, IB, JB,     
     +             DESCB, IPIV, IA, 1, DESCIP, IDUM1 )
*
*        Solve L*X = sub( B ), overwriting sub( B ) with X.
*
         CALL PETRSM( 'Left', 'Lower', 'No transpose', 'Unit', N, NRHS, 
...
*        Solve L'*X = sub( B ), overwriting sub( B ) with X.
*
         CALL PETRSM( 'Left', 'Lower', 'Transpose', 'Unit', N, NRHS,    
     +             ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
*        Apply row interchanges to the solution vectors.
*
         CALL PELAPIV( 'Backward', 'Row', 'Col', N, NRHS, B, IB, JB,    
     +              DESCB, IPIV, IA, 1, DESCIP, IDUM1 )
*
      END IF
*
