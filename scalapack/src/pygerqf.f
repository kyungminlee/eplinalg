      SUBROUTINE PYGERQF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGERQF computes a RQ factorization of a complex distributed M-by-N
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK1MAT, PYGERQ2,    
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGERQF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL PYGERQ2( IB, N-M+I+IB-IA, A, I, JA, DESCA, TAU, WORK,  
     +                   LWORK, IINFO )
*
            IF( I.GT.IA ) THEN
*
...
*
               CALL PYLARFT( 'Backward', 'Rowwise', N-M+I+IB-IA, IB, A, 
     +                       I, JA, DESCA, TAU, WORK, WORK( IPW ) )
*
*              Apply H to A(ia:i-1,ja:ja+n-m+i+ib-ia-1) from the
*              right
*
               CALL PYLARFB( 'Right', 'No transpose', 'Backward',       
...
*
      IF( MU.GT.0 .AND. NU.GT.0 )    CALL PYGERQ2( MU, NU, A, IA, JA, 
