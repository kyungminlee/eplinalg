      SUBROUTINE PYUNM2R( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*  =======
*
*  PYUNM2R overwrites the general complex M-by-N distributed matrix
...
*        Q = H(1) H(2) . . . H(k)
*
*  as returned by PYGEQRF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          PYGEQRF in the K columns of its distributed matrix
...
*          elementary reflectors H(j) as returned by PYGEQRF.
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNM2R', -INFO )
...
*           Apply H(j) or H(j)'
*
            CALL PYELSET2( AJJ, A, IA+J-JA, J, DESCA, ONE )
            IF( NOTRAN ) THEN
               CALL PYLARF( SIDE, MI, NI, A, IA+J-JA, J, DESCA, 1, TAU, 
     +                      C, ICC, JCC, DESCC, WORK )
            ELSE
               CALL PYLARFC( SIDE, MI, NI, A, IA+J-JA, J, DESCA, 1, TAU,
     +                     C, ICC, JCC, DESCC, WORK )
            END IF
            CALL PYELSET( A, IA+J-JA, J, DESCA, AJJ )
*
   10    CONTINUE
*
