      SUBROUTINE PELAQR2( WANTT, WANTZ, N, KTOP, KBOT, NW, A, DESCA,    
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCZ( * )
      REAL(KIND=10)   A( * ), SI( KBOT ), SR( KBOT ), T( LDT, * ),      
     +              V( LDV, * ), WORK( * ), WI( * ), WR( * ),           
     +         Z( * )
*     ..
*
...
*
*  PELAQR2 accepts as input an upper Hessenberg matrix A and performs an
...
*  one processor. Normally, it is called by PELAQR1. All the inputs are
...
*          Normally NW .GE. 3 if PELAQR2 is called by PELAQR1.
*
*  A       (local input/output) REAL(KIND=10) array, dimension
*          (DESCH(LLD_),*)
...
*          LDV >= NW.
*
*  WR      (local workspace) REAL(KIND=10) array, dimension KBOT.
*  WI      (local workspace) REAL(KIND=10) array, dimension KBOT.
*
...
*     .. External Subroutines ..
     ...
     +     ELAQR3, DESCINIT, PEGEMM, PEGEMR2D, EGEMM,                   
     + ELAMOV, EGESD2D, EGERV2D, EGEBS2D, EGEBR2D,                    
     +IGEBS2D, IGEBR2D
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $        1, INFO )
      END IF
      CALL PEGEMR2D( DBLK, DBLK, A, I-DBLK+1, I-DBLK+1, DESCA, T, 1, 1, 
     +     DESCT, CONTXT )
      IF ( MYROW .EQ. II .AND. MYCOL .EQ. JJ ) THEN
         CALL ELASET( 'All', DBLK, DBLK, ZERO, ONE, V, LDV )
         CALL ELAQR3( .TRUE., .TRUE., DBLK, 1, DBLK, DBLK-1, T, LDT, 1, 
     ...
     +DBLK, WORK( 3*DBLK*DBLK+1 ), LWORK-3*DBLK*DBLK )
         CALL EGEBS2D( CONTXT, 'All', ' ', DBLK, DBLK, V, LDV )
         CALL IGEBS2D( CONTXT, 'All', ' ', 1, 1, ND, 1 )
      ELSE
         CALL EGEBR2D( CONTXT, 'All', ' ', DBLK, DBLK, V, LDV, II, JJ )
...
*
         CALL PEGEMR2D( DBLK, DBLK, T, 1, 1, DESCT, A, I-DBLK+1,        
     ...
     $              MYCOL, IROW, ICOL, II, JJ )
               IF( MYROW .EQ. II ) THEN
                  ...
                  DO 10 KKCOL = ICOL, ICOL1, HSTEP
                     ...
                     CALL EGEMM( 'T', 'N', DBLK, KLN, DBLK, ONE, V,     
     +                LDV, A( IROW+(KKCOL-1)*LDA ), LDA, ZERO, WORK,    
     +                 DBLK )
                     CALL ELAMOV( 'A', DBLK, KLN, WORK, DBLK,           
...
*
     ...
     $              MYROW, MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
               ...
               DO 20 KKROW = IROW, IROW1, VSTEP
                  ...
                  CALL EGEMM( 'N', 'N', KLN, DBLK, DBLK, ONE,           
     +       A( KKROW+(ICOL-1)*LDA ), LDA, V, LDV, ZERO, WORK,          
     +        KLN )
                  CALL ELAMOV( 'A', KLN, DBLK, WORK, KLN,               
...
*
   ...
   40                CONTINUE
     ...
     +                    DBLK, DOWN, MYCOL )
                        CALL EGERV2D( CONTXT, D1, KLN, WORK, DBLK, DOWN,
     +                        MYCOL )
                        CALL EGEMM( 'T', 'N', D1, KLN, D1, ONE,         
     +               V, LDV, A( IROW+(KKCOL-1)*LDA ), LDA, ONE,         
     +               WORK, DBLK )
                        CALL ELAMOV( 'A', D1, KLN, WORK, DBLK,          
     +              A( IROW+(KKCOL-1)*LDA ), LDA )
   50                CONTINUE
     ...
     +                LDA, ZERO, WORK, DBLK )
                        CALL EGESD2D( CONTXT, D1, KLN, WORK, DBLK, UP,  
     +                      MYCOL )
     ...
     +), DBLK )
                        CALL ELAMOV( 'A', D2, KLN, WORK( D1+1 ), DBLK,  
     +                      A( IROW+(KKCOL-1)*LDA ), LDA )
   60                CONTINUE
                  END IF
               END IF
...
*
   ...
   70             CONTINUE
     ...
     +         A( KKROW+(ICOL-1)*LDA ), LDA,                     V( 1, 
     +D1+1 ), LDV, ZERO, WORK( 1+D1*KLN ),                     KLN )
...
*
            IF( WANTT ) THEN
               DO 130 KKCOL = I+1, N, HSTEP
                  ...
                  CALL PEGEMM( 'T', 'N', DBLK, KLN, DBLK, ONE, V, 1, 1, 
     +                 DESCV, A, I-DBLK+1, KKCOL, DESCA, ZERO, WORK, 1, 
     +                 1, DESCWH )
                  CALL PEGEMR2D( DBLK, KLN, WORK, 1, 1, DESCWH, A,      
...
*
     ...
     $              0, CONTXT, LLDTMP, INFO )
               CALL PEGEMM( 'N', 'N', KLN, DBLK, DBLK, ONE, A, KKROW,   
     +            I-DBLK+1, DESCA, V, 1, 1, DESCV, ZERO, WORK, 1, 1,    
     +           DESCWV )
               CALL PEGEMR2D( KLN, DBLK, WORK, 1, 1, DESCWV, A, KKROW,  
     +             I-DBLK+1, DESCA, CONTXT )
  140       CONTINUE
*
*           Update vertical slab in Z.
*
     ...
     $                 0, 0, CONTXT, LLDTMP, INFO )
                  CALL PEGEMM( 'N', 'N', KLN, DBLK, DBLK, ONE, Z, KKROW,
     +                  I-DBLK+1, DESCZ, V, 1, 1, DESCV, ZERO, WORK, 1, 
     +                 1, DESCWV )
                  CALL PEGEMR2D( KLN, DBLK, WORK, 1, 1, DESCWV, Z,      
