      SUBROUTINE PEGBTRF( N, BWL, BWU, A, JA, DESCA, IPIV, AF, LAF,     
     +                WORK, LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGBTRF computes a LU factorization
...
*  subsequent calls to PEGBTRS to solve linear systems.
*
...
*     .. External Subroutines ..
     ...
     +                  EGBTRF, EGEMM, EGER, EGERV2D, EGESD2D, EGETRF,  
     +                  ELAMOV, ELASWP, ELATCPY, ESWAP, ETRRV2D,        
     +            ETRSD2D, ETRSM, GLOBCHK, IGAMX2D, IGEBR2D,            
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,
     + 'PEGBTRF, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.( BWL+BWU+1 ) ) ) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT, 'PEGBTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PEGBTRF: auxiliary storage error ',      
     +            -INFO )
         RETURN
      END IF
*
...
*        put minimum value of work into work( 1 )
            WORK( 1 ) = WORK_SIZE_MIN
            CALL PXERBLA( ICTXT, 'PEGBTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGBTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL ETRSD2D( ICTXT, 'U', 'N',                  MIN( BW, 
     +BWU+NUMROC( N, NB, MYCOL+1, 0,                  NPCOL ) ), BW, A( 
     +BIPTR ), LLDA-1, 0, MYCOL+1 )
*
      END IF
*
...
*
      IF( LN.GT.0 ) THEN
*
         CALL EGBTRF( LM, LN, LBWL, LBWU, A( APTR ), LLDA, IPIV, INFO )
*
         IF( INFO.NE.0 ) THEN
            INFO = INFO + NB*MYCOL
            GO TO 80
...
*
         ...
         LDB = LLDA - 1
*
*     Update the last BW columns of A_i (code modified from EGBTRS)
*
...
*
               CALL ESWAP( LNJ, A( LPTR ), LDB, A( JPTR ), LDB )
*
            END IF
*
...
*
            CALL EGER( LMJ, LNJ, -ONE, A( LPTR ), 1, A( JPTR ), LDB,    
     +              A( JPTR+1 ), LDB )
   40    CONTINUE
*
      END IF
...
*
      IF( MYCOL.GT.0 ) THEN
         CALL ETRRV2D( ICTXT, 'U', 'N', MIN( BW, LM ), BW, AF( 1 ), BW, 
     +                 0, MYCOL-1 )
*
...
*
            IF( L.NE.J ) THEN
               CALL ESWAP( BW, AF( ( L-1 )*BW+1 ), 1,                   
     +   AF( ( J-1 )*BW+1 ), 1 )
            END IF
*
            LPTR = BW + 1 + APTR + ( J-1 )*LLDA
*
            CALL EGER( NRHS, LMJ, -ONE, AF( ( J-1 )*BW+1 ), 1,          
...
*
      CALL ELAMOV( 'G', BM, BN, A( DBPTR ), LLDA-1, AF( BBPTR+BW*LDBB ),
     +              LDBB )
*
...
*
         ODPTR = ( LM-BM )*BW + 1
         CALL ELATCPY( 'G', BW, BM, AF( ODPTR ), BW,                  
...
*        performed.
         CALL EGETRF( N-LN, N-LN, AF( BBPTR+BW*LDBB ), LDBB,            
     +     IPIV( LN+1 ), INFO )
*
      END IF
*
...
*
               CALL EGESD2D( ICTXT, BM, 2*BW, AF( BBPTR+BW*LDBB ), LDBB,
     +                        0, NEICOL )
*
               CALL EGERV2D( ICTXT, BMN, 2*BW, AF( BBPTR+BM ), LDBB, 0, 
     +                       NEICOL )
*
               IF( NPACT.EQ.2 ) THEN
*
...
*
                  CALL ELAMOV( 'G', BMN, BW, AF( BBPTR+BM ), LDBB,      
...
*
            ...
            BM2 = BM
*
            CALL EGESD2D( ICTXT, BM, 2*BW, AF( BBPTR+BW*LDBB ), LDBB, 0,
     +                     NEICOL )
*
            CALL ELAMOV( 'G', BM, 2*BW, AF( BBPTR+BW*LDBB ), LDBB,      
     +              AF( BBPTR+BMN ), LDBB )
*
  ...
  130       CONTINUE
*
            CALL EGERV2D( ICTXT, BMN, 2*BW, AF( BBPTR+BW*LDBB ), LDBB,  
     +                   0, NEICOL )
*
            IF( NPACT.EQ.2 ) THEN
*
*                  Copy diagonal block to align whole system
*
               CALL ELAMOV( 'G', BM, BW, AF( BBPTR+BMN ), LDBB,         
...
*
         IF( NPACT.NE.2 ) THEN
*
            CALL EGETRF( BM+BMN, BW, AF( BBPTR+BW*LDBB ), LDBB,         
     +           IPIV( LN+1 ), INFO )
*
*   Backsolve left side
*
  ...
  150       CONTINUE
*
            CALL ELASWP( BW, AF( BBPTR ), LDBB, 1, BW, IPIV( LN+1 ), 1 )
*
            CALL ETRSM( 'L', 'L', 'N', 'U', BW, BW, ONE,                
     +   AF( BBPTR+BW*LDBB ), LDBB, AF( BBPTR ), LDBB )
*
*               Use partial factors to update remainder
*
            CALL EGEMM( 'N', 'N', BM+BMN-BW, BW, BW, -ONE,              
...
*
            NRHS = BW
*
            CALL ELASWP( NRHS, AF( BBPTR+2*BW*LDBB ), LDBB, 1, BW,      
     +              IPIV( LN+1 ), 1 )
*
            CALL ETRSM( 'L', 'L', 'N', 'U', BW, NRHS, ONE,              
     +     AF( BBPTR+BW*LDBB ), LDBB,                   AF( 
     +BBPTR+2*BW*LDBB ), LDBB )
*
*               Use partial factors to update remainder
*
            CALL EGEMM( 'N', 'N', BM+BMN-BW, NRHS, BW, -ONE,            
     +       AF( BBPTR+BW*LDBB+BW ), LDBB,                   AF( 
     +BBPTR+2*BW*LDBB ), LDBB, ONE,                   AF( 
     +BBPTR+2*BW*LDBB+BW ), LDBB )
*
*
...
*
*
               CALL ELAMOV( 'G', BM, BW, AF( BBPTR+BW ), LDBB,          
     +             AF( BBPTR+BW*LDBB ), LDBB )
               CALL ELAMOV( 'G', BM, BW, AF( BBPTR+2*BW*LDBB+BW ), LDBB,
     +                       AF( BBPTR+2*BW*LDBB ), LDBB )
*
...
*
            CALL EGETRF( BM+BMN, BM+BMN, AF( BBPTR+BW*LDBB ), LDBB,     
     +               IPIV( LN+1 ), INFO )
         END IF
*
      END IF
