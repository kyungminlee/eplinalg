      SUBROUTINE PYLATRZ( M, N, L, A, IA, JA, DESCA, TAU, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLATRZ reduces the M-by-N ( M<=N ) complex upper trapezoidal
*  matrix sub( A ) = [A(IA:IA+M-1,JA:JA+M-1) A(IA:IA+M-1,JA+N-L:JA+N-1)]
...
*     ..
*     .. Local Arrays ..
      INTEGER            DESCTAU( DLEN_ )
*     ..
*     .. External Subroutines ..
      EXTERNAL           DESCSET, INFOG1L, PYELSET, PYLACGV,            
     +        PYLARFG, PYLARZ
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
     ...
     $             NPROW )
*
      CALL DESCSET( DESCTAU, DESCA( M_ ), 1, DESCA( MB_ ), 1,
     $              DESCA( RSRC_ ), MYCOL, ICTXT, MAX( 1, MP ) )
*
      IF( M.EQ.N ) THEN
*
...
*           [ A(i, j) A(i,j1:ja+n-1) ]
*
            CALL PYLACGV( 1, A, I, J, DESCA, DESCA( M_ ) )
            CALL PYLACGV( L, A, I, J1, DESCA, DESCA( M_ ) )
            CALL PYLARFG( L+1, AII, I, J, A, I, J1, DESCA, DESCA( M_ ), 
     +                    TAU )
*
*           Apply H(i) to A(ia:i-1,j:ja+n-1) from the right
*
            CALL PYLARZ( 'Right', I-IA, JA+N-J, L, A, I, J1, DESCA,     
     +               DESCA( M_ ), TAU, A, IA, J, DESCA, WORK )
            CALL PYELSET( A, I, J, DESCA, CONJG( AII ) )
*
   20    CONTINUE
*
         CALL PYLACGV( M, TAU, IA, 1, DESCTAU, 1 )
*
      END IF
*
      RETURN
