      RECURSIVE SUBROUTINE PELAQR0( WANTT, WANTZ, N, ILO, IHI, H,      
     +DESCH, WR, WI, ILOZ, IHIZ, Z, DESCZ, WORK, LWORK,      IWORK, 
     +LIWORK, INFO, RECLEVEL )
*
...
*  =======
*
*  PELAQR0 computes the eigenvalues of a Hessenberg matrix H
...
*                   value
*          .GT. 0:  if INFO = i, PELAQR0 failed to compute all of
...
*
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9, RECMAX = 3 )
      ...
      REAL(KIND=10)   WILK1, WILK2
      PARAMETER          ( WILK1 = 0.75E0_10, WILK2 = -0.4375E0_10 )
      REAL(KIND=10)   ZERO, ONE
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10 )
*     ..
*     .. Local Scalars ..
      REAL(KIND=10)   AA, BB, CC, CS, DD, SN, SS, SWAP, ELEM, T0,       
...
*     .. External Functions ..
      ...
      EXTERNAL           PILAENVX_EP, NUMROC, INDXG2P, ICEIL, BLACS_PNUM
*     ..
*     .. Local Arrays ..
      INTEGER            DESCV( DLEN_ ), DESCT( DLEN_ ), DESCW( DLEN_ ),
     $                   PMAP( 64*64 )
      REAL(KIND=10)   ZDUM( 1, 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           PELACPY, PELAQR1, ELANV2, PELAQR3, PELAQR5,    
     +                PEELGET, ELAQR0, ELASET, PEGEMR2D
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      NB = DESCH( MB_ )
*
*     Tiny (sub-) matrices must use PELAQR1. (Stops recursion)
*
      IF( N.LE.NTINY ) THEN
*
...
*
      ELSEIF( N.LE.NB ) THEN
         IF( MYROW.EQ.DESCH(RSRC_) .AND. MYCOL.EQ.DESCH(CSRC_) ) THEN
            CALL ELAQR0( WANTT, WANTZ, N, ILO, IHI, H, DESCH(LLD_),     
     +       WR, WI, ILOZ, IHIZ, Z, DESCZ(LLD_), WORK, LWORK, INFO )
            IF( N.GT.2 )          CALL ELASET( 'L', N-2, N-2, ZERO, 
     +ZERO, H(3),               DESCH(LLD_) )
            LWKOPT = INT( WORK(1) )
            LIWKOPT = 1
         ELSE
...
*        NWR.GE.3.)
*
         NWR = PILAENVX_EP( ICTXT, 13, 'PELAQR0', JBCMPZ, N, ILO, IHI,  
     +       LWORK )
         NWR = MAX( 2, NWR )
...
*
         NWIN = PILAENVX_EP( ICTXT, 19, 'PELAQR0', JBCMPZ, N, NB, NB, 
     +NB )
         NSR = PILAENVX_EP( ICTXT, 15, 'PELAQR0', JBCMPZ, N, ILO, IHI,  
     +       MAX(NWIN,NB) )
         NSR = MIN( NSR, IHI-ILO )
...
*
*        Workspace query call to PELAQR3
*
         CALL PELAQR3( WANTT, WANTZ, N, ILO, IHI, NWR+1, H,         
     +DESCH, ILOZ, IHIZ, Z, DESCZ, LS, LD, WR, WI, H,         DESCH, N, 
     +H, DESCH, N, H, DESCH, WORK, -1, IWORK,         LIWORK, RECLEVEL )
         LWKOPT = LWKOPT + INT( WORK( 1 ) )
         LIWKOPT = IWORK( 1 )
*
*        Workspace query call to PELAQR5
*
         CALL PELAQR5( WANTT, WANTZ, 2, N, 1, N, N, WR, WI, H,         
     +DESCH, ILOZ, IHIZ, Z, DESCZ, WORK, -1, IWORK,         LIWORK )
*
*        Optimal workspace = MAX(PELAQR3, PELAQR5)
*
...
*
         NMIN = PILAENVX_EP( ICTXT, 12, 'PELAQR0', JBCMPZ, N, ILO, IHI, 
     +        LWORK )
         NMIN = MAX( NTINY, NMIN )
*
*        Nibble crossover point.
*
         NIBBLE = PILAENVX_EP( ICTXT, 14, 'PELAQR0', JBCMPZ, N, ILO, 
     +IHI,         LWORK )
         NIBBLE = MAX( 0, NIBBLE )
*
...
*
         KACC22 = PILAENVX_EP( ICTXT, 16, 'PELAQR0', JBCMPZ, N, ILO, 
     +IHI,         LWORK )
         KACC22 = MAX( 1, KACC22 )
         KACC22 = MIN( 2, KACC22 )
*
...
*
               ...
               ELSE
                  ...
                  IF( NW.LT.NWMAX ) THEN
                     ...
                     ELSE
                        KWTOP = KBOT - NW + 1
                        CALL PEELGET( 'All', '1-Tree', ELEM1, H, KWTOP, 
     +                       KWTOP-1, DESCH )
                        CALL PEELGET( 'All', '1-Tree', ELEM2, H,        
...
*
            IWORK(1) = IT
            CALL PELAQR3( WANTT, WANTZ, N, KTOP, KBOT, NW, H,           
     + DESCH, ILOZ, IHIZ, Z, DESCZ, LS, LD, WR, WI,            
     +WORK(IPV), DESCV, NHO, WORK(IPT), DESCT, NVE,            
     +WORK(IPW), DESCW, WORK(IPWRK), LWORK-IPWRK+1,            IWORK, 
     +LIWORK, RECLEVEL )
*
...
*              This may be lowered (slightly) if PELAQR3
...
*              PELAQR3 above or from the eigenvalues
*              of a trailing principal submatrix.
*
               IF( MOD( NDFL, KEXSH ).EQ.0 ) THEN
                  KS = KBOT - NS + 1
                  DO 30 I = KBOT, MAX( KS+1, KTOP+2 ), -2
                     CALL PEELGET( 'All', '1-Tree', ELEM1, H, I, I-1,   
     +                  DESCH )
                     CALL PEELGET( 'All', '1-Tree', ELEM2, H, I-1, I-2, 
     +                    DESCH )
                     CALL PEELGET( 'All', '1-Tree', ELEM3, H, I, I,     
     +                DESCH )
                     ...
                     CC = WILK2*SS
                     DD = AA
                     CALL ELANV2( AA, BB, CC, DD, WR( I-1 ), WI( I-1 ), 
     +                    WR( I ), WI( I ), CS, SN )
 30               CONTINUE
                  IF( KS.EQ.KTOP ) THEN
                     CALL PEELGET( 'All', '1-Tree', ELEM1, H, KS+1,     
     +                KS+1, DESCH )
                     WR( KS+1 ) = ELEM1
                     WI( KS+1 ) = ZERO
                     WR( KS ) = WR( KS+1 )
               ...
               ELSE
*
*                 Got NS/2 or fewer shifts? Use PELAQR0 or
*                 PELAQR1 on a trailing principal submatrix to
*                 get more.
*
                  IF( KBOT-KS+1.LE.NS / 2 ) THEN
                     KS = KBOT - NS + 1
                     KT = N - NS + 1
                     NPMIN = PILAENVX_EP( ICTXT, 23, 'PELAQR0', 'EN', 
     +NS,                     NB, NPROW, NPCOL )
c
c   Temporarily force NPMIN <= 8 since only PELAQR1 is used.
c
                     NPMIN = MIN(NPMIN, 8)
...
*
                        IF( NS.GT.NMIN .AND. RECURSION ) THEN
                           CALL PELACPY( 'All', NS, NS, H, KS, KS,      
     +                     DESCH, WORK(IPT), 1+IROFFH, 1+ICOFFH,        
     +                   DESCT )
                           CALL PELAQR0( .FALSE., .FALSE., IROFFH+NS,   
     +                        1+IROFFH, IROFFH+NS, WORK(IPT),           
     +                DESCT, WR( KS-IROFFH ),                           
     +WI( KS-IROFFH ), 1, 1, ZDUM,                           DESCZ, 
     +WORK( IPWRK ),                           LWORK-IPWRK+1, IWORK, 
     +LIWORK,                           INF, RECLEVEL+1 )
                        ELSE
                           CALL PELAMVE( 'All', NS, NS, H, KS, KS,      
     +                     DESCH, WORK(IPT), 1+IROFFH, 1+ICOFFH,        
     +                   DESCT, WORK(IPWRK) )
                           CALL PELAQR1( .FALSE., .FALSE., IROFFH+NS,   
     +                        1+IROFFH, IROFFH+NS, WORK(IPT),           
     +                DESCT, WR( KS-IROFFH ),                           
...
c                           END IF
 ...
 60                        CONTINUE
     ...
     $                       1, -1, -1, -1, -1, -1 )
                        CALL EGSUM2D( ICTXT, 'All', ' ', NS, 1, WR(KS), 
     +                       NS, -1, -1 )
                        CALL EGSUM2D( ICTXT, 'All', ' ', NS, 1, WI(KS), 
     +                       NS, -1, -1 )
                     END IF
                     KS = KS + INF
*
...
*
                     IF( KS.GE.KBOT ) THEN
                        CALL PEELGET( 'All', '1-Tree', AA, H, KBOT-1,   
     +                     KBOT-1, DESCH )
                        CALL PEELGET( 'All', '1-Tree', CC, H, KBOT,     
     +                   KBOT-1, DESCH )
                        CALL PEELGET( 'All', '1-Tree', BB, H, KBOT-1,   
     +                     KBOT, DESCH )
                        CALL PEELGET( 'All', '1-Tree', DD, H, KBOT,     
     +                   KBOT, DESCH )
                        CALL ELANV2( AA, BB, CC, DD, WR( KBOT-1 ),      
     +                  WI( KBOT-1 ), WR( KBOT ),                       
     + WI( KBOT ), CS, SN )
                        KS = KBOT - 1
                     END IF
...
*
               IF( KBOT-KS+1.EQ.2 ) THEN
                  IF( WI( KBOT ).EQ.ZERO ) THEN
                     CALL PEELGET( 'All', '1-Tree', ELEM, H, KBOT,      
     +               KBOT, DESCH )
...
*
               TOTNS = TOTNS + NS
               SWEEP = SWEEP + 1
               CALL PELAQR5( WANTT, WANTZ, KACC22, N, KTOP, KBOT,       
