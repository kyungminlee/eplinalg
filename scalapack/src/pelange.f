      REAL(KIND=10)   FUNCTION PELANGE( NORM, M, N, A, IA, JA, DESCA,   
     +                                   WORK )
      IMPLICIT NONE
*
...
*          Specifies the value to be returned in PELANGE as described
*          above.
*
*  M       (global input) INTEGER
*          The number of rows to be operated on i.e the number of rows
*          of the distributed submatrix sub( A ). When M = 0, PELANGE
*          is set to zero. M >= 0.
*
*  N       (global input) INTEGER
*          The number of columns to be operated on i.e the number of
*          columns of the distributed submatrix sub( A ). When N = 0,
*          PELANGE is set to zero. N >= 0.
*
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            IEAMAX, NUMROC
      EXTERNAL           LSAME, IEAMAX, NUMROC
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   20       CONTINUE
         END IF
         CALL EGAMX2D( ICTXT, 'All', ' ', 1, 1, VALUE, 1, I, J, -1,     
     +             0, 0 )
*
************************************************************************
* one norm
...
*
         IF( MYROW.EQ.0 ) THEN
            ...
            ELSE
               VALUE = ZERO
            END IF
            CALL EGAMX2D( ICTXT, 'Rowwise', ' ', 1, 1, VALUE, 1, I, J,  
     +                   -1, 0, 0 )
         END IF
*
************************************************************************
...
*
         IF( MYCOL.EQ.0 ) THEN
            ...
            ELSE
               VALUE = ZERO
            END IF
            CALL EGAMX2D( ICTXT, 'Columnwise', ' ', 1, 1, VALUE, 1, I,  
     +                   J, -1, 0, 0 )
         END IF
*
************************************************************************
...
*
         ...
         IF( NQ.GT.0 ) THEN
             DO 70 J = IOFFA, IOFFA + NQ*LDA - 1, LDA
                COLSSQ(1) = ZERO
                COLSSQ(2) = ONE
                CALL ELASSQ( MP, A( J ), 1, COLSSQ(1), COLSSQ(2) )
