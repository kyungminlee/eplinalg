      SUBROUTINE PESYGVX( IBTYPE, JOBZ, RANGE, UPLO, N, A, IA, JA,      
...
*  =======
*
*  PESYGVX computes all the eigenvalues, and optionally,
*  the eigenvectors
...
*          If DESCA( CTXT_ ) is incorrect, PESYGVX cannot guarantee
*          correct error reporting.
*
*  B       (local input/local output) REAL(KIND=10) pointer into the
...
*          insufficient space and PESYGVX is not able to detect this
...
*          PESYGVX is always able to detect insufficient space without
...
*             PESYGVX attempts to maintain orthogonality in
...
*             is returned.  Note that when RANGE='V', PESYGVX does
*             not know how many eigenvectors are requested until
*             the eigenvalues are computed.  Therefore, when RANGE='V'
*             and as long as LWORK is large enough to allow PESYGVX to
*             compute the eigenvalues, PESYGVX will compute the
...
*             PESTEIN will perform no better than ESTEIN on 1 processor.
...
*                  PESYGVX from computing all of the eigenvectors
*                  between VL and VU.  The number of eigenvectors
*                  computed is returned in NZ.
*                if (MOD(INFO/8,2).NE.0), then PESTEBZ failed to
*                  compute eigenvalues.
...
*     .. External Subroutines ..
     ...
     +               ESCAL, PCHK1MAT, PCHK2MAT, PEPOTRF, PESYEVX,       
     +             PESYNGST, PETRMM, PETRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
         ...
         IF( INFO.EQ.0 ) THEN
            IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
               ...
               ELSE
                  ...
                  WORK( 3 ) = ZERO
               END IF
               CALL EGEBS2D( DESCA( CTXT_ ), 'ALL', ' ', 3, 1, WORK, 3 )
            ELSE
               CALL EGEBR2D( DESCA( CTXT_ ), 'ALL', ' ', 3, 1, WORK, 3, 
     +                       0, 0 )
            END IF
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYGVX ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PEPOTRF( UPLO, N, B, IB, JB, DESCB, INFO )
      IF( INFO.NE.0 ) THEN
         IWORK( 1 ) = LIWMIN
         WORK( 1 ) = REAL( LWOPT , KIND=10)
         IFAIL( 1 ) = INFO
...
*
      CALL PESYNGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,      
     +          DESCB, SCALE, WORK, LWORK, INFO )
      CALL PESYEVX( JOBZ, RANGE, UPLO, N, A, IA, JA, DESCA, VL, VU, IL, 
...
*
            ...
            END IF
*
            CALL PETRSM( 'Left', UPLO, TRANS, 'Non-unit', N, NEIG, ONE, 
...
*
            ...
            END IF
*
            CALL PETRMM( 'Left', UPLO, TRANS, 'Non-unit', N, NEIG, ONE, 
     +                   B, IB, JB, DESCB, Z, IZ, JZ, DESCZ )
         END IF
      END IF
*
