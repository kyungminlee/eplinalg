      SUBROUTINE ELARRF2( N, D, L, LD, CLSTRT, CLEND,                   
     +  CLMID1, CLMID2, W, WGAP, WERR, TRYMID,                    
     +SPDIAM, CLGAPL, CLGAPR, PIVMIN, SIGMA,                    DPLUS, 
     +LPLUS, WORK, INFO )
*
...
*  W( CLEND ), ELARRF2 finds a new relatively robust representation
*  L D L^T - SIGMA I = L(+) D(+) L(+)^T such that at least one of the
*  eigenvalues of L(+) D(+) L(+)^T is relatively isolated.
*
*  This is an enhanced version of ELARRF that also tries shifts in
...
*
*  L       (input) REAL(KIND=10) array, dimension (N-1)
*          The (N-1) subdiagonal elements of the unit bidiagonal
*          matrix L.
*
*  LD      (input) REAL(KIND=10) array, dimension (N-1)
...
*
*  W       (input) REAL(KIND=10) array, dimension >=  (CLEND-CLSTRT+1)
*          The eigenvalue APPROXIMATIONS of L D L^T in ascending order.
*          W( CLSTRT ) through W( CLEND ) form the cluster of relatively
*          close eigenalues.
*
*  WGAP    (input/output) REAL(KIND=10) array, dimension >=  (CLEND-CLSTRT+1)
*          The separation from the right neighbor eigenvalue in W.
*
*  WERR    (input) REAL(KIND=10) array, dimension >=  (CLEND-CLSTRT+1)
...
*
*  SIGMA   (output) REAL(KIND=10)
*          The shift used to form L(+) D(+) L(+)^T.
*
*  DPLUS   (output) REAL(KIND=10) array, dimension (N)
*          The N diagonal elements of the diagonal matrix D(+).
*
*  LPLUS   (output) REAL(KIND=10) array, dimension (N-1)
*          The first (N-1) elements of LPLUS contain the subdiagonal
*          elements of the unit bidiagonal matrix L(+).
*
*  WORK    (workspace) REAL(KIND=10) array, dimension (2*N)
*          Workspace.
*
*  Further Details
*  ===============
...
*     .. Parameters ..
     ...
     +        FOUR = 4.0E0_10, QUART = 0.25E0_10,                      
     +MAXGROWTH1 = 8.E0_10,                      MAXGROWTH2 = 8.E0_10 )
*     ..
*     .. Local Scalars ..
