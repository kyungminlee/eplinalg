      SUBROUTINE PYCHEKPAD( ICTXT, MESS, M, N, A, LDA, IPRE, IPOST,     
     +                 CHKVAL )
      IMPLICIT NONE
*
...
*  =======
*
*  PYCHEKPAD checks that the padding around a local array has not
*  been overwritten since the call to PYFILLPAD.  3 types of errors
*  are reported:
*
...
*
     ...
     +                             REAL( A( I ) , KIND=10), AIMAG( A( I 
     +) )
               INFO = IAM
            END IF
   10    CONTINUE
      ELSE
         WRITE( *, FMT = * ) 'WARNING no pre-guardzone in PYCHEKPAD'
      END IF
*
...
*
   ...
   20    CONTINUE
      ELSE
         WRITE( *, FMT = * )          
     + 'WARNING no post-guardzone buffer in PYCHEKPAD'
      END IF
*
...
*
     ...
     +    I-IPRE-LDA*(J-1), J, REAL( A( I ) , KIND=10),                
     +AIMAG( A( I ) )
                  INFO = IAM
               END IF
   30       CONTINUE
