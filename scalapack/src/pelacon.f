      SUBROUTINE PELACON( N, V, IV, JV, DESCV, X, IX, JX, DESCX, ISGN,  
     +                   EST, KASE )
      IMPLICIT NONE
*
...
*  =======
*
*  PELACON estimates the 1-norm of a square, real distributed matrix A.
...
*
*  V       (local workspace) REAL(KIND=10) pointer into the local
...
*
*  X       (local input/local output) REAL(KIND=10) pointer into the
*          local memory to an array of dimension
*          LOCr(N+MOD(IX-1,MB_X)). On an intermediate return, X
*          should be overwritten by
*                A * X,   if KASE=1,
*                A' * X,  if KASE=2,
*          PELACON must be re-called with all the other parameters
*          unchanged.
*
...
*
*
*  EST     (global output) REAL(KIND=10)
*          An estimate (a lower bound) for norm(A).
*
*  KASE    (local input/local output) INTEGER
*          On the initial call to PELACON, KASE should be 0.
*          On an intermediate return, KASE will be 1 or 2, indicating
*          whether X should be overwritten by A * X  or A' * X.
*          On the final return from PELACON, KASE will again be 0.
*
*  Further Details
*  ===============
*
*  The serial version ELACON has been contributed by Nick Higham,
...
*     .. Local Scalars ..
     ...
     $                   K, MYCOL, MYROW, NP, NPCOL, NPROW
      REAL(KIND=10)   ALTSGN, ESTOLD, JLMAX, XMAX
*     ..
*     .. Local Arrays ..
      REAL(KIND=10)   ESTWORK( 1 ), TEMP( 1 ), WORK( 2 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, ECOPY, EGEBR2D, EGEBS2D,       
     +             IGSUM2D, INFOG2L, PEAMAX, PEASUM,                    
     +PEELGET
*     ..
*     .. External Functions ..
   ...
   20 CONTINUE
      IF( N.EQ.1 ) THEN
         IF( MYROW.EQ.IVXROW ) THEN
            ...
            CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, ESTWORK, 1 )
         ELSE
            CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, ESTWORK, 1,   
     +                  IVXROW, MYCOL )
         END IF
*        ... QUIT
         GO TO 150
      END IF
      CALL PEASUM( N, ESTWORK( 1 ), X, IX, JX, DESCX, 1 )
...
*
   40 CONTINUE
      CALL PEAMAX( N, XMAX, J, X, IX, JX, DESCX, 1 )
      IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
         IF( MYROW.EQ.IVXROW ) THEN
            WORK( 1 ) = XMAX
            WORK( 2 ) = REAL( J , KIND=10)
            CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 2, 1, WORK, 2 )
         ELSE
            CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 2, 1, WORK, 2,      
     +               IVXROW, MYCOL )
            XMAX = WORK( 1 )
            J = NINT( WORK( 2 ) )
         END IF
...
*
   70 CONTINUE
      CALL ECOPY( NP, X( IOFFVX ), 1, V( IOFFVX ), 1 )
      ESTOLD = ESTWORK( 1 )
      CALL PEASUM( N, ESTWORK( 1 ), V, IV, JV, DESCV, 1 )
...
*
  110 CONTINUE
     ...
     +               IVXROW, MYCOL )
         ...
         END IF
      END IF
      CALL PEELGET( 'Columnwise', ' ', JLMAX, X, JLAST, JX, DESCX )
  ...
  120 CONTINUE
     ...
     $                DESCX( RSRC_ ), NPROW )-IX+1
         ...
         ELSE
            ALTSGN = ONE
         END IF
         X( I ) = ALTSGN*( ONE+REAL( K-1 , KIND=10) / REAL( N-1 , 
     +KIND=10) )
  130 CONTINUE
      KASE = 1
      JUMP = 5
...
*
  140 CONTINUE
      CALL PEASUM( N, TEMP( 1 ), X, IX, JX, DESCX, 1 )
      IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
         IF( MYROW.EQ.IVXROW ) THEN
     ...
     +               IVXROW, MYCOL )
         END IF
      END IF
      TEMP( 1 ) = TWO*( TEMP( 1 ) / REAL( 3*N , KIND=10) )
