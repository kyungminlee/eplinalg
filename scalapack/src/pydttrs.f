      SUBROUTINE PYDTTRS( TRANS, N, NRHS, DL, D, DU, JA, DESCA, B, IB,  
...
*  =======
*
*  PYDTTRS solves a system of linear equations
*
...
*  stored in A(1:N,JA:JA+N-1) and AF by PYDTTRF.
...
*     .. Local Scalars ..
     ...
     $                   ODD_SIZE, PART_OFFSET, PART_SIZE,
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,      
     + 'PYDTTRS, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*INT_ONE )) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT,       'PYDTTRS, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -15
         CALL PXERBLA( ICTXT,       'PYDTTRS: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYDTTRS', -INFO )
         RETURN
      END IF
*
...
*
      IF( LSAME( TRANS, 'N' ) ) THEN
*
         CALL PYDTTRSV( 'L', 'N', N, NRHS, DL( PART_OFFSET+1 ),         
...
*
      ELSE
*
         CALL PYDTTRSV( 'U', 'C', N, NRHS, DL( PART_OFFSET+1 ),         
     +          D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,        
     +           DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,            
     +       LWORK, INFO )
*
...
*
         CALL PYDTTRSV( 'L', 'C', N, NRHS, DL( PART_OFFSET+1 ),         
     +          D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,        
     +           DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,            
     +       LWORK, INFO )
*
      ELSE
*
         CALL PYDTTRSV( 'U', 'N', N, NRHS, DL( PART_OFFSET+1 ),         
     +          D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,        
     +           DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,            
     +       LWORK, INFO )
*
