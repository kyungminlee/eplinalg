      SUBROUTINE PEROT( N, X, IX, JX, DESCX, INCX, Y, IY, JY, DESCY,    
...
*     .. Array Arguments ..
      INTEGER            DESCX( * ), DESCY( * )
      REAL(KIND=10)   X( * ), Y( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*  PEROT applies a planar rotation defined by CS and SN to the
...
*
*  CS      (global input) REAL(KIND=10)
*  SN      (global input) REAL(KIND=10)
...
*
*  WORK    (local input) REAL(KIND=10) array of dimension LWORK
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEROT', -INFO )
         RETURN
      ELSEIF( LQUERY ) THEN
         WORK( 1 ) = REAL(MNWRK, KIND=10)
         RETURN
      END IF
*
...
*
      IF( NPROCS.EQ.1 ) THEN
         IF( LEFT ) THEN
            CALL EROT( N, X((JX-1)*LLDX+IX), LLDX, Y((JY-1)*LLDY+IY),   
     +         LLDY, CS, SN )
         ELSEIF( RIGHT ) THEN
            CALL EROT( N, X((JX-1)*LLDX+IX), 1, Y((JY-1)*LLDY+IY),      
     +      1, CS, SN )
         END IF
         RETURN
      END IF
...
*
     ...
     $                       MYCOL, XLOC1, XLOC2, RSRC, CSRC )
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
                  IF( RSRC1.NE.RSRC2 ) THEN
                     CALL EGESD2D( ICTXT, 1, XYCOLS,                    
     +          X((XLOC2-1)*LLDX+XLOC1), LLDX,                          
     +    RSRC2, CSRC )
                     CALL EGERV2D( ICTXT, 1, XYCOLS, WORK, 1,           
     +                   RSRC2, CSRC )
                     CALL EROT( XYCOLS, X((XLOC2-1)*LLDX+XLOC1),        
     +                   LLDX, WORK, 1, CS, SN )
                  ELSE
     ...
     +RSRC1, CSRC )
                  CALL EGERV2D( ICTXT, 1, XYCOLS, WORK, 1,              
     +             RSRC1, CSRC )
                  CALL EROT( XYCOLS, WORK, 1, Y((YLOC2-1)*LLDY+YLOC1),  
     +                      LLDY, CS, SN )
               END IF
            END IF
 10      CONTINUE
     ...
     $                       MYCOL, XLOC1, XLOC2, RSRC, CSRC )
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
                  IF( CSRC1.NE.CSRC2 ) THEN
                     CALL EGESD2D( ICTXT, XYROWS, 1,                    
     +          X((XLOC2-1)*LLDX+XLOC1), LLDX,                          
     +    RSRC, CSRC2 )
                     CALL EGERV2D( ICTXT, XYROWS, 1, WORK, XYROWS,      
     +                        RSRC, CSRC2 )
                     CALL EROT( XYROWS, X((XLOC2-1)*LLDX+XLOC1),        
     +                   1, WORK, 1, CS, SN )
                  ELSE
     ...
     $                       MYCOL, YLOC1, YLOC2, RSRC, CSRC )
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
                  CALL EGESD2D( ICTXT, XYROWS, 1,                       
     +    Y((YLOC2-1)*LLDY+YLOC1), LLDY,                           RSRC,
     + CSRC1 )
                  CALL EGERV2D( ICTXT, XYROWS, 1, WORK, XYROWS,         
     +                  RSRC, CSRC1 )
                  CALL EROT( XYROWS, WORK, 1, Y((YLOC2-1)*LLDY+YLOC1),  
     +                      1, CS, SN )
               END IF
            END IF
 20      CONTINUE
