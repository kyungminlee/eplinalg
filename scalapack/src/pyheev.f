      SUBROUTINE PYHEEV( JOBZ, UPLO, N, A, IA, JA, DESCA, W, Z, IZ, JZ, 
...
*  =======
*
*  PYHEEV computes selected eigenvalues and, optionally, eigenvectors
*  of a real Hermitian matrix A by calling the recommended sequence
*  of ScaLAPACK routines.
*
*  In its present form, PYHEEV assumes a homogeneous system and makes
...
*          If DESCA( CTXT_ ) is incorrect, PYHEEV cannot guarantee
...
*                converge in YSTEQR2 after a total of 30*N iterations.
*                If INFO = N+1, then PYHEEV has detected heterogeneity
*                by finding that eigenvalues were not identical across
*                the process grid.  In this case, the accuracy of
*                the results from PYHEEV cannot be guaranteed.
*
...
*     .. Local Scalars ..
     ...
     $                   SIZEPCHETRD, SIZEPCUNMTR
      REAL(KIND=10)               ANRM, BIGNUM, EPS, RMAX, RMIN, SAFMIN,
     + SIGMA,                    SMLNUM
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
     ...
     +             YSTEQR2, DESCINIT, PYELGET, PYGEMR2D, PYHETRD,       
     +             PCHK1MAT, PCHK2MAT, PYLASCL, PYLASET, PYUNMTR,       
     +             PXERBLA, ECOPY, EGAMN2D, EGAMX2D, ESCAL
*     ..
*     .. Intrinsic Functions ..
...
*
            ...
            END IF
*
*           COMPLEX work space for PYHETRD
*
            CALL PYHETRD( UPLO, N, A, IA, JA, DESCA, RWORK( INDD ),     
     +                RWORK( INDE ), WORK( INDTAU ),                    
     + WORK( INDWORK ), -1, IINFO )
            SIZEPCHETRD = INT( ABS( WORK( 1 ) ) )
*
*           COMPLEX work space for PYUNMTR
*
            IF( WANTZ ) THEN
               CALL PYUNMTR( 'L', UPLO, 'N', N, N, A, IA, JA, DESCA,    
...
*           matrix required by YSTEQR2. This specific distribution
*           allows each process to do 1/pth of the work updating matrix
*           Q during YSTEQR2 and achieve some parallelization to an
...
*
     ...
     $                        INFO )
            END IF
*
*           COMPLEX work space for YSTEQR2
*
            IF( WANTZ ) THEN
               SIZECSTEQR2 = N*LDC
            ELSE
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PYHEEV', -INFO )
         IF( WANTZ )
...
*
      CALL PYHETRD( UPLO, N, A, IA, JA, DESCA, RWORK( INDRD ),          
...
*
      DO 10 I = 1, N
         CALL PYELGET( 'A', ' ', WORK( INDD+I-1 ), A, I+IA-1, I+JA-1,   
     +               DESCA )
         RWORK( INDRD+I-1 ) = REAL( WORK( INDD+I-1 ) , KIND=10)
   10 CONTINUE
      IF( LSAME( UPLO, 'U' ) ) THEN
         DO 20 I = 1, N - 1
            CALL PYELGET( 'A', ' ', WORK( INDE+I-1 ), A, I+IA-1, I+JA,  
     +                   DESCA )
            RWORK( INDRE+I-1 ) = REAL( WORK( INDE+I-1 ) , KIND=10)
   20    CONTINUE
      ELSE
         DO 30 I = 1, N - 1
            CALL PYELGET( 'A', ' ', WORK( INDE+I-1 ), A, I+IA, I+JA-1,  
     +                   DESCA )
            RWORK( INDRE+I-1 ) = REAL( WORK( INDE+I-1 ) , KIND=10)
   30    CONTINUE
      END IF
*
      IF( WANTZ ) THEN
*
         CALL PYLASET( 'Full', N, N, CZERO, CONE, WORK( INDWORK ), 1, 1,
     +                  DESCQR )
*
*        YSTEQR2 is a modified version of LAPACK's YSTEQR.  The
*        modifications allow each process to perform partial updates
*        to matrix Q.
*
         CALL YSTEQR2( 'I', N, RWORK( INDRD ), RWORK( INDRE ),          
     +        WORK( INDWORK ), LDC, NRC, RWORK( INDRWORK ),             
     +     INFO )
*
         CALL PYGEMR2D( N, N, WORK( INDWORK ), 1, 1, DESCQR, Z, IA, JA, 
     +                  DESCZ, CONTEXTC )
*
         CALL PYUNMTR( 'L', UPLO, 'N', N, N, A, IA, JA, DESCA,          
     +        WORK( INDTAU ), Z, IZ, JZ, DESCZ,                  WORK( 
     +INDWORK ), LLWORK, IINFO )
*
      ELSE
*
         CALL YSTEQR2( 'N', N, RWORK( INDRD ), RWORK( INDRE ),          
...
*
   ...
   40 CONTINUE
*
      CALL EGAMN2D( DESCA( CTXT_ ), 'All', ' ', J, 1, RWORK( 1+INDTAU ),
     +               J, 1, 1, -1, -1, 0 )
      CALL EGAMX2D( DESCA( CTXT_ ), 'All', ' ', J, 1, RWORK( 1+INDE ),  
     +             J, 1, 1, -1, -1, 0 )
*
      DO 50 I = 1, J
