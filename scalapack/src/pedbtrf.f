      SUBROUTINE PEDBTRF( N, BWL, BWU, A, JA, DESCA, AF, LAF, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * )
      REAL(KIND=10)   A( * ), AF( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PEDBTRF computes a LU factorization
...
*  subsequent calls to PEDBTRS to solve linear systems.
*
...
*
*  A       (local input/local output) REAL(KIND=10) pointer into
...
*
*  AF      (local output) REAL(KIND=10) array, dimension LAF.
*          Auxiliary Fillin Space.
*          Fillin is created during the factorization routine
*          PEDBTRF and this is stored in AF. If a linear system
*          is to be solved using PEDBTRS after the factorization
...
*          REAL(KIND=10) temporary workspace. This space may
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO, EAXPY, EDBTRF, 
     +                   DESC_CONVERT, EGEMM, EGEMV, EGERV2D, EGESD2D,  
     +                  ELAMOV, ELATCPY, ETBTRS, ETRMM, ETRRV2D,        
     +            ETRSD2D, GLOBCHK, IGAMX2D, IGEBR2D, IGEBS2D,          
     +          PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,
     + 'PEDBTRF, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*MAX( BWL, BWU ) ) ) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT, 'PEDBTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PEDBTRF: auxiliary storage error ',      
     +            -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -10
            CALL PXERBLA( ICTXT, 'PEDBTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEDBTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL ETRSD2D( ICTXT, 'U', 'N', NEXT_TRI_SIZE_M,                
...
*
      CALL EDBTRF( ODD_SIZE, ODD_SIZE, BWL, BWU, A( OFST+1 ), LLDA,     
     +         INFO )
*
      IF( INFO.NE.0 ) THEN
         INFO = MYCOL + 1
...
*
         CALL ELATCPY( 'U', BWL, BWL, A( ( OFST+( BWL+BWU+1 )+          
     +        ( ODD_SIZE-BWL )*LLDA ) ), LLDA-1,                  AF( 
     +ODD_SIZE*BWU+2*MBW2+1+MAX_BW-BWL ), MAX_BW )
         CALL ELAMOV( 'L', BWU, BWU, A( ( OFST+1+ODD_SIZE*LLDA ) ),     
     +            LLDA-1, AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1+MAX_BW-      
     +           BWU ), MAX_BW )
*
*         Perform the triangular system solve {L_i}{{BU'}_i} = {B_i}
*
         CALL ETBTRS( 'L', 'N', 'U', BWU, BWL, BWU,                 A( 
     +OFST+BWU+1+( ODD_SIZE-BWU )*LLDA ), LLDA,                 AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1+MAX_BW-BWU ),                 MAX_BW,
     + INFO )
*
*         Perform the triangular solve {U_i}^T{BL'}_i^T = {BL_i}^T
*
         CALL ETBTRS( 'U', 'T', 'N', BWL, BWU, BWL,                 A( 
     +OFST+1+( ODD_SIZE-BWL )*LLDA ), LLDA,                 AF( 
     +ODD_SIZE*BWU+2*MBW2+1+MAX_BW-BWL ), MAX_BW,                 INFO )
*
*         transpose resulting block to its location
*           in main storage.
*
         CALL ELATCPY( 'L', BWL, BWL, AF( ODD_SIZE*BWU+2*MBW2+1+MAX_BW- 
     +                 BWL ), MAX_BW, A( ( OFST+( BWL+BWU+1 )+          
     +        ( ODD_SIZE-BWL )*LLDA ) ), LLDA-1 )
*
*         Move the resulting block back to its location in main storage.
*
         CALL ELAMOV( 'L', BWU, BWU, AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1+  
...
*
*
         CALL EGEMM( 'T', 'N', MAX_BW, MAX_BW, MAX_BW, -ONE,            
     +    AF( ODD_SIZE*BWU+2*MBW2+1 ), MAX_BW,                AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW, ONE,                A( 
...
*
*
         CALL ETRRV2D( ICTXT, 'U', 'N', PREV_TRI_SIZE_M,                
...
*
            DO 60 I1 = 2, ODD_SIZE
               I2 = MIN( I1-1, BWL )
               CALL EGEMV( 'N', BWL, I2, -ONE,                      AF( 
     +WORK_U+1+( I1-1-I2 )*BWL ), BWL,                      A( 
     +OFST+BWU+1+I2+( I1-1-I2 )*LLDA ), LLDA-1,                      
...
*
            CALL ELAMOV( 'L', UP_PREV_TRI_SIZE_N, UP_PREV_TRI_SIZE_M,   
     +                 A( OFST+1 ), LLDA-1, AF( 1 ), BWU )
*
            DO 80 I1 = 1, ODD_SIZE
               I2 = MIN( BWU, I1-1 )
               CALL EGEMV( 'N', BWU, I2, -ONE, AF( ( I1-1-I2 )*BWU+1 ), 
...
*
   ...
   90       CONTINUE
*
            CALL EGEMM( 'N', 'T', BWU, BWL, ODD_SIZE, -ONE, AF( 1 ),    
...
*
            CALL EGESD2D( ICTXT, MAX_BW, MAX_BW,                     
...
*             as per requirements of BLAS routine ETRMM.
*             Since we have GU_i stored,
*             transpose HU_i to HU_i^T.
*
               CALL ELAMOV( 'N', BWL, BWL,                       AF( 
     +WORK_U+( ODD_SIZE-BWL )*BWL+1 ), BWL,                       AF( ( 
     +ODD_SIZE )*BWU+1+( MAX_BW-BWL ) ),                       MAX_BW )
*
               CALL ETRMM( 'R', 'U', 'T', 'N', BWL, BWL, -ONE,          
...
*
               CALL ELAMOV( 'N', BWU, BWU, AF( ( ODD_SIZE-BWU )*BWU+1 ),
     +                       BWU, AF( WORK_U+( ODD_SIZE )*BWL+1+MAX_BW- 
     +                      BWU ), MAX_BW )
*
               CALL ETRMM( 'R', 'L', 'N', 'N', BWU, BWU, -ONE,          
...
*
         CALL EGESD2D( ICTXT, MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+1 ),     
     +             MAX_BW, 0, MYCOL-1 )
*
         CALL EGESD2D( ICTXT, MAX_BW, MAX_BW,                  AF( 
     +WORK_U+ODD_SIZE*BWL+1 ), MAX_BW, 0, MYCOL-1 )
*
      END IF
*
...
*         operations.
*
      CALL ELAMOV( 'N', MAX_BW, MAX_BW, A( OFST+ODD_SIZE*LLDA+BWU+1 ),  
...
*
      IF( MYCOL.LT.NPCOL-1 ) THEN
*
         CALL EGERV2D( ICTXT, MAX_BW, MAX_BW,                  AF( 
     +ODD_SIZE*BWU+2*MBW2+1 ), MAX_BW, 0, MYCOL+1 )
*
*          Add contribution to diagonal block
*
         CALL EAXPY( MBW2, ONE, AF( ODD_SIZE*BWU+2*MBW2+1 ), 1,         
...
*
      IF( MYCOL-LEVEL_DIST.GE.0 ) THEN
         CALL EGERV2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ), MAX_BW, 0,     
     +             MYCOL-LEVEL_DIST )
*
         CALL EAXPY( MBW2, ONE, WORK( 1 ), 1, AF( ODD_SIZE*BWU+MBW2+1 ),
     +                1 )
*
      END IF
*
...
*
      CALL EDBTRF( MAX_BW, MAX_BW, MIN( MAX_BW-1, BWL ),              
...
*             calcs.
*
         CALL ELAMOV( 'N', MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+1 ), MAX_BW,
     +                 AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW )
*
         CALL ELAMOV( 'N', MAX_BW, MAX_BW, AF( WORK_U+ODD_SIZE*BWL+1 ), 
...
*
         CALL EGERV2D( ICTXT, MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+1 ),     
     +             MAX_BW, 0, COMM_PROC )
*
         CALL EGERV2D( ICTXT, MAX_BW, MAX_BW,                  AF( 
     +WORK_U+ODD_SIZE*BWL+1 ), MAX_BW, 0,                  COMM_PROC )
*
         IF( INFO.EQ.0 ) THEN
*
*
...
*
*
            CALL ETBTRS( 'L', 'N', 'U', BWU, MIN( BWL, BWU-1 ), BWU,    
...
*
*
            CALL ETBTRS( 'U', 'T', 'N', BWL, MIN( BWU, BWL-1 ), BWL,    
...
*
         CALL EGEMM( 'T', 'N', MAX_BW, MAX_BW, MAX_BW, -ONE,            
     +    AF( ( ODD_SIZE )*BWU+1 ), MAX_BW,                AF( WORK_U+( 
     +ODD_SIZE )*BWL+1 ), MAX_BW, ZERO,                WORK( 1 ), 
     +MAX_BW )
*
*         Send contribution to diagonal block's owning processor.
*
         CALL EGESD2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ), MAX_BW, 0,     
     +             MYCOL+LEVEL_DIST )
*
      END IF
...
*           left
*
            CALL EGERV2D( ICTXT, MAX_BW, MAX_BW,                     
...
*
*
*         Since ETBTRS has no "left-right" option, we must transpose
*
            CALL ELATCPY( 'N', MAX_BW, MAX_BW,                     AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW,                     WORK( 
     +1 ), MAX_BW )
*
            CALL ETBTRS( 'L', 'N', 'U', MAX_BW, MIN( BWL, MAX_BW-1 ),   
     +                 BWL, AF( ODD_SIZE*BWU+MBW2+1 ), MAX_BW+1,        
     +            WORK( 1+MAX_BW*( MAX_BW-BWL ) ), MAX_BW, INFO )
*
*         Transpose back
*
            CALL ELATCPY( 'N', MAX_BW, MAX_BW, WORK( 1 ), MAX_BW,       
     +              AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW )
*
*
*
...
*
            CALL ELATCPY( 'N', MAX_BW, MAX_BW,                     AF( 
     +ODD_SIZE*BWU+2*MBW2+1 ), MAX_BW,                     WORK( 1 ), 
     +MAX_BW )
*
            CALL ETBTRS( 'U', 'T', 'N', MAX_BW, MIN( BWU, MAX_BW-1 ),   
     +                 BWU, AF( ODD_SIZE*BWU+MBW2+1-MIN( BWU,           
     +         MAX_BW-1 ) ), MAX_BW+1,                    WORK( 
     +1+MAX_BW*( MAX_BW-BWU ) ), MAX_BW, INFO )
*
*         Transpose back
*
            CALL ELATCPY( 'N', MAX_BW, MAX_BW, WORK( 1 ), MAX_BW,       
...
*
         CALL EGEMM( 'N', 'T', MAX_BW, MAX_BW, MAX_BW, -ONE,            
     +    AF( ( ODD_SIZE )*BWU+2*MBW2+1 ), MAX_BW,                AF( 
     +WORK_U+( ODD_SIZE )*BWL+2*MBW2+1 ), MAX_BW,                ZERO, 
     +WORK( 1 ), MAX_BW )
*
*         Send contribution to diagonal block's owning processor.
*
...
*
            CALL EGEMM( 'N', 'N', MAX_BW, MAX_BW, MAX_BW, -ONE,         
     +          AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW,             
