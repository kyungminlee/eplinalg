      SUBROUTINE PELAMVE( UPLO, M, N, A, IA, JA, DESCA, B, IB, JB,      
     +               DESCB, DWORK )
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCB( * )
      REAL(KIND=10)   A( * ), B( * ), DWORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PELAMVE copies all or part of a distributed matrix A to another
...
*
*  DWORK   (local workspace) REAL(KIND=10) array
...
*
      IF( NPROCS.EQ.1 ) THEN
         CALL ELAMOV( UPLO, M, N, A((JA-1)*DESCA(LLD_)+IA),         
     +DESCA(LLD_), B((JB-1)*DESCB(LLD_)+IB),         DESCB(LLD_) )
      ELSEIF( FULL ) THEN
         CALL PEGEMR2D( M, N, A, IA, JA, DESCA, B, IB, JB, DESCB,       
     +  ICTXT )
      ELSE
         CALL PEGEMR2D( M, N, A, IA, JA, DESCA, DWORK, IB, JB, DESCB,   
     +      ICTXT )
         CALL PELACPY( UPLO, M, N, DWORK, IB, JB, DESCB, B, IB, JB,     
     +    DESCB )
      END IF
*
      RETURN
