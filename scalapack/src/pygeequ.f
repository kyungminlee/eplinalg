      SUBROUTINE PYGEEQU( M, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND, 
     +                    AMAX, INFO )
      IMPLICIT NONE
*
...
*     .. Scalar Arguments ..
      ...
      REAL(KIND=10)   AMAX, COLCND, ROWCND
*     ..
*     .. Array Arguments ..
      INTEGER            DESCA( * )
      REAL(KIND=10)   C( * ), R( * )
...
*  =======
*
*  PYGEEQU computes row and column scalings intended to equilibrate an
...
*
*  R       (local output) REAL(KIND=10) array, dimension LOCr(M_A)
...
*
*  C       (local output) REAL(KIND=10) array, dimension LOCc(N_A)
...
*          buted matrix A.
*
*  ROWCND  (global output) REAL(KIND=10)
...
*
*  COLCND  (global output) REAL(KIND=10)
...
*
*  AMAX    (global output) REAL(KIND=10)
...
*     .. Local Scalars ..
     ...
     $                   NPCOL, NPROW, NQ
      REAL(KIND=10)   BIGNUM, RCMAX, RCMIN, SMLNUM
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCSET, EGAMN2D,     
     +               EGAMX2D, IGAMX2D, INFOG2L, PCHK1MAT, PB_TOPGET,    
     +                PXERBLA
*     ..
*     .. External Functions ..
      INTEGER            INDXL2G, NUMROC
      REAL(KIND=10)   PELAMCH
      EXTERNAL           INDXL2G, NUMROC, PELAMCH
*     ..
*     .. Intrinsic Functions ..
...
*     .. Statement Function definitions ..
      ZABS1( ZDUM ) = ABS( REAL( ZDUM , KIND=10) ) + ABS( AIMAG( ZDUM ) 
     +)
*     ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEEQU', -INFO )
         RETURN
      END IF
*
...
*
   ...
   20    CONTINUE
         IOFFA = IOFFA + LDA
   30 CONTINUE
      CALL EGAMX2D( ICTXT, 'Rowwise', ROWCTOP, MP, 1, R( IIA ),         
...
*
      ...
      DO 40 I = IIA, IIA+MP-1
         ...
         RCMIN = MIN( RCMIN, R( I ) )
   40 CONTINUE
      CALL EGAMX2D( ICTXT, 'Columnwise', COLCTOP, 1, 1, RCMAX, 1, IDUMM,
     +               IDUMM, -1, -1, MYCOL )
      CALL EGAMN2D( ICTXT, 'Columnwise', COLCTOP, 1, 1, RCMIN, 1, IDUMM,
     +               IDUMM, -1, -1, MYCOL )
      AMAX = RCMAX
*
...
*
   ...
   80    CONTINUE
         IOFFA = IOFFA + LDA
   90 CONTINUE
      CALL EGAMX2D( ICTXT, 'Columnwise', COLCTOP, 1, NQ, C( JJA ),      
...
*     what merges across grid columns to give the global RCMAX/RCMIN.
      CALL EGAMX2D( ICTXT, 'Rowwise', ROWCTOP, 1, 1, RCMAX, 1, IDUMM,   
     +            IDUMM, -1, -1, MYCOL )
      CALL EGAMN2D( ICTXT, 'Rowwise', ROWCTOP, 1, 1, RCMIN, 1, IDUMM,   
     +            IDUMM, -1, -1, MYCOL )
*
      IF( RCMIN.EQ.ZERO ) THEN
*
