      SUBROUTINE PEGECON( NORM, N, A, IA, JA, DESCA, ANORM, RCOND, WORK,
...
*  =======
*
*  PEGECON estimates the reciprocal of the condition number of a general
*  distributed real matrix A(IA:IA+N-1,JA:JA+N-1), in either the 1-norm
*  or the infinity-norm, using the LU factorization computed by PEGETRF.
*
...
*
*  ANORM   (global input) REAL(KIND=10)
...
*     .. Local Scalars ..
     ...
     $                   NPCOL, NPMOD, NPROW, NQ, NQMOD
      REAL(KIND=10)   AINVNM, SCALE, SL, SMLNUM, SU, WMAX
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCSET, EGEBR2D,     
     +               EGEBS2D, INFOG2L, PCHK1MAT, PEAMAX,                
     +    PELATRS, PELACON, PERSCL, PB_TOPGET,                    
     +PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGECON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
   10 CONTINUE
      CALL PELACON( N, WORK( IPV ), IV, JV, DESCV, WORK( IPX ), IX, JX, 
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PELATRS( 'Lower', 'No transpose', 'Unit', NORMIN,      
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PELATRS( 'Upper', 'No transpose', 'Non-unit', NORMIN,  
     +                   N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,      
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PELATRS( 'Upper', 'Transpose', 'Non-unit', NORMIN,     
     +                N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,         
     +            DESCX, SU, WORK( IPNU ), WORK( IPW ) )
            DESCX( CSRC_ ) = MYCOL
*
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PELATRS( 'Lower', 'Transpose', 'Unit', NORMIN,         
     +            N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,             
     +        DESCX, SL, WORK( IPNL ), WORK( IPW ) )
            DESCX( CSRC_ ) = MYCOL
         END IF
...
*
         ...
         IF( SCALE.NE.ONE ) THEN
            CALL PEAMAX( N, WMAX, IXX, WORK( IPX ), IX, JX, DESCX, 1 )
            IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
               CALL PB_TOPGET( ICTXT, 'Broadcast', 'Columnwise', CBTOP )
               IF( MYROW.EQ.IAROW ) THEN
                  CALL EGEBS2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX, 1 )
               ELSE
                  CALL EGEBR2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX, 1,  
     +                         IAROW, MYCOL )
               END IF
            END IF
            IF( SCALE.LT.ABS( WMAX )*SMLNUM .OR. SCALE.EQ.ZERO )
     $         GO TO 20
            CALL PERSCL( N, SCALE, WORK( IPX ), IX, JX, DESCX, 1 )
         END IF
         GO TO 10
      END IF
*
