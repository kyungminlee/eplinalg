      SUBROUTINE PYUNMRZ( SIDE, TRANS, M, N, K, L, A, IA, JA, DESCA,    
...
*  =======
*
*  PYUNMRZ overwrites the general complex M-by-N distributed matrix
...
*        Q = H(1)' H(2)' . . . H(k)'
*
*  as returned by PYTZRZF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          H(i), IA <= i <= IA+K-1, as returned by PYTZRZF in the
...
*
*  TAU     (local input) COMPLEX(KIND=10), array, dimension LOCc(IA+K-1).
*          This array contains the scalar factors TAU(i) of the
*          elementary reflectors H(i) as returned by PYTZRZF.
...
*     .. External Subroutines ..
     ...
     +                  PB_TOPSET, PXERBLA, PYLARZB, PYLARZT,           
     +         PYUNMR3
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNMRZ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N
         END IF
         CALL PYUNMR3( SIDE, TRANS, MI, NI, IB, L, A, IA, JA, DESCA,    
...
*
         CALL PYLARZT( 'Backward', 'Rowwise', L, IB, A, I, JAA, DESCA,  
     +                TAU, WORK, WORK( IPW ) )
         IF( LEFT ) THEN
*
...
*        Apply H or H'
*
         CALL PYLARZB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI, IB, 
...
*     mirrors PYUNMRQ's structure (pyunmrq.f:454: "(RIGHT .AND. TRAN)
