      SUBROUTINE PETRORD( COMPQ, SELECT, PARA, N, T, IT, JT,      DESCT,
     + Q, IQ, JQ, DESCQ, WR, WI, M, WORK, LWORK,      IWORK, LIWORK, 
     +INFO )
*
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   Q( * ), T( * ), WI( * ), WORK( * ), WR( * )
*     ..
*
*  Purpose
*  =======
*
*  PETRORD reorders the real Schur factorization of a real matrix
...
*
*  T must be in Schur form (as returned by PELAHQR), that is, block
...
*
*  T       (local input/output) REAL(KIND=10) array,
...
*
*  Q       (local input/output) REAL(KIND=10) array,
...
*
*  WR      (global output) REAL(KIND=10) array, dimension (N)
*  WI      (global output) REAL(KIND=10) array, dimension (N)
...
*          0 <= M <= N.
*
*  WORK    (local workspace/output) REAL(KIND=10) array,
*          dimension (LWORK)
...
*  since PELAHQR does not compute Schur forms of submatrices anyway.
*
*  References
*  ==========
*
...
*     .. Parameters ..
     ...
     +RSRC_ = 7, CSRC_ = 8, LLD_ = 9,                      ZERO = 
     +0.0E0_10, ONE = 1.0E0_10 )
*     ..
*     .. Local Scalars ..
     ...
     $                   ROUND, LAST, WIN0S, WIN0E, WINE
      REAL(KIND=10)   ELEM, ELEM1, ELEM2, ELEM3, ELEM4, SN, CS, TMP,    
     +                ELEM5
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
      EXTERNAL           PELACPY, PXERBLA, PCHK1MAT, PCHK2MAT,          
     +          EGEMM, ELAMOV, ILACPY, CHK1MAT,                    
     +INFOG2L, EGSUM2D, EGESD2D, EGERV2D, EGEBS2D,                    
     +EGEBR2D, IGSUM2D, BLACS_GRIDINFO, IGEBS2D,                    
     +IGEBR2D, IGAMX2D, IGAMN2D, BELAAPP, BETREXC
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 .AND. .NOT.LQUERY ) THEN
         M = 0
         CALL PXERBLA( ICTXT, 'PETRORD', -INFO )
         RETURN
      ELSEIF( LQUERY ) THEN
         WORK( 1 ) = REAL(LWMIN, KIND=10)
         IWORK( 1 ) = LIWMIN
         RETURN
      END IF
*
...
*
      ...
      IPIW  = ICSRC + NUMWIN
*
*     Insert some pointers into REAL(KIND=10) workspace - for now we
 ...
 52      CONTINUE
         ...
         IF( ILOS.LT.N ) THEN
            IF( SELECT(ILOS+1).NE.0 .AND. MOD(ILOS,NB).EQ.0 ) THEN
               CALL PEELGET( 'All', TOP, ELEM, T, ILOS+1, ILOS, DESCT )
...
*
            LILO = IWORK(ILILO+K-1)
            IF( LILO.GT.NB ) THEN
               CALL PEELGET( 'All', TOP, ELEM, T, LILO, LILO-1, DESCT )
               IF( ELEM.NE.ZERO ) THEN
...
*
            ...
            IF( LIHI.LT.N ) THEN
               CALL PEELGET( 'All', TOP, ELEM, T, LIHI+1, LIHI, DESCT )
               IF( ELEM.NE.ZERO ) THEN
...
*
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
                  ...
                  IF( BUFFLEN.NE.0 ) THEN
                     DO 180 INDX = 1, ILEN
                        WORK( BUFFER+INDX-1 ) =                        
     +REAL( IWORK(IPIW+INDX-1) , KIND=10)
 180                 CONTINUE
                     CALL ELAMOV( 'All', DLEN, 1, WORK( IPW2 ),         
     +            DLEN, WORK(BUFFER+ILEN), DLEN )
                     IF( NPCOL.GT.1 .AND. DIR.EQ.1 ) THEN
                        CALL EGEBS2D( ICTXT, 'Row', TOP, BUFFLEN, 1,    
     +                    WORK(BUFFER), BUFFLEN )
                     END IF
                     IF( NPROW.GT.1 .AND. DIR.EQ.2 ) THEN
                        CALL EGEBS2D( ICTXT, 'Col', TOP, BUFFLEN, 1,    
     +                    WORK(BUFFER), BUFFLEN )
                     END IF
                  END IF
               ELSEIF( MYROW.EQ.RSRC .OR. MYCOL.EQ.CSRC ) THEN
     ...
     $                 THEN
                     ...
                     BUFFLEN = DLEN + ILEN
                     IF( BUFFLEN.NE.0 ) THEN
                        CALL EGEBR2D( ICTXT, 'Row', TOP, BUFFLEN, 1,    
     ...
     $                 THEN
                     ...
                     BUFFLEN = DLEN + ILEN
                     IF( BUFFLEN.NE.0 ) THEN
                        CALL EGEBR2D( ICTXT, 'Col', TOP, BUFFLEN, 1,    
     +                    WORK(BUFFER), BUFFLEN, RSRC, CSRC )
                     END IF
                  END IF
 ...
 190                    CONTINUE
                        CALL ELAMOV( 'All', DLEN, 1,                    
     +    WORK( BUFFER+ILEN ), DLEN,                        WORK( IPW2 
     +), DLEN )
                     END IF
                  END IF
               END IF
...
*                 account.
*
                  CALL ELASET( 'All', NWIN, NWIN, ZERO, ONE,            
     +      WORK( PDW ), NWIN )
                  CALL BELAAPP( 1, NWIN, NWIN, NCB, WORK( PDW ), NWIN,  
...
*
     ...
     +                  'No transpose',                              
     +'Non-unit', LROWS, KS, ONE,                              WORK( 
     +PDW+NWIN-KS ), NWIN,                              WORK(IPW3), 
     +LROWS )
                              CALL EGEMM( 'No transpose',               
...
*
                              CALL ELAMOV( 'All', LROWS, NWIN-KS,       
     +                       T((JLOC-1)*LLDT+ILOC), LLDT,               
     +               WORK( IPW3+KS*LROWS ), LROWS )
                              CALL ETRMM( 'Right', 'Lower',             
     +                 'No transpose', 'Non-unit',                      
     +        LROWS, NWIN-KS, ONE,                              WORK( 
     +PDW+NWIN*KS ), NWIN,                              WORK( 
     +IPW3+KS*LROWS ), LROWS )
                              CALL EGEMM( 'No transpose',               
...
*                                workspace.
*
                                 CALL ELAMOV( 'All', NWIN-KS, LCOLS,    
     +                             T((JLOC-1)*LLDT+ILOC), LLDT,         
     +                        WORK( IPW3+KS ), NWIN )
                                 CALL ETRMM( 'Left', 'Lower',           
     +                      'Transpose', 'Non-unit',                    
     +             NWIN-KS, LCOLS, ONE,                                 
     +WORK( PDW+NWIN*KS ), NWIN,                                 WORK( 
     +IPW3+KS ), NWIN )
                                 CALL EGEMM( 'Transpose',               
     +                  'No Transpose', NWIN-KS, LCOLS,                 
     +                KS, ONE,                                 WORK( 
     +PDW+NWIN*KS+NWIN-KS ),                                 NWIN, 
     +T((JLOC-1)*LLDT+ILOC1),                                 LLDT, ONE,
     + WORK( IPW3+KS ),                                 NWIN )
*
*                                Copy workspace to T.
*
...
*
         IF( IERR.NE.0 ) THEN
*
*           When calling BETREXC, the block at position I+KKS-1 failed
*           to swap.
*
...
*
                  CALL ELASET( 'All', NWIN, NWIN, ZERO, ZERO,           
     +       WORK( IPW2 ), NWIN )
*
                  PITRAF = IPIW
                  IPW3 = IPW2 + NWIN*NWIN
...
*
                              ...
                              IF( KK.NE.KS ) THEN
                                 NITRAF = LIWORK - PITRAF + 1
                                 NDTRAF = LWORK - PDTRAF + 1
                                 CALL BETREXC( NWIN, WORK(IPW2), NWIN,  
...
*
     ...
     $                          NPROW )
                           JLOC = INDXG2L( I, NB, MYCOL, DESCT( CSRC_ ),
     $                          NPCOL )
                           CALL ELAMOV( 'All', DIM1, DIM1, WORK(IPW2),  
...
*
     ...
     +     REAL( IWORK(IPIW+INDX-1) , KIND=10)
 370                    CONTINUE
                        CALL ELAMOV( 'All', DLEN, 1, WORK( IPW3 ),      
     +                  DLEN, WORK(BUFFER+ILEN), DLEN )
                     END IF
                     IF( NPCOL.GT.1 .AND. DIR.EQ.1 ) THEN
                        CALL EGEBS2D( ICTXT, 'Row', TOP, BUFFLEN, 1,    
 ...
 380                    CONTINUE
                        CALL ELAMOV( 'All', DLEN, 1,                    
     +    WORK( BUFFER+ILEN ), DLEN,                        WORK( IPW3 
     +), DLEN )
                     END IF
                  END IF
                  IF( RSRC1.NE.RSRC4 ) THEN
 ...
 390                       CONTINUE
                           CALL ELAMOV( 'All', DLEN, 1, WORK( IPW3 ),   
     +                        DLEN, WORK(BUFFER+ILEN), DLEN )
                           CALL EGEBS2D( ICTXT, 'Row', TOP, BUFFLEN,    
     ...
     $                    NPCOL.GT.1 ) THEN
                        BUFFER = PDTRAF
                        BUFFLEN = DLEN + ILEN
                        CALL EGEBR2D( ICTXT, 'Row', TOP, BUFFLEN,       
 ...
 395                       CONTINUE
                           CALL ELAMOV( 'All', DLEN, 1, WORK( IPW3 ),   
     +                        DLEN, WORK(BUFFER+ILEN), DLEN )
                           CALL EGEBS2D( ICTXT, 'Col', TOP, BUFFLEN,    
     +                       1, WORK(BUFFER), BUFFLEN )
                        END IF
...
*
     ...
     $                             JLOC1, RSRC, CSRC1 )
                              IF( MYROW.EQ.RSRC ) THEN
                                 CALL ELAMOV( 'All', TROWS, DIM1,       
     +                          T((JLOC1-1)*LLDT+ILOC), LLDT,           
     +                      WORK(IPW5), TROWS )
                                 IF( NPCOL.GT.1 ) THEN
                                    EAST = MOD( MYCOL + 1, NPCOL )
                                    CALL EGESD2D( ICTXT, TROWS, DIM1,   
     +                                 WORK(IPW5), TROWS, RSRC,         
     +                           EAST )
                                    CALL EGERV2D( ICTXT, TROWS, DIM4,   
     ...
     $                             ILOC, JLOC4, RSRC, CSRC4 )
                              IF( MYROW.EQ.RSRC ) THEN
                                 CALL ELAMOV( 'All', TROWS, DIM4,       
     +                          T((JLOC4-1)*LLDT+ILOC), LLDT,           
     +                      WORK(IPW5+TROWS*DIM1), TROWS )
                                 IF( NPCOL.GT.1 ) THEN
                                    WEST = MOD( MYCOL-1+NPCOL, NPCOL )
                                    CALL EGESD2D( ICTXT, TROWS, DIM4,   
     +                                 WORK(IPW5+TROWS*DIM1), TROWS,    
     +                                RSRC, WEST )
                                    CALL EGERV2D( ICTXT, TROWS, DIM1,   
     +                                 WORK(IPW5), TROWS, RSRC,         
     +                           WEST )
                                 END IF
                              END IF
...
*
     ...
     $                                ILOC1, JLOC, RSRC1, CSRC )
                              ...
                              IF( MYCOL.EQ.CSRC ) THEN
                                 CALL ELAMOV( 'All', DIM1, TCOLS,       
     +                          T((JLOC-1)*LLDT+ILOC1), LLDT,           
     +                      WORK(IPW6), NWIN )
                                 IF( NPROW.GT.1 ) THEN
                                    SOUTH = MOD( MYROW + 1, NPROW )
                                    CALL EGESD2D( ICTXT, DIM1, TCOLS,   
     +                                 WORK(IPW6), NWIN, SOUTH,         
     +                           CSRC )
                                    CALL EGERV2D( ICTXT, DIM4, TCOLS,   
     ...
     $                                ILOC4, JLOC, RSRC4, CSRC )
                              END IF
                              IF( MYCOL.EQ.CSRC ) THEN
                                 CALL ELAMOV( 'All', DIM4, TCOLS,       
     +                          T((JLOC-1)*LLDT+ILOC4), LLDT,           
     +                      WORK(IPW6+DIM1), NWIN )
                                 IF( NPROW.GT.1 ) THEN
                                    NORTH = MOD( MYROW-1+NPROW, NPROW )
                                    CALL EGESD2D( ICTXT, DIM4, TCOLS,   
     +                                 WORK(IPW6+DIM1), NWIN, NORTH,    
     +                                CSRC )
                                    CALL EGERV2D( ICTXT, DIM1, TCOLS,   
...
*
     ...
     $                                JLOC1, RSRC, CSRC1 )
                                 IF( MYROW.EQ.RSRC ) THEN
                                    CALL ELAMOV( 'All', QROWS, DIM1,    
     +                                Q((JLOC1-1)*LLDQ+ILOC), LLDQ,     
     +                               WORK(IPW7), QROWS )
                                    IF( NPCOL.GT.1 ) THEN
                                       EAST = MOD( MYCOL + 1, NPCOL )
                                       CALL EGESD2D( ICTXT, QROWS, DIM1,
     +                                       WORK(IPW7), QROWS, RSRC,   
     +                                    EAST )
                                       CALL EGERV2D( ICTXT, QROWS, DIM4,
     ...
     $                                ILOC, JLOC4, RSRC, CSRC4 )
                                 IF( MYROW.EQ.RSRC ) THEN
                                    CALL ELAMOV( 'All', QROWS, DIM4,    
     +                                Q((JLOC4-1)*LLDQ+ILOC), LLDQ,     
     +                               WORK(IPW7+QROWS*DIM1), QROWS )
                                    IF( NPCOL.GT.1 ) THEN
                                       WEST = MOD( MYCOL-1+NPCOL,
     $                                      NPCOL )
                                       CALL EGESD2D( ICTXT, QROWS, DIM4,
     +                                       WORK(IPW7+QROWS*DIM1),     
     +                                  QROWS, RSRC, WEST )
                                       CALL EGERV2D( ICTXT, QROWS, DIM1,
     +                                       WORK(IPW7), QROWS, RSRC,   
     +                                    WEST )
                                    END IF
                                 END IF
...
*
                        CALL ELASET( 'All', NWIN, NWIN, ZERO, ONE,      
     +                  WORK( IPW4 ), NWIN )
                        WORK(IPW8) = REAL(MYROW, KIND=10)
                        WORK(IPW8+1) = REAL(MYCOL, KIND=10)
                        CALL BELAAPP( 1, NWIN, NWIN, NCB, WORK( IPW4 ), 
     ...
     $              ILOC1, JLOC1, TRSRC1, TCSRC1 )
               IF( MYROW.EQ.TRSRC1 .AND. MYCOL.EQ.TCSRC1 ) THEN
                  ...
                  IF( ELEM3.NE.ZERO ) THEN
                     ...
                     CALL ELANV2( ELEM1, ELEM2, ELEM3, ELEM4,           
...
*
     ...
     $        ILOC4, JLOC4, TRSRC4, TCSRC4 )
         IF( MYROW.EQ.TRSRC2 .AND. MYCOL.EQ.TCSRC2 ) THEN
            ...
            IF( TRSRC1.NE.TRSRC2 .OR. TCSRC1.NE.TCSRC2 )          CALL 
     +EGESD2D( ICTXT, 1, 1, ELEM2, 1, TRSRC1, TCSRC1 )
         END IF
         IF( MYROW.EQ.TRSRC3 .AND. MYCOL.EQ.TCSRC3 ) THEN
            ELEM3 = T((JLOC3-1)*LLDT+ILOC3)
            IF( TRSRC1.NE.TRSRC3 .OR. TCSRC1.NE.TCSRC3 )          CALL 
     +EGESD2D( ICTXT, 1, 1, ELEM3, 1, TRSRC1, TCSRC1 )
         END IF
         IF( MYROW.EQ.TRSRC4 .AND. MYCOL.EQ.TCSRC4 ) THEN
            ...
            ELSE
               WORK(2) = ZERO
            END IF
            IF( TRSRC1.NE.TRSRC4 .OR. TCSRC1.NE.TCSRC4 )          CALL 
     +EGESD2D( ICTXT, 2, 1, WORK, 2, TRSRC1, TCSRC1 )
         END IF
         IF( MYROW.EQ.TRSRC1 .AND. MYCOL.EQ.TCSRC1 ) THEN
            ELEM1 = T((JLOC1-1)*LLDT+ILOC1)
            IF( TRSRC1.NE.TRSRC2 .OR. TCSRC1.NE.TCSRC2 )          CALL 
     +EGERV2D( ICTXT, 1, 1, ELEM2, 1, TRSRC2, TCSRC2 )
            IF( TRSRC1.NE.TRSRC3 .OR. TCSRC1.NE.TCSRC3 )          CALL 
     +EGERV2D( ICTXT, 1, 1, ELEM3, 1, TRSRC3, TCSRC3 )
            IF( TRSRC1.NE.TRSRC4 .OR. TCSRC1.NE.TCSRC4 )          CALL 
     +EGERV2D( ICTXT, 2, 1, WORK, 2, TRSRC4, TCSRC4 )
            ELEM4 = WORK(1)
            ELEM5 = WORK(2)
            IF( ELEM5.EQ.ZERO ) THEN
               IF( WR( K ).EQ.ZERO .AND. WI( K ).EQ.ZERO ) THEN
                  CALL ELANV2( ELEM1, ELEM2, ELEM3, ELEM4, WR( K ),     
...
*
      IF( NPROCS.GT.1 ) THEN
         CALL EGSUM2D( ICTXT, 'All', TOP, N, 1, WR, N, -1, -1 )
         CALL EGSUM2D( ICTXT, 'All', TOP, N, 1, WI, N, -1, -1 )
      END IF
*
