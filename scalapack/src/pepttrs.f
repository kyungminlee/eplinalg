      SUBROUTINE PEPTTRS( N, NRHS, D, E, JA, DESCA, B, IB, DESCB, AF,   
     +                  LAF, WORK, LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEPTTRS solves a system of linear equations
*
*            A(1:N, JA:JA+N-1) * X = B(IB:IB+N-1, 1:NRHS)
*
*  where A(1:N, JA:JA+N-1) is the matrix used to produce the factors
*  stored in A(1:N,JA:JA+N-1) and AF by PEPTTRF.
...
*     .. External Subroutines ..
     ...
     +                  ESCAL, GLOBCHK, PEPTTRSV, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      EXTERNAL           NUMROC
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,
     + 'PEPTTRS, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 5*100+4 )
         CALL PXERBLA( ICTXT, 'PEPTTRS, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -13
            CALL PXERBLA( ICTXT, 'PEPTTRS: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPTTRS', -INFO )
         RETURN
      END IF
*
...
*
*
      CALL PEPTTRSV( 'L', N, NRHS, D( PART_OFFSET+1 ),                
...
*
      DO 10 I = PART_OFFSET + 1, PART_OFFSET + ODD_SIZE
         CALL ESCAL( NRHS, REAL( ONE / D( I ) , KIND=10), B( I ), LLDB )
   10 CONTINUE
*
...
*
      IF( MYCOL.LT.NPCOL-1 ) THEN
         ...
         CALL ESCAL( NRHS, ONE / AF( ODD_SIZE+2 ), B( I ), LLDB )
      END IF
*
*     Call backsolve routine
*
*
      CALL PEPTTRSV( 'U', N, NRHS, D( PART_OFFSET+1 ),                
     +E( PART_OFFSET+1 ), JA_NEW, DESCA_1XP, B, IB,                
     +DESCB_PX1, AF, LAF, WORK, LWORK, INFO )
   20 CONTINUE
*
