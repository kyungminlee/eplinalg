      SUBROUTINE YLAMSH( S, LDS, NBULGE, JBLK, H, LDH, N, ULP )
      IMPLICIT NONE
*
...
*     ..
*     .. Array Arguments ..
      COMPLEX(KIND=10)         H( LDH, * ), S( LDS, * )
*     ..
*
*  Purpose
*  =======
*
*  YLAMSH sends multiple shifts through a small (single node) matrix to
...
*  YLAMSH should only be called when there are multiple shifts/bulges
...
*  =========
*
*  S       (local input/output) COMPLEX(KIND=10) array, ( LDS,* )
...
*          Unchanged on exit.
*
*  H       (local input/output) COMPLEX(KIND=10) array ( LDH,N )
