      SUBROUTINE ELARRE2A( RANGE, N, VL, VU, IL, IU, D, E, E2,          
     +           RTOL1, RTOL2, SPLTOL, NSPLIT, ISPLIT,                  
     +    M, DOL, DOU, NEEDIL, NEEDIU,                     W, WERR, 
     +WGAP, IBLOCK, INDEXW, GERS,                      SDIAM, PIVMIN, 
     +WORK, IWORK, MINRGP, INFO )
*
...
*     .. Scalar Arguments ..
     ...
     $                   NEEDIL, NEEDIU
      REAL(KIND=10)   MINRGP, PIVMIN, RTOL1, RTOL2, SPLTOL, VL, VU
*     ..
*     .. Array Arguments ..
...
*  tridiagonal matrix T, ELARRE2 sets any "small" off-diagonal
...
*  The DQDS algorithm (subroutine ELASQ2) is not used, unlike in
...
*          If RANGE='I' or ='A', ELARRE2A computes bounds on the desired 
...
*          sorted in ascending order ( ELARRE2A may use the
...
*  INFO    (output) INTEGER
*          = 0:  successful exit
*          > 0:  A problem occured in ELARRE2A.
*          < 0:  One of the called subroutines signaled an internal problem. 
*                Needs inspection of the corresponding parameter IINFO
*                for further information.
*
*          =-1:  Problem in ELARRD2. 
*          = 2:  No base representation could be found in MAXTRY iterations.
*                Increasing MAXTRY and recompilation might be a remedy.
*          =-3:  Problem in ELARRB2 when computing the refined root 
*                representation
*          =-4:  Problem in ELARRB2 when preforming bisection on the 
...
*     .. External Subroutines ..
      EXTERNAL           ECOPY, ELARNV, ELARRA, ELARRB2,                
     +    ELARRC, ELARRD2
*     ..
*     .. Intrinsic Functions ..
...
*     We call ELARRD2 to find crude approximations to the eigenvalues
*     in the desired range. In case IRANGE = 3, we also obtain the
*     interval (VL,VU] that contains all the wanted eigenvalues.
*     An interval [LEFT,RIGHT] has converged if
*     RIGHT-LEFT.LT.RTOL*MAX(ABS(LEFT),ABS(RIGHT))
*     ELARRD2 needs a WORK of size 4*N, IWORK of size 3*N
      CALL ELARRD2( RANGE, 'B', N, VL, VU, IL, IU, GERS,                
     +   BSRTOL, D, E, E2, PIVMIN, NSPLIT, ISPLIT,                   MM,
     + W, WERR, VL, VU, IBLOCK, INDEXW,                   WORK, IWORK, 
     +DOL, DOU, IINFO )
      IF( IINFO.NE.0 ) THEN
         INFO = -1
         RETURN
...
*           Define 1/4 and 3/4 points of the spectrum
	    ...
	    S2 = ISRGHT - FOURTH * SPDIAM
         ELSE        
*           ELARRD2 has computed IBLOCK and INDEXW for each eigenvalue 
*           approximation. 
*           choose sigma
            IF( USEDQD ) THEN
               S1 = ISLEFT + FOURTH * SPDIAM
...
*        according to the shift of their representation. 
 ...
 134     CONTINUE
*        call ELARRB2 to reduce eigenvalue error of the approximations
*        from ELARRD2
...
*
         ...
         CALL ELARRB2(IN, D(IBEGIN), WORK(IBEGIN),                
     +MYINDL, MYINDU, RTOL1, RTOL2, MYINDL-1,                W(MYWBEG), 
     +WGAP(MYWBEG), WERR(MYWBEG),                WORK( 2*N+1 ), IWORK, 
     +PIVMIN,                 LGPVMN, LGSPDM, IN, IINFO )
         IF( IINFO .NE. 0 ) THEN
            INFO = -4
            RETURN
         END IF
*        ELARRB2 computes all gaps correctly except for the last one
