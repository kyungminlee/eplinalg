      SUBROUTINE PESTEBZ( ICTXT, RANGE, ORDER, N, VL, VU, IL, IU,       
...
*     .. Scalar Arguments ..
     ...
     $                   NSPLIT
      REAL(KIND=10)   ABSTOL, VL, VU
*     ..
*     .. Array Arguments ..
      INTEGER            IBLOCK( * ), ISPLIT( * ), IWORK( * )
      REAL(KIND=10)   D( * ), E( * ), W( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PESTEBZ computes the eigenvalues of a symmetric tridiagonal matrix in
...
*  static partitioning of work is done at the beginning of PESTEBZ which
...
*
*  ABSTOL  (global input) REAL(KIND=10)
...
*          set to the underflow threshold ELAMCH('U'), not zero.
*          Note : If eigenvectors are desired later by inverse iteration
*          ( PESTEIN ), ABSTOL should be set to 2*PELAMCH('S').
*
*  D       (global input) REAL(KIND=10) array, dimension (N)
...
*
*  E       (global input) REAL(KIND=10) array, dimension (N-1)
...
*          1 <= NSPLIT <= N.
*
*  W       (global output) REAL(KIND=10) array, dimension (N)
...
*
*  WORK    (local workspace) REAL(KIND=10) array,
*          dimension ( MAX( 5*N, 7 ) )
*
...
*                    which is less accurate than PELAMCH says.
...
*  ===================
*
*  RELFAC  REAL(KIND=10), default = 2.0
*          The relative tolerance.  An interval [a,b] lies within
*          "relative tolerance" if  b-a < RELFAC*ulp*max(|a|,|b|),
*          where "ulp" is the machine precision (distance from 1 to
*          the next larger floating point number.)
*
*  FUDGE   REAL(KIND=10), default = 2.0
...
*
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, ICHAR, MAX, MIN, MOD
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            BLACS_PNUM
      REAL(KIND=10)   PELAMCH
      EXTERNAL           LSAME, BLACS_PNUM, PELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_FREEBUFF, BLACS_GET, BLACS_GRIDEXIT,     
     +               BLACS_GRIDINFO, BLACS_GRIDMAP, EGEBR2D,            
     +        EGEBS2D, EGERV2D, EGESD2D, ELASRT2, GLOBCHK,              
     +      IGEBR2D, IGEBS2D, IGERV2D, IGESD2D, IGSUM2D,                
     +    PELAEBZ, PELAIECTB, PELAIECTL, PELAPDCT,                    
     +PELASNBT, PXERBLA
*     ..
*     .. Parameters ..
     ...
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      ...
      REAL(KIND=10)   ZERO, ONE, TWO, FIVE, HALF
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10, TWO = 
     +2.0E0_10,                    FIVE = 5.0E0_10, HALF = 1.0E0_10 / 
     +TWO )
      REAL(KIND=10)   FUDGE, RELFAC
      PARAMETER          ( FUDGE = 2.0E0_10, RELFAC = 2.0E0_10 )
*     ..
*     .. Local Scalars ..
      LOGICAL            LQUERY
     ...
     $                   ONEDCONTEXT, P, PREV, REXTRA, RREQ, SELF
      REAL(KIND=10)   ALPHA, ATOLI, BETA, BNORM, DRECV, DSEND, GL,      
...
*
         ...
         IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
            ...
            ELSE
               ...
               WORK( 3 ) = ZERO
            END IF
            CALL EGEBS2D( ICTXT, 'ALL', ' ', 3, 1, WORK, 3 )
         ELSE
            CALL EGEBR2D( ICTXT, 'ALL', ' ', 3, 1, WORK, 3, 0, 0 )
         END IF
     ...
     $      INFO = BIGNUM
         ...
         END IF
      END IF
      WORK( 1 ) = REAL( MAX( 5*N, 7 ) , KIND=10)
      IWORK( 1 ) = MAX( 4*N, 14, NPROW*NPCOL )
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESTEBZ', -INFO )
         RETURN
...
*
      ...
      ELSE IF( IRANGE.EQ.2 ) THEN
         IF( VL.GT.GL ) THEN
            IF( IEFLAG.EQ.0 ) THEN
               CALL PELAPDCT( VL, N, WORK( INDRW1+1 ), PIVMIN, IFRST )
            ELSE IF( IEFLAG.EQ.1 ) THEN
               CALL PELAIECTB( VL, N, WORK( INDRW1+1 ), IFRST )
            ELSE
               CALL PELAIECTL( VL, N, WORK( INDRW1+1 ), IFRST )
            END IF
            IFRST = IFRST + 1
            INITVL = VL
         ELSE
         ...
         IF( VU.LT.GU ) THEN
            IF( IEFLAG.EQ.0 ) THEN
               CALL PELAPDCT( VU, N, WORK( INDRW1+1 ), PIVMIN, ILAST )
            ELSE IF( IEFLAG.EQ.1 ) THEN
               CALL PELAIECTB( VU, N, WORK( INDRW1+1 ), ILAST )
            ELSE
               CALL PELAIECTL( VU, N, WORK( INDRW1+1 ), ILAST )
            END IF
            INITVU = VU
         ELSE
            INITVU = GU
      ...
      ELSE IF( IRANGE.EQ.3 ) THEN
         ...
         IWORK( 5 ) = IL - 1
         IWORK( 6 ) = IU
         CALL PELAEBZ( 0, N, 2, 1, ATOLI, RELTOL, PIVMIN,               
...
*
            IF( GL.LT.INITVL ) THEN
               GL = INITVL
               IF( IEFLAG.EQ.0 ) THEN
                  CALL PELAPDCT( GL, IN, WORK( INDRW1+2*IOFF+1 ),       
     +                     PIVMIN, NGL )
               ELSE IF( IEFLAG.EQ.1 ) THEN
                  CALL PELAIECTB( GL, IN, WORK( INDRW1+2*IOFF+1 ), NGL )
               ELSE
                  CALL PELAIECTL( GL, IN, WORK( INDRW1+2*IOFF+1 ), NGL )
               END IF
            ELSE
               NGL = 0
            END IF
            IF( GU.GT.INITVU ) THEN
               GU = INITVU
               IF( IEFLAG.EQ.0 ) THEN
                  CALL PELAPDCT( GU, IN, WORK( INDRW1+2*IOFF+1 ),       
     +                     PIVMIN, NGU )
               ELSE IF( IEFLAG.EQ.1 ) THEN
                  CALL PELAIECTB( GU, IN, WORK( INDRW1+2*IOFF+1 ), NGU )
               ELSE
                  CALL PELAIECTL( GU, IN, WORK( INDRW1+2*IOFF+1 ), NGU )
               END IF
            ELSE
               NGU = IN
            END IF
...
*
     ...
     $         INXTLOAD = INXTLOAD + 1
            ...
            TMP1 = WORK( 1 )
            ITMP1 = IWORK( 1 )
            CALL PELAEBZ( 1, IN, 1, 1, ATOLI, RELTOL, PIVMIN,           
     ...
     $         LAST = LAST + P
            IF( SELF.NE.LAST ) THEN
               CALL EGESD2D( ONEDCONTEXT, 1, 1, DSEND, 1, 0, NEXT )
               CALL IGESD2D( ONEDCONTEXT, 1, 1, NBETA, 1, 0, NEXT )
            END IF
            IF( SELF.NE.MOD( FOUND, P ) ) THEN
               CALL EGERV2D( ONEDCONTEXT, 1, 1, DRECV, 1, 0, PREV )
...
*
         CALL PELAEBZ( 2, IN, NEIGINT, 1, ATOLI, RELTOL, PIVMIN,        
...
*
     ...
     $                       IWORK( INDRIW2+1 ), IM )
               CALL EGEBS2D( ONEDCONTEXT, 'ALL', ' ', IM, 1,            
     +            WORK( INDRW2+1 ), IM )
               CALL IGEBS2D( ONEDCONTEXT, 'ALL', ' ', IM, 1,
     $                       IWORK( INDRIW1+1 ), IM )
               DO 110 J = 1, IM
  ...
  110          CONTINUE
     ...
     $                       IWORK, TORECV( 1, 1 ), 0, I-1 )
               CALL EGEBR2D( ONEDCONTEXT, 'ALL', ' ', TORECV( 1, 1 ), 1,
...
*
         DO 140 I = 1, M
            IWORK( M+I ) = I
  140    CONTINUE
         CALL ELASRT2( 'I', M, W, IWORK( M+1 ), IINFO )
         DO 150 I = 1, M
            IWORK( I ) = IBLOCK( I )
  150    CONTINUE
         DO 160 I = 1, M
...
*
      END
*
      SUBROUTINE PELAEBZ( IJOB, N, MMAX, MINP, ABSTOL, RELTOL, PIVMIN,  
...
*     .. Scalar Arguments ..
      ...
      REAL(KIND=10)   ABSTOL, LSAVE, PIVMIN, RELTOL
*     ..
*     .. Array Arguments ..
      INTEGER            INTVLCT( * ), NVAL( * )
      REAL(KIND=10)   D( * ), INTVL( * )
*     ..
*
*  Purpose
*  =======
*
*  PELAEBZ contains the iteration loop which computes the eigenvalues
...
*
*  IJOB    (input) INTEGER
*          Specifies the computation done by PELAEBZ
...
*          more than MMAX intervals are generated, then PELAEBZ will
*          quit with INFO = MMAX+1.
*
*  MINP    (input) INTEGER
*          The initial number of intervals. MINP <= MMAX.
*
*  ABSTOL  (input) REAL(KIND=10)
...
*          without overflow.
*          See PELAPDCT for the "paranoid" implementation of the Sturm
*          sequence loop.
*
*  D       (input) REAL(KIND=10) array, dimension (2*N - 1)
...
*
*  INTVL   (input/output) REAL(KIND=10) array, dimension (2*MMAX)
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           PELAECV, PELAIECTB, PELAIECTL, PELAPDCT
*     ..
*     .. Parameters ..
      REAL(KIND=10)   ZERO, TWO, HALF
      PARAMETER          ( ZERO = 0.0E0_10, TWO = 2.0E0_10,             
     +       HALF = 1.0E0_10 / TWO )
*     ..
*     .. Local Scalars ..
      INTEGER            I, ITMAX, J, K, KF, KL, KLNEW, L, LCNT, LREQ,
     $                   NALPHA, NBETA, NMID, RCNT, RREQ
      REAL(KIND=10)   ALPHA, BETA, MID
*     ..
*     .. Executable Statements ..
*
      KF = 1
...
*
         CALL PELAECV( 0, KF, KL, INTVL, INTVLCT, NVAL,                 
...
*
               ...
               IF( IEFLAG.EQ.0 ) THEN
                  CALL PELAPDCT( MID, N, D, PIVMIN, NMID )
               ELSE IF( IEFLAG.EQ.1 ) THEN
                  CALL PELAIECTB( MID, N, D, NMID )
               ELSE
                  CALL PELAIECTL( MID, N, D, NMID )
               END IF
               LREQ = NVAL( K-1 )
               RREQ = NVAL( K )
               IF( KL.EQ.1 )
...
*        Check if some input intervals have "converged"
*
         CALL PELAECV( 1, KF, KL, INTVL, INTVLCT, NVAL,                 
     + MAX( ABSTOL, PIVMIN ), RELTOL )
         IF( KF.GE.KL )
     $      GO TO 60
*
...
*
*
      SUBROUTINE PELAECV( IJOB, KF, KL, INTVL, INTVLCT, NVAL, ABSTOL,   
     +                  RELTOL )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine (version 1.7) --
...
*  =======
*
*  PELAECV checks if the input intervals [ INTVL(2*i-1), INTVL(2*i) ],
*  i = KF, ... , KL-1, have "converged".
*  PELAECV modifies KF to be the index of the last converged interval,
...
*
*  INTVL   (input/output) REAL(KIND=10) array, dimension (2*(KL-KF))
*          The endpoints of the intervals. INTVL(2*j-1) is the left
...
*          See the comments in PELAEBZ for more on the function N(w).
*
...
*     .. Local Scalars ..
      ...
      REAL(KIND=10)   TMP1, TMP2, TMP3, TMP4
*     ..
*     .. Executable Statements ..
*
      KFNEW = KF
...
*
      END
*
      SUBROUTINE PELAPDCT( SIGMA, N, D, PIVMIN, COUNT )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine (version 1.7) --
*     University of Tennessee, Knoxville, Oak Ridge National Laboratory,
...
*  =======
*
*  PELAPDCT counts the number of negative eigenvalues of (T - SIGMA I).
...
*  floating point number. PELAPDCT will be referred to as the "paranoid"
...
*  =========
*
*  SIGMA   (input) REAL(KIND=10)
*          The shift. PELAPDCT finds the number of eigenvalues of T less
