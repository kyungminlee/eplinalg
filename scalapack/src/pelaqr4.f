      SUBROUTINE PELAQR4( WANTT, WANTZ, N, ILO, IHI, A, DESCA, WR, WI,  
...
*  =======
*
*  PELAQR4 is an auxiliary routine used to find the Schur decomposition
...
*  Normally, it is called by PELAQR1.  All the inputs are assumed to be
...
*          ILO = 1). PELAQR4 works primarily with the Hessenberg
...
*
*  T       (local workspace) REAL(KIND=10) array, dimension LDT*NW.
*
*  LDT     (local input) INTEGER
*          The leading dimension of the array T.
*          LDT >= IHI-ILO+1.
*
*  V       (local workspace) REAL(KIND=10) array, dimension LDV*NW.
*
...
*          PELAQR1. (LDS = 385, see PELAQR1)
*
*  INFO    (global output) INTEGER
*          < 0: parameter number -INFO incorrect or inconsistent;
*          = 0: successful exit;
*          > 0: PELAQR4 failed to compute all the eigenvalues ILO to IHI
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, ELASET,               
     +     ELAHQR, ELAQR4, DESCINIT, PEGEMM, PEGEMR2D,                  
     +  EGEMM, ELAMOV, EGESD2D, EGERV2D,                    EGEBS2D, 
     +EGEBR2D, IGEBS2D, IGEBR2D
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $        1, IERR )
      END IF
      CALL PEGEMR2D( NH, NH, A, ILO, ILO, DESCA, T, 1, 1, DESCT,      
     +CONTXT )
      IF ( MYROW .EQ. II .AND. MYCOL .EQ. JJ ) THEN
         CALL ELASET( 'All', NH, NH, ZERO, ONE, V, LDV )
         NMIN = ILAENV( 12, 'ELAQR3', 'SV', NH, 1, NH, LWORK )
         IF( NH .GT. NMIN ) THEN
            CALL ELAQR4( .TRUE., .TRUE., NH, 1, NH, T, LDT, WR( ILO ),  
     +          WI( ILO ), 1, NH, V, LDV, WORK, LWORK, INFO )
*           Clean up the scratch used by ELAQR4.
            CALL ELASET( 'L', NH-2, NH-2, ZERO, ZERO, T( 3, 1 ), LDT )
         ELSE
            CALL ELAHQR( .TRUE., .TRUE., NH, 1, NH, T, LDT, WR( ILO ),  
     +          WI( ILO ), 1, NH, V, LDV, INFO )
         END IF
         CALL EGEBS2D( CONTXT, 'All', ' ', NH, NH, V, LDV )
         CALL IGEBS2D( CONTXT, 'All', ' ', 1, 1, INFO, 1 )
      ELSE
         CALL EGEBR2D( CONTXT, 'All', ' ', NH, NH, V, LDV, II, JJ )
...
*
      CALL PEGEMR2D( NH, NH, T, 1, 1, DESCT, A, ILO, ILO, DESCA,      
     +CONTXT )
*
*     Update T and Z.
*
...
*        Call EGEMM directly to perform the update.
*
         HSTEP = LWORK / NH
         VSTEP = HSTEP
*
     ...
     $           MYCOL, IROW, ICOL, II, JJ )
            IF( MYROW .EQ. II ) THEN
               ...
               DO 10 KKCOL = ICOL, ICOL1, HSTEP
                  ...
                  CALL EGEMM( 'T', 'N', NH, KLN, NH, ONE, V,            
     +      LDV, A( IROW+(KKCOL-1)*LDA ), LDA, ZERO, WORK,              
     +    NH )
                  CALL ELAMOV( 'A', NH, KLN, WORK, NH,                  
...
*
     ...
     $              MYROW, MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
               ...
               DO 20 KKROW = IROW, IROW1, VSTEP
                  ...
                  CALL EGEMM( 'N', 'N', KLN, NH, NH, ONE,               
     +   A( KKROW+(ICOL-1)*LDA ), LDA, V, LDV, ZERO,                  
     +WORK, KLN )
                  CALL ELAMOV( 'A', KLN, NH, WORK, KLN,                 
...
*        processor mesh.
*        Call EGEMM locally and communicate by pair.
*
            ...
            ELSE
               IF( MYROW .EQ. II ) THEN
                  ...
                  DO 50 KKCOL = ICOL, ICOL1, HSTEP
                     KLN = MIN( HSTEP, ICOL1-KKCOL+1 )
                     CALL EGEMM( 'T', 'N', D2, KLN, D1, ONE,            
     +         V( 1, D1+1 ), LDV, A( IROW+(KKCOL-1)*LDA ),              
     +       LDA, ZERO, WORK( D1+1 ), NH )
                     CALL EGESD2D( CONTXT, D2, KLN, WORK( D1+1 ),       
     +              NH, DOWN, MYCOL )
                     CALL EGERV2D( CONTXT, D1, KLN, WORK, NH, DOWN,     
     +                MYCOL )
                     CALL EGEMM( 'T', 'N', D1, KLN, D1, ONE,            
     +         V, LDV, A( IROW+(KKCOL-1)*LDA ), LDA, ONE,               
     +      WORK, NH )
                     CALL ELAMOV( 'A', D1, KLN, WORK, NH,               
     +      A( IROW+(KKCOL-1)*LDA ), LDA )
   50             CONTINUE
               ELSE IF( UP .EQ. II ) THEN
                  ...
                  DO 60 KKCOL = ICOL, ICOL1, HSTEP
                     KLN = MIN( HSTEP, ICOL1-KKCOL+1 )
                     CALL EGEMM( 'T', 'N', D1, KLN, D2, ONE,            
     +         V( D1+1, 1 ), LDV, A( IROW+(KKCOL-1)*LDA ),              
     +       LDA, ZERO, WORK, NH )
                     CALL EGESD2D( CONTXT, D1, KLN, WORK, NH, UP,       
     +              MYCOL )
                     CALL EGERV2D( CONTXT, D2, KLN, WORK( D1+1 ),       
     +              NH, UP, MYCOL )
                     CALL EGEMM( 'T', 'N', D2, KLN, D2, ONE,            
     +         V( D1+1, D1+1 ), LDV,                     A( 
     +IROW+(KKCOL-1)*LDA ), LDA, ONE,                     WORK( D1+1 ), 
     +NH )
                     CALL ELAMOV( 'A', D2, KLN, WORK( D1+1 ), NH,       
     +              A( IROW+(KKCOL-1)*LDA ), LDA )
   60             CONTINUE
               END IF
            END IF
   ...
   70             CONTINUE
     ...
     $                 MYROW, MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
                  ...
                  DO 80 KKROW = IROW, IROW1, VSTEP
                     KLN = MIN( VSTEP, IROW1-KKROW+1 )
                     CALL EGEMM( 'N', 'N', KLN, D2, D1, ONE,            
     +         A( KKROW+(ICOL-1)*LDA ), LDA, V( 1, D1+1 ),              
     +       LDV, ZERO, WORK( 1+D1*KLN ), KLN )
                     CALL EGESD2D( CONTXT, KLN, D2, WORK( 1+D1*KLN ),   
     +                  KLN, MYROW, RIGHT )
                     CALL EGERV2D( CONTXT, KLN, D1, WORK, KLN, MYROW,   
     +                  RIGHT )
                     CALL EGEMM( 'N', 'N', KLN, D1, D1, ONE,            
     +         A( KKROW+(ICOL-1)*LDA ), LDA, V, LDV, ONE,               
     +      WORK, KLN )
                     CALL ELAMOV( 'A', KLN, D1, WORK, KLN,              
     +       A( KKROW+(ICOL-1)*LDA ), LDA )
   80             CONTINUE
     ...
     $                 MYROW, MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
                  ...
                  DO 90 KKROW = IROW, IROW1, VSTEP
                     KLN = MIN( VSTEP, IROW1-KKROW+1 )
                     CALL EGEMM( 'N', 'N', KLN, D1, D2, ONE,            
     +         A( KKROW+(ICOL-1)*LDA ), LDA, V( D1+1, 1 ),              
     +       LDV, ZERO, WORK, KLN )
                     CALL EGESD2D( CONTXT, KLN, D1, WORK, KLN, MYROW,   
     +                  LEFT )
                     CALL EGERV2D( CONTXT, KLN, D2, WORK( 1+D1*KLN ),   
     +                  KLN, MYROW, LEFT )
                     CALL EGEMM( 'N', 'N', KLN, D2, D2, ONE,            
     +         A( KKROW+(ICOL-1)*LDA ), LDA, V( D1+1, D1+1 ),           
     +          LDV, ONE, WORK( 1+D1*KLN ), KLN )
                     CALL ELAMOV( 'A', KLN, D2, WORK( 1+D1*KLN ), KLN,  
     +                   A( KKROW+(ICOL-1)*LDA ), LDA )
   90             CONTINUE
               END IF
            END IF
...
*
  ...
  110             CONTINUE
     ...
     +                  KLN, MYROW, LEFT )
                     CALL EGEMM( 'N', 'N', KLN, D2, D2, ONE,            
     +         Z( KKROW+(ICOL-1)*LDZ ), LDZ,                     V( 
     +D1+1, D1+1 ), LDV, ONE, WORK( 1+D1*KLN ),                     KLN 
     +)
                     CALL ELAMOV( 'A', KLN, D2, WORK( 1+D1*KLN ),       
...
*        Treat V as a distributed matrix and call PEGEMM.
*
...
*
            DO 130 KKCOL = I+1, N, HSTEP
               ...
               CALL PEGEMM( 'T', 'N', NH, KLN, NH, ONE, V, 1, 1,        
     +       DESCV, A, ILO, KKCOL, DESCA, ZERO, WORK, 1, 1,             
     +  DESCWH )
               CALL PEGEMR2D( NH, KLN, WORK, 1, 1, DESCWH, A,           
...
*
     ...
     $              CONTXT, LLDTMP, IERR )
               CALL PEGEMM( 'N', 'N', KLN, NH, NH, ONE, A, KKROW,       
     +        ILO, DESCA, V, 1, 1, DESCV, ZERO, WORK, 1, 1,             
     +  DESCWV )
               CALL PEGEMR2D( KLN, NH, WORK, 1, 1, DESCWV, A, KKROW,    
     +           ILO, DESCA, CONTXT )
  140       CONTINUE
         END IF
*
...
*
         IF( WANTZ ) THEN
            DO 150 KKROW = ILOZ, IHIZ, VSTEP
               ...
               CALL DESCINIT( DESCWV, KLN, NH, LWORK / NH, NH, 0, 0,
     $              CONTXT, LLDTMP, IERR )
               CALL PEGEMM( 'N', 'N', KLN, NH, NH, ONE, Z, KKROW,       
     +        ILO, DESCZ, V, 1, 1, DESCV, ZERO, WORK, 1, 1,             
     +  DESCWV )
               CALL PEGEMR2D( KLN, NH, WORK, 1, 1, DESCWV, Z,           
