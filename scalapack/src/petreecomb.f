      SUBROUTINE PETREECOMB( ICTXT, SCOPE, N, MINE, RDEST0, CDEST0,     
     +                   SUBPTR )
      IMPLICIT NONE
*
...
*  =======
*
*  PETREECOMB does a 1-tree parallel combine operation on scalars,
...
*
*  MINE    (local input/global output) REAL(KIND=10) array of
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGEBR2D, EGEBS2D,              
     +      EGERV2D, EGESD2D
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
            ...
            END IF
*
            CALL EGESD2D( ICTXT, N, 1, MINE, N, RMSSG, CMSSG )
*
            GO TO 20
*
         ELSE
...
*
               CALL EGERV2D( ICTXT, N, 1, HIS, N, RMSSG, CMSSG )
...
*
      IF( BCAST ) THEN
         IF( MYDIST2.EQ.0 ) THEN
            CALL EGEBS2D( ICTXT, SCOPE, ' ', N, 1, MINE, N )
         ELSE
            CALL EGEBR2D( ICTXT, SCOPE, ' ', N, 1, MINE, N,             
     +        TRDEST, TCDEST )
         END IF
      END IF
*
...
*
*     .. Array Arguments ..
      REAL(KIND=10)   V1( 2 ), V2( 2 )
*     ..
*
*  Purpose
*  =======
*
*  ECOMBAMAX finds the element having max. absolute value as well
*  as its corresponding globl index.
*
*  Arguments
*  =========
*
*  V1        (local input/local output) REAL(KIND=10) array of
*            dimension 2.  The first maximum absolute value element and
*            its global index. V1(1) = AMAX, V1(2) = INDX.
*
*  V2        (local input) REAL(KIND=10) array of dimension 2.
...
*  =======
*
*  ECOMBSSQ does a scaled sum of squares on two scalars.
*
*  Arguments
*  =========
*
...
*  =======
*
*  ECOMBNRM2 combines local norm 2 results, taking care not to cause
*  unnecessary overflow.
*
*  Arguments
*  =========
*
*  X       (local input) REAL(KIND=10)
*  Y       (local input) REAL(KIND=10)
