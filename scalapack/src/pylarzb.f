      SUBROUTINE PYLARZB( SIDE, TRANS, DIRECT, STOREV, M, N, K, L, V,   
...
*     .. Array Arguments ..
      INTEGER            DESCC( * ), DESCV( * )
      COMPLEX(KIND=10)         C( * ), T( * ), V( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYLARZB applies a complex block reflector Q or its conjugate
...
*          Householder transformation as returned by PYTZRZF.
*          LLD_V >= LOCr(IV+K-1).
*
...
*
*  T       (local input) COMPLEX(KIND=10) array, dimension MB_V by MB_V
...
*     .. External Subroutines ..
     ...
     +          PBYMATADD, PB_TOPGET, PXERBLA, PBYTRAN,                 
     +   YGEBR2D, YGEBS2D, YGEMM,                    YGSUM2D, YLACGV, 
     +YLAMOV, YLASET,                    YTRBR2D, YTRBS2D, YTRMM
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYLARZB', -INFO )
...
*        M+ICOFFV as PBYTRAN's N argument while sizing WORK( IPV ) / LV
*        from MPC20 = local rows of L. Whenever M > L (the typical case
*        — M = K+L from PYUNMRZ's caller), PBYTRAN's LDC contract
...
*        value") *without* writing WORK( IPV ), and the outer YGEMM at
...
*        buffer then matches PBYTRAN's NP requirement under the
...
*        WORK( IPT ) is the workspace for PBYTRAN
*
         IPV = 1
         IPW = IPV + MPC20 * K
         IPT = IPW + K * MQV0
...
*
         IF( MYROW.EQ.IVROW ) THEN
            IF( MYCOL.EQ.IVCOL ) THEN
               CALL YLAMOV( 'All', K, NQV, V( IOFFV ), LDV,             
...
*
         CALL PBYTRAN( ICTXT, 'Rowwise', 'Conjugate transpose', K,      
...
*
         IF( MPC2.GT.0 ) THEN
            CALL YGEMM( 'Transpose', 'No transpose', NQC2, K, MPC2,     
     +              ONE, C( IOFFC2 ), LDC, WORK( IPV ), LV, ZERO,       
     +            WORK( IPW ), LW )
         ELSE
            CALL YLASET( 'All', NQC2, K, ZERO, ZERO, WORK( IPW ), LW )
         END IF
*
...
*
   10       CONTINUE
            IF( IIBEG.LE.IINXT ) THEN
               CALL PBYMATADD( ICTXT, 'Transpose', NQC2, IINXT-IIBEG+1, 
     ...
     +                        WORK( IPW+ITOP ), LW )
         ...
         END IF
*
         CALL YGSUM2D( ICTXT, 'Columnwise', ' ', NQC2, K, WORK( IPW ),  
     +                LW, IVROW, MYCOL )
*
...
*
               CALL YTRBS2D( ICTXT, 'Rowwise', ' ', 'Lower', 'Non unit',
     +                        K, K, T, MBV )
            ELSE
               CALL YTRBR2D( ICTXT, 'Rowwise', ' ', 'Lower', 'Non unit',
     +                        K, K, T, MBV, MYROW, IVCOL )
            END IF
            CALL YTRMM( 'Right', 'Lower', TRANST, 'Non unit', NQC2, K,  
     +                 ONE, T, MBV, WORK( IPW ), LW )
*
            CALL YGEBS2D( ICTXT, 'Columnwise', ' ', NQC2, K,            
     +         WORK( IPW ), LW )
         ELSE
            CALL YGEBR2D( ICTXT, 'Columnwise', ' ', NQC2, K,            
...
*
   20       CONTINUE
            IF( IIBEG.LE.IINXT ) THEN
               CALL PBYMATADD( ICTXT, 'Transpose', IINXT-IIBEG+1, NQC2, 
...
*
         DO 30 J = 1, K
            CALL YLACGV( MPC2, WORK( IPV+(J-1)*LV ), 1 )
   30    CONTINUE
         CALL YGEMM( 'No transpose', 'Transpose', MPC2, NQC2, K, -ONE,  
...
*
         ...
         IF( MYROW.EQ.IVROW ) THEN
            CALL YGEBS2D( ICTXT, 'Columnwise', COLBTOP, K, NQC2,        
     +             V( IOFFV ), LDV )
            IF( MYCOL.EQ.IVCOL )          CALL YTRBS2D( ICTXT, 
     +'Columnwise', COLBTOP, 'Lower',                        'Non unit',
     + K, K, T, MBV )
            CALL YLAMOV( 'All', K, NQC2, V( IOFFV ), LDV, WORK( IPV ),  
     +                  LV )
         ELSE
            CALL YGEBR2D( ICTXT, 'Columnwise', COLBTOP, K, NQC2,        
     +             WORK( IPV ), LV, IVROW, MYCOL )
            IF( MYCOL.EQ.IVCOL )          CALL YTRBR2D( ICTXT, 
     +'Columnwise', COLBTOP, 'Lower',                        'Non unit',
...
*
         IF( NQC2.GT.0 ) THEN
            CALL YGEMM( 'No Transpose', 'Transpose', MPC2, K, NQC2,     
     +             ONE, C( IOFFC2 ), LDC, WORK( IPV ), LV, ZERO,        
     +          WORK( IPW ), LW )
         ELSE
            CALL YLASET( 'All', MPC2, K, ZERO, ZERO, WORK( IPW ), LW )
         END IF
*
...
*
   40       CONTINUE
            IF( JJBEG.LE.JJNXT ) THEN
               CALL PBYMATADD( ICTXT, 'No transpose', MPC2,             
     +             JJNXT-JJBEG+1, ONE,                          C( 
     +IIC1+(JJBEG-1)*LDC ), LDC, ONE,                          WORK( 
     +IPW+ILEFT*LW ), LW )
               MYDIST = MYDIST + NPCOL
         ...
         END IF
*
         CALL YGSUM2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),     
     +             LW, MYROW, IVCOL )
*
*        WORK( IPW ) = WORK( IPW ) * T' or WORK( IPW ) * T
*
         IF( MYCOL.EQ.IVCOL ) THEN
            DO 50 J = 1, K
               CALL YLACGV( K-J+1, T( J+(J-1)*MBV ), 1 )
   50       CONTINUE
            CALL YTRMM( 'Right', 'Lower', TRANS, 'Non unit', MPC2, K,   
     +                ONE, T, MBV, WORK( IPW ), LW )
            CALL YGEBS2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),  
     +                   LW )
            DO 60 J = 1, K
               CALL YLACGV( K-J+1, T( J+(J-1)*MBV ), 1 )
   60       CONTINUE
         ELSE
            CALL YGEBR2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),  
     +                   LW, MYROW, IVCOL )
         END IF
*
*        C1 = C1 - WORK( IPW )
...
*
   70       CONTINUE
            IF( JJBEG.LE.JJNXT ) THEN
               CALL PBYMATADD( ICTXT, 'No transpose', MPC2,             
     +             JJNXT-JJBEG+1, -ONE,                          WORK( 
     +IPW+ILEFT*LW ), LW, ONE,                          C( 
     +IIC1+(JJBEG-1)*LDC ), LDC )
               MYDIST = MYDIST + NPCOL
               ILEFT = MYDIST * NBC - ICOFFC1
               JJBEG = JJNXT +1
               JJNXT = MIN( JJNXT+NBC, JJEND )
...
*
         DO 80 J = 1, NQC2
            CALL YLACGV( K, WORK( IPV+(J-1)*LV ), 1 )
   80    CONTINUE
         IF( IOFFC2.GT.0 )       CALL YGEMM( 'No transpose', 
     +'No transpose', MPC2, NQC2, K,                   -ONE, WORK( IPW 
     +), LW, WORK( IPV ), LV, ONE,                   C( IOFFC2 ), LDC )
*
      END IF
*
      RETURN
