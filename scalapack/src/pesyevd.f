      SUBROUTINE PESYEVD( JOBZ, UPLO, N, A, IA, JA, DESCA, W, Z, IZ, JZ,
...
*  =======
*
*  PESYEVD computes  all the eigenvalues and eigenvectors
*  of a real symmetric matrix A by calling the recommended sequence
*  of ScaLAPACK routines.
*
*  In its present form, PESYEVD assumes a homogeneous system and makes
...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME, INDXG2P, NUMROC, PELAMCH, PELANSY
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, ESCAL, PCHK1MAT,      
     +              PELARED1D, PELASCL, PELASET, PEORMTR, PESTEDC,      
     +              PESYTRD, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                  INFO )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYEVD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PELARED1D( N, IA, JA, DESCA, WORK( INDD ), W,                
     + WORK( INDWORK ), LLWORK )
*
      CALL PELARED1D( N, IA, JA, DESCA, WORK( INDE2 ), WORK( INDE ),    
     +             WORK( INDWORK ), LLWORK )
*
      CALL PELASET( 'Full', N, N, ZERO, ONE, Z, 1, 1, DESCZ )
*
      IF( UPPER ) THEN
         OFFSET = 1
      ELSE
         OFFSET = 0
      END IF
      CALL PESTEDC( 'I', N, W, WORK( INDE+OFFSET ), Z, IZ, JZ, DESCZ,   
