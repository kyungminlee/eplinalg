      SUBROUTINE PESTEDC( COMPZ, N, D, E, Q, IQ, JQ, DESCQ, WORK, LWORK,
     +                     IWORK, LIWORK, INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCQ( * ), IWORK( * )
      REAL(KIND=10)   D( * ), E( * ), Q( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*  PESTEDC computes all eigenvalues and eigenvectors of a
...
*  without guard digits, but we know of none.  See ELAED3 for details.
*
*  Arguments
*  =========
*
...
*
*  D       (global input/output) REAL(KIND=10) array, dimension (N)
*          On entry, the diagonal elements of the tridiagonal matrix.
*          On exit, if INFO = 0, the eigenvalues in descending order.
*
*  E       (global input/output) REAL(KIND=10) array, dimension (N-1)
*          On entry, the subdiagonal elements of the tridiagonal matrix.
*          On exit, E has been destroyed.
*
*  Q       (local output) REAL(KIND=10) array,
...
*     .. External Functions ..
      ...
      REAL(KIND=10)   ELANST
      EXTERNAL           INDXG2P, LSAME, NUMROC, ELANST
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, ELASCL, ESTEDC,       
     +             INFOG2L, PELAED0, PELASRT, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD
*     ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCQ( CTXT_ ), 'PESTEDC', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF( N.LE.NB ) THEN
         IF( ( MYROW.EQ.IQROW ) .AND. ( MYCOL.EQ.IQCOL ) ) THEN
            IPQ = IIQ + ( JJQ-1 )*LDQ
            CALL ESTEDC( 'I', N, D, E, Q( IPQ ), LDQ, WORK, LWORK,      
     +              IWORK, LIWORK, INFO )
            IF( INFO.NE.0 ) THEN
               INFO = ( N+1 ) + N
               GO TO 10
      ...
      END IF
*
*     If P=NPROW*NPCOL=1, solve the problem with ESTEDC.
*
      IF( NPCOL*NPROW.EQ.1 ) THEN
         IPQ = IIQ + ( JJQ-1 )*LDQ
         CALL ESTEDC( 'I', N, D, E, Q( IPQ ), LDQ, WORK, LWORK, IWORK,  
     +               LIWORK, INFO )
         GO TO 10
      END IF
*
...
*
      ...
      IF( ORGNRM.NE.ZERO ) THEN
         ...
         CALL ELASCL( 'G', 0, 0, ORGNRM, ONE, N-1, 1, E, N-1, INFO )
      END IF
*
      CALL PELAED0( N, D, E, Q, IQ, JQ, DESCQ, WORK, IWORK, INFO )
*
*     Sort eigenvalues and corresponding eigenvectors
*
      CALL PELASRT( 'I', N, D, Q, IQ, JQ, DESCQ, WORK, LWORK, IWORK,    
     +           LIWORK, INFO )
*
*           Scale back.
*
      IF( ORGNRM.NE.ZERO )    CALL ELASCL( 'G', 0, 0, ONE, ORGNRM, N, 1,
     + D, N, INFO )
*
   10 CONTINUE
*
      IF( LWORK.GT.0 )    WORK( 1 ) = REAL( LWMIN , KIND=10)
      IF( LIWORK.GT.0 )
     $   IWORK( 1 ) = LIWMIN
      RETURN
*
