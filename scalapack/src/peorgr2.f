      SUBROUTINE PEORGR2( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEORGR2 generates an M-by-N real distributed matrix Q denoting
...
*          returned by PEGERQF in the K rows of its distributed
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension LOCr(IA+M-1)
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORGR2', -INFO )
...
*
         CALL PELASET( 'All', M-K, N-M, ZERO, ZERO, A, IA, JA, DESCA )
         CALL PELASET( 'All', M-K, M, ZERO, ONE, A, IA, JA+N-M, DESCA )
*
      END IF
*
      TAUI = ZERO
...
*
         CALL PEELSET( A, I, JA+N-M+I-IA, DESCA, ONE )
         CALL PELARF( 'Right', I-IA, I-IA+N-M+1, A, I, JA, DESCA,       
     ...
     $                    NPROW )
         IF( MYROW.EQ.IAROW )
     $      TAUI = TAU( MIN( II, MP ) )
         CALL PESCAL( I-IA+N-M, -TAUI, A, I, JA, DESCA, DESCA( M_ ) )
         CALL PEELSET( A, I, JA+N-M+I-IA, DESCA, ONE-TAUI )
*
*        Set A(i,ja+n-m+i-ia+1:ja+n-1) to zero
*
         CALL PELASET( 'All', 1, IA+M-1-I, ZERO, ZERO, A, I,            
     +      JA+N-M+I-IA+1, DESCA )
*
   10 CONTINUE
*
