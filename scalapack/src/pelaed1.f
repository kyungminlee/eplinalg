      SUBROUTINE PELAED1( N, N1, D, ID, Q, IQ, JQ, DESCQ, RHO, WORK,    
     +                 IWORK, INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCQ( * ), IWORK( * )
      REAL(KIND=10)   D( * ), Q( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PELAED1 computes the updated eigensystem of a diagonal
...
*        performed by the routine PELAED2.
*
*        The second stage consists of calculating the updated
*        eigenvalues. This is done by finding the roots of the secular
*        equation via the routine ELAED4 (as called by PELAED3).
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, ECOPY, DESCINIT, INFOG1L,      
     +              INFOG2L, PEGEMM, PELAED2, PELAED3, PELAEDZ,         
     +           PELASET, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCQ( CTXT_ ), 'PELAED1', -INFO )
         RETURN
      END IF
*
...
*
      IPWORK = IDLMDA
      CALL PELAEDZ( N, N1, ID, Q, IQ, JQ, LDQ, DESCQ, WORK( IZ ),       
     +        WORK( IPWORK ) )
*
*     Deflate eigenvalues.
*
      IPQ = IIQ + ( JJQ-1 )*LDQ
      CALL PELAED2( ICTXT, K, N, N1, NB, D, IQROW, IQCOL, Q( IPQ ), LDQ,
...
*
      IF( K.NE.0 ) THEN
         CALL PELASET( 'A', N, N, ZERO, ONE, WORK( IPU ), 1, 1, DESCU )
         CALL PELAED3( ICTXT, K, N, NB, D, IQROW, IQCOL, RHO,           
...
*
         ...
         IF( NN1.GT.0 ) THEN
            INQ = IQ - 1 + ID
            JNQ = JQ - 1 + ID + IQQ - 1
            CALL PEGEMM( 'N', 'N', N1, NN, NN1, ONE, WORK( IPQ2 ), 1,   
     ...
     +             ZERO, Q, INQ, JNQ, DESCQ )
         ...
         IF( NN2.GT.0 ) THEN
            INQ = IQ - 1 + ID + N1
            JNQ = JQ - 1 + ID + IQQ - 1
            CALL PEGEMM( 'N', 'N', N-N1, NN, NN2, ONE, WORK( IPQ2 ),    
...
*
         DO 10 J = K + 1, N
            ...
            IF( MYCOL.EQ.COL ) THEN
               ...
               INQ = IPQ + ( JJC-1 )*LDQ
               CALL ECOPY( NP, WORK( IQ2 ), 1, Q( INQ ), 1 )
            END IF
   10    CONTINUE
      END IF
*
