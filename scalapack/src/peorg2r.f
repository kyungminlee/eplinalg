      SUBROUTINE PEORG2R( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEORG2R generates an M-by-N real distributed matrix Q denoting
...
*          returned by PEGEQRF in the K columns of its array
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORG2R', -INFO )
...
*
      CALL PELASET( 'All', K, N-K, ZERO, ZERO, A, IA, JA+K, DESCA )
      CALL PELASET( 'All', M-K, N-K, ZERO, ONE, A, IA+K, JA+K, DESCA )
*
      TAUJ = ZERO
...
*
         IF( J.LT.JA+N-1 ) THEN
            CALL PEELSET( A, IA+J-JA, J, DESCA, ONE )
            CALL PELARF( 'Left', M-J+JA, JA+N-J-1, A, IA+J-JA, J, DESCA,
...
*
     ...
     $                    NPCOL )
         IF( MYCOL.EQ.IACOL )
     $      TAUJ = TAU( MIN( JJ, KQ ) )
         IF( J-JA.LT.M-1 )       CALL PESCAL( M-J+JA-1, -TAUJ, A, 
     +IA+J-JA+1, J, DESCA, 1 )
         CALL PEELSET( A, IA+J-JA, J, DESCA, ONE-TAUJ )
*
*        Set A(ia:ia+j-ja-1,j) to zero
*
         CALL PELASET( 'All', J-JA, 1, ZERO, ZERO, A, IA, J, DESCA )
*
   10 CONTINUE
*
