      SUBROUTINE PYUNMQL( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*  =======
*
*  PYUNMQL overwrites the general complex M-by-N distributed matrix
...
*     .. External Subroutines ..
     ...
     +                PYLARFT, PYUNM2L, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNMQL', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. .NOT.NOTRAN ) ) THEN
         ...
         ELSE
            NI = N - K + JB
         END IF
         CALL PYUNM2L( SIDE, TRANS, MI, NI, JB, A, IA, JA, DESCA, TAU,  
...
*
         CALL PYLARFT( 'Backward', 'Columnwise', NQ-K+J+JB-JA, JB,      
...
*        Apply H or H'
*
         CALL PYLARFB( SIDE, TRANS, 'Backward', 'Columnwise', MI, NI,   
