      SUBROUTINE PEYSUM1( N, ASUM, X, IX, JX, DESCX, INCX )
      IMPLICIT NONE
*
...
*  =======
*
*  PEYSUM1 returns the sum of absolute values of a complex
*  distributed vector sub( X ) in ASUM,
*
*  where sub( X ) denotes X(IX:IX+N-1,JX:JX), if INCX = 1,
*                         X(IX:IX,JX:JX+N-1), if INCX = M_X.
*
*  Based on PEYASUM from the Level 1 PBLAS. The change is
...
*          N >= 0.
*
*  ASUM    (local output) pointer to REAL(KIND=10)
*          The sum of absolute values of the distributed vector sub( X )
*          only in its scope.
*
*  X       (local input) COMPLEX(KIND=10) array containing the local
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGSUM2D, INFOG2L, PB_TOPGET
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      REAL(KIND=10)   EYSUM1
      EXTERNAL           EYSUM1, NUMROC
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MOD
*     ..
...
*
         IF( MYROW.EQ.IXROW ) THEN
            ...
            IF( MYCOL.EQ.IXCOL )
     $         NQ = NQ-ICOFF
            ASUM = EYSUM1( NQ, X( IIX+(JJX-1)*LDX ), LDX )
            CALL EGSUM2D( ICTXT, 'Rowwise', RCTOP, 1, 1, ASUM, 1,       
     +              -1, MYCOL )
         END IF
*
      ELSE
...
*
         IF( MYCOL.EQ.IXCOL ) THEN
            ...
            IF( MYROW.EQ.IXROW )
     $         NP = NP-IROFF
            ASUM = EYSUM1( NP, X( IIX+(JJX-1)*LDX ), 1 )
            CALL EGSUM2D( ICTXT, 'Columnwise', CCTOP, 1, 1, ASUM, 1,    
     +                 -1, MYCOL )
         END IF
*
      END IF
