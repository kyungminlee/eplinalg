      SUBROUTINE PYUNM2L( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCC( * )
      COMPLEX(KIND=10)            A( * ), C( * ), TAU( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYUNM2L overwrites the general complex M-by-N distributed matrix
...
*        Q = H(k) . . . H(2) H(1)
*
*  as returned by PYGEQLF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          PYGEQLF in the K columns of its distributed matrix
...
*          elementary reflectors H(j) as returned by PYGEQLF.
...
*     .. External Subroutines ..
     ...
     +          YGEBS2D, YGERV2D, YGESD2D, CHK1MAT,                    
     +YSCAL, INFOG2L, PYELSET, PYELSET2,                    PYLARF, 
     +PYLARFC, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNM2L', -INFO )
...
*
     ...
     $                      NPCOL )
               IF( MYCOL.EQ.IACOL ) THEN
                  ...
                  END IF
                  CALL YGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, AJJ, 1 )
                  CALL YSCAL( NQ-JCC+1, AJJ,                         C( 
     +ICC+(JCC-1)*DESCC( LLD_ ) ),                         DESCC( LLD_ 
     +) )
               ELSE
                  CALL YGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, AJJ, 1,    
     +                       IAROW, IACOL )
                  CALL YSCAL( NQ-JCC+1, AJJ,                         C( 
     +ICC+(JCC-1)*DESCC( LLD_ ) ),                         DESCC( LLD_ 
     +) )
...
*
            IF( IACOL.NE.ICCOL ) THEN
               IF( MYCOL.EQ.IACOL )             CALL YGESD2D( ICTXT, 1, 
     +1, AJJ, 1, MYROW, ICCOL )
               IF( MYCOL.EQ.ICCOL )             CALL YGERV2D( ICTXT, 1, 
     +1, AJJ, 1, MYROW, IACOL )
            END IF
*
     ...
     $                      NPROW )
               CALL YSCAL( MP-ICC+1, AJJ, C( ICC+(JCC-1)*               
     +       DESCC( LLD_ ) ), 1 )
            END IF
*
         END IF
...
*           Apply H(j) or H(j)'
*
            CALL PYELSET2( AJJ, A, IA+NQ-K+J-JA, J, DESCA, ONE )
            IF( NOTRAN ) THEN
               CALL PYLARF( SIDE, MI, NI, A, IA, J, DESCA, 1, TAU, C,   
     +                    IC, JC, DESCC, WORK )
            ELSE
               CALL PYLARFC( SIDE, MI, NI, A, IA, J, DESCA, 1, TAU, C,  
     +                      IC, JC, DESCC, WORK )
            END IF
            CALL PYELSET( A, IA+NQ-K+J-JA, J, DESCA, AJJ )
*
   10    CONTINUE
*
