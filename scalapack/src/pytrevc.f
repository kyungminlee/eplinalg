      SUBROUTINE PYTREVC( SIDE, HOWMNY, SELECT, N, T, DESCT, VL, DESCVL,
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)               RWORK( * )
      COMPLEX(KIND=10)            T( * ), VL( * ), VR( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYTREVC computes some or all of the right and/or left eigenvectors of
...
*          Additional workspace may be required if PYLATTRS is updated
*          to use WORK.
*
...
*  been implemented in PYLATTRS which is called by this routine to solve
*  the triangular systems.  PYLATTRS just calls PYTRSV.
*
...
*     .. Local Scalars ..
     ...
     $                   MYCOL, MYROW, NB, NPCOL, NPROW, RSRC
      REAL(KIND=10)               SELF
      REAL(KIND=10)               OVFL, REMAXD, SCALE, SMLNUM, ULP, UNFL
      COMPLEX(KIND=10)            CDUM, REMAXC, SHIFT
*     ..
*     .. Local Arrays ..
      INTEGER            DESCW( DLEN_ )
      REAL(KIND=10)               SMIN( 1 )
...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME, PELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DESCINIT, EGSUM2D, IGAMN2D,    
     +                INFOG2L, PELABAD, PEYASUM, PXERBLA, PYAMAX,       
     +             PYCOPY, PYESCAL, PYGEMV, PYLASET, PYLATTRS,          
     +          YGSUM2D
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
...
*
     ...
     $              -1, -1 )
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( CONTXT, 'PYTREVC', -INFO )
         RETURN
      END IF
*
...
*     but not used.  For use in PYLATTRS.
*
      RWORK( 1 ) = ZERO
      DO 30 J = 2, N
         CALL PEYASUM( J-1, RWORK( J ), T, 1, J, DESCT, 1 )
   30 CONTINUE
*     I replicate the norms in RWORK.  Should they be distributed
*     over the process rows?
      CALL EGSUM2D( CONTXT, 'Row', ' ', N, 1, RWORK, N, -1, -1 )
*
      IF( RIGHTV ) THEN
*
...
*
     ...
     $                    IROW, ICOL, ITMP1, ITMP2 )
            ...
            END IF
            CALL EGSUM2D( CONTXT, 'ALL', ' ', 1, 1, SMIN, 1, -1, -1 )
            CALL YGSUM2D( CONTXT, 'ALL', ' ', 1, 1, SHIFT, 1, -1, -1 )
*
...
*
            IF( KI.GT.1 ) THEN
               CALL PYCOPY( KI-1, T, 1, KI, DESCT, 1, WORK, 1, 1, DESCW,
     +                       1 )
            END IF
            DO 40 K = 1, KI - 1
...
*
            IF( .NOT.OVER ) THEN
               CALL PYCOPY( KI, WORK, 1, 1, DESCW, 1, VR, 1, IS, DESCVR,
     +                       1 )
*
               CALL PYAMAX( KI, REMAXC, II, VR, 1, IS, DESCVR, 1 )
               REMAXD = ONE / MAX( CABS1( REMAXC ), UNFL )
               CALL PYESCAL( KI, REMAXD, VR, 1, IS, DESCVR, 1 )
*
               CALL PYLASET( ' ', N-KI, 1, CZERO, CZERO, VR, KI+1, IS,  
     +                      DESCVR )
            ELSE
               IF( KI.GT.1 )             CALL PYGEMV( 'N', N, KI-1, 
     +CONE, VR, 1, 1, DESCVR,                          WORK, 1, 1, 
     +DESCW, 1, CMPLX( SCALE , KIND=10),                          VR, 1,
     + KI, DESCVR, 1 )
*
               CALL PYAMAX( N, REMAXC, II, VR, 1, KI, DESCVR, 1 )
               REMAXD = ONE / MAX( CABS1( REMAXC ), UNFL )
               CALL PYESCAL( N, REMAXD, VR, 1, KI, DESCVR, 1 )
            END IF
*
...
*
            IF( KI.LT.N ) THEN
               CALL PYCOPY( N-KI, T, KI, KI+1, DESCT, N, WORK, KI+1, 1, 
     +                      DESCW, 1 )
            END IF
            DO 80 K = KI + 1, N
               CALL INFOG2L( K, 1, DESCW, NPROW, NPCOL, MYROW, MYCOL,
...
*
            IF( KI.LT.N ) THEN
               CALL PYLATTRS( 'Upper', 'Conjugate transpose', 'Nonunit',
...
*
            IF( .NOT.OVER ) THEN
               CALL PYCOPY( N-KI+1, WORK, KI, 1, DESCW, 1, VL, KI, IS,  
     +                     DESCVL, 1 )
*
               CALL PYAMAX( N-KI+1, REMAXC, II, VL, KI, IS, DESCVL, 1 )
               REMAXD = ONE / MAX( CABS1( REMAXC ), UNFL )
               CALL PYESCAL( N-KI+1, REMAXD, VL, KI, IS, DESCVL, 1 )
*
               CALL PYLASET( ' ', KI-1, 1, CZERO, CZERO, VL, 1, IS,     
     +                   DESCVL )
            ELSE
               IF( KI.LT.N )             CALL PYGEMV( 'N', N, N-KI, 
     +CONE, VL, 1, KI+1, DESCVL,                          WORK, KI+1, 1,
     + DESCW, 1, CMPLX( SCALE , KIND=10),                          VL, 
     +1, KI, DESCVL, 1 )
*
               CALL PYAMAX( N, REMAXC, II, VL, 1, KI, DESCVL, 1 )
               REMAXD = ONE / MAX( CABS1( REMAXC ), UNFL )
               CALL PYESCAL( N, REMAXD, VL, 1, KI, DESCVL, 1 )
            END IF
*
*           Set back the original diagonal elements of T.
*
