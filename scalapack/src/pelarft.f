      SUBROUTINE PELARFT( DIRECT, STOREV, N, K, V, IV, JV, DESCV, TAU,  
     +                   T, WORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCV( * )
      REAL(KIND=10)   TAU( * ), T( * ), V( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PELARFT forms the triangular factor T of a real block reflector H
...
*
*  V       (input/output) REAL(KIND=10) pointer into the local memory
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension LOCr(IV+K-1)
...
*
*  T       (local output) REAL(KIND=10) array, dimension (NB_V,NB_V)
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, ECOPY, EGEMV, EGSUM2D,         
     +           ELASET, ETRMV, INFOG2L
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
               ...
               IF( NP-II+IIV.GT.0 ) THEN
                  CALL EGEMV( 'Transpose', NP-II+IIV, ITMP0,            
     +             -TAU( JJ ), V( II+(JJV-1)*LDV ), LDV,                
     +         V( II+(JJ-1)*LDV ), 1, ZERO,                         
     +WORK( IW ), 1 )
               ELSE
                  CALL ELASET( 'All', ITMP0, 1, ZERO, ZERO, WORK( IW ), 
     +                         ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
   10       CONTINUE
*
            CALL EGSUM2D( ICTXT, 'Columnwise', ' ', IW-1, 1, WORK, IW-1,
     +                     IVROW, MYCOL )
*
            IF( MYROW.EQ.IVROW ) THEN
*
...
*
                  ...
                  CALL ECOPY( ITMP0, WORK( IW ), 1, T( ITMP1 ), 1 )
                  IW = IW + ITMP0
*
...
*
               ...
               IF( II-IIV+1.GT.0 ) THEN
                  CALL EGEMV( 'Transpose', II-IIV+1, ITMP0, -TAU( JJ ), 
     +                        V( IIV+JJ*LDV ), LDV,                     
     +    V( IIV+(JJ-1)*LDV ), 1, ZERO,                         WORK( 
     +IW ), 1 )
               ELSE
                  CALL ELASET( 'All', ITMP0, 1, ZERO, ZERO, WORK( IW ), 
     +                         ITMP0 )
...
*
                  ...
                  IW = IW + ITMP0
*
                  CALL ETRMV( 'Lower', 'No transpose', 'Non-unit',      
...
*
               ...
               IF( NQ-JJ+JJV.GT.0 ) THEN
                  CALL EGEMV( 'No transpose', ITMP0, NQ-JJ+JJV,         
     +                -TAU(II), V( IIV+(JJ-1)*LDV ), LDV,               
     +          V( II+(JJ-1)*LDV ), LDV, ZERO,                         
     +WORK( IW ), 1 )
               ELSE
                  CALL ELASET( 'All', ITMP0, 1, ZERO, ZERO,             
     +             WORK( IW ), ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
   50       CONTINUE
*
            CALL EGSUM2D( ICTXT, 'Rowwise', ' ', IW-1, 1, WORK, IW-1,   
     +                  MYROW, IVCOL )
*
            IF( MYCOL.EQ.IVCOL ) THEN
*
...
*
               ...
               IF( JJ-JJV+1.GT.0 ) THEN
                  CALL EGEMV( 'No transpose', ITMP0, JJ-JJV+1,          
