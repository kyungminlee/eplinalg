      SUBROUTINE PEGEHRD( N, ILO, IHI, A, IA, JA, DESCA, TAU, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEHRD reduces a real general distributed matrix sub( A )
...
*     .. External Subroutines ..
     ...
     +               INFOG2L, PCHK1MAT, PEGEMM, PEGEHD2,                
     +    PELAHRD, PELARFB, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEHRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PELAHRD( IHI, K, IB, A, IA, J, DESCA, TAU, WORK( IPT ),   
...
*
         CALL PEELSET2( EI, A, I+IB, J+IB-1, DESCA, ONE )
         CALL PEGEMM( 'No transpose', 'Transpose', IHI, IHI-K-IB+1, IB, 
     +                -ONE, WORK( IPY ), 1, JY, DESCY, A, I+IB, J,      
     +           DESCA, ONE, A, IA, J+IB, DESCA )
         CALL PEELSET( A, I+IB, J+IB-1, DESCA, EI )
*
...
*
      CALL PEGEHD2( N, K, IHI, A, IA, JA, DESCA, TAU, WORK, LWORK,      
     +         IINFO )
*
