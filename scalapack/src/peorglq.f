      SUBROUTINE PEORGLQ( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEORGLQ generates an M-by-N real distributed matrix Q denoting
...
*          returned by PEGELQF in the K rows of its distributed matrix
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension LOCr(IA+K-1).
*          This array contains the scalar factors TAU(i) of the
*          elementary reflectors H(i) as returned by PEGELQF.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK1MAT, PELARFB,    
     +                PELARFT, PELASET, PEORGL2, PB_TOPGET,             
     +       PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORGLQ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PELASET( 'All', IA+M-IL, IL-IA, ZERO, ZERO, A, IL, JA,       
     +        DESCA )
*
*     Use unblocked code for the last or only block.
*
      CALL PEORGL2( IA+M-IL, N-IL+IA, IA+K-IL, A, IL, JA+IL-IA, DESCA,  
...
*
               CALL PELARFT( 'Forward', 'Rowwise', N-I+IA, IB, A, I, J, 
     +                       DESCA, TAU, WORK, WORK( IPW ) )
*
*              Apply H' to A(i+ib:ia+m-1,j:ja+n-1) from the right
*
               CALL PELARFB( 'Right', 'Transpose', 'Forward', 'Rowwise',
...
*
            CALL PEORGL2( IB, N-I+IA, IB, A, I, J, DESCA, TAU, WORK,    
     +                 LWORK, IINFO )
*
*           Set columns ia:i-1 of current block to zero
*
            CALL PELASET( 'All', IB, I-IA, ZERO, ZERO, A, I, JA, DESCA )
   10    CONTINUE
*
      END IF
*
...
*        H = H(i) H(i+1) . . . H(i+ib-1)
*
         CALL PELARFT( 'Forward', 'Rowwise', N, IB, A, IA, JA, DESCA,   
     +               TAU, WORK, WORK( IPW ) )
*
*        Apply H' to A(ia+ib:ia+m-1,ja:ja+n-1) from the right
*
         CALL PELARFB( 'Right', 'Transpose', 'Forward', 'Rowwise', M-IB,
     +                  N, IB, A, IA, JA, DESCA, WORK, A, IA+IB, JA,    
     +              DESCA, WORK( IPW ) )
*
*        Apply H' to columns ja:ja+n-1 of current block
*
         CALL PEORGL2( IB, N, IB, A, IA, JA, DESCA, TAU, WORK, LWORK,   
     +               IINFO )
*
      END IF
*
