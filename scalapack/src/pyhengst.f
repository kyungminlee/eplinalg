      SUBROUTINE PYHENGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,
...
*  =======
*
*  PYHENGST reduces a complex Hermitian-definite generalized
*  eigenproblem to standard form.
*
*  PYHENGST performs the same function as PYHEGST, but is based on
*  rank 2K updates, which are faster and more scalable than
*  triangular solves (the basis of PYHENGST).
*
*  PYHENGST calls PYHEGST when UPLO='U', hence PYHENGST provides
*  improved performance only when UPLO='L', IBTYPE=1.
*
*  PYHENGST also calls PYHEGST when insufficient workspace is
*  provided,  hence PYHENGST provides improved
...
*
*          When IBTYPE = 1 and UPLO = 'L', PYHENGST provides improved
...
*
*     .. Parameters ..
      COMPLEX(KIND=10)            ONEHALF, ONE, MONE
      REAL(KIND=10)               RONE
      PARAMETER          ( ONEHALF = ( 0.5E0_10, 0.0E0_10 ),            
     +        ONE = ( 1.0E0_10, 0.0E0_10 ),                    MONE = ( 
     +-1.0E0_10, 0.0E0_10 ), RONE = 1.0E0_10 )
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCSET, PYGEMM,      
     +              PYHEGST, PYHEMM, PYHER2K, PCHK2MAT, PYLACPY,        
     +            PYTRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHENGST', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF( IBTYPE.NE.1 .OR. UPPER .OR. LWORK.LT.LWOPT ) THEN
         CALL PYHEGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,    
     +              DESCB, SCALE, INFO )
         RETURN
      END IF
*
...
*
*
         CALL PYLACPY( 'A', N-POSTK+1, KB, B, POSTK+IB-1, K+JB-1, DESCB,
     +                  WORK( INDG ), POSTK, 1, DESCG )
         CALL PYLACPY( 'A', N-POSTK+1, KB, A, POSTK+IA-1, K+JA-1, DESCA,
     +                  WORK( INDR ), POSTK, 1, DESCR )
         CALL PYLACPY( 'A', KB, K-1, A, K+IA-1, JA, DESCA,              
     +    WORK( INDRT ), 1, 1, DESCRT )
*
         CALL PYLACPY( 'L', KB, KB, A, K+IA-1, K+JA-1, DESCA,           
     +       WORK( INDR ), K, 1, DESCR )
         CALL PYTRSM( 'Right', 'L', 'N', 'N', NPK, KB, MONE, B, K+IB-1, 
     +                K+JB-1, DESCB, WORK( INDG ), POSTK, 1, DESCG )
*
         CALL PYHEMM( 'Right', 'L', NPK, KB, ONEHALF, A, K+IA-1, K+JA-1,
     +                 DESCA, WORK( INDG ), POSTK, 1, DESCG, ONE,       
     +          WORK( INDR ), POSTK, 1, DESCR )
*
         CALL PYHER2K( 'Lower', 'No T', NPK, KB, ONE, WORK( INDG ),     
     +             POSTK, 1, DESCG, WORK( INDR ), POSTK, 1, DESCR,      
     +            RONE, A, POSTK+IA-1, POSTK+JA-1, DESCA )
*
         CALL PYGEMM( 'No T', 'No Conj', NPK, K-1, KB, ONE,             
     +    WORK( INDG ), POSTK, 1, DESCG, WORK( INDRT ), 1,              
     +   1, DESCRT, ONE, A, POSTK+IA-1, JA, DESCA )
*
         CALL PYHEMM( 'Right', 'L', NPK, KB, ONE, WORK( INDR ), K, 1,   
     +              DESCR, WORK( INDG ), POSTK, 1, DESCG, ONE, A,       
     +          POSTK+IA-1, K+JA-1, DESCA )
*
         CALL PYTRSM( 'Left', 'Lower', 'No Conj', 'Non-unit', KB, K-1,  
...
*
     ...
     $               = CONJG( WORK( INDAA+I-1+( J-1 )*DESCAA( LLD_ ) ) )
   10          CONTINUE
   20       CONTINUE
         END IF
*
         CALL PYTRSM( 'Left', 'Lower', 'No Conj', 'Non-unit', KB, KB,   
     +              ONE, B, K+IB-1, K+JB-1, DESCB, WORK( INDAA ), 1,    
     +             1, DESCAA )
*
         CALL PYTRSM( 'Right', 'Lower', 'Conj', 'Non-unit', KB, KB, ONE,
     +                 B, K+IB-1, K+JB-1, DESCB, WORK( INDAA ), 1, 1,   
     +              DESCAA )
*
         CALL PYLACPY( 'L', KB, KB, WORK( INDAA ), 1, 1, DESCAA, A,     
     +             K+IA-1, K+JA-1, DESCA )
*
         CALL PYTRSM( 'Right', 'Lower', 'Conj', 'Non-unit', NPK, KB,    
