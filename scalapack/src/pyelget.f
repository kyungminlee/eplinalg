      SUBROUTINE PYELGET( SCOPE, TOP, ALPHA, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*  =======
*
*  PYELGET sets alpha to the distributed matrix entry A( IA, JA ).
      ...
      IF( LSAME( SCOPE, 'R' ) ) THEN
         IF( MYROW.EQ.IAROW ) THEN
            IF( MYCOL.EQ.IACOL ) THEN
               ...
               CALL YGEBS2D( ICTXT, SCOPE, TOP, 1, 1, A( IOFFA ), 1 )
               ALPHA = A( IOFFA )
            ELSE
               CALL YGEBR2D( ICTXT, SCOPE, TOP, 1, 1, ALPHA, 1,         
     +               IAROW, IACOL )
            END IF
         END IF
