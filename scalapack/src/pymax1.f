      SUBROUTINE PYMAX1( N, AMAX, INDX, X, IX, JX, DESCX, INCX )
      IMPLICIT NONE
*
...
*  =======
*
*  PYMAX1 computes the global index of the maximum element in absolute
...
*
*  Based on PYAMAX from Level 1 PBLAS. The change is to use the
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YCOMBAMAX1, YGAMX2D,           
     +         IGEBR2D, IGEBS2D, INFOG2L, PYTREECOMB,                   
     + PB_TOPGET
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            IYMAX1, INDXL2G, NUMROC
      EXTERNAL           IYMAX1, INDXL2G, NUMROC
*     ..
*     .. Intrinsic Functions ..
...
*
               IF( NQ.GT.0 ) THEN
                  LCINDX = JJX-1+IYMAX1( NQ, X( IIX+(JJX-1)*LDX ), LDX )
                  WORK( 1 ) = X( IIX+(LCINDX-1)*LDX )
                  WORK( 2 ) = CMPLX( REAL( INDXL2G( LCINDX,             
     +  DESCX( NB_ ), MYCOL, DESCX( CSRC_ ), NPCOL ) , KIND=10) , 
     +KIND=10)
               ELSE
                  WORK( 1 ) = ZERO
                  WORK( 2 ) = ZERO
               END IF
*
               CALL PYTREECOMB( ICTXT, 'Row', 2, WORK, -1, MYCOL,       
     +                    YCOMBAMAX1 )
*
               AMAX = WORK( 1 )
...
*
               CALL YGAMX2D( ICTXT, 'Rowwise', RCTOP, 1, 1, AMAX, 1,    
...
*
               IF( NP.GT.0 ) THEN
                  LCINDX = IIX-1+IYMAX1( NP, X( IIX+(JJX-1)*LDX ), 1 )
                  WORK( 1 ) = X( LCINDX + (JJX-1)*LDX )
                  WORK( 2 ) = CMPLX( REAL( INDXL2G( LCINDX,             
     +  DESCX( MB_ ), MYROW, DESCX( RSRC_ ), NPROW ) , KIND=10) , 
     +KIND=10)
               ELSE
                  WORK( 1 ) = ZERO
                  WORK( 2 ) = ZERO
               END IF
*
               CALL PYTREECOMB( ICTXT, 'Column', 2, WORK, -1, MYCOL,    
     +                       YCOMBAMAX1 )
*
               AMAX = WORK( 1 )
               IF( AMAX.EQ.ZERO ) THEN
...
*              Find the maximum value
*
               CALL YGAMX2D( ICTXT, 'Columnwise', CCTOP, 1, 1, AMAX, 1, 
...
*
*     .. Array Arguments ..
      COMPLEX(KIND=10)            V1( 2 ), V2( 2 )
*     ..
*
*  Purpose
*  =======
*
*  YCOMBAMAX1 finds the element having maximum real part absolute
...
*     .. Executable Statements ..
*
      IF( ABS( REAL( V1( 1 ) , KIND=10) ).LT.ABS( REAL( V2( 1 ) , 
     +KIND=10) ) ) THEN
         V1( 1 ) = V2( 1 )
         V1( 2 ) = V2( 2 )
      END IF
