      SUBROUTINE PEGESVX( FACT, TRANS, N, NRHS, A, IA, JA, DESCA, AF,   
...
*  =======
*
*  PEGESVX uses the LU factorization to compute the solution to a real
...
*
*  AF      (local input or local output) REAL(KIND=10) pointer
...
*          A(IA:IA+N-1,JA:JA+N-1) = P*L*U as computed by PEGETRF.
...
*          PEGETRF; IPIV(i) -> The global row local row i was
...
*          output variable.
*
*  R       (local input or local output) REAL(KIND=10) array,
*                                                  dimension LOCr(M_A).
...
*
*  C       (local input or local output) REAL(KIND=10) array,
*                                                  dimension LOCc(N_A).
...
*
*  B       (local input/local output) REAL(KIND=10) pointer
*          into the local memory to an array of local dimension
...
*
*  X       (local input/local output) REAL(KIND=10) pointer
*          into the local memory to an array of local dimension
...
*
*  FERR    (local output) REAL(KIND=10) array, dimension LOCc(N_B)
...
*
*  BERR    (local output) REAL(KIND=10) array, dimension LOCc(N_B).
...
*
*  WORK    (local workspace/local output) REAL(KIND=10) array,
*                                                    dimension (LWORK)
...
*          LWORK = MAX( PEGECON( LWORK ), PEGERFS( LWORK ) )
*                  + LOCr( N_A ).
*
...
*     .. Local Scalars ..
     ...
     $                   NQ, NQB, NRHSQ, RFSWRK, ICOLEQU
      REAL(KIND=10)   AMAX, ANORM, BIGNUM, COLCND, RCMAX, RCMIN,        
     +            ROWCND, SMLNUM
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
     ...
     +                EGEBR2D, EGEBS2D, EGAMN2D,                    
     +EGAMX2D, INFOG2L, PECOPY, PEGECON,                    PEGEEQU, 
     +PEGERFS, PEGETRF, PEGETRS,                    PELACPY, PELAQGE, 
     +PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      ...
      EXTERNAL           ICEIL, ILCM, INDXG2P, LSAME, NUMROC, PELANGE,  
     +                  PELAMCH
*     ..
*     .. Intrinsic Functions ..
            ...
            ELSE
               IF( ROWEQU ) THEN
                  ...
                  DO 10 J = IIA, IIA + NP - 1
                     ...
                     RCMAX = MAX( RCMAX, R( J ) )
   10             CONTINUE
                  CALL EGAMN2D( ICTXT, 'Columnwise', ' ', 1, 1, RCMIN,  
     +                         1, IDUMM, IDUMM, -1, -1, MYCOL )
                  CALL EGAMX2D( ICTXT, 'Columnwise', ' ', 1, 1, RCMAX,  
     +                         1, IDUMM, IDUMM, -1, -1, MYCOL )
     ...
     $                        MIN( RCMAX, BIGNUM )
               ...
               IF( COLEQU .AND. INFO.EQ.0 ) THEN
                  ...
                  DO 20 J = JJA, JJA+NQ-1
                     ...
                     RCMAX = MAX( RCMAX, C( J ) )
   20             CONTINUE
                  CALL EGAMN2D( ICTXT, 'Rowwise', ' ', 1, 1, RCMIN,     
     +                      1, IDUMM, IDUMM, -1, -1, MYCOL )
                  CALL EGAMX2D( ICTXT, 'Rowwise', ' ', 1, 1, RCMAX,     
     +                      1, IDUMM, IDUMM, -1, -1, MYCOL )
                  IF( RCMIN.LE.ZERO ) THEN
                     INFO = -15
                  ELSE IF( N.GT.0 ) THEN
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGESVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PEGEEQU( N, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND,    
     +              AMAX, INFEQU )
         IF( INFEQU.EQ.0 ) THEN
*
*           Equilibrate the matrix.
*
            CALL PELAQGE( N, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND, 
     +                    AMAX, EQUED )
            ROWEQU = LSAME( EQUED, 'R' ) .OR. LSAME( EQUED, 'B' )
            COLEQU = LSAME( EQUED, 'C' ) .OR. LSAME( EQUED, 'B' )
            IF( COLEQU ) ICOLEQU = 1
...
*
     ...
     $                 IACOL, ICTXT, 1 )
         CALL PECOPY( N, C, 1, JA, CDESC, CDESC( LLD_ ), WORK, IB, JB,  
     +               DESCB, 1 )
         IF( MYCOL.EQ.IBCOL ) THEN
            CALL EGEBS2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IIB ),    
     +                 DESCB( LLD_ ) )
         ELSE
            CALL EGEBR2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IIB ),    
...
*
         CALL PELACPY( 'Full', N, N, A, IA, JA, DESCA, AF, IAF, JAF,    
     +              DESCAF )
         CALL PEGETRF( N, N, AF, IAF, JAF, DESCAF, IPIV, INFO )
*
...
*
      ...
      ELSE
         NORM = 'I'
      END IF
      ANORM = PELANGE( NORM, N, N, A, IA, JA, DESCA, WORK )
*
*     Compute the reciprocal of the condition number of A.
*
      CALL PEGECON( NORM, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,  
...
*
      CALL PELACPY( 'Full', N, NRHS, B, IB, JB, DESCB, X, IX, JX,       
     +        DESCX )
      CALL PEGETRS( TRANS, N, NRHS, AF, IAF, JAF, DESCAF, IPIV, X, IX,  
     +             JX, DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PEGERFS( TRANS, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,     
...
*
            CALL DESCSET( CDESC, 1, N+ICOFFA, 1, DESCA( NB_ ), MYROW,
     $                    IACOL, ICTXT, 1 )
            CALL PECOPY( N, C, 1, JA, CDESC, CDESC( LLD_ ), WORK, IX,   
     +                 JX, DESCX, 1 )
            IF( MYCOL.EQ.IBCOL ) THEN
                CALL EGEBS2D( ICTXT, 'Rowwise', ' ', NP, 1,             
     +            WORK( IIX ), DESCX( LLD_ ) )
            ELSE
                CALL EGEBR2D( ICTXT, 'Rowwise', ' ', NP, 1,             
