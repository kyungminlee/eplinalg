      SUBROUTINE PYUNGL2( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYUNGL2 generates an M-by-N complex distributed matrix Q denoting
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNGL2', -INFO )
...
*
         CALL PYLASET( 'All', M-K, K, ZERO, ZERO, A, IA+K, JA, DESCA )
         CALL PYLASET( 'All', M-K, N-K, ZERO, ONE, A, IA+K, JA+K,       
     +           DESCA )
*
      END IF
*
...
*
     ...
     $      TAUI = TAU( MIN( II, KP ) )
         IF( J.LT.JA+N-1 ) THEN
            CALL PYLACGV( N-J+JA-1, A, I, J+1, DESCA, DESCA( M_ ) )
            IF( I.LT.IA+M-1 ) THEN
               CALL PYELSET( A, I, J, DESCA, ONE )
               CALL PYLARFC( 'Right', M-I+IA-1, N-J+JA, A, I, J, DESCA, 
     +                       DESCA( M_ ), TAU, A, I+1, J, DESCA, WORK )
            END IF
            CALL PYSCAL( N-J+JA-1, -TAUI, A, I, J+1, DESCA,             
     +            DESCA( M_ ) )
            CALL PYLACGV( N-J+JA-1, A, I, J+1, DESCA, DESCA( M_ ) )
         END IF
         CALL PYELSET( A, I, J, DESCA, ONE-CONJG( TAUI ) )
*
*        Set A(i,ja:j-1) to zero
*
         CALL PYLASET( 'All', 1, J-JA, ZERO, ZERO, A, I, JA, DESCA )
*
   10 CONTINUE
*
