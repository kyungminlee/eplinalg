      SUBROUTINE PYHEEVR( JOBZ, RANGE, UPLO, N, A, IA, JA,              
     +        DESCA, VL, VU, IL, IU, M, NZ, W, Z, IZ,                   
     +  JZ, DESCZ,                      WORK, LWORK, RWORK, LRWORK, 
     +IWORK, LIWORK,                     INFO )
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   W( * ), RWORK( * )
      COMPLEX(KIND=10)         A( * ), WORK( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PYHEEVR computes selected eigenvalues and, optionally, eigenvectors
...
*
*  A       (local input/workspace) 2D block cyclic COMPLEX(KIND=10) array,
...
*          If DESCA is incorrect, PYHEEVR cannot work correctly.
...
*
*  Z       (local output) COMPLEX(KIND=10) array,
*          global dimension (N, N),
...
*
*  WORK    (local workspace/output) COMPLEX(KIND=10)  array,
*          dimension (LWORK)
...
*
*          ICTXT = DESCA( CTXT_ )
*          ANB = PJLAENV_EP( ICTXT, 3, 'PYHETTRD', 'L', 0, 0, 0, 0 )
...
*
*  RWORK    (local workspace/output) REAL(KIND=10)  array,
*          dimension (LRWORK)
...
*
*  PYHEEVR assumes IEEE 754 standard compliant arithmetic. 
*
...
*     .. Local Scalars ..
     ...
     $                   ZOFFSET
      ...
      REAL(KIND=10)            PIVMIN, SAFMIN, SCALE, VLL, VUU, WL,     
     +                        WU
*
*     .. Local Arrays ..
...
*     .. External Functions ..
      ...
      EXTERNAL            ICEIL, INDXG2P, LSAME, NUMROC, PELAMCH,       
     +              PJLAENV_EP
*     ..
*     .. External Subroutines ..
      EXTERNAL            BLACS_GRIDINFO, CHK1MAT, ECOPY, EGEBR2D,      
     +               EGEBS2D, EGERV2D, EGESD2D, ELARRC, ELASRT2,        
     +             ESTEGR2A, ESTEGR2B, ESTEGR2, IGEBR2D,                
     +     IGEBS2D, IGERV2D, IGESD2D, IGSUM2D, PCHK1MAT,                
     +     PCHK2MAT, PELARED1D, PXERBLA, PYELGET,                     
     +PYHENTRD, PYLAEVSWP, PYUNMTR
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC           ABS, REAL, CMPLX, ICHAR, INT, MAX, MIN, MOD,
     $                   SQRT
*     ..
...
*
      ANB = PJLAENV_EP( ICTXT, 3, 'PYHETTRD', 'L', 0, 0, 0, 0 )
      SQNPC = INT( SQRT( REAL( NPROCS , KIND=10) ) )
      NPS = MAX( NUMROC( N, 1, 0, 0, SQNPC ), 2*ANB )
...
*
     ...
     $                     JZ, DESCZ, 19, 4, IDUM1, IDUM2, INFO )
         ELSE
            IDUM1( 1 ) = ICHAR( 'N' )
            CALL PCHK1MAT( N, 4, N, 4, IA, JA, DESCA, 8, 4, IDUM1,
     $                     IDUM2, INFO )
         END IF
         WORK( 1 ) = CMPLX( LWOPT , KIND=10)
         RWORK( 1 ) = REAL( LRWOPT , KIND=10)
         IWORK( 1 ) = LIWMIN
      END IF
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHEEVR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
***********************************************************************
      ...
      CALL PYHENTRD( UPLO, N, A, IA, JA, DESCA, RWORK( INDD ),          
     +      RWORK( INDE ), WORK( INDTAU ), WORK( INDWORK ),             
     +   LLWORK, RWORK( INDRWORK ), LLRWORK,IINFO )


      IF (IINFO .NE. 0) THEN
         CALL PXERBLA( ICTXT, 'PYHENTRD', -IINFO )
         RETURN
      END IF
...
***********************************************************************
     ...
     $   THEN
         CALL PELARED1D( N, IA, JA, DESCA, RWORK( INDD ),               
     +      RWORK( INDD2 ), RWORK( INDRWORK ), LLRWORK )
*
         CALL PELARED1D( N, IA, JA, DESCA, RWORK( INDE ),               
     +      RWORK( INDE2 ), RWORK( INDRWORK ), LLRWORK )
         IF( .NOT.LOWER )
     $      OFFSET = 1
      ELSE
         DO 10 I = 1, N
            CALL PYELGET( 'A', ' ', WORK( INDWORK ), A,                 
     +     I+IA-1, I+JA-1, DESCA )
            RWORK( INDD2+I-1 ) = REAL( WORK( INDWORK ) , KIND=10)
   10    CONTINUE
         IF( LSAME( UPLO, 'U' ) ) THEN
            DO 20 I = 1, N - 1
               CALL PYELGET( 'A', ' ', WORK( INDWORK ), A,              
     +           I+IA-1, I+JA, DESCA )
               RWORK( INDE2+I-1 ) = REAL( WORK( INDWORK ) , KIND=10)
   20       CONTINUE
         ELSE
            DO 30 I = 1, N - 1
               CALL PYELGET( 'A', ' ', WORK( INDWORK ), A,              
...
***********************************************************************
      ...
      ELSE IF ( INDEIG ) THEN
         ...
         IIU = IU
      ELSE IF ( VALEIG ) THEN
         CALL ELARRC('T', N, VLL, VUU, RWORK( INDD2 ),      RWORK( 
     +INDE2 + OFFSET ), SAFMIN, EIGCNT, IIL, IIU, INFO)
...
*
         ...
         IF ( MYIL.GT.0 ) THEN
            DOL = 1
            DOU = MYIU - MYIL + 1
            CALL ESTEGR2( JOBZ, 'I', N,  RWORK( INDD2 ),                
     +   RWORK( INDE2+OFFSET ), VLL, VUU, MYIL, MYIU,                   
     +IM, W( 1 ), RWORK( INDRW ), N,                    MYIU - MYIL + 1,
     +                   IWORK( 1 ), RWORK( INDRWORK ), SIZE1,          
     +          IWORK( 2*N+1 ), SIZE2,                    DOL, DOU, 
     +ZOFFSET, IINFO )
*           ESTEGR2 zeroes out the entire W array, so we can't just give
...
*           W( X ) is at W(X - IIL + 1 )
         END IF
         IF (IINFO .NE. 0) THEN
            CALL PXERBLA( ICTXT, 'ESTEGR2', -IINFO )
            RETURN
         END IF
...
*
     ...
     +     IWORK( 1 ), RWORK( INDRWORK ), SIZE1,                    
     +IWORK( 2*N+1 ), SIZE2, DOL, DOU,                   ZOFFSET, IINFO 
     +)
         ENDIF
         IF (IINFO .NE. 0) THEN
            CALL PXERBLA( ICTXT, 'ESTEGR2', -IINFO )
...
*        Part 1. compute root representations and root eigenvalues
         IF ( MYIL.GT.0 ) THEN
            DOL = MYIL - IIL + 1
            DOU = MYIU - IIL + 1
            CALL ESTEGR2A( JOBZ, 'I', N,  RWORK( INDD2 ),               
     +    RWORK( INDE2+OFFSET ), VLL, VUU, IIL, IIU,                   
     +IM, W( 1 ), RWORK( INDRW ), N,                    N, RWORK( 
     +INDRWORK ), SIZE1,                    IWORK( 2*N+1 ), SIZE2, DOL, 
     +                   DOU, NEEDIL, NEEDIU,                   INDERR, 
     +NSPLIT, PIVMIN, SCALE, WL, WU,                   IINFO )
         ENDIF
         IF (IINFO .NE. 0) THEN
            CALL PXERBLA( ICTXT, 'ESTEGR2A', -IINFO )
            RETURN
         END IF
*
...
*                    Copy eigenvalues into communication buffer
                     CALL ECOPY(LENGTHI,W( STARTI ),1,                  
     +         RWORK( INDD ), 1)                    
*                    Copy uncertainties into communication buffer
                     CALL ECOPY(LENGTHI,RWORK(IINDERR+STARTI-1),1,      
     +                     RWORK( INDD+LENGTHI ), 1)                    
*                    send buffer
                     CALL EGESD2D( ICTXT, LENGTHI2,                     
     + 1, RWORK( INDD ), LENGTHI2,                     DSTROW, DSTCOL )
                  END IF
...
*              Broadcast eigenvalues and errors to all processors
               CALL ECOPY(LENGTHI,W ,1, RWORK( INDD ), 1)               
     +  
               CALL ECOPY(LENGTHI,RWORK( IINDERR ),1,                   
     +        RWORK( INDD+LENGTHI ), 1)                    
               CALL EGEBS2D( ICTXT, 'A', ' ', LENGTHI2, 1,              
     +  RWORK(INDD), LENGTHI2 )
            ELSE
               SRCROW = 0
               SRCCOL = 0
               CALL EGEBR2D( ICTXT, 'A', ' ', LENGTHI2, 1,              
     +RWORK(INDD), LENGTHI2, SRCROW, SRCCOL )
               CALL ECOPY( LENGTHI, RWORK(INDD), 1, W, 1)
               CALL ECOPY(LENGTHI,RWORK(INDD+LENGTHI),1,                
     +           RWORK( IINDERR ), 1)                   
            END IF
         ELSE
  ...
  46                 CONTINUE
     ...
     $                             SRCROW, SRCCOL )
                     ...
                     IF ((RSTARTI.GE.1 ) .AND. (RLENGTHI.GE.1 )) THEN
                        ...
                        CALL EGERV2D( ICTXT, RLENGTHI2, 1,              
     +         RWORK(INDE), RLENGTHI2,                       SRCROW, 
     +SRCCOL )
*                       copy eigenvalues from communication buffer
                        CALL ECOPY( RLENGTHI,RWORK(INDE), 1,            
     +               W( RSTARTI ), 1)                    
*                       copy uncertainties (errors) from communication buffer
                        CALL ECOPY(RLENGTHI,RWORK(INDE+RLENGTHI),1,     
...
*      
 100     CONTINUE
         IF ( MYIL.GT.0 ) THEN
            CALL ESTEGR2B( JOBZ, N,  RWORK( INDD2 ),                   
     +RWORK( INDE2+OFFSET ),                    IM, W( 1 ), RWORK( 
     +INDRW ), N, N,                   IWORK( 1 ), RWORK( INDRWORK ), 
     +SIZE1,                    IWORK( 2*N+1 ), SIZE2, DOL,             
     +       DOU, NEEDIL, NEEDIU, INDWLC,                   PIVMIN, 
     +SCALE, WL, WU,                   VSTART, FINISH,                  
...
*                          copy uncertainties (errors) from communication buffer
                           CALL ECOPY(RLENGTHI,RWORK(INDE+RLENGTHI),    
 ...
 147              CONTINUE
         ...
         ENDIF
         IF (IINFO .NE. 0) THEN
            CALL PXERBLA( ICTXT, 'ESTEGR2B', -IINFO )
            RETURN
         END IF
*
      ENDIF
...
***********************************************************************
     ...
     $                    DSTROW, DSTCOL )
            IF ((STARTI.GE.1).AND.(LENGTHI.GE.1)) THEN
               CALL EGESD2D( ICTXT, LENGTHI,                1, W( 
     +STARTI ), LENGTHI,               DSTROW, DSTCOL )
            ENDIF
         ELSE IF (MYPROC .EQ. 0) THEN
            SRCROW = (I-1) / NPCOL
            SRCCOL = MOD(I-1, NPCOL)
     ...
     $                    SRCROW, SRCCOL )
            ...
            LENGTHI = IWORK(2)
            IF ((STARTI.GE.1).AND.(LENGTHI.GE.1)) THEN
               CALL EGERV2D( ICTXT, LENGTHI, 1,                  W( 
     +STARTI ), LENGTHI, SRCROW, SRCCOL )
            ENDIF
         ENDIF
   50 CONTINUE
...
*     Broadcast eigenvalues to all processors
      IF (MYPROC .EQ. 0) THEN
*        Send eigenvalues
         CALL EGEBS2D( ICTXT, 'A', ' ', M, 1, W, M )
      ELSE
         SRCROW = 0
         SRCCOL = 0
         CALL EGEBR2D( ICTXT, 'A', ' ', M, 1,            W, M, SRCROW, 
     +SRCCOL )
      END IF
*
...
*
      DO 160 I = 1, M
         IWORK( NPROCS+1+I ) = I
  160 CONTINUE
      CALL ELASRT2( 'I', M, W, IWORK( NPROCS+2 ), IINFO )
      IF (IINFO.NE.0) THEN
         CALL PXERBLA( ICTXT, 'ELASRT2', -IINFO )
         RETURN
      END IF
...
***********************************************************************
  ...
  170    CONTINUE
*        Store NVS in IWORK(1:NPROCS+1) for PYLAEVSWP
         IWORK( 1 ) = 0
         DO 180 I = 1, NPROCS
...
*           Has already been computed by PMPIM2 and stored
  ...
  180    CONTINUE
         ...
         IF ( FIRST ) THEN
            CALL PYLAEVSWP(N, RWORK( INDRW ), N, Z, IZ, JZ,         
     +DESCZ, IWORK( 1 ), IWORK( NPROCS+M+2 ), RWORK( INDRWORK ),        
     + SIZE1 )
         ELSE
            CALL PYLAEVSWP(N, RWORK( INDRW + N ), N, Z, IZ, JZ,         
     +DESCZ, IWORK( 1 ), IWORK( NPROCS+M+2 ), RWORK( INDRWORK ),        
     +SIZE1 )
         END IF
*
...
***********************************************************************
         IF( NZ.GT.0 ) THEN
           CALL PYUNMTR( 'L', UPLO, 'N', N, NZ, A, IA, JA, DESCA,       
     +              WORK( INDTAU ), Z, IZ, JZ, DESCZ,                   
     +  WORK( INDWORK ), LLWORK, IINFO )
         END IF
         IF (IINFO.NE.0) THEN
            CALL PXERBLA( ICTXT, 'PYUNMTR', -IINFO )
            RETURN
         END IF
*
