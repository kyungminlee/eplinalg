      SUBROUTINE PYLARFB( SIDE, TRANS, DIRECT, STOREV, M, N, K, V, IV,  
...
*  =======
*
*  PYLARFB applies a complex block reflector Q or its conjugate
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGEBR2D, YGEBS2D,YGEMM,        
     +            YGSUM2D, YLAMOV, YLASET, YTRBR2D,                    
     +YTRBS2D, YTRMM, INFOG1L, INFOG2L, PB_TOPGET,                    
     +PBYTRAN
*     ..
*     .. Intrinsic Functions ..
...
*
            ...
            IF( MYCOL.EQ.IVCOL ) THEN
               CALL YGEBS2D( ICTXT, 'Rowwise', ROWBTOP, MPC, K,         
     +               V( IOFFV ), LDV )
               IF( MYROW.EQ.IVROW )             CALL YTRBS2D( ICTXT, 
     +'Rowwise', ROWBTOP, UPLO,                           'Non unit', K,
     + K, T, NBV )
               CALL YLAMOV( 'All', MPC, K, V( IOFFV ), LDV, WORK( IPV ),
     +                       LV )
            ELSE
               CALL YGEBR2D( ICTXT, 'Rowwise', ROWBTOP, MPC, K,         
     +               WORK( IPV ), LV, MYROW, IVCOL )
               IF( MYROW.EQ.IVROW )             CALL YTRBR2D( ICTXT, 
     +'Rowwise', ROWBTOP, UPLO,                           'Non unit', K,
     + K, T, NBV, MYROW, IVCOL )
            END IF
*
...
*
   10          CONTINUE
               IF( K-ITOP .GT.0 ) THEN
                  CALL YLASET( 'Upper', IINXT-IIBEG+1, K-ITOP, ZERO,    
...
*
   20          CONTINUE
               IF( JJ.LE.( JJV+K-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL YLASET( 'All', KP, ITOP-JJ+JJV, ZERO, ZERO,      
     +                    WORK( IPV+II-IIV+(JJ-JJV)*LV ), LV )
                  CALL YLASET( 'Lower', KP, HEIGHT, ZERO, ONE,          
...
*
            IF( MPC.GT.0 ) THEN
               CALL YGEMM( 'Conjugate transpose', 'No transpose', NQC,  
     +                   K, MPC, ONE, C( IOFFC ), LDC, WORK( IPV ), LV, 
     +                    ZERO, WORK( IPW ), LW )
            ELSE
               CALL YLASET( 'All', NQC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL YGSUM2D( ICTXT, 'Columnwise', ' ', NQC, K, WORK( IPW ),
     +                     LW, IVROW, MYCOL )
*
...
*
               CALL YTRMM( 'Right', UPLO, TRANST, 'Non unit', NQC, K,   
     +                   ONE, T, NBV, WORK( IPW ), LW )
               CALL YGEBS2D( ICTXT, 'Columnwise', ' ', NQC, K,          
     +              WORK( IPW ), LW )
            ELSE
               CALL YGEBR2D( ICTXT, 'Columnwise', ' ', NQC, K,          
...
*
            CALL YGEMM( 'No transpose', 'Conjugate transpose', MPC, NQC,
...
*
            IF( MYCOL.EQ.IVCOL ) THEN
               IF( MYROW.EQ.IVROW ) THEN
                  CALL YLASET( 'All', IROFFV, K, ZERO, ZERO,            
     +              WORK( IPW ), LW )
                  IPW1 = IPW + IROFFV
                  CALL YLAMOV( 'All', NPV, K, V( IOFFV ), LDV,          
     +                WORK( IPW1 ), LW )
               ELSE
                  IPW1 = IPW
                  CALL YLAMOV( 'All', NPV, K, V( IOFFV ), LDV,          
...
*
   40             CONTINUE
     ...
     +                      WORK( IPW1+II-IIV+ITOP*LW ), LW )
            ...
            END IF
*
            CALL PBYTRAN( ICTXT, 'Columnwise', 'Conjugate transpose',   
...
*
            IF( NQC.GT.0 ) THEN
               CALL YGEMM( 'No transpose', 'Conjugate transpose', MPC,  
     +                    K, NQC, ONE, C( IOFFC ), LDC, WORK( IPV ),    
     +                  LV, ZERO, WORK( IPW ), LW )
            ELSE
               CALL YLASET( 'All', MPC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL YGSUM2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),   
     +                  LW, MYROW, IVCOL )
*
*           WORK( IPW ) = WORK( IPW ) * T' or WORK( IPW ) * T
*
...
*
                  CALL YTRBS2D( ICTXT, 'Columnwise', ' ', UPLO,         
     +                  'Non unit', K, K, T, NBV )
               ELSE
                  CALL YTRBR2D( ICTXT, 'Columnwise', ' ', UPLO,         
     +                  'Non unit', K, K, T, NBV, IVROW, MYCOL )
               END IF
               CALL YTRMM( 'Right', UPLO, TRANS, 'Non unit', MPC, K,    
     +                  ONE, T, NBV, WORK( IPW ), LW )
*
               CALL YGEBS2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     +                        LW )
            ELSE
               CALL YGEBR2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     +                        LW, MYROW, IVCOL )
            END IF
*
*               C            C      -     W       *     V'
...
*                        MPC x NQC    MPC x K         K x NQC
*
            CALL YGEMM( 'No transpose', 'No transpose', MPC, NQC, K,    
...
*
            IF( MYROW.EQ.IVROW ) THEN
               IF( MYCOL.EQ.IVCOL ) THEN
                  CALL YLASET( 'All', K, ICOFFV, ZERO, ZERO,            
     +              WORK( IPW ), LW )
                  IPW1 = IPW + ICOFFV * LW
                  CALL YLAMOV( 'All', K, MQV, V( IOFFV ), LDV,          
     +                WORK( IPW1 ), LW )
               ELSE
                  IPW1 = IPW
                  CALL YLAMOV( 'All', K, MQV, V( IOFFV ), LDV,          
...
*
   50             CONTINUE
                  IF( ( K-ILEFT ).GT.0 ) THEN
                     CALL YLASET( 'Lower', K-ILEFT, JJNXT-JJBEG+1, ZERO,
     +                             ONE,                             
...
*
   60             CONTINUE
                  IF( II.LE.( IIV+K-1 ) ) THEN
                     WIDE = IRIGHT - ILEFT
                     CALL YLASET( 'All', ILEFT-II+IIV, KQ, ZERO, ZERO,  
     +                           WORK( IPW1+II-IIV+(JJ-JJV)*LW ), LW )
                     CALL YLASET( 'Upper', WIDE, KQ, ZERO, ONE,         
...
*
                  CALL YTRBS2D( ICTXT, 'Rowwise', ' ', UPLO, 'Non unit',
     +                           K, K, T, MBV )
               ELSE
                  CALL YTRBR2D( ICTXT, 'Rowwise', ' ', UPLO, 'Non unit',
...
*
            ...
            IF( MYROW.EQ.IVROW ) THEN
               CALL YGEBS2D( ICTXT, 'Columnwise', COLBTOP, K, NQC,      
     +                  V( IOFFV ), LDV )
               IF( MYCOL.EQ.IVCOL )             CALL YTRBS2D( ICTXT, 
     +'Columnwise', COLBTOP, UPLO,                           'Non unit',
     + K, K, T, MBV )
               CALL YLAMOV( 'All', K, NQC, V( IOFFV ), LDV, WORK( IPV ),
     +                       LV )
            ELSE
               CALL YGEBR2D( ICTXT, 'Columnwise', COLBTOP, K, NQC,      
