      SUBROUTINE PEELGET( SCOPE, TOP, ALPHA, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*  =======
*
*  PEELGET sets alpha to the distributed matrix entry A( IA, JA ).
...
*
*  ALPHA   (global output) REAL(KIND=10), the scalar alpha.
*
      ...
      IF( LSAME( SCOPE, 'R' ) ) THEN
         IF( MYROW.EQ.IAROW ) THEN
            IF( MYCOL.EQ.IACOL ) THEN
               ...
               CALL EGEBS2D( ICTXT, SCOPE, TOP, 1, 1, A( IOFFA ), 1 )
               ALPHA = A( IOFFA )
            ELSE
               CALL EGEBR2D( ICTXT, SCOPE, TOP, 1, 1, ALPHA, 1,         
     +               IAROW, IACOL )
            END IF
         END IF
