      SUBROUTINE PYLABRD( M, N, NB, A, IA, JA, DESCA, D, E, TAUQ, TAUP, 
...
*  =======
*
*  PYLABRD reduces the first NB rows and columns of a complex general
...
*
*  This is an auxiliary routine called by PYGEBRD.
*
*  Notes
*  =====
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DESCSET, INFOG2L, PYCOPY,      
     +              PYELGET, PYELSET, PYGEMV, PYLACGV,                  
     +  PYLARFG, PYSCAL, PEELSET
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MIN, MOD
*     ..
...
*
            IF( K.GT.1 ) THEN
               CALL PYGEMV( 'No transpose', M-K+1, K-1, -ONE, A, I, JA, 
     +                      DESCA, Y, IY, JY+K-1, DESCY, 1, ONE, A, I,  
     +                     J, DESCA, 1 )
               CALL PYGEMV( 'No transpose', M-K+1, K-1, -ONE, X, IX+K-1,
     +                       JX, DESCX, A, IA, J, DESCA, 1, ONE, A, I, 
     +J,                       DESCA, 1 )
               CALL PYELSET( A, I-1, J, DESCA, ALPHA )
            END IF
*
*           Generate reflection Q(i) to annihilate A(i+1:ia+m-1,j)
*
            CALL PYLARFG( M-K+1, ALPHA, I, J, A, I+1, J, DESCA, 1,      
     +               TAUQ )
            CALL PEELSET( D, 1, J, DESCD, REAL( ALPHA , KIND=10) )
            CALL PYELSET( A, I, J, DESCA, ONE )
*
*           Compute Y(IA+I:IA+N-1,J)
*
            CALL PYGEMV( 'Conjugate transpose', M-K+1, N-K, ONE, A, I,  
     +                  J+1, DESCA, A, I, J, DESCA, 1, ZERO,            
     +        WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PYGEMV( 'Conjugate transpose', M-K+1, K-1, ONE, A, I,  
     +                  JA, DESCA, A, I, J, DESCA, 1, ZERO, WORK, IW,   
     +                 1, DESCW, 1 )
            CALL PYGEMV( 'Conjugate transpose', K-1, N-K, -ONE, Y, IY,  
     +                  JY+K, DESCY, WORK, IW, 1, DESCW, 1, ONE,        
     +            WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PYGEMV( 'Conjugate transpose', M-K+1, K-1, ONE, X,     
     +               IX+K-1, JX, DESCX, A, I, J, DESCA, 1, ZERO,        
     +            WORK, IW, 1, DESCW, 1 )
            CALL PYGEMV( 'Conjugate transpose', K-1, N-K, -ONE, A, IA,  
     +                  J+1, DESCA, WORK, IW, 1, DESCW, 1, ONE,         
     +           WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ) )
*
            CALL PYELGET( 'Rowwise', ' ', TAU, TAUQ, 1, J, DESCTQ )
            CALL PYSCAL( N-K, TAU, WORK( IPY ), 1, JWY, DESCWY,         
     +           DESCWY( M_ ) )
            CALL PYLACGV( N-K, WORK( IPY ), 1, JWY, DESCWY,             
     +        DESCWY( M_ ) )
            CALL PYCOPY( N-K, WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ),
     +                    Y, IY+K-1, JY+K, DESCY, DESCY( M_ ) )
*
*           Update A(i,j+1:ja+n-1)
*
            CALL PYLACGV( N-K, A, I, J+1, DESCA, DESCA( M_ ) )
            CALL PYLACGV( K, A, I, JA, DESCA, DESCA( M_ ) )
            CALL PYGEMV( 'Conjugate transpose', K, N-K, -ONE, Y, IY,    
     +                JY+K, DESCY, A, I, JA, DESCA, DESCA( M_ ), ONE,   
     +                 A, I, J+1, DESCA, DESCA( M_ ) )
            CALL PYLACGV( K, A, I, JA, DESCA, DESCA( M_ ) )
            CALL PYLACGV( K-1, X, IX+K-1, JX, DESCX, DESCX( M_ ) )
            CALL PYGEMV( 'Conjugate transpose', K-1, N-K, -ONE, A, IA,  
     +                  J+1, DESCA, X, IX+K-1, JX, DESCX, DESCX( M_ ),  
     +                  ONE, A, I, J+1, DESCA, DESCA( M_ ) )
            CALL PYLACGV( K-1, X, IX+K-1, JX, DESCX, DESCX( M_ ) )
            CALL PYELSET( A, I, J, DESCA, CMPLX( REAL( ALPHA , KIND=10) 
     +, KIND=10) )
*
*           Generate reflection P(i) to annihilate A(i,j+2:ja+n-1)
*
            CALL PYLARFG( N-K, ALPHA, I, J+1, A, I,                     
     +MIN( J+2, N+JA-1 ), DESCA, DESCA( M_ ), TAUP )
            CALL PEELSET( E, I, 1, DESCE, REAL( ALPHA , KIND=10) )
            CALL PYELSET( A, I, J+1, DESCA, ONE )
*
*           Compute X(I+1:IA+M-1,J)
*
            CALL PYGEMV( 'No transpose', M-K, N-K, ONE, A, I+1, J+1,    
     +                DESCA, A, I, J+1, DESCA, DESCA( M_ ), ZERO, X,    
     +                IX+K, JX+K-1, DESCX, 1 )
            CALL PYGEMV( 'No transpose', K, N-K, ONE, Y, IY, JY+K,      
     +              DESCY, A, I, J+1, DESCA, DESCA( M_ ), ZERO,         
     +           WORK, IW, 1, DESCW, 1 )
            CALL PYGEMV( 'No transpose', M-K, K, -ONE, A, I+1, JA,      
     +              DESCA, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,         
     +           JX+K-1, DESCX, 1 )
            CALL PYGEMV( 'No transpose', K-1, N-K, ONE, A, IA, J+1,     
     +               DESCA, A, I, J+1, DESCA, DESCA( M_ ), ZERO,        
     +            WORK, IW, 1, DESCW, 1 )
            CALL PYGEMV( 'No transpose', M-K, K-1, -ONE, X, IX+K, JX,   
     +                 DESCX, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,      
     +              JX+K-1, DESCX, 1 )
*
            CALL PYELGET( 'Columnwise', ' ', TAU, TAUP, I, 1, DESCTP )
            CALL PYSCAL( M-K, TAU, X, IX+K, JX+K-1, DESCX, 1 )
            CALL PYLACGV( N-K, A, I, J+1, DESCA, DESCA( M_ ) )
   10    CONTINUE
*
      ELSE
...
*           Update A(i,j:ja+n-1)
*
            CALL PYLACGV( N-K+1, A, I, J, DESCA, DESCA( M_ ) )
            IF( K.GT.1 ) THEN
               CALL PYLACGV( K-1, A, I, JA, DESCA, DESCA( M_ ) )
               CALL PYGEMV( 'Conjugate transpose', K-1, N-K+1, -ONE, Y, 
     +                      IY, JY+K-1, DESCY, A, I, JA, DESCA,         
     +              DESCA( M_ ), ONE, A, I, J, DESCA,                   
     +    DESCA( M_ ) )
               CALL PYLACGV( K-1, A, I, JA, DESCA, DESCA( M_ ) )
               CALL PYLACGV( K-1, X, IX+K-1, JX, DESCX, DESCX( M_ ) )
               CALL PYGEMV( 'Conjugate transpose', K-1, N-K+1, -ONE, A, 
     +                      IA, J, DESCA, X, IX+K-1, JX, DESCX,         
     +              DESCX( M_ ), ONE, A, I, J, DESCA,                   
     +    DESCA( M_ ) )
               CALL PYLACGV( K-1, X, IX+K-1, JX, DESCX, DESCX( M_ ) )
               CALL PYELSET( A, I, J-1, DESCA, CMPLX( REAL( ALPHA , 
     +KIND=10) , KIND=10) )
            END IF
*
*           Generate reflection P(i) to annihilate A(i,j+1:ja+n-1)
*
            CALL PYLARFG( N-K+1, ALPHA, I, J, A, I, J+1, DESCA,         
     +            DESCA( M_ ), TAUP )
            CALL PEELSET( D, I, 1, DESCD, REAL( ALPHA , KIND=10) )
            CALL PYELSET( A, I, J, DESCA, ONE )
*
*           Compute X(i+1:ia+m-1,j)
*
            CALL PYGEMV( 'No transpose', M-K, N-K+1, ONE, A, I+1, J,    
     +                DESCA, A, I, J, DESCA, DESCA( M_ ), ZERO, X,      
     +              IX+K, JX+K-1, DESCX, 1 )
            CALL PYGEMV( 'No transpose', K-1, N-K+1, ONE, Y, IY, JY+K-1,
     +                    DESCY, A, I, J, DESCA, DESCA( M_ ), ZERO,     
     +               WORK, IW, 1, DESCW, 1 )
            CALL PYGEMV( 'No transpose', M-K, K-1, -ONE, A, I+1, JA,    
     +                DESCA, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,       
     +             JX+K-1, DESCX, 1 )
            CALL PYGEMV( 'No transpose', K-1, N-K+1, ONE, A, IA, J,     
...
*
     ...
     +                 DESCA, 1 )
            CALL PYGEMV( 'No transpose', M-K, K, -ONE, X, IX+K, JX,     
     +               DESCX, A, IA, J, DESCA, 1, ONE, A, I+1, J,         
     +           DESCA, 1 )
            CALL PYELSET( A, I, J, DESCA, ALPHA )
*
*           Generate reflection Q(i) to annihilate A(i+2:ia+m-1,j)
*
            CALL PYLARFG( M-K, ALPHA, I+1, J, A, MIN( I+2, M+IA-1 ),    
     +                 J, DESCA, 1, TAUQ )
            CALL PEELSET( E, 1, J, DESCE, REAL( ALPHA , KIND=10) )
            CALL PYELSET( A, I+1, J, DESCA, ONE )
*
*           Compute Y(ia+i:ia+n-1,j)
*
            CALL PYGEMV( 'Conjugate transpose', M-K, N-K, ONE, A, I+1,  
     +                  J+1, DESCA, A, I+1, J, DESCA, 1, ZERO,          
     +          WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PYGEMV( 'Conjugate transpose', M-K, K-1, ONE, A, I+1,  
     +                  JA, DESCA, A, I+1, J, DESCA, 1, ZERO, WORK, IW, 
     +                   1, DESCW, 1 )
            CALL PYGEMV( 'Conjugate transpose', K-1, N-K, -ONE, Y, IY,  
     +                  JY+K, DESCY, WORK, IW, 1, DESCW, 1, ONE,        
     +            WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PYGEMV( 'Conjugate transpose', M-K, K, ONE, X, IX+K,   
     +                 JX, DESCX, A, I+1, J, DESCA, 1, ZERO, WORK, IW,  
     +                  1, DESCW, 1 )
            CALL PYGEMV( 'Conjugate transpose', K, N-K, -ONE, A, IA,    
     +                J+1, DESCA, WORK, IW, 1, DESCW, 1, ONE,           
     +         WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ) )
*
            CALL PYELGET( 'Rowwise', ' ', TAU, TAUQ, 1, J, DESCTQ )
