      SUBROUTINE PYHETD2( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,   
     +                  LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYHETD2 reduces a complex Hermitian matrix sub( A ) to Hermitian
...
*     .. Local Scalars ..
     ...
     $                   NPROW
      COMPLEX(KIND=10)            ALPHA, TAUI, DOTC
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, YAXPY, YYDOTC,    
     +                YGEBR2D, YGEBS2D, CHK1MAT, YHEMV,                 
     +   YHER2, YLARFG, INFOG2L, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHETD2', -INFO )
...
*
                  ALPHA = A( IK+JK*LDA )
                  CALL YLARFG( J, ALPHA, A( II+JK*LDA ), 1, TAUI )
                  E( JK+1 ) = REAL( ALPHA , KIND=10)
*
...
*
                     CALL YHEMV( UPLO, J, TAUI, A( II+(JJ-1)*LDA ),     
     +                       LDA, A( II+JK*LDA ), 1, ZERO,              
     +              TAU( JJ ), 1 )
*
*                    Compute  w := x - 1/2 * tau * (x'*v) * v
*
                     CALL YYDOTC( J, DOTC, TAU( JJ ), 1, A( II+JK*LDA ),
     +                            1 )
                     ALPHA = -HALF*TAUI*DOTC
                     CALL YAXPY( J, ALPHA, A( II+JK*LDA ), 1,           
     +                 TAU( JJ ), 1 )
*
*                    Apply the transformation as a rank-2 update:
*                       A := A - v * w' - w * v'
*
                     CALL YHER2( UPLO, J, -ONE, A( II+JK*LDA ), 1,      
...
*
                  A( IK+JK*LDA ) = CMPLX( E( JK+1 ) , KIND=10)
                  D( JK+1 ) = REAL( A( IK+1+JK*LDA ) , KIND=10)
                  WORK( J+1 ) = CMPLX( D( JK+1 ) , KIND=10)
                  WORK( N+J+1 ) = CMPLX( E( JK+1 ) , KIND=10)
                  TAU( JK+1 ) = TAUI
                  WORK( 2*N+J+1 ) = TAU( JK+1 )
*
   10          CONTINUE
               D( JJ ) = REAL( A( II+(JJ-1)*LDA ) , KIND=10)
               WORK( 1 ) = CMPLX( D( JJ ) , KIND=10)
               WORK( N+1 ) = ZERO
               WORK( 2*N+1 ) = ZERO
*
               CALL YGEBS2D( ICTXT, 'Columnwise', ' ', 1, 3*N, WORK, 1 )
*
            ELSE
               CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 3*N, WORK, 1, 
     +                       IAROW, IACOL )
               DO 20 J = 2, N
                  JN = JJ + J - 1
                  D( JN ) = REAL( WORK( J ) , KIND=10)
                  E( JN ) = REAL( WORK( N+J ) , KIND=10)
                  TAU( JN ) = WORK( 2*N+J )
   20          CONTINUE
               D( JJ ) = REAL( WORK( 1 ) , KIND=10)
            END IF
         END IF
*
      ELSE
...
*
               A( II+(JJ-1)*LDA ) = REAL( A( II+(JJ-1)*LDA ) , KIND=10)
               DO 30 J = 1, N - 1
                  IK = II + J - 1
                  JK = JJ + J - 1
*
...
*
                  ...
                  CALL YLARFG( N-J, ALPHA, A( IK+2+(JK-1)*LDA ), 1,     
     +                     TAUI )
                  E( JK ) = REAL( ALPHA , KIND=10)
*
                  IF( TAUI.NE.ZERO ) THEN
*
*                    Apply H(i) from both sides to
...
*
                     CALL YHEMV( UPLO, N-J, TAUI, A( IK+1+JK*LDA ),     
     +                       LDA, A( IK+1+(JK-1)*LDA ), 1,              
     +              ZERO, TAU( JK ), 1 )
*
*                    Compute  w := x - 1/2 * tau * (x'*v) * v
*
                     CALL YYDOTC( N-J, DOTC, TAU( JK ), 1,              
     +               A( IK+1+(JK-1)*LDA ), 1 )
                     ALPHA = -HALF*TAUI*DOTC
                     CALL YAXPY( N-J, ALPHA, A( IK+1+(JK-1)*LDA ),      
     +                      1, TAU( JK ), 1 )
*
*                    Apply the transformation as a rank-2 update:
*                       A := A - v * w' - w * v'
*
                     CALL YHER2( UPLO, N-J, -ONE,                       
     +     A( IK+1+(JK-1)*LDA ), 1,                            TAU( JK 
     +), 1, A( IK+1+JK*LDA ),                            LDA )
                  END IF
*
...
*                 columnwise.
*
                  A( IK+1+(JK-1)*LDA ) = CMPLX( E( JK ) , KIND=10)
                  D( JK ) = REAL( A( IK+(JK-1)*LDA ) , KIND=10)
                  WORK( J ) = CMPLX( D( JK ) , KIND=10)
                  WORK( N+J ) = CMPLX( E( JK ) , KIND=10)
                  TAU( JK ) = TAUI
                  WORK( 2*N+J ) = TAU( JK )
   30          CONTINUE
               JN = JJ + N - 1
               D( JN ) = REAL( A( II+N-1+(JN-1)*LDA ) , KIND=10)
               WORK( N ) = CMPLX( D( JN ) , KIND=10)
               TAU( JN ) = ZERO
               WORK( 2*N ) = ZERO
*
               CALL YGEBS2D( ICTXT, 'Columnwise', ' ', 1, 3*N-1, WORK,  
     +                           1 )
*
            ELSE
               CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 3*N-1, WORK,  
     +                      1, IAROW, IACOL )
               DO 40 J = 1, N - 1
                  JN = JJ + J - 1
                  D( JN ) = REAL( WORK( J ) , KIND=10)
