      SUBROUTINE PYLAREAD( FILNAM, A, DESCA, IRREAD, ICREAD, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLAREAD reads from a file named FILNAM a matrix and distribute
...
*     .. Local Scalars ..
     ...
     $                   JJ, K, LDA, M, MYCOL, MYROW, N, NPCOL, NPROW
      REAL(KIND=10)               REAL_PART, IMAG_PART
*    ..
*     .. Local Arrays ..
      INTEGER            IWORK( 2 )
*     ..
...
*
      IF( M.GT.DESCA( M_ ).OR. N.GT.DESCA( N_ ) ) THEN
         IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
            WRITE( *, FMT = * ) 'PYLAREAD: Matrix too big to fit in'
            WRITE( *, FMT = * ) 'Abort ...'
         END IF
   ...
   10                CONTINUE
               ...
               ELSE
                  IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
                     CALL YGERV2D( ICTXT, IB, 1, A( II+(JJ+H-1)*LDA ),  
     +                            LDA, IRREAD, ICREAD )
                   ELSE IF( MYROW.EQ.IRREAD .AND. MYCOL.EQ.ICREAD ) THEN
                     DO 20 K = 1, IB
                        READ( NIN, FMT = * ) REAL_PART, IMAG_PART
                        WORK(K)=CMPLX(REAL_PART,IMAG_PART, KIND=10)
   20                CONTINUE
                     CALL YGESD2D( ICTXT, IB, 1, WORK, DESCA( MB_ ),    
     +                          ICURROW, ICURCOL )
                  END IF
               END IF
               IF( MYROW.EQ.ICURROW )
