      REAL(KIND=10)             FUNCTION PYLANSY( NORM, UPLO, N, A, IA, 
     +JA,                                    DESCA, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLANSY  returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PYLANSY as described
*          above.
*
...
*          sub( A ). When N = 0, PYLANSY is set to zero. N >= 0.
*
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 400 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  COLSSQ(1) = ZERO
                  COLSSQ(2) = ONE
                  CALL YLASSQ( IIA+NP-II, A( II+K ), 1,                 
     +         COLSSQ(1), COLSSQ(2) )
                  IF( MYROW.EQ.IAROW )
     $               II = II + 1
                  CALL YLASSQ( IIA+NP-II, A( II+K ), 1,                 
