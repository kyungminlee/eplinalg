      SUBROUTINE PYGESVX( FACT, TRANS, N, NRHS, A, IA, JA, DESCA, AF,   
...
*  =======
*
*  PYGESVX uses the LU factorization to compute the solution to a
...
*          A(IA:IA+N-1,JA:JA+N-1) = P*L*U as computed by PYGETRF.
...
*          PYGETRF; IPIV(i) -> The global row local row i was
...
*          LWORK = MAX( PYGECON( LWORK ), PYGERFS( LWORK ) )
*                  + LOCr( N_A ).
*
...
*     .. Local Scalars ..
     ...
     $                   NQ, NQB, NRHSQ, RFSWRK, ICOLEQU
      REAL(KIND=10)               AMAX, ANORM, BIGNUM, COLCND, RCMAX, 
     +RCMIN,                    ROWCND, SMLNUM
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
     ...
     +                INFOG2L, PYGECON, PYGEEQU, PYGERFS,               
     +     PYGETRF, PYGETRS, PYLACPY,                    PYLAQGE, 
     +PECOPY, PXERBLA, EGEBR2D,                    EGEBS2D, EGAMN2D, 
     +EGAMX2D
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      ...
      EXTERNAL           ICEIL, ILCM, INDXG2P, LSAME, NUMROC, PYLANGE,  
     +                  PELAMCH
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGESVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PYGEEQU( N, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND,    
     +              AMAX, INFEQU )
         IF( INFEQU.EQ.0 ) THEN
*
*           Equilibrate the matrix.
*
            CALL PYLAQGE( N, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND, 
     +                    AMAX, EQUED )
            ROWEQU = LSAME( EQUED, 'R' ) .OR. LSAME( EQUED, 'B' )
            COLEQU = LSAME( EQUED, 'C' ) .OR. LSAME( EQUED, 'B' )
            IF( COLEQU ) ICOLEQU = 1
...
*
     ...
     $                 IACOL, ICTXT, 1 )
         CALL PECOPY( N, C, 1, JA, CDESC, CDESC( LLD_ ), RWORK, IB, JB, 
     +                DESCB, 1 )
         IF( MYCOL.EQ.IBCOL ) THEN
            CALL EGEBS2D( ICTXT, 'Rowwise', ' ', NP, 1, RWORK( IIB ),   
     +                  DESCB( LLD_ ) )
         ELSE
            CALL EGEBR2D( ICTXT, 'Rowwise', ' ', NP, 1, RWORK( IIB ),   
...
*
     ...
     +              DESCAF )
         CALL PYGETRF( N, N, AF, IAF, JAF, DESCAF, IPIV, INFO )
*
...
*
      ...
      ELSE
         NORM = 'I'
      END IF
      ANORM = PYLANGE( NORM, N, N, A, IA, JA, DESCA, RWORK )
*
*     Compute the reciprocal of the condition number of A.
*
      CALL PYGECON( NORM, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,  
...
*
     ...
     +        DESCX )
      CALL PYGETRS( TRANS, N, NRHS, AF, IAF, JAF, DESCAF, IPIV, X, IX,  
     +             JX, DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PYGERFS( TRANS, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,     
...
*
            CALL DESCSET( CDESC, 1, N+ICOFFA, 1, DESCA( NB_ ), MYROW,
     $                    IACOL, ICTXT, 1 )
            CALL PECOPY( N, C, 1, JA, CDESC, CDESC( LLD_ ), RWORK, IX,  
     +                  JX, DESCX, 1 )
            IF( MYCOL.EQ.IBCOL ) THEN
