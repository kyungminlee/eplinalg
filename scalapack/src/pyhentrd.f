      SUBROUTINE PYHENTRD( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,  
...
*
*
*  Support for UPLO='U' is limited to calling the old, slow, PYHETRD
*  code.
*
*
*  Purpose
...
*  =======
*
*  PYHENTRD is a prototype version of PYHETRD which uses tailored
*  codes (either the serial, YHETRD, or the parallel code, PYHETTRD)
*  when the workspace provided by the user is adequate.
*
*
*  PYHENTRD reduces a complex Hermitian matrix sub( A ) to Hermitian
...
*  ========
*
*  PYHENTRD is faster than PYHETRD on almost all matrices,
...
*  by PYHETRD to keep the interface simple.  These restrictions are
...
*
*
*  RWORK   (local workspace/local output) COMPLEX array,
*                                                  dimension (LRWORK)
...
*     .. External Subroutines ..
     ...
     +               BLACS_GRIDINIT, YHETRD, CHK1MAT, DESCSET,          
     +          IGAMN2D, PYELSET, PYHER2K, PYHETD2, PYHETTRD,           
     +         PCHK1MAT, PYLAMR1D, PYLATRD, PYTRMR2D,                   
     + PELAMR1D, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ICHAR, INT, MAX, MIN, MOD, SQRT
*     ..
...
*
     ...
     $           NPCOL ) )
            ...
            ANB = PJLAENV_EP( ICTXT, 3, 'PYHETTRD', 'L', 0, 0, 0, 0 )
            MINSZ = PJLAENV_EP( ICTXT, 5, 'PYHETTRD', 'L', 0, 0, 0, 0 )
            ...
            TTLRWMIN = 2*NPS
*
            WORK( 1 ) = CMPLX( REAL( TTLWMIN , KIND=10) , KIND=10)
            RWORK( 1 ) = REAL( TTLRWMIN , KIND=10)
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHENTRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
*
*     Use the serial, LAPACK, code:  CTRD on small matrices if we
...
*
         CALL PYTRMR2D( UPLO, 'N', N, N, A, IA, JA, DESCA, WORK( INDB ),
     +                   1, 1, DESCB, ICTXT )
*
*
...
*
            IF( NPROWB.EQ.1 ) THEN
               CALL YHETRD( UPLO, N, WORK( INDB ), NPS, RWORK( INDRD ), 
     +                      RWORK( INDRE ), WORK( INDTAU ),             
     +          WORK( INDW ), LLWORK, INFO )
            ELSE
*
               CALL PYHETTRD( 'L', N, WORK( INDB ), 1, 1, DESCB,        
...
*           way that PYHENTRD expects it.
*
         CALL PELAMR1D( N-1, RWORK( INDRE ), 1, 1, DESCB, E, 1, JA,     
     +              DESCA )
*
         CALL PELAMR1D( N, RWORK( INDRD ), 1, 1, DESCB, D, 1, JA,       
     +            DESCA )
*
         CALL PYLAMR1D( N, WORK( INDTAU ), 1, 1, DESCB, TAU, 1, JA,     
     +              DESCA )
*
         CALL PYTRMR2D( UPLO, 'N', N, N, WORK( INDB ), 1, 1, DESCB, A,  
     +                 IA, JA, DESCA, ICTXT )
*
         IF( MYROWB.GE.0 )
...
*           the matrix
*
               CALL PYLATRD( UPLO, K+JB-1, JB, A, IA, JA, DESCA, D, E,  
...
*
               CALL PYHER2K( UPLO, 'No transpose', K-1, JB, -CONE, A,   
...
*
               ...
               CALL PYELSET( A, I-1, J, DESCA, CMPLX( E( JX ) , 
     +KIND=10) )
*
...
*
            CALL PYHETD2( UPLO, MIN( N, NB ), A, IA, JA, DESCA, D, E,   
...
*           of the matrix
*
               CALL PYLATRD( UPLO, N-K+1, NB, A, I, J, DESCA, D, E, TAU,
...
*
               CALL PYHER2K( UPLO, 'No transpose', N-K-NB+1, NB, -CONE, 
...
*
               ...
               CALL PYELSET( A, I+NB, J+NB-1, DESCA, CMPLX( E( JX ) , 
     +KIND=10) )
*
...
*        Use unblocked code to reduce the last or only block
*
            CALL PYHETD2( UPLO, KK, A, IA+K-1, JA+K-1, DESCA, D, E, TAU,
     +                     WORK, LWORK, IINFO )
         END IF
*
