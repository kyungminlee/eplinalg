      SUBROUTINE PYLARFT( DIRECT, STOREV, N, K, V, IV, JV, DESCV, TAU,  
     +                   T, WORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCV( * )
      COMPLEX(KIND=10)            TAU( * ), T( * ), V( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYLARFT forms the triangular factor T of a complex block reflector H
...
*
*  T       (local output) COMPLEX array, dimension (NB_V,NB_V)
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YCOPY, YGEMV, YGSUM2D,         
     +           YLACGV, YLASET, YTRMV, INFOG2L
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
               ...
               IF( NP-II+IIV.GT.0 ) THEN
                  CALL YGEMV( 'Conjugate transpose', NP-II+IIV, ITMP0,  
     +                       -TAU( JJ ), V( II+(JJV-1)*LDV ), LDV,      
     +                   V( II+(JJ-1)*LDV ), 1, ZERO,                   
     +      WORK( IW ), 1 )
               ELSE
                  CALL YLASET( 'All', ITMP0, 1, ZERO, ZERO, WORK( IW ), 
     +                         ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
   10       CONTINUE
*
            CALL YGSUM2D( ICTXT, 'Columnwise', ' ', IW-1, 1, WORK, IW-1,
     +                     IVROW, MYCOL )
*
            IF( MYROW.EQ.IVROW ) THEN
*
...
*
                  ...
                  CALL YCOPY( ITMP0, WORK( IW ), 1, T( ITMP1 ), 1 )
                  IW = IW + ITMP0
*
...
*
               ...
               IF( II-IIV+1.GT.0 ) THEN
                  CALL YGEMV( 'Conjugate transpose', II-IIV+1, ITMP0,   
     +                      -TAU( JJ ), V( IIV+JJ*LDV ), LDV,           
...
*
                  ...
                  IW = IW + ITMP0
*
                  CALL YTRMV( 'Lower', 'No transpose', 'Non-unit',      
...
*
               ...
               IF( NQ-JJ+JJV.GT.0 ) THEN
                  CALL YLACGV( NQ-JJ+JJV, V( II+(JJ-1)*LDV ), LDV )
                  CALL YGEMV( 'No transpose', ITMP0, NQ-JJ+JJV,         
     +                -TAU(II), V( IIV+(JJ-1)*LDV ), LDV,               
     +          V( II+(JJ-1)*LDV ), LDV, ZERO,                         
     +WORK( IW ), 1 )
                  CALL YLACGV( NQ-JJ+JJV, V( II+(JJ-1)*LDV ), LDV )
               ELSE
                  CALL YLASET( 'All', ITMP0, 1, ZERO, ZERO,             
     +             WORK( IW ), ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
   50       CONTINUE
*
            CALL YGSUM2D( ICTXT, 'Rowwise', ' ', IW-1, 1, WORK, IW-1,   
     +                  MYROW, IVCOL )
*
            IF( MYCOL.EQ.IVCOL ) THEN
*
...
*
               ...
               IF( JJ-JJV+1.GT.0 ) THEN
                  CALL YLACGV( JJ-JJV+1, V( II+(JJV-1)*LDV ), LDV )
                  CALL YGEMV( 'No transpose', ITMP0, JJ-JJV+1,          
