      SUBROUTINE PEDTTRF( N, DL, D, DU, JA, DESCA, AF, LAF, WORK, LWORK,
     +                     INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEDTTRF computes a LU factorization
...
*  subsequent calls to PEDTTRS to solve linear systems.
*
...
*
*  DL      (local input/local output) REAL(KIND=10) pointer to local
...
*
*  D       (local input/local output) REAL(KIND=10) pointer to local
...
*          Must be of size >= DESCA( NB_ ).
*
*  DU       (local input/local output) REAL(KIND=10) pointer to local
...
*          PEDTTRF and this is stored in AF. If a linear system
*          is to be solved using PEDTTRS after the factorization
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO, EDTTRF,        
     +            EDTTRSV, DESC_CONVERT, EGERV2D, EGESD2D,              
     +      ETRRV2D, ETRSD2D, GLOBCHK, IGAMX2D, IGEBR2D,                
     +    IGEBS2D, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      REAL(KIND=10)   EDOT
      EXTERNAL           NUMROC, EDOT
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD
*     ..
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,
     + 'PEDTTRF, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT, 'PEDTTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PEDTTRF: auxiliary storage error ',      
     +            -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -10
            CALL PXERBLA( ICTXT, 'PEDTTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEDTTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL ETRSD2D( ICTXT, 'U', 'N', 1, 1,                  DU( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDA-1, 0,                  MYCOL+1 )
*
      END IF
*
*
...
*
      CALL EDTTRF( ODD_SIZE, DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),   
...
*
*
         DL( PART_OFFSET+ODD_SIZE+1 ) = ( DL( PART_OFFSET+ODD_SIZE+1 ) )
...
*
            CALL EDTTRSV( 'L', 'N', ODD_SIZE, INT_ONE,                  
...
*
            CALL ETRRV2D( ICTXT, 'U', 'N', 1, 1, AF( 1 ), ODD_SIZE, 0,  
     +                   MYCOL-1 )
*
            CALL EDTTRSV( 'U', 'T', ODD_SIZE, INT_ONE,                  
     +   DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),                     
     +DU( PART_OFFSET+1 ), AF( 1 ), ODD_SIZE, INFO )
*
*
*         Calculate the update block for previous proc, E_i = GL_i{GU_i}
*
            AF( ODD_SIZE+3 ) = -ONE*EDOT( ODD_SIZE, AF( 1 ), 1,         
     +                 AF( WORK_U+1 ), 1 )
*
*
...
*
            CALL EGESD2D( ICTXT, INT_ONE, INT_ONE, AF( ODD_SIZE+3 ),    
     +                 INT_ONE, 0, MYCOL-1 )
*
*
            IF( MYCOL.LT.NP-1 ) THEN
...
*
               AF( ODD_SIZE+1 ) = -ONE*( DL( PART_OFFSET+ODD_SIZE+1 )*
     $                            AF( WORK_U+ODD_SIZE ) )
*
*
               AF( WORK_U+( ODD_SIZE )+1 ) = -ONE*
     $            DU( PART_OFFSET+ODD_SIZE )*( AF( ODD_SIZE ) )
*
            END IF
*
         END IF
...
*
         CALL EGESD2D( ICTXT, INT_ONE, INT_ONE, AF( ODD_SIZE+1 ),       
     +           INT_ONE, 0, MYCOL-1 )
*
         CALL EGESD2D( ICTXT, INT_ONE, INT_ONE, AF( WORK_U+ODD_SIZE+1 ),
     +                  INT_ONE, 0, MYCOL-1 )
*
      END IF
*
...
*         operations.
*
      AF( ODD_SIZE+2 ) = REAL( D( PART_OFFSET+ODD_SIZE+1 ) , KIND=10)
*
*       Receive cont. to diagonal block that is stored on this proc.
*
      IF( MYCOL.LT.NPCOL-1 ) THEN
*
         CALL EGERV2D( ICTXT, INT_ONE, INT_ONE, AF( ODD_SIZE+2+1 ),     
     +             INT_ONE, 0, MYCOL+1 )
*
...
*
      IF( MYCOL-LEVEL_DIST.GE.0 ) THEN
         CALL EGERV2D( ICTXT, INT_ONE, INT_ONE, WORK( 1 ), INT_ONE, 0,  
     +                MYCOL-LEVEL_DIST )
*
...
*
         CALL EGERV2D( ICTXT, INT_ONE, INT_ONE, AF( ODD_SIZE+1 ),       
     +           INT_ONE, 0, COMM_PROC )
*
         CALL EGERV2D( ICTXT, INT_ONE, INT_ONE, AF( WORK_U+ODD_SIZE+1 ),
     +                  INT_ONE, 0, COMM_PROC )
*
         IF( INFO.EQ.0 ) THEN
*
...
*
         WORK( 1 ) = -ONE*( AF( ODD_SIZE+1 ) )*
     $               AF( WORK_U+( ODD_SIZE )+1 )
*
*         Send contribution to diagonal block's owning processor.
*
         CALL EGESD2D( ICTXT, INT_ONE, INT_ONE, WORK( 1 ), INT_ONE, 0,  
     +                MYCOL+LEVEL_DIST )
*
      END IF
...
*           left
*
            CALL EGERV2D( ICTXT, INT_ONE, INT_ONE,                     
     +AF( WORK_U+ODD_SIZE+2+1 ), INT_ONE, 0,                     
     +MYCOL-LEVEL_DIST / 2 )
*
*           Receive offdiagonal block(s) from proc level_dist/2 to the
*           left
...
*
         WORK( 1 ) = -ONE*AF( ODD_SIZE+3 )*( AF( WORK_U+ODD_SIZE+3 ) )
*
*         Send contribution to diagonal block's owning processor.
*
         CALL EGESD2D( ICTXT, INT_ONE, INT_ONE, WORK( 1 ), INT_ONE, 0,  
...
*
            WORK( 1 ) = -ONE*AF( WORK_U+ODD_SIZE+3 )*AF( ODD_SIZE+1 )
*
*           Send contribution to offdiagonal block's owning processor.
*
            CALL EGESD2D( ICTXT, INT_ONE, INT_ONE, WORK( 1 ), INT_ONE,  
     +                   0, COMM_PROC )
*
            WORK( 1 ) = -ONE*AF( ODD_SIZE+3 )*AF( WORK_U+ODD_SIZE+1 )
*
*           Send contribution to offdiagonal block's owning processor.
*
            CALL EGESD2D( ICTXT, INT_ONE, INT_ONE, WORK( 1 ), INT_ONE,  
