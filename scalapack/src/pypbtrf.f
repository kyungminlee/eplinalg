      SUBROUTINE PYPBTRF( UPLO, N, BW, A, JA, DESCA, AF, LAF, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYPBTRF computes a Cholesky factorization
...
*  subsequent calls to PYPBTRS to solve linear systems.
*
...
*          PYPBTRF and this is stored in AF. If a linear system
*          is to be solved using PYPBTRS after the factorization
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,      
     + 'PYPBTRF, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*BW )) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT,       'PYPBTRF, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT,       'PYPBTRF: auxiliary storage error ',
     +       -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
           INFO = -10
           CALL PXERBLA( ICTXT,       'PYPBTRF: worksize error ',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPBTRF', -INFO )
         RETURN
      END IF
*
...
*
     ...
     +    NEXT_TRI_SIZE_N, A( OFST+ODD_SIZE*LLDA+(BW+1) ),              
     +     LLDA-1, 0, MYCOL+1 )
*
        ENDIF
*
...
*
        CALL YPBTRF( UPLO, ODD_SIZE, BW, A( OFST + 1),                  
     +   LLDA, INFO )
*
        IF( INFO.NE.0 ) THEN
           INFO = MYCOL+1
...
*
          CALL YLATCPY( 'U', BW, BW,                   A(( 
     +OFST+(BW+1)+(ODD_SIZE-BW)*LLDA )), LLDA-1,                   AF( 
     +ODD_SIZE*BW+2*MBW2+1+BW-BW ), BW )
*
*         Perform the triangular system solve {L_i}{{B'}_i}^C = {B_i}^C
*
          CALL YTRTRS( 'L', 'N', 'N', BW, BW,                  A( 
     +OFST+1+(ODD_SIZE-BW)*LLDA ), LLDA-1,                  AF( 
     +ODD_SIZE*BW+2*MBW2+1 ), BW, INFO )
*
*
*         conjugate transpose resulting block to its location
*           in main storage.
*
          CALL YLATCPY( 'L', BW, BW, AF( ODD_SIZE*BW+2*MBW2+1+BW-BW ),  
     +                 BW, A(( OFST+(BW+1)+(ODD_SIZE-BW)*LLDA )),       
     +            LLDA-1 )
*
*
...
*
*
          CALL YHERK( UPLO, 'C', BW, BW, -ONE,                 AF( 
     +ODD_SIZE*BW+2*MBW2+1 ), BW, ONE,                 A( 
     +OFST+1+ODD_SIZE*LLDA ), LLDA-1 )
*
        ENDIF
...
*
          CALL YTBTRS( 'L', 'N', 'N', ODD_SIZE, BW, BW, A( OFST + 1 ),  
     +                LLDA, AF( 1 ), ODD_SIZE, INFO )
*
*
*         Calculate the update block for previous proc, E_i = G_i{G_i}^C
*
          CALL YHERK( 'L', 'C', BW, ODD_SIZE,                      -ONE,
     + AF( 1 ), ODD_SIZE, ZERO,                      AF( 1 + 
     +(ODD_SIZE+2*BW)*BW), BW )
*
*
...
*
          CALL YGESD2D( ICTXT, BW, BW, AF( ODD_SIZE*BW+2*MBW2+1 ), BW,  
     +                 0, MYCOL-1 )
*
*
          IF ( MYCOL .LT. NP-1 ) THEN
...
*             H_i^C to H_i.
*
            CALL YLATCPY( 'N', BW, BW,                AF( ODD_SIZE-BW+1 
     +), ODD_SIZE,                AF( (ODD_SIZE)*BW+1), BW  )
*
            CALL YTRMM( 'R', 'U', 'C', 'N', BW, BW, -CONE,              
...
*
          CALL YGESD2D( ICTXT, BW, BW,                        AF( 
     +ODD_SIZE*BW+1 ),                        BW, 0, MYCOL-1 )
*
        ENDIF
*
...
*         operations.
*
        CALL YLAMOV( 'N', BW, BW,                     A( 
     +OFST+ODD_SIZE*LLDA+1 ),                     LLDA-1, AF( 
     +ODD_SIZE*BW+MBW2+1 ),                     BW )
*
*       Receive cont. to diagonal block that is stored on this proc.
*
        IF( MYCOL.LT. NPCOL-1 ) THEN
*
           CALL YGERV2D( ICTXT, BW, BW,                AF( 
     +ODD_SIZE*BW+2*MBW2+1 ),                BW, 0,                
     +MYCOL+1 )
*
*          Add contribution to diagonal block
*
           CALL YAXPY( MBW2, CONE,                AF( 
     +ODD_SIZE*BW+2*MBW2+1 ),                1, AF( ODD_SIZE*BW+MBW2+1 
     +), 1 )
*
        ENDIF
*
...
*
          IF( MYCOL-LEVEL_DIST .GE. 0 ) THEN
            CALL YGERV2D( ICTXT, BW, BW, WORK( 1 ),                     
     + BW, 0, MYCOL-LEVEL_DIST )
*
            CALL YAXPY( MBW2, CONE, WORK( 1 ), 1,                AF( 
     +ODD_SIZE*BW+MBW2+1 ), 1 )
*
          ENDIF
*
...
*
        CALL YPOTRF( 'L', BW, AF( ODD_SIZE*BW+MBW2+1 ),                 
     +    BW, INFO )
*
        IF( INFO.NE.0 ) THEN
               INFO = NPCOL + MYCOL
...
*             calcs.
*
          CALL YLAMOV( 'N', BW, BW, AF( ODD_SIZE*BW+1 ), BW,            
...
*
          CALL YGERV2D( ICTXT, BW, BW,         AF( ODD_SIZE*BW+1 ),     
     +                   BW, 0, COMM_PROC )
*
          IF( INFO .EQ. 0 ) THEN
*
...
*
*
          CALL YTRSM( 'L', 'L', 'N', 'N', BW, BW, CONE,                 
     +  AF( ODD_SIZE*BW+MBW2+1 ), BW,                   AF( 
     +ODD_SIZE*BW+1 ), BW )
*
          ENDIF
*         End of "if ( info.eq.0 ) then"
...
*
          CALL YHERK( 'L', 'C', BW, BW, -ONE,                   AF( 
     +(ODD_SIZE)*BW+1 ), BW, ZERO,                   WORK( 1 ), BW )
*
*         Send contribution to diagonal block's owning processor.
*
          CALL YGESD2D( ICTXT, BW, BW, WORK( 1 ), BW,                   
     +   0, MYCOL+LEVEL_DIST )
*
        ENDIF
...
*
           CALL YGERV2D( ICTXT, BW, BW,             AF( 
     +ODD_SIZE*BW+2*MBW2+1 ),             BW, 0, MYCOL-LEVEL_DIST/2 )
*
          ENDIF
*
*
...
*
          CALL YTRSM( 'R', 'L', 'C', 'N', BW, BW, CONE,                 
     +AF( ODD_SIZE*BW+MBW2+1 ), BW,                 AF( 
     +ODD_SIZE*BW+2*MBW2+1 ), BW )
*
          ENDIF
*         End of "if( info.eq.0 ) then"
*
...
*
          CALL YHERK( 'L', 'N', BW, BW, -ONE,                   AF( 
     +(ODD_SIZE+2*BW)*BW+1 ), BW, ZERO,                   WORK( 1 ), BW 
     +)
*
*         Send contribution to diagonal block's owning processor.
*
...
*
              CALL YGEMM( 'N', 'N', BW, BW, BW, -CONE,                  
     +   AF( ODD_SIZE*BW+2*MBW2+1 ), BW,                     AF( 
     +ODD_SIZE*BW+1 ), BW, CZERO, WORK( 1 ),                     BW )
*
...
*
*
*       Factor main partition A_i^C = U_i {U_i}^C in each processor
*
        CALL YPBTRF( UPLO, ODD_SIZE, BW, A( OFST + 1),                  
     +   LLDA, INFO )
*
...
*
          CALL YLAMOV( 'L', BW, BW, A( ( OFST+1+ODD_SIZE*LLDA ) ),      
     +            LLDA-1, AF( ODD_SIZE*BW+2*MBW2+1+BW-BW ), BW )
*
*
*         Perform the triangular solve {L_i}{{B'}_i}^C = {B_i}^C
*
          CALL YTRTRS( 'U', 'C', 'N', BW, BW,                  A( 
     +OFST+BW+1+(ODD_SIZE-BW)*LLDA ), LLDA-1,                  AF( 
     +ODD_SIZE*BW+2*MBW2+1 ), BW, INFO )
*
*         Move the resulting block back to its location in main storage.
*
          CALL YLAMOV( 'L', BW, BW, AF( ODD_SIZE*BW+2*MBW2+1+BW-BW ),   
     +               BW, A(( OFST+1+ODD_SIZE*LLDA )), LLDA-1 )
*
*
*         Compute contribution to diagonal block(s) of reduced system.
*          {C'}_i^C = {C_i}^C-{{B'}_i}^C{{B'}_i}
*
*         The following method uses more flops than necessary but
*           does not necessitate the writing of a new BLAS routine.
*
*
          CALL YHERK( UPLO, 'C', BW, BW, -ONE,                 AF( 
     +ODD_SIZE*BW+2*MBW2+1 ), BW, ONE,                 A( 
     +OFST+BW+1+ODD_SIZE*LLDA ), LLDA-1 )
*
        ENDIF
*       End of "if ( MYCOL .lt. NP-1 )..." loop
*
...
*
          CALL YLATCPY( 'L', PREV_TRI_SIZE_N, PREV_TRI_SIZE_M,          
     +         A( OFST+1 ), LLDA-1, AF( 1 ), ODD_SIZE )
*
          IF ( INFO.EQ.0 ) THEN
*
          CALL YTBTRS( 'U', 'C', 'N', ODD_SIZE, BW, BW,                 
...
*
     ...
     +), ODD_SIZE,                AF( (ODD_SIZE)*BW+1), BW  )
*
            CALL YTRMM( 'R', 'L', 'N', 'N', BW, BW, -CONE,              
     +     A( ( OFST+1+ODD_SIZE*LLDA ) ), LLDA-1,                   AF( 
     +(ODD_SIZE)*BW+1 ), BW )
*
          ENDIF
*
...
*         operations.
*
        CALL YLATCPY( 'U', BW, BW,                     A( OFST+ 
     +ODD_SIZE*LLDA+1+BW ),                     LLDA-1, AF( 
     +ODD_SIZE*BW+MBW2+1 ),                     BW )
*
*       Receive cont. to diagonal block that is stored on this proc.
*
...
*
   22   CONTINUE
        IF( MOD( (MYCOL+1)/LEVEL_DIST, 2) .NE. 0 ) GOTO 21
*
*         Receive and add contribution to diagonal block from the left
*
          IF( MYCOL-LEVEL_DIST .GE. 0 ) THEN
