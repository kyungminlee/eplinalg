      SUBROUTINE PYGGQRF( N, M, P, A, IA, JA, DESCA, TAUA, B, IB, JB,   
...
*  =======
*
*  PYGGQRF computes a generalized QR factorization of
...
*  To form Q explicitly, use ScaLAPACK subroutine PYUNGQR.
*  To use Q to update another matrix, use ScaLAPACK subroutine PYUNMQR.
*
...
*  To form Z explicitly, use ScaLAPACK subroutine PYUNGRQ.
*  To use Z to update another matrix, use ScaLAPACK subroutine PYUNMRQ.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PYGEQRF, PYGERQF,     
     +               PCHK2MAT, PYUNMQR, PXERBLA
*     ..
*     .. Local Arrays ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGGQRF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PYGEQRF( N, M, A, IA, JA, DESCA, TAUA, WORK, LWORK, INFO )
      LWMIN = INT( WORK( 1 ) )
*
*     Update sub( B ) := Q'*sub( B ).
*
      CALL PYUNMQR( 'Left', 'Conjugate Transpose', N, P, MIN( N, M ), A,
...
*
      CALL PYGERQF( N, P, B, IB, JB, DESCB, TAUB, WORK, LWORK, INFO )
