      REAL(KIND=10)             FUNCTION PYLANHE( NORM, UPLO, N, A, IA, 
     +JA,                                    DESCA, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLANHE returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PYLANHE as described
*          above.
*
...
*          sub( A ). When N = 0, PYLANHE is set to zero. N >= 0.
*
...
*     .. Local Scalars ..
     ...
     $                   LDA, LL, MYCOL, MYROW, NP, NPCOL, NPROW, NQ
      REAL(KIND=10)               ABSA, SCALE, SUM, VALUE
*     ..
*     .. Local Arrays ..
      REAL(KIND=10)               RWORK( 2 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YLASSQ, PECOL2ROW,             
     +       PETREECOMB, EAXPY, ECOMBSSQ,                    EGAMX2D, 
     +EGSUM2D, EGEBR2D, EGEBS2D
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, IEAMAX, NUMROC
      EXTERNAL           ICEIL, IEAMAX, LSAME, NUMROC
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL EGAMX2D( ICTXT, 'All', ' ', 1, 1, VALUE, 1, I, K, -1,     
     +             IAROW, IACOL )
*
               ...
               DO 220 K = II, II+IB-1
                  ...
                  IF( MYCOL.EQ.IACOL ) THEN
                     IF( JJA+NQ.GT.JJ ) THEN
                        SUM = ABS( REAL( A( K+(JJ-1)*LDA ) , KIND=10) )
               ...
               DO 290 K = 0, IB-1
                  ...
                  IF( MYROW.EQ.IAROW ) THEN
                     IF( IIA+NP.GT.II ) THEN
                        SUM = ABS( REAL( A( IOFFA+II ) , KIND=10) )
...
*
               IF( MYCOL.EQ.ICURCOL ) THEN
                  ...
                  DO 330 K = 0, IB-1
                     ...
                     IF( MYROW.EQ.ICURROW ) THEN
                        IF( IIA+NP.GT.II ) THEN
                           SUM = ABS( REAL( A( II+IOFFA ) , KIND=10) )
...
*
  360       CONTINUE
         END IF
*
*        After calls to EGSUM2D, process row 0 will have global
...
*
         IF( MYCOL.EQ.IACOL )
     $      NQ = NQ + ICOFF
         CALL EGSUM2D( ICTXT, 'Columnwise', ' ', 1, NQ, WORK( ICSR ), 1,
     +                  IAROW, MYCOL )
         IF( MYROW.EQ.IAROW )
     $      NP = NP + IROFF
         CALL EGSUM2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IRSC ),      
     +            MAX( 1, NP ), MYROW, IACOL )
*
         CALL PECOL2ROW( ICTXT, N, 1, DESCA( MB_ ), WORK( IRSC ),       
...
*
         IF( MYROW.EQ.IAROW ) THEN
            IF( MYCOL.EQ.IACOL )
     $         NQ = NQ - ICOFF
            CALL EAXPY( NQ, ONE, WORK( IRSR0 ), 1, WORK( ICSR0 ), 1 )
            IF( NQ.LT.1 ) THEN
               VALUE = ZERO
            ELSE
               VALUE = WORK( IEAMAX( NQ, WORK( ICSR0 ), 1 ) )
            END IF
            CALL EGAMX2D( ICTXT, 'Rowwise', ' ', 1, 1, VALUE, 1, I, K,  
     +                   -1, IAROW, IACOL )
         END IF
*
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 370 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  CALL YLASSQ( II-IIA, A( IIA+K ), 1, SCALE, SUM )
                  CALL YLASSQ( II-IIA, A( IIA+K ), 1, SCALE, SUM )
                  IF( MYROW.EQ.IAROW ) THEN
                     IF( REAL( A( II+K ) , KIND=10).NE.ZERO ) THEN
                        ABSA = ABS( REAL( A( II+K ) , KIND=10) )
            ...
            IF( MYCOL.EQ.IACOL ) THEN
               DO 400 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  IF( MYROW.EQ.IAROW ) THEN
                     ...
                     II = II + 1
                  END IF
                  CALL YLASSQ( IIA+NP-II, A( II+K ), 1, SCALE, SUM )
                  CALL YLASSQ( IIA+NP-II, A( II+K ), 1, SCALE, SUM )
  400          CONTINUE
*
               JJ = JJ + IB
...
*
         ...
         RWORK( 2 ) = SUM
*
         CALL PETREECOMB( ICTXT, 'All', 2, RWORK, IAROW, IACOL,         
     +            ECOMBSSQ )
...
*
      ...
      ELSE
          CALL EGEBR2D( ICTXT, 'All', ' ', 1, 1, VALUE, 1, IAROW,       
     +            IACOL )
      END IF
*
      PYLANHE = VALUE
