      SUBROUTINE PEGERQF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGERQF computes a RQ factorization of a real distributed M-by-N
...
*
*  TAU     (local output) REAL(KIND=10) array, dimension LOCr(IA+M-1)
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK1MAT, PEGERQ2,    
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGERQF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL PEGERQ2( IB, N-M+I+IB-IA, A, I, JA, DESCA, TAU, WORK,  
     +                   LWORK, IINFO )
*
            IF( I.GT.IA ) THEN
*
...
*
               CALL PELARFT( 'Backward', 'Rowwise', N-M+I+IB-IA, IB, A, 
     +                       I, JA, DESCA, TAU, WORK, WORK( IPW ) )
*
*              Apply H to A(ia:i-1,ja:ja+n-m+i+ib-ia-1) from the
*              right
*
               CALL PELARFB( 'Right', 'No transpose', 'Backward',       
...
*
      IF( MU.GT.0 .AND. NU.GT.0 )    CALL PEGERQ2( MU, NU, A, IA, JA, 
     +DESCA, TAU, WORK, LWORK,                  IINFO )
*
