      SUBROUTINE PYLASSQ( N, X, IX, JX, DESCX, INCX, SCALE, SUMSQ )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLASSQ  returns the values  scl  and  smsq  such that
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, PETREECOMB, ECOMBSSQ
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      EXTERNAL           NUMROC
...
*
     ...
     $      NQ = NQ - ICOFF
*
*        Code direct from LAPACK's YLASSQ, (save subroutine call)
*
         IF( NQ.GT.0 ) THEN
            IOFF = IIX + ( JJX - 1 ) * LDX
            DO 10 I = 1, NQ
               IF( REAL( X( IOFF ) , KIND=10).NE.ZERO ) THEN
                  TEMP1 = ABS( REAL( X( IOFF ) , KIND=10) )
                  ...
                  END IF
               END IF
               IF( AIMAG( X( IOFF ) ).NE.ZERO ) THEN
                  TEMP1 = ABS( AIMAG( X( IOFF ) ) )
                  IF( SCALE.LT.TEMP1 ) THEN
                     SUMSQ = 1 + SUMSQ*( SCALE / TEMP1 )**2
                     SCALE = TEMP1
                  ELSE
...
*
         ...
         WORK( 2 ) = SUMSQ
*
         CALL PETREECOMB( ICTXT, 'Rowwise', 2, WORK, -1, IXCOL,         
     +            ECOMBSSQ )
*
         SCALE = WORK( 1 )
         SUMSQ = WORK( 2 )
...
*
         ...
         WORK( 2 ) = SUMSQ
*
         CALL PETREECOMB( ICTXT, 'Columnwise', 2, WORK, -1, IXCOL,      
     +               ECOMBSSQ )
*
         SCALE = WORK( 1 )
         SUMSQ = WORK( 2 )
