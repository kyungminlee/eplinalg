      SUBROUTINE PEGEQLF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEQLF computes a QL factorization of a real distributed M-by-N
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK1MAT, PEGEQL2,    
     +                PELARFB, PELARFT, PB_TOPGET, PB_TOPSET,           
     +         PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEQLF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL PEGEQL2( M-N+J+JB-JA, JB, A, IA, J, DESCA, TAU, WORK,  
     +                   LWORK, IINFO )
*
            IF( J.GT.JA ) THEN
*
...
*              left
*
               CALL PELARFB( 'Left', 'Transpose', 'Backward',           
...
*
      IF( MU.GT.0 .AND. NU.GT.0 )    CALL PEGEQL2( MU, NU, A, IA, JA, 
