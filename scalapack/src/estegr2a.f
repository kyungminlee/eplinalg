      SUBROUTINE ESTEGR2A( JOBZ, RANGE, N, D, E, VL, VU, IL, IU,        
     +            M, W, Z, LDZ, NZC, WORK, LWORK, IWORK,                
     +    LIWORK, DOL, DOU, NEEDIL, NEEDIU,                    INDERR, 
     +NSPLIT, PIVMIN, SCALE, WL, WU,                    INFO )
*
...
*     .. Scalar Arguments ..
     ...
     $                   LWORK, M, N, NEEDIL, NEEDIU, NSPLIT, NZC
      REAL(KIND=10) PIVMIN, SCALE, VL, VU, WL, WU
...
*  =======
*
*  ESTEGR2A computes selected eigenvalues and initial representations.
*  needed for eigenvector computations in ESTEGR2B. It is invoked in the 
*  ScaLAPACK MRRR driver PESYEVR and the corresponding Hermitian
*  version when both eigenvalues and eigenvectors are computed in parallel.
*  on multiple processors. For this case, ESTEGR2A implements the FIRST 
*  part of the MRRR algorithm, parallel eigenvalue computation and finding
*  the root RRR. At the end of ESTEGR2A,
...
*
*     ESTEGR2A LOCALLY only computes the eigenvalues 
...
*     this processor.
*
*  3. While no eigenvectors are computed in ESTEGR2A itself (this is
*     done later in ESTEGR2B), the interface
*     If JOBZ = 'V' then, depending on RANGE and DOL, DOU, ESTEGR2A 
*     might need more workspace in Z then the original ESTEGR. 
...
*
*  D       (input/output) REAL(KIND=10) array, dimension (N)
*          On entry, the N diagonal elements of the tridiagonal matrix
*          T. On exit, D is overwritten.
*
*  E       (input/output) REAL(KIND=10) array, dimension (N)
*          On entry, the (N-1) subdiagonal elements of the tridiagonal
*          matrix T in elements 1 to N-1 of E. E(N) need not be set on
*          input, but is used internally as workspace.
*          On exit, E is overwritten.
*
*  VL      (input) REAL(KIND=10)
*  VU      (input) REAL(KIND=10)
...
*
*  W       (output) REAL(KIND=10) array, dimension (N)
...
*
*  Z       (output) REAL(KIND=10) array, dimension (LDZ, max(1,M) )
*          ESTEGR2A does not compute eigenvectors, this is done 
*          in ESTEGR2B. The argument Z as well as all related
...
*
*  WORK    (workspace/output) REAL(KIND=10) array, dimension (LWORK)
...
*
*  PIVMIN  (output) REAL(KIND=10)
*          The minimum pivot in the sturm sequence for T.
*
*  SCALE   (output) REAL(KIND=10) 
*          The scaling factor for the tridiagonal T.
*
*  WL      (output) REAL(KIND=10)
*  WU      (output) REAL(KIND=10)
*          The interval (WL, WU] contains all the wanted eigenvalues.         
*          It is either given by the user or computed in ELARRE2A.
*
*  INFO    (output) INTEGER
*          On exit, INFO
*          = 0:  successful exit
*          other:if INFO = -i, the i-th argument had an illegal value
*                if INFO = 10X, internal error in ELARRE2A,
*                Here, the digit X = ABS( IINFO ) < 10, where IINFO is 
*                the nonzero error code returned by ELARRE2A.
*
*  =====================================================================
*
*     .. Parameters ..
      REAL(KIND=10)   ZERO, ONE, FOUR, MINRGP
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10,             
     +         FOUR = 4.0E0_10,                      MINRGP = 1.0E-3_10 
     +)
*     ..
*     .. Local Scalars ..
     ...
     $                   ITMP2, J, LIWMIN, LWMIN, NZCMIN
      REAL(KIND=10)   BIGNUM, EPS, RMAX, RMIN, RTOL1, RTOL2, SAFMIN,    
     +                SMLNUM, THRESH, TNRM
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      REAL(KIND=10)   ELAMCH, ELANST
      EXTERNAL           LSAME, ELAMCH, ELANST
*     ..
*     .. External Subroutines ..
      EXTERNAL           ELARRC, ELARRE2A, ESCAL
*     ..
*     .. Intrinsic Functions ..
...
*     ESTEGR2A needs WORK of size 6*N, IWORK of size 3*N.
*     In addition, ELARRE2A needs WORK of size 6*N, IWORK of size 5*N.
*     Furthermore, ELARRV2 needs WORK of size 12*N, IWORK of size 7*N.
*     Workspace is kept consistent with ESTEGR2B even though 
*     ELARRV2 is not called here.
      IF( WANTZ ) THEN
         LWMIN = 18*N
         LIWMIN = 10*N
      ELSE
...
*
         ...
         ELSE IF( WANTZ .AND. VALEIG ) THEN
            CALL ELARRC( 'T', N, VL, VU, D, E, SAFMIN,                  
...
*     Initialize NEEDIL and NEEDIU, these values are changed in ELARRE2A
      NEEDIL = DOU
      NEEDIU = DOL
*
...
*
      ...
      IF( SCALE.NE.ONE ) THEN
         CALL ESCAL( N, SCALE, D, 1 )
         CALL ESCAL( N-1, SCALE, E, 1 )
         TNRM = TNRM*SCALE
         IF( VALEIG ) THEN
...
*     are small
*     THRESH is the splitting parameter for ELARRA in ELARRE2A      
...
*     Set the tolerance parameters for bisection
      IF( .NOT.WANTZ ) THEN
*        ELARRE2A computes the eigenvalues to full precision.   
         RTOL1 = FOUR * EPS
         RTOL2 = FOUR * EPS
      ELSE   
*        ELARRE2A computes the eigenvalues to less than full precision.
*        ELARRV2 will refine the eigenvalue approximations, and we can
*        need less accurate initial bisection in ELARRE2A.
         RTOL1 = FOUR*SQRT(EPS)
         RTOL2 = MAX( SQRT(EPS)*5.0E-3_10, FOUR * EPS )
      ENDIF
      CALL ELARRE2A( RANGE, N, WL, WU, IIL, IIU, D, E,               
     +WORK(INDE2), RTOL1, RTOL2, THRESH, NSPLIT,               IWORK( 
     +IINSPL ), M, DOL, DOU, NEEDIL, NEEDIU,              W, WORK( 
     +INDERR ),              WORK( INDGP ), IWORK( IINDBL ),            
     +  IWORK( IINDW ), WORK( INDGRS ),               WORK( INDSDM ), 
     +PIVMIN,              WORK( INDWRK ), IWORK( IINDWK ),             
     +  MINRGP, IINFO )
      IF( IINFO.NE.0 ) THEN
         INFO = 100 + ABS( IINFO )
         RETURN
      END IF
*     Note that if RANGE .NE. 'V', ELARRE2A computes bounds on the desired
