...
*  =======
*
*  PYPOTRF computes the Cholesky factorization of an N-by-N complex
...
*     .. External Subroutines ..
     ...
     +                  PB_TOPSET, PYPOTF2, PYHERK, PYTRSM,             
     +       PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPOTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL PYPOTF2( UPLO, JB, A, IA, JA, DESCA, INFO )
         IF( INFO.NE.0 )
     $      GO TO 30
*
         IF( JB+1.LE.N ) THEN
...
*
            CALL PYHERK( UPLO, 'Conjugate transpose', N-JB, JB, -ONE, A,
...
*           Perform unblocked Cholesky factorization on JB block
*
            CALL PYPOTF2( UPLO, JB, A, I, J, DESCA, INFO )
            IF( INFO.NE.0 ) THEN
               INFO = INFO + J - JA
               GO TO 30
            END IF
...
*              Update the trailing matrix, A = A - U'*U
*
               CALL PYHERK( UPLO, 'Conjugate transpose', N-J-JB+JA, JB, 
...
*
            CALL PYHERK( UPLO, 'No Transpose', N-JB, JB, -ONE, A, IA+JB,
...
*              Update the trailing matrix, A = A - L*L'
*
               CALL PYHERK( UPLO, 'No Transpose', N-J-JB+JA, JB, -ONE,  
