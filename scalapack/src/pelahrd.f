      SUBROUTINE PELAHRD( N, K, NB, A, IA, JA, DESCA, TAU, T, Y, IY, JY,
     +                     DESCY, WORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER             DESCA( * ), DESCY( * )
      REAL(KIND=10)    A( * ), T( * ), TAU( * ), WORK( * ), Y( * )
*     ..
*
*  Purpose
*  =======
*
*  PELAHRD reduces the first NB columns of a real general N-by-(N-K+1)
...
*
*  This is an auxiliary routine called by PEGEHRD. In the following
...
*
*  TAU     (local output) REAL(KIND=10) array, dimension LOCc(JA+N-2)
*          The scalar factors of the elementary reflectors (see Further
*          Details). TAU is tied to the distributed matrix A.
*
*  T       (local output) REAL(KIND=10) array, dimension (NB_A,NB_A)
...
*
*  WORK    (local workspace) REAL(KIND=10) array, dimension (NB)
*
*  Further Details
*  ===============
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EAXPY, DESCSET, ECOPY,         
     +           ESCAL, ETRMV, INFOG2L, PEELSET,                    
     +PEGEMV, PELARFG, PESCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MIN, MOD
*     ..
...
*
            CALL PEGEMV( 'No transpose', N, L-1, -ONE, Y, IY, JY, DESCY,
...
*
            IF( IPROC ) THEN
               CALL ECOPY( L-1, A( (JJ+L-2)*DESCA( LLD_ )+II ), 1,      
     +                WORK( JW ), 1 )
               CALL ETRMV( 'Lower', 'Transpose', 'Unit', L-1,           
...
*           w := w + V2'*b2
*
            CALL PEGEMV( 'Transpose', N-K-L+1, L-1, ONE, A, I+1, JA,    
     +                DESCA, A, I+1, J, DESCA, 1, ONE, WORK, 1, JW,     
     +               DESCW, DESCW( M_ ) )
*
*           w := T'*w
*
            IF( IPROC )          CALL ETRMV( 'Upper', 'Transpose', 
     +'Non-unit', L-1, T,                      DESCA( NB_ ), WORK( JW ),
     + 1 )
*
*           b2 := b2 - V2*w
*
            CALL PEGEMV( 'No transpose', N-K-L+1, L-1, -ONE, A, I+1, JA,
...
*
            IF( IPROC ) THEN
               CALL ETRMV( 'Lower', 'No transpose', 'Unit', L-1,        
     +              A( (JJ-1)*DESCA( LLD_ )+II ), DESCA( LLD_ ),        
     +              WORK( JW ), 1 )
               CALL EAXPY( L-1, -ONE, WORK( JW ), 1,                    
     +  A( ( JJ+L-2 )*DESCA( LLD_ )+II ), 1 )
            END IF
            CALL PEELSET( A, I, J-1, DESCA, EI )
         END IF
*
*        Generate the elementary reflector H(i) to annihilate
*        A(ia+k+i:ia+n-1,j)
*
         CALL PELARFG( N-K-L+1, EI, I+1, J, A, MIN( I+2, N+IA-1 ), J,   
     +               DESCA, 1, TAU )
         CALL PEELSET( A, I+1, J, DESCA, ONE )
*
*        Compute  Y(iy:y+n-1,jy+l-1)
*
         CALL PEGEMV( 'No transpose', N, N-K-L+1, ONE, A, IA, J+1,      
     +           DESCA, A, I+1, J, DESCA, 1, ZERO, Y, IY, JY+L-1,       
     +          DESCY, 1 )
         CALL PEGEMV( 'Transpose', N-K-L+1, L-1, ONE, A, I+1, JA, DESCA,
     ...
     +         JY+L-1, DESCY, 1 )
         ...
         CALL PESCAL( N, TAU( JL ), Y, IY, JY+L-1, DESCY, 1 )
*
*        Compute T(1:i,i)
*
         IF( IPROC ) THEN
            JT = ( L-1 ) * DESCA( NB_ )
            CALL ESCAL( L-1, -TAU( JL ), WORK( JW ), 1 )
            CALL ECOPY( L-1, WORK( JW ), 1, T( JT+1 ), 1 )
            CALL ETRMV( 'Upper', 'No transpose', 'Non-unit', L-1, T,    
     +               DESCA( NB_ ), T( JT+1 ), 1 )
            T( JT+L ) = TAU( JL )
         END IF
   10 CONTINUE
*
      CALL PEELSET( A, K+NB+IA-1, J, DESCA, EI )
*
      RETURN
*
*     End of PELAHRD
