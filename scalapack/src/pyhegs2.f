*
*
      SUBROUTINE PYHEGS2( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB, 
     +                    DESCB, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYHEGS2 reduces a complex Hermitian-definite generalized eigenproblem
*  to standard form.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_EXIT, BLACS_GRIDINFO, YAXPY, YHER2,      
     +              CHK1MAT, YLACGV, YESCAL, YTRMV, YTRSV, INFOG2L,     
     +               PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHEGS2', -INFO )
...
*
               AKK = REAL( A( IOFFA-LDA ) , KIND=10)
               BKK = REAL( B( IOFFB-LDB ) , KIND=10)
               AKK = AKK / BKK**2
               A( IOFFA-LDA ) = AKK
               IF( K.LT.N ) THEN
                  CALL YESCAL( N-K, ONE / BKK, A( IOFFA ), LDA )
                  CT = -HALF*AKK
                  CALL YLACGV( N-K, A( IOFFA ), LDA )
                  CALL YLACGV( N-K, B( IOFFB ), LDB )
                  CALL YAXPY( N-K, CT, B( IOFFB ), LDB, A( IOFFA ),     
     +                    LDA )
                  CALL YHER2( UPLO, N-K, -CONE, A( IOFFA ), LDA,        
     +                 B( IOFFB ), LDB, A( IOFFA+1 ), LDA )
                  CALL YAXPY( N-K, CT, B( IOFFB ), LDB, A( IOFFA ),     
     +                    LDA )
                  CALL YLACGV( N-K, B( IOFFB ), LDB )
                  CALL YTRSV( UPLO, 'Conjugate transpose', 'Non-unit',  
...
*              A(ia+k-1:ia+n-a,ia+k-1:ia+n-1)
*
               AKK = REAL( A( IOFFA-1 ) , KIND=10)
               BKK = REAL( B( IOFFB-1 ) , KIND=10)
               AKK = AKK / BKK**2
               A( IOFFA-1 ) = AKK
*
               IF( K.LT.N ) THEN
                  CALL YESCAL( N-K, ONE / BKK, A( IOFFA ), 1 )
                  CT = -HALF*AKK
                  CALL YAXPY( N-K, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
                  CALL YHER2( UPLO, N-K, -CONE, A( IOFFA ), 1,          
...
*
               AKK = REAL( A( IOFFA+K-1 ) , KIND=10)
               BKK = REAL( B( IOFFB+K-1 ) , KIND=10)
               CALL YTRMV( UPLO, 'No transpose', 'Non-unit', K-1,       
     +               B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ), 1 )
               CT = HALF*AKK
               CALL YAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL YHER2( UPLO, K-1, CONE, A( IOFFA ), 1, B( IOFFB ),  
     +                    1, A( IIA+( JJA-1 )*LDA ), LDA )
               CALL YAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL YESCAL( K-1, BKK, A( IOFFA ), 1 )
...
*
               AKK = REAL( A( IOFFA+( K-1 )*LDA ) , KIND=10)
               BKK = REAL( B( IOFFB+( K-1 )*LDB ) , KIND=10)
               CALL YLACGV( K-1, A( IOFFA ), LDA )
               CALL YTRMV( UPLO, 'Conjugate transpose', 'Non-unit', K-1,
     +                      B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ),    
     +                  LDA )
               CT = HALF*AKK
               CALL YLACGV( K-1, B( IOFFB ), LDB )
               CALL YAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL YHER2( UPLO, K-1, CONE, A( IOFFA ), LDA, B( IOFFB ),
     +                      LDB, A( IIA+( JJA-1 )*LDA ), LDA )
               CALL YAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL YLACGV( K-1, B( IOFFB ), LDB )
               CALL YESCAL( K-1, BKK, A( IOFFA ), LDA )
               CALL YLACGV( K-1, A( IOFFA ), LDA )
