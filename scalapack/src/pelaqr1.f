      RECURSIVE SUBROUTINE PELAQR1( WANTT, WANTZ, N, ILO, IHI, A,       
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   A( * ), WI( * ), WORK( * ), WR( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PELAQR1 is an auxiliary routine used to find the Schur decomposition
*    and or eigenvalues of a matrix already in Hessenberg form from
*    cols ILO to IHI.
*
*  This is a modified version of PELAHQR from ScaLAPACK version 1.7.3.
...
*      subdiagonal elements by PELACONSB) is abandoned.
...
*    o For some reason, the original version of PELAHQR sometimes did
...
*          ILO = 1). PELAQR1 works primarily with the Hessenberg
*          submatrix in rows and columns ILO to IHI, but applies
*          transformations to all of H if WANTT is .TRUE..
*          1 <= ILO <= max(1,IHI); IHI <= N.
*
*  A       (global input/output) REAL(KIND=10) array, dimension
*          (DESCA(LLD_),*)
...
*
*  WR      (global replicated output) REAL(KIND=10) array,
*                                                         dimension (N)
*  WI      (global replicated output) REAL(KIND=10) array,
*                                                         dimension (N)
...
*
*  Z       (global input/output) REAL(KIND=10) array.
*          If WANTZ is .TRUE., on entry Z must contain the current
*          matrix Z of transformations accumulated by PEHSEQR, and on
...
*
*  WORK    (local output) REAL(KIND=10) array of size LWORK
*
...
*          = 0: successful exit
*          > 0: PELAQR1 failed to compute all the eigenvalues ILO to IHI
...
*  Subroutines:
*       This routine calls:
*           PELAWIL   -> Given the shift, get the transformation
*           ELASORTE  -> Pair up eigenvalues so that reals are paired.
*           PELACP3   -> Parallel array to local replicated array copy &
*                        back.
*           ELAREF    -> Row/column reflector applier. Core routine here.
*           PELASMSUB -> Finds negligible subdiagonal elements.
*
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      REAL(KIND=10)   ZERO, ONE, HALF
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10, HALF = 
     +0.5E0_10 )
      REAL(KIND=10)   CONST
      PARAMETER          ( CONST = 1.50E0_10 )
      INTEGER            IBLK, LDS
...
*     .. Local Scalars ..
     ...
     $                   DBLK, NIBBLE, ND, NS, LTOP, LWKOPT, S1, S2, S3
      REAL(KIND=10)   AVE, DISC, H00, H10, H11, H12, H21, H22, H33,     
...
*     .. Local Arrays ..
     ...
     $                   KP2ROW( IBLK ), KROW( IBLK ), LOCALK2( IBLK )
      REAL(KIND=10)   SMALLA( 6, 6, IBLK ), VCOPY( 3 )
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC, ILAENV
      REAL(KIND=10)   PELAMCH
      EXTERNAL           ILCM, NUMROC, ILAENV, PELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, ECOPY, EGEBR2D, EGEBS2D,       
     +             EGERV2D, EGESD2D, EGSUM2D, ELAHQR, ELAREF,           
     +         ELARFG, ELASORTE, IGAMN2D, INFOG1L, INFOG2L,             
     +       PELABAD, PELACP3, PELASMSUB,                    PELAWIL, 
     +PXERBLA, ELANV2, PELAQR2, PELAQR4
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $              -1, -1 )
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( CONTXT, 'PELAQR1', -INFO )
         WORK( 1 ) = REAL( LWKOPT , KIND=10)
         RETURN
      END IF
*
...
*     call ELAHQR directly.
*
      IF( NH .LE. LDS ) THEN
         CALL PELAQR4( WANTT, WANTZ, N, ILO, IHI, A, DESCA, WR, WI,     
...
*
      UNFL = PELAMCH( CONTXT, 'SAFE MINIMUM' )
      OVFL = ONE / UNFL
      CALL PELABAD( CONTXT, UNFL, OVFL )
      ULP = PELAMCH( CONTXT, 'PRECISION' )
      SMLNUM = UNFL*( NH / ULP )
*
...
*
         CALL PELASMSUB( A, DESCA, I, L, K, SMLNUM, WORK( IRBUF+1 ),    
     +                LWORK-IRBUF )
         L = K
*
         IF( L.GT.ILO ) THEN
...
*           Exceptional shift.
*
            CALL PELACP3( 2*JBLK, I-2*JBLK+1, A, DESCA, WORK( S1+1 ),   
     +                  LDS, -1, -1, 0 )
            DO 20 II = 2*JBLK, 2, -1
...
*
            IF( AED ) THEN
               DBLK = ILAENV( 13, 'ELAQR0', 'SV', N, L, I, 4*LDS*LDS )
               DBLK = MAX( 2*JBLK, DBLK ) + 1
               DBLK = MIN( NH, LDS, DBLK )
               CALL PELAQR2( WANTT, WANTZ, N, L, I, DBLK, A, DESCA,     
...
*
               NIBBLE = ILAENV( 14, 'ELAQR0', 'SV', N, L, I, 4*LDS*LDS )
               NIBBLE = MAX( 0, NIBBLE )
               I = I - ND
               DBLK = DBLK - ND
...
*               IF( ND.GE.0 .AND. ND+DBLK.GE.64 ) THEN
               IF( .FALSE. ) THEN
                  CALL ELASET( 'L', DBLK-1, DBLK-1, ZERO, ZERO,         
     +                 WORK( S1+2 ), LDS )
...
*
   ...
   22             CONTINUE
               ...
               ELSE
                  CALL ELAHQR( .FALSE., .FALSE., DBLK, 1, DBLK,         
     +                 WORK( S1+1 ), LDS, WORK( IRBUF+1 ),              
     +            WORK( ICBUF+1 ), 1, DBLK, Z, LDZ, IERR )
               END IF
            ELSE
               DBLK = 2*JBLK
               CALL PELACP3( DBLK, I-DBLK+1, A, DESCA, WORK( S1+1 ),    
     +                    LDS, -1, -1, 0 )
               CALL ELAHQR( .FALSE., .FALSE., DBLK, 1, DBLK,            
     +           WORK( S1+1 ), LDS, WORK( IRBUF+1 ),                    
     +   WORK( ICBUF+1 ), 1, DBLK, Z, LDZ, IERR )
...
*           PELACONSB is the routine that does this.
*
*         CALL PELACONSB( A, DESCA, I, L, M, H44, H33, H43H34,
...
*    $      GO TO 80
*
*        In principle PELACONSB needs to check all shifts to decide
...
*
*            CALL ELASORTE( S1( 2*( JBLK-NBULGE )+1,
*     $                     2*( JBLK-NBULGE )+1 ), 3*IBLK, 2*NBULGE,
*     $                     WORK( IRBUF+1 ), IERR )
            CALL ELASORTE( WORK(S1+DBLK-2*NBULGE+1+(DBLK-2*NBULGE)*LDS),
...
*
   ...
   31    CONTINUE
         ITMP1 = ISTARTROW
         ITMP2 = ISTARTCOL
         CALL PELAWIL( ITMP1, ITMP2, M, A, DESCA, H44, H33, H43H34,     
     +             VCOPY )
         V1SAVE = VCOPY( 1 )
         V2SAVE = VCOPY( 2 )
         V3SAVE = VCOPY( 3 )
   ...
   40       CONTINUE
     ...
     $                    *WORK(S1+DBLK-2*IBULGE+(DBLK-2*IBULGE-2)*LDS)
                  ITMP1 = ISTARTROW
                  ITMP2 = ISTARTCOL
                  CALL PELAWIL( ITMP1, ITMP2, M, A, DESCA, H44, H33,    
     +                       H43H34, VCOPY )
                  V1SAVE = VCOPY( 1 )
                  V2SAVE = VCOPY( 2 )
                  V3SAVE = VCOPY( 3 )
...
*
     ...
     $                             MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
                     CALL PELACP3( MIN( 6, N-K+2 ), K-1, A, DESCA,      
...
*
     ...
     $                             MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
                     CALL PELACP3( MIN( 6, N-K+3 ), K-2, A, DESCA,      
     +                        SMALLA( 1, 1, KI ), 6, ITMP1, ITMP2,      
     +                        0 )
                  END IF
               END IF
*
*           ELAHQR used to have a single row application and a single
...
*
     ...
     $             ( ISTART.LT.MIN( I-1, ISTOP+1 ) ) ) THEN
                  ...
                  NR = MIN( 3, I-K+1 )
                  IF( K.GT.M ) THEN
                     CALL ECOPY( NR, SMALLA( 2, 1, KI ), 1, VCOPY, 1 )
                  ELSE
                     VCOPY( 1 ) = V1SAVE
                     VCOPY( 2 ) = V2SAVE
                     VCOPY( 3 ) = V3SAVE
                  END IF
                  CALL ELARFG( NR, VCOPY( 1 ), VCOPY( 2 ), 1, T1COPY )
                  IF( K.GT.M ) THEN
...
*
     ...
     $             ( ISTART.LE.MIN( I, ISTOP ) ) ) THEN
                  ...
                  NR = MIN( 3, I-K+1 )
                  IF( K.GT.M ) THEN
                     CALL ECOPY( NR, SMALLA( 3, 2, KI ), 1, VCOPY, 1 )
                  ELSE
                     VCOPY( 1 ) = V1SAVE
                     VCOPY( 2 ) = V2SAVE
                     VCOPY( 3 ) = V3SAVE
...
*
                  ...
                  IF( ISTART.GT.M ) THEN
                     ...
                     NR = MIN( 3, I-ISTART+1 )
                     CALL ELARFG( NR, VCOPY( 1 ), VCOPY( 2 ), 1,        
     +                     T1COPY )
...
*
     ...
     $                ( NPROW.GT.1 ) .AND. ( ISTART.LE.ISTOP ) ) THEN
                     ...
                     IF( MYROW.NE.JJ ) THEN
                        CALL EGEBR2D( CONTXT, 'COL', ' ',               
     +                  3*( ISTOP-ISTART+1 ), 1,                        
     +         WORK( VECSIDX+( ISTART-1 )*3+1 ),                        
     +         3*( ISTOP-ISTART+1 ), JJ, MYCOL )
                     ELSE
                        CALL EGEBS2D( CONTXT, 'COL', ' ',               
     +                  3*( ISTOP-ISTART+1 ), 1,                        
     +         WORK( VECSIDX+( ISTART-1 )*3+1 ),                        
     +         3*( ISTOP-ISTART+1 ) )
                     END IF
...
*
                        T2 = T1*V2
                        T3 = T1*V3
                        CALL ELAREF( 'Col', A, LDA, .FALSE., Z, LDZ,    
     ...
     $                      HBL ).LT.HBL-2 ) ) THEN
                           T2 = T1*V2
                           T3 = T1*V3
                           CALL ELAREF( 'Row', A, LDA, .FALSE., Z, LDZ, 
...
*
     ...
     $                          ITMP1, ITMP2 )
                  ...
                  II = KROW( KI )
                  CALL ELAREF( 'Row', A, LDA, WANTZ, Z, LDZ, .TRUE., II,
...
*
     ...
     $                      ( NPROW.EQ.1 ) ) THEN
                           T2 = T1*V2
                           T3 = T1*V3
                           CALL ELAREF( 'Row', A, LDA, WANTZ, Z, LDZ,   
     +                                .FALSE., IROW1, IROW1, ISTART,    
     ...
     $                      ( NPROW.GT.1 ) ) THEN
                           IF( IROW1.EQ.IROW2 ) THEN
                              CALL EGESD2D( CONTXT, 1, ICOL2-ICOL1+1,   
     +                                    A( ( ICOL1-1 )*LDA+IROW2 ),   
     +                                    LDA, UP, MYCOL )
                           END IF
                        END IF
...
*
     ...
     $                      ( NPROW.GT.1 ) ) THEN
                           IF( IROW1.NE.IROW2 ) THEN
                              CALL EGERV2D( CONTXT, 1, ICOL2-ICOL1+1,   
     +                                    WORK( IRBUF+1 ), 1, DOWN,     
     +                                  MYCOL )
                              T2 = T1*V2
                              T3 = T1*V3
...
*
     ...
     $                 IROW1, IROW2 )
                  ...
                  ELSE
                     ITMP2 = -1
                  END IF
                  CALL ELAREF( 'Col', A, LDA, WANTZ, Z, LDZ, .TRUE.,    
...
*
     ...
     $                   ( NPCOL.GT.1 ) ) THEN
                        IF( ICOL1.EQ.ICOL2 ) THEN
                           CALL EGESD2D( CONTXT, IROW2-IROW1+1, 1,      
     +                              A( ( ICOL1-1 )*LDA+IROW1 ),         
     +                           LDA, MYROW, LEFT )
                           CALL EGERV2D( CONTXT, IROW2-IROW1+1, 1,      
     +                              A( ( ICOL1-1 )*LDA+IROW1 ),         
     +                           LDA, MYROW, LEFT )
                        ELSE
                           CALL EGERV2D( CONTXT, IROW2-IROW1+1, 1,      
...
*
  ...
  340                      CONTINUE
                        ELSE
                           IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                              CALL EGERV2D( CONTXT, 1, LIHIH-LILOH+1,   
     +                                    WORK( IRBUF+1 ), 1, UP,       
     +                                MYCOL )
     ...
     $                              LDA+ITMP1 ) - SUM*T2
  350                         CONTINUE
                              CALL EGESD2D( CONTXT, 1, LIHIH-LILOH+1,   
     +                                    WORK( IRBUF+1 ), 1, UP,       
     +                                MYCOL )
                           END IF
                        END IF
...
*                 LOCAL A(LILOZ:LIHIZ,LOCALK2:LOCALK2+2)
  ...
  360                      CONTINUE
                        ELSE
                           ITMP1 = LOCALK2( KI )
                           IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                              CALL EGERV2D( CONTXT, LIHIH-LILOH+1, 1,   
     +                                    WORK( ICBUF+1 ),              
  ...
  370                         CONTINUE
                              CALL EGESD2D( CONTXT, LIHIH-LILOH+1, 1,   
     +                                    WORK( ICBUF+1 ),              
     +                         LIHIH-LILOH+1, MYROW, LEFT )
                           END IF
                        END IF
...
*                       IF WE ACTUALLY OWN COLUMN K
                              IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                                 CALL EGERV2D( CONTXT, LIHIZ-LILOZ+1, 1,
     +                                          WORK( ICBUF+1 ), LDZ,   
     +                                       MYROW, LEFT )
                                 ITMP1 = ( ITMP1-1 )*LDZ
  ...
  390                            CONTINUE
                                 CALL EGESD2D( CONTXT, LIHIZ-LILOZ+1, 1,
     +                                          WORK( ICBUF+1 ), LDZ,   
     +                                       MYROW, LEFT )
                                 LOCALK2( KI ) = LOCALK2( KI ) + 1
                              END IF
...
*
         CALL PEELGET( 'All', ' ', H11, A, L, L, DESCA )
         CALL PEELGET( 'All', ' ', H21, A, I, L, DESCA )
         CALL PEELGET( 'All', ' ', H12, A, L, I, DESCA )
         CALL PEELGET( 'All', ' ', H22, A, I, I, DESCA )
         CALL ELANV2( H11, H12, H21, H22, WR( L ), WI( L ), WR( I ),    
     +             WI( I ), CS, SN )
         CALL PEELSET( A, L, L, DESCA, H11 )
         CALL PEELSET( A, I, L, DESCA, H21 )
         CALL PEELSET( A, L, I, DESCA, H12 )
         CALL PEELSET( A, I, I, DESCA, H22 )
*
*        Transform H to the standard Schur form
*
         IF( WANTT ) THEN
            IF(I .LT. N) CALL PEROT( N-I, A, L, I+1, DESCA, DESCA( M_ ),
     ...
     +                    LWORK-VECSIDX, IERR )
         ...
         ELSE
            LTOP = I1
         END IF
         IF (L .GT. LTOP) CALL PEROT( L-LTOP, A, LTOP, L, DESCA, 1, A,  
     +                               LTOP, I, DESCA, 1, CS, SN,         
     +                        WORK( VECSIDX+1 ), LWORK-VECSIDX,         
     +                        IERR )
         IF( WANTZ ) THEN
            CALL PEROT( IHIZ-ILOZ+1, Z, ILOZ, L, DESCZ, 1, Z, ILOZ, I,  
...
*
         NH = I - L + 1
         IF( NH .LE. LDS ) THEN
            CALL PELAQR4( WANTT, WANTZ, N, L, I, A, DESCA, WR, WI,      
     +               ILOZ, IHIZ, Z, DESCZ, WORK( S1+1 ), NH,            
     +         WORK( S2+1 ), NH, WORK( S3+1 ), 4*LDS*LDS,               
     +      INFO )
            IF( INFO.NE.0 ) THEN
...
*
      IF( NUM.GT.1 ) THEN
         CALL EGSUM2D( CONTXT, 'All', ' ', IHI-ILO+1, 1, WR(ILO), N,    
     +     -1, -1 )
         CALL EGSUM2D( CONTXT, 'All', ' ', IHI-ILO+1, 1, WI(ILO), N,    
     +     -1, -1 )
      END IF
*
      WORK( 1 ) = REAL( LWKOPT , KIND=10)
