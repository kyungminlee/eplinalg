      SUBROUTINE PYPTTRSV( UPLO, TRANS, N, NRHS, D, E, JA, DESCA, B, IB,
...
*  =======
*
*  PYPTTRSV solves a tridiagonal triangular system of linear equations
*
...
*  Cholesky factorization code PYPTTRF
...
*
*  Routine PYPTTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*          PYPTTRF and this is stored in AF. If a linear system
*          is to be solved using PYPTTRS after the factorization
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 3 )
         CALL PXERBLA( ICTXT,      
     + 'PYPTTRSV, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*INT_ONE )) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT,       'PYPTTRSV, D&C alg.: NB too small', 
     +      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -15
         CALL PXERBLA( ICTXT,       'PYPTTRSV: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      PARAM_CHECK( 16, 2 ) = 1105
      PARAM_CHECK( 15, 2 ) = 1104
      PARAM_CHECK( 14, 2 ) = 1103
      PARAM_CHECK( 13, 2 ) = 1102
      PARAM_CHECK( 12, 2 ) = 1101
      PARAM_CHECK( 11, 2 ) = 10
      PARAM_CHECK( 10, 2 ) = 805
      PARAM_CHECK(  9, 2 ) = 804
      PARAM_CHECK(  8, 2 ) = 803
      PARAM_CHECK(  7, 2 ) = 801
      PARAM_CHECK(  6, 2 ) = 7
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPTTRSV', -INFO )
         RETURN
      END IF
*
...
*
        CALL YPTTRSV( UPLO, 'N', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),    
...
*
            CALL YAXPY( NRHS, -E( PART_OFFSET+ODD_SIZE ),             
...
*
            CALL YGEMM( 'C', 'N', 1, NRHS, ODD_SIZE, -CONE, AF( 1 ),    
...
*
        CALL YTRTRS( 'L', 'N', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),    
...
*       Solve with diagonal block
*
        CALL YTRTRS( 'L', 'C', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),    
     +            INT_ONE, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
          GO TO 1000
...
*
          CALL YGEMM( 'N', 'N', ODD_SIZE, NRHS, 1, -CONE, AF( 1 ),      
     +           ODD_SIZE, WORK( 1+INT_ONE-1 ), INT_ONE, CONE,          
...
*           locally stored portion of right hand side(s)
*
            CALL YAXPY( NRHS, -CONJG( E( PART_OFFSET+ODD_SIZE ) ),      
...
*       Use main partition in each processor to solve locally
*
        CALL YPTTRSV( UPLO, 'C', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),    
     +             E( PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,        
     +         INFO )
*
      ENDIF
...
*           processor's righthand-side.
*
            CALL YGEMM( 'T', 'N', 1, NRHS, ODD_SIZE, -CONE, AF( 1 ),    
     +               ODD_SIZE, B( PART_OFFSET+1 ), LLDB, CZERO,         
     +          WORK( 1+INT_ONE-1 ), INT_ONE )
        ENDIF
*
...
*         Calculate contribution from this block to next diagonal block
*
          CALL YGEMM( 'T', 'N', INT_ONE, NRHS, INT_ONE, -CONE,          
     +            AF( (ODD_SIZE)*1+1 ),                      INT_ONE,   
     +                   B( PART_OFFSET+ODD_SIZE+1 ),                   
     +   LLDB, CZERO,                      WORK( 1 ),                   
     +   INT_ONE )
...
*           processor's solution.
*
          CALL YGEMM( 'C', 'N', ODD_SIZE, NRHS, 1, -CONE, AF( 1 ),      
     +           INT_ONE, WORK( 1 ), INT_ONE, CONE,                 B( 
     +PART_OFFSET+1 ), LLDB )
*
        ENDIF
*
