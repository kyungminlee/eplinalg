      SUBROUTINE PYHEEVD( JOBZ, UPLO, N, A, IA, JA, DESCA, W, Z, IZ, JZ,
...
*  =======
*
*  PYHEEVD computes all the eigenvalues and eigenvectors of a Hermitian
...
*
*  A       (local input/workspace) block cyclic COMPLEX(KIND=10) array,
...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME, INDXG2L, INDXG2P, NUMROC, PYLANHE,      
     +              PELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCINIT, INFOG2L,    
     +                PYELGET, PYHETRD, PCHK2MAT, PYLASCL, PYLASET,     
     +               PYUNMTR, PELARED1D, PELASET, PESTEDC, PXERBLA,     
     +               ESCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ICHAR, MAX, MIN, MOD, SQRT
*     ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PYHEEVD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $     THEN
         CALL PELARED1D( N, IA, JA, DESCA, RWORK( INDD ), W,            
     +        RWORK( INDRWORK ), LLRWORK )
*
         CALL PELARED1D( N, IA, JA, DESCA, RWORK( INDE2 ),              
     +      RWORK( INDE ), RWORK( INDRWORK ), LLRWORK )
         IF( .NOT.LOWER )
     $      OFFSET = 1
      ELSE
         DO 10 I = 1, N
            CALL PYELGET( 'A', ' ', WORK( INDWORK ), A, I+IA-1, I+JA-1, 
     +                    DESCA )
            W( I ) = REAL( WORK( INDWORK ) , KIND=10)
   10    CONTINUE
         IF( LSAME( UPLO, 'U' ) ) THEN
            DO 20 I = 1, N - 1
               CALL PYELGET( 'A', ' ', WORK( INDWORK ), A, I+IA-1, I+JA,
     +                        DESCA )
               RWORK( INDE+I-1 ) = REAL( WORK( INDWORK ) , KIND=10)
   20       CONTINUE
         ELSE
            DO 30 I = 1, N - 1
               CALL PYELGET( 'A', ' ', WORK( INDWORK ), A, I+IA, I+JA-1,
     +                        DESCA )
               RWORK( INDE+I-1 ) = REAL( WORK( INDWORK ) , KIND=10)
   30       CONTINUE
         END IF
      END IF
*
*     Call PESTEDC to compute eigenvalues and eigenvectors.
*
      INDZ = INDE + N
     ...
     $               DESCZ( CTXT_ ), LDR, INFO )
      CALL PYLASET( 'Full', N, N, CZERO, CONE, Z, IZ, JZ, DESCZ )
      CALL PELASET( 'Full', N, N, ZERO, ONE, RWORK( INDZ ), 1, 1,       
     +        DESCRZ )
      CALL PESTEDC( 'I', N, W, RWORK( INDE+OFFSET ), RWORK( INDZ ), IZ, 
