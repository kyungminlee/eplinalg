      SUBROUTINE PYLARFG( N, ALPHA, IAX, JAX, X, IX, JX, DESCX, INCX,   
     +                  TAU )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCX( * )
      COMPLEX(KIND=10)            TAU( * ), X( * )
*     ..
*
*  Purpose
*  =======
*
*  PYLARFG generates a complex elementary reflector H of order n, such
*  that
*
...
*     .. Local Scalars ..
     ...
     $                   KNT, MYCOL, MYROW, NPCOL, NPROW
      REAL(KIND=10)               ALPHI, ALPHR, BETA, RSAFMN, SAFMIN, 
     +XNORM, CR,                    CI
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGEBR2D, YGEBS2D, PYSCAL,      
     +              PYESCAL, INFOG2L, PEYNRM2, ELADIV
*     ..
*     .. External Functions ..
      REAL(KIND=10)               ELAMCH, ELAPY3
      EXTERNAL           ELAPY3, ELAMCH
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, SIGN
*     ..
...
*
         IF( MYCOL.EQ.IXCOL ) THEN
            ...
            CALL YGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, X( J ), 1 )
            ALPHA = X( J )
         ELSE
            CALL YGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA, 1,        
     +             MYROW, IXCOL )
         END IF
*
         INDXTAU = IIAX
...
*
         IF( MYROW.EQ.IXROW ) THEN
            J = IIAX+(JJAX-1)*DESCX( LLD_ )
            CALL YGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, X( J ), 1 )
            ALPHA = X( J )
         ELSE
            CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, ALPHA, 1,     
     +                IXROW, MYCOL )
         END IF
*
         INDXTAU = JJAX
...
*
      ...
      END IF
*
      CALL PEYNRM2( N-1, XNORM, X, IX, JX, DESCX, INCX )
...
*
   ...
   10       CONTINUE
            KNT = KNT + 1
            CALL PYESCAL( N-1, RSAFMN, X, IX, JX, DESCX, INCX )
            BETA = BETA*RSAFMN
            ALPHI = ALPHI*RSAFMN
            ALPHR = ALPHR*RSAFMN
...
*
     ...
     +                 -ALPHI / BETA , KIND=10)
            CALL ELADIV( ONE, ZERO, REAL( ALPHA-BETA , KIND=10),        
     +       AIMAG( ALPHA-BETA ), CR, CI )
            ALPHA = CMPLX( CR, CI , KIND=10)
            CALL PYSCAL( N-1, ALPHA, X, IX, JX, DESCX, INCX )
*
