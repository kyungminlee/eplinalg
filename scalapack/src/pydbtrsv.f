      SUBROUTINE PYDBTRSV( UPLO, TRANS, N, BWL, BWU, NRHS, A, JA, DESCA,
...
*  =======
*
*  PYDBTRSV solves a banded triangular system of linear equations
*
...
*
*  Routine PYDBTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*     .. External Subroutines ..
     ...
     +               YGEMM, YGERV2D, YGESD2D, YLAMOV, YMATADD,          
     +          YTBTRS, YTRMM, YTRTRS, DESC_CONVERT, GLOBCHK,           
     +         PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 3 )
         CALL PXERBLA( ICTXT,      
     + 'PYDBTRSV, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*MAX(BWL,BWU) )) THEN
         INFO = -( 9*100+4 )
         CALL PXERBLA( ICTXT,       'PYDBTRSV, D&C alg.: NB too small', 
     +      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -16
         CALL PXERBLA( ICTXT,       'PYDBTRSV: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYDBTRSV', -INFO )
         RETURN
      END IF
*
...
*
        CALL YTBTRS( UPLO, 'N', 'U', ODD_SIZE,                     BWL, 
     +NRHS,                     A( OFST+1+BWU ), LLDA,                  
...
*             then use it on RHS
*
            CALL YLAMOV( 'N', BWL, NRHS,                 B( 
     +PART_OFFSET+ODD_SIZE-BWL+1), LLDB,                 WORK( 1 ), 
     +MAX_BW )
*
            CALL YTRMM( 'L', 'U', 'N', 'N', BWL, NRHS, -CONE,           
     +        A(( OFST+(BWL+BWU+1)+(ODD_SIZE-BWL)*LLDA )),              
     +     LLDA-1, WORK( 1 ), MAX_BW )
*
            CALL YMATADD( BWL, NRHS, CONE, WORK( 1 ), MAX_BW,           
...
*
            CALL YGEMM( 'C', 'N', BWU, NRHS, ODD_SIZE, -CONE, AF( 1 ),  
...
*
        IF( MYCOL .GT. 0) THEN
*
          CALL YGESD2D( ICTXT, MAX_BW, NRHS,                      WORK( 
     +1 ), MAX_BW,                      0, MYCOL - 1 )
*
        ENDIF
*
...
*
          CALL YGERV2D( ICTXT, MAX_BW, NRHS,                      WORK( 
     +1 ), MAX_BW,                      0, MYCOL + 1 )
*
*         Combine contribution to locally stored right hand sides
*
          CALL YMATADD( MAX_BW, NRHS, CONE,                 WORK( 1 ), 
     +MAX_BW, CONE,                 B( PART_OFFSET+ODD_SIZE + 1 ), LLDB 
     +)
*
        ENDIF
*
...
*
            CALL YGERV2D( ICTXT, MAX_BW, NRHS,                      
     +WORK( 1 ),                      MAX_BW, 0, MYCOL-LEVEL_DIST )
*
            CALL YMATADD( MAX_BW, NRHS, CONE,                 WORK( 1 ),
     + MAX_BW, CONE,                 B( PART_OFFSET+ODD_SIZE + 1 ), 
     +LLDB )
*
          ENDIF
*
...
*
        CALL YTBTRS( 'L', 'N', 'U', MAX_BW, MIN( BWL, MAX_BW-1 ), NRHS, 
...
*
          CALL YGEMM( 'C', 'N', MAX_BW, NRHS, MAX_BW, -CONE,            
...
*
          CALL YGEMM( 'N', 'N', MAX_BW, NRHS, MAX_BW, -CONE,            
     +          AF( ODD_SIZE*BWU+2*MBW2+1 ),                      
     +MAX_BW,                      B( PART_OFFSET+ODD_SIZE+1 ),         
     +             LLDB, CZERO,                      WORK( 1 ),         
     +             MAX_BW )
*
*         Send contribution to diagonal block's owning processor.
...
*       Solve with diagonal block
*
        CALL YTBTRS( 'L', 'C', 'U', MAX_BW, MIN( BWL, MAX_BW-1 ), NRHS, 
     +               AF( ODD_SIZE*BWU+MBW2+1 ), MAX_BW+1,               
     + B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*
          IF( MYCOL+LEVEL_DIST .LT. NPCOL-1 ) THEN
*
          CALL YGESD2D( ICTXT, MAX_BW, NRHS,                      B( 
     +PART_OFFSET+ODD_SIZE+1 ),                      LLDB, 0, 
     +MYCOL+LEVEL_DIST )
*
          ENDIF
*
...
*
          CALL YGESD2D( ICTXT, MAX_BW, NRHS,                      B( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDB,                      0, MYCOL +1 )
*
        ENDIF
*
*       Receive modifications to processor's right hand sides
...
*
          CALL YGEMM( 'N', 'N', ODD_SIZE, NRHS, BWU, -CONE, AF( 1 ),    
...
*           then use it on RHS
*
          CALL YLAMOV( 'N', BWL, NRHS, B( PART_OFFSET+ODD_SIZE+1), LLDB,
     +                  WORK( 1+MAX_BW-BWL ), MAX_BW )
*
          CALL YTRMM( 'L', 'U', 'C', 'N', BWL, NRHS, -CONE,             
     +    A(( OFST+(BWL+BWU+1)+(ODD_SIZE-BWL)*LLDA )),                 
     +LLDA-1, WORK( 1+MAX_BW-BWL ), MAX_BW )
*
          CALL YMATADD( BWL, NRHS, CONE, WORK( 1+MAX_BW-BWL ), MAX_BW,  
...
*       Use main partition in each processor to solve locally
*
        CALL YTBTRS( UPLO, 'C', 'U', ODD_SIZE,                     BWL, 
     +NRHS,                     A( OFST+1+BWU ),                     
     +LLDA,  B( PART_OFFSET+1 ),                     LLDB, INFO )
*
      ENDIF
...
*       Use main partition in each processor to solve locally
*
        CALL YTBTRS( UPLO, 'C', 'N', ODD_SIZE,                     BWU, 
     +NRHS,                     A( OFST+1 ), LLDA,                     
     +B( PART_OFFSET+1 ), LLDB, INFO )
*
*
...
*             then use it on RHS
*
            CALL YLAMOV( 'N', BWU, NRHS,                 B( 
     +PART_OFFSET+ODD_SIZE-BWU+1), LLDB,                 WORK( 1 ), 
     +MAX_BW )
*
            CALL YTRMM( 'L', 'L', 'C', 'N', BWU, NRHS, -CONE,           
     +        A(( OFST+1+ODD_SIZE*LLDA )), LLDA-1, WORK( 1 ),           
     +        MAX_BW )
*
            CALL YMATADD( BWU, NRHS, CONE, WORK( 1 ), MAX_BW,           
     +      CONE, B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
        ENDIF
*
...
*           processor's righthand-side.
*
            CALL YGEMM( 'C', 'N', BWL, NRHS, ODD_SIZE, -CONE,           
...
*       Solve with diagonal block
*
        CALL YTBTRS( 'U', 'C', 'N', MAX_BW, MIN( BWU, MAX_BW-1 ), NRHS, 
...
*       Solve with diagonal block
*
        CALL YTBTRS( 'U', 'N', 'N', MAX_BW, MIN( BWU, MAX_BW-1 ), NRHS, 
     +               AF( ODD_SIZE*BWU+MBW2+1-MIN( BWU, MAX_BW-1 ) ),    
     +            MAX_BW+1, B( PART_OFFSET+ODD_SIZE+1 ), LLDB,          
     +      INFO )
*
...
*           processor's solution.
*
          CALL YGEMM( 'N', 'N', ODD_SIZE, NRHS, BWL, -CONE,             
...
*           then use it on RHS
*
          CALL YLAMOV( 'N', BWU, NRHS, B( PART_OFFSET+ODD_SIZE+1), LLDB,
     +                  WORK( 1+MAX_BW-BWU ), MAX_BW+BWL )
*
          CALL YTRMM( 'L', 'L', 'N', 'N', BWU, NRHS, -CONE,             
     +    A(( OFST+1+ODD_SIZE*LLDA )), LLDA-1,                 WORK( 
     +1+MAX_BW-BWU ), MAX_BW+BWL )
*
          CALL YMATADD( BWU, NRHS, CONE, WORK( 1+MAX_BW-BWU ),          
     +         MAX_BW+BWL, CONE,                   B( 
     +PART_OFFSET+ODD_SIZE-BWU+1 ), LLDB )
*
        ENDIF
*
*       Use main partition in each processor to solve locally
*
        CALL YTBTRS( UPLO, 'N', 'N', ODD_SIZE,                     BWU, 
     +NRHS,                     A( OFST+1 ),                     LLDA,  
     +B( PART_OFFSET+1 ),                     LLDB, INFO )
*
      ENDIF
