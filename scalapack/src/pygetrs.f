      SUBROUTINE PYGETRS( TRANS, N, NRHS, A, IA, JA, DESCA, IPIV, B,    
     +                 IB, JB, DESCB, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGETRS solves a system of distributed linear equations
*
*                   op( sub( A ) ) * X = sub( B )
*
*  with a general N-by-N distributed matrix sub( A ) using the LU
*  factorization computed by PYGETRF.
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGETRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PYLAPIV( 'Forward', 'Row', 'Col', N, NRHS, B, IB, JB,     
     +             DESCB, IPIV, IA, 1, DESCIP, IDUM1 )
*
*        Solve L*X = sub( B ), overwriting sub( B ) with X.
*
         CALL PYTRSM( 'Left', 'Lower', 'No transpose', 'Unit', N, NRHS, 
...
*        Solve U'*X = sub( B ), overwriting sub( B ) with X.
*
         CALL PYTRSM( 'Left', 'Upper', TRANS, 'Non-unit', N, NRHS,      
     +           ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
*        Solve L'*X = sub( B ), overwriting sub( B ) with X.
*
         CALL PYTRSM( 'Left', 'Lower', TRANS, 'Unit', N, NRHS, ONE,     
     +            A, IA, JA, DESCA, B, IB, JB, DESCB )
*
*        Apply row interchanges to the solution vectors.
*
         CALL PYLAPIV( 'Backward', 'Row', 'Col', N, NRHS, B, IB, JB,    
     +              DESCB, IPIV, IA, 1, DESCIP, IDUM1 )
*
      END IF
*
