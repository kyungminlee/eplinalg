      SUBROUTINE ESTEGR2B( JOBZ, N, D, E,                     M, W, Z, 
     +LDZ, NZC, ISUPPZ, WORK, LWORK, IWORK,                    LIWORK, 
     +DOL, DOU, NEEDIL, NEEDIU,                    INDWLC, PIVMIN, 
     +SCALE, WL, WU,                    VSTART, FINISH, MAXCLS,         
...
*     .. Scalar Arguments ..
     ...
     $                   ZOFFSET
      ...
      REAL(KIND=10) PIVMIN, SCALE, WL, WU
...
*  =======
*
*  ESTEGR2B should only be called after a call to ESTEGR2A.
*  From eigenvalues and initial representations computed by ESTEGR2A,
*  ESTEGR2B computes the selected eigenvalues and eigenvectors
...
*     ESTEGR2B  ONLY computes the eigenVECTORS 
...
*
*  3. ESTEGR2B needs more workspace in Z than the sequential ESTEGR. 
...
*          in ESTEGR2A.  0 <= M <= N.
*
*  W       (input) REAL(KIND=10) array, dimension (N)
...
*          still to be computed. Initially computed by ELARRE2A,
*          modified in the course of the algorithm.
*
*  INDWLC  (output) REAL(KIND=10)
...
*                if INFO = 20X, internal error in ELARRV2.
*                Here, the digit X = ABS( IINFO ) < 10, where IINFO is 
*                the nonzero error code returned by ELARRV2.
*
*     .. Parameters ..
      REAL(KIND=10)   ONE, FOUR, MINRGP
      PARAMETER          ( ONE = 1.0E0_10,                      FOUR = 
     +4.0E0_10,                      MINRGP = 1.0E-3_10 )
*     ..
*     .. Local Scalars ..
...
*     ESTEGR2B needs WORK of size 6*N, IWORK of size 3*N.
*     In addition, ELARRE2A needed WORK of size 6*N, IWORK of size 5*N.
*     Workspace is kept consistent even though ELARRE2A is not called here.
...
*        eigenvalues
*
         CALL ELARRV2( N, WL, WU, D, E,                 PIVMIN, IWORK( 
     +IINSPL ), M,                  DOL, DOU, NEEDIL, NEEDIU, MINRGP, 
     +RTOL1, RTOL2,                  W, WORK( INDERR ), WORK( INDGP ), 
     +IWORK( IINDBL ),                 IWORK( IINDW ), WORK( INDGRS ),  
     +                WORK( INDSDM ), Z, LDZ,                 ISUPPZ, 
     +WORK( INDWRK ), IWORK( IINDWK ),                  VSTART, FINISH, 
...
*
      ELSE
*        ELARRE2A computed eigenvalues of the (shifted) root representation
*        ELARRV2 returns the eigenvalues of the unshifted matrix.
*        However, if the eigenvectors are not desired by the user, we need
*        to apply the corresponding shifts from ELARRE2A to obtain the 
