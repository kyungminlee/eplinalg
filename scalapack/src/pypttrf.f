      SUBROUTINE PYPTTRF( N, D, E, JA, DESCA, AF, LAF, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
*
...
*  =======
*
*  PYPTTRF computes a Cholesky factorization
...
*  subsequent calls to PYPTTRS to solve linear systems.
*
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,      
     + 'PYPTTRF, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*INT_ONE )) THEN
         INFO = -( 5*100+4 )
         CALL PXERBLA( ICTXT,       'PYPTTRF, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT,       'PYPTTRF: auxiliary storage error ',
     +       -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
           INFO = -9
           CALL PXERBLA( ICTXT,       'PYPTTRF: worksize error ',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPTTRF', -INFO )
         RETURN
      END IF
*
...
*
        CALL YPTTRF( ODD_SIZE, D( PART_OFFSET+1 ), E( PART_OFFSET+1 ),  
     +                   INFO )
*
        IF( INFO.NE.0 ) THEN
           INFO = MYCOL+1
...
*
     ...
     +    D( PART_OFFSET+ODD_SIZE )*REAL( E( PART_OFFSET+ODD_SIZE )*    
     +    CONJG( E( PART_OFFSET+ODD_SIZE ) ) , KIND=10)
*
        ENDIF
...
*
          CALL YPTTRSV( 'L', 'N', ODD_SIZE, INT_ONE, D( PART_OFFSET+1 ),
