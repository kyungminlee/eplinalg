      SUBROUTINE PYLAHRD( N, K, NB, A, IA, JA, DESCA, TAU, T, Y, IY, JY,
     +                     DESCY, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLAHRD reduces the first NB columns of a complex general
...
*
*  This is an auxiliary routine called by PYGEHRD. In the following
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YAXPY, YCOPY, YSCAL,           
     +         YTRMV, DESCSET, INFOG2L, PYELSET,                    
     +PYGEMV, PYLACGV, PYLARFG, PYSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MIN, MOD
*     ..
...
*
            CALL PYLACGV( L-1, A, I, JA, DESCA, DESCA( M_ ) )
            CALL PYGEMV( 'No transpose', N, L-1, -ONE, Y, IY, JY, DESCY,
...
*
            IF( IPROC ) THEN
               CALL YCOPY( L-1, A( (JJ+L-2)*DESCA( LLD_ )+II ), 1,      
     +                WORK( JW ), 1 )
               CALL YTRMV( 'Lower', 'Conjugate transpose', 'Unit', L-1, 
...
*           w := w + V2'*b2
*
            CALL PYGEMV( 'Conjugate transpose', N-K-L+1, L-1, ONE, A,   
     +                 I+1, JA, DESCA, A, I+1, J, DESCA, 1, ONE, WORK,  
     +                  1, JW, DESCW, DESCW( M_ ) )
*
*           w := T'*w
*
            IF( IPROC )          CALL YTRMV( 'Upper', 
     +'Conjugate transpose', 'Non-unit',                      L-1, T, 
     +DESCA( NB_ ), WORK( JW ), 1 )
*
*           b2 := b2 - V2*w
*
            CALL PYGEMV( 'No transpose', N-K-L+1, L-1, -ONE, A, I+1, JA,
...
*
            IF( IPROC ) THEN
               CALL YTRMV( 'Lower', 'No transpose', 'Unit', L-1,        
     +              A( (JJ-1)*DESCA( LLD_ )+II ), DESCA( LLD_ ),        
     +              WORK( JW ), 1 )
               CALL YAXPY( L-1, -ONE, WORK( JW ), 1,                    
     +  A( ( JJ+L-2 )*DESCA( LLD_ )+II ), 1 )
            END IF
            CALL PYELSET( A, I, J-1, DESCA, EI )
         END IF
*
*        Generate the elementary reflector H(i) to annihilate
*        A(ia+k+i:ia+n-1,j)
*
         CALL PYLARFG( N-K-L+1, EI, I+1, J, A, MIN( I+2, N+IA-1 ), J,   
     +               DESCA, 1, TAU )
         CALL PYELSET( A, I+1, J, DESCA, ONE )
*
*        Compute  Y(iy:y+n-1,jy+l-1)
*
         CALL PYGEMV( 'No transpose', N, N-K-L+1, ONE, A, IA, J+1,      
     +           DESCA, A, I+1, J, DESCA, 1, ZERO, Y, IY, JY+L-1,       
     +          DESCY, 1 )
         CALL PYGEMV( 'Conjugate transpose', N-K-L+1, L-1, ONE, A, I+1, 
     ...
     +         JY+L-1, DESCY, 1 )
         ...
         CALL PYSCAL( N, TAU( JL ), Y, IY, JY+L-1, DESCY, 1 )
*
*        Compute T(1:i,i)
*
         IF( IPROC ) THEN
            JT = ( L-1 ) * DESCA( NB_ )
            CALL YSCAL( L-1, -TAU( JL ), WORK( JW ), 1 )
            CALL YCOPY( L-1, WORK( JW ), 1, T( JT+1 ), 1 )
            CALL YTRMV( 'Upper', 'No transpose', 'Non-unit', L-1, T,    
     +               DESCA( NB_ ), T( JT+1 ), 1 )
            T( JT+L ) = TAU( JL )
         END IF
   10 CONTINUE
*
      CALL PYELSET( A, K+NB+IA-1, J, DESCA, EI )
*
      RETURN
*
*     End of PYLAHRD
