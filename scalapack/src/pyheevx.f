      SUBROUTINE PYHEEVX( JOBZ, RANGE, UPLO, N, A, IA, JA, DESCA, VL,   
...
*  =======
*
*  PYHEEVX computes selected eigenvalues and, optionally, eigenvectors
...
*
*  PYHEEVX assumes IEEE 754 standard compliant arithmetic.  To port
...
*          If DESCA( CTXT_ ) is incorrect, PYHEEVX cannot guarantee
...
*          insufficient space and PYHEEVX is not able to detect this
...
*          PYHEEVX is always able to detect insufficient space without
...
*             PYHEEVX attempts to maintain orthogonality in
...
*             is returned.  Note that when RANGE='V', PYHEEVX does
*             not know how many eigenvectors are requested until
*             the eigenvalues are computed.  Therefore, when RANGE='V'
*             and as long as LRWORK is large enough to allow PYHEEVX to
*             compute the eigenvalues, PYHEEVX will compute the
...
*             PYSTEIN will perform no better than YSTEIN on 1
*             processor.
...
*                  PYHEEVX from computing all of the eigenvectors
...
*  =====================================================================
*
*  Differences between PYHEEVX and YHEEVX
*  ======================================
*
*  A, LDA -> A, IA, JA, DESCA
*  Z, LDZ -> Z, IZ, JZ, DESCZ
*  WORKSPACE needs are larger for PYHEEVX.
...
*  PYHEEVX does not promise orthogonality for eigenvectors associated
*  with tighly clustered eigenvalues.
*  PYHEEVX does not reorthogonalize eigenvectors
...
*     .. Local Scalars ..
     ...
     $                   SIZESTEIN, SQNPC
      REAL(KIND=10)               ABSTLL, ANRM, BIGNUM, EPS, RMAX, RMIN,
     + SAFMIN,                    SIGMA, SMLNUM, VLL, VUU
*     ..
*     .. Local Arrays ..
...
*     .. External Functions ..
      ...
      REAL(KIND=10)               PYLANHE, PELAMCH
      EXTERNAL           LSAME, ICEIL, INDXG2P, NUMROC, PJLAENV_EP,     
     +               PYLANHE, PELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, IGAMN2D, PYELGET,     
     +               PYHENTRD, PCHK1MAT, PCHK2MAT, PYLASCL, PYSTEIN,    
     +                PYUNMTR, PELARED1D, PESTEBZ, PXERBLA, EGEBR2D,    
     +                EGEBS2D, ELASRT, ESCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, CMPLX, REAL, ICHAR, INT, MAX, MIN, MOD,
     $                   SQRT
*     ..
         ...
         IF( INFO.EQ.0 ) THEN
            IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
               ...
               ELSE
                  ...
                  RWORK( 3 ) = ZERO
               END IF
               CALL EGEBS2D( ICTXT, 'ALL', ' ', 3, 1, RWORK, 3 )
            ELSE
               CALL EGEBR2D( ICTXT, 'ALL', ' ', 3, 1, RWORK, 3, 0, 0 )
            END IF
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHEEVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      ...
      END IF
*
      ANRM = PYLANHE( 'M', UPLO, N, A, IA, JA, DESCA,        RWORK( 
     +INDRWORK ) )
*
...
*
      IF( ISCALE.EQ.1 ) THEN
         CALL PYLASCL( UPLO, ONE, SIGMA, N, N, A, IA, JA, DESCA, IINFO )
         IF( ABSTOL.GT.0 )
     $      ABSTLL = ABSTOL*SIGMA
         IF( VALEIG ) THEN
            VLL = VL*SIGMA
      ...
      END IF
*
*     Call PYHENTRD to reduce Hermitian matrix to tridiagonal form.
*
      LALLWORK = LLRWORK
*
...
*
     ...
     $      OFFSET = 1
      ELSE
         DO 10 I = 1, N
            CALL PYELGET( 'A', ' ', WORK( INDD2+I-1 ), A, I+IA-1,       
     +              I+JA-1, DESCA )
            RWORK( INDD2+I-1 ) = REAL( WORK( INDD2+I-1 ) , KIND=10)
   10    CONTINUE
         IF( LSAME( UPLO, 'U' ) ) THEN
            DO 20 I = 1, N - 1
               CALL PYELGET( 'A', ' ', WORK( INDE2+I-1 ), A, I+IA-1,    
     +                    I+JA, DESCA )
               RWORK( INDE2+I-1 ) = REAL( WORK( INDE2+I-1 ) , KIND=10)
   20       CONTINUE
         ELSE
            DO 30 I = 1, N - 1
               CALL PYELGET( 'A', ' ', WORK( INDE2+I-1 ), A, I+IA,      
     +                  I+JA-1, DESCA )
               RWORK( INDE2+I-1 ) = REAL( WORK( INDE2+I-1 ) , KIND=10)
   30       CONTINUE
         END IF
      END IF
*
*     Call PESTEBZ and, if eigenvectors are desired, PYSTEIN.
*
      IF( WANTZ ) THEN
         ORDER = 'B'
      ELSE
...
*           NZ that we can feed to PYSTEIN and PYUNMTR
*
...
*
            END IF
         END IF
         CALL PYSTEIN( N, RWORK( INDD2 ), RWORK( INDE2+OFFSET ), NZ, W, 
