      SUBROUTINE PEGGQRF( N, M, P, A, IA, JA, DESCA, TAUA, B, IB, JB,   
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCB( * )
      REAL(KIND=10)   A( * ), B( * ), TAUA( * ), TAUB( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PEGGQRF computes a generalized QR factorization of
...
*
*  TAUA    (local output) REAL(KIND=10) array, dimension
...
*
*  TAUB    (local output) REAL(KIND=10) array, dimension LOCr(IB+N-1)
...
*  To form Q explicitly, use ScaLAPACK subroutine PEORGQR.
*  To use Q to update another matrix, use ScaLAPACK subroutine PEORMQR.
*
...
*  To form Z explicitly, use ScaLAPACK subroutine PEORGRQ.
*  To use Z to update another matrix, use ScaLAPACK subroutine PEORMRQ.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PEGEQRF,    
     +                PEGERQF, PEORMQR, PXERBLA
*     ..
*     .. Local Arrays ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGGQRF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PEGEQRF( N, M, A, IA, JA, DESCA, TAUA, WORK, LWORK, INFO )
      LWMIN = INT( WORK( 1 ) )
*
*     Update sub( B ) := Q'*sub( B ).
*
      CALL PEORMQR( 'Left', 'Transpose', N, P, MIN( N, M ), A, IA, JA,  
     +             DESCA, TAUA, B, IB, JB, DESCB, WORK, LWORK, INFO )
      LWMIN = MIN( LWMIN, INT( WORK( 1 ) ) )
*
*     RQ factorization of N-by-P matrix sub( B ): sub( B ) = T*Z.
*
      CALL PEGERQF( N, P, B, IB, JB, DESCB, TAUB, WORK, LWORK, INFO )
      WORK( 1 ) = REAL( MAX( LWMIN, INT( WORK( 1 ) ) ) , KIND=10)
*
      RETURN
*
*     End of PEGGQRF
