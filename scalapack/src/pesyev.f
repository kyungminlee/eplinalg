...
*  =======
*
*  PESYEV computes all eigenvalues and, optionally, eigenvectors
*  of a real symmetric matrix A by calling the recommended sequence
*  of ScaLAPACK routines.
*
*  In its present form, PESYEV assumes a homogeneous system and makes
...
*          If DESCA( CTXT_ ) is incorrect, PESYEV cannot guarantee
...
*          where
*             SIZESYTRD = The workspace requirement for PESYTRD
...
*             LDC = MAX( 1, NRC )
*             SIZEMQRLEFT = The workspace requirement for PEORMTR
...
*                converge in ESTEQR2 after a total of 30*N iterations.
*                If INFO = N+1, then PESYEV has detected heterogeneity
*                by finding that eigenvalues were not identical across
*                the process grid.  In this case, the accuracy of
*                the results from PESYEV cannot be guaranteed.
*
...
*     .. Parameters ..
     ...
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      REAL(KIND=10)   FIVE, ONE, TEN, ZERO
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10,             
     +         TEN = 10.0E0_10, FIVE = 5.0E0_10 )
...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME, NUMROC, PELAMCH, PELANSY,               
     +     SL_GRIDRESHAPE
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO, CHK1MAT, ECOPY,
     +                    DESCINIT, ESCAL, ESTEQR2, PCHK1MAT, PCHK2MAT, 
     +                   PEELGET, PEGEMR2D, PELASCL, PELASET, PEORMTR,  
     +                  PESYTRD, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, ICHAR, MAX, MIN, MOD, SQRT, INT
*     ..
...
*           matrix required by ESTEQR2. This specific distribution
*           allows each process to do 1/pth of the work updating matrix
*           Q during ESTEQR2 and achieve some parallelization to an
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PESYEV', -INFO )
...
*
      CALL PESYTRD( UPLO, N, A, IA, JA, DESCA, WORK( INDD ),            
...
*
      DO 10 I=1,N
         CALL PEELGET( 'A', ' ', WORK(INDD2+I-1), A,                  
     +I+IA-1, I+JA-1, DESCA )
 10   CONTINUE
      IF( LSAME( UPLO, 'U') ) THEN
          DO 20 I=1,N-1
             CALL PEELGET( 'A', ' ', WORK(INDE2+I-1), A,                
     +       I+IA-1, I+JA, DESCA )
 20       CONTINUE
      ELSE
          DO 30 I=1,N-1
...
*
      IF( WANTZ ) THEN
*
         CALL PELASET( 'Full', N, N, ZERO, ONE, WORK( INDWORK ), 1, 1,  
     +                DESCQR )
*
*        ESTEQR2 is a modified version of LAPACK's ESTEQR.  The
*        modifications allow each process to perform partial updates
*        to matrix Q.
*
         CALL ESTEQR2( 'I', N, WORK( INDD2 ), WORK( INDE2 ),            
     +      WORK( INDWORK ), LDC, NRC, WORK( INDWORK2 ),                
     +   INFO )
*
         CALL PEGEMR2D( N, N, WORK( INDWORK ), 1, 1, DESCQR, Z, IA, JA, 
     +                  DESCZ, CONTEXTC )
*
         CALL PEORMTR( 'L', UPLO, 'N', N, N, A, IA, JA, DESCA,          
     +        WORK( INDTAU ), Z, IZ, JZ, DESCZ,                  WORK( 
     +INDWORK ), LLWORK, IINFO )
*
      ELSE
*
         CALL ESTEQR2( 'N', N, WORK( INDD2 ), WORK( INDE2 ),            
...
*
 ...
 40   CONTINUE
*
      CALL EGAMN2D( DESCA( CTXT_ ), 'a', ' ', J, 1, WORK( 1+INDTAU ),   
     +            J, 1, 1, -1, -1, 0 )
      CALL EGAMX2D( DESCA( CTXT_ ), 'a', ' ', J, 1, WORK( 1+INDE ),     
     +          J, 1, 1, -1, -1, 0 )
*
      DO 50 I = 1, J
