      SUBROUTINE PEGGRQF( M, P, N, A, IA, JA, DESCA, TAUA, B, IB, JB,   
...
*  =======
*
*  PEGGRQF computes a generalized RQ factorization of
...
*
*  TAUA    (local output) REAL(KIND=10) array, dimension LOCr(IA+M-1)
...
*
*  TAUB    (local output) REAL(KIND=10) array, dimension
...
*  To form Q explicitly, use ScaLAPACK subroutine PEORGRQ.
*  To use Q to update another matrix, use ScaLAPACK subroutine PEORMRQ.
*
...
*  To form Z explicitly, use ScaLAPACK subroutine PEORGQR.
*  To use Z to update another matrix, use ScaLAPACK subroutine PEORMQR.
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGGRQF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PEGERQF( M, N, A, IA, JA, DESCA, TAUA, WORK, LWORK, INFO )
      LWMIN = INT( WORK( 1 ) )
*
*     Update sub( B ) := sub( B )*Q'
*
      CALL PEORMRQ( 'Right', 'Transpose', P, N, MIN( M, N ), A,         
...
*
      CALL PEGEQRF( P, N, B, IB, JB, DESCB, TAUB, WORK, LWORK, INFO )
