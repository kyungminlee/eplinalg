      SUBROUTINE PELAED3( ICTXT, K, N, NB, D, DROW, DCOL, RHO, DLAMDA,  
...
*     .. Array Arguments ..
     ...
     $                   INDROW( * ), INDX( * ), INDXC( * ), INDXR( * )
      REAL(KIND=10)   BUF( * ), D( * ), DLAMDA( * ), U( LDU, * ),       
     +             W( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PELAED3 finds the roots of the secular equation, as defined by the
*  values in D, W, and RHO, between 1 and K.  It makes the
*  appropriate calls to ELAED4
*
...
*         ELAED4 to form the secular equation.
*
*  W      (global output) REAL(KIND=10) array, dimension (N)
*         The first k values of the final deflation-altered z-vector
*         which will be passed to ELAED4.
*
...
*         process.
*
*  U     (global output) REAL(KIND=10) array
*         global dimension (N, N), local dimension (LDU, NQ).
*         (See PELAED0 for definition of NQ.)
...
*
*  BUF    (workspace) REAL(KIND=10) array, dimension 3*N
*
*
...
*
      DO 50 I = 1, K
         DLAMDA( I ) = ELAMC3( DLAMDA( I ), DLAMDA( I ) ) - DLAMDA( I )
         Z( I ) = ONE
   50 CONTINUE
      IF( MYKLR.GT.0 ) THEN
         KK = PDR
         DO 80 I = 1, MYKLR
            CALL ELAED4( K, KK, DLAMDA, W, BUF, RHO, BUF( K+I ), IINFO )
            IF( IINFO.NE.0 ) THEN
               INFO = KK
            END IF
*
...
*
         IF( MYROW.NE.DROW ) THEN
            CALL ECOPY( K, Z, 1, BUF, 1 )
            CALL EGESD2D( ICTXT, K+MYKLR, 1, BUF, K+MYKLR, DROW, MYCOL )
         ELSE
            IPD = 2*K + 1
            CALL ECOPY( MYKLR, BUF( K+1 ), 1, BUF( IPD ), 1 )
            IF( KLR.GT.0 ) THEN
               IPD = MYKLR + IPD
               ROW = MOD( DROW+1, NPROW )
               DO 100 I = 1, NPROW - 1
                  CALL EGERV2D( ICTXT, K+KLR, 1, BUF, K+KLR, ROW,       
     +                    MYCOL )
                  CALL ECOPY( KLR, BUF( K+1 ), 1, BUF( IPD ), 1 )
                  DO 90 J = 1, K
                     Z( J ) = Z( J )*BUF( J )
   90             CONTINUE
                  IPD = IPD + KLR
...
*
      IF( MYROW.EQ.DROW ) THEN
         IF( MYCOL.NE.DCOL .AND. MYKL.NE.0 ) THEN
            CALL ECOPY( K, Z, 1, BUF, 1 )
            CALL ECOPY( MYKL, BUF( 2*K+1 ), 1, BUF( K+1 ), 1 )
            CALL EGESD2D( ICTXT, K+MYKL, 1, BUF, K+MYKL, MYROW, DCOL )
         ELSE IF( MYCOL.EQ.DCOL ) THEN
            ...
            DO 120 I = 1, NPCOL - 1
               ...
               IF( KL.NE.0 ) THEN
                  CALL EGERV2D( ICTXT, K+KL, 1, BUF, K+KL, MYROW, COL )
                  CALL ECOPY( KL, BUF( K+1 ), 1, BUF( IPD ), 1 )
                  DO 110 J = 1, K
                     Z( J ) = Z( J )*BUF( J )
  110             CONTINUE
               END IF
...
*
      IF( MYROW.EQ.DROW .AND. MYCOL.EQ.DCOL ) THEN
         CALL ECOPY( K, Z, 1, BUF, 1 )
         CALL ECOPY( K, BUF( 2*K+1 ), 1, BUF( K+1 ), 1 )
         CALL EGEBS2D( ICTXT, 'All', ' ', 2*K, 1, BUF, 2*K )
      ELSE
         CALL EGEBR2D( ICTXT, 'All', ' ', 2*K, 1, BUF, 2*K, DROW, DCOL )
         CALL ECOPY( K, BUF, 1, Z, 1 )
      END IF
*
...
*
      IF( MYKL.NE.0 ) THEN
         DO 180 J = 1, MYKL
            ...
            CALL ELAED4( K, KK, DLAMDA, W, BUF, RHO, AUX, IINFO )
            IF( IINFO.NE.0 ) THEN
               INFO = KK
            END IF
