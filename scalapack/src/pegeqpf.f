      SUBROUTINE PEGEQPF( M, N, A, IA, JA, DESCA, IPIV, TAU, WORK,      
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), IPIV( * )
      REAL(KIND=10)   A( * ), TAU( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PEGEQPF computes a QR factorization with column pivoting of a
...
*     .. Local Scalars ..
     ...
     $                   MYCOL, MYROW, NPCOL, NPROW, NQ, NQ0, PVT
      REAL(KIND=10)   AJJ, ALPHA, TEMP, TEMP2, TOL3Z
*     ..
*     .. Local Arrays ..
      INTEGER            DESCN( DLEN_ ), IDUM1( 1 ), IDUM2( 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, ECOPY, DESCSET,       
     +             EGEBR2D, EGEBS2D, EGERV2D,                    
     +EGESD2D, ELARFG, ESWAP, IGERV2D,                    IGESD2D, 
     +INFOG1L, INFOG2L, PCHK1MAT, PEAMAX,                    PEELSET, 
     +PELARF, PELARFG, PENRM2,                    PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEQPF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      ...
      IF( MYCOL.EQ.IACOL ) THEN
         DO 60 KK = 0, JB-1
            CALL PENRM2( M, WORK( JJ+KK ), A, IA, JA+KK, DESCA, 1 )
...
*
         IF( MYCOL.EQ.ICURCOL ) THEN
            DO 70 KK = 0, JB-1
               CALL PENRM2( M, WORK( JJ+KK ), A, IA, J+KK, DESCA, 1 )
               WORK( NQ+JJ+KK ) = WORK( JJ+KK )
   70       CONTINUE
            JJ = JJ + JB
         END IF
...
*
     ...
     $                 JJ, ICURCOL )
         K = JA + N - J
         IF( K.GT.1 ) THEN
            CALL PEAMAX( K, TEMP, PVT, WORK( IPN ), 1, J, DESCN,        
     +            DESCN( M_ ) )
         ELSE
            PVT = J
         END IF
     ...
     $                    DESCA( CSRC_ ), JJPVT, IPCOL )
            IF( ICURCOL.EQ.IPCOL ) THEN
               IF( MYCOL.EQ.ICURCOL ) THEN
                  CALL ESWAP( MP, A( IIA+(JJ-1)*LDA ), 1,               
     +          A( IIA+(JJPVT-1)*LDA ), 1 )
                  ITEMP = IPIV( JJPVT )
                  IPIV( JJPVT ) = IPIV( JJ )
                  IPIV( JJ ) = ITEMP
            ...
            ELSE
               IF( MYCOL.EQ.ICURCOL ) THEN
*
                  CALL EGESD2D( ICTXT, MP, 1, A( IIA+(JJ-1)*LDA ), LDA, 
     +                          MYROW, IPCOL )
                  WORK( IPW )   = REAL( IPIV( JJ ) , KIND=10)
                  WORK( IPW+1 ) = WORK( IPN + JJ - 1 )
                  WORK( IPW+2 ) = WORK( IPN + NQ + JJ - 1 )
                  CALL EGESD2D( ICTXT, 3, 1, WORK( IPW ), 3, MYROW,     
     +                      IPCOL )
*
                  CALL EGERV2D( ICTXT, MP, 1, A( IIA+(JJ-1)*LDA ), LDA, 
     +                          MYROW, IPCOL )
...
*
                  CALL EGESD2D( ICTXT, MP, 1, A( IIA+(JJPVT-1)*LDA ),   
     +                        LDA, MYROW, ICURCOL )
                  CALL IGESD2D( ICTXT, 1, 1, IPIV( JJPVT ), 1, MYROW,
     $                          ICURCOL )
*
                  CALL EGERV2D( ICTXT, MP, 1, A( IIA+(JJPVT-1)*LDA ),   
     +                        LDA, MYROW, ICURCOL )
                  CALL EGERV2D( ICTXT, 3, 1, WORK( IPW ), 3, MYROW,     
     +                      ICURCOL )
                  IPIV( JJPVT ) = INT( WORK( IPW ) )
...
*
         ...
         IF( DESCA( M_ ).EQ.1 ) THEN
            IF( MYROW.EQ.ICURROW ) THEN
               IF( MYCOL.EQ.ICURCOL ) THEN
                  ...
                  AJJ = A( IOFFA )
                  CALL ELARFG( 1, AJJ, A( IOFFA ), 1, TAU( JJ ) )
                  IF( N.GT.1 ) THEN
                     ALPHA = ONE - TAU( JJ )
                     CALL EGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA,  
     +                                 1 )
                     CALL ESCAL( NQ-JJ, ALPHA, A( IOFFA+DESCA( LLD_ ) ),
     +                            DESCA( LLD_ ) )
                  END IF
     ...
     +                  TAU( JJ ), 1 )
               ...
               ELSE
                  IF( N.GT.1 ) THEN
                     CALL EGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA,  
     +                            1, ICURROW, ICURCOL )
                     CALL ESCAL( NQ-JJ+1, ALPHA, A( I ), DESCA( LLD_ ) )
                  END IF
               END IF
            ELSE IF( MYCOL.EQ.ICURCOL ) THEN
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAU( JJ ), 
     +                       1, ICURROW, ICURCOL )
            END IF
*
         ELSE
*
            CALL PELARFG( M-J+JA, AJJ, I, J, A, MIN( I+1, IA+M-1 ), J,  
     +                   DESCA, 1, TAU )
            IF( J.LT.JA+N-1 ) THEN
*
...
*
               ...
               CALL PELARF( 'Left', M-J+JA, JA+N-1-J, A, I, J, DESCA,   
     +                    1, TAU, A, I, J+1, DESCA, WORK( IPW ) )
            END IF
            CALL PEELSET( A, I, J, DESCA, AJJ )
*
         END IF
*
...
*
     ...
     $      ICURCOL = MOD( ICURCOL+1, NPCOL )
         IF( (JJA+NQ-JJ).GT.0 ) THEN
            IF( MYROW.EQ.ICURROW ) THEN
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, JJA+NQ-JJ,    
     +                    A( II+( MIN( JJA+NQ-1, JJ )-1 )*LDA ),        
     +                LDA )
               CALL ECOPY( JJA+NQ-JJ, A( II+( MIN( JJA+NQ-1, JJ )       
     +               -1)*LDA ), LDA, WORK( IPW+MIN( JJA+NQ-1,           
     +          JJ )-1 ), 1 )
            ELSE
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', JJA+NQ-JJ, 1,    
            ...
            DO 90 LL = JJ-1, JJ + JN - J - 2
               IF( WORK( IPN+LL ).NE.ZERO ) THEN
                  ...
                  IF( TEMP2.LE.TOL3Z ) THEN
                     IF( IA+M-1.GT.I ) THEN
                        CALL PENRM2( IA+M-I-1, WORK( IPN+LL ), A, I+1,  
     +                              J+LL-JJ+2, DESCA, 1 )
...
*
     ...
     $                         ( WORK( IPN+LL ) / WORK( IPN+NQ+LL ) )**2
                     IF( TEMP2.LE.TOL3Z ) THEN
                        IF( IA+M-1.GT.I ) THEN
                           CALL PENRM2( IA+M-I-1, WORK( IPN+LL ), A,    
