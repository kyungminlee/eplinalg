      SUBROUTINE PEPOTF2( UPLO, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEPOTF2 computes the Cholesky factorization of a real symmetric
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, EGEMV,   
     +                 ESCAL, IGEBR2D, IGEBS2D, INFOG2L, PB_TOPGET,     
     +               PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD, SQRT
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPOTF2', -INFO )
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + LDA
                     CALL EGEMV( 'Transpose', J-JA, JA+N-J-1, -ONE,     
     +                       A( IOFFA+LDA ), LDA, A( IOFFA ), 1,        
     +                    ONE, A( ICURR ), LDA )
                     CALL ESCAL( N-J+JA-1, ONE / AJJ, A( ICURR ), LDA )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + LDA
   10          CONTINUE
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + 1
                     CALL EGEMV( 'No transpose', JA+N-J-1, J-JA, -ONE,  
     +                          A( IOFFA+1 ), LDA, A( IOFFA ), LDA,     
     +                       ONE, A( ICURR ), 1 )
                     CALL ESCAL( JA+N-J-1, ONE / AJJ, A( ICURR ), 1 )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + 1
   30          CONTINUE
