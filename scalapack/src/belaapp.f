      SUBROUTINE BELAAPP( ISIDE, M, N, NB, A, LDA, NITRAF, ITRAF,       
     +              DTRAF, WORK )
      IMPLICIT NONE
*
*     .. Scalar Arguments ..
...
*
*  A       (input/output) REAL(KIND=10) array, dimension (LDA,N)
...
*
                  CALL EROT( NNB, A(IT,J), LDA, A(IT+1,J), LDA,         
     +               DTRAF(PD), DTRAF(PD+1) )
                  PD = PD + 2
...
*
                  TAU = DTRAF(PD)
                  DTRAF(PD) = ONE
                  CALL ELARFX( 'Left', 3, NNB, DTRAF(PD), TAU,          
     +                A(IT-M,J), LDA, WORK )
                  DTRAF(PD) = TAU
                  PD = PD + 3
               ELSE
...
*              Apply Givens rotation.
*
               CALL EROT( M, A(1,IT), 1, A(1,IT+1), 1, DTRAF(PD),       
     +              DTRAF(PD+1) )
               PD = PD + 2
...
*
               TAU = DTRAF(PD)
               DTRAF(PD) = ONE
               CALL ELARFX( 'Right', M, 3, DTRAF(PD), TAU, A(1,IT-N),   
     +                    LDA, WORK )
               DTRAF(PD) = TAU
               PD = PD + 3
            ELSE
...
*
               TAU = DTRAF(PD+2)
               DTRAF(PD+2) = ONE
               CALL ELARFX( 'Right', M, 3, DTRAF(PD), TAU, A(1,IT-2*N), 
     +                      LDA, WORK )
               DTRAF(PD+2) = TAU
               PD = PD + 3
            END IF
