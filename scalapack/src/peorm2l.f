      SUBROUTINE PEORM2L( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*  =======
*
*  PEORM2L overwrites the general real M-by-N distributed matrix
...
*        Q = H(k) . . . H(2) H(1)
*
*  as returned by PEGEQLF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          PEGEQLF in the K columns of its distributed matrix
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension LOCc(JA+N-1)
*          This array contains the scalar factors TAU(j) of the
*          elementary reflectors H(j) as returned by PEGEQLF.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, EGEBR2D, 
     +                   EGEBS2D, EGERV2D, EGESD2D, ESCAL,              
     +      INFOG2L, PEELSET, PEELSET2, PELARF,                    
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORM2L', -INFO )
...
*
     ...
     $                      NPCOL )
               IF( MYCOL.EQ.IACOL ) THEN
                  AJJ = ONE - TAU( JJ )
                  CALL EGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, AJJ, 1 )
                  CALL ESCAL( NQ-JCC+1, AJJ,                         C( 
     +ICC+(JCC-1)*DESCC( LLD_ ) ),                         DESCC( LLD_ 
     +) )
               ELSE
                  CALL EGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, AJJ, 1,    
     +                       IAROW, IACOL )
                  CALL ESCAL( NQ-JCC+1, AJJ,                         C( 
     +ICC+(JCC-1)*DESCC( LLD_ ) ),                         DESCC( LLD_ 
     +) )
...
*
            IF( IACOL.NE.ICCOL ) THEN
               IF( MYCOL.EQ.IACOL )             CALL EGESD2D( ICTXT, 1, 
     +1, AJJ, 1, MYROW, ICCOL )
               IF( MYCOL.EQ.ICCOL )             CALL EGERV2D( ICTXT, 1, 
     +1, AJJ, 1, MYROW, IACOL )
            END IF
*
     ...
     $                      NPROW )
               CALL ESCAL( MP-ICC+1, AJJ, C( ICC+(JCC-1)*               
     +       DESCC( LLD_ ) ), 1 )
            END IF
*
         END IF
...
*           Apply H(j) or H(j)'
*
            CALL PEELSET2( AJJ, A, IA+NQ-K+J-JA, J, DESCA, ONE )
            CALL PELARF( SIDE, MI, NI, A, IA, J, DESCA, 1, TAU, C, IC,  
     +                  JC, DESCC, WORK )
            CALL PEELSET( A, IA+NQ-K+J-JA, J, DESCA, AJJ )
*
   10    CONTINUE
*
