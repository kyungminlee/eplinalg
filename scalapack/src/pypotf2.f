      SUBROUTINE PYPOTF2( UPLO, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYPOTF2 computes the Cholesky factorization of a complex hermitian
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, YGEMV,   
     +                 YLACGV, YESCAL, IGEBR2D, IGEBS2D,                
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPOTF2', -INFO )
...
*
                  AJJ = REAL( A( IDIAG ) , KIND=10) -                   
     +YDOTC( J-JA, A( IOFFA ), 1, A( IOFFA ), 1 )
                  IF( AJJ.LE.ZERO ) THEN
                     A( IDIAG ) = AJJ
                     INFO = J - JA + 1
                     GO TO 20
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + LDA
                     CALL YLACGV( J-JA, A( IOFFA ), 1 )
                     CALL YGEMV( 'Transpose', J-JA, JA+N-J-1, -CONE,    
     +                        A( IOFFA+LDA ), LDA, A( IOFFA ), 1,       
     +                     CONE, A( ICURR ), LDA )
                     CALL YLACGV( J-JA, A( IOFFA ), 1 )
                     CALL YESCAL( JA+N-J-1, ONE / AJJ, A( ICURR ),      
     +                       LDA )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + LDA
...
*
                  AJJ = REAL( A( IDIAG ) , KIND=10) -                   
     +YDOTC( J-JA, A( IOFFA ), LDA, A( IOFFA ), LDA )
                  IF ( AJJ.LE.ZERO ) THEN
                     A( IDIAG ) = AJJ
                     INFO = J - JA + 1
                     GO TO 40
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + 1
                     CALL YLACGV( J-JA, A( IOFFA ), LDA )
                     CALL YGEMV( 'No transpose', JA+N-J-1, J-JA, -CONE, 
     +                           A( IOFFA+1 ), LDA, A( IOFFA ), LDA,    
     +                        CONE, A( ICURR ), 1 )
                     CALL YLACGV( J-JA, A( IOFFA ), LDA )
                     CALL YESCAL( JA+N-J-1, ONE / AJJ, A( ICURR ), 1 )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + 1
   30          CONTINUE
