      SUBROUTINE PYGBTRF( N, BWL, BWU, A, JA, DESCA, IPIV, AF, LAF,     
     +                WORK, LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGBTRF computes a LU factorization
...
*  subsequent calls to PYGBTRS to solve linear systems.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO, YAXPY, YGEMM,  
     +                  YGERV2D, YGESD2D, YLAMOV, YLATCPY, YPBTRF,      
     +              YPOTRF, YSYRK, YTBTRS, YTRMM, YTRRV2D, YTRSD2D,     
     +               YTRSM, YTRTRS, DESC_CONVERT, GLOBCHK, PXERBLA,     
     +               RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,      
     + 'PYGBTRF, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.(BWL+BWU+1) )) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT,       'PYGBTRF, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT,       'PYGBTRF: auxiliary storage error ',
     +       -INFO )
         RETURN
      ENDIF
*
...
*        put minimum value of work into work( 1 )
         WORK( 1 ) = WORK_SIZE_MIN
         CALL PXERBLA( ICTXT,       'PYGBTRF: worksize error ',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGBTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL YTRSD2D( ICTXT, 'U', 'N',        MIN( BW, BWU+NUMROC( N, 
     +NB, MYCOL+1, 0, NPCOL ) ),        BW, A(BIPTR), LLDA-1, 0, 
     +MYCOL+1)
*
      ENDIF
*
...
*
      IF (LN .GT. 0) THEN
*
         CALL YGBTRF(LM,LN, LBWL,LBWU, A(APTR),LLDA, IPIV, INFO)
*
         IF( INFO.NE.0 ) THEN
            INFO = INFO + NB*MYCOL
            GO TO 90
...
*
         ...
         LDB = LLDA-1
*
*     Update the last BW columns of A_i (code modified from YGBTRS)
*
...
*
                  CALL YSWAP( LNJ, A(LPTR),LDB, A(JPTR), LDB )
*
               ENDIF
*
...
*
               CALL YGERU(LMJ,LNJ,-CONE, A(LPTR),1, A(JPTR),LDB,        
     +    A(JPTR+1),LDB)
   23       CONTINUE
*
      ENDIF
...
*
      IF (MYCOL .GT. 0) THEN
*
         CALL YTRRV2D( ICTXT, 'U', 'N', MIN(BW, LM), BW, AF( 1 ),       
     +  LM, 0, MYCOL-1)
*
*
...
*
            IF( L .NE. J ) THEN
*
                CALL YSWAP(NRHS,  AF(L), LM, AF(J), LM )
            ENDIF
*
            LPTR = BW+1+APTR + (J-1)*LLDA
*
            CALL YGERU( LMJ,NRHS, -CONE, A(LPTR),1,            AF(J), 
     +LM, AF(J+1), LM)
*
   24   CONTINUE
*
...
*
      CALL YLAMOV('G',BM,BN, A(DBPTR),LLDA-1,      AF(BBPTR + BW*LDBB),
     +LDBB)
*
...
*
         ODPTR = LM-BM+1
         CALL YLAMOV('G',BM,BW, AF(ODPTR),LM,         AF(BBPTR 
     ++2*BW*LDBB),LDBB)
      ENDIF
*
      IF (NPCOL.EQ.1) THEN
...
*        performed.
         CALL YGETRF( N-LN, N-LN, AF(BBPTR+BW*LDBB), LDBB,         
     +IPIV(LN+1), INFO)
*
      ENDIF
*
...
*
                   CALL YGESD2D( ICTXT, BM, 2*BW, AF(BBPTR+BW*LDBB),    
     +               LDBB, 0, NEICOL )
*
                   CALL YGERV2D( ICTXT, BMN, 2*BW, AF(BBPTR+BM),        
     +           LDBB, 0, NEICOL)
*
                   IF( NPACT .EQ. 2 ) THEN
*
...
*
                      CALL YLAMOV( 'G', BMN, BW, AF( BBPTR+BM ),        
...
*
     ...
     +         LDBB, 0, NEICOL )
*
                CALL YLAMOV('G',BM, 2*BW, AF(BBPTR+BW*LDBB),LDBB,       
     +         AF(BBPTR+BMN),LDBB)
*
   ...
   31           CONTINUE
*
                CALL YGERV2D( ICTXT, BMN, 2*BW, AF(BBPTR+BW*LDBB),      
     +          LDBB, 0, NEICOL)
*
                IF( NPACT .EQ. 2 ) THEN
*
*                  Copy diagonal block to align whole system
*
                   CALL YLAMOV( 'G', BM, BW, AF( BBPTR+BMN ),           
...
*
             IF (NPACT .NE. 2) THEN
*
                CALL YGETRF(BM+BMN, BW, AF(BBPTR+BW*LDBB), LDBB,        
     +        IPIV(LN+1), INFO)
*
*   Backsolve left side
*
  ...
  301           CONTINUE
*
                CALL YLASWP(BW, AF(BBPTR), LDBB, 1, BW,                
     +IPIV(LN+1), 1)
*
                CALL YTRSM('L','L','N','U', BW, BW, CONE,               
     +  AF(BBPTR+BW*LDBB), LDBB, AF(BBPTR), LDBB)
*
*               Use partial factors to update remainder
*
                CALL YGEMM( 'N', 'N', BM+BMN-BW, BW, BW,                
     + -CONE, AF(BBPTR+BW*LDBB+BW), LDBB,                 AF( BBPTR ), 
     +LDBB, CONE,                 AF( BBPTR+BW ), LDBB )
*
*   Backsolve right side
*
                NRHS = BW
*
                CALL YLASWP(NRHS, AF(BBPTR+2*BW*LDBB), LDBB, 1, BW,     
     +           IPIV(LN+1), 1)
*
...
*
     ...
     +   -CONE, AF(BBPTR+BW*LDBB+BW), LDBB,                 AF( 
     +BBPTR+2*BW*LDBB ), LDBB, CONE,                 AF( 
...
*
                   CALL YLAMOV('G',BM,BW,                   
     +AF(BBPTR+BW),                   LDBB, AF(BBPTR+BW*LDBB), LDBB)
                   CALL YLAMOV('G',BM,BW,                   
...
*
                CALL YGETRF(BM+BMN,BM+BMN, AF(BBPTR+BW*LDBB), LDBB,     
     +           IPIV(LN+1), INFO)
             ENDIF
*
          ENDIF
