      SUBROUTINE PEORGL2( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEORGL2 generates an M-by-N real distributed matrix Q denoting
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORGL2', -INFO )
...
*
         CALL PELASET( 'All', M-K, K, ZERO, ZERO, A, IA+K, JA, DESCA )
         CALL PELASET( 'All', M-K, N-K, ZERO, ONE, A, IA+K, JA+K,       
     +           DESCA )
*
      END IF
*
     ...
     $      TAUI = TAU( MIN( II, KP ) )
         IF( J.LT.JA+N-1 ) THEN
            IF( I.LT.IA+M-1 ) THEN
               ...
               CALL PELARF( 'Right', M-I+IA-1, N-J+JA, A, I, J, DESCA,  
     +                     DESCA( M_ ), TAU, A, I+1, J, DESCA, WORK )
            END IF
            CALL PESCAL( N-J+JA-1, -TAUI, A, I, J+1, DESCA,             
     +            DESCA( M_ ) )
         END IF
         CALL PEELSET( A, I, J, DESCA, ONE-TAUI )
*
*        Set A(i,ja:j-1) to zero
*
         CALL PELASET( 'All', 1, J-JA, ZERO, ZERO, A, I, JA, DESCA )
*
   10 CONTINUE
*
