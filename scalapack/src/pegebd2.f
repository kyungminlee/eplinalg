      SUBROUTINE PEGEBD2( M, N, A, IA, JA, DESCA, D, E, TAUQ, TAUP,     
     +                WORK, LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEBD2 reduces a real general M-by-N distributed matrix
...
*     .. External Subroutines ..
     ...
     +                   EGEBR2D, EGEBS2D, ELARFG, INFOG2L,             
     +       PELARF, PELARFG, PEELSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEBD2', -INFO )
      ...
      IF( M.EQ.1 .AND. N.EQ.1 ) THEN
         IF( MYCOL.EQ.IACOL ) THEN
            IF( MYROW.EQ.IAROW ) THEN
               ...
               CALL ELARFG( 1, A( I ), A( I ), 1, TAUQ( JJ ) )
               D( JJ ) = A( I )
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, D( JJ ),   
     +                     1 )
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, TAUQ( JJ ),
     +                        1 )
            ELSE
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, D( JJ ),   
     +                     1, IAROW, IACOL )
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAUQ( JJ ),
     +                        1, IAROW, IACOL )
            END IF
         END IF
         IF( MYROW.EQ.IAROW )
...
*           A(ia+i:ia+m-1,j)
*
            CALL PELARFG( M-K+1, ALPHA, I, J, A, MIN( I+1, M+IA-1 ),    
     +                 J, DESCA, 1, TAUQ )
...
*
            CALL PELARF( 'Left', M-K+1, N-K, A, I, J, DESCA, 1, TAUQ, A,
     +                    I, J+1, DESCA, WORK )
...
*
               CALL PELARF( 'Right', M-K, N-K, A, I, J+1, DESCA,        
     +               DESCA( M_ ), TAUP, A, I+1, J+1, DESCA,             
     +          WORK )
               CALL PEELSET( A, I, J+1, DESCA, ALPHA )
            ELSE
               CALL PEELSET( TAUP, I, 1, DESCE, ZERO )
            END IF
   10    CONTINUE
*
      ELSE
...
*           A(i,ja+j:ja+n-1)
*
            CALL PELARFG( N-K+1, ALPHA, I, J, A, I,                     
...
*
            CALL PELARF( 'Right', M-K, N-K+1, A, I, J, DESCA,           
...
*              A(i+2:ia+m-1,j)
*
               CALL PELARFG( M-K, ALPHA, I+1, J, A,                     
...
*
               CALL PELARF( 'Left', M-K, N-K, A, I+1, J, DESCA, 1, TAUQ,
     +                       A, I+1, J+1, DESCA, WORK )
               CALL PEELSET( A, I+1, J, DESCA, ALPHA )
            ELSE
               CALL PEELSET( TAUQ, 1, J, DESCE, ZERO )
            END IF
   20    CONTINUE
      END IF
*
