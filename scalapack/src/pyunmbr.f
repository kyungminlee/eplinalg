      SUBROUTINE PYUNMBR( VECT, SIDE, TRANS, M, N, K, A, IA, JA, DESCA, 
...
*  =======
*
*  If VECT = 'Q', PYUNMBR overwrites the general complex distributed
...
*
*  If VECT = 'P', PYUNMBR overwrites sub( C ) with
*
*                       SIDE = 'L'           SIDE = 'R'
...
*  PYGEBRD when reducing a complex distributed matrix A(IA:*,JA:*) to
...
*          distributed matrix reduced by PYGEBRD.
...
*
*  TAU     (local input) COMPLEX(KIND=10) array, dimension
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNMBR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         IF( NQ.GE.K ) THEN
*
*           Q was determined by a call to PYGEBRD with nq >= k
*
            CALL PYUNMQR( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
     +                   C, IC, JC, DESCC, WORK, LWORK, IINFO )
         ELSE IF( NQ.GT.1 ) THEN
*
*           Q was determined by a call to PYGEBRD with nq < k
*
            CALL PYUNMQR( SIDE, TRANS, MI, NI, NQ-1, A, IA+1, JA, DESCA,
...
*
         ...
         IF( NQ.GT.K ) THEN
*
*           P was determined by a call to PYGEBRD with nq > k
*
            CALL PYUNMLQ( SIDE, TRANST, M, N, K, A, IA, JA, DESCA, TAU, 
     +                    C, IC, JC, DESCC, WORK, LWORK, IINFO )
         ELSE IF( NQ.GT.1 ) THEN
*
*           P was determined by a call to PYGEBRD with nq <= k
*
            CALL PYUNMLQ( SIDE, TRANST, MI, NI, NQ-1, A, IA, JA+1,      
