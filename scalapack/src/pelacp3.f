      SUBROUTINE PELACP3( M, I, A, DESCA, B, LDB, II, JJ, REV )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * )
      REAL(KIND=10)   A( * ), B( LDB, * )
*     ..
*
*  Purpose
*  =======
*
*  PELACP3 is an auxiliary routine that copies from a global parallel
...
*
*  B       (local input/output) REAL(KIND=10) array of size (LDB,M)
...
*
                  IF( REV.EQ.0 ) THEN
                     CALL EGEBS2D( CONTXT, 'All', ' ', IROW2-IROW1+1,   
...
*
                  IF( REV.EQ.0 ) THEN
                     CALL EGEBS2D( CONTXT, 'Col', ' ', IROW2-IROW1+1,   
     +                           ICOL2-ICOL1+1, A( ( ICOL1-1 )*LDA+     
     +                         IROW1 ), LDA )
                  END IF
               END IF
...
*
                  IF( REV.EQ.0 ) THEN
                     CALL EGEBS2D( CONTXT, 'Row', ' ', IROW2-IROW1+1,   
     +                           ICOL2-ICOL1+1, A( ( ICOL1-1 )*LDA+     
     +                         IROW1 ), LDA )
                  END IF
               END IF
...
*
                  IF( REV.EQ.0 ) THEN
                     CALL EGESD2D( CONTXT, IROW2-IROW1+1, ICOL2-ICOL1+1,
     +                              A( ( ICOL1-1 )*LDA+IROW1 ), LDA, II,
     +                              JJ )
                  ELSE
                     CALL EGERV2D( CONTXT, IROW2-IROW1+1, ICOL2-ICOL1+1,
   ...
   80             CONTINUE
            ...
            ELSE
               IF( ( II.EQ.-1 ) .AND. ( JJ.EQ.-1 ) ) THEN
                  IF( REV.EQ.0 ) THEN
                     CALL EGEBR2D( CONTXT, 'All', ' ', IROW2-IROW1+1,   
     ...
     +                            LDB, ROW, COL )
               ...
               IF( ( II.EQ.-1 ) .AND. ( JJ.EQ.MYCOL ) ) THEN
                  IF( REV.EQ.0 ) THEN
                     CALL EGEBR2D( CONTXT, 'Col', ' ', IROW2-IROW1+1,   
     +                           ICOL2-ICOL1+1, B( IDI-I+1, IDJ-I+1 ),  
     +                            LDB, ROW, COL )
                  END IF
               END IF
               IF( ( II.EQ.MYROW ) .AND. ( JJ.EQ.-1 ) ) THEN
                  IF( REV.EQ.0 ) THEN
                     CALL EGEBR2D( CONTXT, 'Row', ' ', IROW2-IROW1+1,   
     +                           ICOL2-ICOL1+1, B( IDI-I+1, IDJ-I+1 ),  
     +                            LDB, ROW, COL )
                  END IF
               END IF
