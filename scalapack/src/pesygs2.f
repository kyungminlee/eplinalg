*
*
      SUBROUTINE PESYGS2( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB, 
     +                    DESCB, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PESYGS2 reduces a real symmetric-definite generalized eigenproblem
*  to standard form.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_EXIT, BLACS_GRIDINFO, CHK1MAT, EAXPY,    
     +                ESCAL, ESYR2, ETRMV, ETRSV, INFOG2L, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD
*     ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYGS2', -INFO )
...
*
               ...
               A( IOFFA-LDA ) = AKK
               IF( K.LT.N ) THEN
                  CALL ESCAL( N-K, ONE / BKK, A( IOFFA ), LDA )
                  CT = -HALF*AKK
                  CALL EAXPY( N-K, CT, B( IOFFB ), LDB, A( IOFFA ),     
     +                    LDA )
                  CALL ESYR2( UPLO, N-K, -ONE, A( IOFFA ), LDA,         
...
*
               IF( K.LT.N ) THEN
                  CALL ESCAL( N-K, ONE / BKK, A( IOFFA ), 1 )
                  CT = -HALF*AKK
                  CALL EAXPY( N-K, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
                  CALL ESYR2( UPLO, N-K, -ONE, A( IOFFA ), 1,           
...
*
               AKK = A( IOFFA+K-1 )
               BKK = B( IOFFB+K-1 )
               CALL ETRMV( UPLO, 'No transpose', 'Non-unit', K-1,       
     +               B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ), 1 )
               CT = HALF*AKK
               CALL EAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL ESYR2( UPLO, K-1, ONE, A( IOFFA ), 1, B( IOFFB ), 1,
     +                      A( IIA+( JJA-1 )*LDA ), LDA )
               CALL EAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL ESCAL( K-1, BKK, A( IOFFA ), 1 )
...
*
               AKK = A( IOFFA+( K-1 )*LDA )
               BKK = B( IOFFB+( K-1 )*LDB )
               CALL ETRMV( UPLO, 'Transpose', 'Non-unit', K-1,          
     +            B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ),              
     +        LDA )
               CT = HALF*AKK
               CALL EAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL ESYR2( UPLO, K-1, ONE, A( IOFFA ), LDA, B( IOFFB ), 
     +                     LDB, A( IIA+( JJA-1 )*LDA ), LDA )
               CALL EAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL ESCAL( K-1, BKK, A( IOFFA ), LDA )
