      SUBROUTINE PETZRZF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PETZRZF reduces the M-by-N ( M<=N ) real upper trapezoidal matrix
...
*     .. External Subroutines ..
     ...
     +                PELATRZ, PELARZB, PELARZT, PB_TOPGET,             
     +       PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PETZRZF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL PELATRZ( IB, JA+N-J, L, A, I, J, DESCA, TAU, WORK )
*
            IF( I.GT.IA ) THEN
*
...
*
               CALL PELARZT( 'Backward', 'Rowwise', L, IB, A, I, JM1,   
     +                     DESCA, TAU, WORK, WORK( IPW ) )
*
*              Apply H to A(ia:i-1,j:ja+n-1) from the right
*
               CALL PELARZB( 'Right', 'No transpose', 'Backward',       
...
*
         CALL PELATRZ( IN-IA+1, N, N-M, A, IA, JA, DESCA, TAU, WORK )
*
