      SUBROUTINE PYGEQL2( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGEQL2 computes a QL factorization of a complex distributed M-by-N
...
*
*  TAU     (local output) COMPLEX, array, dimension LOCc(JA+N-1)
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, YGEBR2D, 
     +                   YGEBS2D, YLARFG, YSCAL,                    
     +INFOG2L, PYELSET, PYLARFC,                    PYLARFG, PB_TOPGET, 
     +PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEQL2', -INFO )
...
*
     ...
     $                    NPCOL )
         IF( MYROW.EQ.IAROW ) THEN
            IF( MYCOL.EQ.IACOL ) THEN
               ...
               AJJ = A( I )
               CALL YLARFG( 1, AJJ, A( I ), 1, TAU( JJ+NQ-1 ) )
               IF( N.GT.1 ) THEN
                  ALPHA = ONE - CONJG( TAU( JJ+NQ-1 ) )
                  CALL YGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA, 1 )
                  CALL YSCAL( NQ-1, ALPHA, A( II+(JJ-1)*DESCA( LLD_ ) ),
     +                         DESCA( LLD_ ) )
               END IF
     ...
     +                      1, IAROW, IACOL )
                  CALL YSCAL( NQ, ALPHA, A( II+(JJ-1)*DESCA( LLD_ ) ),  
     +                       DESCA( LLD_ ) )
               END IF
            END IF
...
*           A(ia:i+m-k-1,j+n-k)
*
            CALL PYLARFG( M-K+I-IA+1, AJJ, M-K+I, N-K+J, A, IA,         
     +            N-K+J, DESCA, 1, TAU )
*
*           Apply H(j)' to A(ia:i+m-k,ja:j+n-k-1) from the left
*
            CALL PYELSET( A, I+M-K, J+N-K, DESCA, ONE )
            CALL PYLARFC( 'Left', M-K+I-IA+1, N-K+J-JA, A, IA, N-K+J,   
     +                  DESCA, 1, TAU, A, IA, JA, DESCA, WORK )
            CALL PYELSET( A, I+M-K, J+N-K, DESCA, AJJ )
*
   10    CONTINUE
*
      END IF
