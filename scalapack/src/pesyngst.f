      SUBROUTINE PESYNGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,
...
*  =======
*
*  PESYNGST reduces a complex Hermitian-definite generalized
*  eigenproblem to standard form.
*
*  PESYNGST performs the same function as PDHEGST, but is based on
*  rank 2K updates, which are faster and more scalable than
*  triangular solves (the basis of PESYNGST).
*
*  PESYNGST calls PDHEGST when UPLO='U', hence PDHENGST provides
*  improved performance only when UPLO='L', IBTYPE=1.
*
*  PESYNGST also calls PDHEGST when insufficient workspace is
*  provided,  hence PESYNGST provides improved
...
*
*  SCALE   (global output) REAL(KIND=10)
...
*
*          When IBTYPE = 1 and UPLO = 'L', PESYNGST provides improved
...
*
*     .. Parameters ..
      REAL(KIND=10)   ONEHALF, ONE, MONE
      PARAMETER          ( ONEHALF = 0.5E0_10, ONE = 1.0E0_10, MONE = 
     +-1.0E0_10 )
...
*     .. External Subroutines ..
     ...
     +                PEGEMM, PELACPY, PESYGST, PESYMM, PESYR2K,        
     +            PETRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYNGST', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF( IBTYPE.NE.1 .OR. UPPER .OR. LWORK.LT.LWOPT ) THEN
         CALL PESYGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,    
     +              DESCB, SCALE, INFO )
         RETURN
      END IF
*
...
*
*
         CALL PELACPY( 'A', N-POSTK+1, KB, B, POSTK+IB-1, K+JB-1, DESCB,
     +                  WORK( INDG ), POSTK, 1, DESCG )
         CALL PELACPY( 'A', N-POSTK+1, KB, A, POSTK+IA-1, K+JA-1, DESCA,
     +                  WORK( INDR ), POSTK, 1, DESCR )
         CALL PELACPY( 'A', KB, K-1, A, K+IA-1, JA, DESCA,              
     +    WORK( INDRT ), 1, 1, DESCRT )
*
         CALL PELACPY( 'L', KB, KB, A, K+IA-1, K+JA-1, DESCA,           
     +       WORK( INDR ), K, 1, DESCR )
         CALL PETRSM( 'Right', 'L', 'N', 'N', NPK, KB, MONE, B, K+IB-1, 
     +                K+JB-1, DESCB, WORK( INDG ), POSTK, 1, DESCG )
*
         CALL PESYMM( 'Right', 'L', NPK, KB, ONEHALF, A, K+IA-1, K+JA-1,
     +                 DESCA, WORK( INDG ), POSTK, 1, DESCG, ONE,       
     +          WORK( INDR ), POSTK, 1, DESCR )
*
         CALL PESYR2K( 'Lower', 'No T', NPK, KB, ONE, WORK( INDG ),     
     +             POSTK, 1, DESCG, WORK( INDR ), POSTK, 1, DESCR,      
     +            ONE, A, POSTK+IA-1, POSTK+JA-1, DESCA )
*
         CALL PEGEMM( 'No T', 'No Conj', NPK, K-1, KB, ONE,             
     +    WORK( INDG ), POSTK, 1, DESCG, WORK( INDRT ), 1,              
     +   1, DESCRT, ONE, A, POSTK+IA-1, JA, DESCA )
*
         CALL PESYMM( 'Right', 'L', NPK, KB, ONE, WORK( INDR ), K, 1,   
     +              DESCR, WORK( INDG ), POSTK, 1, DESCG, ONE, A,       
     +          POSTK+IA-1, K+JA-1, DESCA )
*
         CALL PETRSM( 'Left', 'Lower', 'No Conj', 'Non-unit', KB, K-1,  
...
*
   ...
   20       CONTINUE
         END IF
*
         CALL PETRSM( 'Left', 'Lower', 'No Conj', 'Non-unit', KB, KB,   
     +              ONE, B, K+IB-1, K+JB-1, DESCB, WORK( INDAA ), 1,    
     +             1, DESCAA )
*
         CALL PETRSM( 'Right', 'Lower', 'Conj', 'Non-unit', KB, KB, ONE,
     +                 B, K+IB-1, K+JB-1, DESCB, WORK( INDAA ), 1, 1,   
     +              DESCAA )
*
         CALL PELACPY( 'L', KB, KB, WORK( INDAA ), 1, 1, DESCAA, A,     
     +             K+IA-1, K+JA-1, DESCA )
*
         CALL PETRSM( 'Right', 'Lower', 'Conj', 'Non-unit', NPK, KB,    
