      SUBROUTINE PEPBTRSV( UPLO, TRANS, N, BW, NRHS, A, JA, DESCA, B,   
...
*  =======
*
*  PEPBTRSV solves a banded triangular system of linear equations
*
...
*  Cholesky factorization code PEPBTRF
...
*
*  Routine PEPBTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*     .. External Subroutines ..
     ...
     +             ETBTRS, ETRMM, ETRTRS, GLOBCHK, PXERBLA,             
     +       RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 3 )
         CALL PXERBLA( ICTXT,                  
     +'PEPBTRSV, D&C alg.: only 1 block per proc',                  
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*BW ) ) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT, 'PEPBTRSV, D&C alg.: NB too small',       
     +           -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -15
            CALL PXERBLA( ICTXT, 'PEPBTRSV: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPBTRSV', -INFO )
         RETURN
      END IF
*
...
*
            CALL ETBTRS( UPLO, 'N', 'N', ODD_SIZE, BW, NRHS,            
...
*             then use it on RHS
*
               CALL ELAMOV( 'N', BW, NRHS,                       B( 
     +PART_OFFSET+ODD_SIZE-BW+1 ), LLDB,                       WORK( 1 
     +), BW )
*
               CALL ETRMM( 'L', 'U', 'N', 'N', BW, NRHS, -ONE,          
     +            A( ( OFST+( BW+1 )+( ODD_SIZE-BW )*LLDA ) ),          
     +            LLDA-1, WORK( 1 ), BW )
*
               CALL EMATADD( BW, NRHS, ONE, WORK( 1 ), BW, ONE,         
...
*
               CALL EGEMM( 'T', 'N', BW, NRHS, ODD_SIZE, -ONE, AF( 1 ), 
...
*
            IF( MYCOL.GT.0 ) THEN
*
               CALL EGESD2D( ICTXT, BW, NRHS, WORK( 1 ), BW, 0,         
     +               MYCOL-1 )
*
            END IF
*
...
*
               CALL EGERV2D( ICTXT, BW, NRHS, WORK( 1 ), BW, 0,         
     +               MYCOL+1 )
*
...
*
            CALL ETRTRS( 'L', 'N', 'N', BW, NRHS,                    
     +AF( ODD_SIZE*BW+MBW2+1 ), BW,                    B( 
...
*
               CALL EGEMM( 'T', 'N', BW, NRHS, BW, -ONE,                
     +      AF( ( ODD_SIZE )*BW+1 ), BW,                      B( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDB, ZERO,                      WORK( 
     +1 ), BW )
*
...
*
               CALL EGEMM( 'N', 'N', ODD_SIZE, NRHS, BW, -ONE, AF( 1 ), 
...
*           then use it on RHS
*
               CALL ELAMOV( 'N', BW, NRHS, B( PART_OFFSET+ODD_SIZE+1 ), 
     +                      LLDB, WORK( 1+BW-BW ), BW )
*
               CALL ETRMM( 'L', 'U', 'T', 'N', BW, NRHS, -ONE,          
     +            A( ( OFST+( BW+1 )+( ODD_SIZE-BW )*LLDA ) ),          
     +            LLDA-1, WORK( 1+BW-BW ), BW )
*
               CALL EMATADD( BW, NRHS, ONE, WORK( 1+BW-BW ), BW, ONE,   
...
*       Use main partition in each processor to solve locally
*
            CALL ETBTRS( UPLO, 'T', 'N', ODD_SIZE, BW, NRHS,            
     +        A( OFST+1 ), LLDA, B( PART_OFFSET+1 ), LLDB,              
     +      INFO )
*
         END IF
...
*
     ...
     +), BW )
*
               CALL ETRMM( 'L', 'L', 'T', 'N', BW, NRHS, -ONE,          
...
*
     ...
     +                      LLDB, WORK( 1+BW-BW ), BW )
*
               CALL ETRMM( 'L', 'L', 'N', 'N', BW, NRHS, -ONE,          
     +            A( ( OFST+1+ODD_SIZE*LLDA ) ), LLDA-1,                
     +      WORK( 1+BW-BW ), BW )
*
               CALL EMATADD( BW, NRHS, ONE, WORK( 1+BW-BW ), BW, ONE,   
