      SUBROUTINE PYGGRQF( M, P, N, A, IA, JA, DESCA, TAUA, B, IB, JB,   
...
*  =======
*
*  PYGGRQF computes a generalized RQ factorization of
...
*  To form Q explicitly, use ScaLAPACK subroutine PYUNGRQ.
*  To use Q to update another matrix, use ScaLAPACK subroutine PYUNMRQ.
*
...
*  To form Z explicitly, use ScaLAPACK subroutine PYUNGQR.
*  To use Z to update another matrix, use ScaLAPACK subroutine PYUNMQR.
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGGRQF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PYGERQF( M, N, A, IA, JA, DESCA, TAUA, WORK, LWORK, INFO )
      LWMIN = INT( WORK( 1 ) )
*
*     Update sub( B ) := sub( B )*Q'
*
      CALL PYUNMRQ( 'Right', 'Conjugate Transpose', P, N, MIN( M, N ),  
...
*
      CALL PYGEQRF( P, N, B, IB, JB, DESCB, TAUB, WORK, LWORK, INFO )
