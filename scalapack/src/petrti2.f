      SUBROUTINE PETRTI2( UPLO, DIAG, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PETRTI2 computes the inverse of a real upper or lower triangular
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, ESCAL,   
     +                 ETRMV, INFOG2L, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PETRTI2', -INFO )
...
*
                  CALL ETRMV( 'Upper', 'No transpose', DIAG, NA,        
     +                 A( IOFFA ), LDA, A( ICURR ), 1 )
                  CALL ESCAL( NA, AJJ, A( ICURR ), 1 )
                  IDIAG = IDIAG + LDA + 1
                  ICURR = ICURR + LDA
   10          CONTINUE
*
...
*
                  CALL ETRMV( 'Lower', 'No transpose', DIAG, NA,        
     +                 A( ICURR ), LDA, A( IOFFA ), 1 )
                  CALL ESCAL( NA, AJJ, A( IOFFA ), 1 )
                  ICURR = IDIAG
                  IDIAG = IDIAG - LDA - 1
                  IOFFA = IDIAG + 1
   30          CONTINUE
