      SUBROUTINE PYTZRZF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYTZRZF reduces the M-by-N ( M<=N ) complex upper trapezoidal matrix
...
*     .. External Subroutines ..
     ...
     +                PYLATRZ, PYLARZB, PYLARZT, PB_TOPGET,             
     +       PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYTZRZF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL PYLATRZ( IB, JA+N-J, L, A, I, J, DESCA, TAU, WORK )
*
            IF( I.GT.IA ) THEN
*
...
*
               CALL PYLARZT( 'Backward', 'Rowwise', L, IB, A, I, JM1,   
     +                     DESCA, TAU, WORK, WORK( IPW ) )
*
*              Apply H to A(ia:i-1,j:ja+n-1) from the right
*
               CALL PYLARZB( 'Right', 'No transpose', 'Backward',       
...
*
         CALL PYLATRZ( IN-IA+1, N, N-M, A, IA, JA, DESCA, TAU, WORK )
*
