      SUBROUTINE PEGETRI( N, A, IA, JA, DESCA, IPIV, WORK, LWORK,       
     +              IWORK, LIWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGETRI computes the inverse of a distributed matrix using the LU
*  factorization computed by PEGETRF. This method inverts U and then
...
*          factorization sub( A ) = P*L*U computed by PEGETRF. On
...
*     .. External Subroutines ..
     ...
     +                PEGEMM, PELACPY, PELASET, PELAPIV,                
     +    PETRSM, PETRTRI, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGETRI', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $   RETURN
*
*     Form inv(U).  If INFO > 0 from PETRTRI, then U is singular,
*     and the inverse is not computed.
*
      CALL PETRTRI( 'Upper', 'Non-unit', N, A, IA, JA, DESCA, INFO )
      IF( INFO.GT.0 )
     $   RETURN
*
...
*
         CALL PELACPY( 'Lower', JA+N-1-J, JB, A, I+1, J, DESCA,         
     +         WORK, IW+J-JA+1, 1, DESCW )
         CALL PELASET( 'Lower', JA+N-1-J, JB, ZERO, ZERO, A, I+1, J,    
     +              DESCA )
*
*        Compute current block column of inv(A).
*
         IF( J+JB.LE.JA+N-1 )       CALL PEGEMM( 'No transpose', 
     +'No transpose', N, JB,                    JA+N-J-JB, -ONE, A, IA, 
     +J+JB, DESCA, WORK,                    IW+J+JB-JA, 1, DESCW, ONE, 
     +A, IA, J, DESCA )
         CALL PETRSM( 'Right', 'Lower', 'No transpose', 'Unit', N, JB,  
...
*     Copy current block column of L to WORK and replace with zeros.
*
      CALL PELACPY( 'Lower', N-1, JB, A, IA+1, JA, DESCA, WORK, IW+1,   
     +            1, DESCW )
      CALL PELASET( 'Lower', N-1, JB, ZERO, ZERO, A, IA+1, JA, DESCA )
*
*     Compute current block column of inv(A).
*
      IF( JA+JB.LE.JA+N-1 )    CALL PEGEMM( 'No transpose', 
     +'No transpose', N, JB,                 N-JB, -ONE, A, IA, JA+JB, 
     +DESCA, WORK, IW+JB, 1,                 DESCW, ONE, A, IA, JA, 
     +DESCA )
      CALL PETRSM( 'Right', 'Lower', 'No transpose', 'Unit', N, JB,     
...
*
     ...
     $              MP+DESCA( MB_ ) )
      CALL PELAPIV( 'Backward', 'Columns', 'Column', N, N, A, IA,       
