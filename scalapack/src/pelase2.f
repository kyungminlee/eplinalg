      SUBROUTINE PELASE2( UPLO, M, N, ALPHA, BETA, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*  =======
*
*  PELASE2 initializes an M-by-N distributed matrix sub( A ) denoting
*  A(IA:IA+M-1,JA:JA+N-1) to BETA on the diagonal and ALPHA on the
*  offdiagonals.  PELASE2 requires that only dimension of the matrix
...
*
*  ALPHA   (global input) REAL(KIND=10)
*          The constant to which the offdiagonal elements are to be
*          set.
*
*  BETA    (global input) REAL(KIND=10)
*          The constant to which the diagonal elements are to be set.
*
*  A       (local output) REAL(KIND=10) pointer into the local memory
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, ELASET, INFOG2L
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
   10          CONTINUE
               IF( ( N-ITOP ).GT.0 ) THEN
                  CALL ELASET( UPLO, IINXT-IIBEG+1, N-ITOP, ALPHA, BETA,
...
*
   20          CONTINUE
               IF( JJ.LE.( JJA+N-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL ELASET( 'All', MP, ITOP-JJ+JJA, ALPHA, ALPHA,    
     +                      A( II+(JJ-1)*LDA ), LDA )
                  CALL ELASET( UPLO, MP, HEIGHT, ALPHA, BETA,           
...
*
   30          CONTINUE
               IF( JJ.LE.( JJA+N-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL ELASET( 'All', MPA, ITOP-JJ+JJA, ALPHA, ALPHA,   
     +                       A( IIA+(JJ-1)*LDA ), LDA )
                  CALL ELASET( 'All', MPA-MP, HEIGHT, ALPHA, ALPHA,     
     +                     A( IIA+(JJA+ITOP-1)*LDA ), LDA )
                  CALL ELASET( 'All', MP, HEIGHT, ALPHA, BETA,          
     +                A( II+(JJA+ITOP-1)*LDA ), LDA )
                  MP = MAX( 0, MP - HEIGHT )
                  II = II + HEIGHT
                  JJ = JJA + IBASE
...
*
   40          CONTINUE
               IF( ( M-ILEFT ).GT.0 ) THEN
                  CALL ELASET( UPLO, M-ILEFT, JJNXT-JJBEG+1, ALPHA,     
...
*
   50          CONTINUE
               IF( II.LE.( IIA+M-1 ) ) THEN
                  WIDE = IRIGHT - ILEFT
                  CALL ELASET( 'All', ILEFT-II+IIA, NQ, ALPHA, ALPHA,   
     +                       A( II+(JJ-1)*LDA ), LDA )
                  CALL ELASET( UPLO, WIDE, NQ, ALPHA, BETA,             
...
*
   60          CONTINUE
               IF( II.LE.( IIA+M-1 ) ) THEN
                  WIDE = IRIGHT - ILEFT
                  CALL ELASET( 'All', ILEFT-II+IIA, NQA, ALPHA, ALPHA,  
     +                        A( II+(JJA-1)*LDA ), LDA )
                  CALL ELASET( 'All', WIDE, NQA-NQ, ALPHA, ALPHA,       
     +                   A( IIA+ILEFT+(JJA-1)*LDA ), LDA )
                  CALL ELASET( 'All', WIDE, NQ, ALPHA, BETA,            
     +              A( IIA+ILEFT+(JJ-1)*LDA ), LDA )
                  NQ = MAX( 0, NQ - WIDE )
                  II = IIA + IRIGHT
                  JJ = JJ + WIDE
