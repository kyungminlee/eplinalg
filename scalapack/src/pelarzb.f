      SUBROUTINE PELARZB( SIDE, TRANS, DIRECT, STOREV, M, N, K, L, V,   
...
*  =======
*
*  PELARZB applies a real block reflector Q or its transpose Q**T to
...
*          Householder transformation as returned by PETZRZF.
*          LLD_V >= LOCr(IV+K-1).
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, EGEBR2D,          
     +          EGEBS2D,EGEMM, EGSUM2D, ELAMOV,                    
     +ELASET, ETRBR2D, ETRBS2D, ETRMM,                    INFOG2L, 
     +PBEMATADD, PBETRAN, PB_TOPGET, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PELARZB', -INFO )
...
*        M+ICOFFV as PBETRAN's N argument while sizing WORK( IPV ) / LV
*        from MPC20 = local rows of L. Whenever M > L (the typical case
*        — M = K+L from PEORMRZ's caller), PBETRAN's LDC contract
...
*        value") *without* writing WORK( IPV ), and the outer EGEMM at
...
*        matches PBETRAN's NP requirement under the alignment hypothesis
...
*
         IF( MYROW.EQ.IVROW ) THEN
            IF( MYCOL.EQ.IVCOL ) THEN
               CALL ELAMOV( 'All', K, NQV, V( IOFFV ), LDV,             
...
*
         CALL PBETRAN( ICTXT, 'Rowwise', 'Transpose', K, L+ICOFFV,      
     +            DESCV( NB_ ), WORK( IPW ), LW, ZERO,                  
     +WORK( IPV ), LV, IVROW, IVCOL, ICROW2, -1,                  WORK( 
     +IPT ) )
*
...
*
         IF( MPC2.GT.0 ) THEN
            CALL EGEMM( 'Transpose', 'No transpose', NQC2, K, MPC2,     
     +              ONE, C( IOFFC2 ), LDC, WORK( IPV ), LV, ZERO,       
     +            WORK( IPW ), LW )
         ELSE
            CALL ELASET( 'All', NQC2, K, ZERO, ZERO, WORK( IPW ), LW )
         END IF
*
...
*
   10       CONTINUE
            IF( IIBEG.LE.IINXT ) THEN
               CALL PBEMATADD( ICTXT, 'Transpose', NQC2, IINXT-IIBEG+1, 
     ...
     +                        WORK( IPW+ITOP ), LW )
         ...
         END IF
*
         CALL EGSUM2D( ICTXT, 'Columnwise', ' ', NQC2, K, WORK( IPW ),  
     +                LW, IVROW, MYCOL )
*
...
*
               CALL ETRBS2D( ICTXT, 'Rowwise', ' ', 'Lower', 'Non unit',
     +                        K, K, T, MBV )
            ELSE
               CALL ETRBR2D( ICTXT, 'Rowwise', ' ', 'Lower', 'Non unit',
     +                        K, K, T, MBV, MYROW, IVCOL )
            END IF
            CALL ETRMM( 'Right', 'Lower', TRANST, 'Non unit', NQC2, K,  
     +                 ONE, T, MBV, WORK( IPW ), LW )
*
            CALL EGEBS2D( ICTXT, 'Columnwise', ' ', NQC2, K,            
     +         WORK( IPW ), LW )
         ELSE
            CALL EGEBR2D( ICTXT, 'Columnwise', ' ', NQC2, K,            
...
*
   20       CONTINUE
            IF( IIBEG.LE.IINXT ) THEN
               CALL PBEMATADD( ICTXT, 'Transpose', IINXT-IIBEG+1, NQC2, 
...
*
         CALL EGEMM( 'No transpose', 'Transpose', MPC2, NQC2, K, -ONE,  
...
*
         ...
         IF( MYROW.EQ.IVROW ) THEN
            CALL EGEBS2D( ICTXT, 'Columnwise', COLBTOP, K, NQC2,        
     +             V( IOFFV ), LDV )
            IF( MYCOL.EQ.IVCOL )          CALL ETRBS2D( ICTXT, 
     +'Columnwise', COLBTOP, 'Lower',                        'Non unit',
     + K, K, T, MBV )
            CALL ELAMOV( 'All', K, NQC2, V( IOFFV ), LDV, WORK( IPV ),  
     +                  LV )
         ELSE
            CALL EGEBR2D( ICTXT, 'Columnwise', COLBTOP, K, NQC2,        
...
*
         IF( NQC2.GT.0 ) THEN
            CALL EGEMM( 'No Transpose', 'Transpose', MPC2, K, NQC2,     
     +             ONE, C( IOFFC2 ), LDC, WORK( IPV ), LV, ZERO,        
     +          WORK( IPW ), LW )
         ELSE
            CALL ELASET( 'All', MPC2, K, ZERO, ZERO, WORK( IPW ), LW )
         END IF
*
...
*
   30       CONTINUE
            IF( JJBEG.LE.JJNXT ) THEN
               CALL PBEMATADD( ICTXT, 'No transpose', MPC2,             
     +             JJNXT-JJBEG+1, ONE,                          C( 
     ...
     +IPW+ILEFT*LW ), LW )
         ...
         END IF
*
         CALL EGSUM2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),     
     +             LW, MYROW, IVCOL )
*
*        WORK( IPW ) = WORK( IPW ) * T' or WORK( IPW ) * T
*
         IF( MYCOL.EQ.IVCOL ) THEN
            CALL ETRMM( 'Right', 'Lower', TRANS, 'Non unit', MPC2, K,   
     +                ONE, T, MBV, WORK( IPW ), LW )
            CALL EGEBS2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),  
     +                   LW )
         ELSE
            CALL EGEBR2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),  
     +                   LW, MYROW, IVCOL )
         END IF
*
*        C1 = C1 - WORK( IPW )
...
*                     MPC2 x NQC2    MPC2 x K      K x NQC2
*
         IF( IOFFC2.GT.0 )       CALL EGEMM( 'No transpose', 
