      SUBROUTINE PEORMQL( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*  =======
*
*  PEORMQL overwrites the general real M-by-N distributed matrix
...
*     .. External Subroutines ..
     ...
     +                PELARFT, PEORM2L, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORMQL', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. .NOT.NOTRAN ) ) THEN
         ...
         ELSE
            NI = N - K + JB
         END IF
         CALL PEORM2L( SIDE, TRANS, MI, NI, JB, A, IA, JA, DESCA, TAU,  
...
*
         CALL PELARFT( 'Backward', 'Columnwise', NQ-K+J+JB-JA, JB,      
...
*        Apply H or H'
*
         CALL PELARFB( SIDE, TRANS, 'Backward', 'Columnwise', MI, NI,   
