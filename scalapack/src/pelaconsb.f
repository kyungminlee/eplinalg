      SUBROUTINE PELACONSB( A, DESCA, I, L, M, H44, H33, H43H34, BUF,   
     +                  LWORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELACONSB looks for two consecutive small subdiagonal elements by
...
*  =========
*
*  A       (global input) REAL(KIND=10) array, dimension
*          (DESCA(LLD_),*)
...
*  H44
*  H33
*  H43H34  (global input) REAL(KIND=10)
*          These three values are for the double shift QR iteration.
*
*  BUF     (local output) REAL(KIND=10) array of size LWORK.
*
...
*        the LAPACK algorithm ELAHQR, a loop of M goes from I-2 down to
*        L and examines
...
*     .. Local Scalars ..
     ...
     $                   MYROW, NPCOL, NPROW, NUM, RIGHT, UP
      REAL(KIND=10)   H00, H10, H11, H12, H21, H22, H33S, H44S, S,      
     +              TST1, ULP, V1, V2, V3
*     ..
*     .. External Functions ..
      INTEGER            ILCM
      REAL(KIND=10)   PELAMCH
      EXTERNAL           ILCM, PELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGERV2D, EGESD2D, IGAMX2D,     
     +               INFOG2L, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MOD
...
*
     ...
     $   ISTR2 = ISTR2 + 1
      ...
      IF( LWORK.LT.7*ISTR2 ) THEN
         CALL PXERBLA( CONTXT, 'PELACONSB', 10 )
         RETURN
      END IF
      ISTR3 = 3*ISTR2
      ISTR4 = ISTR3 + ISTR2
...
*
      IF( IBUF1.GT.0 ) THEN
         CALL EGESD2D( CONTXT, IBUF1, 1, BUF( ISTR1+1 ), IBUF1, DOWN,   
     +               RIGHT )
      END IF
      IF( IBUF2.GT.0 ) THEN
         CALL EGESD2D( CONTXT, IBUF2, 1, BUF( ISTR2+1 ), IBUF2, UP,     
     +             MYCOL )
      END IF
      IF( IBUF3.GT.0 ) THEN
         CALL EGESD2D( CONTXT, IBUF3, 1, BUF( ISTR3+1 ), IBUF3, MYROW,  
     +                LEFT )
      END IF
      IF( IBUF4.GT.0 ) THEN
         CALL EGESD2D( CONTXT, IBUF4, 1, BUF( ISTR4+1 ), IBUF4, UP,     
     +             LEFT )
      END IF
      IF( IBUF5.GT.0 ) THEN
         CALL EGESD2D( CONTXT, IBUF5, 1, BUF( ISTR5+1 ), IBUF5, MYROW,  
     +                RIGHT )
      END IF
*
...
*
      IF( IRCV1.GT.0 ) THEN
         CALL EGERV2D( CONTXT, IRCV1, 1, BUF( ISTR1+1 ), IRCV1, UP,     
     +             LEFT )
      END IF
      IF( IRCV2.GT.0 ) THEN
         CALL EGERV2D( CONTXT, IRCV2, 1, BUF( ISTR2+1 ), IRCV2, DOWN,   
     +               MYCOL )
      END IF
      IF( IRCV3.GT.0 ) THEN
         CALL EGERV2D( CONTXT, IRCV3, 1, BUF( ISTR3+1 ), IRCV3, MYROW,  
     +                RIGHT )
      END IF
      IF( IRCV4.GT.0 ) THEN
         CALL EGERV2D( CONTXT, IRCV4, 1, BUF( ISTR4+1 ), IRCV4, DOWN,   
     +               RIGHT )
      END IF
      IF( IRCV5.GT.0 ) THEN
         CALL EGERV2D( CONTXT, IRCV5, 1, BUF( ISTR5+1 ), IRCV5, MYROW,  
     +                LEFT )
      END IF
*
*     Start main loop
