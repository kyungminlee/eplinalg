      SUBROUTINE PEORMHR( SIDE, TRANS, M, N, ILO, IHI, A, IA, JA, DESCA,
...
*  =======
*
*  PEORMHR overwrites the general real M-by-N distributed matrix
...
*  product of IHI-ILO elementary reflectors, as returned by PEGEHRD:
*
...
*          of PEGEHRD. Q is equal to the unit matrix except in the
...
*          define the elementary reflectors, as returned by PEGEHRD.
*
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension LOCc(JA+M-2)
*          if SIDE = 'L', and LOCc(JA+N-2) if SIDE = 'R'. This array
*          contains the scalar factors TAU(j) of the elementary
*          reflectors H(j) as returned by PEGEHRD. TAU is tied to
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PEORMQR,    
     +                PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORMHR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $   RETURN
*
      CALL PEORMQR( SIDE, TRANS, MI, NI, NH, A, IAA, JAA, DESCA, TAU,   
