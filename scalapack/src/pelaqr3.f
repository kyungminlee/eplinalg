      RECURSIVE SUBROUTINE PELAQR3( WANTT, WANTZ, N, KTOP, KBOT, NW, H, 
...
*     .. Array Arguments ..
     ...
     $                   DESCW( * ), IWORK( * )
      REAL(KIND=10)   H( * ), SI( KBOT ), SR( KBOT ), T( * ),           
     +         V( * ), WORK( * ), WV( * ),                    Z( * )
*     ..
*
*  Purpose
...
*
*  H       (local input/output) REAL(KIND=10) array, dimension
*             (DESCH(LLD_),*)
...
*
*  Z       (input/output) REAL(KIND=10) array, dimension
*             (DESCH(LLD_),*)
...
*          subroutine.
*
*  SR      (global output) REAL(KIND=10) array, dimension KBOT
*  SI      (global output) REAL(KIND=10) array, dimension KBOT
...
*
*  V       (global workspace) REAL(KIND=10) array, dimension 
*             (DESCV(LLD_),*)
...
*
*  T       (global workspace) REAL(KIND=10) array, dimension 
*             (DESCV(LLD_),*)
*
...
*          values of LWORK.
*
*          If LWORK = -1, then a workspace query is assumed; PELAQR3
...
*     ..
*     .. Local Scalars ..
      REAL(KIND=10)   AA, BB, BETA, CC, CS, DD, EVI, EVK, FOO, S,       
...
*     ..
*     .. External Functions ..
      REAL(KIND=10)   ELAMCH, PELANGE
      INTEGER            PILAENVX_EP, NUMROC, INDXG2P, ICEIL, BLACS_PNUM
      EXTERNAL           ELAMCH, PILAENVX_EP, NUMROC, INDXG2P, PELANGE, 
     +                   MPI_WTIME, ICEIL, BLACS_PNUM
*     ..
*     .. External Subroutines ..
      EXTERNAL           PECOPY, PEGEHRD, PEGEMM, ELABAD, PELACPY,      
     +              PELAQR1, ELANV2, PELAQR0, PELARF, PELARFG,          
     +          PELASET, PETRORD, PEELGET, PEELSET,                    
     +PELAMVE, BLACS_GRIDINFO, BLACS_GRIDMAP,                    
     +BLACS_GRIDEXIT, PEGEMR2D
*     ..
*     .. Intrinsic Functions ..
...
*
      PAR(1) = PILAENVX_EP(ICTXT, 17, 'PELAQR3', 'SV', JW, NB, -1, -1)
      PAR(2) = PILAENVX_EP(ICTXT, 18, 'PELAQR3', 'SV', JW, NB, -1, -1)
      PAR(3) = PILAENVX_EP(ICTXT, 19, 'PELAQR3', 'SV', JW, NB, -1, -1)
      PAR(4) = PILAENVX_EP(ICTXT, 20, 'PELAQR3', 'SV', JW, NB, -1, -1)
      PAR(5) = PILAENVX_EP(ICTXT, 21, 'PELAQR3', 'SV', JW, NB, -1, -1)
      PAR(6) = PILAENVX_EP(ICTXT, 22, 'PELAQR3', 'SV', JW, NB, -1, -1)
*
...
*
      ...
      ELSE
*
*        Workspace query calls to PEGEHRD and PEORMHR.
*
         TAUROWS = NUMROC( 1, 1, MYCOL, DESCV(RSRC_), NPROW )
         TAUCOLS = NUMROC( JW+IROFFH, NB, MYCOL, DESCV(CSRC_),
     $        NPCOL )
         CALL PEGEHRD( JW, 1, JW, T, 1, 1, DESCT, WORK, WORK, -1,       
     +  INFO )
         LWK1 = INT( WORK( 1 ) ) + TAUROWS*TAUCOLS
*
*        Workspace query call to PEORMHR.
*
         CALL PEORMHR( 'Right', 'No', JW, JW, 1, JW, T, 1, 1, DESCT,    
     +     WORK, V, 1, 1, DESCV, WORK, -1, INFO )
         LWK2 = INT( WORK( 1 ) )
*
*        Workspace query call to PELAQR0.
*
         NMIN = PILAENVX_EP( ICTXT, 12, 'PELAQR3', 'SV', JW, 1, JW, 
     +LWORK )
         NMAX = ( N-1 ) / 3
         IF( JW+IROFFH.GT.NMIN .AND. JW+IROFFH.LE.NMAX
     $        .AND. RECLEVEL.LT.RECMAX ) THEN
            CALL PELAQR0( .TRUE., .TRUE., JW+IROFFH, 1+IROFFH,          
     +  JW+IROFFH, T, DESCT, SR, SI, 1, JW, V, DESCV,            WORK, 
     +-1, IWORK, LIWORK-NSEL, INFQR,             RECLEVEL+1 )
            LWK3 = INT( WORK( 1 ) )
            IWRK1 = IWORK( 1 )
         ELSE
            RSRC = DESCT( RSRC_ )
            ...
            DESCT( RSRC_ ) = 0
            DESCT( CSRC_ ) = 0
            CALL PELAQR1( .TRUE., .TRUE., JW+IROFFH, 1, JW+IROFFH, T,   
...
*
         CALL PETRORD( 'Vectors', IWORK, PAR, JW+IROFFH, T, 1, 1,       
...
*        (workspace for PELARF approximated for simplicity).
*
...
*
      WORK( 1 ) = REAL( LWKOPT , KIND=10)
*
*     IWORK(1:NSEL) is used as the array SELECT for PETRORD.
*
...
*
      ...
      SAFMAX = ONE / SAFMIN
      CALL ELABAD( SAFMIN, SAFMAX )
      ULP = ELAMCH( 'PRECISION' )
      SMLNUM = SAFMIN*( REAL( N , KIND=10) / ULP )
*
...
*
      ...
      IF( KWTOP.EQ.KTOP ) THEN
         S = ZERO
      ELSE
         CALL PEELGET( 'All', '1-Tree', S, H, KWTOP, KWTOP-1, DESCH )
      END IF
*
...
*
         CALL PEELGET( 'All', '1-Tree', SR( KWTOP ), H, KWTOP, KWTOP,   
     +      DESCH )
         SI( KWTOP ) = ZERO
         NS = 1
         ND = 0
     ...
     $        THEN
            NS = 0
            ND = 1
            IF( KWTOP.GT.KTOP )          CALL PEELSET( H, KWTOP, 
     +KWTOP-1 , DESCH, ZERO )
         END IF
         RETURN
      END IF
...
*
         CALL PEELGET( 'All', '1-Tree', AA, H, KWTOP, KWTOP, DESCH )
         CALL PEELGET( 'All', '1-Tree', BB, H, KWTOP, KWTOP+1, DESCH )
         CALL PEELGET( 'All', '1-Tree', CC, H, KWTOP+1, KWTOP, DESCH )
         CALL PEELGET( 'All', '1-Tree', DD, H, KWTOP+1, KWTOP+1, DESCH )
         CALL ELANV2( AA, BB, CC, DD, SR(KWTOP), SI(KWTOP),         
     +SR(KWTOP+1), SI(KWTOP+1), CS, SN )
         NS = 0
         ND = 2
         IF( CC.EQ.ZERO ) THEN
            I = KWTOP
            IF( I+2.LE.N .AND. WANTT )          CALL PEROT( N-I-1, H, I,
     + I+2, DESCH, DESCH(M_), H, I+1,               I+2, DESCH, 
     +DESCH(M_), CS, SN, WORK, LWORK, INFO )
            IF( I.GT.1 )          CALL PEROT( I-1, H, 1, I, DESCH, 1, H,
     + 1, I+1, DESCH, 1,               CS, SN, WORK, LWORK, INFO )
            IF( WANTZ )          CALL PEROT( IHIZ-ILOZ+1, Z, ILOZ, I, 
     +DESCZ, 1, Z, ILOZ,               I+1, DESCZ, 1, CS, SN, WORK, 
     +LWORK, INFO )
            CALL PEELSET( H, I, I, DESCH, AA )
            CALL PEELSET( H, I, I+1, DESCH, BB )
            CALL PEELSET( H, I+1, I, DESCH, CC )
            CALL PEELSET( H, I+1, I+1, DESCH, DD )
         END IF
         WORK( 1 ) = REAL( LWKOPT , KIND=10)
         RETURN
      END IF
...
*
      CALL PELASET( 'All', IROFFH, JW+IROFFH, ZERO, ONE, T, 1, 1,      
     +DESCT )
      CALL PELASET( 'All', JW, IROFFH, ZERO, ZERO, T, 1+IROFFH, 1,      
     +DESCT )
      CALL PELACPY( 'All', 1, JW, H, KWTOP, KWTOP, DESCH, T, 1+IROFFH,  
     +    1+IROFFH, DESCT )
      CALL PELACPY( 'Upper', JW-1, JW-1, H, KWTOP+1, KWTOP, DESCH, T,   
     +   1+IROFFH+1, 1+IROFFH, DESCT )
      IF( JW.GT.2 )    CALL PELASET( 'Lower', JW-2, JW-2, ZERO, ZERO, T,
     + 1+IROFFH+2,         1+IROFFH, DESCT )
      CALL PELACPY( 'All', JW-1, 1, H, KWTOP+1, KWTOP+JW-1, DESCH, T,   
     +   1+IROFFH+1, 1+IROFFH+JW-1, DESCT )
*
*     Initialize the working orthogonal matrix.
*
      CALL PELASET( 'All', JW+IROFFH, JW+IROFFH, ZERO, ONE, V, 1, 1,    
     +  DESCV )
*
*     Compute the Schur form of T.
*
      NPMIN = PILAENVX_EP( ICTXT, 23, 'PELAQR3', 'SV', JW, NB, NPROW,   
     +   NPCOL )
      NMIN = PILAENVX_EP( ICTXT, 12, 'PELAQR3', 'SV', JW, 1, JW, LWORK )
      NMAX = ( N-1 ) / 3
...
*
     ...
     $        .AND. RECLEVEL.LT.RECMAX ) THEN
            CALL PELAQR0( .TRUE., .TRUE., JW+IROFFH, 1+IROFFH,          
     +  JW+IROFFH, T, DESCT, SR( KWTOP-IROFFH ),            SI( 
     +KWTOP-IROFFH ), 1+IROFFH, JW+IROFFH, V, DESCV,            WORK, 
     +LWORK, IWORK(NSEL+1), LIWORK-NSEL, INFQR,            RECLEVEL+1 )
         ELSE
            IF( DESCT(RSRC_).EQ.0 .AND. DESCT(CSRC_).EQ.0 ) THEN
               IF( JW+IROFFH.GT.DESCT( MB_ ) ) THEN
                  CALL PELAQR1( .TRUE., .TRUE., JW+IROFFH, 1,           
     +       JW+IROFFH, T, DESCT, SR( KWTOP-IROFFH ),                  
     +SI( KWTOP-IROFFH ), 1, JW+IROFFH, V,                  DESCV, WORK,
     + LWORK, IWORK(NSEL+1), LIWORK-NSEL,                  INFQR )
               ELSE
                  IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
                     CALL ELAHQR( .TRUE., .TRUE., JW+IROFFH, 1+IROFFH,  
     ...
     $              THEN
                  CALL ELAHQR( .TRUE., .TRUE., JW+IROFFH, 1+IROFFH,     
     +             JW+IROFFH, T, DESCT(LLD_),                  SR( 
     +KWTOP-IROFFH ), SI( KWTOP-IROFFH ),                  1+IROFFH, 
     +JW+IROFFH, V, DESCV(LLD_), INFQR )
               ELSE
                  INFQR = 0
               END IF
               IF( NPROCS.GT.1 )
     ...
     $              0, ICTXT, MAX(1,TZROWS0), IERR0 )
               ...
               CALL PELAMVE( 'All', JW+IROFFH, JW+IROFFH, T, 1, 1,      
     +         DESCT, WORK(IPT0), 1, 1, DESCTZ0, WORK(IPW0) )
               CALL PELASET( 'All', JW+IROFFH, JW+IROFFH, ZERO, ONE,    
     +           WORK(IPZ0), 1, 1, DESCTZ0 )
               CALL PELAQR1( .TRUE., .TRUE., JW+IROFFH, 1,              
     + JW+IROFFH, WORK(IPT0), DESCTZ0,               SR( KWTOP-IROFFH ),
     + SI( KWTOP-IROFFH ),               1, JW+IROFFH, WORK(IPZ0),      
     +         DESCTZ0, WORK(IPW0), LWORK-IPW0+1, IWORK(NSEL+1),        
     +       LIWORK-NSEL, INFQR )
               CALL PELAMVE( 'All', JW+IROFFH, JW+IROFFH, WORK(IPT0), 1,
     +               1, DESCTZ0, T, 1, 1, DESCT, WORK(IPW0) )
               CALL PELAMVE( 'All', JW+IROFFH, JW+IROFFH, WORK(IPZ0), 1,
...
*
 ...
 20      CONTINUE
     ...
     $           0, ICTXT_NEW, MAX(1,TZROWS0), IERR0 )
         ...
         ELSE
            ...
            INFQR = 0
         END IF
         CALL PEGEMR2D( JW, JW, T, 1+IROFFH, 1+IROFFH, DESCT,         
     +WORK(IPT0), 1, 1, DESCTZ0, ICTXT )
         IF( ICTXT_NEW.GE.0 ) THEN
            CALL PELASET( 'All', JW, JW, ZERO, ONE, WORK(IPZ0), 1, 1,   
     +         DESCTZ0 )
            NMIN = PILAENVX_EP( ICTXT_NEW, 12, 'PELAQR3', 'SV', JW, 1, 
     +JW,            LWORK )
            IF( JW.GT.NMIN .AND. JW.LE.NMAX .AND. RECLEVEL.LT.1 ) THEN
               CALL PELAQR0( .TRUE., .TRUE., JW, 1, JW, WORK(IPT0),     
     +          DESCTZ0, SR( KWTOP ), SI( KWTOP ), 1, JW,               
     +WORK(IPZ0), DESCTZ0, WORK(IPW0), LWORK-IPW0+1,               
     +IWORK(NSEL+1), LIWORK-NSEL, INFQR,               RECLEVEL+1 )
            ELSE
               CALL PELAQR1( .TRUE., .TRUE., JW, 1, JW, WORK(IPT0),     
     +          DESCTZ0, SR( KWTOP ), SI( KWTOP ), 1, JW,               
     +WORK(IPZ0), DESCTZ0, WORK(IPW0), LWORK-IPW0+1,               
     +IWORK(NSEL+1), LIWORK-NSEL, INFQR )
            END IF
         END IF
         CALL PEGEMR2D( JW, JW, WORK(IPT0), 1, 1, DESCTZ0, T, 1+IROFFH, 
     +        1+IROFFH, DESCT, ICTXT )
         CALL PEGEMR2D( JW, JW, WORK(IPZ0), 1, 1, DESCTZ0, V, 1+IROFFH, 
 ...
 40         CONTINUE
     ...
     $        -1, -1, -1 )
         CALL EGSUM2D( ICTXT, 'All', ' ', JW, 1, SR(KWTOP), JW, -1, -1 )
         CALL EGSUM2D( ICTXT, 'All', ' ', JW, 1, SI(KWTOP), JW, -1, -1 )
      END IF
*
...
*
     ...
     $   INFQR = INFQR - IROFFH
*
*     PETRORD needs a clean margin near the diagonal.
*
      DO 50 J = 1, JW - 3
         CALL PEELSET( T, J+2, J, DESCT, ZERO )
         CALL PEELSET( T, J+3, J, DESCT, ZERO )
 50   CONTINUE
      IF( JW.GT.2 )    CALL PEELSET( T, JW, JW-2, DESCT, ZERO )
*
*     Check local residual for AED Schur decomposition.
*
      RESAED = 0.0E00_10
*
*     Clean up the array SELECT for PETRORD.
*
      DO 60 J = 1, NSEL
         IWORK( J ) = 0
 60   CONTINUE
...
*
      ...
      ILST = INFQR + 1 + IROFFH
      IF( ILST.GT.1 ) THEN
         CALL PEELGET( 'All', '1-Tree', ELEM, T, ILST, ILST-1, DESCT )
         BULGE = ELEM.NE.ZERO
...
*
         ...
         IF( LILST.GT.1 ) THEN
            CALL PEELGET( 'All', '1-Tree', ELEM, T, LILST, LILST-1,     
     +       DESCT )
            BULGE = ELEM.NE.ZERO
 ...
 100     CONTINUE
         IF( LILST.LE.NS+IROFFH ) THEN
            IF( NS.EQ.1 ) THEN
               BULGE = .FALSE.
            ELSE
               CALL PEELGET( 'All', '1-Tree', ELEM, T, NS+IROFFH,       
     +        NS+IROFFH-1, DESCT )
               BULGE = ELEM.NE.ZERO
            END IF
*
...
*
     ...
     +        NS+IROFFH, DESCT )
               ...
               IF( FOO.EQ.ZERO )
     $            FOO = ABS( S )
               CALL PEELGET( 'All', '1-Tree', ELEM, V, 1+IROFFH,        
     +       NS+IROFFH, DESCV )
...
*
 ...
 110              CONTINUE
                  IWORK( IFST+IROFFH ) = 1
                  CALL PETRORD( 'Vectors', IWORK, PAR, JW+IROFFH, T, 1, 
     +                 1, DESCT, V, 1, 1, DESCV, WORK,                  
     +WORK(JW+IROFFH+1), MLOC,                  WORK(2*(JW+IROFFH)+1), 
     +LWORK-2*(JW+IROFFH),                  IWORK(NSEL+1), LIWORK-NSEL, 
     +INFO )
*
*                 Adjust the array SELECT explicitly so that it does not
*                 rely on the output of PETRORD.
*
                  IWORK( IFST+IROFFH ) = 0
                  IWORK( LILST ) = 1
                  LILST = LILST + 1
...
*
               CALL PEELGET( 'All', '1-Tree', ELEM1, T, NS+IROFFH,      
     +         NS+IROFFH, DESCT )
               CALL PEELGET( 'All', '1-Tree', ELEM2, T, NS+IROFFH,      
     +         NS+IROFFH-1, DESCT )
               CALL PEELGET( 'All', '1-Tree', ELEM3, T, NS+IROFFH-1,    
     +           NS+IROFFH, DESCT )
               FOO = ABS( ELEM1 ) + SQRT( ABS( ELEM2 ) )*
     $              SQRT( ABS( ELEM3 ) )
               IF( FOO.EQ.ZERO )
     $            FOO = ABS( S )
               CALL PEELGET( 'All', '1-Tree', ELEM1, V, 1+IROFFH,       
     +        NS+IROFFH, DESCV )
               CALL PEELGET( 'All', '1-Tree', ELEM2, V, 1+IROFFH,       
     +        NS+IROFFH-1, DESCV )
...
*
 ...
 130     CONTINUE
     ...
     +WORK(2*(JW+IROFFH)+1), LWORK-2*(JW+IROFFH),         IWORK(NSEL+1),
     + LIWORK-NSEL, INFO )
         ILST = M + 1
*
...
*
      CALL ECOPY( JW, WORK(1+IROFFH), 1, SR( KWTOP ), 1 )
      CALL ECOPY( JW, WORK(JW+2*IROFFH+1), 1, SI( KWTOP ), 1 )
*
...
*
            CALL PELASET( 'All', NS+IROFFH, 1, ZERO, ZERO, WORK(ITAU),  
     +          1, 1, DESCTAU )
*
            CALL PECOPY( NS, V, 1+IROFFH, 1+IROFFH, DESCV, DESCV(M_),   
     +         WORK(IR), 1+IROFFH, 1, DESCR, 1 )
            CALL PELARFG( NS, BETA, 1+IROFFH, 1, WORK(IR), 2+IROFFH, 1, 
     +           DESCR, 1, WORK(ITAU+IROFFH) )
            CALL PEELSET( WORK(IR), 1+IROFFH, 1, DESCR, ONE )
*
            CALL PELASET( 'Lower', JW-2, JW-2, ZERO, ZERO, T, 3+IROFFH, 
     +           1+IROFFH, DESCT )
*
            CALL PELARF( 'Left', NS, JW, WORK(IR), 1+IROFFH, 1, DESCR,  
     +          1, WORK(ITAU+IROFFH), T, 1+IROFFH, 1+IROFFH,            
     +DESCT, WORK( IPW ) )
            CALL PELARF( 'Right', NS, NS, WORK(IR), 1+IROFFH, 1, DESCR, 
     +           1, WORK(ITAU+IROFFH), T, 1+IROFFH, 1+IROFFH,           
     + DESCT, WORK( IPW ) )
            CALL PELARF( 'Right', JW, NS, WORK(IR), 1+IROFFH, 1, DESCR, 
     +           1, WORK(ITAU+IROFFH), V, 1+IROFFH, 1+IROFFH,           
     + DESCV, WORK( IPW ) )
*
            ITAU = 1
            IPW = ITAU + DESCTAU( LLD_ ) * TAUCOLS
            CALL PEGEHRD( JW+IROFFH, 1+IROFFH, NS+IROFFH, T, 1, 1,      
...
*
         IF( KWTOP.GT.1 ) THEN
            CALL PEELGET( 'All', '1-Tree', ELEM, V, 1+IROFFH,           
     + 1+IROFFH, DESCV )
            CALL PEELSET( H, KWTOP, KWTOP-1, DESCH, S*ELEM )
         END IF
         CALL PELACPY( 'Upper', JW-1, JW-1, T, 1+IROFFH+1, 1+IROFFH,    
     +     DESCT, H, KWTOP+1, KWTOP, DESCH )
         CALL PELACPY( 'All', 1, JW, T, 1+IROFFH, 1+IROFFH, DESCT, H,   
     +      KWTOP, KWTOP, DESCH )
         CALL PELACPY( 'All', JW-1, 1, T, 1+IROFFH+1, 1+IROFFH+JW-1,    
...
*
         IF( NS.GT.1 .AND. S.NE.ZERO ) THEN
            CALL PEORMHR( 'Right', 'No', JW+IROFFH, NS+IROFFH, 1+IROFFH,
...
*
     ...
     $        HHRSRC, HHCSRC, ICTXT, MAX(1, HHROWS), IERR )
         CALL PEGEMM( 'No', 'No', KLN, JW, JW, ONE, H, LTOP,         
     +KWTOP, DESCH, V, 1+IROFFH, 1+IROFFH, DESCV, ZERO,         WORK, 
     +1+IROFFHH, 1+ICOFFHH, DESCHH )
         CALL PELACPY( 'All', KLN, JW, WORK, 1+IROFFHH, 1+ICOFFHH,      
...
*
     ...
     $           HHRSRC, HHCSRC, ICTXT, MAX(1, HHROWS), IERR )
            CALL PEGEMM( 'Tr', 'No', JW, KLN, JW, ONE, V,            
     +1+IROFFH, 1+IROFFH, DESCV, H, KWTOP, KBOT+1,            DESCH, 
     +ZERO, WORK, 1+IROFFHH, 1+ICOFFHH, DESCHH )
            CALL PELACPY( 'All', JW, KLN, WORK, 1+IROFFHH, 1+ICOFFHH,   
...
*
     ...
     $           ZZRSRC, ZZCSRC, ICTXT, MAX(1, ZZROWS), IERR )
            CALL PEGEMM( 'No', 'No', KLN, JW, JW, ONE, Z, ILOZ,         
     +   KWTOP, DESCZ, V, 1+IROFFH, 1+IROFFH, DESCV,            ZERO, 
     +WORK, 1+IROFFZZ, 1+ICOFFZZ, DESCZZ )
            CALL PELACPY( 'All', KLN, JW, WORK, 1+IROFFZZ, 1+ICOFFZZ,   
