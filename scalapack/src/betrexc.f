      SUBROUTINE BETREXC( N, T, LDT, IFST, ILST, NITRAF, ITRAF,         
...
*  =======
*
*  BETREXC reorders the real Schur factorization of a real matrix
*  A = Q*T*Q**T, so that the diagonal block of T with row index IFST is
*  moved to row ILST.
*
*  The real Schur form T is reordered by an orthogonal similarity
*  transformation Z**T*T*Z. In contrast to the LAPACK routine ETREXC,
...
*
*  DTRAF   (output) REAL(KIND=10) array, length NDTRAF
*          List of parameters for representing the transformation
*          matrix Z, see further details.
*
...
*
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BELAEXC( N, T, LDT, HERE, NBF, NBNEXT, ITRAF(CITRAF),  
...
*
     ...
     $            NBNEXT = 2
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BELAEXC( N, T, LDT, HERE+1, 1, NBNEXT, ITRAF(CITRAF),  
     +                   DTRAF(CDTRAF), WORK, INFO )
            IF( INFO.NE.0 ) THEN
               ILST = HERE
               NITRAF = CITRAF - 1
...
*
               ...
               IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                  ...
                  RETURN
               END IF
               CALL BELAEXC( N, T, LDT, HERE, 1, NBNEXT, ITRAF(CITRAF), 
     +                       DTRAF(CDTRAF), WORK, INFO )
               CITRAF = CITRAF + LITRAF
               CDTRAF = CDTRAF + LDTRAF
               HERE = HERE + 1
...
*
                  ...
                  IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                     ...
                     RETURN
                  END IF
                  CALL BELAEXC( N, T, LDT, HERE, 1, NBNEXT,             
...
*
                  ...
                  IF( CDTRAF+2*LDTRAF-1.GT.NDTRAF ) THEN
                     ...
                     RETURN
                  END IF
                  CALL BELAEXC( N, T, LDT, HERE, 1, 1, ITRAF(CITRAF),   
     +                        DTRAF(CDTRAF), WORK, INFO )
                  CITRAF = CITRAF + LITRAF
                  CDTRAF = CDTRAF + LDTRAF
                  CALL BELAEXC( N, T, LDT, HERE+1, 1, 1, ITRAF(CITRAF), 
     +                          DTRAF(CDTRAF), WORK, INFO )
                  CITRAF = CITRAF + LITRAF
                  CDTRAF = CDTRAF + LDTRAF
                  HERE = HERE + 2
...
*
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BELAEXC( N, T, LDT, HERE-NBNEXT, NBNEXT, NBF,          
...
*
     ...
     $            NBNEXT = 2
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BELAEXC( N, T, LDT, HERE-NBNEXT, NBNEXT, 1,            
     +         ITRAF(CITRAF), DTRAF(CDTRAF), WORK, INFO )
            IF( INFO.NE.0 ) THEN
               ILST = HERE
               NITRAF = CITRAF - 1
...
*
               ...
               IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                  ...
                  RETURN
               END IF
               CALL BELAEXC( N, T, LDT, HERE, NBNEXT, 1, ITRAF(CITRAF), 
     +                       DTRAF(CDTRAF), WORK, INFO )
               CITRAF = CITRAF + LITRAF
               CDTRAF = CDTRAF + LDTRAF
               HERE = HERE - 1
...
*
                  ...
                  IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                     ...
                     RETURN
                  END IF
                  CALL BELAEXC( N, T, LDT, HERE-1, 2, 1, ITRAF(CITRAF), 
     +                          DTRAF(CDTRAF), WORK, INFO )
                  IF( INFO.NE.0 ) THEN
                     INFO = 2
                     ILST = HERE
