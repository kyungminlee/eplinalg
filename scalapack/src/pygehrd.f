      SUBROUTINE PYGEHRD( N, ILO, IHI, A, IA, JA, DESCA, TAU, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGEHRD reduces a complex general distributed matrix sub( A )
...
*     .. External Subroutines ..
     ...
     +               INFOG2L, PYGEMM, PYGEHD2, PCHK1MAT,                
     +    PYLAHRD, PYLARFB, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEHRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PYLAHRD( IHI, K, IB, A, IA, J, DESCA, TAU, WORK( IPT ),   
...
*
         CALL PYELSET2( EI, A, I+IB, J+IB-1, DESCA, ONE )
         CALL PYGEMM( 'No transpose', 'Conjugate transpose', IHI,       
     +          IHI-K-IB+1, IB, -ONE, WORK( IPY ), 1, JY, DESCY,        
     +         A, I+IB, J, DESCA, ONE, A, IA, J+IB, DESCA )
         CALL PYELSET( A, I+IB, J+IB-1, DESCA, EI )
*
...
*
      CALL PYGEHD2( N, K, IHI, A, IA, JA, DESCA, TAU, WORK, LWORK,      
     +         IINFO )
*
