      SUBROUTINE PEPOTRS( UPLO, N, NRHS, A, IA, JA, DESCA, B, IB, JB,   
     +                  DESCB, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEPOTRS solves a system of linear equations
*
*                      sub( A ) * X = sub( B )
...
*  factorization sub( A ) = U**T*U or L*L**T computed by PEPOTRF.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PETRSM,     
     +               PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPOTRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PETRSM( 'Left', 'Upper', 'Transpose', 'Non-unit', N, NRHS,
...
*
         CALL PETRSM( 'Left', 'Lower', 'No transpose', 'Non-unit', N,   
     +            NRHS, ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
*        Solve L'*X = sub( B ), overwriting sub( B ) with X.
*
         CALL PETRSM( 'Left', 'Lower', 'Transpose', 'Non-unit', N, NRHS,
     +               ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
      END IF
*
      RETURN
