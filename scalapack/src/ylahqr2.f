      SUBROUTINE YLAHQR2( WANTT, WANTZ, N, ILO, IHI, H, LDH, W, ILOZ,   
     +                  IHIZ, Z, LDZ, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  YLAHQR2 is an auxiliary routine called by YHSEQR to update the
*    eigenvalues and Schur decomposition already computed by YHSEQR, by
*    dealing with the Hessenberg submatrix in rows and columns ILO to IHI.
*  This version of YLAHQR (not the standard LAPACK version) uses a
*    double-shift algorithm (like LAPACK's ELAHQR).
...
*
*  H       (input/output) COMPLEX(KIND=10) array, dimension (LDH,N)
...
*
*  W       (output) COMPLEX(KIND=10) array, dimension (N)
...
*
*  Z       (input/output) COMPLEX(KIND=10) array, dimension (LDZ,N)
...
*     .. Parameters ..
      ...
      PARAMETER          ( DAT1 = 0.75E0_10, DAT2 = -0.4375E0_10 )
*     ..
*     .. Local Scalars ..
      INTEGER            I, I1, I2, ITN, ITS, J, K, L, M, NH, NR, NZ
      REAL(KIND=10)   CS, OVFL, S, SMLNUM, TST1, ULP, UNFL
      COMPLEX(KIND=10)         CDUM, H00, H10, H11, H12, H21, H22, H33, 
     +H33S,                    H43H34, H44, H44S, SN, SUM, T1, T2, T3, 
     +V1, V2,                    V3
*     ..
*     .. Local Arrays ..
...
*     .. External Functions ..
      ...
      EXTERNAL           ELAMCH, YLANHS
*     ..
*     .. External Subroutines ..
      EXTERNAL           ELABAD, YCOPY, YLANV2, YLARFG, YROT
*     ..
*     .. Intrinsic Functions ..
...
*
         DO 20 K = I, L + 1, -1
            ...
            IF( TST1.EQ.RZERO )          TST1 = YLANHS( '1', I-L+1, H( 
     +L, L ), LDH, RWORK )
...
*
            NR = MIN( 3, I-K+1 )
            IF( K.GT.M )          CALL YCOPY( NR, H( K, K-1 ), 1, V, 1 )
            CALL YLARFG( NR, V( 1 ), V( 2 ), 1, T1 )
            IF( K.GT.M ) THEN
               H( K, K-1 ) = V( 1 )
               H( K+1, K-1 ) = ZERO
               IF( K.LT.I-1 )
...
*
               DO 60 J = K, I2
                  SUM = CONJG( T1 )*H( K, J ) +                   
     +CONJG( T2 )*H( K+1, J ) +                   CONJG( T3 )*H( K+2, J 
     +)
                  H( K, J ) = H( K, J ) - SUM
...
*
         CALL YLANV2( H( I-1, I-1 ), H( I-1, I ), H( I, I-1 ),          
...
*
            IF( I2.GT.I )          CALL YROT( I2-I, H( I-1, I+1 ), LDH, 
     +H( I, I+1 ), LDH,                     CS, SN )
            CALL YROT( I-I1-1, H( I1, I-1 ), 1, H( I1, I ), 1, CS,      
     +            CONJG( SN ) )
         END IF
         IF( WANTZ ) THEN
*
...
*
            CALL YROT( NZ, Z( ILOZ, I-1 ), 1, Z( ILOZ, I ), 1, CS,      
     +            CONJG( SN ) )
         END IF
*
      END IF
