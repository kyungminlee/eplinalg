      SUBROUTINE PETRRFS( UPLO, TRANS, DIAG, N, NRHS, A, IA, JA, DESCA, 
...
*  =======
*
*  PETRRFS provides error bounds and backward error estimates for the
*  solution to a system of linear equations with a triangular
*  coefficient matrix.
*
*  The solution matrix X must be computed by PETRTRS or some other
*  means before entering this routine.  PETRRFS does not do iterative
...
*
*  A       (local input) REAL(KIND=10) pointer into the local memory
...
*
*  B       (local input) REAL(KIND=10) pointer into the local memory
...
*
*  X       (local input) REAL(KIND=10) pointer into the local memory
...
*     .. External Subroutines ..
     ...
     +               EGEBR2D, EGEBS2D, INFOG2L, PCHK1MAT,               
     +     PCHK2MAT, PEATRMV, PEAXPY, PECOPY,                    
     +PELACON, PETRMV, PETRSV, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                  IX, JX, DESCX, 17, 5, IDUM1, IDUM2, INFO )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PETRRFS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PECOPY( N, X, IX, JX+K, DESCX, 1, WORK( IPR ), IW, JW,    
     +             DESCW, 1 )
         CALL PETRMV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA,           
     +      WORK( IPR ), IW, JW, DESCW, 1 )
         CALL PEAXPY( N, -ONE, B, IB, JB+K, DESCB, 1, WORK( IPR ), IW,  
     +               JW, DESCW, 1 )
*
...
*
   ...
   20          CONTINUE
         ...
         END IF
*
         CALL PEATRMV( UPLO, TRANS, DIAG, N, ONE, A, IA, JA, DESCA, X,  
...
*
               CALL PETRSV( UPLO, TRANST, DIAG, N, A, IA, JA, DESCA,    
     +                   WORK( IPR ), IW, JW, DESCW, 1 )
               IF( MYCOL.EQ.IXBCOL ) THEN
                  IF( NP.GT.0 ) THEN
...
*
   ...
   70                CONTINUE
                  END IF
               END IF
               CALL PETRSV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA,     
     +                  WORK( IPR ), IW, JW, DESCW, 1 )
            END IF
            GO TO 50
         END IF
...
*           where op(A) = A or A', depending on TRANS.
*
            CALL PECOPY( N, X, IX, J+K, DESCX, 1, WORK( IPR ), IW, JW,  
     +                  DESCW, 1 )
            CALL PETRMV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA,        
     +            WORK( IPR ), IW, JW, DESCW, 1 )
            CALL PEAXPY( N, -ONE, B, IB, J+K, DESCB, 1, WORK( IPR ),    
     +                IW, JW, DESCW, 1 )
*
*           Compute componentwise relative backward error from formula
*
...
*
  ...
  100             CONTINUE
            ...
            END IF
*
            CALL PEATRMV( UPLO, TRANS, DIAG, N, ONE, A, IA, JA, DESCA,  
...
*
  ...
  160             CONTINUE
               END IF
               CALL EGAMX2D( ICTXT, 'Column', ' ', 1, 1, LSTRES, 1,     
     +                   IDUM, IDUM, 1, -1, MYCOL )
               IF( LSTRES.NE.ZERO )
     $            FERR( JJFBE ) = EST / LSTRES
*
