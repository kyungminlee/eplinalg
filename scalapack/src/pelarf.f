      SUBROUTINE PELARF( SIDE, M, N, V, IV, JV, DESCV, INCV, TAU,       
     +             C, IC, JC, DESCC, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELARF applies a real elementary reflector Q (or Q**T) to a real
...
*     .. External Subroutines ..
     ...
     +             EGEMV, EGER, EGERV2D, EGESD2D,                    
     +EGSUM2D, ELASET, INFOG2L, PB_TOPGET,                    PBETRNV
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
               IPW = MP+1
               CALL PBETRNV( ICTXT, 'Rowwise', 'Transpose', M,          
...
*
                     IF( MP.GT.0 ) THEN
                        CALL EGEMV( 'Transpose', MP, NQ, ONE,           
     +                    C( IOFFC ), LDC, WORK, 1, ZERO,               
     +                WORK( IPW ), 1 )
                     ELSE
                        CALL ELASET( 'All', NQ, 1, ZERO, ZERO,          
     +                      WORK( IPW ), MAX( 1, NQ ) )
                     END IF
                     CALL EGSUM2D( ICTXT, 'Columnwise', ' ', NQ, 1,     
     +                         WORK( IPW ), MAX( 1, NQ ), RDEST,        
     +                      MYCOL )
*
*                    sub( C ) := sub( C ) - v * w'
*
                     CALL EGER( MP, NQ, -TAULOC( 1 ), WORK, 1,          
...
*                       sub( C ) := sub( C ) - v * w'
*
                        CALL EGER( MP, NQ, -TAULOC( 1 ), V( IOFFV ), 1, 
     +                             WORK, 1, C( IOFFC ), LDC )
                     END IF
*
                  END IF
...
*
                     IPW = MP+1
                     CALL ECOPY( MP, V( IOFFV ), 1, WORK, 1 )
                     WORK( IPW ) = TAU( JJV )
...
*
                  IF( MP.GT.0 ) THEN
                     IF( IOFFC.GT.0 )                   CALL EGEMV( 
     +'Transpose', MP, NQ, ONE,                               C( IOFFC 
     +), LDC, WORK, 1, ZERO,                               WORK( IPW ), 
     +1 )
                  ELSE
                     CALL ELASET( 'All', NQ, 1, ZERO, ZERO,             
     +                WORK( IPW ), MAX( 1, NQ ) )
...
*                 sub( C ) := sub( C ) - v * w'
*
                  IF( IOFFC.GT.0 )                CALL EGER( MP, NQ, 
     +-TAULOC( 1 ), WORK, 1,                           WORK( IPW ), 1, 
     +C( IOFFC ), LDC )
               END IF
*
            ELSE
...
*
                        IF( NQ.GT.0 ) THEN
                           CALL EGEMV( 'No transpose', MP, NQ, ONE,     
     +                             C( IOFFC ), LDC, V( IOFFV ), LDV,    
     +                              ZERO, WORK, 1 )
                        ELSE
                           CALL ELASET( 'All', MP, 1, ZERO, ZERO,       
     +                            WORK, MAX( 1, MP ) )
                        END IF
                        CALL EGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,     
...
*
                     IPW = NQ+1
                     CALL ECOPY( NQ, V( IOFFV ), LDV, WORK, 1 )
                     WORK(IPW) = TAU( IIV )
                     CALL EGESD2D( ICTXT, IPW, 1, WORK, IPW, ICROW,     
     +                         MYCOL )
*
...
*                       sub( C ) := sub( C ) - w * v'
*
                        CALL EGER( MP, NQ, -TAULOC( 1 ), WORK( IPW ), 1,
     +                              WORK, 1, C( IOFFC ), LDC )
                     END IF
*
                  END IF
...
*
               IPW = NQ+1
               CALL PBETRNV( ICTXT, 'Columnwise', 'Transpose', N,       
...
*
     ...
     +          WORK( IPW ), 1 )
                  ELSE
                     CALL ELASET( 'All', MP, 1, ZERO, ZERO, WORK( IPW ),
     +                             MAX( 1, MP ) )
                  END IF
                  CALL EGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,           
     +                WORK( IPW ), MAX( 1, MP ), RDEST,                 
...
*                 sub( C ) := sub( C ) - w * v'
*
                  CALL EGER( MP, NQ, -TAULOC( 1 ), WORK( IPW ), 1, WORK,
     +                        1, C( IOFFC ), LDC )
               END IF
*
            END IF
