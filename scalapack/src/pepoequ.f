      SUBROUTINE PEPOEQU( N, A, IA, JA, DESCA, SR, SC, SCOND, AMAX,     
     +                INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * )
      REAL(KIND=10)   A( * ), SC( * ), SR( * )
*     ..
*
*  Purpose
*  =======
*
*  PEPOEQU computes row and column scalings intended to
...
*
*  SR      (local output) REAL(KIND=10) array, dimension LOCr(M_A)
...
*
*  SC      (local output) REAL(KIND=10) array, dimension LOCc(N_A)
...
*
*  SCOND   (global output) REAL(KIND=10)
...
*     .. External Subroutines ..
     ...
     +               EGAMX2D, EGSUM2D, IGAMN2D, INFOG2L,                
     +    PCHK1MAT, PB_TOPGET, PXERBLA
*     ..
*     .. External Functions ..
      INTEGER            ICEIL, NUMROC
      REAL(KIND=10)   PELAMCH
      EXTERNAL           ICEIL, NUMROC, PELAMCH
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPOEQU', -INFO )
         RETURN
      END IF
*
...
*
      CALL EGSUM2D( ICTXT, 'Columnwise', COLCTOP, 1, NQ, SC( JJA ),     
     +          1, -1, MYCOL )
      CALL EGSUM2D( ICTXT, 'Rowwise', ROWCTOP, NP, 1, SR( IIA ),        
     +       MAX( 1, NP ), -1, MYCOL )
*
      CALL EGAMX2D( ICTXT, 'All', ALLCTOP, 1, 1, AMAX, 1, IDUMM, IDUMM, 
     +              -1, -1, MYCOL )
      CALL EGAMN2D( ICTXT, 'All', ALLCTOP, 1, 1, SMIN, 1, IDUMM, IDUMM, 
     +              -1, -1, MYCOL )
*
