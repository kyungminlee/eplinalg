      SUBROUTINE ESTEGR2( JOBZ, RANGE, N, D, E, VL, VU, IL, IU,         
...
*  =======
*
*  ESTEGR2 computes selected eigenvalues and, optionally, eigenvectors
...
*
*  ESTEGR2 has been adapted from LAPACK's ESTEGR. Please note the
...
*     ESTEGR2  ONLY computes the eigenpairs
...
*                if INFO = 10X, internal error in ELARRE2,
*                if INFO = 20X, internal error in ELARRV.
*                Here, the digit X = ABS( IINFO ) < 10, where IINFO is 
*                the nonzero error code returned by ELARRE2 or 
*                ELARRV, respectively.
*
*  =====================================================================
*
...
*     .. Local Scalars ..
     ...
     $                   NZCMIN
      REAL(KIND=10)   BIGNUM, EPS, PIVMIN, RMAX, RMIN, RTOL1, RTOL2,    
...
*     .. External Subroutines ..
      EXTERNAL           ECOPY, ELAE2, ELAEV2, ELARRC, ELARRE2,         
     +           ELARRV, ELASRT, ESCAL, ESWAP
*     ..
*     .. Intrinsic Functions ..
...
*     ESTEGR2 needs WORK of size 6*N, IWORK of size 3*N.
*     In addition, ELARRE2 needs WORK of size 6*N, IWORK of size 5*N.
...
*        It is either given by the user or computed in ELARRE2.
         WL = VL
         WU = VU
      ELSEIF( INDEIG ) THEN
...
*     are small
*     THRESH is the splitting parameter for ELARRE2      
...
*     Set the tolerance parameters for bisection
      IF( .NOT.WANTZ ) THEN
*        ELARRE2 computes the eigenvalues to full precision.   
         RTOL1 = FOUR * EPS
         RTOL2 = FOUR * EPS
      ELSE   
*        ELARRE2 computes the eigenvalues to less than full precision.
*        ELARRV will refine the eigenvalue approximations, and we can
*        need less accurate initial bisection in ELARRE2.
*        Note: these settings do only affect the subset case and ELARRE2
         RTOL1 = SQRT(EPS)
         RTOL2 = MAX( SQRT(EPS)*5.0E-3_10, FOUR * EPS )
      ENDIF
      CALL ELARRE2( RANGE, N, WL, WU, IIL, IIU, D, E,               
     +WORK(INDE2), RTOL1, RTOL2, THRESH, NSPLIT,               IWORK( 
     +IINSPL ), M, DOL, DOU,              W, WORK( INDERR ),            
     +  WORK( INDGP ), IWORK( IINDBL ),              IWORK( IINDW ), 
     +WORK( INDGRS ), PIVMIN,              WORK( INDWRK ), IWORK( 
     +IINDWK ), IINFO )
      IF( IINFO.NE.0 ) THEN
         INFO = 100 + ABS( IINFO )
         RETURN
      END IF
*     Note that if RANGE .NE. 'V', ELARRE2 computes bounds on the desired
...
*        eigenvalues
*
         CALL ELARRV( N, WL, WU, D, E,                 PIVMIN, IWORK( 
     +IINSPL ), M,                  DOL, DOU, MINRGP, RTOL1, RTOL2,     
     ...
     +    ISUPPZ, WORK( INDWRK ), IWORK( IINDWK ), IINFO )
         ...
         END IF
      ELSE
*        ELARRE2 computes eigenvalues of the (shifted) root representation
*        ELARRV returns the eigenvalues of the unshifted matrix.
*        However, if the eigenvectors are not desired by the user, we need
*        to apply the corresponding shifts from ELARRE2 to obtain the 
...
*
      IF( NSPLIT.GT.1 ) THEN
         IF( .NOT. WANTZ ) THEN
            CALL ELASRT( 'I', DOU - DOL +1, W(DOL), IINFO )
            IF( IINFO.NE.0 ) THEN
               INFO = 3
               RETURN
            END IF
