      SUBROUTINE PYLATRD( UPLO, N, NB, A, IA, JA, DESCA, D, E, TAU, W,  
     +                   IW, JW, DESCW, WORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCW( * )
      REAL(KIND=10)               D( * ), E( * )
      COMPLEX(KIND=10)            A( * ), TAU( * ), W( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYLATRD reduces NB rows and columns of a complex Hermitian
...
*
*  If UPLO = 'U', PYLATRD reduces the last NB rows and columns of a
*  matrix, of which the upper triangle is supplied;
*  if UPLO = 'L', PYLATRD reduces the first NB rows and columns of a
*  matrix, of which the lower triangle is supplied.
*
*  This is an auxiliary routine called by PYHETRD.
*
*  Notes
*  =====
*
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      COMPLEX(KIND=10)            HALF, ONE, ZERO
      PARAMETER          ( HALF = ( 0.5E0_10, 0.0E0_10 ),               
     +     ONE = ( 1.0E0_10, 0.0E0_10 ),                    ZERO = ( 
     +0.0E0_10, 0.0E0_10 ) )
*     ..
*     .. Local Scalars ..
      INTEGER            I, IACOL, IAROW, ICTXT, II, J, JJ, JP, JWK, K,
     $                   KW, MYCOL, MYROW, NPCOL, NPROW, NQ
      COMPLEX(KIND=10)            AII, ALPHA, BETA
*     ..
*     .. Local Arrays ..
      INTEGER            DESCD( DLEN_ ), DESCE( DLEN_ ), DESCWK( DLEN_ )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DESCSET, INFOG2L, PYAXPY,      
     +              PYDOTC, PYELGET, PYELSET, PYGEMV,                   
     + PYHEMV, PYLACGV, PYLARFG, PYSCAL,                    PEELSET, 
     +EGEBR2D, EGEBS2D
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*           Update A(IA:I,I)
*
            CALL PYELGET( 'E', ' ', AII, A, I, J, DESCA )
            CALL PYELSET( A, I, J, DESCA, CMPLX( REAL( AII , KIND=10) , 
     +KIND=10) )
            CALL PYLACGV( N-K, W, IW+K-1, JW+KW, DESCW, DESCW( M_ ) )
            CALL PYGEMV( 'No transpose', K, N-K, -ONE, A, IA, J+1,      
     +              DESCA, W, IW+K-1, JW+KW, DESCW, DESCW( M_ ),        
     +            ONE, A, IA, J, DESCA, 1 )
            CALL PYLACGV( N-K, W, IW+K-1, JW+KW, DESCW, DESCW( M_ ) )
            CALL PYLACGV( N-K, A, I, J+1, DESCA, DESCA( M_ ) )
            CALL PYGEMV( 'No transpose', K, N-K, -ONE, W, IW, JW+KW,    
     ...
     +               IA, J, DESCA, 1 )
            ...
            CALL PYELSET( A, I, J, DESCA, CMPLX( REAL( AII , KIND=10) , 
     +KIND=10) )
            IF( N-K.GT.0 )          CALL PYELSET( A, I, J+1, DESCA, 
     +CMPLX( E( JP ) , KIND=10) )
*
...
*
            JP = MIN( JJ+KW-1, NQ )
            CALL PYLARFG( K-1, BETA, I-1, J, A, IA, J, DESCA, 1,        
     +             TAU )
            CALL PEELSET( E, 1, J, DESCE, REAL( BETA , KIND=10) )
            CALL PYELSET( A, I-1, J, DESCA, ONE )
*
*           Compute W(IW:IW+K-2,JW+KW-1)
*
            CALL PYHEMV( 'Upper', K-1, ONE, A, IA, JA, DESCA, A, IA, J, 
     +                   DESCA, 1, ZERO, W, IW, JW+KW-1, DESCW, 1 )
*
            JWK = MOD( K-1, DESCWK( NB_ ) ) + 2
            CALL PYGEMV( 'Conjugate transpose', K-1, N-K, ONE, W, IW,   
     ...
     +                 JWK, DESCWK, DESCWK( M_ ) )
            CALL PYGEMV( 'No transpose', K-1, N-K, -ONE, W, IW, JW+KW,  
     +                  DESCW, WORK, 1, JWK, DESCWK, DESCWK( M_ ), ONE, 
     +                   W, IW, JW+KW-1, DESCW, 1 )
            CALL PYSCAL( K-1, TAU( JP ), W, IW, JW+KW-1, DESCW, 1 )
*
            CALL PYDOTC( K-1, ALPHA, W, IW, JW+KW-1, DESCW, 1, A, IA, J,
     +                    DESCA, 1 )
            IF( MYCOL.EQ.IACOL )
     $         ALPHA = -HALF*TAU( JP )*ALPHA
            CALL PYAXPY( K-1, ALPHA, A, IA, J, DESCA, 1, W, IW, JW+KW-1,
     +                    DESCW, 1 )
            CALL PYELGET( 'E', ' ', BETA, A, I, J, DESCA )
            CALL PEELSET( D, 1, J, DESCD, REAL( BETA , KIND=10) )
*
   10    CONTINUE
*
      ELSE
...
*
            ...
            CALL PYELSET( A, I, J, DESCA, CMPLX( REAL( AII , KIND=10) , 
     +KIND=10) )
            CALL PYLACGV( K-1, W, IW+K-1, JW, DESCW, DESCW( M_ ) )
            CALL PYGEMV( 'No transpose', N-K+1, K-1, -ONE, A, I, JA,    
     +                DESCA, W, IW+K-1, JW, DESCW, DESCW( M_ ), ONE,    
     +                A, I, J, DESCA, 1 )
            CALL PYLACGV( K-1, W, IW+K-1, JW, DESCW, DESCW( M_ ) )
            CALL PYLACGV( K-1, A, I, JA, DESCA, DESCA( M_ ) )
            CALL PYGEMV( 'No transpose', N-K+1, K-1, -ONE, W, IW+K-1,   
     ...
     +                A, I, J, DESCA, 1 )
            ...
            CALL PYELSET( A, I, J, DESCA, CMPLX( REAL( AII , KIND=10) , 
     +KIND=10) )
            IF( K.GT.1 )          CALL PYELSET( A, I, J-1, DESCA, 
     +CMPLX( E( JP ) , KIND=10) )
*
*
*           Generate elementary reflector H(i) to annihilate
...
*
            JP = MIN( JJ+K-1, NQ )
            CALL PYLARFG( N-K, BETA, I+1, J, A, I+2, J, DESCA, 1,       
     +              TAU )
            CALL PEELSET( E, 1, J, DESCE, REAL( BETA , KIND=10) )
            CALL PYELSET( A, I+1, J, DESCA, ONE )
*
...
*
            CALL PYHEMV( 'Lower', N-K, ONE, A, I+1, J+1, DESCA, A, I+1, 
     +                   J, DESCA, 1, ZERO, W, IW+K, JW+K-1, DESCW, 1 )
*
            CALL PYGEMV( 'Conjugate Transpose', N-K, K-1, ONE, W, IW+K, 
     +                   JW, DESCW, A, I+1, J, DESCA, 1, ZERO, WORK, 1, 
     +                   1, DESCWK, DESCWK( M_ ) )
            CALL PYGEMV( 'No transpose', N-K, K-1, -ONE, A, I+1, JA,    
     +               DESCA, WORK, 1, 1, DESCWK, DESCWK( M_ ), ONE, W,   
     +                IW+K, JW+K-1, DESCW, 1 )
            CALL PYGEMV( 'Conjugate transpose', N-K, K-1, ONE, A, I+1,  
     +                 JA, DESCA, A, I+1, J, DESCA, 1, ZERO, WORK, 1,   
     +                1, DESCWK, DESCWK( M_ ) )
            CALL PYGEMV( 'No transpose', N-K, K-1, -ONE, W, IW+K, JW,   
     +                DESCW, WORK, 1, 1, DESCWK, DESCWK( M_ ), ONE, W,  
     +                 IW+K, JW+K-1, DESCW, 1 )
            CALL PYSCAL( N-K, TAU( JP ), W, IW+K, JW+K-1, DESCW, 1 )
            CALL PYDOTC( N-K, ALPHA, W, IW+K, JW+K-1, DESCW, 1, A, I+1, 
     +                   J, DESCA, 1 )
            IF( MYCOL.EQ.IACOL )
     $         ALPHA = -HALF*TAU( JP )*ALPHA
            CALL PYAXPY( N-K, ALPHA, A, I+1, J, DESCA, 1, W, IW+K,      
     +              JW+K-1, DESCW, 1 )
            CALL PYELGET( 'E', ' ', BETA, A, I, J, DESCA )
            CALL PEELSET( D, 1, J, DESCD, REAL( BETA , KIND=10) )
*
...
*
      IF( MYCOL.EQ.IACOL ) THEN
         IF( MYROW.EQ.IAROW ) THEN
            CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, NB, D( JJ ), 1 )
         ELSE
            CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, NB, D( JJ ), 1,  
     +                   IAROW, MYCOL )
         END IF
      END IF
*
