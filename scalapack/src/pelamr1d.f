      SUBROUTINE PELAMR1D( N, A, IA, JA, DESCA, B, IB, JB, DESCB )
      IMPLICIT NONE
*
...
*
*  PELAMR1D has not been tested except withint the contect of
...
*  =======
*
*  PELAMR1D redistributes a one-dimensional row vector from one data
*  decomposition to another.
*
*  This is an auxiliary routine called by PESYTRD to redistribute D, E
*  and TAU.
*
*  Notes
*  =====
*
*  Although all processes call PEGEMR2D, only the processes that own
*  the first column of A send data and only processes that own the
*  first column of B receive data.  The calls to EGEBS2D/EGEBR2D
...
*
*  A       (local output) COMPLEX(KIND=10) pointer into the
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGEBR2D, EGEBS2D, PEGEMR2D
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      EXTERNAL           NUMROC
...
*
      ICTXT = DESCB( CTXT_ )
      CALL PEGEMR2D( 1, N, A, IA, JA, DESCAA, B, IB, JB, DESCBB, ICTXT )
*
      CALL BLACS_GRIDINFO( ICTXT, NPROW, NPCOL, MYROW, MYCOL )
      NQ = NUMROC( N, DESCB( NB_ ), MYCOL, 0, NPCOL )
*
      IF( MYROW.EQ.0 ) THEN
         CALL EGEBS2D( ICTXT, 'C', ' ', NQ, 1, B, NQ )
      ELSE
         CALL EGEBR2D( ICTXT, 'C', ' ', NQ, 1, B, NQ, 0, MYCOL )
      END IF
*
      RETURN
*
