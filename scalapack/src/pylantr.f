      REAL(KIND=10)               FUNCTION PYLANTR( NORM, UPLO, DIAG, M,
     + N, A,                                      IA, JA, DESCA, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLANTR returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PYLANTR as described
*          above.
*
...
*          of the distributed submatrix sub( A ). When M = 0, PYLANTR is
*          set to zero. M >= 0.
*
*  N       (global input) INTEGER
*          The number of columns to be operated on i.e the number of
*          columns of the distributed submatrix sub( A ). When N = 0,
*          PYLANTR is set to zero. N >= 0.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YLASSQ, INFOG2L, PETREECOMB,   
     +                 ECOMBSSQ, EGEBR2D, EGEBS2D,                    
     +EGAMX2D, EGSUM2D
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*                 including diagonal block.
                  IF( UDIAG ) THEN
                     DO 840 LL = JJ, JJ + JB -1
                        COLSSQ(1) = ZERO
                        COLSSQ(2) = ONE
                        CALL YLASSQ( MIN( II+LL-JJ-1, IIA+MP-1 )-IIA+1, 
     +                               A( IIA+IOFFA ), 1,                 
     +               COLSSQ(1), COLSSQ(2) )
  ...
  840                CONTINUE
                  ELSE
                     DO 850 LL = JJ, JJ + JB -1
                        COLSSQ(1) = ZERO
                        COLSSQ(2) = ONE
                        CALL YLASSQ( MIN( II+LL-JJ, IIA+MP-1 )-IIA+1,   
     +                             A( IIA+IOFFA ), 1,                   
     +             COLSSQ(1), COLSSQ(2) )
                        CALL ECOMBSSQ( SSQ, COLSSQ )
                        IOFFA = IOFFA + LDA
...
*                 It seems this lassq will be length 0, since ii = iia.
                  DO 860 LL = JJ, JJ + JB -1
                     COLSSQ(1) = ZERO
                     COLSSQ(2) = ONE
                     CALL YLASSQ( MIN( II-1, IIA+MP-1 )-IIA+1,          
     +                   A( IIA+IOFFA ), 1,                             
     +COLSSQ(1), COLSSQ(2) )
                     CALL ECOMBSSQ( SSQ, COLSSQ )
                     IOFFA = IOFFA + LDA
  ...
  920                CONTINUE
               ...
               ELSE
                  DO 930 LL = JJ, JJ + JB -1
                     COLSSQ(1) = ZERO
                     COLSSQ(2) = ONE
                     CALL YLASSQ( IIA+MP-II, A( II+IOFFA ), 1,          
     +                   COLSSQ(1), COLSSQ(2) )
                     CALL ECOMBSSQ( SSQ, COLSSQ )
                     IOFFA = IOFFA + LDA
  930             CONTINUE
