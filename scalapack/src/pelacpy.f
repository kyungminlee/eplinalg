      SUBROUTINE PELACPY( UPLO, M, N, A, IA, JA, DESCA, B, IB, JB,      
     +               DESCB )
      IMPLICIT NONE
*
...
*  =======
*
*  PELACPY copies all or part of a distributed matrix A to another
*  distributed matrix B.  No communication is performed, PELACPY
...
*
     ...
     $    N.LE.( DESCA( NB_ ) - MOD( JA-1, DESCA( NB_ ) ) ) ) THEN
         CALL PELACP2( UPLO, M, N, A, IA, JA, DESCA,                  B,
     + IB, JB, DESCB )
      ELSE
*
         IF( LSAME( UPLO, 'U' ) ) THEN
            CALL PELACP2( UPLO, IN-IA+1, N, A, IA, JA, DESCA,           
     +          B, IB, JB, DESCB )
            DO 10 I = IN+1, IA+M-1, DESCA( MB_ )
               ...
               JBB = JB + ITMP
               JAA = JA + ITMP
               CALL PELACP2( UPLO, IBLK, N-ITMP, A, I, JAA, DESCA,      
     +                  B, IBB, JBB, DESCB )
   10       CONTINUE
         ELSE IF( LSAME( UPLO, 'L' ) ) THEN
            CALL PELACP2( UPLO, M, JN-JA+1, A, IA, JA, DESCA,           
     +          B, IB, JB, DESCB )
            DO 20 J = JN+1, JA+N-1, DESCA( NB_ )
               ...
               JBB = JB + JTMP
               IAA = IA + JTMP
               CALL PELACP2( UPLO, M-JTMP, JBLK, A, IAA, J, DESCA,      
     +                  B, IBB, JBB, DESCB )
   20       CONTINUE
         ELSE
            IF( M.LE.N ) THEN
     ...
     +                B, IB, JB, DESCB )
               DO 30 I = IN+1, IA+M-1, DESCA( MB_ )
                  ...
                  IBLK = MIN( DESCA( MB_ ), M-ITMP )
                  IBB = IB+ITMP
                  CALL PELACP2( UPLO, IBLK, N, A, I, JA, DESCA,         
     +                  B, IBB, JB, DESCB )
   30          CONTINUE
            ELSE
               CALL PELACP2( UPLO, M, JN-JA+1, A, IA, JA, DESCA,        
     +                B, IB, JB, DESCB )
               DO 40 J = JN+1, JA+N-1, DESCA( NB_ )
                  ...
                  JBLK = MIN( DESCA( NB_ ), N-JTMP )
                  JBB = JB+JTMP
                  CALL PELACP2( UPLO, M, JBLK, A, IA, J, DESCA,         
     +                  B, IB, JBB, DESCB )
   40          CONTINUE
            END IF
         END IF
