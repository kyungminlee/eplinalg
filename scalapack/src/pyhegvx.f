      SUBROUTINE PYHEGVX( IBTYPE, JOBZ, RANGE, UPLO, N, A, IA, JA,      
...
*  =======
*
*  PYHEGVX computes all the eigenvalues, and optionally,
*  the eigenvectors
...
*
*  A       (local input/local output) COMPLEX(KIND=10) pointer into the
...
*          If DESCA( CTXT_ ) is incorrect, PYHEGVX cannot guarantee
*          correct error reporting.
*
*  B       (local input/local output) COMPLEX(KIND=10) pointer into the
...
*          insufficient space and PYHEGVX is not able to detect this
...
*          PYHEGVX is always able to detect insufficient space without
...
*             PYHEGVX attempts to maintain orthogonality in
...
*             is returned.  Note that when RANGE='V', PYHEGVX does
*             not know how many eigenvectors are requested until
*             the eigenvalues are computed.  Therefore, when RANGE='V'
*             and as long as LRWORK is large enough to allow PYHEGVX to
*             compute the eigenvalues, PYHEGVX will compute the
...
*             PYSTEIN will perform no better than YSTEIN on 1 processor.
...
*                  PYHEGVX from computing all of the eigenvectors
...
*     .. Parameters ..
     ...
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      ...
      REAL(KIND=10)   FIVE, ZERO
      PARAMETER          ( FIVE = 5.0E0_10, ZERO = 0.0E0_10 )
      INTEGER            IERRNPD
...
*     .. External Subroutines ..
     ...
     +               ESCAL, PCHK1MAT, PCHK2MAT, PXERBLA, PYHEEVX,       
     +             PYHENGST, PYPOTRF, PYTRMM, PYTRSM
*     ..
*     .. Intrinsic Functions ..
         ...
         IF( INFO.EQ.0 ) THEN
            IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
               ...
               ELSE
                  ...
                  RWORK( 3 ) = ZERO
               END IF
               CALL EGEBS2D( DESCA( CTXT_ ), 'ALL', ' ', 3, 1, RWORK,   
     +                     3 )
            ELSE
               CALL EGEBR2D( DESCA( CTXT_ ), 'ALL', ' ', 3, 1, RWORK, 3,
     +                        0, 0 )
            END IF
...
*
     ...
     $               ABS( ABSTOL ) ) THEN
         ...
         END IF
         IDUM2( 5 ) = 5
         CALL PCHK2MAT( N, 5, N, 5, IA, JA, DESCA, 9, N, 5, N, 5, IB,
     $                  JB, DESCB, 13, 5, IDUM1, IDUM2, INFO )
         CALL PCHK1MAT( N, 5, N, 5, IZ, JZ, DESCZ, 26, 0, IDUM1, IDUM2,
     $                  INFO )
      END IF
*
      IWORK( 1 ) = LIWMIN
      WORK( 1 ) = CMPLX( REAL( LWOPT , KIND=10) , KIND=10)
      RWORK( 1 ) = REAL( LRWOPT , KIND=10)
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHEGVX ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PYPOTRF( UPLO, N, B, IB, JB, DESCB, INFO )
      IF( INFO.NE.0 ) THEN
         IWORK( 1 ) = LIWMIN
         WORK( 1 ) = CMPLX( REAL( LWOPT , KIND=10) , KIND=10)
         RWORK( 1 ) = REAL( LRWOPT , KIND=10)
...
*
      CALL PYHENGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,      
     +          DESCB, SCALE, WORK, LWORK, INFO )
      CALL PYHEEVX( JOBZ, RANGE, UPLO, N, A, IA, JA, DESCA, VL, VU, IL, 
...
*
            ...
            END IF
*
            CALL PYTRSM( 'Left', UPLO, TRANS, 'Non-unit', N, NEIG, ONE, 
...
*
            ...
            END IF
*
            CALL PYTRMM( 'Left', UPLO, TRANS, 'Non-unit', N, NEIG, ONE, 
     +                   B, IB, JB, DESCB, Z, IZ, JZ, DESCZ )
         END IF
      END IF
*
      IF( SCALE.NE.ONE ) THEN
         CALL ESCAL( N, SCALE, W, 1 )
      END IF
*
      IWORK( 1 ) = LIWMIN
      WORK( 1 ) = CMPLX( REAL( LWOPT , KIND=10) , KIND=10)
