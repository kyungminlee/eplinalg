      SUBROUTINE PYUNMRQ( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*  =======
*
*  PYUNMRQ overwrites the general complex M-by-N distributed matrix
...
*        Q = H(1)' H(2)' . . . H(k)'
*
*  as returned by PYGERQF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          H(i), IA <= i <= IA+K-1, as returned by PYGERQF in the
...
*          elementary reflectors H(i) as returned by PYGERQF.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PYLARFB,    
     +                PYLARFT, PYUNMR2, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNMRQ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N - K + IB
         END IF
         CALL PYUNMR2( SIDE, TRANS, MI, NI, IB, A, IA, JA, DESCA, TAU,  
...
*
         CALL PYLARFT( 'Backward', 'Rowwise', NQ-K+I+IB-IA, IB,         
...
*        Apply H or H'
*
         CALL PYLARFB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI,     
