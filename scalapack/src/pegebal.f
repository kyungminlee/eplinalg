      SUBROUTINE PEGEBAL( JOB, N, A, DESCA, ILO, IHI, SCALE, INFO )
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * )
      REAL(KIND=10)   A( * ), SCALE( * )
*     ..
*
*  Purpose
*  =======
*
*  PEGEBAL balances a general real matrix A.  This involves, first,
...
*
*  SCALE   (global output) REAL(KIND=10) array, dimension (N)
...
*     .. External Functions ..
      ...
      INTEGER            IEAMAX
      REAL(KIND=10)   ELAMCH
      EXTERNAL           EISNAN, LSAME, ELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           PESCAL, PESWAP, PEAMAX, PXERBLA,               
     +     BLACS_GRIDINFO, CHK1MAT, EGSUM2D,                    INFOG2L,
     + PEELGET
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $    .NOT.LSAME( JOB, 'S' ) .AND. .NOT.LSAME( JOB, 'B' ) ) THEN
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEBAL', -INFO )
         RETURN
      END IF
*
...
*
   20 CONTINUE
     ...
     $   GO TO 30
*
      CALL PESWAP( L, A, 1, J, DESCA, 1, A, 1, M, DESCA, 1 )
      CALL PESWAP( N-K+1, A, J, K, DESCA, DESCA(M_), A, M, K, DESCA,    
     +          DESCA(M_) )
*
   30 CONTINUE
      GO TO ( 40, 80 )IEXC
...
*
            CALL PEELGET( 'All', '1-Tree', ELEM, A, J, I, DESCA )
            IF( ELEM.NE.ZERO )
     $         GO TO 70
   60    CONTINUE
*
...
*           All processors need the information to make correct decisions.
*
            CALL PEELGET( 'All', '1-Tree', ELEM, A, I, J, DESCA )
            IF( ELEM.NE.ZERO )
     $         GO TO 110
  100    CONTINUE
*
...
*
  ...
  150    CONTINUE
         CR( 1 ) = C
         CR( 2 ) = R
         CALL EGSUM2D( ICTXT, 'All', '1-Tree', 2, 1, CR, 2, -1, -1 )
         C = CR( 1 )
         R = CR( 2 )
*
*        Find global maximum absolute values and indices in parallel.
*
         CALL PEAMAX( L, CA, ICA, A, 1, I, DESCA, 1 )
         CALL PEAMAX( N-K+1, RA, IRA, A, I, K, DESCA, DESCA(M_) )
*
...
*
  190    CONTINUE
     ...
     $         GO TO 200
         ...
         NOCONV = .TRUE.
*
         CALL PESCAL( N-K+1, G, A, I, K, DESCA, DESCA(M_) )
         CALL PESCAL( L, F, A, 1, I, DESCA, 1 )
*
  200 CONTINUE
*
      IF( NOCONV )
