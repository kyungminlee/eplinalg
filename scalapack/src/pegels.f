      SUBROUTINE PEGELS( TRANS, M, N, NRHS, A, IA, JA, DESCA, B, IB, JB,
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCB( * )
      REAL(KIND=10)   A( * ), B( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PEGELS solves overdetermined or underdetermined real linear
...
*            factorization as returned by PEGEQRF;
*          if M <  N, sub( A ) is overwritten by details of its LQ
*            factorization as returned by PEGELQF.
*
...
*     .. External Functions ..
      ...
      EXTERNAL           ILCM, INDXG2P, LSAME, NUMROC, PELAMCH,         
     +           PELANGE
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PEGELQF,    
     +                PEGEQRF, PELABAD, PELASCL, PELASET,               
     +     PEORMLQ, PEORMQR, PETRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGELS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF( MIN( M, N, NRHS ).EQ.0 ) THEN
         CALL PELASET( 'Full', MAX( M, N ), NRHS, ZERO, ZERO, B,        
     +          IB, JB, DESCB )
         RETURN
      END IF
*
...
*
      SMLNUM = PELAMCH( ICTXT, 'S' )
      SMLNUM = SMLNUM / PELAMCH( ICTXT, 'P' )
      BIGNUM = ONE / SMLNUM
      CALL PELABAD( ICTXT, SMLNUM, BIGNUM )
*
*     Scale A, B if max entry outside range [SMLNUM,BIGNUM]
*
      ANRM = PELANGE( 'M', M, N, A, IA, JA, DESCA, RWORK )
      IASCL = 0
      IF( ANRM.GT.ZERO .AND. ANRM.LT.SMLNUM ) THEN
*
*        Scale matrix norm up to SMLNUM
*
         CALL PELASCL( 'G', ANRM, SMLNUM, M, N, A, IA, JA, DESCA,       
     +           INFO )
         IASCL = 1
...
*
         CALL PELASCL( 'G', ANRM, BIGNUM, M, N, A, IA, JA, DESCA,       
     +           INFO )
         IASCL = 2
...
*
         CALL PELASET( 'F', MAX( M, N ), NRHS, ZERO, ZERO, B, IB, JB,   
     +               DESCB )
         GO TO 10
      END IF
*
     ...
     $   BROW = N
*
      BNRM = PELANGE( 'M', BROW, NRHS, B, IB, JB, DESCB, RWORK )
*
      IBSCL = 0
...
*        Scale matrix norm up to SMLNUM
*
         CALL PELASCL( 'G', BNRM, SMLNUM, BROW, NRHS, B, IB, JB,        
     +          DESCB, INFO )
         IBSCL = 1
...
*        Scale matrix norm down to BIGNUM
*
         CALL PELASCL( 'G', BNRM, BIGNUM, BROW, NRHS, B, IB, JB,        
     +          DESCB, INFO )
         IBSCL = 2
      END IF
*
...
*
         CALL PEGEQRF( M, N, A, IA, JA, DESCA, WORK, WORK( IPW ),       
     +           LWORK-LTAU, INFO )
*
...
*
            CALL PEORMQR( 'Left', 'Transpose', M, NRHS, N, A, IA, JA,   
...
*
            CALL PETRSM( 'Left', 'Upper', 'No transpose', 'Non-unit', N,
...
*           sub( B ) := inv(R') * sub( B )
*
            CALL PETRSM( 'Left', 'Upper', 'Transpose', 'Non-unit', N,   
     +                 NRHS, ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
*           B(IB+N:IB+M-1,JB:JB+NRHS-1) = ZERO
*
            CALL PELASET( 'All', M-N, NRHS, ZERO, ZERO, B, IB+N, JB,    
     +                 DESCB )
*
*           B(IB:IB+M-1,JB:JB+NRHS-1) := Q(1:N,:) *
*                                        B(IB:IB+N-1,JB:JB+NRHS-1)
*
            CALL PEORMQR( 'Left', 'No transpose', M, NRHS, N, A, IA, JA,
     +                     DESCA, WORK, B, IB, JB, DESCB, WORK( IPW ),  
     +                   LWORK-LTAU, INFO )
*
*           workspace at least NRHS, optimally NRHS*NB
...
*
         CALL PEGELQF( M, N, A, IA, JA, DESCA, WORK, WORK( IPW ),       
     +           LWORK-LTAU, INFO )
*
...
*
            CALL PETRSM( 'Left', 'Lower', 'No transpose', 'Non-unit', M,
     +                    NRHS, ONE, A, IA, JA, DESCA, B, IB, JB, DESCB 
     +)
*
*           B(IB+M:IB+N-1,JB:JB+NRHS-1) = 0
*
            CALL PELASET( 'All', N-M, NRHS, ZERO, ZERO, B, IB+M, JB,    
     +                 DESCB )
*
*           B(IB:IB+N-1,JB:JB+NRHS-1) := Q(1:N,:)' *
*                                        B(IB:IB+M-1,JB:JB+NRHS-1)
*
            CALL PEORMLQ( 'Left', 'Transpose', N, NRHS, M, A, IA, JA,   
     +                  DESCA, WORK, B, IB, JB, DESCB, WORK( IPW ),     
     +                LWORK-LTAU, INFO )
*
*           workspace at least NRHS, optimally NRHS*NB
...
*
            CALL PEORMLQ( 'Left', 'No transpose', N, NRHS, M, A, IA, JA,
     +                     DESCA, WORK, B, IB, JB, DESCB, WORK( IPW ),  
     +                   LWORK-LTAU, INFO )
*
*           workspace at least NRHS, optimally NRHS*NB
...
*                                        B(IB:IB+M-1,JB:JB+NRHS-1)
*
            CALL PETRSM( 'Left', 'Lower', 'Transpose', 'Non-unit', M,   
     +                 NRHS, ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
            SCLLEN = M
*
...
*
      IF( IASCL.EQ.1 ) THEN
         CALL PELASCL( 'G', ANRM, SMLNUM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      ELSE IF( IASCL.EQ.2 ) THEN
         CALL PELASCL( 'G', ANRM, BIGNUM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         CALL PELASCL( 'G', SMLNUM, BNRM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      ELSE IF( IBSCL.EQ.2 ) THEN
         CALL PELASCL( 'G', BIGNUM, BNRM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      END IF
*
   10 CONTINUE
