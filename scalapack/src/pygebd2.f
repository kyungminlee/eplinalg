      SUBROUTINE PYGEBD2( M, N, A, IA, JA, DESCA, D, E, TAUQ, TAUP,     
     +                WORK, LWORK, INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * )
      REAL(KIND=10)               D( * ), E( * )
      COMPLEX(KIND=10)            A( * ), TAUP( * ), TAUQ( * ), WORK( * 
     +)
*     ..
*
*  Purpose
*  =======
*
*  PYGEBD2 reduces a complex general M-by-N distributed matrix
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, YGEBR2D,          
     +          YGEBS2D, CHK1MAT, YLARFG, DESCSET, INFOG2L,             
     +       PYELSET, PYLACGV, PYLARF, PYLARFC,                    
     +PYLARFG, PEELSET, PXERBLA, EGEBR2D,                    EGEBS2D
*     ..
*     .. External Functions ..
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEBD2', -INFO )
      ...
      IF( M.EQ.1 .AND. N.EQ.1 ) THEN
         IF( MYCOL.EQ.IACOL ) THEN
            IF( MYROW.EQ.IAROW ) THEN
               ...
               CALL YLARFG( 1, A( I ), A( I ), 1, TAUQ( JJ ) )
               D( JJ ) = REAL( A( I ) , KIND=10)
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, D( JJ ),   
     +                     1 )
               CALL YGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, TAUQ( JJ ),
     +                        1 )
            ELSE
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, D( JJ ),   
     +                     1, IAROW, IACOL )
               CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAUQ( JJ ),
     +                        1, IAROW, IACOL )
            END IF
         END IF
         IF( MYROW.EQ.IAROW )
...
*           A(ia+i:ia+m-1,j)
*
            CALL PYLARFG( M-K+1, ALPHA, I, J, A, MIN( I+1, M+IA-1 ),    
     +                 J, DESCA, 1, TAUQ )
...
*
            CALL PYLARFC( 'Left', M-K+1, N-K, A, I, J, DESCA, 1, TAUQ,  
     +                   A, I, J+1, DESCA, WORK )
...
*
               CALL PYLARF( 'Right', M-K, N-K, A, I, J+1, DESCA,        
     ...
     +KIND=10) , KIND=10) )
               CALL PYLACGV( N-K, A, I, J+1, DESCA, DESCA( M_ ) )
            ELSE
               CALL PYELSET( TAUP, I, 1, DESCE, ZERO )
            END IF
   10    CONTINUE
*
      ELSE
...
*
            ...
            CALL PYLARFG( N-K+1, ALPHA, I, J, A, I,                     
...
*
            CALL PYLARF( 'Right', M-K, N-K+1, A, I, J, DESCA,           
...
*              A(i+2:ia+m-1,j)
*
               CALL PYLARFG( M-K, ALPHA, I+1, J, A,                     
...
*
               CALL PYLARFC( 'Left', M-K, N-K, A, I+1, J, DESCA, 1,     
     +                   TAUQ, A, I+1, J+1, DESCA, WORK )
               CALL PYELSET( A, I+1, J, DESCA, CMPLX( REAL( ALPHA , 
     +KIND=10) , KIND=10) )
            ELSE
               CALL PYELSET( TAUQ, 1, J, DESCE, ZERO )
            END IF
   20    CONTINUE
      END IF
*
