...
*  =======
*
*  PJLAENV_EP is called from the ScaLAPACK symmetric and Hermitian
...
*          'TTRD'
*
* (PJLAENV_EP) (global or local output) INTEGER
*          >= 0: the value of the parameter specified by ISPEC
*          < 0:  if PJLAENV_EP = -k, the k-th argument had an illegal
*          value.
*
*          Most parameters set via a call to PJLAENV_EP must be identical
*          on all processors and hence PJLAENV_EP will return the same
*          value to all procesors (i.e. global output).  However some,
*          in particular, the panel blocking factor can be different
*          on each processor and hence PJLAENV_EP can return different
*          values on different processors (i.e. local output).
*
*  Further Details
*  ===============
*
*  The following conventions have been used when calling PJLAENV_EP from
...
*      passed a value of -1.
*  3)  The parameter value returned by PJLAENV_EP is checked for validity
*      in the calling subroutine.  For example, PJLAENV_EP is used to
*      retrieve the optimal blocksize for ETRTRI as follows:
*
*      NB = PJLAENV_EP( 1, 'ETRTRI', UPLO // DIAG, N, -1, -1, -1 )
*      IF( NB.LE.1 ) NB = MAX( 1, N )
*
*  PJLAENV_EP is patterned after ILAENV and keeps the same interface in
*  anticipation of future needs, even though PJLAENV_EP is only sparsely
...
*  call PJLAENV_EP in this release.  pXYYtevx.f and pXYYtgvx.f redistribute
...
*     (e.g. 'PESYTTRD') to PJLAENV_EP; the upstream PJLAENV_EP's strict
...
*           V and H interleaved (default is not interleaved)
            ...
            END IF
*
*           Two ZGEMMs (default is one YGEMM)
            IF( N1.EQ.2 ) THEN
               PJLAENV_EP = 0
            END IF
