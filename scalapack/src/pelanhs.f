      REAL(KIND=10)   FUNCTION PELANHS( NORM, N, A, IA, JA, DESCA,      
     +                                WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELANHS returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PELANHS as described
*          above.
*
*  N       (global input) INTEGER
*          The number of rows and columns to be operated on i.e the
*          number of rows and columns of the distributed submatrix
*          sub( A ). When N = 0, PELANHS is set to zero. N >= 0.
*
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 440 LL = JJ, JJ+JB-1
                  CALL ELASSQ( MIN( II+LL-JJ+1, IIA+NP-1 )-IIA+1,       
  ...
  470             CONTINUE
               ELSE
                  DO 480 LL = JJ, JJ + JB -1
                     CALL ELASSQ( MIN( II-1, IIA+NP-1 )-IIA+1,          
     +                   A( IIA+IOFFA ), 1, SCALE, SUM )
                     IOFFA = IOFFA + LDA
  480             CONTINUE
                  IF( MYROW.EQ.INXTROW .AND. II.LE.IIA+NP-1 )           
     +     CALL ELASSQ( 1, A( II+(JJ+JB-2)*LDA ), 1,                    
     +         SCALE, SUM )
               END IF
               JJ = JJ + JB
            END IF
