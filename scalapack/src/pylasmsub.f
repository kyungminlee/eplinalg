      SUBROUTINE PYLASMSUB( A, DESCA, I, L, K, SMLNUM, BUF, LWORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLASMSUB looks for a small subdiagonal element from the bottom
...
*           IF( TST1.EQ.ZERO )
*    $         TST1 = YLANHS( '1', I-L+1, H( L, L ), LDH, WORK )
...
*     .. Local Scalars ..
     ...
     $                   RIGHT, UP
      REAL(KIND=10)               TST1, ULP
      COMPLEX(KIND=10)            CDUM, H10, H11, H22
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC
...
*
     ...
     +               RIGHT )
      END IF
      IF( IBUF2.GT.0 ) THEN
         CALL YGESD2D( CONTXT, IBUF2, 1, BUF( ISTR2+1 ), IBUF2, MYROW,  
     +                RIGHT )
      END IF
*
...
*
     ...
     +             LEFT )
      END IF
      IF( IRCV2.GT.0 ) THEN
         CALL YGERV2D( CONTXT, IRCV2, 1, BUF( ISTR2+1 ), IRCV2, MYROW,  
     +                LEFT )
      END IF
*
*     START MAIN LOOP
