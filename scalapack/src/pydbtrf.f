      SUBROUTINE PYDBTRF( N, BWL, BWU, A, JA, DESCA, AF, LAF, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYDBTRF computes a LU factorization
...
*  subsequent calls to PYDBTRS to solve linear systems.
*
...
*          PYDBTRF and this is stored in AF. If a linear system
*          is to be solved using PYDBTRS after the factorization
...
*     .. External Subroutines ..
     ...
     +               YAXPY, YGEMM, YGERV2D, YGESD2D, YLAMOV,            
     +        YLATCPY, YPBTRF, YPOTRF, YSYRK, YTBTRS, YTRMM,            
     +        YTRRV2D, YTRSD2D, YTRSM, YTRTRS, DESC_CONVERT,            
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,      
     + 'PYDBTRF, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*MAX(BWL,BWU) )) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT,       'PYDBTRF, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT,       'PYDBTRF: auxiliary storage error ',
     +       -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
           INFO = -10
           CALL PXERBLA( ICTXT,       'PYDBTRF: worksize error ',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYDBTRF', -INFO )
         RETURN
      END IF
*
...
*
          CALL YTRSD2D( ICTXT, 'U', 'N', NEXT_TRI_SIZE_M,               
     +    NEXT_TRI_SIZE_N,                   A( 
     +OFST+(MY_NUM_COLS-BWL)*LLDA+(BWL+BWU+1) ),                   
     +LLDA-1, 0, MYCOL+1 )
*
        ENDIF
*
...
*
        CALL YDBTRF( ODD_SIZE, ODD_SIZE, BWL, BWU, A( OFST + 1),        
     +             LLDA, INFO )
*
        IF( INFO.NE.0 ) THEN
           INFO = MYCOL+1
...
*
          CALL YLATCPY( 'U', BWL, BWL,                   A(( 
     +OFST+(BWL+BWU+1)+(ODD_SIZE-BWL)*LLDA )),                   LLDA-1,
     + AF( ODD_SIZE*BWU+2*MBW2+1+MAX_BW-BWL ),                   MAX_BW 
     +)
          CALL YLAMOV( 'L', BWU, BWU, A( ( OFST+1+ODD_SIZE*LLDA ) ),    
     +              LLDA-1,                  AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1+MAX_BW-BWU ),                  
     +MAX_BW )
*
*         Perform the triangular system solve {L_i}{{BU'}_i} = {B_i}
*
          CALL YTBTRS( 'L', 'N', 'U', BWU, BWL, BWU,                  
     +A( OFST+BWU+1+(ODD_SIZE-BWU )*LLDA ), LLDA,                  AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1+MAX_BW-BWU ),                  
     +MAX_BW, INFO )
*
*         Perform the triangular solve {U_i}^C{BL'}_i^C = {BL_i}^C
*
          CALL YTBTRS( 'U', 'C', 'N', BWL, BWU, BWL,                  
     +A( OFST+1+(ODD_SIZE-BWL)*LLDA ), LLDA,                  AF( 
     +ODD_SIZE*BWU+2*MBW2+1+MAX_BW-BWL ), MAX_BW,                  INFO 
     +)
*
*         conjugate transpose resulting block to its location
*           in main storage.
*
          CALL YLATCPY( 'L', BWL, BWL,                   AF( 
     +ODD_SIZE*BWU+2*MBW2+1+MAX_BW-BWL ), MAX_BW,                   A(( 
     +OFST+(BWL+BWU+1)+(ODD_SIZE-BWL)*LLDA )),                   LLDA-1 
     +)
*
*         Move the resulting block back to its location in main storage.
*
          CALL YLAMOV( 'L', BWU, BWU,                  AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1+MAX_BW-BWU ),                  
...
*
*
          CALL YGEMM( 'C', 'N', MAX_BW, MAX_BW, MAX_BW, -CONE ,         
     + AF( ODD_SIZE*BWU+2*MBW2+1), MAX_BW,          AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1), MAX_BW, CONE,          A( 
     +OFST+ODD_SIZE*LLDA+1+BWU ), LLDA-1 )
*
        ENDIF
...
*
*
          CALL YTRRV2D( ICTXT, 'U', 'N', PREV_TRI_SIZE_M,               
...
*
          CALL YTBTRS( 'L', 'N', 'U', ODD_SIZE, BWL, BWL,               
...
*
          CALL YLATCPY( 'L', UP_PREV_TRI_SIZE_N, UP_PREV_TRI_SIZE_M,    
     +               A( OFST+1 ), LLDA-1, AF( 1 ), ODD_SIZE )
*
          CALL YTBTRS( 'U', 'C', 'N', ODD_SIZE, BWU, BWU,               
     +      A( OFST + 1 ), LLDA,                     AF( 1 ), ODD_SIZE, 
     +INFO )
*
*
...
*
   ...
   30   CONTINUE
*
          CALL YGEMM( 'C', 'N', BWU, BWL, ODD_SIZE,                     
     +-CONE, AF( 1 ), ODD_SIZE,                     AF( WORK_U+1 ), 
     +ODD_SIZE, CZERO,                     AF( 1+MAX(0,
     +BWL-BWU)+ODD_SIZE*BWU+                       (2*MAX_BW+MAX(0,
     +BWU-BWL))*MAX_BW),                     MAX_BW )
*
*
*         Initiate send of E_i to previous processor to overlap
*           with next computation.
*
          CALL YGESD2D( ICTXT, MAX_BW, MAX_BW,                   AF( 
     +ODD_SIZE*BWU+2*MBW2+1 ), MAX_BW, 0,                   MYCOL-1 )
*
*
          IF ( MYCOL .LT. NP-1 ) THEN
*
...
*             as per requirements of BLAS routine YTRMM.
*             Since we have GU_i stored,
*             conjugate transpose HU_i to HU_i^C.
*
            CALL YLATCPY( 'N', BWL, BWL,                AF( 
     +WORK_U+ODD_SIZE-BWL+1 ), ODD_SIZE,         AF( 
     +(ODD_SIZE)*BWU+1+(MAX_BW-BWL) ),                MAX_BW  )
*
            CALL YTRMM( 'R', 'U', 'C', 'N', BWL, BWL, -CONE,            
...
*
            CALL YLATCPY( 'N', BWU, BWU,                AF( 
     +ODD_SIZE-BWU+1 ), ODD_SIZE,                AF( 
     +WORK_U+(ODD_SIZE)*BWL+1+MAX_BW-BWU ),                MAX_BW  )
*
            CALL YTRMM( 'R', 'L', 'N', 'N', BWU, BWU, -CONE,            
...
*
          CALL YGESD2D( ICTXT, MAX_BW, MAX_BW,                        
     +AF( ODD_SIZE*BWU+1 ),                        MAX_BW, 0, MYCOL-1 )
*
          CALL YGESD2D( ICTXT, MAX_BW, MAX_BW,                        
...
*         operations.
*
        CALL YLAMOV( 'N', MAX_BW, MAX_BW,                     A( 
     +OFST+ODD_SIZE*LLDA+BWU+1 ),                     LLDA-1, AF( 
     +ODD_SIZE*BWU+MBW2+1 ),                     MAX_BW )
*
*       Receive cont. to diagonal block that is stored on this proc.
*
        IF( MYCOL.LT. NPCOL-1 ) THEN
*
           CALL YGERV2D( ICTXT, MAX_BW, MAX_BW,                AF( 
     +ODD_SIZE*BWU+2*MBW2+1 ),                MAX_BW, 0,                
     +MYCOL+1 )
*
*          Add contribution to diagonal block
*
           CALL YAXPY( MBW2, CONE,                AF( 
     +ODD_SIZE*BWU+2*MBW2+1 ),                1, AF( 
     +ODD_SIZE*BWU+MBW2+1 ), 1 )
*
        ENDIF
*
*
...
*
          IF( MYCOL-LEVEL_DIST .GE. 0 ) THEN
            CALL YGERV2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ),             
     +         MAX_BW, 0, MYCOL-LEVEL_DIST )
*
            CALL YAXPY( MBW2, CONE, WORK( 1 ), 1,                AF( 
     +ODD_SIZE*BWU+MBW2+1 ), 1 )
*
          ENDIF
*
...
*
       CALL YDBTRF( MAX_BW, MAX_BW, MIN( MAX_BW-1, BWL ),               
...
*             calcs.
*
          CALL YLAMOV( 'N', MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+1 ),       
     +           MAX_BW, AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ),            
     +      MAX_BW )
*
          CALL YLAMOV( 'N', MAX_BW, MAX_BW, AF( WORK_U+ODD_SIZE*BWL+1 ),
...
*
          CALL YGERV2D( ICTXT, MAX_BW, MAX_BW,         AF( 
     +ODD_SIZE*BWU+1 ),                        MAX_BW, 0, COMM_PROC )
*
          CALL YGERV2D( ICTXT, MAX_BW, MAX_BW,         AF( 
     +WORK_U+ODD_SIZE*BWL+1 ),                        MAX_BW, 0, 
     +COMM_PROC )
*
          IF( INFO .EQ. 0 ) THEN
*
...
*
*
          CALL YTBTRS(      'L', 'N', 'U', BWU, MIN( BWL, BWU-1 ), BWU, 
     +     AF( ODD_SIZE*BWU+      MBW2+1+(MAX_BW+1)*(MAX_BW-BWU)), 
     +MAX_BW+1,      AF( WORK_U+ODD_SIZE*BWL+1+MAX_BW-BWU), MAX_BW, 
     +INFO )
*
*         Modify lower off_diagonal block with diagonal block
*
*
          CALL YTBTRS(      'U', 'C', 'N', BWL, MIN( BWU, BWL-1 ), BWL, 
     +     AF( ODD_SIZE*BWU+      MBW2+1-MIN( BWU, BWL-1 
     +)+(MAX_BW+1)*(MAX_BW-BWL)), MAX_BW+1,      AF( 
     +ODD_SIZE*BWU+1+MAX_BW-BWL), MAX_BW, INFO )
*
          ENDIF
*         End of "if ( info.eq.0 ) then"
*
...
*
          CALL YGEMM( 'C', 'N', MAX_BW, MAX_BW, MAX_BW, -CONE,          
     +       AF( (ODD_SIZE)*BWU+1 ), MAX_BW,                 AF( 
     +WORK_U+(ODD_SIZE)*BWL+1 ), MAX_BW, CZERO,                 WORK( 1 
     +), MAX_BW )
*
*         Send contribution to diagonal block's owning processor.
*
          CALL YGESD2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ), MAX_BW,       
     +               0, MYCOL+LEVEL_DIST )
*
        ENDIF
...
*
           CALL YGERV2D( ICTXT, MAX_BW, MAX_BW,             AF( 
     +WORK_U+ODD_SIZE*BWL+2*MBW2+1 ),             MAX_BW, 0, 
     +MYCOL-LEVEL_DIST/2 )
*
*           Receive offdiagonal block(s) from proc level_dist/2 to the
*           left
...
*
*
*         Since YTBTRS has no "left-right" option, we must transpose
*
          CALL YLATCPY( 'N', MAX_BW, MAX_BW, AF( WORK_U+ODD_SIZE*BWL+   
     +    2*MBW2+1), MAX_BW, WORK( 1 ), MAX_BW )
*
          CALL YTBTRS(           'L', 'N', 'U', MAX_BW, MIN( BWL, 
     +MAX_BW-1 ), BWL,           AF( ODD_SIZE*BWU+MBW2+1), MAX_BW+1,    
     +       WORK( 1+MAX_BW*(MAX_BW-BWL) ), MAX_BW, INFO )
*
*         Transpose back
*
          CALL YLATCPY(          'N', MAX_BW, MAX_BW, WORK( 1 ), MAX_BW,
...
*         Since YTBTRS has no "left-right" option, we must transpose
*
          CALL YLATCPY( 'N', MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+       
     +2*MBW2+1), MAX_BW, WORK( 1 ), MAX_BW )
*
          CALL YTBTRS(           'U', 'C', 'N', MAX_BW, MIN( BWU, 
     +MAX_BW-1 ), BWU,           AF( ODD_SIZE*BWU+MBW2+1-MIN( BWU, 
     +MAX_BW-1 )), MAX_BW+1,           WORK( 1+MAX_BW*(MAX_BW-BWU) ), 
     +MAX_BW, INFO )
*
*         Transpose back
*
...
*
          CALL YGEMM( 'N', 'C', MAX_BW, MAX_BW, MAX_BW, -CONE,          
     +       AF( (ODD_SIZE)*BWU+2*MBW2+1 ), MAX_BW,                 AF( 
     +WORK_U+(ODD_SIZE)*BWL+2*MBW2+1 ), MAX_BW,                 CZERO, 
     +WORK( 1 ), MAX_BW )
*
*         Send contribution to diagonal block's owning processor.
*
...
*
              CALL YGEMM( 'N', 'N', MAX_BW, MAX_BW, MAX_BW, -CONE,      
     +               AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW,        
