      SUBROUTINE PYPOSVX( FACT, UPLO, N, NRHS, A, IA, JA, DESCA, AF,    
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   BERR( * ), FERR( * ), SC( * ),                    
     +SR( * ), RWORK( * )
      COMPLEX(KIND=10)         A( * ), AF( * ),                    B( * 
     +), WORK( * ), X( * )
*     ..
*
*  Purpose
*  =======
*
*  PYPOSVX uses the Cholesky factorization A = U**H*U or A = L*L**H to
...
*
*  A       (local input/local output) COMPLEX(KIND=10) pointer into
...
*
*  AF      (local input or local output) COMPLEX(KIND=10) pointer
...
*          output variable.
*
*  SR      (local input/local output) COMPLEX(KIND=10) array,
*                                                    dimension (LLD_A)
...
*          positive.
*
*  SC      (local input/local output) COMPLEX(KIND=10) array,
*                                              dimension (LOC(N_A))
...
*          positive.
*
*  B       (local input/local output) COMPLEX(KIND=10) pointer into
*          the local memory to an array of local dimension
...
*
*  X       (local input/local output) COMPLEX(KIND=10) pointer into
*          the local memory to an array of local dimension
...
*
*  RCOND   (global output) REAL(KIND=10)
...
*
*  FERR    (local output) REAL(KIND=10) array, dimension (LOC(N_B))
...
*
*  BERR    (local output) REAL(KIND=10) array, dimension (LOC(N_B))
*          The componentwise relative backward error of each solution
*          vector X(j) (i.e., the smallest relative change in
*          any entry of A or B that makes X(j) an exact solution).
*
*  WORK    (local workspace/local output) COMPLEX(KIND=10) array,
*                                                dimension (LWORK)
...
*          LWORK = MAX( PYPOCON( LWORK ), PYPORFS( LWORK ) )
*                  + LOCr( N_A ).
*          LWORK = 3*DESCA( LLD_ )
*
...
*
*  RWORK   (local workspace/local output) REAL(KIND=10) array,
*                                                dimension (LRWORK)
...
*     .. Local Scalars ..
     ...
     $                   NPMOD, NQMOD,
     $                   NQ
      REAL(KIND=10)   AMAX, ANORM, BIGNUM, SCOND, SMAX, SMIN, SMLNUM
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
     ...
     +       EGAMN2D, EGAMX2D, INFOG2L,                    PXERBLA, 
     +PYPOCON, PYPOEQU,                    PYPORFS, PYPOTRF, PYPOTRS,   
     +                 PYLACPY, PYLAQSY
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, INDXG2P, NUMROC
      REAL(KIND=10)   PELAMCH, PYLANHE
      EXTERNAL           ICEIL, PELAMCH, INDXG2P, LSAME, NUMROC, PYLANHE
*     ..
*     .. Intrinsic Functions ..
...
*           PYPOCON / PYPORFS calls (see comment at line 311 of the
*           upstream prologue documenting the intended formula).
*           Upstream sets LWMIN = 3*DESCA(LLD_) and LRWMIN = MAX(2*NQ,
*           NP), both insufficient — PYPOCON aborts with INFO=-10.
*           PYPOCON LWMIN  = 2*NPMOD + MAX(2, MAX(NB*ICEIL(NPROW-1,
*                                                          NPCOL),
*                                          NQMOD + NB*ICEIL(NPCOL-1,
*                                                           NPROW)))
*           PYPOCON LRWMIN = 2*NQMOD
*           PYPORFS LWMIN  = 2*NPMOD,   LRWMIN = NPMOD
*           PYLANHE('1') needs RWORK = 2*NQMOD + NPMOD + LDW (LDW=0 on
*           square grids); upstream's LRWMIN = MAX(2*NQ, NP) is too
*           small here too — pylanhe scribbles past the buffer and
*           triggers free()-time heap corruption a few iterations later.
*           NPMOD/NQMOD are the unadjusted NUMROCs (PYPOCON's own
*           definition).
...
*           (it's always >= the LDW term).
     ...
     $                    NQ )
            ...
            ELSE
               RCEQU = LSAME( EQUED, 'Y' )
               SMLNUM = PELAMCH( ICTXT, 'Safe minimum' )
               BIGNUM = ONE / SMLNUM
            END IF
...
*
                  ...
                  DO 10 J = IIA, IIA + NP - 1
                     ...
                     SMAX = MAX( SMAX, SR( J ) )
   10             CONTINUE
                  CALL EGAMN2D( ICTXT, 'Columnwise', ' ', 1, 1, SMIN,   
     +                        1, IDUMM, IDUMM, -1, -1, MYCOL )
                  CALL EGAMX2D( ICTXT, 'Columnwise', ' ', 1, 1, SMAX,   
     +                        1, IDUMM, IDUMM, -1, -1, MYCOL )
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPOSVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PYPOEQU( N, A, IA, JA, DESCA, SR, SC, SCOND, AMAX,        
     +          INFEQU )
*
         IF( INFEQU.EQ.0 ) THEN
*
...
*
            CALL PYLAQSY( UPLO, N, A, IA, JA, DESCA, SR, SC, SCOND,     
     +                AMAX, EQUED )
            RCEQU = LSAME( EQUED, 'Y' )
         END IF
      END IF
...
*
         CALL PYLACPY( 'Full', N, N, A, IA, JA, DESCA, AF, IAF, JAF,    
     +              DESCAF )
         CALL PYPOTRF( UPLO, N, AF, IAF, JAF, DESCAF, INFO )
*
...
*
      ANORM = PYLANHE( '1', UPLO, N, A, IA, JA, DESCA, RWORK )
*
*     Compute the reciprocal of the condition number of A.
*
      CALL PYPOCON( UPLO, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,  
     +             LWORK, RWORK, LRWORK, INFO )
*
*     Return if the matrix is singular to working precision.
*
      IF( RCOND.LT.PELAMCH( ICTXT, 'Epsilon' ) ) THEN
         INFO = IA + N
         RETURN
      END IF
*
...
*
      CALL PYLACPY( 'Full', N, NRHS, B, IB, JB, DESCB, X, IX, JX,       
     +        DESCX )
      CALL PYPOTRS( UPLO, N, NRHS, AF, IAF, JAF, DESCAF, X, IX, JX,     
     +          DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PYPORFS( UPLO, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,      
