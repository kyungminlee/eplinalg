      SUBROUTINE PYGELS( TRANS, M, N, NRHS, A, IA, JA, DESCA, B, IB, JB,
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCB( * )
      COMPLEX(KIND=10)            A( * ), B( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYGELS solves overdetermined or underdetermined complex linear
...
*            factorization as returned by PYGEQRF;
*          if M <  N, sub( A ) is overwritten by details of its LQ
*            factorization as returned by PYGELQF.
*
...
*     .. External Functions ..
      ...
      REAL(KIND=10)               PYLANGE, PELAMCH
      EXTERNAL           ILCM, INDXG2P, LSAME, NUMROC, PYLANGE,         
     +           PELAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PYGELQF,    
     +                PYGEQRF, PELABAD, PYLASCL, PYLASET,               
     +     PYTRSM, PYUNMLQ, PYUNMQR, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                DESCA( MB_ ) * DESCA( MB_ )
            ...
            WORK( 1 ) = CMPLX( REAL( LWMIN , KIND=10) , KIND=10)
            LQUERY = ( LWORK.EQ.-1 )
*
            TPSD = .TRUE.
            IF( LSAME( TRANS, 'N' ) )
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGELS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF( MIN( M, N, NRHS ).EQ.0 ) THEN
         CALL PYLASET( 'Full', MAX( M, N ), NRHS, CZERO, CZERO, B,      
     +            IB, JB, DESCB )
         RETURN
      END IF
*
...
*
      ANRM = PYLANGE( 'M', M, N, A, IA, JA, DESCA, RWORK )
      IASCL = 0
      IF( ANRM.GT.ZERO .AND. ANRM.LT.SMLNUM ) THEN
*
*        Scale matrix norm up to SMLNUM
*
         CALL PYLASCL( 'G', ANRM, SMLNUM, M, N, A, IA, JA, DESCA,       
     +           INFO )
         IASCL = 1
...
*
         CALL PYLASCL( 'G', ANRM, BIGNUM, M, N, A, IA, JA, DESCA,       
     +           INFO )
         IASCL = 2
...
*
         CALL PYLASET( 'F', MAX( M, N ), NRHS, CZERO, CZERO, B, IB,     
     +             JB, DESCB )
         GO TO 10
      END IF
*
     ...
     $   BROW = N
*
      BNRM = PYLANGE( 'M', BROW, NRHS, B, IB, JB, DESCB, RWORK )
*
      IBSCL = 0
...
*        Scale matrix norm up to SMLNUM
*
         CALL PYLASCL( 'G', BNRM, SMLNUM, BROW, NRHS, B, IB, JB,        
     +          DESCB, INFO )
         IBSCL = 1
...
*        Scale matrix norm down to BIGNUM
*
         CALL PYLASCL( 'G', BNRM, BIGNUM, BROW, NRHS, B, IB, JB,        
     +          DESCB, INFO )
         IBSCL = 2
      END IF
*
...
*
         CALL PYGEQRF( M, N, A, IA, JA, DESCA, WORK, WORK( IPW ),       
     +           LWORK-LTAU, INFO )
*
...
*
            CALL PYUNMQR( 'Left', 'Conjugate transpose', M, NRHS, N, A, 
...
*
            CALL PYTRSM( 'Left', 'Upper', 'No transpose', 'Non-unit', N,
...
*           sub( B ) := inv(R') * sub( B )
*
            CALL PYTRSM( 'Left', 'Upper', 'Conjugate transpose',        
     +            'Non-unit', N, NRHS, CONE, A, IA, JA, DESCA,          
     +          B, IB, JB, DESCB )
*
*           B(IB+N:IB+M-1,JB:JB+NRHS-1) = ZERO
*
            CALL PYLASET( 'All', M-N, NRHS, CZERO, CZERO, B, IB+N, JB,  
     +                   DESCB )
*
*           B(IB:IB+M-1,JB:JB+NRHS-1) := Q(1:N,:) *
*                                        B(IB:IB+N-1,JB:JB+NRHS-1)
*
            CALL PYUNMQR( 'Left', 'No transpose', M, NRHS, N, A, IA, JA,
...
*
         CALL PYGELQF( M, N, A, IA, JA, DESCA, WORK, WORK( IPW ),       
     +           LWORK-LTAU, INFO )
*
...
*
            CALL PYTRSM( 'Left', 'Lower', 'No transpose', 'Non-unit', M,
     +                    NRHS, CONE, A, IA, JA, DESCA, B, IB, JB,      
     +              DESCB )
*
*           B(IB+M:IB+N-1,JB:JB+NRHS-1) = 0
*
            CALL PYLASET( 'All', N-M, NRHS, CZERO, CZERO, B, IB+M, JB,  
     +                   DESCB )
*
*           B(IB:IB+N-1,JB:JB+NRHS-1) := Q(1:N,:)' *
*                                        B(IB:IB+M-1,JB:JB+NRHS-1)
*
            CALL PYUNMLQ( 'Left', 'Conjugate transpose', N, NRHS, M, A, 
     +                    IA, JA, DESCA, WORK, B, IB, JB, DESCB,        
     +             WORK( IPW ), LWORK-LTAU, INFO )
*
*           workspace at least NRHS, optimally NRHS*NB
...
*
            CALL PYUNMLQ( 'Left', 'No transpose', N, NRHS, M, A, IA, JA,
     +                     DESCA, WORK, B, IB, JB, DESCB, WORK( IPW ),  
     +                   LWORK-LTAU, INFO )
*
*           workspace at least NRHS, optimally NRHS*NB
...
*                                        B(IB:IB+M-1,JB:JB+NRHS-1)
*
            CALL PYTRSM( 'Left', 'Lower', 'Conjugate transpose',        
...
*
      IF( IASCL.EQ.1 ) THEN
         CALL PYLASCL( 'G', ANRM, SMLNUM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      ELSE IF( IASCL.EQ.2 ) THEN
         CALL PYLASCL( 'G', ANRM, BIGNUM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         CALL PYLASCL( 'G', SMLNUM, BNRM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      ELSE IF( IBSCL.EQ.2 ) THEN
         CALL PYLASCL( 'G', BIGNUM, BNRM, SCLLEN, NRHS, B, IB, JB,      
     +            DESCB, INFO )
      END IF
*
   10 CONTINUE
