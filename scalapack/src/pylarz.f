      SUBROUTINE PYLARZ( SIDE, M, N, L, V, IV, JV, DESCV, INCV, TAU, C, 
     +                   IC, JC, DESCC, WORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCC( * ), DESCV( * )
      COMPLEX(KIND=10)         C( * ), TAU( * ), V( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYLARZ applies a complex elementary reflector Q to a complex M-by-N
...
*
*  Q is a product of k elementary reflectors as returned by PYTZRZF.
*
*  Notes
*  =====
*
...
*
*  V       (local input) COMPLEX(KIND=10) pointer into the local memory
...
*
*  TAU     (local input) COMPLEX(KIND=10), array, dimension  LOCc(JV) if
...
*
*  C       (local input/local output) COMPLEX(KIND=10) pointer into the
...
*
*  WORK    (local workspace) COMPLEX(KIND=10) array, dimension (LWORK)
*          If INCV = 1,
*            if SIDE = 'L',
*              if IVCOL = ICCOL,
*                LWORK >= NqC0
...
*     .. Local Scalars ..
     ...
     $                   NQC2, NQV, RDEST
      COMPLEX(KIND=10)         TAULOC( 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, PB_TOPGET, PBYTRNV,   
     +                 YAXPY, YCOPY, YGEBR2D, YGEBS2D,                  
     +  YGEMV, YGERC, YGERU, YGERV2D, YGESD2D,                    
     +YGSUM2D, YLACGV, YLASET
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*        from procrow ICROW2 with offset IROFFC2), since PBYTRNV transposes
*        V into a column at ICROW2 and YGEMV/YGERU below act on sub(C2).
...
*        PYUNMR3 may violate as it iterates IV across blocks.
...
*        ICCOL2 with offset ICOFFC2), since the col-V SIDE='R' PBYTRNV
*        calls transpose V into a row at ICCOL2 and the downstream
*        YGEMV/YGERU act on sub(C2).  In the row-V SIDE='R' branch this
...
*
               IPW = MPV+1
               CALL PBYTRNV( ICTXT, 'Rowwise', 'Transpose', L,          
     +              DESCV( NB_ ), IROFFC2, V( IOFFV ), LDV,             
     +           ZERO,                        WORK, 1, IVROW, IVCOL, 
     +ICROW2, ICCOL2,                        WORK( IPW ) )
*
...
*
                     CALL YGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1,      
     +                        TAU( IIV ), 1 )
                     TAULOC( 1 ) = TAU( IIV )
*
                  ELSE
*
                     CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1,      
...
*
                     IF( MPV.GT.0 ) THEN
                        CALL YGEMV( 'Conjugate transpose', MPV, NQC2,   
     +                            ONE, C( IOFFC2 ), LDC, WORK, 1,       
     +                        ZERO, WORK( IPW ), 1 )
                     ELSE
                        CALL YLASET( 'All', NQC2, 1, ZERO, ZERO,        
     +                        WORK( IPW ), MAX( 1, NQC2 ) )
                     END IF
*                    UPSTREAM BUG FIX (vs ScaLAPACK 2.2.3): SIDE='L'
*                    needs an in-place conjugation of the partial
*                    accumulator before adding C1 and a YGERU (not
*                    YGERC) to apply the rank-1 update. Cf. LAPACK's
*                    YLARZ which uses two YLACGV + YGERU. Without this,
*                    the imaginary part of v^H*C is sign-flipped,
*                    producing an O(1) residual on SIDE='L' for complex
*                    inputs (real path is unaffected since EGEMV/EGER
*                    have no conjugation).
                     CALL YLACGV( NQC2, WORK( IPW ), 1 )
                     IF( MYROW.EQ.ICROW1 )                   CALL 
     +YAXPY( NQC2, ONE, C( IOFFC1 ), LDC,                               
     +WORK( IPW ), 1 )
*
                     CALL YGSUM2D( ICTXT, 'Columnwise', ' ', NQC2, 1,   
...
*
                     IF( MYROW.EQ.ICROW1 )                   CALL 
     +YAXPY( NQC2, -TAULOC( 1 ), WORK( IPW ),                           
     +    1, C( IOFFC1 ), LDC )
                     CALL YGERU( MPV, NQC2, -TAULOC( 1 ), WORK, 1,      
...
*
                        IF( MYROW.EQ.ICROW1 )                      CALL 
     +YAXPY( NQC2, -TAULOC( 1 ), WORK,                                  
     +1, C( IOFFC1 ),                                  LDC )
                        CALL YGERU( MPV, NQC2, -TAULOC( 1 ), V( IOFFV ),
...
*
                     IPW = MPV+1
                     CALL YCOPY( MPV, V( IOFFV ), 1, WORK, 1 )
                     WORK( IPW ) = TAU( JJV )
                     CALL YGESD2D( ICTXT, IPW, 1, WORK, IPW, MYROW,     
     +                         ICCOL2 )
*
                  ELSE IF( MYCOL.EQ.ICCOL2 ) THEN
*
                     IPW = MPV+1
                     CALL YGERV2D( ICTXT, IPW, 1, WORK, IPW, MYROW,     
     +                         IVCOL )
                     TAULOC( 1 ) = WORK( IPW )
*
                     IF( TAULOC( 1 ).NE.ZERO ) THEN
...
*
               ELSE
*
                  CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAULOC, 
     +                          1, IVROW, MYCOL )
*
               END IF
*
...
*
                  IF( MPV.GT.0 ) THEN
                     CALL YGEMV( 'Conjugate transpose', MPV, NQC2, ONE, 
     ...
     +                  WORK( IPW ), MAX( 1, NQC2 ) )
                  END IF
                  CALL YLACGV( NQC2, WORK( IPW ), 1 )
                  IF( MYROW.EQ.ICROW1 )                CALL YAXPY( NQC2,
     + ONE, C( IOFFC1 ), LDC,                            WORK( IPW ), 1 
     +)
*
                  CALL YGSUM2D( ICTXT, 'Columnwise', ' ', NQC2, 1,      
     +                     WORK( IPW ), MAX( 1, NQC2 ), RDEST,          
...
*
                  ...
                  CALL YCOPY( MPV, V( IOFFV ), 1, WORK, 1 )
                  WORK( IPW ) = TAU( JJV )
                  CALL YGEBS2D( ICTXT, 'Rowwise', ROWBTOP, IPW, 1,      
     +                     WORK, IPW )
                  TAULOC( 1 ) = TAU( JJV )
*
               ELSE
*
                  IPW = MPV+1
                  CALL YGEBR2D( ICTXT, 'Rowwise', ROWBTOP, IPW, 1, WORK,
     +                           IPW, MYROW, IVCOL )
                  TAULOC( 1 ) = WORK( IPW )
*
               END IF
...
*
                        IF( NQV.GT.0 ) THEN
                           CALL YGEMV( 'No transpose', MPC2, NQV, ONE,  
     +                                C( IOFFC2 ), LDC, V( IOFFV ),     
     +                             LDV, ZERO, WORK, 1 )
                        ELSE
                           CALL YLASET( 'All', MPC2, 1, ZERO, ZERO,     
     +                              WORK, MAX( 1, MPC2 ) )
                        END IF
                        IF( MYCOL.EQ.ICCOL1 )                      CALL 
     +YAXPY( MPC2, ONE, C( IOFFC1 ), 1,                                 
     +   WORK, 1 )
*
                        CALL YGSUM2D( ICTXT, 'Rowwise', ' ', MPC2, 1,   
     +                              WORK, MAX( 1, MPC2 ), RDEST,        
     +                        ICCOL2 )
*
                        IF( MYCOL.EQ.ICCOL1 )                      CALL 
     +YAXPY( MPC2, -TAULOC( 1 ), WORK, 1,                               
     +   C( IOFFC1 ), 1 )
*
*                       sub( C ) := sub( C ) - w * v'
*
                        IF( MPC2.GT.0 .AND. NQV.GT.0 )                  
     +    CALL YGERC( MPC2, NQV, -TAULOC( 1 ), WORK, 1,                 
...
*
                     IPW = NQV+1
                     CALL YCOPY( NQV, V( IOFFV ), LDV, WORK, 1 )
                     WORK( IPW ) = TAU( IIV )
                     CALL YGESD2D( ICTXT, IPW, 1, WORK, IPW, ICROW2,    
     +                          MYCOL )
*
                  ELSE IF( MYROW.EQ.ICROW2 ) THEN
*
                     IPW = NQV+1
                     CALL YGERV2D( ICTXT, IPW, 1, WORK, IPW, IVROW,     
     +                         MYCOL )
                     TAULOC( 1 ) = WORK( IPW )
*
                     IF( TAULOC( 1 ).NE.ZERO ) THEN
...
*
     ...
     +                         RDEST, ICCOL2 )
                        IF( MYCOL.EQ.ICCOL1 )                      CALL 
     +YAXPY( MPC2, -TAULOC( 1 ), WORK( IPW ),                           
     +       1, C( IOFFC1 ), 1 )
*
*                       sub( C ) := sub( C ) - w * v'
*
                        CALL YGERC( MPC2, NQV, -TAULOC( 1 ),            
     +                   WORK( IPW ), 1, WORK, 1,                       
     +        C( IOFFC2 ), LDC )
                     END IF
*
...
*
               IPW = NQV+1
               CALL PBYTRNV( ICTXT, 'Columnwise', 'Transpose', L,       
...
*
                  IF( MYCOL.EQ.IVCOL ) THEN
*
                     CALL YGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1,         
     +                     TAU( JJV ), 1 )
                     TAULOC( 1 ) = TAU( JJV )
*
                  ELSE
*
                     CALL YGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, TAULOC, 
     +                             1, MYROW, IVCOL )
*
                  END IF
*
...
*
     ...
     +                       ICCOL2 )
                     IF( MYCOL.EQ.ICCOL1 )                   CALL 
     +YAXPY( MPC2, -TAULOC( 1 ), WORK( IPW ), 1,                        
     +       C( IOFFC1 ), 1 )
*
*                    sub( C ) := sub( C ) - w * v'
*
                     CALL YGERC( MPC2, NQV, -TAULOC( 1 ), WORK( IPW ),  
     +                          1, WORK, 1, C( IOFFC2 ), LDC )
                  END IF
*
               END IF
...
*
                  ...
                  CALL YCOPY( NQV, V( IOFFV ), LDV, WORK, 1 )
                  WORK( IPW ) = TAU( IIV )
                  CALL YGEBS2D( ICTXT, 'Columnwise', COLBTOP, IPW, 1,   
     +                        WORK, IPW )
                  TAULOC( 1 ) = TAU( IIV )
*
               ELSE
*
                  IPW = NQV+1
                  CALL YGEBR2D( ICTXT, 'Columnwise', COLBTOP, IPW, 1,   
...
*
     ...
     +                  WORK( IPW ), MAX( 1, MPC2 ) )
                  END IF
                  IF( MYCOL.EQ.ICCOL1 )                CALL YAXPY( MPC2,
     + ONE, C( IOFFC1 ), 1,                            WORK( IPW ), 1 )
*
                  CALL YGSUM2D( ICTXT, 'Rowwise', ' ', MPC2, 1,         
     +                  WORK( IPW ), MAX( 1, MPC2 ), RDEST,             
     +              ICCOL2 )
...
*                 sub( C ) := sub( C ) - w * v'
*
                  CALL YGERC( MPC2, NQV, -TAULOC( 1 ), WORK( IPW ), 1,  
     +                       WORK, 1, C( IOFFC2 ), LDC )
               END IF
*
            ELSE
...
*
               IF( MYCOL.EQ.IVCOL ) THEN
*
                  CALL YGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, TAU( JJV ),
     +                           1 )
                  TAULOC( 1 ) = TAU( JJV )
*
               ELSE
*
                  CALL YGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, TAULOC, 1, 
     +                          MYROW, IVCOL )
*
               END IF
*
