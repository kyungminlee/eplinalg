      SUBROUTINE PESYTRD( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,   
     +                  LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PESYTRD reduces a real symmetric matrix sub( A ) to symmetric
...
*     .. External Subroutines ..
     ...
     +                PELATRD, PESYR2K, PESYTD2, PB_TOPGET,             
     +       PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYTRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*           the matrix
*
            CALL PELATRD( UPLO, K+JB-1, JB, A, IA, JA, DESCA, D, E, TAU,
...
*
            CALL PESYR2K( UPLO, 'No transpose', K-1, JB, -ONE, A, IA, J,
...
*
            CALL PESYR2K( UPLO, 'No transpose', N-K-NB+1, NB, -ONE, A,  
...
*        Use unblocked code to reduce the last or only block
*
         CALL PESYTD2( UPLO, KK, A, IA+K-1, JA+K-1, DESCA, D, E,        
     +          TAU, WORK, LWORK, IINFO )
      END IF
*
