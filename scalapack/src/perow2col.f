      SUBROUTINE PEROW2COL( ICTXT, M, N, NB, VS, LDVS, VD, LDVD,        
...
*     ..
*     .. Array Arguments ..
      REAL(KIND=10)   VD( LDVD, * ), VS( LDVS, * ), WORK( * )
*     ..
*
*  Purpose
*  =======
...
*
*  WORK    (local workspace) REAL(KIND=10)
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGESD2D, EGERV2D, ELACPY
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC
...
*
                  DO 10 II = ISTART, MQ, NB*RBLKSKIP
                     JB = MIN( NB, MQ-II+1 )
                     CALL ELACPY( 'G', JB, N, VS(II,1), LDVS,           
     +                  WORK(JJ), JB )
                     JJ = JJ + NB*N
   10             CONTINUE
*
...
*
                  JJ = JJ - 1
                  IF( JJ.GT.0 )                CALL EGESD2D( ICTXT, JJ, 
     +1, WORK, JJ, IRDEST,                              CDEST )
*
...
*
                  ...
                  IF( JJ.GT.0 )                CALL EGERV2D( ICTXT, JJ, 
     +N, WORK, JJ, RSRC, ICSRC )
*
...
*
                  ...
                  DO 30 II = ISTART, MP, NB*CBLKSKIP
                     JB = MIN( NB, MP-II+1 )
                     CALL ELACPY( 'G', JB, N, WORK(JJ), JB, VD(II,1),   
     +                          LDVD )
                     JJ = JJ + NB*N
   30             CONTINUE
*
...
*
               ELSE
                  ...
                  DO 40 II = ISTART, MP, NB*CBLKSKIP
                     JB = MIN( NB, MP-II+1 )
                     CALL ELACPY( 'G', JB, N, VS(JJ,1), LDVS, VD(II,1), 
     +                            LDVD )
                     JJ = JJ + NB*RBLKSKIP
   40             CONTINUE
               END IF
...
*
            ...
            IRDEST = MOD( RDEST+MYDIST, NPROW )
            IF( (MYROW.NE.IRDEST).OR.(MYCOL.NE.CDEST) ) THEN
               CALL EGESD2D( ICTXT, MQ, N, VS, LDVS, IRDEST, CDEST )
            ELSE
               CALL ELACPY( 'G', MQ, N, VS, LDVS, VD, LDVD )
            END IF
         END IF
         IF( MYCOL.EQ.CDEST ) THEN
*
...
*
            ...
            MP = NUMROC( M, NB, MYROW, RDEST, NPROW )
            ICSRC = MOD( CSRC+MYDIST, NPCOL )
            IF( (MYCOL.NE.ICSRC).OR.(MYROW.NE. RSRC) )          CALL 
     +EGERV2D( ICTXT, MP, N, VD, LDVD, RSRC, ICSRC )
         END IF
      END IF
*
      RETURN
