      SUBROUTINE PELAREAD( FILNAM, A, DESCA, IRREAD, ICREAD, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELAREAD reads from a file named FILNAM a matrix and distribute
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, EGERV2D, EGESD2D,     
     +               IGEBS2D, IGEBR2D
*     ..
*     .. External Functions ..
      INTEGER            ICEIL
...
*
      IF( M.GT.DESCA( M_ ).OR. N.GT.DESCA( N_ ) ) THEN
         IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
            WRITE( *, FMT = * ) 'PELAREAD: Matrix too big to fit in'
            WRITE( *, FMT = * ) 'Abort ...'
         END IF
   ...
   10                CONTINUE
               ...
               ELSE
                  IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
                     CALL EGERV2D( ICTXT, IB, 1, A( II+(JJ+H-1)*LDA ),  
     +                            LDA, IRREAD, ICREAD )
                   ELSE IF( MYROW.EQ.IRREAD .AND. MYCOL.EQ.ICREAD ) THEN
                     DO 20 K = 1, IB
                        READ( NIN, FMT = * ) WORK( K )
   20                CONTINUE
                     CALL EGESD2D( ICTXT, IB, 1, WORK, DESCA( MB_ ),    
     +                          ICURROW, ICURCOL )
                  END IF
               END IF
               IF( MYROW.EQ.ICURROW )
