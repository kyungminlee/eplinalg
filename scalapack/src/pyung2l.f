      SUBROUTINE PYUNG2L( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYUNG2L generates an M-by-N complex distributed matrix Q denoting
...
*          as returned by PYGEQLF in the K columns of its distributed
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNG2L', -INFO )
...
*
      CALL PYLASET( 'All', M-N, N-K, ZERO, ZERO, A, IA, JA, DESCA )
      CALL PYLASET( 'All', N, N-K, ZERO, ONE, A, IA+M-N, JA, DESCA )
*
      TAUJ = ZERO
...
*
         CALL PYELSET( A, IA+M-N+J-JA, J, DESCA, ONE )
         CALL PYLARF( 'Left', M-N+J-JA+1, J-JA, A, IA, J, DESCA, 1, TAU,
     +                 A, IA, JA, DESCA, WORK )
*
     ...
     $      TAUJ = TAU( MIN( JJ, NQA0 ) )
         CALL PYSCAL( M-N+J-JA, -TAUJ, A, IA, J, DESCA, 1 )
         CALL PYELSET( A, IA+M-N+J-JA, J, DESCA, ONE-TAUJ )
*
*        Set A(ia+m-n+j-ja+1:ia+m-1,j) to zero
*
         CALL PYLASET( 'All', JA+N-1-J, 1, ZERO, ZERO, A, IA+M-N+J-JA+1,
     +                  J, DESCA )
*
   10 CONTINUE
*
