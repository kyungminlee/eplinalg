      SUBROUTINE PESYNTRD( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,  
     +                    LWORK, INFO )
      IMPLICIT NONE
*
...
*
*
*  Support for UPLO='U' is limited to calling the old, slow, PESYTRD
*  code.
*
*
*  Purpose
...
*  =======
*
*  PESYNTRD is a prototype version of PESYTRD which uses tailored
*  codes (either the serial, ESYTRD, or the parallel code, PESYTTRD)
*  when the workspace provided by the user is adequate.
*
*
*  PESYNTRD reduces a real symmetric matrix sub( A ) to symmetric
...
*  ========
*
*  PESYNTRD is faster than PESYTRD on almost all matrices,
...
*  by PESYTRD to keep the interface simple.  These restrictions are
...
*     .. External Subroutines ..
     ...
     +               BLACS_GRIDINIT, CHK1MAT, DESCSET, ESYTRD,          
     +          IGAMN2D, PCHK1MAT, PEELSET, PELAMR1D, PELATRD,          
     +          PESYR2K, PESYTD2, PESYTTRD, PETRMR2D,                   
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYNTRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PETRMR2D( UPLO, 'N', N, N, A, IA, JA, DESCA, WORK( INDB ),
     +                   1, 1, DESCB, ICTXT )
*
*
...
*
            IF( NPROWB.EQ.1 ) THEN
               CALL ESYTRD( UPLO, N, WORK( INDB ), NPS, WORK( INDD ),   
     +                    WORK( INDE ), WORK( INDTAU ), WORK( INDW ),   
     +                    LLWORK, INFO )
            ELSE
*
               CALL PESYTTRD( 'L', N, WORK( INDB ), 1, 1, DESCB,        
     +                 WORK( INDD ), WORK( INDE ),                      
...
*           way that PESYNTRD expects it.
*
         CALL PELAMR1D( N-1, WORK( INDE ), 1, 1, DESCB, E, 1, JA,       
     +            DESCA )
*
         CALL PELAMR1D( N, WORK( INDD ), 1, 1, DESCB, D, 1, JA, DESCA )
*
         CALL PELAMR1D( N, WORK( INDTAU ), 1, 1, DESCB, TAU, 1, JA,     
     +              DESCA )
*
         CALL PETRMR2D( UPLO, 'N', N, N, WORK( INDB ), 1, 1, DESCB, A,  
     +                 IA, JA, DESCA, ICTXT )
*
         IF( MYROWB.GE.0 )
...
*           the matrix
*
               CALL PELATRD( UPLO, K+JB-1, JB, A, IA, JA, DESCA, D, E,  
...
*
               CALL PESYR2K( UPLO, 'No transpose', K-1, JB, -ONE, A, IA,
...
*
               ...
               CALL PEELSET( A, I-1, J, DESCA, E( JX ) )
*
...
*
            CALL PESYTD2( UPLO, MIN( N, NB ), A, IA, JA, DESCA, D, E,   
...
*           of the matrix
*
               CALL PELATRD( UPLO, N-K+1, NB, A, I, J, DESCA, D, E, TAU,
...
*
               CALL PESYR2K( UPLO, 'No transpose', N-K-NB+1, NB, -ONE,  
...
*
               ...
               CALL PEELSET( A, I+NB, J+NB-1, DESCA, E( JX ) )
*
...
*        Use unblocked code to reduce the last or only block
*
            CALL PESYTD2( UPLO, KK, A, IA+K-1, JA+K-1, DESCA, D, E, TAU,
     +                     WORK, LWORK, IINFO )
         END IF
*
