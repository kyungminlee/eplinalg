      SUBROUTINE PELARFB( SIDE, TRANS, DIRECT, STOREV, M, N, K, V, IV,  
...
*     .. Array Arguments ..
      INTEGER            DESCC( * ), DESCV( * )
      REAL(KIND=10)   C( * ), T( * ), V( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PELARFB applies a real block reflector Q or its transpose Q**T to a
...
*
*  T       (local input) REAL(KIND=10) array, dimension MB_V by MB_V
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EGEBR2D, EGEBS2D,EGEMM,        
     +            EGSUM2D, ELAMOV, ELASET, ETRBR2D,                    
     +ETRBS2D, ETRMM, INFOG1L, INFOG2L, PB_TOPGET,                    
     +PBETRAN
*     ..
*     .. Intrinsic Functions ..
...
*
            ...
            IF( MYCOL.EQ.IVCOL ) THEN
               CALL EGEBS2D( ICTXT, 'Rowwise', ROWBTOP, MPC, K,         
     +               V( IOFFV ), LDV )
               IF( MYROW.EQ.IVROW )             CALL ETRBS2D( ICTXT, 
     +'Rowwise', ROWBTOP, UPLO,                           'Non unit', K,
     + K, T, NBV )
               CALL ELAMOV( 'All', MPC, K, V( IOFFV ), LDV, WORK( IPV ),
     +                       LV )
            ELSE
               CALL EGEBR2D( ICTXT, 'Rowwise', ROWBTOP, MPC, K,         
     +               WORK( IPV ), LV, MYROW, IVCOL )
               IF( MYROW.EQ.IVROW )             CALL ETRBR2D( ICTXT, 
     +'Rowwise', ROWBTOP, UPLO,                           'Non unit', K,
     + K, T, NBV, MYROW, IVCOL )
            END IF
*
...
*
   10          CONTINUE
               IF( K-ITOP .GT.0 ) THEN
                  CALL ELASET( 'Upper', IINXT-IIBEG+1, K-ITOP, ZERO,    
...
*
   20          CONTINUE
               IF( JJ.LE.( JJV+K-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL ELASET( 'All', KP, ITOP-JJ+JJV, ZERO, ZERO,      
     +                    WORK( IPV+II-IIV+(JJ-JJV)*LV ), LV )
                  CALL ELASET( 'Lower', KP, HEIGHT, ZERO, ONE,          
...
*
            IF( MPC.GT.0 ) THEN
               CALL EGEMM( 'Transpose', 'No transpose', NQC, K, MPC,    
     +                 ONE, C( IOFFC ), LDC, WORK( IPV ), LV, ZERO,     
     +                WORK( IPW ), LW )
            ELSE
               CALL ELASET( 'All', NQC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL EGSUM2D( ICTXT, 'Columnwise', ' ', NQC, K, WORK( IPW ),
     +                     LW, IVROW, MYCOL )
*
...
*
               CALL ETRMM( 'Right', UPLO, TRANST, 'Non unit', NQC, K,   
     +                   ONE, T, NBV, WORK( IPW ), LW )
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', NQC, K,          
     +              WORK( IPW ), LW )
            ELSE
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', NQC, K,          
...
*
            CALL EGEMM( 'No transpose', 'Transpose', MPC, NQC, K, -ONE, 
...
*           WORK( IPT ) is the workspace for PBETRAN
*
            IPV = 1
            IPW = IPV + K * NQC0
            IPT = IPW + NPV0 * K
...
*
            IF( MYCOL.EQ.IVCOL ) THEN
               IF( MYROW.EQ.IVROW ) THEN
                  CALL ELASET( 'All', IROFFV, K, ZERO, ZERO,            
     +              WORK( IPW ), LW )
                  IPW1 = IPW + IROFFV
                  CALL ELAMOV( 'All', NPV, K, V( IOFFV ), LDV,          
     +                WORK( IPW1 ), LW )
               ELSE
                  IPW1 = IPW
                  CALL ELAMOV( 'All', NPV, K, V( IOFFV ), LDV,          
...
*
   40             CONTINUE
     ...
     +                      WORK( IPW1+II-IIV+ITOP*LW ), LW )
            ...
            END IF
*
            CALL PBETRAN( ICTXT, 'Columnwise', 'Transpose', N+IROFFV, K,
     +                     MBV, WORK( IPW ), LW, ZERO, WORK( IPV ), LV, 
     +                    IVROW, IVCOL, -1, ICCOL, WORK( IPT ) )
*
...
*
            IF( NQC.GT.0 ) THEN
               CALL EGEMM( 'No transpose', 'Transpose', MPC, K, NQC,    
     +                  ONE, C( IOFFC ), LDC, WORK( IPV ), LV, ZERO,    
     +                  WORK( IPW ), LW )
            ELSE
               CALL ELASET( 'All', MPC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL EGSUM2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),   
     +                  LW, MYROW, IVCOL )
*
*           WORK( IPW ) = WORK( IPW ) * T' or WORK( IPW ) * T
*
...
*
                  CALL ETRBS2D( ICTXT, 'Columnwise', ' ', UPLO,         
     +                  'Non unit', K, K, T, NBV )
               ELSE
                  CALL ETRBR2D( ICTXT, 'Columnwise', ' ', UPLO,         
     +                  'Non unit', K, K, T, NBV, IVROW, MYCOL )
               END IF
               CALL ETRMM( 'Right', UPLO, TRANS, 'Non unit', MPC, K,    
     +                  ONE, T, NBV, WORK( IPW ), LW )
*
               CALL EGEBS2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     +                        LW )
            ELSE
               CALL EGEBR2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     +                        LW, MYROW, IVCOL )
            END IF
*
*               C            C      -     W       *     V'
...
*                        MPC x NQC    MPC x K         K x NQC
*
            CALL EGEMM( 'No transpose', 'No transpose', MPC, NQC, K,    
...
*
            IF( MYROW.EQ.IVROW ) THEN
               IF( MYCOL.EQ.IVCOL ) THEN
                  CALL ELASET( 'All', K, ICOFFV, ZERO, ZERO,            
     +              WORK( IPW ), LW )
                  IPW1 = IPW + ICOFFV * LW
                  CALL ELAMOV( 'All', K, MQV, V( IOFFV ), LDV,          
     +                WORK( IPW1 ), LW )
               ELSE
                  IPW1 = IPW
                  CALL ELAMOV( 'All', K, MQV, V( IOFFV ), LDV,          
...
*
   50             CONTINUE
                  IF( ( K-ILEFT ).GT.0 ) THEN
                     CALL ELASET( 'Lower', K-ILEFT, JJNXT-JJBEG+1, ZERO,
     +                             ONE,                             
     +WORK( IPW1+ILEFT+(JJBEG-JJV)*LW ),                             LW 
     +)
                     MYDIST = MYDIST + NPCOL
...
*
   60             CONTINUE
                  IF( II.LE.( IIV+K-1 ) ) THEN
                     WIDE = IRIGHT - ILEFT
                     CALL ELASET( 'All', ILEFT-II+IIV, KQ, ZERO, ZERO,  
     +                           WORK( IPW1+II-IIV+(JJ-JJV)*LW ), LW )
                     CALL ELASET( 'Upper', WIDE, KQ, ZERO, ONE,         
...
*
            CALL PBETRAN( ICTXT, 'Rowwise', 'Transpose', K, M+ICOFFV,   
     +                  NBV, WORK( IPW ), LW, ZERO, WORK( IPV ), LV,    
     +                 IVROW, IVCOL, ICROW, -1, WORK( IPT ) )
*
...
*
                  CALL ETRBS2D( ICTXT, 'Rowwise', ' ', UPLO, 'Non unit',
     +                           K, K, T, MBV )
               ELSE
                  CALL ETRBR2D( ICTXT, 'Rowwise', ' ', UPLO, 'Non unit',
...
*
            ...
            IF( MYROW.EQ.IVROW ) THEN
               CALL EGEBS2D( ICTXT, 'Columnwise', COLBTOP, K, NQC,      
     +                  V( IOFFV ), LDV )
               IF( MYCOL.EQ.IVCOL )             CALL ETRBS2D( ICTXT, 
     +'Columnwise', COLBTOP, UPLO,                           'Non unit',
     + K, K, T, MBV )
               CALL ELAMOV( 'All', K, NQC, V( IOFFV ), LDV, WORK( IPV ),
     +                       LV )
            ELSE
               CALL EGEBR2D( ICTXT, 'Columnwise', COLBTOP, K, NQC,      
     +                  WORK( IPV ), LV, IVROW, MYCOL )
               IF( MYCOL.EQ.IVCOL )             CALL ETRBR2D( ICTXT, 
     +'Columnwise', COLBTOP, UPLO,                           'Non unit',
