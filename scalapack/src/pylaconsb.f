      SUBROUTINE PYLACONSB( A, DESCA, I, L, M, H44, H33, H43H34, BUF,   
     +                    LWORK )
      IMPLICIT NONE
*
...
*     .. Scalar Arguments ..
      INTEGER            I, L, LWORK, M
      COMPLEX(KIND=10)            H33, H43H34, H44
*     ..
*     .. Array Arguments ..
      INTEGER            DESCA( * )
      COMPLEX(KIND=10)            A( * ), BUF( * )
*     ..
*
*  Purpose
*  =======
*
*  PYLACONSB looks for two consecutive small subdiagonal elements by
...
*        the LAPACK algorithm YLAHQR, a loop of M goes from I-2 down to
*        L and examines
...
*     .. Local Scalars ..
     ...
     $                   MYROW, NPCOL, NPROW, NUM, RIGHT, UP
      REAL(KIND=10)               S, TST1, ULP
      COMPLEX(KIND=10)            CDUM, H00, H10, H11, H12, H21, H22, 
     +H33S, H44S,                    V1, V2, V3
*     ..
*     .. External Functions ..
      INTEGER            ILCM
...
*
     ...
     $   ISTR2 = ISTR2 + 1
      ...
      IF( LWORK.LT.7*ISTR2 ) THEN
         CALL PXERBLA( CONTXT, 'PYLACONSB', 10 )
         RETURN
      END IF
      ISTR3 = 3*ISTR2
      ISTR4 = ISTR3 + ISTR2
...
*
      IF( IBUF1.GT.0 ) THEN
         CALL YGESD2D( CONTXT, IBUF1, 1, BUF( ISTR1+1 ), IBUF1, DOWN,   
     +               RIGHT )
      END IF
      IF( IBUF2.GT.0 ) THEN
         CALL YGESD2D( CONTXT, IBUF2, 1, BUF( ISTR2+1 ), IBUF2, UP,     
     +             MYCOL )
      END IF
      IF( IBUF3.GT.0 ) THEN
         CALL YGESD2D( CONTXT, IBUF3, 1, BUF( ISTR3+1 ), IBUF3, MYROW,  
     +                LEFT )
      END IF
      IF( IBUF4.GT.0 ) THEN
         CALL YGESD2D( CONTXT, IBUF4, 1, BUF( ISTR4+1 ), IBUF4, UP,     
     +             LEFT )
      END IF
      IF( IBUF5.GT.0 ) THEN
         CALL YGESD2D( CONTXT, IBUF5, 1, BUF( ISTR5+1 ), IBUF5, MYROW,  
     +                RIGHT )
      END IF
*
...
*
      IF( IRCV1.GT.0 ) THEN
         CALL YGERV2D( CONTXT, IRCV1, 1, BUF( ISTR1+1 ), IRCV1, UP,     
     +             LEFT )
      END IF
      IF( IRCV2.GT.0 ) THEN
         CALL YGERV2D( CONTXT, IRCV2, 1, BUF( ISTR2+1 ), IRCV2, DOWN,   
     +               MYCOL )
      END IF
      IF( IRCV3.GT.0 ) THEN
         CALL YGERV2D( CONTXT, IRCV3, 1, BUF( ISTR3+1 ), IRCV3, MYROW,  
     +                RIGHT )
      END IF
      IF( IRCV4.GT.0 ) THEN
         CALL YGERV2D( CONTXT, IRCV4, 1, BUF( ISTR4+1 ), IRCV4, DOWN,   
     +               RIGHT )
      END IF
      IF( IRCV5.GT.0 ) THEN
         CALL YGERV2D( CONTXT, IRCV5, 1, BUF( ISTR5+1 ), IRCV5, MYROW,  
     +                LEFT )
      END IF
*
*     Start main loop
