      SUBROUTINE PYTRRFS( UPLO, TRANS, DIAG, N, NRHS, A, IA, JA, DESCA, 
...
*     .. Array Arguments ..
      ...
      COMPLEX(KIND=10)            A( * ), B( * ), WORK( * ), X( * )
*     ..
*
*  Purpose
*  =======
*
*  PYTRRFS provides error bounds and backward error estimates for the
*  solution to a system of linear equations with a triangular
*  coefficient matrix.
*
*  The solution matrix X must be computed by PYTRTRS or some other
*  means before entering this routine.  PYTRRFS does not do iterative
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      REAL(KIND=10)               ZERO, RONE
      PARAMETER          ( ZERO = 0.0E0_10, RONE = 1.0E0_10 )
      COMPLEX(KIND=10)            ONE
...
*     .. Local Scalars ..
     ...
     $                   MYROW, NP, NP0, NPCOL, NPMOD, NPROW, NZ
      REAL(KIND=10)               EPS, EST, LSTRES, S, SAFE1, SAFE2, 
     +SAFMIN
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGEBR2D, YGEBS2D, CHK1MAT,     
     +               DESCSET, INFOG2L, PYATRMV, PYAXPY, PCHK1MAT,       
     +             PCHK2MAT, PYCOPY, PYLACON, PYTRMV,                   
     + PYTRSV, PXERBLA, EGAMX2D
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                  IX, JX, DESCX, 17, 5, IDUM1, IDUM2, INFO )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYTRRFS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PYCOPY( N, X, IX, JX+K, DESCX, 1, WORK( IPR ), IW, JW,    
     +             DESCW, 1 )
         CALL PYTRMV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA,           
     +      WORK( IPR ), IW, JW, DESCW, 1 )
         CALL PYAXPY( N, -ONE, B, IB, JB+K, DESCB, 1, WORK( IPR ), IW,  
     +               JW, DESCW, 1 )
*
...
*
   ...
   20          CONTINUE
         ...
         END IF
*
         CALL PYATRMV( UPLO, TRANS, DIAG, N, RONE, A, IA, JA, DESCA, X, 
...
*
*        Use PYLACON to estimate the infinity-norm of the matrix
*           inv(op(A)) * diag(W),
...
*
   ...
   50    CONTINUE
         IF( MYCOL.EQ.IXBCOL ) THEN
            CALL YGEBS2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IPR ),    
     +                 DESCW( LLD_ ) )
         ELSE
            CALL YGEBR2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IPR ),    
     +                 DESCW( LLD_ ), MYROW, IXBCOL )
         END IF
         DESCW( CSRC_ ) = MYCOL
         CALL PYLACON( N, WORK( IPV ), IW, JW, DESCW, WORK( IPR ),      
...
*
               CALL PYTRSV( UPLO, TRANST, DIAG, N, A, IA, JA, DESCA,    
     +                   WORK( IPR ), IW, JW, DESCW, 1 )
               IF( MYCOL.EQ.IXBCOL ) THEN
                  IF( NP.GT.0 ) THEN
...
*
   ...
   70                CONTINUE
                  END IF
               END IF
               CALL PYTRSV( UPLO, TRANSN, DIAG, N, A, IA, JA, DESCA,    
     +                   WORK( IPR ), IW, JW, DESCW, 1 )
            END IF
            GO TO 50
         END IF
...
*           where op(A) = A or A', depending on TRANS.
*
            CALL PYCOPY( N, X, IX, J+K, DESCX, 1, WORK( IPR ), IW, JW,  
     +                  DESCW, 1 )
            CALL PYTRMV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA,        
     +            WORK( IPR ), IW, JW, DESCW, 1 )
            CALL PYAXPY( N, -ONE, B, IB, J+K, DESCB, 1, WORK( IPR ),    
     +                IW, JW, DESCW, 1 )
*
*           Compute componentwise relative backward error from formula
*
...
*
  ...
  100             CONTINUE
            ...
            END IF
*
            CALL PYATRMV( UPLO, TRANS, DIAG, N, RONE, A, IA, JA, DESCA, 
