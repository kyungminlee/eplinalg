      SUBROUTINE PYLAWIL( II, JJ, M, A, DESCA, H44, H33, H43H34, V )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLAWIL gets the transform given by H44,H33, & H43H34 into V
*     starting at row M.
*
*  Notes
*  =====
...
*     .. Local Scalars ..
     ...
     $                   RSRC, UP
      REAL(KIND=10)               S
      COMPLEX(KIND=10)            CDUM, H22, H33S, H44S, V1, V2
*     ..
*     .. Local Arrays ..
      COMPLEX(KIND=10)            BUF( 4 ), V3( 1 ), H11( 1 ), H12( 1 ),
     + H21( 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, YGERV2D, YGESD2D
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MOD
*     ..
...
*
     ...
     $                    IROW, ICOL, RSRC, JSRC )
            ...
            CALL YGESD2D( CONTXT, 1, 1, BUF, 1, II, JJ )
         END IF
     ...
     $                    ICOL, RSRC, JSRC )
            ...
            CALL YGESD2D( CONTXT, 4, 1, BUF, 4, II, JJ )
         END IF
         IF( ( MYROW.EQ.II ) .AND. ( MYCOL.EQ.JJ ) ) THEN
            CALL INFOG2L( M+2, M+2, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $                    IROW, ICOL, RSRC, JSRC )
            IF( NPCOL.GT.1 ) THEN
               CALL YGERV2D( CONTXT, 1, 1, V3, 1, MYROW, LEFT )
            ELSE
               V3( 1 ) = A( ( ICOL-2 )*LDA+IROW )
            END IF
            IF( NUM.GT.1 ) THEN
               CALL YGERV2D( CONTXT, 4, 1, BUF, 4, UP, LEFT )
               H11( 1 ) = BUF( 1 )
               H21( 1 ) = BUF( 2 )
               H12( 1 ) = BUF( 3 )
               H22 = BUF( 4 )
     ...
     $        THEN
            CALL INFOG2L( M, M, DESCA, NPROW, NPCOL, MYROW, MYCOL, IROW,
     $                    ICOL, RSRC, JSRC )
            CALL YGESD2D( CONTXT, 1, 1, A( ( ICOL-1 )*LDA+IROW ), 1, II,
     +                     JJ )
         END IF
     ...
     $                    IROW, ICOL, RSRC, JSRC )
            IF( NUM.GT.1 ) THEN
               CALL YGERV2D( CONTXT, 1, 1, H11( 1 ), 1, UP, LEFT )
            ELSE
               H11( 1 ) = A( ( ICOL-3 )*LDA+IROW-2 )
            END IF
            IF( NPROW.GT.1 ) THEN
               CALL YGERV2D( CONTXT, 1, 1, H12, 1, UP, MYCOL )
            ELSE
               H12( 1 ) = A( ( ICOL-2 )*LDA+IROW-2 )
            END IF
            IF( NPCOL.GT.1 ) THEN
               CALL YGERV2D( CONTXT, 1, 1, H21( 1 ), 1, MYROW, LEFT )
            ELSE
               H21( 1 ) = A( ( ICOL-3 )*LDA+IROW-1 )
            END IF
            H22 = A( ( ICOL-2 )*LDA+IROW-1 )
