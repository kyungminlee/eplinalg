      SUBROUTINE PYGECON( NORM, N, A, IA, JA, DESCA, ANORM, RCOND, WORK,
...
*  =======
*
*  PYGECON estimates the reciprocal of the condition number of a general
...
*     .. Local Scalars ..
     ...
     $                   NPCOL, NPMOD, NPROW, NQ, NQMOD
      REAL(KIND=10)               AINVNM, SCALE, SL, SMLNUM, SU
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCSET, YGEBR2D,     
     +               YGEBS2D, INFOG2L, PYAMAX, PCHK1MAT,                
     +    PYLATRS, PYLACON, PYERSCL, PB_TOPGET,                    
     +PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGECON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PYLATRS( 'Lower', 'No transpose', 'Unit', NORMIN,      
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PYLATRS( 'Lower', 'Conjugate transpose', 'Unit',       
