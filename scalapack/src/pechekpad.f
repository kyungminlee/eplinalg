      SUBROUTINE PECHEKPAD( ICTXT, MESS, M, N, A, LDA, IPRE, IPOST,     
     +                 CHKVAL )
      IMPLICIT NONE
*
...
*  =======
*
*  PECHEKPAD checks that the padding around a local array has not
*  been overwritten since the call to PEFILLPAD.  3 types of errors
*  are reported:
*
...
*
*  A       (local input) REAL(KIND=10) array of dimension (LDA,N).
...
*
*  CHKVAL  (local input) REAL(KIND=10)
...
*
   ...
   10    CONTINUE
      ELSE
         WRITE( *, FMT = * ) 'WARNING no pre-guardzone in PECHEKPAD'
      END IF
*
...
*
   ...
   20    CONTINUE
      ELSE
         WRITE( *, FMT = * )          
     + 'WARNING no post-guardzone buffer in PECHEKPAD'
      END IF
*
