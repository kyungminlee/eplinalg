      SUBROUTINE PYDTTRF( N, DL, D, DU, JA, DESCA, AF, LAF, WORK, LWORK,
     +                     INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYDTTRF computes a LU factorization
...
*  subsequent calls to PYDTTRS to solve linear systems.
*
...
*     .. External Subroutines ..
     ...
     +               YAXPY, YYDOTC, YGEMM, YGERV2D, YGESD2D, YLAMOV,    
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,      
     + 'PYDTTRF, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*INT_ONE )) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT,       'PYDTTRF, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT,       'PYDTTRF: auxiliary storage error ',
     +       -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
           INFO = -10
           CALL PXERBLA( ICTXT,       'PYDTTRF: worksize error ',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYDTTRF', -INFO )
         RETURN
      END IF
*
...
*
        CALL YDTTRF( ODD_SIZE, DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ), 
...
*
          CALL YDTTRSV( 'L', 'N', ODD_SIZE, INT_ONE,                   
     +DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),                   DU( 
     +PART_OFFSET+1 ), AF( WORK_U+1 ), ODD_SIZE,                   INFO 
     +)
*
*
*         Calculate the "spike" fillin, ${U_i}^C {{GL}_i}^C = {DU_i}^C$
*
          CALL YTRRV2D( ICTXT, 'U', 'N', 1, 1, AF( 1 ), ODD_SIZE, 0,    
     +               MYCOL-1 )
*
          AF( 1 ) = CONJG( AF( 1 ) )
*
          CALL YDTTRSV( 'U', 'C', ODD_SIZE, INT_ONE,             DL( 
     +PART_OFFSET+2 ), D( PART_OFFSET+1 ),             DU( 
     +PART_OFFSET+1 ),             AF( 1 ), ODD_SIZE, INFO )
*
*
*         Calculate the update block for previous proc, E_i = GL_i{GU_i}
*
          CALL YYDOTC( ODD_SIZE, DOTC, AF( 1 ), 1, AF( WORK_U+1 ), 1 )
...
*
          CALL YGESD2D( ICTXT, INT_ONE, INT_ONE, AF( ODD_SIZE+3 ),      
     +             INT_ONE, 0, MYCOL-1 )
*
*
          IF ( MYCOL .LT. NP-1 ) THEN
...
*
          CALL YGESD2D( ICTXT, INT_ONE, INT_ONE,                        
     +AF( ODD_SIZE+1 ),                        INT_ONE, 0, MYCOL-1 )
*
          CALL YGESD2D( ICTXT, INT_ONE, INT_ONE,                        
     +AF( WORK_U+ODD_SIZE+1 ),                        INT_ONE, 0, 
     +MYCOL-1 )
*
        ENDIF
*
...
*         operations.
*
        AF( ODD_SIZE+2 ) =         CMPLX( D( PART_OFFSET+ODD_SIZE+1 ) , 
     +KIND=10)
*
...
*
           CALL YGERV2D( ICTXT, INT_ONE, INT_ONE,                AF( 
     +ODD_SIZE+2+1 ),                INT_ONE, 0,                MYCOL+1 
     +)
*
...
*
          IF( MYCOL-LEVEL_DIST .GE. 0 ) THEN
            CALL YGERV2D( ICTXT, INT_ONE, INT_ONE, WORK( 1 ),           
...
*
          CALL YGERV2D( ICTXT, INT_ONE, INT_ONE,         AF( ODD_SIZE+1 
     +),                        INT_ONE, 0, COMM_PROC )
*
          CALL YGERV2D( ICTXT, INT_ONE, INT_ONE,         AF( 
     +WORK_U+ODD_SIZE+1 ),                        INT_ONE, 0, COMM_PROC 
     +)
*
          IF( INFO .EQ. 0 ) THEN
*
...
*
          CALL YGESD2D( ICTXT, INT_ONE, INT_ONE, WORK( 1 ), INT_ONE,    
     +                  0, MYCOL+LEVEL_DIST )
*
        ENDIF
...
*
           CALL YGERV2D( ICTXT, INT_ONE, INT_ONE,             AF( 
     +WORK_U+ODD_SIZE+2+1 ),             INT_ONE, 0, MYCOL-LEVEL_DIST/2 
     +)
*
*           Receive offdiagonal block(s) from proc level_dist/2 to the
*           left
*
           CALL YGERV2D( ICTXT, INT_ONE, INT_ONE,             AF( 
     +ODD_SIZE+2+1 ),             INT_ONE, 0, MYCOL-LEVEL_DIST/2 )
*
          ENDIF
*
*
