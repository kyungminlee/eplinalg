      SUBROUTINE PYDTTRSV( UPLO, TRANS, N, NRHS, DL, D, DU, JA, DESCA,  
...
*  =======
*
*  PYDTTRSV solves a tridiagonal triangular system of linear equations
*
...
*  Gaussian elimination code PC@(dom_pre)TTRF
...
*
*  Routine PYDTTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*          PYDTTRF and this is stored in AF. If a linear system
*          is to be solved using PYDTTRS after the factorization
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 3 )
         CALL PXERBLA( ICTXT,      
     + 'PYDTTRSV, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*INT_ONE )) THEN
         INFO = -( 9*100+4 )
         CALL PXERBLA( ICTXT,       'PYDTTRSV, D&C alg.: NB too small', 
     +      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -16
         CALL PXERBLA( ICTXT,       'PYDTTRSV: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYDTTRSV', -INFO )
         RETURN
      END IF
*
...
*
        CALL YDTTRSV( UPLO, 'N', ODD_SIZE, NRHS, DL( PART_OFFSET+2 ),   
     +              D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ),            
...
*
            CALL YAXPY( NRHS, -DL( PART_OFFSET+ODD_SIZE+1 ),            
     + B( PART_OFFSET+ODD_SIZE ), LLDB,             B( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
        ENDIF
*
*
...
*
            CALL YGEMM( 'C', 'N', INT_ONE, NRHS, ODD_SIZE, -CONE,       
     +            AF( 1 ), ODD_SIZE, B( PART_OFFSET+1 ), LLDB,          
     +         CZERO, WORK( 1+INT_ONE-INT_ONE ), INT_ONE )
        ENDIF
*
*
************************************************
...
*
        IF( MYCOL .GT. 0) THEN
*
          CALL YGESD2D( ICTXT, INT_ONE, NRHS,                      
     +WORK( 1 ), INT_ONE,                      0, MYCOL - 1 )
*
        ENDIF
*
...
*
          CALL YGERV2D( ICTXT, INT_ONE, NRHS,                      
     +WORK( 1 ), INT_ONE,                      0, MYCOL + 1 )
*
*         Combine contribution to locally stored right hand sides
*
          CALL YMATADD( INT_ONE, NRHS, CONE,                 WORK( 1 ), 
     +INT_ONE, CONE,                 B( PART_OFFSET+ODD_SIZE + 1 ), 
     +LLDB )
*
        ENDIF
*
...
*
            CALL YGERV2D( ICTXT, INT_ONE, NRHS,                      
     +WORK( 1 ),                      INT_ONE, 0, MYCOL-LEVEL_DIST )
*
            CALL YMATADD( INT_ONE, NRHS, CONE,                 WORK( 1 
     +), INT_ONE, CONE,                 B( PART_OFFSET+ODD_SIZE + 1 ), 
     +LLDB )
...
*
        CALL YTBTRS( 'L', 'N', 'U', INT_ONE, MIN( INT_ONE, INT_ONE-1 ), 
     +               NRHS, AF( ODD_SIZE+2 ), INT_ONE+1,                
...
*
          CALL YGEMM( 'C', 'N', INT_ONE, NRHS, INT_ONE, -CONE,          
     +            AF( (ODD_SIZE)*INT_ONE+1 ),                      
     +INT_ONE,                      B( PART_OFFSET+ODD_SIZE+1 ),        
     +              LLDB, CZERO,                      WORK( 1 ),        
     +              INT_ONE )
*
...
*
          CALL YGEMM( 'N', 'N', INT_ONE, NRHS, INT_ONE, -CONE,          
     +            AF( ODD_SIZE*INT_ONE+2+1 ),                      
     +INT_ONE,                      B( PART_OFFSET+ODD_SIZE+1 ),        
     +              LLDB, CZERO,                      WORK( 1 ),        
     +              INT_ONE )
*
...
*       Solve with diagonal block
*
        CALL YTBTRS( 'L', 'C', 'U', INT_ONE, MIN( INT_ONE, INT_ONE-1 ), 
     +               NRHS, AF( ODD_SIZE+2 ), INT_ONE+1,                
     +B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*
          IF( MYCOL+LEVEL_DIST .LT. NPCOL-1 ) THEN
*
          CALL YGESD2D( ICTXT, INT_ONE, NRHS,                      B( 
...
*
          CALL YGEMM( 'N', 'N', ODD_SIZE, NRHS, INT_ONE, -CONE, AF( 1 ),
     +                 ODD_SIZE, WORK( 1+INT_ONE-INT_ONE ), INT_ONE,    
     +             CONE, B( PART_OFFSET+1 ), LLDB )
*
        ENDIF
*
*
...
*           locally stored portion of right hand side(s)
*
            CALL YAXPY( NRHS, -CONJG( DL( PART_OFFSET+ODD_SIZE+1 ) ),   
...
*       Use main partition in each processor to solve locally
*
        CALL YDTTRSV( UPLO, 'C', ODD_SIZE, NRHS, DL( PART_OFFSET+2 ),   
     +              D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ),            
     +     B( PART_OFFSET+1 ), LLDB, INFO )
*
      ENDIF
...
*           locally stored portion of right hand side(s)
*
            CALL YAXPY( NRHS, -CONJG( DU( PART_OFFSET+ODD_SIZE ) ),     
     +        B( PART_OFFSET+ODD_SIZE ), LLDB,             B( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
        ENDIF
...
*
     ...
     +          LLDB, CZERO, WORK( 1+INT_ONE-INT_ONE ),                 
     +  INT_ONE )
        ENDIF
*
*
...
*       Solve with diagonal block
*
        CALL YTBTRS( 'U', 'C', 'N', INT_ONE, MIN( INT_ONE, INT_ONE-1 ), 
     +               NRHS, AF( ODD_SIZE+2 ), INT_ONE+1,                
     +B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*
          CALL YGEMM( 'N', 'N', INT_ONE, NRHS, INT_ONE, -CONE,          
     +            AF( WORK_U+ODD_SIZE*INT_ONE+2+1 ),                    
     +  INT_ONE,                      B( PART_OFFSET+ODD_SIZE+1 ),      
     +                LLDB, CZERO,                      WORK( 1 ),      
     +                INT_ONE )
*
...
*       Solve with diagonal block
*
        CALL YTBTRS( 'U', 'N', 'N', INT_ONE, MIN( INT_ONE, INT_ONE-1 ), 
     +               NRHS, AF( ODD_SIZE+2 ), INT_ONE+1,                
     +B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*           processor's solution.
*
          CALL YGEMM( 'N', 'N', ODD_SIZE, NRHS, INT_ONE, -CONE,         
     +        AF( WORK_U+1 ), ODD_SIZE,                 WORK( 
     +1+INT_ONE-INT_ONE ), INT_ONE, CONE,                 B( 
     +PART_OFFSET+1 ), LLDB )
*
        ENDIF
*
...
*           locally stored portion of right hand side(s)
*
            CALL YAXPY( NRHS, -( DU( PART_OFFSET+ODD_SIZE ) ),          
     +        B( PART_OFFSET+ODD_SIZE+1 ), LLDB,                  B( 
     +PART_OFFSET+ODD_SIZE ), LLDB )
*
        ENDIF
*
*       Use main partition in each processor to solve locally
*
        CALL YDTTRSV( UPLO, 'N', ODD_SIZE, NRHS, DU( PART_OFFSET+2 ),   
     +              D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ),            
     +     B( PART_OFFSET+1 ), LLDB, INFO )
*
      ENDIF
