      SUBROUTINE PEORMBR( VECT, SIDE, TRANS, M, N, K, A, IA, JA, DESCA, 
...
*  =======
*
*  If VECT = 'Q', PEORMBR overwrites the general real distributed M-by-N
...
*
*  If VECT = 'P', PEORMBR overwrites sub( C ) with
*
*                       SIDE = 'L'           SIDE = 'R'
...
*  PEGEBRD when reducing a real distributed matrix A(IA:*,JA:*) to
...
*          distributed matrix reduced by PEGEBRD.
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension
...
*          as returned by PEGEBRD in its array argument TAUQ or TAUP.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PEORMLQ,    
     +                PEORMQR, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORMBR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         IF( NQ.GE.K ) THEN
*
*           Q was determined by a call to PEGEBRD with nq >= k
*
            CALL PEORMQR( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
     +                   C, IC, JC, DESCC, WORK, LWORK, IINFO )
         ELSE IF( NQ.GT.1 ) THEN
*
*           Q was determined by a call to PEGEBRD with nq < k
*
            CALL PEORMQR( SIDE, TRANS, MI, NI, NQ-1, A, IA+1, JA, DESCA,
...
*
         ...
         IF( NQ.GT.K ) THEN
*
*           P was determined by a call to PEGEBRD with nq > k
*
            CALL PEORMLQ( SIDE, TRANST, M, N, K, A, IA, JA, DESCA, TAU, 
     +                    C, IC, JC, DESCC, WORK, LWORK, IINFO )
         ELSE IF( NQ.GT.1 ) THEN
*
*           P was determined by a call to PEGEBRD with nq <= k
*
            CALL PEORMLQ( SIDE, TRANST, MI, NI, NQ-1, A, IA, JA+1,      
