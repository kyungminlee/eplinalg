      SUBROUTINE PELARED2D( N, IA, JA, DESC, BYROW, BYALL, WORK, LWORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESC( * )
      REAL(KIND=10)   BYALL( * ), BYROW( * ), WORK( LWORK )
*     ..
*
*  Purpose
*  =======
*
*  PELARED2D redistributes a 1D array
*
...
*
*  BYROW   (local input) distributed block cyclic REAL(KIND=10) array
...
*
      DO 30 PROW = 0, NPROW - 1
         ...
         IF( MYROW.EQ.PROW ) THEN
            CALL ECOPY( BUFLEN, BYROW, 1, WORK, 1 )
            CALL EGEBS2D( DESC( CTXT_ ), 'C', ' ', BUFLEN, 1, WORK,     
     +                BUFLEN )
         ELSE
            CALL EGEBR2D( DESC( CTXT_ ), 'C', ' ', BUFLEN, 1, WORK,     
     +                BUFLEN, PROW, MYCOL )
         END IF
*
         ALLI = PROW*MB
