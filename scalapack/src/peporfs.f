      SUBROUTINE PEPORFS( UPLO, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,
...
*  =======
*
*  PEPORFS improves the computed solution to a system of linear
...
*
*  AF      (local input) REAL(KIND=10) pointer into the local memory
...
*     .. External Subroutines ..
     ...
     +     PEASYMV, PEAXPY, PECOPY, PESYMV,                    PEPOTRS, 
     +PELACON, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                  IX, JX, DESCX, 19, 5, IDUM1, IDUM2, INFO )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPORFS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     +             DESCW, 1 )
         CALL PESYMV( UPLO, N, -ONE, A, IA, JA, DESCA, X, IX, JX+K,     
...
*
   ...
   30          CONTINUE
         ...
         END IF
*
         CALL PEASYMV( UPLO, N, ONE, A, IA, JA, DESCA, X, IX, JX+K,     
...
*
            CALL PEPOTRS( UPLO, N, 1, AF, IAF, JAF, DESCAF,             
...
*
     ...
     +                  DESCW, 1 )
            CALL PESYMV( UPLO, N, -ONE, A, IA, JA, DESCA, X, IX, J+K,   
     +                DESCX, 1, ONE, WORK( IPR ), IW, JW, DESCW, 1 )
*
*           Compute componentwise relative backward error from formula
*
...
*
  ...
  120             CONTINUE
            ...
            END IF
*
            CALL PEASYMV( UPLO, N, ONE, A, IA, JA, DESCA, X, IX, J+K,   
     +                  DESCX, 1, ONE, WORK( IPB ), IW, JW, DESCW, 1 )
*
            S = ZERO
            IF( MYCOL.EQ.ICURCOL ) THEN
