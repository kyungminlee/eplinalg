      SUBROUTINE PYUNG2R( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYUNG2R generates an M-by-N complex distributed matrix Q denoting
...
*          returned by PYGEQRF in the K columns of its array
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNG2R', -INFO )
...
*
      CALL PYLASET( 'All', K, N-K, ZERO, ZERO, A, IA, JA+K, DESCA )
      CALL PYLASET( 'All', M-K, N-K, ZERO, ONE, A, IA+K, JA+K, DESCA )
*
      TAUJ = ZERO
...
*
         IF( J.LT.JA+N-1 ) THEN
            CALL PYELSET( A, IA+J-JA, J, DESCA, ONE )
            CALL PYLARF( 'Left', M-J+JA, JA+N-J-1, A, IA+J-JA, J, DESCA,
...
*
     ...
     $                    NPCOL )
         IF( MYCOL.EQ.IACOL )
     $      TAUJ = TAU( MIN( JJ, KQ ) )
         IF( J-JA.LT.M-1 )       CALL PYSCAL( M-J+JA-1, -TAUJ, A, 
     +IA+J-JA+1, J, DESCA, 1 )
         CALL PYELSET( A, IA+J-JA, J, DESCA, ONE-TAUJ )
*
*        Set A(ia:ia+j-ja-1,j) to zero
*
         CALL PYLASET( 'All', J-JA, 1, ZERO, ZERO, A, IA, J, DESCA )
*
   10 CONTINUE
*
