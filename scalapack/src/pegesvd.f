      ...
      SUBROUTINE PEGESVD(JOBU,JOBVT,M,N,A,IA,JA,DESCA,S,U,IU,JU,DESCU,  
...
*  =======
*
*  PEGESVD computes the singular value decomposition (SVD) of an
...
*
*  A       (local input/workspace) block cyclic REAL(KIND=10)
*          array,
...
*
*  S       (global output) REAL(KIND=10)   array, dimension SIZE
*          The singular values of A, sorted so that S(i) >= S(i+1).
*
*  U       (local output) REAL(KIND=10)   array, local dimension
...
*
*  VT      (local output) REAL(KIND=10)   array, local dimension
...
*
*  WORK    (local workspace/output) REAL(KIND=10)   array, dimension
*          (LWORK)
...
*          PELANGE, PELARED1D, PELARED2D, PEGEBRD. Using the
*          standard notation
*
...
*          to the workspace required for the subprograms EBDSQR,
*          PEORMBR(QLN), and PEORMBR(PRT), where QLN and PRT are the
*          values of the arguments VECT, SIDE, and TRANS in the call
*          to PEORMBR. NRU is equal to the local number of rows of
...
*          > 0:  if EBDSQR did not converge
*                If INFO = MIN(M,N) + 1, then PEGESVD has detected
*                heterogeneity by finding that eigenvalues were not
*                identical across the process grid. In this case, the
*                accuracy of the results from PEGESVD cannot be
*                guaranteed.
*
*  =====================================================================
*
*  The results of PEGEBRD, and therefore PEGESVD, may vary slightly
...
*  ======================
*
*  The routine PEGESVD inherits the same alignement requirement as
*  the routine PEGEBRD, namely:
*
...
*     .. Local Scalars ..
     ...
     +        WPDORMBRQLN
      REAL(KIND=10) ANRM,BIGNUM,EPS,RMAX,RMIN,SAFMIN,SIGMA,SMLNUM
*     ..
*     .. Local Arrays ..
...
*     .. External Functions ..
      LOGICAL LSAME
      INTEGER NUMROC
      REAL(KIND=10) PELAMCH,PELANGE
      EXTERNAL LSAME,NUMROC,PELAMCH,PYLANGE
*     ..
*     .. External Subroutines ..
      EXTERNAL BLACS_GET,BLACS_GRIDEXIT,BLACS_GRIDINFO,BLACS_GRIDINIT,  
     +        CHK1MAT,EBDSQR,DESCINIT,EGAMN2D,EGAMX2D,ESCAL,IGAMX2D,    
     +      IGEBR2D,IGEBS2D,PCHK1MAT,PEGEBRD,PEGEMR2D,PELARED1D,        
     +  PELARED2D,PELASCL,PELASET,PEORMBR,PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF (INFO.NE.0) THEN
          CALL PXERBLA(DESCA(CTXT_),'PEGESVD',-INFO)
          RETURN
...
*
      SAFMIN = PELAMCH(DESCA(CTXT_),'Safe minimum')
      EPS = PELAMCH(DESCA(CTXT_),'Precision')
      SMLNUM = SAFMIN/EPS
      BIGNUM = ONE/SMLNUM
      RMIN = SQRT(SMLNUM)
...
*
      ANRM = PELANGE('1',M,N,A,IA,JA,DESCA,WORK(INDWORK))
...
*
      IF (ISCALE.EQ.1) THEN
          CALL PELASCL('G',ONE,SIGMA,M,N,A,IA,JA,DESCA,INFO)
      END IF
*
      CALL PEGEBRD(M,N,A,IA,JA,DESCA,WORK(INDD),WORK(INDE),             
...
*
      IF (M.GE.N) THEN
*        Distribute D
          CALL PELARED1D(N+IOFFD,IA,JA,DESCA,WORK(INDD),WORK(INDD2),    
     +                WORK(INDWORK),LLWORK)
*        Distribute E
          CALL PELARED2D(M+IOFFE,IA,JA,DESCA,WORK(INDE),WORK(INDE2),    
     +                WORK(INDWORK),LLWORK)
      ELSE
*        Distribute D
          CALL PELARED2D(M+IOFFD,IA,JA,DESCA,WORK(INDD),WORK(INDD2),    
     +                WORK(INDWORK),LLWORK)
*        Distribute E
          CALL PELARED1D(N+IOFFE,IA,JA,DESCA,WORK(INDE),WORK(INDE2),    
     +                WORK(INDWORK),LLWORK)
      END IF
*
...
*
      IF (WANTU.EQ.1) THEN
          CALL PELASET('Full',M,SIZE,ZERO,ONE,WORK(INDU),1,1,DESCTU)
      ELSE
          NRU = 0
      END IF
*
      IF (WANTVT.EQ.1) THEN
          CALL PELASET('Full',SIZE,N,ZERO,ONE,WORK(INDV),1,1,DESCTVT)
      ELSE
          NCVT = 0
      END IF
*
      CALL EBDSQR(UPLO,SIZE,NCVT,NRU,0,WORK(INDD2+IOFFD),             
     +WORK(INDE2+IOFFE),WORK(INDV),SIZE,WORK(INDU),LDU,C,1,             
     +WORK(INDWORK),INFO)
*
*     Redistribute elements of U and VT in the block-cyclic fashion.
*
      IF (WANTU.EQ.1) CALL PEGEMR2D(M,SIZE,WORK(INDU),1,1,DESCTU,U,IU,  
     +                             JU,DESCU,DESCU(CTXT_))
*
      IF (WANTVT.EQ.1) CALL PEGEMR2D(SIZE,N,WORK(INDV),1,1,DESCTVT,VT,  
     +                              IVT,JVT,DESCVT,DESCVT(CTXT_))
*
*     Set to ZERO "non-square" elements of the larger matrices U, VT.
*
      IF (M.GT.N .AND. WANTU.EQ.1) THEN
          CALL PELASET('Full',M-SIZE,SIZE,ZERO,ZERO,U,IA+SIZE,JU,DESCU)
      ELSE IF (N.GT.M .AND. WANTVT.EQ.1) THEN
          CALL PELASET('Full',SIZE,N-SIZE,ZERO,ZERO,VT,IVT,JVT+SIZE,    
     +              DESCVT)
      END IF
*
*     Multiply Householder rotations from bidiagonalized matrix.
*
      IF (WANTU.EQ.1) CALL PEORMBR('Q','L','N',M,SIZE,N,A,IA,JA,DESCA,  
     +                            WORK(INDTAUQ),U,IU,JU,DESCU,          
     +                    WORK(INDWORK),LLWORK,INFO)
*
      IF (WANTVT.EQ.1) CALL PEORMBR('P','R','T',SIZE,N,M,A,IA,JA,DESCA, 
...
*
   ...
   20 CONTINUE
*
      CALL EGAMN2D(DESCA(CTXT_),'a',' ',J,1,WORK(1+INDE),J,1,1,-1,-1,0)
      CALL EGAMX2D(DESCA(CTXT_),'a',' ',J,1,WORK(1+INDD2),J,1,1,-1,-1,0)
*
      DO 30 I = 1,J
