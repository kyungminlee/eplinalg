      SUBROUTINE PELASRT( ID, N, D, Q, IQ, JQ, DESCQ, WORK, LWORK,      
     +                IWORK, LIWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PELASRT Sort the numbers in D in increasing order and the
...
*
*  D       (global input/output) REAL(KIND=10) array, dimmension (N)
*          On exit, the number in D are sorted in increasing order.
*
*  Q       (local input) REAL(KIND=10) pointer into the local memory
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PXERBLA, ECOPY,       
     +             EGERV2D, EGESD2D, ELAMOV, ELAPST
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PELASRT', -INFO )
         RETURN
      END IF
*
*     Set Pointers
...
*
      CALL ELAPST( 'I', N, D, IWORK( INDX ), INFO )
*
      DO 10 L = 0, N - 1
...
*
   ...
   60       CONTINUE
            DO 70 L = 0, LEND - 1
               ...
               CALL ECOPY( NP, Q( IPQ ), 1, WORK( IPWORK ), 1 )
   ...
   80       CONTINUE
            DO 90 L = 0, K - 1
               ...
               CALL ECOPY( NP, WORK( IPWORK ), 1, WORK( IPQ ), 1 )
   ...
   90       CONTINUE
            DO 100 CL = 1, NPCOL - 1
               ...
               IF( SBUF.NE.0 ) THEN
                  ...
                  CALL EGESD2D( DESCQ( CTXT_ ), NP, SBUF,               
...
*
            IF( K.NE.0 ) THEN
               CALL EGERV2D( DESCQ( CTXT_ ), NP, K, WORK( IPW ), NP,    
     +                    MYROW, QCOL )
               DO 110 L = 0, K - 1
  ...
  110          CONTINUE
         ...
         GO TO 20
      END IF
      CALL ELAMOV( 'Full', NP, NQ, WORK, NP, Q( IIQ ), LDQ )
*
*     End of PELASRT
*
      END
