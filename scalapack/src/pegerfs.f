      SUBROUTINE PEGERFS( TRANS, N, NRHS, A, IA, JA, DESCA, AF, IAF,    
...
*  =======
*
*  PEGERFS improves the computed solution to a system of linear
...
*
*  A       (local input) REAL(KIND=10) pointer into the local
...
*
*  AF      (local input) REAL(KIND=10) pointer into the local
...
*          by PEGETRF. IPIV(i) -> The global row local row i
*          was swapped with. This array is tied to the distributed
*          matrix A.
*
*  B       (local input) REAL(KIND=10) pointer into the local
...
*
*  X       (local input and output) REAL(KIND=10) pointer into the
...
*
*  FERR    (local output) REAL(KIND=10) array of local dimension
*          LOCc(JB+NRHS-1).
...
*
*  BERR    (local output) REAL(KIND=10) array of local dimension
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      ...
      REAL(KIND=10)   TWO, THREE
      PARAMETER          ( TWO = 2.0E0_10, THREE = 3.0E0_10 )
*     ..
*     .. Local Scalars ..
     ...
     $                   MYROW, NP, NP0, NPCOL, NPMOD, NPROW, NZ
      REAL(KIND=10)   EPS, EST, LSTRES, S, SAFE1, SAFE2, SAFMIN
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCSET, EGAMX2D,     
     +               EGEBR2D, EGEBS2D, INFOG2L, PCHK2MAT,               
     +     PEAGEMV, PEAXPY, PECOPY, PEGEMV,                    PEGETRS, 
     +PELACON, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                  IX, JX, DESCX, 20, 5, IDUM1, IDUM2, INFO )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGERFS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*        depending on TRANS.
*
         CALL PECOPY( N, B, IB, JB+K, DESCB, 1, WORK( IPR ), IW, JW,    
     +             DESCW, 1 )
         CALL PEGEMV( TRANS, N, N, -ONE, A, IA, JA, DESCA, X, IX,       
...
*
   ...
   30          CONTINUE
         ...
         END IF
*
         CALL PEAGEMV( TRANS, N, N, ONE, A, IA, JA, DESCA, X, IX, JX+K, 
...
*
   ...
   40          CONTINUE
         ...
         END IF
*
         CALL EGAMX2D( ICTXT, 'All', ' ', 1, 1, S, 1, IDUM, IDUM, 1,    
     +              -1, MYCOL )
         IF( MYCOL.EQ.IXBCOL )
     $      BERR( JJFBE ) = S
*
...
*
            CALL PEGETRS( TRANS, N, 1, AF, IAF, JAF, DESCAF, IPIV,      
     +               WORK( IPR ), IW, JW, DESCW, INFO )
            CALL PEAXPY( N, ONE, WORK( IPR ), IW, JW, DESCW, 1, X, IX,  
     +                  JX+K, DESCX, 1 )
            LSTRES = S
            COUNT = COUNT + 1
            GO TO 20
...
*        SAFE2.
*
*        Use PELACON to estimate the infinity-norm of the matrix
...
*
   ...
   60    CONTINUE
         IF( MYCOL.EQ.IXBCOL ) THEN
            CALL EGEBS2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IPR ),    
     +                 DESCW( LLD_ ) )
         ELSE
            CALL EGEBR2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IPR ),    
     +                 DESCW( LLD_ ), MYROW, IXBCOL )
         END IF
         DESCW( CSRC_ ) = MYCOL
         CALL PELACON( N, WORK( IPV ), IW, JW, DESCW, WORK( IPR ),      
...
*
               CALL PEGETRS( TRANST, N, 1, AF, IAF, JAF, DESCAF,        
...
*
   ...
   90          CONTINUE
            END IF
            CALL EGAMX2D( ICTXT, 'Column', ' ', 1, 1, LSTRES, 1, IDUM,  
     +                   IDUM, 1, -1, MYCOL )
            IF( LSTRES.NE.ZERO )
...
*           depending on TRANS.
*
            CALL PECOPY( N, B, IB, J+K, DESCB, 1, WORK( IPR ), IW, JW,  
     +                  DESCW, 1 )
            CALL PEGEMV( TRANS, N, N, -ONE, A, IA, JA, DESCA, X,        
...
*
  ...
  120             CONTINUE
            ...
            END IF
*
            CALL PEAGEMV( TRANS, N, N, ONE, A, IA, JA, DESCA, X, IX,    
...
*
               CALL PEGETRS( TRANS, N, 1, AF, IAF, JAF, DESCAF, IPIV,   
     +                     WORK( IPR ), IW, JW, DESCW, INFO )
               CALL PEAXPY( N, ONE, WORK( IPR ), IW, JW, DESCW, 1, X,   
     +                    IX, J+K, DESCX, 1 )
               LSTRES = S
               COUNT = COUNT + 1
               GO TO 110
...
*
  ...
  170                   CONTINUE
                  ...
                  END IF
*
                  CALL PEGETRS( TRANS, N, 1, AF, IAF, JAF, DESCAF,      
...
*
  ...
  180             CONTINUE
               END IF
               CALL EGAMX2D( ICTXT, 'Column', ' ', 1, 1, LSTRES,        
