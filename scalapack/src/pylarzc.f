      SUBROUTINE PYLARZC( SIDE, M, N, L, V, IV, JV, DESCV, INCV, TAU, C,
     +                     IC, JC, DESCC, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLARZC applies a complex elementary reflector Q**H to a
...
*        with offset IROFFC2), since PBYTRNV transposes V into a column
*        at ICROW2 and YGEMV/YGERC below act on sub(C2). The upstream
*        MPV (line ~288) computed from V's distribution only coincides
*        when the alignment restriction holds, which PYUNMR3 may
...
*        since the col-V SIDE='R' PBYTRNV transposes V into a row at
...
*                    pairs of YAXPY calls in PYLARZC that target
*                    WORK / WORK(IPW) used INCY = MAX(1,NQC2) (a leading
*                    dimension) where WORK is a contiguous vector — the
*                    correct stride is 1, matching LAPACK's YLARZ. Same
*                    fix applied in pylarz.f / pelarz.f overrides.
