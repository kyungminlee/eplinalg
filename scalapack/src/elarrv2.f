      SUBROUTINE ELARRV2( N, VL, VU, D, L, PIVMIN,                    
     +ISPLIT, M, DOL, DOU, NEEDIL, NEEDIU,                    MINRGP, 
     +RTOL1, RTOL2, W, WERR, WGAP,                    IBLOCK, INDEXW, 
     +GERS, SDIAM,                     Z, LDZ, ISUPPZ,                  
     +  WORK, IWORK, VSTART, FINISH,                     MAXCLS, NDEPTH,
     + PARITY, ZOFFSET, INFO )
...
*     .. Scalar Arguments ..
     ...
     $                   NDEPTH, NEEDIL, NEEDIU, PARITY, ZOFFSET
      REAL(KIND=10)   MINRGP, PIVMIN, RTOL1, RTOL2, VL, VU
      LOGICAL VSTART, FINISH 
*     ..
*     .. Array Arguments ..
      INTEGER            IBLOCK( * ), INDEXW( * ), ISPLIT( * ),
     $                   ISUPPZ( * ), IWORK( * )
      REAL(KIND=10)   D( * ), GERS( * ), L( * ), SDIAM( * ),            
     +         W( * ), WERR( * ),                    WGAP( * ), WORK( * 
     +)
      REAL(KIND=10)  Z( LDZ, * )
*
...
*  =======
*
*  ELARRV2 computes the eigenvectors of the tridiagonal matrix
*  T = L D L^T given L, D and APPROXIMATIONS to the eigenvalues of L D L^T.
*  The input eigenvalues should have been computed by ELARRE2A
*  or by precious calls to ELARRV2.
*
...
*     ELARRV2  ONLY computes the eigenVECTORS 
...
*
*  3. ELARRV2 needs more workspace in Z than the sequential ELARRV. 
...
*
*  L       (input/output) REAL(KIND=10) array, dimension (N)
*          On entry, the (N-1) subdiagonal elements of the unit
*          bidiagonal matrix L are in elements 1 to N-1 of L 
*          (if the matrix is not splitted.) At the end of each block
*          is stored the corresponding shift as given by ELARRE.
*          On exit, L is overwritten.
*
*  PIVMIN  (in) REAL(KIND=10)
...
*
*  GERS    (input) REAL(KIND=10) array, dimension (2*N)
...
*
*  SDIAM   (input) REAL(KIND=10) array, dimension (N)
...
*          ISUPPZ( 2*I ).
*
*  WORK    (workspace) REAL(KIND=10) array, dimension (12*N)
*
...
*          = 0:  successful exit
*
*          > 0:  A problem occured in ELARRV2.
*          < 0:  One of the called subroutines signaled an internal problem. 
*                Needs inspection of the corresponding parameter IINFO
*                for further information.
*
*          =-1:  Problem in ELARRB2 when refining a child's eigenvalues.
*          =-2:  Problem in ELARRF2 when computing the RRR of a child.
...
*          =-3:  Problem in ELARRB2 when refining a single eigenvalue
...
*     .. Parameters ..
     ...
     $                     USE32A=3210, USE32B = 3211 )
      REAL(KIND=10)   ZERO, ONE, TWO, THREE, FOUR, HALF
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10,             
     +          TWO = 2.0E0_10, THREE = 3.0E0_10,                      
     +FOUR = 4.0E0_10, HALF = 0.5E0_10)
*     ..
*     .. Local Arrays ..
      INTEGER            SPLACE( 4 )
*     ..
*     .. Local Scalars ..
     ...
     $                   ZUSEDL, ZUSEDU, ZUSEDW
      REAL(KIND=10)   AVGAP, BSTRES, BSTW, ENUFGP, EPS, FUDGE, GAP,     
...
*     .. External Functions ..
      ...
      EXTERNAL           EDOT, ELAMCH, ENRM2
*     ..
*     .. External Subroutines ..
      EXTERNAL           EAXPY, ECOPY, ELAR1VA, ELARRB2,                
     +    ELARRF2, ELASET, ESCAL
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL ELASET( 'Full', N, ZUSEDW, ZERO, ZERO,                    
...
*        Compute ORTOL parameter, similar to ESTEIN
         ORTOL = SPDIAM*1.0E-3_10
*        Compute average gap
         AVGAP = SPDIAM/REAL(IN-1, KIND=10)
...
*           reset NCLUS to count the number of child clusters 
            NCLUS = 0
*
            LOG_IN = LOG(REAL(IN, KIND=10))
*
            RLTL30 = MIN( 1.0E-2_10, ONE / REAL( IN , KIND=10) )
*
            IF( PARITY.EQ.0 ) THEN
...
*                       Get representation from the right end of Z array 
                     ...
                     ENDIF
                  ENDIF
                  CALL ECOPY( IN, Z( IBEGIN, (J-ZOFFSET) ),             
     +    1, D( IBEGIN ), 1 )
                  CALL ECOPY( IN-1, Z( IBEGIN, (J+1-ZOFFSET) ),         
     +        1, L( IBEGIN ),1 )
                  SIGMA = Z( IEND, (J+1-ZOFFSET) )
*                 Set the corresponding entries in Z to zero
                  CALL ELASET( 'Full', IN, 2, ZERO, ZERO,               
...
*                 eigenvalues to the precision needed.
                  CALL ELARRB2( IN, D( IBEGIN ),                        
     +   WORK(INDLLD+IBEGIN-1),                          P, Q, RTOL1, 
     +RTOL2, OFFSET,                           WORK(WBEGIN),
     +WGAP(WBEGIN),WERR(WBEGIN),                          WORK( INDWRK 
     +), IWORK( IINDWK ),                          PIVMIN, LGPVMN, 
     +LGSPDM, IN, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     INFO = -1
                     RETURN
...
*                 WBEGIN-1+OLDLST are correctly computed in ELARRB2.
...
*
     ...
     +RQTOL, OFFSET,                         WORK(WBEGIN),WGAP(WBEGIN), 
...
*
C                    ELARRF2 needs LWORK = 2*N
                     CALL ELARRF2( IN, D( IBEGIN ), L( IBEGIN ),        
     +                  WORK(INDLD+IBEGIN-1),                           
     +SPLACE(1), SPLACE(2),                           SPLACE(3), 
     +SPLACE(4), WORK(WBEGIN),                          WGAP(WBEGIN), 
     +WERR(WBEGIN), TRYMID,                          SPDIAM, LGAP, RGAP,
     + PIVMIN, TAU,                           Z( IBEGIN, NEWFTT ),      
     +                    Z( IBEGIN, NEWFTT+1 ),                        
     +  WORK( INDWRK ), IINFO )
                     IF( IINFO.EQ.0 ) THEN
*                       a new RRR for the cluster was found by ELARRF2
...
*                          eigenvalues to the precision needed.
                           CALL ELARRB2( IN,                           
     +Z(IBEGIN, NEWFTT ),                          
     +WORK(INDWRK+IBEGIN-1),                          P, Q, RTOL1, 
     +RTOL2, OFFSET,                           WORK(WBEGIN),
     +WGAP(WBEGIN),WERR(WBEGIN),                          WORK( 
     +INDWRK+N ), IWORK( IINDWK ),                          PIVMIN, 
...
*                    Given LAMBDA, compute the eigenvector.
                     CALL ELAR1VA( IN, 1, IN, LAMBDA, D(IBEGIN),        
     +             L( IBEGIN ), WORK(INDLD+IBEGIN-1),                   
     +   WORK(INDLLD+IBEGIN-1),                     PIVMIN, GAPTOL, Z( 
     +IBEGIN, ZINDEX),                     .NOT.USEDBS, NEGCNT, ZTZ, 
     +MINGMA,                     ITWIST, ISUPPZ( 2*WINDEX-1 ),         
