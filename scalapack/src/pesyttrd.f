      SUBROUTINE PESYTTRD( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,  
     +                    LWORK, INFO )
      IMPLICIT NONE
*
...
*     =======
*
*     PESYTTRD reduces a complex Hermitian matrix sub( A ) to Hermitian
...
*
*     A  (local input/local output) REAL(KIND=10) pointer into the
...
*
*     D       (local output) REAL(KIND=10) array, dim LOCq(JA+N-1)
*     The diagonal elements of the tridiagonal matrix T:
*     D(i) = A(i,i). D is tied to the distributed matrix A.
*
*     E       (local output) REAL(KIND=10) array, dim LOCq(JA+N-1)
...
*
*     TAU     (local output) REAL(KIND=10) array, dimension
...
*         ANB = PJLAENV_EP( DESCA( CTXT_ ), 3, 'PESYTTRD', 'L', 0, 0,
*           0, 0 )
*
...
*     =========================
*
*     PESYTTRD is not intended to be called directly.  All users are
*     encourage to call PESYTRD which will then call PDHETTRD if
...
*     ===============
*
*     PESYTTRD uses five local arrays:
...
*     .. Parameters ..
     ...
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      ...
      REAL(KIND=10)   Z_ONE, Z_NEGONE, Z_ZERO
...
*
     ...
     $                   PBMIN, PBSIZE, PNB, ROWSPERPROC
      REAL(KIND=10)   ALPHA, BETA, C, CONJTOPH, CONJTOPV, NORM,         
...
*
      ...
      REAL(KIND=10)   CC( 3 ), DTMP( 5 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, ECOMBNRM2, EGEBR2D,   
     +                 EGEBS2D, EGEMM, EGEMV, EGERV2D, EGESD2D,         
     +           EGSUM2D, ELAMOV, ESCAL, ETRMVT, PCHK1MAT,              
     +      PETREECOMB, PXERBLA
*     ..
*     .. External Functions ..
*
      ...
      REAL(KIND=10)   ENRM2, PELAMCH
      EXTERNAL           LSAME, ICEIL, NUMROC, PJLAENV_EP, ENRM2, 
     +PELAMCH
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ICHAR, MAX, MIN, MOD, SIGN, SQRT
*     ..
*
*
...
*
      ...
      ELSE
*
*     Here we set execution options for PESYTTRD
*
         PNB = PJLAENV_EP( ICTXT, 2, 'PESYTTRD', 'L', 0, 0, 0, 0 )
         ANB = PJLAENV_EP( ICTXT, 3, 'PESYTTRD', 'L', 0, 0, 0, 0 )
*
         INTERLEAVE = ( PJLAENV_EP( ICTXT, 4, 'PESYTTRD', 'L', 1, 0, 0, 
     +                0 ).EQ.1 )
         TWOGEMMS = ( PJLAENV_EP( ICTXT, 4, 'PESYTTRD', 'L', 2, 0, 0,   
     +            0 ).EQ.1 )
         BALANCED = ( PJLAENV_EP( ICTXT, 4, 'PESYTTRD', 'L', 3, 0, 0,   
     +            0 ).EQ.1 )
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYTTRD', -INFO )
         RETURN
      END IF
*
...
*
               ...
               END IF
*
               NORM = ENRM2( NPM1, A( LIIP1+( LIJ-1 )*LDA ), 1 )
               DTMP( 1 ) = NORM
*
...
*
            IF( MYCOL.EQ.CURCOL ) THEN
               ...
               CALL EGEBS2D( ICTXT, 'R', ' ', NPM1+NPM1+1, 1,           
     +             WORK( INV+LIIP1-1+BINDEX*LDV ),                      
     +  NPM1+NPM1+1 )
            ELSE
               CALL EGEBR2D( ICTXT, 'R', ' ', NPM1+NPM1+1, 1,           
     +             WORK( INV+LIIP1-1+BINDEX*LDV ),                      
...
*
   ...
   60          CONTINUE
            ELSE
               CALL EGESD2D( ICTXT, NPM1+NPM1, 1,                       
     + WORK( INV+LIIP1-1+BINDEX*LDV ), NPM1+NPM1,                       
     + MYCOL, MYROW )
               CALL EGERV2D( ICTXT, NQM1+NQM1, 1,                       
...
*           are not always involved in the calls to ETRMVT
*           for two reasons:
...
*
  ...
  100          CONTINUE
            ELSE
               IF( LTNM1.GT.0 )             CALL ETRMVT( 'L', LTNM1, A( 
...
*     tmp = HT * v
*
               CALL EGEMV( 'C', NUMROWS, BINDEX+1, Z_ONE,               
...
*
*
               CALL EGSUM2D( ICTXT, 'C', ' ', 2*( BINDEX+1 ), 1,        
     +                WORK( INTMP ), 2*( BINDEX+1 ), -1, -1 )
            ELSE
*     tmp = HT * v
*
               CALL EGEMV( 'C', NQB, BINDEX+1, Z_ONE, WORK( INHTB ),    
     +                  LDV, WORK( INHTB+( BINDEX+1 )*LDV ), 1,         
     +             Z_ZERO, WORK( INTMP ), 1 )
*     tmp2 = VT * v
               CALL EGEMV( 'C', NQB, BINDEX+1, Z_ONE, WORK( INVTB ),    
     +                  LDV, WORK( INHTB+( BINDEX+1 )*LDV ), 1,         
...
*
               CALL EGSUM2D( ICTXT, 'R', ' ', 2*( BINDEX+1 ), 1,        
     +                WORK( INTMP ), 2*( BINDEX+1 ), -1, -1 )
*
*
*     v = v + V * tmp
               IF( INDEX.GT.1.E0_10 ) THEN
                  CALL EGEMV( 'N', NUMROWS, BINDEX+1, Z_NEGONE,         
...
*
            ELSE
*     v = v + V * tmp
               CALL EGEMV( 'N', NPB, BINDEX+1, Z_NEGONE, WORK( INVB ),  
     +                    LDV, WORK( INTMP ), 1, Z_ONE,                 
     +     WORK( INVB+( BINDEX+1 )*LDV ), 1 )
*
*
*     v = v + H * tmp2
               CALL EGEMV( 'N', NPB, BINDEX+1, Z_NEGONE, WORK( INHB ),  
...
*
  ...
  120          CONTINUE
            ELSE
               CALL EGESD2D( ICTXT, NQM1, 1,                        
     +WORK( INVT+LIJP1-1+( BINDEX+1 )*LDV ),                        
     +NQM1, MYCOL, MYROW )
               CALL EGERV2D( ICTXT, NPM1, 1, WORK( INTMP ), NPM1, MYCOL,
     +                        MYROW )
*
            END IF
            DO 130 I = 0, NPM1 - 1
...
*
  ...
  140          CONTINUE
               ...
               ELSE
                  ...
                  CC( 3 ) = Z_ZERO
               END IF
               CALL EGSUM2D( ICTXT, 'C', ' ', 3, 1, CC, 3, -1, NXTCOL )
*
               TOPV = CC( 2 )
               C = CC( 1 )
               TOPH = CC( 3 )
...
*
            IF( .NOT.TWOGEMMS ) THEN
               IF( INTERLEAVE ) THEN
                  LDZG = LDV / 2
               ELSE
                  CALL ELAMOV( 'A', LTNM1, ANB, WORK( INHT+LIJP1-1 ),   
     +                       LDV, WORK( INVT+LIJP1-1+ANB*LDV ), LDV )
*
                  CALL ELAMOV( 'A', LTNM1, ANB, WORK( INV+LTLIP1-1 ),   
