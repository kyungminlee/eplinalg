      SUBROUTINE PYGEBRD( M, N, A, IA, JA, DESCA, D, E, TAUQ, TAUP,     
     +                WORK, LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGEBRD reduces a complex general M-by-N distributed matrix
...
*     .. External Subroutines ..
     ...
     +                PYELSET, PYGEBD2, PYGEMM, PYLABRD,                
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEBRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*        part of the matrix.
*
         CALL PYLABRD( M-K+1, N-K+1, JB, A, I, J, DESCA, D, E, TAUQ,    
...
*
         CALL PYGEMM( 'No transpose', 'No transpose', M-K-JB+1,         
...
*
     ...
     $                MP )
            IF( JS.GT.0 )          CALL PYELSET( A, I+JB-1, J+JB, DESCA,
     + CMPLX( E( JS ) , KIND=10) )
         ELSE
            JS = MIN( INDXG2L( J+JB-1, NB, 0, DESCA( CSRC_ ), NPCOL ),
     $                NQ )
            IF( JS.GT.0 )          CALL PYELSET( A, I+JB, J+JB-1, DESCA,
     + CMPLX( E( JS ) , KIND=10) )
         END IF
*
         K = K + JB
...
*
      CALL PYGEBD2( M-K+1, N-K+1, A, IA+K-1, JA+K-1, DESCA, D, E, TAUQ, 
