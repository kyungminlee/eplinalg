      SUBROUTINE PELATRD( UPLO, N, NB, A, IA, JA, DESCA, D, E, TAU, W,  
     +                   IW, JW, DESCW, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELATRD reduces NB rows and columns of a real symmetric distributed
...
*
*  If UPLO = 'U', PELATRD reduces the last NB rows and columns of a
*  matrix, of which the upper triangle is supplied;
*  if UPLO = 'L', PELATRD reduces the first NB rows and columns of a
*  matrix, of which the lower triangle is supplied.
*
*  This is an auxiliary routine called by PESYTRD.
*
*  Notes
*  =====
*
...
*
*  D       (local output) REAL(KIND=10) array, dimension LOCc(JA+N-1)
*          The diagonal elements of the tridiagonal matrix T:
*          D(i) = A(i,i). D is tied to the distributed matrix A.
*
*  E       (local output) REAL(KIND=10) array, dimension LOCc(JA+N-1)
...
*          matrix A.
*
*  W       (local output) REAL(KIND=10) pointer into the local memory
...
*
*  WORK    (local workspace) REAL(KIND=10) array, dimension (NB_A)
*
*  Further Details
*  ===============
*
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      REAL(KIND=10)   HALF, ONE, ZERO
      PARAMETER          ( HALF = 0.5E0_10, ONE = 1.0E0_10, ZERO = 
     +0.0E0_10 )
*     ..
*     .. Local Scalars ..
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DESCSET, EGEBR2D, EGEBS2D,     
     +               INFOG2L, PEAXPY, PEDOT, PEELGET,                   
     + PEELSET, PEGEMV, PELARFG, PESCAL,                    PESYMV
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*           Update A(IA:I,I)
*
            CALL PEGEMV( 'No transpose', K, N-K, -ONE, A, IA, J+1,      
     +              DESCA, W, IW+K-1, JW+KW, DESCW, DESCW( M_ ),        
     +            ONE, A, IA, J, DESCA, 1 )
            CALL PEGEMV( 'No transpose', K, N-K, -ONE, W, IW, JW+KW,    
     +                DESCW, A, I, J+1, DESCA, DESCA( M_ ), ONE, A,     
     +               IA, J, DESCA, 1 )
            IF( N-K.GT.0 )          CALL PEELSET( A, I, J+1, DESCA, E( 
     +JP ) )
*
...
*
            JP = MIN( JJ+KW-1, NQ )
            CALL PELARFG( K-1, E( JP ), I-1, J, A, IA, J, DESCA, 1,     
     +                TAU )
            CALL PEELSET( A, I-1, J, DESCA, ONE )
*
*           Compute W(IW:IW+K-2,JW+KW-1)
*
            CALL PESYMV( 'Upper', K-1, ONE, A, IA, JA, DESCA, A, IA, J, 
     +                   DESCA, 1, ZERO, W, IW, JW+KW-1, DESCW, 1 )
*
            JWK = MOD( K-1, DESCWK( NB_ ) ) + 2
            CALL PEGEMV( 'Transpose', K-1, N-K, ONE, W, IW, JW+KW,      
     ...
     +                    DESCWK( M_ ) )
            CALL PEGEMV( 'No transpose', K-1, N-K, -ONE, W, IW, JW+KW,  
     +                  DESCW, WORK, 1, JWK, DESCWK, DESCWK( M_ ), ONE, 
     +                   W, IW, JW+KW-1, DESCW, 1 )
            CALL PESCAL( K-1, TAU( JP ), W, IW, JW+KW-1, DESCW, 1 )
*
            CALL PEDOT( K-1, ALPHA, W, IW, JW+KW-1, DESCW, 1, A, IA, J, 
     +                  DESCA, 1 )
            IF( MYCOL.EQ.IACOL )
     $         ALPHA = -HALF*TAU( JP )*ALPHA
            CALL PEAXPY( K-1, ALPHA, A, IA, J, DESCA, 1, W, IW, JW+KW-1,
     +                    DESCW, 1 )
            IF( MYCOL.EQ.IACOL ) THEN
               CALL PEELGET( 'E', ' ', D( JP ), A, I, J, DESCA )
            END IF
*
   10    CONTINUE
*
...
*           Update A(J:JA+N-1,J)
*
            CALL PEGEMV( 'No transpose', N-K+1, K-1, -ONE, A, I, JA,    
     +                DESCA, W, IW+K-1, JW, DESCW, DESCW( M_ ), ONE,    
     +                A, I, J, DESCA, 1 )
            CALL PEGEMV( 'No transpose', N-K+1, K-1, -ONE, W, IW+K-1,   
     +                 JW, DESCW, A, I, JA, DESCA, DESCA( M_ ), ONE,    
     +                A, I, J, DESCA, 1 )
            IF( K.GT.1 )          CALL PEELSET( A, I, J-1, DESCA, E( JP 
     +) )
*
*
*           Generate elementary reflector H(i) to annihilate
...
*
            JP = MIN( JJ+K-1, NQ )
            CALL PELARFG( N-K, E( JP ), I+1, J, A, I+2, J, DESCA, 1,    
     +                 TAU )
            CALL PEELSET( A, I+1, J, DESCA, ONE )
*
*           Compute W(IW+K:IW+N-1,JW+K-1)
*
            CALL PESYMV( 'Lower', N-K, ONE, A, I+1, J+1, DESCA, A, I+1, 
     +                   J, DESCA, 1, ZERO, W, IW+K, JW+K-1, DESCW, 1 )
*
            CALL PEGEMV( 'Transpose', N-K, K-1, ONE, W, IW+K, JW, DESCW,
     +                    A, I+1, J, DESCA, 1, ZERO, WORK, 1, 1, DESCWK,
     +                    DESCWK( M_ ) )
            CALL PEGEMV( 'No transpose', N-K, K-1, -ONE, A, I+1, JA,    
     +               DESCA, WORK, 1, 1, DESCWK, DESCWK( M_ ), ONE, W,   
     +                IW+K, JW+K-1, DESCW, 1 )
            CALL PEGEMV( 'Transpose', N-K, K-1, ONE, A, I+1, JA, DESCA, 
     +                  A, I+1, J, DESCA, 1, ZERO, WORK, 1, 1, DESCWK,  
     +                 DESCWK( M_ ) )
            CALL PEGEMV( 'No transpose', N-K, K-1, -ONE, W, IW+K, JW,   
     +                DESCW, WORK, 1, 1, DESCWK, DESCWK( M_ ), ONE, W,  
     +                 IW+K, JW+K-1, DESCW, 1 )
            CALL PESCAL( N-K, TAU( JP ), W, IW+K, JW+K-1, DESCW, 1 )
            CALL PEDOT( N-K, ALPHA, W, IW+K, JW+K-1, DESCW, 1, A, I+1,  
     +                 J, DESCA, 1 )
            IF( MYCOL.EQ.IACOL )
     $         ALPHA = -HALF*TAU( JP )*ALPHA
            CALL PEAXPY( N-K, ALPHA, A, I+1, J, DESCA, 1, W, IW+K,      
     +              JW+K-1, DESCW, 1 )
            IF( MYCOL.EQ.IACOL ) THEN
               CALL PEELGET( 'E', ' ', D( JP ), A, I, J, DESCA )
            END IF
