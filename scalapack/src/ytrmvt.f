      SUBROUTINE YTRMVT( UPLO, N, T, LDT, X, INCX, Y, INCY, W, INCW, Z, 
     +                   INCZ )
      IMPLICIT NONE
*
...
*  =======
*
*  YTRMVT  performs the matrix-vector operations
*
...
*           Unchanged on exit.
*
*  T      - COMPLEX(KIND=10) array of DIMENSION ( LDT, n ).
...
*           Unchanged on exit.
*
*  X      - COMPLEX(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCX ) ).
*           On exit, X = T' * y
*
*  INCX   - INTEGER.
...
*           Unchanged on exit.
*
*  Y      - COMPLEX(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCY ) ).
...
*           Unchanged on exit.
*
*  W      - COMPLEX(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCW ) ).
*           On exit, W = T * z
*
*  INCW   - INTEGER.
...
*           Unchanged on exit.
*
*  Z      - COMPLEX(KIND=10) array of dimension at least
*           ( 1 + ( n - 1 )*abs( INCZ ) ).
...
*
     ...
     $    .TRUE. ) THEN
         CALL YCOPY( N, Y, INCY, X, INCX )
         CALL YTRMV( UPLO, 'C', 'N', N, T, LDT, X, INCX )
         CALL YCOPY( N, Z, INCZ, W, INCW )
         CALL YTRMV( UPLO, 'N', 'N', N, T, LDT, W, INCW )
         RETURN
      END IF
*
      RETURN
