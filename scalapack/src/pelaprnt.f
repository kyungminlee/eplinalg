      SUBROUTINE PELAPRNT( M, N, A, IA, JA, DESCA, IRPRNT, ICPRNT,      
     +                CMATNM, NOUT, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELAPRNT prints to the standard output a distributed matrix sub( A )
...
*
*  A       (local input) REAL(KIND=10) pointer into the local memory to a
   ...
   10          CONTINUE
         ...
         ELSE
            IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
               CALL EGESD2D( ICTXT, IB, 1, A( II+(JJ+H-1)*LDA ), LDA,   
     +                     IRPRNT, ICPRNT )
            ELSE IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
               CALL EGERV2D( ICTXT, IB, 1, WORK, DESCA( MB_ ),          
     +              ICURROW, ICURCOL )
               DO 20 K = 1, IB
                  WRITE( NOUT, FMT = 9999 )
...
*
   ...
   30             CONTINUE
            ...
            ELSE
               IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
                  CALL EGESD2D( ICTXT, IB, 1, A( II+(JJ+H-1)*LDA ),     
     +                      LDA, IRPRNT, ICPRNT )
               ELSE IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                  CALL EGERV2D( ICTXT, IB, 1, WORK, DESCA( MB_ ),       
     +                    ICURROW, ICURCOL )
