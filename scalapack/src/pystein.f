      SUBROUTINE PYSTEIN( N, D, E, M, W, IBLOCK, ISPLIT, ORFAC, Z, IZ,  
...
*  =======
*
*  PYSTEIN computes the eigenvectors of a symmetric tridiagonal matrix
*  in parallel, using inverse iteration. The eigenvectors found
*  correspond to user specified eigenvalues. PYSTEIN does not
...
*  process. PYSTEIN decides on the allocation of work among the
...
*          iterations (as in YSTEIN), then INFO > 0 is returned.
...
*     .. External Subroutines ..
     ...
     +               IGEBS2D, PCHK1MAT, PYLAEVSWP, PXERBLA, EGEBR2D,    
     +                EGEBS2D, ELASRT2, ESTEIN2
*     ..
*     .. Parameters ..
...
*     .. Local Scalars ..
     ...
     $                   OLNBLK, P, ROW, SELF, TILL, TOTERR
      REAL(KIND=10)               DIFF, MINGAP, ONENRM, ORGFAC, ORTOL, 
     +TMPFAC
*     ..
*     .. Local Arrays ..
...
*
     ...
     +KIND=10)
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCZ( CTXT_ ), 'PYSTEIN', -INFO )
         RETURN
...
*
*
      CALL PYLAEVSWP( N, WORK( INDRW+1 ), N, Z, IZ, JZ, DESCZ, IWORK,   
