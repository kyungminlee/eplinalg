      SUBROUTINE PYPBTRS( UPLO, N, BW, NRHS, A, JA, DESCA, B, IB, DESCB,
...
*  =======
*
*  PYPBTRS solves a system of linear equations
*
*            A(1:N, JA:JA+N-1) * X = B(IB:IB+N-1, 1:NRHS)
*
*  where A(1:N, JA:JA+N-1) is the matrix used to produce the factors
*  stored in A(1:N,JA:JA+N-1) and AF by PYPBTRF.
...
*  A(1:N, JA:JA+N-1) = U'*U or L*L' as computed by PYPBTRF.
*
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,      
     + 'PYPBTRS, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*BW )) THEN
         INFO = -( 7*100+4 )
         CALL PXERBLA( ICTXT,       'PYPBTRS, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -14
         CALL PXERBLA( ICTXT,       'PYPBTRS: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPBTRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PYPBTRSV( 'L', 'N', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      ELSE
*
         CALL PYPBTRSV( 'U', 'C', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      ENDIF
...
*
      IF( LSAME( UPLO, 'L' ) ) THEN
*
         CALL PYPBTRSV( 'L', 'C', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      ELSE
*
         CALL PYPBTRSV( 'U', 'N', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      ENDIF
