      SUBROUTINE PYGEQR2( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGEQR2 computes a QR factorization of a complex distributed M-by-N
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEQR2', -INFO )
...
*
     ...
     $                   NPCOL )
            ...
            IF( MYCOL.EQ.IACOL ) THEN
               AJJ = A( I )
               CALL YLARFG( 1, AJJ, A( I ), 1, TAU( JJ ) )
               IF( N.GT.1 ) THEN
                  ALPHA = ONE - CONJG( TAU( JJ ) )
                  CALL YGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA, 1 )
                  CALL YSCAL( NQ-JJ, ALPHA, A( I+DESCA( LLD_ ) ),       
     +                  DESCA( LLD_ ) )
               END IF
               CALL YGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, TAU( JJ ), 
     +                       1 )
               A( I ) = AJJ
            ELSE
               IF( N.GT.1 ) THEN
     ...
     +                      1, IAROW, IACOL )
         ...
         ELSE IF( MYCOL.EQ.IACOL ) THEN
            CALL YGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAU( JJ ), 1, 
     +                    IAROW, IACOL )
         END IF
*
      ELSE
...
*
               CALL PYLARFC( 'Left', M-J+JA, N-J+JA-1, A, I, J, DESCA,  
