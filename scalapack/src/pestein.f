      SUBROUTINE PESTEIN( N, D, E, M, W, IBLOCK, ISPLIT, ORFAC, Z, IZ,  
...
*  =======
*
*  PESTEIN computes the eigenvectors of a symmetric tridiagonal matrix
*  in parallel, using inverse iteration. The eigenvectors found
*  correspond to user specified eigenvalues. PESTEIN does not
...
*  process. PESTEIN decides on the allocation of work among the
*  processes and then calls ESTEIN2 (modified LAPACK routine) on each
...
*
*  W       (global input/global output) REAL(KIND=10) array, dim (M)
...
*          W from PESTEBZ with ORDER='b' is expected here). This
...
*          ELAMCH('U') --- ABSTOL is an input parameter
*          to PESTEBZ )
*
...
*          ISPLIT from PESTEBZ is expected here.)
*
...
*
*  WORK    (local workspace/global output) REAL(KIND=10) array,
*          dimension ( LWORK )
...
*          orthogonality is similar to that obtained from ESTEIN2).
...
*          iterations (as in ESTEIN), then INFO > 0 is returned.
...
*
*  GAP     (global output) REAL(KIND=10) array, dimension (P)
...
*     .. External Subroutines ..
     ...
     +               ELASRT2, ESTEIN2, IGAMN2D, IGEBR2D, IGEBS2D,       
     +             PCHK1MAT, PELAEVSWP, PXERBLA
*     ..
*     .. Parameters ..
     ...
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      REAL(KIND=10)   ZERO, NEGONE, ODM1, FIVE, ODM3, ODM18
      PARAMETER          ( ZERO = 0.0E0_10, NEGONE = -1.0E0_10,         
     +           ODM1 = 1.0E-1_10, FIVE = 5.0E0_10, ODM3 = 1.0E-3_10,   
     +                 ODM18 = 1.0E-18_10 )
*     ..
*     .. Local Scalars ..
     ...
     $                   OLNBLK, P, ROW, SELF, TILL, TOTERR
      REAL(KIND=10)   DIFF, MINGAP, ONENRM, ORGFAC, ORTOL, TMPFAC
*     ..
*     .. Local Arrays ..
...
*
     ...
     $         MAXVEC = ( LLWORK-INDRW ) / N
            ...
            IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
               TMPFAC = ORFAC
               CALL EGEBS2D( DESCZ( CTXT_ ), 'ALL', ' ', 1, 1, TMPFAC,  
     +                      1 )
            ELSE
               CALL EGEBR2D( DESCZ( CTXT_ ), 'ALL', ' ', 1, 1, TMPFAC,  
     +                      1, 0, 0 )
            END IF
*
...
*
     ...
     +KIND=10)
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCZ( CTXT_ ), 'PESTEIN', -INFO )
         RETURN
...
*
  ...
  150 CONTINUE
      INFO = ( K-1 )*( M+1 )
*
*     Call ESTEIN2 to find the eigenvectors
*
      CALL ESTEIN2( N, D, E, IM, W( IWORK( SELF+1 )+1 ),               
     +IBLOCK( IWORK( SELF+1 )+1 ), ISPLIT, ORGFAC,               WORK( 
     +INDRW+1 ), N, WORK, IWORK( P+2 ),               IFAIL( IWORK( 
     +SELF+1 )+1 ), LOCINFO )
*
...
*
  ...
  160 CONTINUE
*
      CALL ELASRT2( 'I', M, W, IWORK( P+2 ), IINFO )
*
      DO 170 I = 1, M
...
*
*
      CALL PELAEVSWP( N, WORK( INDRW+1 ), N, Z, IZ, JZ, DESCZ, IWORK,   
