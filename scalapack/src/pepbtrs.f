      SUBROUTINE PEPBTRS( UPLO, N, BW, NRHS, A, JA, DESCA, B, IB, DESCB,
...
*  =======
*
*  PEPBTRS solves a system of linear equations
*
*            A(1:N, JA:JA+N-1) * X = B(IB:IB+N-1, 1:NRHS)
*
*  where A(1:N, JA:JA+N-1) is the matrix used to produce the factors
*  stored in A(1:N,JA:JA+N-1) and AF by PEPBTRF.
...
*  A(1:N, JA:JA+N-1) = U'*U or L*L' as computed by PEPBTRF.
*
...
*     .. External Subroutines ..
     ...
     +                  GLOBCHK, PEPBTRSV, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     + 'PEPBTRS, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*BW ) ) THEN
         INFO = -( 7*100+4 )
         CALL PXERBLA( ICTXT, 'PEPBTRS, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -14
            CALL PXERBLA( ICTXT, 'PEPBTRS: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPBTRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PEPBTRSV( 'L', 'N', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      ELSE
*
         CALL PEPBTRSV( 'U', 'T', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      END IF
...
*
      IF( LSAME( UPLO, 'L' ) ) THEN
*
         CALL PEPBTRSV( 'L', 'T', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      ELSE
*
         CALL PEPBTRSV( 'U', 'N', N, BW, NRHS, A( PART_OFFSET+1 ),      
     +             JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,        
     +           WORK, LWORK, INFO )
*
      END IF
