      SUBROUTINE PYROW2COL( ICTXT, M, N, NB, VS, LDVS, VD, LDVD,        
...
*     ..
*     .. Array Arguments ..
      COMPLEX(KIND=10)            VD( LDVD, * ), VS( LDVS, * ), WORK( * 
     +)
*     ..
*
*  Purpose
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGESD2D, YGERV2D, YLACPY
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC
...
*
                  DO 10 II = ISTART, MQ, NB*RBLKSKIP
                     JB = MIN( NB, MQ-II+1 )
                     CALL YLACPY( 'G', JB, N, VS(II,1), LDVS,           
     +                  WORK(JJ), JB )
                     JJ = JJ + NB*N
   10             CONTINUE
*
...
*
                  JJ = JJ - 1
                  IF( JJ.GT.0 )                CALL YGESD2D( ICTXT, JJ, 
...
*
                  ...
                  IF( JJ.GT.0 )                CALL YGERV2D( ICTXT, JJ, 
...
*
                  ...
                  DO 30 II = ISTART, MP, NB*CBLKSKIP
                     JB = MIN( NB, MP-II+1 )
                     CALL YLACPY( 'G', JB, N, WORK(JJ), JB, VD(II,1),   
     +                          LDVD )
                     JJ = JJ + NB*N
   30             CONTINUE
*
...
*
               ELSE
                  ...
                  DO 40 II = ISTART, MP, NB*CBLKSKIP
                     JB = MIN( NB, MP-II+1 )
                     CALL YLACPY( 'G', JB, N, VS(JJ,1), LDVS, VD(II,1), 
     +                            LDVD )
                     JJ = JJ + NB*RBLKSKIP
   40             CONTINUE
               END IF
...
*
            ...
            IRDEST = MOD( RDEST+MYDIST, NPROW )
            IF( (MYROW.NE.IRDEST).OR.(MYCOL.NE.CDEST) ) THEN
               CALL YGESD2D( ICTXT, MQ, N, VS, LDVS, IRDEST, CDEST )
            ELSE
               CALL YLACPY( 'G', MQ, N, VS, LDVS, VD, LDVD )
            END IF
         END IF
         IF( MYCOL.EQ.CDEST ) THEN
*
...
*
     ...
     +YGERV2D( ICTXT, MP, N, VD, LDVD, RSRC, ICSRC )
         END IF
      END IF
*
      RETURN
