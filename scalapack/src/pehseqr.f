      SUBROUTINE PEHSEQR( JOB, COMPZ, N, ILO, IHI, H, DESCH, WR, WI, Z, 
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   H( * ), WI( N ), WORK( * ), WR( N ), Z( * )
*     ..
*  Purpose
*  =======
*
*  PEHSEQR computes the eigenvalues of an upper Hessenberg matrix H
...
*          set by a previous call to PEGEBAL, and then passed to PEGEHRD
*          when the matrix output by PEGEBAL is reduced to Hessenberg
*          form. Otherwise ILO and IHI should be set to 1 and N
*          respectively.  If N.GT.0, then 1.LE.ILO.LE.IHI.LE.N.
*          If N = 0, then ILO = 1 and IHI = 0.
*
*  H       (global input/output) REAL(KIND=10) array, dimension
*          (DESCH(LLD_),*)
...
*          matrix Z of accumulated transformations from, e.g., PEGEHRD,
...
*          .GT. 0:  if INFO = i, PEHSEQR failed to compute all of
...
*                accessed.
*
*          = -7777: PELAQR0 failed to converge and PELAQR1 was called
*                   instead. This could happen. Mostly due to a bug.
*                   Please, send a bug report to the authors.
*          = -8888: PELAQR1 failed to converge and PELAQR0 was called
...
*     than or equal to six (6) due to restrictions in PELAQR1, PELAQR5
*     and ELAQR6. Moreover, H and Z need to be distributed identically
...
*     .. Local Scalars ..
     ...
     $                   HRSRC4, HCSRC4, LIWKOPT
      ...
      REAL(KIND=10)   TMP1, TMP2, TMP3, TMP4, DUM1, DUM2, DUM3,         
     +           DUM4, ELEM1, ELEM4,                    CS, SN, ELEM5, 
     +TMP, LWKOPT
*     ..
*     .. Local Arrays ..
      INTEGER            DESCH2( DLEN_ )
      REAL(KIND=10)   ELEM2( 1 ), ELEM3( 1 )
*     ..
*     .. External Functions ..
      INTEGER            PILAENVX_EP, NUMROC, ICEIL
      LOGICAL            LSAME
      EXTERNAL           PILAENVX_EP, LSAME, NUMROC, ICEIL
*     ..
*     .. External Subroutines ..
      EXTERNAL           PELACPY, PELAQR1, PELAQR0, PELASET, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      CALL PELAQR1( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,      
     +ILO, IHI, Z, DESCZ, WORK, -1, IWORK, -1, INFO )
      LWKOPT = WORK(1)
      LIWKOPT = IWORK(1)
      CALL PELAQR0( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,      
     +ILO, IHI, Z, DESCZ, WORK, -1, IWORK, -1, INFO, 0 )
      IF( N.LT.NL ) THEN
         ...
         WORK(1) = WORK(1) + REAL(2*HROWS*HCOLS, KIND=10)
      END IF
...
*
         CALL PXERBLA( ICTXT, 'PEHSEQR', -INFO )
         RETURN
*
      ELSE IF( N.EQ.0 ) THEN
*
...
*
      ELSE
*
*        Copy eigenvalues isolated by PEGEBAL.
*
         DO 10 I = 1, ILO - 1
     ...
     $           JJ, HRSRC, HCSRC )
            ...
            WI( I ) = ZERO
   10    CONTINUE
         IF( ILO.GT.1 )       CALL EGSUM2D( ICTXT, 'All', '1-Tree', 
     +ILO-1, 1, WR, N, -1,            -1 )
         DO 20 I = IHI + 1, N
            CALL INFOG2L( I, I, DESCH, NPROW, NPCOL, MYROW, MYCOL, II,
     $           JJ, HRSRC, HCSRC )
            ...
            WI( I ) = ZERO
   20    CONTINUE
         IF( IHI.LT.N )       CALL EGSUM2D( ICTXT, 'All', '1-Tree', 
     +N-IHI, 1, WR(IHI+1),            N, -1, -1 )
*
*        Initialize Z, if requested.
*
         IF( INITZ )       CALL PELASET( 'A', N, N, ZERO, ONE, Z, 1, 1, 
     +DESCZ )
*
...
*
     ...
     $           MYCOL, II, JJ, HRSRC, HCSRC )
            IF( MYROW.EQ.HRSRC .AND. MYCOL.EQ.HCSRC ) THEN
               ...
               IF( NPROCS.GT.1 )             CALL EGEBS2D( ICTXT, 'All',
     + '1-Tree', 1, 1, WR(ILO),                  1 )
            ELSE
               CALL EGEBR2D( ICTXT, 'All', '1-Tree', 1, 1, WR(ILO),     
     +          1, HRSRC, HCSRC )
            END IF
            WI( ILO ) = ZERO
            RETURN
         END IF
*
*        PELAQR1/PELAQR0 crossover point.
*
         NH = IHI-ILO+1
         NMIN = PILAENVX_EP( ICTXT, 12, 'PEHSEQR',         JOB( : 1 ) 
     +// COMPZ( : 1 ), N, ILO, IHI, LWORK )
         NMIN = MAX( NTINY, NMIN )
*
*        PELAQR0 for big matrices; PELAQR1 for small ones.
*
...
*
*              A rare PELAQR0 failure!  PELAQR1 sometimes succeeds
*              when PELAQR0 fails.
*
               KBOT = INFO
               CALL PELAQR1( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR,   
...
*
            IF( INFO.GT.0 ) THEN
*
*              A rare PELAQR1 failure!  PELAQR0 sometimes succeeds
*              when PELAQR1 fails.
*
               KBOT = INFO
*
...
*                 space to call PELAQR0 directly.
*
                  CALL PELAQR0( WANTT, WANTZ, N, ILO, KBOT, H, DESCH,   
     +               WR, WI, ILO, IHI, Z, DESCZ, WORK, LWORK,           
     +       IWORK, LIWORK, INFO, 0 )
               ELSE
*
*                 Tiny matrices don't have enough subdiagonal
*                 scratch space to benefit from PELAQR0.  Hence,
*                 tiny matrices must be copied into a larger
*                 array before calling PELAQR0.
*
                  HROWS = NUMROC( NL, NB, MYROW, DESCH(RSRC_), NPROW )
                  HCOLS = NUMROC( NL, NB, MYCOL, DESCH(CSRC_), NPCOL )
                  CALL DESCINIT( DESCH2, NL, NL, NB, NB, DESCH(RSRC_),
     $                 DESCH(CSRC_), ICTXT, MAX(1, HROWS), INFO )
                  CALL PELACPY( 'All', N, N, H, 1, 1, DESCH, WORK, 1,   
     +               1, DESCH2 )
                  CALL PEELSET( WORK, N+1, N, DESCH2, ZERO )
                  CALL PELASET( 'All', NL, NL-N, ZERO, ZERO, WORK, 1,   
     +               N+1, DESCH2 )
                  IPW = 1 + DESCH2(LLD_)*HCOLS
                  CALL PELAQR0( WANTT, WANTZ, NL, ILO, KBOT, WORK,      
     +            DESCH2, WR, WI, ILO, IHI, Z, DESCZ,                  
     +WORK(IPW), LWORK-IPW+1, IWORK,                  LIWORK, INFO, 0 )
                  IF( WANTT .OR. INFO.NE.0 )                CALL 
     +PELACPY( 'All', N, N, WORK, 1, 1, DESCH2,                     H, 
     +1, 1, DESCH )
               END IF
               INFO = -8888
            END IF
...
*
         IF( ( WANTT .OR. INFO.NE.0 ) .AND. N.GT.2 )       CALL 
     +PELASET( 'L', N-2, N-2, ZERO, ZERO, H, 3, 1, DESCH )
*
*        Force any 2-by-2 blocks to be complex conjugate pairs of
*        eigenvalues by removing false such blocks.
*
         DO 30 I = ILO, IHI-1
            CALL PEELGET( 'All', ' ', TMP3, H, I+1, I, DESCH )
            IF( TMP3.NE.0.0E00_10 ) THEN
               CALL PEELGET( 'All', ' ', TMP1, H, I, I, DESCH )
               CALL PEELGET( 'All', ' ', TMP2, H, I, I+1, DESCH )
               CALL PEELGET( 'All', ' ', TMP4, H, I+1, I+1, DESCH )
               CALL ELANV2( TMP1, TMP2, TMP3, TMP4, DUM1, DUM2, DUM3,   
     +            DUM4, CS, SN )
               IF( TMP3.EQ.0.0E00_10 ) THEN
                  IF( WANTT ) THEN
                     IF( I+2.LE.N )                   CALL PEROT( N-I-1,
     + H, I, I+2, DESCH,                        DESCH(M_), H, I+1, I+2, 
     +DESCH, DESCH(M_),                        CS, SN, WORK, LWORK, 
     +INFO )
                     CALL PEROT( I-1, H, 1, I, DESCH, 1, H, 1, I+1,     
     +                DESCH, 1, CS, SN, WORK, LWORK, INFO )
                  END IF
                  IF( WANTZ ) THEN
                     CALL PEROT( N, Z, 1, I, DESCZ, 1, Z, 1, I+1, DESCZ,
     +                     1, CS, SN, WORK, LWORK, INFO )
                  END IF
                  CALL PEELSET( H, I, I, DESCH, TMP1 )
                  CALL PEELSET( H, I, I+1, DESCH, TMP2 )
                  CALL PEELSET( H, I+1, I, DESCH, TMP3 )
                  CALL PEELSET( H, I+1, I+1, DESCH, TMP4 )
               END IF
            END IF
 30      CONTINUE
*
     ...
     $                 MYCOL, ILOC1, JLOC1, HRSRC1, HCSRC1 )
                  IF( MYROW.EQ.HRSRC1 .AND. MYCOL.EQ.HCSRC1 ) THEN
                     ...
                     IF( ELEM3( 1 ).NE.ZERO ) THEN
                        ...
                        CALL ELANV2( ELEM1, ELEM2( 1 ), ELEM3( 1 ),     
...
*
     ...
     +CALL EGESD2D( ICTXT, 2, 1, WORK, 2, HRSRC1, HCSRC1 )
            END IF
            IF( MYROW.EQ.HRSRC1 .AND. MYCOL.EQ.HCSRC1 ) THEN
               ELEM1 = H((JLOC1-1)*LLDH+ILOC1)
               IF( HRSRC1.NE.HRSRC2 .OR. HCSRC1.NE.HCSRC2 )             
     +CALL EGERV2D( ICTXT, 1, 1, ELEM2, 1, HRSRC2, HCSRC2)
               IF( HRSRC1.NE.HRSRC3 .OR. HCSRC1.NE.HCSRC3 )             
     +CALL EGERV2D( ICTXT, 1, 1, ELEM3, 1, HRSRC3, HCSRC3)
               IF( HRSRC1.NE.HRSRC4 .OR. HCSRC1.NE.HCSRC4 )             
     +CALL EGERV2D( ICTXT, 2, 1, WORK, 2, HRSRC4, HCSRC4 )
               ELEM4 = WORK(1)
               ELEM5 = WORK(2)
               IF( ELEM5.EQ.ZERO ) THEN
                  IF( WR( K ).EQ.ZERO .AND. WI( K ).EQ.ZERO ) THEN
                     CALL ELANV2( ELEM1, ELEM2( 1 ), ELEM3( 1 ), ELEM4, 
...
*
         IF( NPROCS.GT.1 ) THEN
            CALL EGSUM2D( ICTXT, 'All', ' ', IHI-ILO+1, 1, WR(ILO), N,  
     +          -1, -1 )
            CALL EGSUM2D( ICTXT, 'All', ' ', IHI-ILO+1, 1, WI(ILO), N,  
     +          -1, -1 )
         END IF
*
      END IF
