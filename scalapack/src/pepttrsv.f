      SUBROUTINE PEPTTRSV( UPLO, N, NRHS, D, E, JA, DESCA, B, IB, DESCB,
...
*  =======
*
*  PEPTTRSV solves a tridiagonal triangular system of linear equations
*
...
*  Cholesky factorization code PEPTTRF
...
*
*  Routine PEPTTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*
*  E       (local input/local output) REAL(KIND=10) pointer to local
...
*          PEPTTRF and this is stored in AF. If a linear system
*          is to be solved using PEPTTRS after the factorization
...
*     .. External Subroutines ..
     ...
     +           DESC_CONVERT, EGEMM, EGERV2D, EGESD2D, EMATADD,        
     +            EPTTRSV, ETRTRS, GLOBCHK, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,                  
     +'PEPTTRSV, D&C alg.: only 1 block per proc',                  
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 7*100+4 )
         CALL PXERBLA( ICTXT, 'PEPTTRSV, D&C alg.: NB too small',       
     +           -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -14
            CALL PXERBLA( ICTXT, 'PEPTTRSV: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPTTRSV', -INFO )
         RETURN
      END IF
*
...
*
         CALL EPTTRSV( 'N', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),         
...
*
            CALL EAXPY( NRHS, -E( PART_OFFSET+ODD_SIZE ),               
...
*
            CALL EGEMM( 'T', 'N', 1, NRHS, ODD_SIZE, -ONE, AF( 1 ),     
...
*
         IF( MYCOL.GT.0 ) THEN
*
            CALL EGESD2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE, 0,  
     +                   MYCOL-1 )
*
         END IF
*
...
*
            CALL EGERV2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE, 0,  
     +                   MYCOL+1 )
*
*         Combine contribution to locally stored right hand sides
*
            CALL EMATADD( INT_ONE, NRHS, ONE, WORK( 1 ), INT_ONE, ONE,  
...
*
         CALL ETRTRS( 'L', 'N', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),   
...
*
     ...
     +          AF( ( ODD_SIZE )*1+1 ), INT_ONE,                   B( 
     +PART_OFFSET+ODD_SIZE+1 ), LLDB, ZERO,                   WORK( 1 ),
     + INT_ONE )
*
...
*       Solve with diagonal block
*
         CALL ETRTRS( 'L', 'T', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),   
     +              INT_ONE, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
         IF( INFO.NE.0 ) THEN
            GO TO 90
...
*
         IF( MYCOL+LEVEL_DIST.LT.NPCOL-1 ) THEN
*
            CALL EGESD2D( ICTXT, INT_ONE, NRHS,                     B( 
...
*
            CALL EGEMM( 'N', 'N', ODD_SIZE, NRHS, 1, -ONE, AF( 1 ),     
...
*       Use main partition in each processor to solve locally
*
         CALL EPTTRSV( 'T', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),         
     +         E( PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,            
     +      INFO )
*
*
