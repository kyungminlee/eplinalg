      SUBROUTINE PYGETRI( N, A, IA, JA, DESCA, IPIV, WORK, LWORK,       
     +              IWORK, LIWORK, INFO )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      ...
      COMPLEX(KIND=10)            A( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PYGETRI computes the inverse of a distributed matrix using the LU
*  factorization computed by PYGETRF. This method inverts U and then
...
*          factorization sub( A ) = P*L*U computed by PYGETRF. On
...
*     .. External Subroutines ..
     ...
     +                PYGEMM, PYLACPY, PYLASET, PYLAPIV,                
     +    PYTRSM, PYTRTRI, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGETRI', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $   RETURN
*
*     Form inv(U).  If INFO > 0 from PYTRTRI, then U is singular,
*     and the inverse is not computed.
*
      CALL PYTRTRI( 'Upper', 'Non-unit', N, A, IA, JA, DESCA, INFO )
      IF( INFO.GT.0 )
     $   RETURN
*
...
*
         CALL PYLACPY( 'Lower', JA+N-1-J, JB, A, I+1, J, DESCA,         
     +         WORK, IW+J-JA+1, 1, DESCW )
         CALL PYLASET( 'Lower', JA+N-1-J, JB, ZERO, ZERO, A, I+1, J,    
     +              DESCA )
*
*        Compute current block column of inv(A).
*
         IF( J+JB.LE.JA+N-1 )       CALL PYGEMM( 'No transpose', 
     +'No transpose', N, JB,                    JA+N-J-JB, -ONE, A, IA, 
     +J+JB, DESCA, WORK,                    IW+J+JB-JA, 1, DESCW, ONE, 
     +A, IA, J, DESCA )
         CALL PYTRSM( 'Right', 'Lower', 'No transpose', 'Unit', N, JB,  
...
*     Copy current block column of L to WORK and replace with zeros.
*
      CALL PYLACPY( 'Lower', N-1, JB, A, IA+1, JA, DESCA, WORK, IW+1,   
     +            1, DESCW )
      CALL PYLASET( 'Lower', N-1, JB, ZERO, ZERO, A, IA+1, JA, DESCA )
*
*     Compute current block column of inv(A).
*
      IF( JA+JB.LE.JA+N-1 )    CALL PYGEMM( 'No transpose', 
...
*
     ...
     $              MP+DESCA( MB_ ) )
      CALL PYLAPIV( 'Backward', 'Columns', 'Column', N, N, A, IA,       
