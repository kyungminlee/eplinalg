      SUBROUTINE PELARED1D( N, IA, JA, DESC, BYCOL, BYALL, WORK, LWORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESC( * )
      REAL(KIND=10)   BYALL( * ), BYCOL( * ), WORK( LWORK )
*     ..
*
*  Purpose
*  =======
*
*  PELARED1D redistributes a 1D array
*
...
*
*  BYCOL   (local input) distributed block cyclic REAL(KIND=10) array
...
*
*  BYALL   (global output) REAL(KIND=10) global dimension( N )
*          local dimension (N)
...
*
*  WORK    (local workspace) REAL(KIND=10) dimension (LWORK)
...
*
      DO 30 PCOL = 0, NPCOL - 1
         ...
         IF( MYCOL.EQ.PCOL ) THEN
            CALL ECOPY( BUFLEN, BYCOL, 1, WORK, 1 )
            CALL EGEBS2D( DESC( CTXT_ ), 'R', ' ', 1, BUFLEN, WORK, 1 )
         ELSE
            CALL EGEBR2D( DESC( CTXT_ ), 'R', ' ', 1, BUFLEN, WORK, 1,  
     +                   MYROW, PCOL )
         END IF
*
         ALLI = PCOL*NB
