      SUBROUTINE PEGBTRS( TRANS, N, BWL, BWU, NRHS, A, JA, DESCA, IPIV, 
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   A( * ), AF( * ), B( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PEGBTRS solves a system of linear equations
*
...
*  stored in A(1:N,JA:JA+N-1) and AF by PEGBTRF.
*  A(1:N, JA:JA+N-1) is an N-by-N real
*  banded distributed
*  matrix with bandwidth BWL, BWU.
*
*  Routine PEGBTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*
*  B       (local input/local output) REAL(KIND=10) pointer into
...
*          PEGBTRF and this is stored in AF. If a linear system
*          is to be solved using PEGBTRS after the factorization
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO, ECOPY,         
     +           DESC_CONVERT, EGEMM, EGEMV, EGER, EGERV2D,             
     +       EGESD2D, EGETRS, ELAMOV, ELASWP, ESCAL, ESWAP,             
     +       ETRSM, GLOBCHK, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     + 'PEGBTRS, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.( BWL+BWU+1 ) ) ) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT, 'PEGBTRS, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -16
            CALL PXERBLA( ICTXT, 'PEGBTRS: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGBTRS', -INFO )
         RETURN
      END IF
*
...
*
      IF( MYCOL.LT.NPCOL-1 ) THEN
         CALL EGESD2D( ICTXT, BWU, NRHS, B( NB-BWU+1 ), LLDB, 0,        
     +          MYCOL+1 )
      END IF
*
      IF( MYCOL.LT.NPCOL-1 ) THEN
...
*
      LDW = NB + BWU + 2*BW + BWU
*
      CALL ELAMOV( 'G', LM, NRHS, B( 1 ), LLDB, WORK( WPTR ), LDW )
*
...
*
      IF( MYCOL.GT.0 ) THEN
         CALL EGERV2D( ICTXT, BWU, NRHS, WORK( 1 ), LDW, 0, MYCOL-1 )
      END IF
*
********************************************************************
...
*
         IF( L.NE.J ) THEN
            CALL ESWAP( NRHS, WORK( L ), LDW, WORK( J ), LDW )
         END IF
*
         LPTR = BW + 1 + ( J-1 )*LLDA + APTR
*
         CALL EGER( LMJ, NRHS, -ONE, A( LPTR ), 1, WORK( J ), LDW,      
     +         WORK( J+1 ), LDW )
*
   30 CONTINUE
*
...
*        performed.
         CALL EGETRS( 'N', N-LN, NRHS, AF( BBPTR+BW*LDBB ), LDBB,       
...
*
     ...
     $                  BWU
               END IF
*
               CALL EGESD2D( ICTXT, BM, NRHS, WORK( LN+1 ), LDW, 0,     
     +                   NEICOL )
*
               IF( NPACT.NE.2 ) THEN
*
...
*
                  CALL EGERV2D( ICTXT, BM+BMN-BW, NRHS, WORK( LN+1 ),   
     +                        LDW, 0, NEICOL )
*
                  BM = BM + BMN - BW
*
...
*
            ...
            END IF
*
            CALL ELAMOV( 'G', BM, NRHS, WORK( LN+1 ), LDW,              
     +      WORK( NB+BWU+BMN+1 ), LDW )
*
            CALL EGERV2D( ICTXT, BMN, NRHS, WORK( NB+BWU+1 ), LDW, 0,   
     +                  NEICOL )
*
...
*
               CALL ELASWP( NRHS, WORK( NB+BWU+1 ), LDW, 1, BW,         
     +              IPIV( LN+1 ), 1 )
*
               CALL ETRSM( 'L', 'L', 'N', 'U', BW, NRHS, ONE,           
     +           AF( BBPTR+BW*LDBB ), LDBB, WORK( NB+BWU+1 ),           
     +           LDW )
*
*                  Use soln just calculated to update RHS
*
               CALL EGEMM( 'N', 'N', BM+BMN-BW, NRHS, BW, -ONE,         
     +             AF( BBPTR+BW*LDBB+BW ), LDBB,                      
     +WORK( NB+BWU+1 ), LDW, ONE,                      WORK( 
     +NB+BWU+1+BW ), LDW )
*
*                  Give answers back to partner processor
*
               CALL EGESD2D( ICTXT, BM+BMN-BW, NRHS,                    
...
*
               CALL ELASWP( NRHS, WORK( NB+BWU+1 ), LDW, 1, BM+BMN,     
     +                  IPIV( LN+1 ), 1 )
*
               CALL ETRSM( 'L', 'L', 'N', 'U', BM+BMN, NRHS, ONE,       
     +               AF( BBPTR+BW*LDBB ), LDBB, WORK( NB+BWU+1 ),       
     +               LDW )
            END IF
*
...
*
            IF( NPACT.GT.2 ) THEN
*
               CALL EGESD2D( ICTXT, 2*BW, NRHS, WORK( LN+1 ), LDW, 0,   
     +                     NEICOL )
*
               CALL EGERV2D( ICTXT, BW, NRHS, WORK( LN+1 ), LDW, 0,     
     +                   NEICOL )
*
            ELSE
*
...
*
            CALL ELAMOV( 'G', BW, NRHS, WORK( NB+BWU+1 ), LDW,          
     +          WORK( NB+BWU+BW+1 ), LDW )
*
            CALL EGERV2D( ICTXT, 2*BW, NRHS, WORK( LN+1 ), LDW, 0,      
     +               NEICOL )
*
            CALL EGEMM( 'N', 'N', BW, NRHS, BN, -ONE, AF( BBPTR ), LDBB,
...
*
            CALL ETRSM( 'L', 'U', 'N', 'N', BN+BNN, NRHS, ONE,          
     +         AF( BBPTR+BW*LDBB ), LDBB, WORK( NB+BWU+1 ),             
     +      LDW )
*
*              Send new solution to neighbor
*
            CALL EGESD2D( ICTXT, BW, NRHS, WORK( NB+BWU+1 ), LDW, 0,    
     +                 NEICOL )
*
*              Shift solutions into expected positions
*
            CALL ELAMOV( 'G', BNN+BN-BW, NRHS, WORK( NB+BWU+1+BW ), LDW,
     +                    WORK( LN+1 ), LDW )
*
*
...
*
               DO 70 J = 1, BW
                  CALL ECOPY( NRHS, WORK( NB+BWU+J ), LDW,              
     +           WORK( LN+BW+J ), LDW )
   70          CONTINUE
*
            END IF
...
*
      IF( MYCOL.LT.NPCOL-1 ) THEN
*
         CALL EGESD2D( ICTXT, BW, NRHS, WORK( NB-BW+1 ), LDW, 0,        
     +          MYCOL+1 )
*
      END IF
*
      IF( MYCOL.GT.0 ) THEN
*
         CALL EGERV2D( ICTXT, BW, NRHS, WORK( NB+BWU+1 ), LDW, 0,       
     +           MYCOL-1 )
*
*        Modify local right hand sides with received rhs's
*
         CALL EGEMM( 'T', 'N', LM-BM, NRHS, BW, -ONE, AF( 1 ), BW,      
...
*
         CALL EGEMV( 'T', LMJ, NRHS, -ONE, WORK( J+1 ), LDW, A( LPTR ), 
     +               LLDA-1, ONE, WORK( J ), LDW )
*
*        Divide by diagonal element
*
         CALL ESCAL( NRHS, ONE / A( LPTR-LLDA+1 ), WORK( J ), LDW )
   90 CONTINUE
*
*
*
      CALL ELAMOV( 'G', ODD_SIZE, NRHS, WORK( 1 ), LDW, B( 1 ), LLDB )
*
