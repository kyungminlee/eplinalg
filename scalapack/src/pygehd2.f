      SUBROUTINE PYGEHD2( N, ILO, IHI, A, IA, JA, DESCA, TAU, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGEHD2 reduces a complex general distributed matrix sub( A )
...
*     .. External Subroutines ..
     ...
     +                   PYLARF, PYLARFC, PYLARFG, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEHD2', -INFO )
...
*        A(i+2:ihi+ia-1,j)
*
         CALL PYLARFG( IHI-K, AII, I+1, J, A, MIN( I+2, N+IA-1 ), J,    
     +              DESCA, 1, TAU )
         CALL PYELSET( A, I+1, J, DESCA, ONE )
*
*        Apply H(k) to A(ia:ihi+ia-1,j+1:ihi+ja-1) from the right
*
         CALL PYLARF( 'Right', IHI, IHI-K, A, I+1, J, DESCA, 1, TAU, A, 
     +                IA, J+1, DESCA, WORK )
*
*        Apply H(j) to A(i+1:ia+ihi-1,j+1:ja+n-1) from the left
*
         CALL PYLARFC( 'Left', IHI-K, N-K, A, I+1, J, DESCA, 1, TAU, A, 
     +                 I+1, J+1, DESCA, WORK )
*
         CALL PYELSET( A, I+1, J, DESCA, AII )
   10 CONTINUE
*
      WORK( 1 ) = CMPLX( REAL( LWMIN , KIND=10) , KIND=10)
*
