      SUBROUTINE PEGEQRF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEQRF computes a QR factorization of a real distributed M-by-N
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK1MAT, PEGEQR2,    
     +                PELARFB, PELARFT, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEQRF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PEGEQR2( M, JB, A, IA, JA, DESCA, TAU, WORK, LWORK, IINFO )
*
...
*
         CALL PELARFB( 'Left', 'Transpose', 'Forward', 'Columnwise', M, 
...
*        A(i:ia+m-1,j:j+jb-1)
*
         CALL PEGEQR2( M-J+JA, JB, A, I, J, DESCA, TAU, WORK, LWORK,    
     +              IINFO )
*
...
*
            CALL PELARFT( 'Forward', 'Columnwise', M-J+JA, JB, A, I, J, 
     +                    DESCA, TAU, WORK, WORK( IPW ) )
*
*           Apply H' to A(i:ia+m-1,j+jb:ja+n-1) from the left
*
            CALL PELARFB( 'Left', 'Transpose', 'Forward', 'Columnwise', 
