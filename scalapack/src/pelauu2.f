      SUBROUTINE PELAUU2( UPLO, N, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*  =======
*
*  PELAUU2 computes the product U * U' or L' * L, where the triangular
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, EGEMV, ESCAL
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      REAL(KIND=10)   EDOT
...
*
            DO 10 NA = N-1, 1, -1
               AII = A( IDIAG )
               ICURR = IDIAG + LDA
               A( IDIAG ) = AII*AII + EDOT( NA, A( ICURR ), LDA,        
     +                             A( ICURR ), LDA )
               CALL EGEMV( 'No transpose', N-NA-1, NA, ONE,             
...
*
            DO 20 NA = 1, N-1
               AII = A( IDIAG )
               ICURR = IDIAG + 1
               A(IDIAG) = AII*AII + EDOT( N-NA, A( ICURR ), 1,          
     +                           A( ICURR ), 1 )
               CALL EGEMV( 'Transpose', N-NA, NA-1, ONE, A( IOFFA+1 ),  
     +                    LDA, A( ICURR ), 1, AII, A( IOFFA ), LDA )
               IDIAG = IDIAG + LDA + 1
               IOFFA = IOFFA + 1
   20       CONTINUE
            AII = A( IDIAG )
            CALL ESCAL( N, AII, A( IOFFA ), LDA )
*
         END IF
*
      END IF
