      SUBROUTINE PEDTTRSV( UPLO, TRANS, N, NRHS, DL, D, DU, JA, DESCA,  
...
*  =======
*
*  PEDTTRSV solves a tridiagonal triangular system of linear equations
*
...
*
*  Routine PEDTTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO, EAXPY, EDTTRSV,
     +                    DESC_CONVERT, EGEMM, EGERV2D, EGESD2D, 
     +EMATADD,                    ETBTRS, GLOBCHK, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 3 )
         CALL PXERBLA( ICTXT,                  
     +'PEDTTRSV, D&C alg.: only 1 block per proc',                  
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 9*100+4 )
         CALL PXERBLA( ICTXT, 'PEDTTRSV, D&C alg.: NB too small',       
     +           -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -16
            CALL PXERBLA( ICTXT, 'PEDTTRSV: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEDTTRSV', -INFO )
         RETURN
      END IF
*
...
*
            CALL EDTTRSV( UPLO, 'N', ODD_SIZE, NRHS,                    
     + DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),                     DU( 
     +PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,                     
     +INFO )
*
*
            IF( MYCOL.LT.NP-1 ) THEN
...
*
               CALL EAXPY( NRHS, -DL( PART_OFFSET+ODD_SIZE+1 ),         
...
*
               CALL EGEMM( 'T', 'N', INT_ONE, NRHS, ODD_SIZE, -ONE,     
...
*
            IF( MYCOL.GT.0 ) THEN
*
               CALL EGESD2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE,  
     +                      0, MYCOL-1 )
*
            END IF
*
...
*
               CALL EGERV2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE,  
     +                      0, MYCOL+1 )
*
*         Combine contribution to locally stored right hand sides
*
               CALL EMATADD( INT_ONE, NRHS, ONE, WORK( 1 ), INT_ONE,    
...
*
            CALL ETBTRS( 'L', 'N', 'U', INT_ONE,                    
     +MIN( INT_ONE, INT_ONE-1 ), NRHS,                    AF( 
     +ODD_SIZE+2 ), INT_ONE+1,                    B( 
...
*
               CALL EGEMM( 'T', 'N', INT_ONE, NRHS, INT_ONE, -ONE,      
...
*
               CALL EGEMM( 'N', 'N', INT_ONE, NRHS, INT_ONE, -ONE,      
...
*
            IF( MYCOL+LEVEL_DIST.LT.NPCOL-1 ) THEN
*
               CALL EGESD2D( ICTXT, INT_ONE, NRHS,                      
...
*
               CALL EGEMM( 'N', 'N', ODD_SIZE, NRHS, INT_ONE, -ONE,     
...
*       Use main partition in each processor to solve locally
*
            CALL EDTTRSV( UPLO, 'T', ODD_SIZE, NRHS,                    
     + DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),                     DU( 
     +PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,                     
     +INFO )
*
...
*           locally stored portion of right hand side(s)
*
               CALL EAXPY( NRHS, -( DU( PART_OFFSET+ODD_SIZE ) ),       
     +               B( PART_OFFSET+ODD_SIZE ), LLDB,                   
     +   B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
            END IF
...
*
     ...
     +WORK( 1+INT_ONE-INT_ONE ), INT_ONE, ONE,                      B( 
     +PART_OFFSET+1 ), LLDB )
*
            END IF
*
...
*
            CALL EDTTRSV( UPLO, 'N', ODD_SIZE, NRHS,                    
     + DU( PART_OFFSET+2 ), D( PART_OFFSET+1 ),                     DU( 
     +PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,                     
     +INFO )
*
         END IF
