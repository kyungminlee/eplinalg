      REAL(KIND=10)   FUNCTION PELANTR( NORM, UPLO, DIAG, M, N, A,      
     +                                IA, JA, DESCA, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELANTR returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PELANTR as described
*          above.
*
...
*          of the distributed submatrix sub( A ). When M = 0, PELANTR is
*          set to zero. M >= 0.
*
*  N       (global input) INTEGER
*          The number of columns to be operated on i.e the number of
*          columns of the distributed submatrix sub( A ). When N = 0,
*          PELANTR is set to zero. N >= 0.
*
...
*     .. Local Scalars ..
     ...
     $                   MYCOL, MYROW, NP, NPCOL, NPROW, NQ
      REAL(KIND=10)   SUM, VALUE
*     ..
*     .. Local Arrays ..
      REAL(KIND=10)   SSQ( 2 ), COLSSQ( 2 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, ECOMBSSQ, EGEBR2D,             
     +       EGEBS2D, EGAMX2D, EGSUM2D, ELASSQ,                    
     +INFOG2L, PETREECOMB
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, IEAMAX, NUMROC
      EXTERNAL           LSAME, ICEIL, IEAMAX, NUMROC
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL EGAMX2D( ICTXT, 'All', ' ', 1, 1, VALUE, 1, KK, LL, -1,   
     +               0, 0 )
*
************************************************************************
* one norm
...
*        process grid
*
         CALL EGSUM2D( ICTXT, 'Columnwise', ' ', 1, NQ, WORK, 1,        
     +          0, MYCOL )
*
...
*
         IF( MYROW.EQ.0 ) THEN
            IF( NQ.GT.0 ) THEN
               VALUE = WORK( IEAMAX( NQ, WORK, 1 ) )
            ELSE
               VALUE = ZERO
            END IF
            CALL EGAMX2D( ICTXT, 'Rowwise', ' ', 1, 1, VALUE, 1, KK, LL,
     +                     -1, 0, 0 )
         END IF
*
************************************************************************
...
*        process grid
*
         CALL EGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1, WORK, MAX( 1, MP ),
     +                  MYROW, 0 )
*
...
*
         IF( MYCOL.EQ.0 ) THEN
            IF( MP.GT.0 ) THEN
               VALUE = WORK( IEAMAX( MP, WORK, 1 ) )
            ELSE
               VALUE = ZERO
            END IF
            CALL EGAMX2D( ICTXT, 'Columnwise', ' ', 1, 1, VALUE, 1, KK, 
     +                    LL, -1, 0, 0 )
         END IF
*
************************************************************************
...
*
         IF( UDIAG ) THEN
            SSQ(1) = ONE
            SSQ(2) = REAL( MIN( M, N ) , KIND=10) / REAL( NPROW*NPCOL , 
     +KIND=10)
         ELSE
            SSQ(1) = ZERO
            SSQ(2) = ONE
...
*                 including diagonal block.
                  IF( UDIAG ) THEN
                     DO 840 LL = JJ, JJ + JB -1
                        COLSSQ(1) = ZERO
                        COLSSQ(2) = ONE
                        CALL ELASSQ( MIN( II+LL-JJ-1, IIA+MP-1 )-IIA+1, 
     +                               A( IIA+IOFFA ), 1,                 
     +               COLSSQ(1), COLSSQ(2) )
                        CALL ECOMBSSQ( SSQ, COLSSQ )
                        IOFFA = IOFFA + LDA
  840                CONTINUE
                  ELSE
                     DO 850 LL = JJ, JJ + JB -1
                        COLSSQ(1) = ZERO
                        COLSSQ(2) = ONE
                        CALL ELASSQ( MIN( II+LL-JJ, IIA+MP-1 )-IIA+1,   
     +                             A( IIA+IOFFA ), 1,                   
     +             COLSSQ(1), COLSSQ(2) )
                        CALL ECOMBSSQ( SSQ, COLSSQ )
                        IOFFA = IOFFA + LDA
...
*                 It seems this lassq will be length 0, since ii = iia.
                  DO 860 LL = JJ, JJ + JB -1
                     COLSSQ(1) = ZERO
                     COLSSQ(2) = ONE
                     CALL ELASSQ( MIN( II-1, IIA+MP-1 )-IIA+1,          
     +                   A( IIA+IOFFA ), 1,                             
     +COLSSQ(1), COLSSQ(2) )
                     CALL ECOMBSSQ( SSQ, COLSSQ )
                     IOFFA = IOFFA + LDA
  ...
  920                CONTINUE
               ...
               ELSE
                  DO 930 LL = JJ, JJ + JB -1
                     COLSSQ(1) = ZERO
                     COLSSQ(2) = ONE
                     CALL ELASSQ( IIA+MP-II, A( II+IOFFA ), 1,          
     +                   COLSSQ(1), COLSSQ(2) )
                     CALL ECOMBSSQ( SSQ, COLSSQ )
                     IOFFA = IOFFA + LDA
  930             CONTINUE
...
*
         CALL PETREECOMB( ICTXT, 'All', 2, SSQ, 0, 0, ECOMBSSQ )
...
*
      IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
         CALL EGEBS2D( ICTXT, 'All', ' ', 1, 1, VALUE, 1 )
      ELSE
         CALL EGEBR2D( ICTXT, 'All', ' ', 1, 1, VALUE, 1, 0, 0 )
      END IF
*
      PELANTR = VALUE
*
