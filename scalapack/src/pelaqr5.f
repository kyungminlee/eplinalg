      SUBROUTINE PELAQR5( WANTT, WANTZ, KACC22, N, KTOP, KBOT, NSHFTS,  
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   H( * ), SI( * ), SR( * ), Z( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  This auxiliary subroutine called by PELAQR0 performs a
...
*          orthogonal updates.
*     = 1: PELAQR5 accumulates reflections and uses matrix-matrix
*          multiply to update the far-from-diagonal matrix entries.
*     = 2: PELAQR5 accumulates reflections, uses matrix-matrix
*          multiply to update the far-from-diagonal matrix entries,
...
*
*   SR     (global input) REAL(KIND=10) array of size (NSHFTS)
*   SI     (global input) REAL(KIND=10) array of size (NSHFTS)
*          SR contains the real parts and SI contains the imaginary
*          parts of the NSHFTS shifts of origin that define the
*          multi-shift QR sweep.
*
*   H      (local input/output) REAL(KIND=10) array of size 
*          (DESCH(LLD_),*)
...
*
*   Z      (local input/output) REAL(KIND=10) array of size
*          (DESCZ(LLD_),*)
...
*
*   WORK   (local workspace) REAL(KIND=10) array, dimension(DWORK)
*
...
*     ..
*     .. Local Scalars ..
      REAL(KIND=10)   ALPHA, BETA, H11, H12, H21, H22, REFSUM,          
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            PILAENVX_EP, ICEIL, INDXG2P, INDXG2L, NUMROC
      REAL(KIND=10)   ELAMCH, ELANGE
      EXTERNAL           ELAMCH, PILAENVX_EP, ICEIL, INDXG2P, INDXG2L,  
     +                  NUMROC, LSAME, ELANGE
*     ..
*     .. Intrinsic Functions ..
...
*     .. External Subroutines ..
      EXTERNAL           EGEMM, ELABAD, ELAMOV, ELAQR1, ELARFG, ELASET, 
     +                   ETRMM, ELAQR6
*     ..
...
*
      NWIN = PILAENVX_EP( ICTXT, 19, 'PELAQR5', JBCMPZ, N, NB, NB, NB )
...
*     Clear trash.
*
      IF( .NOT. LQUERY .AND. KTOP+2.LE.KBOT )    CALL PEELSET( H, 
     +KTOP+2, KTOP, DESCH, ZERO )
*
...
*
*              Set the kind of job to do in ELAQR6:
...
*              least (NTINY+1)-by-(NTINY+1) to fit into ELAQR6 - if not,
...
*
     ...
     $              DIM.LT.NTINY+1 ) THEN
                  CALL ELASET( 'All', NTINY+1, NTINY+1, ZERO, ONE,      
     +            WORK(IPH), NTINY+1 )
               END IF
               CALL ELAMOV( 'Upper', DIM, DIM, H(II+(JJ-1)*LLDH), LLDH, 
     +              WORK(IPH), MAX(NTINY+1,DIM) )
               CALL ECOPY(  DIM-1, H(II+(JJ-1)*LLDH+1), LLDH+1,         
     +      WORK(IPH+1), MAX(NTINY+1,DIM)+1 )
               IF( LSAME( JOB, 'C' ) .OR. LSAME( JOB, 'O') ) THEN
                  CALL ECOPY(  DIM-2, H(II+(JJ-1)*LLDH+2), LLDH+1,      
     +            WORK(IPH+2), MAX(NTINY+1,DIM)+1 )
                  CALL ECOPY(  DIM-3, H(II+(JJ-1)*LLDH+3), LLDH+1,      
     +            WORK(IPH+3), MAX(NTINY+1,DIM)+1 )
                  CALL ELASET( 'Lower', DIM-4, DIM-4, ZERO,             
     +     ZERO, WORK(IPH+4), MAX(NTINY+1,DIM) )
               ELSE
                  CALL ELASET( 'Lower', DIM-2, DIM-2, ZERO,             
...
*
               ...
               CALL ELASET( 'All', MAX(NTINY+1,DIM),               
...
*
               ...
               CALL ELAQR6( JOB, WANTT, .TRUE., LKACC22,               
     +MAX(NTINY+1,DIM), LLKTOP, LLKBOT, LNS, SR( LKS ),               
     +SI( LKS ), WORK(IPH), MAX(NTINY+1,DIM), LLKTOP,               
     +LLKBOT, WORK(IPUU), MAX(NTINY+1,DIM), WORK(IPU),               3, 
     +WORK( IPH+KU-1 ),               MAX(NTINY+1,DIM), NVE, WORK( 
     +IPH+KWV-1 ),               MAX(NTINY+1,DIM), NHO, WORK( 
     +IPH-1+KU+(KWH-1)*               MAX(NTINY+1,DIM) ), MAX(NTINY+1,
     +DIM) )
*
*              Copy submatrix of H back.
*
               CALL ELAMOV( 'Upper', DIM, DIM, WORK(IPH),               
     +MAX(NTINY+1,DIM), H(II+(JJ-1)*LLDH), LLDH )
               CALL ECOPY( DIM-1, WORK(IPH+1), MAX(NTINY+1,DIM)+1,      
     +         H(II+(JJ-1)*LLDH+1), LLDH+1 )
               IF( LSAME( JOB, 'I' ) .OR. LSAME( JOB, 'C' ) ) THEN
                  CALL ECOPY( DIM-2, WORK(IPH+2), DIM+1,                
     +  H(II+(JJ-1)*LLDH+2), LLDH+1 )
                  CALL ECOPY( DIM-3, WORK(IPH+3), DIM+1,                
...
*
 49      CONTINUE
     ...
     $                    NPCOL.GT.1 ) THEN
                        CALL EGEBS2D( ICTXT, 'Row', '1-Tree', LENRBUF,  
     +                      1, WORK, LENRBUF )
                     ELSEIF( DIR.EQ.2 .AND. LENCBUF.GT.0 .AND.
     $                    NPROW.GT.1 ) THEN
                        CALL EGEBS2D( ICTXT, 'Col', '1-Tree', LENCBUF,  
     +                      1, WORK, LENCBUF )
                     END IF
                     IF( LENRBUF.GT.0 )                   CALL ELAMOV( 
...
*
     ...
     $                          CSRC1 )
                           IF( MYROW.EQ.RSRC1.AND.MYCOL.EQ.CSRC1 ) THEN
                              ...
                              CALL EGEMM('No transpose', 'No transpose',
     +                              LROWS, LNWIN, LNWIN, ONE,           
     +                   H((JLOC-1)*LLDH+ILOC), LLDH,                   
     +           WORK( IPU ), LNWIN, ZERO,                              
     +WORK(IPW),                              LROWS )
                              CALL ELAMOV( 'All', LROWS, LNWIN,         
     +                     WORK(IPW), LROWS,                            
     +  H((JLOC-1)*LLDH+ILOC), LLDH )
                           END IF
 80                     CONTINUE
...
*
     ...
     $                          RSRC1, CSRC1 )
                           IF( MYROW.EQ.RSRC1.AND.MYCOL.EQ.CSRC1 ) THEN
                              CALL EGEMM( 'Transpose', 'No Transpose',  
     +                            LNWIN, LCOLS, LNWIN, ONE, WORK(IPU),  
     +                            LNWIN, H((JLOC-1)*LLDH+ILOC), LLDH,   
     +                           ZERO, WORK(IPW), LNWIN )
                              CALL ELAMOV( 'All', LNWIN, LCOLS,         
     +                     WORK(IPW), LNWIN,                            
     +  H((JLOC-1)*LLDH+ILOC), LLDH )
                           END IF
                        END IF
...
*
     ...
     $                             MYCOL, DESCH( CSRC_ ), NPCOL )
                              LROWS = MIN( NB, LKTOP-INDX )
                              CALL ELAMOV( 'All', LROWS, KS,            
     +                  H((JLOC1-1)*LLDH+ILOC ), LLDH,                  
     +            WORK(IPW), LROWS )
                              CALL ETRMM( 'Right', 'Upper',             
...
*
                              CALL ELAMOV( 'All', LROWS, LNWIN-KS,      
     +                        H((JLOC-1)*LLDH+ILOC), LLDH,              
     +                WORK( IPW+KS*LROWS ), LROWS )
                              CALL ETRMM( 'Right', 'Lower',             
     +                 'No transpose', 'Non-Unit',                      
     +        LROWS, LNWIN-KS, ONE,                              WORK( 
     +IPU+LNWIN*KS ), LNWIN,                              WORK( 
     +IPW+KS*LROWS ), LROWS )
                              CALL EGEMM('No transpose', 'No transpose',
...
*
     ...
     $                             MYROW, DESCH( RSRC_ ), NPROW )
                              LCOLS = MIN( NB, N-INDX+1 )
                              CALL ELAMOV( 'All', KS, LCOLS,            
     +                  H((JLOC-1)*LLDH+ILOC1), LLDH,                   
     +           WORK(IPW), LNWIN )
                              CALL ETRMM( 'Left', 'Upper', 'Transpose', 
...
*
                              CALL ELAMOV( 'All', LNWIN-KS, LCOLS,      
     +                        H((JLOC-1)*LLDH+ILOC), LLDH,              
     +                WORK( IPW+KS ), LNWIN )
                              CALL ETRMM( 'Left', 'Lower', 'Transpose', 
...
*
                  ...
                  IF( DIM.LT.LNWIN ) THEN
                     CALL ELASET( 'All', LNWIN, LNWIN, ZERO,            
     +         ONE, WORK( IPH ), LNWIN )
                  ELSE
                     CALL ELASET( 'All', DIM, DIM, ZERO,                
     +     ZERO, WORK( IPH ), LNWIN )
                  END IF
*
...
*
     ...
     $                    DESCH( CSRC_ ), NPCOL )
                     CALL ELAMOV( 'All', DIM1, DIM1,                    
     + H((JLOC-1)*LLDH+ILOC), LLDH, WORK(IPH),                     
     +LNWIN )
                     IF( RSRC1.NE.RSRC4 .OR. CSRC1.NE.CSRC4 ) THEN
*                       Proc#1 <==> Proc#4
                        CALL EGESD2D( ICTXT, DIM1, DIM1,                
     +        WORK(IPH), LNWIN, RSRC4, CSRC4 )
                        CALL EGERV2D( ICTXT, DIM4, DIM4,                
     +        WORK(IPH+DIM1*LNWIN+DIM1),                        LNWIN, 
     +RSRC4, CSRC4 )
                     END IF
                  END IF
     ...
     $                    DESCH( CSRC_ ), NPCOL )
                     CALL ELAMOV( 'All', DIM4, DIM4,                    
     + H((JLOC-1)*LLDH+ILOC), LLDH,                     
     +WORK(IPH+DIM1*LNWIN+DIM1),                     LNWIN )
                     IF( RSRC4.NE.RSRC1 .OR. CSRC4.NE.CSRC1 ) THEN
*                       Proc#4 <==> Proc#1
                        CALL EGESD2D( ICTXT, DIM4, DIM4,                
     +        WORK(IPH+DIM1*LNWIN+DIM1),                        LNWIN, 
     +RSRC1, CSRC1 )
                        CALL EGERV2D( ICTXT, DIM1, DIM1,                
     ...
     $                    DESCH( RSRC_ ), NPROW )
                     JLOC = INDXG2L( LKTOP+DIM1, NB, MYCOL,
     $                    DESCH( CSRC_ ), NPCOL )
                     CALL ELAMOV( 'All', DIM1, DIM4,                    
     + H((JLOC-1)*LLDH+ILOC), LLDH,                     
     +WORK(IPH+DIM1*LNWIN), LNWIN )
                     IF( RSRC2.NE.RSRC1 .OR. CSRC2.NE.CSRC1 ) THEN
*                       Proc#2 ==> Proc#1
                        CALL EGESD2D( ICTXT, DIM1, DIM4,                
     +        WORK(IPH+DIM1*LNWIN),                        LNWIN, RSRC1,
     + CSRC1 )
                     END IF
                  END IF
                  IF( MYROW.EQ.RSRC2 .AND. MYCOL.EQ.CSRC2 ) THEN
...
*                       Proc#2 ==> Proc#4
                        CALL EGESD2D( ICTXT, DIM1, DIM4,                
     +        WORK(IPH+DIM1*LNWIN),                        LNWIN, RSRC4,
     + CSRC4 )
                     END IF
                  END IF
     ...
     $                    DESCH( CSRC_ ), NPCOL )
                     CALL ELAMOV( 'All', 1, 1,                     
     +H((JLOC-1)*LLDH+ILOC), LLDH,                     
     +WORK(IPH+(DIM1-1)*LNWIN+DIM1),                     LNWIN )
                     IF( RSRC3.NE.RSRC1 .OR. CSRC3.NE.CSRC1 ) THEN
*                       Proc#3 ==> Proc#1
                        CALL EGESD2D( ICTXT, 1, 1,                      
     +  WORK(IPH+(DIM1-1)*LNWIN+DIM1),                        LNWIN, 
     +RSRC1, CSRC1 )
                     END IF
                  END IF
...
*                       Proc#1 <== Proc#2
                        CALL EGERV2D( ICTXT, DIM1, DIM4,                
     +        WORK(IPH+DIM1*LNWIN),                        LNWIN, RSRC2,
     + CSRC2 )
                     END IF
                     IF( RSRC1.NE.RSRC3 .OR. CSRC1.NE.CSRC3 ) THEN
*                       Proc#1 <== Proc#3
                        CALL EGERV2D( ICTXT, 1, 1,                      
     +  WORK(IPH+(DIM1-1)*LNWIN+DIM1),                        LNWIN, 
     +RSRC3, CSRC3 )
                     END IF
                  END IF
...
*                       Proc#4 <== Proc#3
     ...
     +RSRC3, CSRC3 )
                  ...
                  END IF
*
*                 Prepare for call to ELAQR6 - it could happen that no
...
*
     ...
     $                    ( DIM1.LE.LCHAIN .OR. DIM1.LE.NTINY ) ) THEN
                     ...
                     KWV = KDU + 4
                     NVE = LNWIN - KDU - KWV + 1
                     CALL ELASET( 'All', LNWIN, LNWIN,                  
     +   ZERO, ONE, WORK(IPUU), LNWIN )
*
*                    Small-bulge multi-shift QR sweep.
*
                     LKS = MAX(1, NS - WIN*LNS + 1)
                     CALL ELAQR6( JOB, WANTT, .TRUE., LKACC22, LNWIN,   
     +                  1, DIM, LNS, SR( LKS ), SI( LKS ),              
     +       WORK(IPH), LNWIN, 1, DIM,                     WORK(IPUU), 
     +LNWIN, WORK(IPU), 3,                     WORK( IPH+KU-1 ), LNWIN, 
     +NVE,                     WORK( IPH+KWV-1 ), LNWIN, NHO,           
...
*
     ...
     $                       DESCH( RSRC_ ), NPROW )
                        JLOC = INDXG2L( LKTOP, NB, MYCOL,
     $                       DESCH( CSRC_ ), NPCOL )
                        CALL ELAMOV( 'All', DIM1, DIM1, WORK(IPH),      
...
*                       Proc#2 <== Proc#4
     ...
     $                    DESCH( CSRC_ ), NPCOL )
                     IF( RSRC3.NE.RSRC1 .OR. CSRC3.NE.CSRC1 ) THEN
*                       Proc#3 <== Proc#1
                        CALL EGERV2D( ICTXT, RWS3, CLS3,                
     +        WORK( IPH+(DIM1-CLS3)*LNWIN+DIM1 ),                       
     + LNWIN, RSRC1, CSRC1 )
                     END IF
                     CALL ELAMOV( 'Upper', RWS3, CLS3,                  
     +   WORK( IPH+(DIM1-CLS3)*LNWIN+DIM1 ),                     LNWIN, 
     +H((JLOC-1)*LLDH+ILOC),                     LLDH )
                     IF( RWS3.GT.1 .AND. CLS3.GT.1 ) THEN
                        ELEM = WORK( IPH+(DIM1-CLS3)*LNWIN+DIM1+1 )
                        IF( ELEM.NE.ZERO ) THEN
                           CALL ELAMOV( 'Lower', RWS3-1, CLS3-1,        
...
*
     ...
     $                       .AND. RSRC4.NE.RSRC1 ) ) THEN
                           CALL EGEBS2D( ICTXT, 'Row', '1-Tree',        
     +                   LENRBUF, 1, WORK, LENRBUF )
                        ELSE
                           CALL EGEBR2D( ICTXT, 'Row', '1-Tree',        
     ...
     $                       .AND. CSRC4.NE.CSRC1 ) ) THEN
                           CALL EGEBS2D( ICTXT, 'Col', '1-Tree',        
     +                   LENCBUF, 1, WORK, LENCBUF )
                        ELSE
                           CALL EGEBR2D( ICTXT, 'Col', '1-Tree',        
     ...
     +          CALL ELAMOV( 'All', LENRBUF, 1, WORK, LENRBUF,          
     +              WORK(1+LENRBUF), LENCBUF )
                     BCDONE = .TRUE.
...
*
     ...
     $                             JLOC1, RSRC, CSRC1 )
                              IF( MYROW.EQ.RSRC ) THEN
                                 CALL ELAMOV( 'All', HROWS, DIM1,       
     +                          H((JLOC1-1)*LLDH+ILOC), LLDH,           
     +                      WORK(IPW), HROWS )
                                 IF( NPCOL.GT.1 ) THEN
                                    EAST = MOD( MYCOL + 1, NPCOL )
                                    CALL EGESD2D( ICTXT, HROWS, DIM1,   
     +                                 WORK(IPW), HROWS, RSRC, EAST )
                                    CALL EGERV2D( ICTXT, HROWS, DIM4,   
     ...
     $                             ILOC, JLOC4, RSRC, CSRC4 )
                              IF( MYROW.EQ.RSRC ) THEN
                                 CALL ELAMOV( 'All', HROWS, DIM4,       
     +                          H((JLOC4-1)*LLDH+ILOC), LLDH,           
     +                      WORK(IPW+HROWS*DIM1), HROWS )
                                 IF( NPCOL.GT.1 ) THEN
                                    WEST = MOD( MYCOL - 1 + NPCOL,
     $                                   NPCOL )
                                    CALL EGESD2D( ICTXT, HROWS, DIM4,   
     +                                 WORK(IPW+HROWS*DIM1), HROWS,     
     +                               RSRC, WEST )
                                    CALL EGERV2D( ICTXT, HROWS, DIM1,   
...
*
     ...
     $                                ILOC1, JLOC, RSRC1, CSRC )
                              ...
                              IF( MYCOL.EQ.CSRC ) THEN
                                 CALL ELAMOV( 'All', DIM1, HCOLS,       
     +                          H((JLOC-1)*LLDH+ILOC1), LLDH,           
     +                      WORK(IPW1), LNWIN )
                                 IF( NPROW.GT.1 ) THEN
                                    SOUTH = MOD( MYROW + 1, NPROW )
                                    CALL EGESD2D( ICTXT, DIM1, HCOLS,   
     +                                 WORK(IPW1), LNWIN, SOUTH,        
     +                            CSRC )
                                    CALL EGERV2D( ICTXT, DIM4, HCOLS,   
...
*
     ...
     $                             ILOC, JLOC1, RSRC, CSRC1 )
                              IF( MYROW.EQ.RSRC ) THEN
                                 CALL ELAMOV( 'All', ZROWS, DIM1,       
     +                          Z((JLOC1-1)*LLDZ+ILOC), LLDZ,           
     +                      WORK(IPW2), ZROWS )
                                 IF( NPCOL.GT.1 ) THEN
                                    EAST = MOD( MYCOL + 1, NPCOL )
                                    CALL EGESD2D( ICTXT, ZROWS, DIM1,   
     +                                 WORK(IPW2), ZROWS, RSRC,         
     +                           EAST )
                                    CALL EGERV2D( ICTXT, ZROWS, DIM4,   
     ...
     $                             CSRC4 )
                              IF( MYROW.EQ.RSRC ) THEN
                                 CALL ELAMOV( 'All', ZROWS, DIM4,       
     +                          Z((JLOC4-1)*LLDZ+ILOC), LLDZ,           
     +                      WORK(IPW2+ZROWS*DIM1), ZROWS )
                                 IF( NPCOL.GT.1 ) THEN
                                    WEST = MOD( MYCOL - 1 + NPCOL,
     $                                   NPCOL )
                                    CALL EGESD2D( ICTXT, ZROWS, DIM4,   
     +                                 WORK(IPW2+ZROWS*DIM1),           
     +                         ZROWS, RSRC, WEST )
                                    CALL EGERV2D( ICTXT, ZROWS, DIM1,   
     +                                 WORK(IPW2), ZROWS, RSRC,         
     +                           WEST )
                                 END IF
                              END IF
...
*
     ...
     +           WORK( IPW3 ), ZROWS )
                                 CALL ETRMM( 'Right', 'Lower',          
     +                       'No transpose',                            
     +     'Non-unit', ZROWS, DIM4,                                 ONE,
     + WORK( IPU+LNWIN*KS ),                                 LNWIN, 
     +WORK( IPW3 ), ZROWS )
                                 CALL EGEMM( 'No transpose',            
     +                     'No transpose', ZROWS, DIM4,                 
     +                KS, ONE,                                 WORK( 
     +IPW2+ZROWS*(DIM4)),                                 ZROWS,        
