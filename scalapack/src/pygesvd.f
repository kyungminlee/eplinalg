      ...
      SUBROUTINE PYGESVD(JOBU,JOBVT,M,N,A,IA,JA,DESCA,S,U,IU,JU,DESCU,  
...
*  =======
*
*  PYGESVD computes the singular value decomposition (SVD) of an
...
*          PYLANGE, PELARED1D, PELARED2D, PYGEBRD. Using the
*          standard notation
*
...
*          to the workspace required for the subprograms YBDSQR,
*          PYUNMBR(QLN), and PYUNMBR(PRT), where QLN and PRT are the
*          values of the arguments VECT, SIDE, and TRANS in the call
*          to PYUNMBR. NRU is equal to the local number of rows of
...
*          > 0:  if YBDSQR did not converge
*                If INFO = MIN(M,N) + 1, then PYGESVD has detected
*                heterogeneity by finding that eigenvalues were not
*                identical across the process grid. In this case, the
*                accuracy of the results from PYGESVD cannot be
*                guaranteed.
*
*  =====================================================================
*
*  The results of PYGEBRD, and therefore PYGESVD, may vary slightly
...
*  ======================
*
*  The routine PYGESVD inherits the same alignement requirement as
*  the routine PYGEBRD, namely:
*
...
*     .. Parameters ..
     ...
     +          MB_=5,NB_=6,RSRC_=7,CSRC_=8,LLD_=9,ITHVAL=10)
      ...
      PARAMETER (ZERO= ((0.0E0_10,0.0E0_10)),ONE= ((1.0E0_10,0.0E0_10)))
      REAL(KIND=10) DZERO,DONE
...
*     .. External Functions ..
      LOGICAL LSAME
      INTEGER NUMROC
      REAL(KIND=10) PELAMCH,PYLANGE
      EXTERNAL LSAME,NUMROC,PELAMCH,PYLANGE
*     ..
*     .. External Subroutines ..
      EXTERNAL BLACS_GET,BLACS_GRIDEXIT,BLACS_GRIDINFO,BLACS_GRIDINIT,  
     +        CHK1MAT,YBDSQR,DESCINIT,EGAMN2D,EGAMX2D,ESCAL,IGAMX2D,    
     +      IGEBR2D,IGEBS2D,PCHK1MAT,PYGEBRD,PYGEMR2D,PELARED1D,        
     +  PELARED2D,PYLASCL,PYLASET,PYUNMBR,PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
              ...
              WORK(1) = CMPLX(LWMIN,0D+00, KIND=10)
              RWORK(1) = REAL(1+4*SIZEB, KIND=10)
*
...
*
      IF (INFO.NE.0) THEN
          CALL PXERBLA(DESCA(CTXT_),'PYGESVD',-INFO)
          RETURN
...
*
      ANRM = PYLANGE('1',M,N,A,IA,JA,DESCA,WORK(INDWORK))
...
*
      IF (ISCALE.EQ.1) THEN
          CALL PYLASCL('G',DONE,SIGMA,M,N,A,IA,JA,DESCA,INFO)
      END IF
*
      CALL PYGEBRD(M,N,A,IA,JA,DESCA,RWORK(INDD),RWORK(INDE),           
...
*
      IF (M.GE.N) THEN
*        Distribute D
          CALL PELARED1D(N+IOFFD,IA,JA,DESCA,RWORK(INDD),RWORK(INDD2),  
     +                  WORK(INDWORK),LLWORK)
*        Distribute E
          CALL PELARED2D(M+IOFFE,IA,JA,DESCA,RWORK(INDE),RWORK(INDE2),  
     +                  WORK(INDWORK),LLWORK)
      ELSE
*        Distribute D
          CALL PELARED2D(M+IOFFD,IA,JA,DESCA,RWORK(INDD),RWORK(INDD2),  
     +                  WORK(INDWORK),LLWORK)
*        Distribute E
          CALL PELARED1D(N+IOFFE,IA,JA,DESCA,RWORK(INDE),RWORK(INDE2),  
     +                  WORK(INDWORK),LLWORK)
      END IF
*
...
*
      IF (WANTU.EQ.1) THEN
          CALL PYLASET('Full',M,SIZE,ZERO,ONE,WORK(INDU),1,1,DESCTU)
      ELSE
          NRU = 0
      END IF
*
      IF (WANTVT.EQ.1) THEN
          CALL PYLASET('Full',SIZE,N,ZERO,ONE,WORK(INDV),1,1,DESCTVT)
      ELSE
          NCVT = 0
      END IF
*
      CALL YBDSQR(UPLO,SIZE,NCVT,NRU,0,RWORK(INDD2+IOFFD),             
     +RWORK(INDE2+IOFFE),WORK(INDV),SIZE,WORK(INDU),LDU,C,1,            
     + WORK(INDWORK),INFO)
*
*     Redistribute elements of U and VT in the block-cyclic fashion.
*
      IF (WANTU.EQ.1) CALL PYGEMR2D(M,SIZE,WORK(INDU),1,1,DESCTU,U,IU,  
     +                             JU,DESCU,DESCU(CTXT_))
*
      IF (WANTVT.EQ.1) CALL PYGEMR2D(SIZE,N,WORK(INDV),1,1,DESCTVT,VT,  
     +                              IVT,JVT,DESCVT,DESCVT(CTXT_))
*
*     Set to ZERO "non-square" elements of the larger matrices U, VT.
*
      IF (M.GT.N .AND. WANTU.EQ.1) THEN
          CALL PYLASET('Full',M-SIZE,SIZE,ZERO,ZERO,U,IA+SIZE,JU,DESCU)
      ELSE IF (N.GT.M .AND. WANTVT.EQ.1) THEN
          CALL PYLASET('Full',SIZE,N-SIZE,ZERO,ZERO,VT,IVT,JVT+SIZE,    
     +              DESCVT)
      END IF
*
*     Multiply Householder rotations from bidiagonalized matrix.
*
      IF (WANTU.EQ.1) CALL PYUNMBR('Q','L','N',M,SIZE,N,A,IA,JA,DESCA,  
     +                            WORK(INDTAUQ),U,IU,JU,DESCU,          
     +                    WORK(INDWORK),LLWORK,INFO)
*
      IF (WANTVT.EQ.1) CALL PYUNMBR('P','R','C',SIZE,N,M,A,IA,JA,DESCA, 
...
*
   ...
   20 CONTINUE
*
      CALL EGAMN2D(DESCA(CTXT_),'a',' ',J,1,RWORK(1+INDE),J,1,1,-1,-1,0)
      CALL EGAMX2D(DESCA(CTXT_),'a',' ',J,1,RWORK(1+INDD2),J,1,1,-1,-1, 
     +             0)
*
      DO 30 I = 1,J
