      SUBROUTINE PELARZ( SIDE, M, N, L, V, IV, JV, DESCV, INCV, TAU, C, 
     +                   IC, JC, DESCC, WORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCC( * ), DESCV( * )
      REAL(KIND=10)   C( * ), TAU( * ), V( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PELARZ applies a real elementary reflector Q (or Q**T) to a real
...
*
*  Q is a product of k elementary reflectors as returned by PETZRZF.
*
*  Notes
*  =====
*
...
*
*  V       (local input) REAL(KIND=10) pointer into the local memory
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension  LOCc(JV) if
...
*
*  C       (local input/local output) REAL(KIND=10) pointer into the
...
*     .. Local Scalars ..
     ...
     $                   NQC2, NQV, RDEST
      REAL(KIND=10)   TAULOC( 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EAXPY, ECOPY, EGEBR2D,         
     +           EGEBS2D, EGEMV, EGER, EGERV2D,                    
     +EGESD2D, EGSUM2D, ELASET, INFOG2L,                    PB_TOPGET, 
     +PBETRNV
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*        from procrow ICROW2 with offset IROFFC2), since PBETRNV transposes
*        V into a column at ICROW2 and EGEMV/EGER below act on sub(C2).
...
*        PEORMR3 may violate as it iterates IV across blocks.
...
*        ICCOL2 with offset ICOFFC2), since the col-V SIDE='R' PBETRNV
*        calls transpose V into a row at ICCOL2 and the downstream
*        EGEMV/EGER act on sub(C2).  In the row-V SIDE='R' branch this
...
*
               IPW = MPV+1
               CALL PBETRNV( ICTXT, 'Rowwise', 'Transpose', L,          
...
*
                     CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1,      
     +                        TAU( IIV ), 1 )
                     TAULOC( 1 ) = TAU( IIV )
*
                  ELSE
*
                     CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1,      
...
*                    doc/UPSTREAM_BUGS.md.
     ...
     +EAXPY( NQC2, ONE, C( IOFFC1 ), LDC,                               
     +WORK( IPW ), 1 )
*
                     CALL EGSUM2D( ICTXT, 'Columnwise', ' ', NQC2, 1,   
...
*
                     IF( MYROW.EQ.ICROW1 )                   CALL 
     +EAXPY( NQC2, -TAULOC( 1 ), WORK( IPW ),                           
     +    1, C( IOFFC1 ), LDC )
                     CALL EGER( MPV, NQC2, -TAULOC( 1 ), WORK, 1,       
...
*
                        IF( MYROW.EQ.ICROW1 )                      CALL 
     +EAXPY( NQC2, -TAULOC( 1 ), WORK,                                  
     +1, C( IOFFC1 ),                                  LDC )
                        CALL EGER( MPV, NQC2, -TAULOC( 1 ), V( IOFFV ), 
...
*
                     IPW = MPV+1
                     CALL ECOPY( MPV, V( IOFFV ), 1, WORK, 1 )
                     WORK( IPW ) = TAU( JJV )
                     CALL EGESD2D( ICTXT, IPW, 1, WORK, IPW, MYROW,     
     +                         ICCOL2 )
*
                  ELSE IF( MYCOL.EQ.ICCOL2 ) THEN
*
                     IPW = MPV+1
                     CALL EGERV2D( ICTXT, IPW, 1, WORK, IPW, MYROW,     
     +                         IVCOL )
                     TAULOC( 1 ) = WORK( IPW )
*
                     IF( TAULOC( 1 ).NE.ZERO ) THEN
...
*
               ELSE
*
                  CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAULOC, 
     +                          1, IVROW, MYCOL )
*
               END IF
*
...
*
     ...
     +                  WORK( IPW ), MAX( 1, NQC2 ) )
                  END IF
                  IF( MYROW.EQ.ICROW1 )                CALL EAXPY( NQC2,
...
*
                  ...
                  CALL ECOPY( MPV, V( IOFFV ), 1, WORK, 1 )
                  WORK( IPW ) = TAU( JJV )
                  CALL EGEBS2D( ICTXT, 'Rowwise', ROWBTOP, IPW, 1,      
     +                     WORK, IPW )
                  TAULOC( 1 ) = TAU( JJV )
*
               ELSE
*
                  IPW = MPV+1
                  CALL EGEBR2D( ICTXT, 'Rowwise', ROWBTOP, IPW, 1, WORK,
     +                           IPW, MYROW, IVCOL )
                  TAULOC( 1 ) = WORK( IPW )
*
               END IF
...
*
                        IF( NQV.GT.0 ) THEN
                           CALL EGEMV( 'No transpose', MPC2, NQV, ONE,  
     +                                C( IOFFC2 ), LDC, V( IOFFV ),     
     +                             LDV, ZERO, WORK, 1 )
                        ELSE
                           CALL ELASET( 'All', MPC2, 1, ZERO, ZERO,     
     +                              WORK, MAX( 1, MPC2 ) )
                        END IF
                        IF( MYCOL.EQ.ICCOL1 )                      CALL 
     +EAXPY( MPC2, ONE, C( IOFFC1 ), 1,                                 
     +   WORK, 1 )
*
                        CALL EGSUM2D( ICTXT, 'Rowwise', ' ', MPC2, 1,   
     +                              WORK, MAX( 1, MPC2 ), RDEST,        
     +                        ICCOL2 )
*
                        IF( MYCOL.EQ.ICCOL1 )                      CALL 
     +EAXPY( MPC2, -TAULOC( 1 ), WORK, 1,                               
     +   C( IOFFC1 ), 1 )
*
*                       sub( C ) := sub( C ) - w * v'
*
                        IF( MPC2.GT.0 .AND. NQV.GT.0 )                  
     +    CALL EGER( MPC2, NQV, -TAULOC( 1 ), WORK, 1,                  
...
*
                     IPW = NQV+1
                     CALL ECOPY( NQV, V( IOFFV ), LDV, WORK, 1 )
                     WORK( IPW ) = TAU( IIV )
                     CALL EGESD2D( ICTXT, IPW, 1, WORK, IPW, ICROW2,    
     +                          MYCOL )
*
                  ELSE IF( MYROW.EQ.ICROW2 ) THEN
*
                     IPW = NQV+1
                     CALL EGERV2D( ICTXT, IPW, 1, WORK, IPW, IVROW,     
     +                         MYCOL )
                     TAULOC( 1 ) = WORK( IPW )
*
                     IF( TAULOC( 1 ).NE.ZERO ) THEN
...
*
     ...
     +                         RDEST, ICCOL2 )
                        IF( MYCOL.EQ.ICCOL1 )                      CALL 
     +EAXPY( MPC2, -TAULOC( 1 ), WORK( IPW ),                           
     +       1, C( IOFFC1 ), 1 )
*
*                       sub( C ) := sub( C ) - w * v'
*
                        CALL EGER( MPC2, NQV, -TAULOC( 1 ), WORK( IPW ),
     +                              1, WORK, 1, C( IOFFC2 ), LDC )
                     END IF
*
                  END IF
...
*
               IPW = NQV+1
               CALL PBETRNV( ICTXT, 'Columnwise', 'Transpose', L,       
...
*
                  IF( MYCOL.EQ.IVCOL ) THEN
*
                     CALL EGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1,         
     +                     TAU( JJV ), 1 )
                     TAULOC( 1 ) = TAU( JJV )
*
                  ELSE
*
                     CALL EGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, TAULOC, 
     +                             1, MYROW, IVCOL )
*
                  END IF
*
...
*
     ...
     +                       ICCOL2 )
                     IF( MYCOL.EQ.ICCOL1 )                   CALL 
     +EAXPY( MPC2, -TAULOC( 1 ), WORK( IPW ), 1,                        
     +       C( IOFFC1 ), 1 )
*
*                    sub( C ) := sub( C ) - w * v'
*
                     CALL EGER( MPC2, NQV, -TAULOC( 1 ), WORK( IPW ), 1,
     +                           WORK, 1, C( IOFFC2 ), LDC )
                  END IF
*
               END IF
...
*
                  ...
                  CALL ECOPY( NQV, V( IOFFV ), LDV, WORK, 1 )
                  WORK( IPW ) = TAU( IIV )
                  CALL EGEBS2D( ICTXT, 'Columnwise', COLBTOP, IPW, 1,   
     +                        WORK, IPW )
                  TAULOC( 1 ) = TAU( IIV )
*
               ELSE
*
                  IPW = NQV+1
                  CALL EGEBR2D( ICTXT, 'Columnwise', COLBTOP, IPW, 1,   
...
*
     ...
     +                  WORK( IPW ), MAX( 1, MPC2 ) )
                  END IF
                  IF( MYCOL.EQ.ICCOL1 )                CALL EAXPY( MPC2,
...
*
               IF( MYCOL.EQ.IVCOL ) THEN
*
                  CALL EGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, TAU( JJV ),
     +                           1 )
                  TAULOC( 1 ) = TAU( JJV )
*
               ELSE
*
                  CALL EGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, TAULOC, 1, 
     +                          MYROW, IVCOL )
*
               END IF
*
