      SUBROUTINE PYLAPRNT( M, N, A, IA, JA, DESCA, IRPRNT, ICPRNT,      
     +                CMATNM, NOUT, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLAPRNT prints to the standard output a distributed matrix sub( A )
...
*
     ...
     +IA+K, JA+H,                    REAL( A(II+K+(JJ+H-1)*LDA) , 
     +KIND=10),                    AIMAG( A(II+K+(JJ+H-1)*LDA) )
   10          CONTINUE
            END IF
         ELSE
            IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
               CALL YGESD2D( ICTXT, IB, 1, A( II+(JJ+H-1)*LDA ), LDA,   
     +                     IRPRNT, ICPRNT )
            ELSE IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
               CALL YGERV2D( ICTXT, IB, 1, WORK, DESCA( MB_ ),          
     +              ICURROW, ICURCOL )
               DO 20 K = 1, IB
                  WRITE( NOUT, FMT = 9999 )                    CMATNM, 
     +IA+K-1, JA+H, REAL( WORK( K ) , KIND=10),                    
     +AIMAG( WORK( K ) )
   20          CONTINUE
            END IF
         END IF
         ...
         DO 50 I = IN+1, IA+M-1, DESCA( MB_ )
            ...
            IF( ICURROW.EQ.IRPRNT .AND. ICURCOL.EQ.ICPRNT ) THEN
               IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                  DO 30 K = 0, IB-1
                     WRITE( NOUT, FMT = 9999 )                       
     +CMATNM, I+K, JA+H,                       REAL( A( 
     +II+K+(JJ+H-1)*LDA ) , KIND=10),                       AIMAG( A( 
     +II+K+(JJ+H-1)*LDA ) )
   30             CONTINUE
               END IF
            ELSE
               IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
                  CALL YGESD2D( ICTXT, IB, 1, A( II+(JJ+H-1)*LDA ),     
     +                      LDA, IRPRNT, ICPRNT )
               ELSE IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                  CALL YGERV2D( ICTXT, IB, 1, WORK, DESCA( MB_ ),       
     +                    ICURROW, ICURCOL )
                  DO 40 K = 1, IB
                     WRITE( NOUT, FMT = 9999 )                       
     +CMATNM, I+K-1, JA+H, REAL( WORK( K ) , KIND=10),                  
     +     AIMAG( WORK( K ) )
   40             CONTINUE
               END IF
            END IF
...
*
   ...
   70             CONTINUE
     ...
     +                    ICURROW, ICURCOL )
                  DO 80 K = 1, IB
                     WRITE( NOUT, FMT = 9999 )                       
     +CMATNM, IA+K-1, J+H, REAL( WORK( K ) , KIND=10),                  
     +     AIMAG( WORK( K ) )
   80             CONTINUE
               END IF
            END IF
            ...
            DO 110 I = IN+1, IA+M-1, DESCA( MB_ )
               ...
               IF( ICURROW.EQ.IRPRNT .AND. ICURCOL.EQ.ICPRNT ) THEN
                  IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                     DO 90 K = 0, IB-1
                        WRITE( NOUT, FMT = 9999 )                       
     +   CMATNM, I+K, J+H,                          REAL( A( 
     +II+K+(JJ+H-1)*LDA ) , KIND=10),                          AIMAG( 
     +A( II+K+(JJ+H-1)*LDA ) )
   90                CONTINUE
                  END IF
               ELSE
     ...
     +                          ICURROW, ICURCOL )
                     DO 100 K = 1, IB
                        WRITE( NOUT, FMT = 9999 )                       
     +   CMATNM, I+K-1, J+H, REAL( WORK( K ) , KIND=10),                
     +          AIMAG( WORK( K ) )
  100                CONTINUE
                  END IF
               END IF
...
*
  130 CONTINUE
*
 9999 FORMAT(A,'(',I6,',',I6,')=',E16.8, '+i*(',E16.8, ')')
*
      RETURN
*
*     End of PYLAPRNT
