      SUBROUTINE PYFILLPAD( ICTXT, M, N, A, LDA, IPRE, IPOST, CHKVAL )
      IMPLICIT NONE
*
...
*  =======
*
*  PYFILLPAD surrounds a two dimensional local array with a guard-
*  zone initialized to the value CHKVAL. The user may later call the
*  routine PYCHEKPAD to discover if the guardzone has been
...
*
*  A       (local input/local output) COMPLEX, array of
...
*
   ...
   10    CONTINUE
      ELSE
         WRITE( *, FMT = * ) 'WARNING no pre-guardzone in PYFILLPAD'
      END IF
*
...
*
   ...
   20    CONTINUE
      ELSE
         WRITE( *, FMT = * ) 'WARNING no post-guardzone in PYFILLPAD'
      END IF
*
