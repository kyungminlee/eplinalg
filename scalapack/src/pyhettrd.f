      SUBROUTINE PYHETTRD( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,  
     +                    LWORK, INFO )
      IMPLICIT NONE
*
...
*     =======
*
*     PYHETTRD reduces a complex Hermitian matrix sub( A ) to Hermitian
...
*         ANB = PJLAENV_EP( DESCA( CTXT_ ), 3, 'PYHETTRD', 'L', 0, 0,
*           0, 0 )
*
...
*     =========================
*
*     PYHETTRD is not intended to be called directly.  All users are
*     encourage to call PYHETRD which will then call PYHETTRD if
...
*     ===============
*
*     PYHETTRD uses five local arrays:
...
*
*       The traditional
*       way of computing v (and the one used in pylatrd.f and
*       ylatrd.f) is:
*         v = tau * v
*         c = v' * h
*         alpha = - tau * c / 2
...
*       same call to EGSUM2D which performs the sum-to-all of c.
*
...
*     .. Parameters ..
     ...
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      ...
      COMPLEX(KIND=10)            Z_ONE, Z_NEGONE, Z_ZERO
      PARAMETER          ( Z_ONE = 1.0E0_10, Z_NEGONE = -1.0E0_10,      
     +              Z_ZERO = 0.0E0_10 )
      REAL(KIND=10)               ZERO
...
*
     ...
     $                   PBMIN, PBSIZE, PNB, ROWSPERPROC
      REAL(KIND=10)               NORM, SAFMAX, SAFMIN
      COMPLEX(KIND=10)            ALPHA, BETA, C, ONEOVERBETA, TOPH, 
     +TOPNV,                    TOPTAU, TOPV, TTOPH, TTOPV
*     ..
*     .. Local Arrays ..
*
*
...
*
      ...
      REAL(KIND=10)               DTMP( 5 )
      COMPLEX(KIND=10)            CC( 3 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGEBR2D, YGEBS2D, YGEMM, YGEMV,
     +                    YGERV2D, YGESD2D, YGSUM2D, CHK1MAT, YLAMOV,   
     +                 YSCAL, YTRMVT, PCHK1MAT, PETREECOMB, PXERBLA,    
     +                ECOMBNRM2, EGEBR2D, EGEBS2D, EGSUM2D
*     ..
*     .. External Functions ..
*
      LOGICAL            LSAME
      INTEGER            ICEIL, NUMROC, PJLAENV_EP
      REAL(KIND=10)               PELAMCH, EYNRM2
      EXTERNAL           LSAME, ICEIL, NUMROC, PJLAENV_EP, PELAMCH, 
     +EYNRM2
*     ..
*     .. Intrinsic Functions ..
...
*
      SAFMAX = SQRT( PELAMCH( ICTXT, 'O' ) ) / N
      SAFMIN = SQRT( PELAMCH( ICTXT, 'S' ) )
*
...
*
      ...
      ELSE
*
*     Here we set execution options for PYHETTRD
*
         PNB = PJLAENV_EP( ICTXT, 2, 'PYHETTRD', 'L', 0, 0, 0, 0 )
         ANB = PJLAENV_EP( ICTXT, 3, 'PYHETTRD', 'L', 0, 0, 0, 0 )
*
         INTERLEAVE = ( PJLAENV_EP( ICTXT, 4, 'PYHETTRD', 'L', 1, 0, 0, 
     +                0 ).EQ.1 )
         TWOGEMMS = ( PJLAENV_EP( ICTXT, 4, 'PYHETTRD', 'L', 2, 0, 0,   
     +            0 ).EQ.1 )
         BALANCED = ( PJLAENV_EP( ICTXT, 4, 'PYHETTRD', 'L', 3, 0, 0,   
     +            0 ).EQ.1 )
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHETTRD', -INFO )
         RETURN
      END IF
*
...
*
               IF( MYROW.EQ.CURROW ) THEN
                  DTMP( 2 ) = REAL( A( LII+( LIJ-1 )*LDA ) , KIND=10)
               ELSE
                  DTMP( 2 ) = ZERO
               END IF
               IF( MYROW.EQ.NXTROW ) THEN
                  DTMP( 3 ) = REAL( A( LIIP1+( LIJ-1 )*LDA ) , KIND=10)
               ...
               END IF
*
               NORM = EYNRM2( NPM1, A( LIIP1+( LIJ-1 )*LDA ), 1 )
               DTMP( 1 ) = NORM
*
*              IF DTMP(5) = 1.0, NORM is too large and might cause
*              overflow, hence PETREECOMB must be called.  IF DTMP(5)
...
*
               ...
               CALL EGSUM2D( ICTXT, 'C', ' ', 5, 1, DTMP, 5, -1,        
     +                CURCOL )
               IF( DTMP( 5 ).EQ.ZERO ) THEN
                  DTMP( 1 ) = SQRT( DTMP( 1 ) )
               ELSE
                  DTMP( 1 ) = NORM
                  CALL PETREECOMB( ICTXT, 'C', 1, DTMP, -1, MYCOL,      
     +                        ECOMBNRM2 )
               END IF
*
               NORM = DTMP( 1 )
...
*
*
               ALPHA = CMPLX( DTMP( 3 ), DTMP( 4 ) , KIND=10)
*
               NORM = SIGN( NORM, REAL( ALPHA , KIND=10) )
*
               ...
               ELSE
                  BETA = NORM + ALPHA
                  TOPTAU = BETA / NORM
                  ONEOVERBETA = 1.0E0_10 / BETA
*
                  CALL YSCAL( NPM1, ONEOVERBETA,                        
     + A( LIIP1+( LIJ-1 )*LDA ), 1 )
               END IF
*
               IF( MYROW.EQ.NXTROW ) THEN
...
*
            IF( MYCOL.EQ.CURCOL ) THEN
               ...
               CALL YGEBS2D( ICTXT, 'R', ' ', NPM1+NPM1+1, 1,           
     +             WORK( INV+LIIP1-1+BINDEX*LDV ),                      
     +  NPM1+NPM1+1 )
            ELSE
               CALL YGEBR2D( ICTXT, 'R', ' ', NPM1+NPM1+1, 1,           
     +             WORK( INV+LIIP1-1+BINDEX*LDV ),                      
...
*
   ...
   60          CONTINUE
            ELSE
               CALL YGESD2D( ICTXT, NPM1+NPM1, 1,                       
     + WORK( INV+LIIP1-1+BINDEX*LDV ), NPM1+NPM1,                       
     + MYCOL, MYROW )
               CALL YGERV2D( ICTXT, NQM1+NQM1, 1,                       
...
*
     ...
     $                  CONJG( WORK( INHT+J-1+BINDEX*LDV ) ) -
     $                  WORK( INH+LIIP1-1+BINDEX*LDV+I )*
     $                  CONJG( WORK( INVT+J-1+BINDEX*LDV ) )
   80             CONTINUE
   90          CONTINUE
            END IF
*
...
*           are not always involved in the calls to YTRMVT
*           for two reasons:
...
*
  ...
  100          CONTINUE
            ELSE
               IF( LTNM1.GT.0 )             CALL YTRMVT( 'L', LTNM1, A( 
     +LTLIP1+( LIJP1-1 )*LDA ),                          LDA, WORK( 
     +INVT+LIJP1-1+( BINDEX+1 )*                          LDV ), 1, 
     +WORK( INH+LTLIP1-1+( BINDEX+                          1 )*LDV ), 
     +1, WORK( INV+LTLIP1-1+                          ( BINDEX+1 )*LDV 
     +), 1,                          WORK( INHT+LIJP1-1+( BINDEX+1 
     +)*LDV ),                          1 )
*
            END IF
*
...
*     tmp = HT * v
*
               CALL YGEMV( 'C', NUMROWS, BINDEX+1, Z_ONE,               
...
*
*
               CALL YGSUM2D( ICTXT, 'C', ' ', 2*( BINDEX+1 ), 1,        
     +                WORK( INTMP ), 2*( BINDEX+1 ), -1, -1 )
            ELSE
*     tmp = HT * v
*
               CALL YGEMV( 'C', NQB, BINDEX+1, Z_ONE, WORK( INHTB ),    
     +                  LDV, WORK( INHTB+( BINDEX+1 )*LDV ), 1,         
     +             Z_ZERO, WORK( INTMP ), 1 )
*     tmp2 = VT * v
               CALL YGEMV( 'C', NQB, BINDEX+1, Z_ONE, WORK( INVTB ),    
     +                  LDV, WORK( INHTB+( BINDEX+1 )*LDV ), 1,         
...
*
               CALL YGSUM2D( ICTXT, 'R', ' ', 2*( BINDEX+1 ), 1,        
     +                WORK( INTMP ), 2*( BINDEX+1 ), -1, -1 )
*
*
*     v = v + V * tmp
               IF( INDEX.GT.1.E0_10 ) THEN
                  CALL YGEMV( 'N', NUMROWS, BINDEX+1, Z_NEGONE,         
     +                WORK( INVB+MYFIRSTROW-1 ), LDV,                   
     +      WORK( INTMP ), 1, Z_ONE,                         WORK( 
...
*     v = v + H * tmp2
     ...
     +WORK( INVB+MYFIRSTROW-1+( BINDEX+1 )*                         LDV 
     +), 1 )
               END IF
*
            ELSE
*     v = v + V * tmp
               CALL YGEMV( 'N', NPB, BINDEX+1, Z_NEGONE, WORK( INVB ),  
     +                    LDV, WORK( INTMP ), 1, Z_ONE,                 
     +     WORK( INVB+( BINDEX+1 )*LDV ), 1 )
*
*
*     v = v + H * tmp2
               CALL YGEMV( 'N', NPB, BINDEX+1, Z_NEGONE, WORK( INHB ),  
...
*
  ...
  120          CONTINUE
            ELSE
               CALL YGESD2D( ICTXT, NQM1, 1,                        
     +WORK( INVT+LIJP1-1+( BINDEX+1 )*LDV ),                        
     +NQM1, MYCOL, MYROW )
               CALL YGERV2D( ICTXT, NPM1, 1, WORK( INTMP ), NPM1, MYCOL,
     +                        MYROW )
*
            END IF
            DO 130 I = 0, NPM1 - 1
...
*
  ...
  140          CONTINUE
               ...
               ELSE
                  ...
                  CC( 3 ) = Z_ZERO
               END IF
               CALL YGSUM2D( ICTXT, 'C', ' ', 3, 1, CC, 3, -1, NXTCOL )
*
               TOPV = CC( 2 )
               C = CC( 1 )
               TOPH = CC( 3 )
...
*
     ...
     $               CONJG( TOPTAU ) / 2*WORK( INH+LIIP1-1+( BINDEX+1 )*
     $               LDV+I ) )
  150          CONTINUE
*
            END IF
...
*
            IF( .NOT.TWOGEMMS ) THEN
               IF( INTERLEAVE ) THEN
                  LDZG = LDV / 2
               ELSE
                  CALL YLAMOV( 'A', LTNM1, ANB, WORK( INHT+LIJP1-1 ),   
     +                       LDV, WORK( INVT+LIJP1-1+ANB*LDV ), LDV )
*
                  CALL YLAMOV( 'A', LTNM1, ANB, WORK( INV+LTLIP1-1 ),   
...
*
      IF( MYCOL.EQ.NXTCOL ) THEN
         IF( MYROW.EQ.NXTROW ) THEN
*
            D( NQ ) = REAL( A( NP+( NQ-1 )*LDA ) , KIND=10)
            A( NP+( NQ-1 )*LDA ) = D( NQ )
*
            CALL EGEBS2D( ICTXT, 'C', ' ', 1, 1, D( NQ ), 1 )
         ELSE
            CALL EGEBR2D( ICTXT, 'C', ' ', 1, 1, D( NQ ), 1, NXTROW,    
     +                 NXTCOL )
         END IF
      END IF
*
