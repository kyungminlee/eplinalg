*
*
      SUBROUTINE PESYGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB, 
     +                    DESCB, SCALE, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PESYGST reduces a real symmetric-definite generalized eigenproblem
*  to standard form.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PESYGS2,    
     +                PESYMM, PESYR2K, PETRMM, PETRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYGST', -INFO )
         RETURN
      END IF
*
...
*
            CALL PESYGS2( IBTYPE, UPLO, KB, A, IA+K-1, JA+K-1, DESCA, B,
     +                     IB+K-1, IB+K-1, DESCB, INFO )
            IF( K+KB.LE.N ) THEN
               CALL PETRSM( 'Left', UPLO, 'Transpose', 'Non-unit', KB,  
     +                     N-K-KB+1, ONE, B, IB+K-1, JB+K-1, DESCB, A,  
     +                     IA+K-1, JA+K+KB-1, DESCA )
               CALL PESYMM( 'Left', UPLO, KB, N-K-KB+1, -HALF, A,       
     +                IA+K-1, JA+K-1, DESCA, B, IB+K-1, JB+K+KB-1,      
     +                 DESCB, ONE, A, IA+K-1, JA+K+KB-1, DESCA )
               CALL PESYR2K( UPLO, 'Transpose', N-K-KB+1, KB, -ONE, A,  
     ...
     +                IA+K-1, JA+K-1, DESCA, B, IB+K-1, JB+K+KB-1,      
     +                 DESCB, ONE, A, IA+K-1, JA+K+KB-1, DESCA )
               CALL PETRSM( 'Right', UPLO, 'No transpose', 'Non-unit',  
...
*
     ...
     +                 IA+K-1, JA+K-1, DESCA, B, IB+K+KB-1, JB+K-1,     
     +                  DESCB, ONE, A, IA+K+KB-1, JA+K-1, DESCA )
               CALL PETRSM( 'Left', UPLO, 'No transpose', 'Non-unit',   
...
*
            CALL PETRMM( 'Left', UPLO, 'No transpose', 'Non-unit', K-1, 
     +                   KB, ONE, B, IB, JB, DESCB, A, IA, JA+K-1,      
     +              DESCA )
            CALL PESYMM( 'Right', UPLO, K-1, KB, HALF, A, IA+K-1,       
     +             JA+K-1, DESCA, B, IB, JB+K-1, DESCB, ONE, A,         
     +           IA, JA+K-1, DESCA )
            CALL PESYR2K( UPLO, 'No transpose', K-1, KB, ONE, A, IA,    
     +                 JA+K-1, DESCA, B, IB, JB+K-1, DESCB, ONE, A,     
     +                IA, JA, DESCA )
            CALL PESYMM( 'Right', UPLO, K-1, KB, HALF, A, IA+K-1,       
     +             JA+K-1, DESCA, B, IB, JB+K-1, DESCB, ONE, A,         
     +           IA, JA+K-1, DESCA )
            CALL PETRMM( 'Right', UPLO, 'Transpose', 'Non-unit', K-1,   
...
*
            CALL PETRMM( 'Right', UPLO, 'No transpose', 'Non-unit', KB, 
     +                   K-1, ONE, B, IB, JB, DESCB, A, IA+K-1, JA,     
     +               DESCA )
            CALL PESYMM( 'Left', UPLO, KB, K-1, HALF, A, IA+K-1, JA+K-1,
     +                    DESCA, B, IB+K-1, JB, DESCB, ONE, A, IA+K-1,  
     +                  JA, DESCA )
            CALL PESYR2K( UPLO, 'Transpose', K-1, KB, ONE, A, IA+K-1,   
     +                  JA, DESCA, B, IB+K-1, JB, DESCB, ONE, A, IA,    
     +                 JA, DESCA )
            CALL PESYMM( 'Left', UPLO, KB, K-1, HALF, A, IA+K-1, JA+K-1,
     +                    DESCA, B, IB+K-1, JB, DESCB, ONE, A, IA+K-1,  
     +                  JA, DESCA )
            CALL PETRMM( 'Left', UPLO, 'Transpose', 'Non-unit', KB, K-1,
