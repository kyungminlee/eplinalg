      SUBROUTINE PYLAMR1D( N, A, IA, JA, DESCA, B, IB, JB, DESCB )
      IMPLICIT NONE
*
...
*
*  PYLAMR1D has not been tested except withint the contect of
...
*  =======
*
*  PYLAMR1D redistributes a one-dimensional row vector from one data
*  decomposition to another.
*
*  This is an auxiliary routine called by PYHETRD to redistribute D, E
*  and TAU.
*
*  Notes
*  =====
*
*  Although all processes call PYGEMR2D, only the processes that own
*  the first column of A send data and only processes that own the
*  first column of B receive data.  The calls to YGEBS2D/YGEBR2D
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGEBR2D, YGEBS2D, PYGEMR2D
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      EXTERNAL           NUMROC
...
*
      ICTXT = DESCB( CTXT_ )
      CALL PYGEMR2D( 1, N, A, IA, JA, DESCAA, B, IB, JB, DESCBB, ICTXT )
*
      CALL BLACS_GRIDINFO( ICTXT, NPROW, NPCOL, MYROW, MYCOL )
      NQ = NUMROC( N, DESCB( NB_ ), MYCOL, 0, NPCOL )
*
      IF( MYROW.EQ.0 ) THEN
         CALL YGEBS2D( ICTXT, 'C', ' ', NQ, 1, B, NQ )
      ELSE
         CALL YGEBR2D( ICTXT, 'C', ' ', NQ, 1, B, NQ, 0, MYCOL )
      END IF
*
      RETURN
*
