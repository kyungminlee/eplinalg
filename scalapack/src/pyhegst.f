*
*
      SUBROUTINE PYHEGST( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB, 
     +                    DESCB, SCALE, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYHEGST reduces a complex Hermitian-definite generalized eigenproblem
*  to standard form.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PYHEGS2, PYHEMM,      
     +              PYHER2K, PCHK2MAT, PYTRMM, PYTRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYHEGST', -INFO )
         RETURN
      END IF
*
...
*
            CALL PYHEGS2( IBTYPE, UPLO, KB, A, IA+K-1, JA+K-1, DESCA, B,
     +                     IB+K-1, IB+K-1, DESCB, INFO )
            IF( K+KB.LE.N ) THEN
               CALL PYTRSM( 'Left', UPLO, 'Conjugate Transpose',        
     +               'Non-unit', KB, N-K-KB+1, CONE, B, IB+K-1,         
     +              JB+K-1, DESCB, A, IA+K-1, JA+K+KB-1, DESCA )
               CALL PYHEMM( 'Left', UPLO, KB, N-K-KB+1, -HALF, A,       
     +                IA+K-1, JA+K-1, DESCA, B, IB+K-1, JB+K+KB-1,      
     +                 DESCB, CONE, A, IA+K-1, JA+K+KB-1, DESCA )
               CALL PYHER2K( UPLO, 'Conjugate Transpose', N-K-KB+1, KB, 
     ...
     +                IA+K-1, JA+K-1, DESCA, B, IB+K-1, JB+K+KB-1,      
     +                 DESCB, CONE, A, IA+K-1, JA+K+KB-1, DESCA )
               CALL PYTRSM( 'Right', UPLO, 'No transpose', 'Non-unit',  
...
*
     ...
     +                 IA+K-1, JA+K-1, DESCA, B, IB+K+KB-1, JB+K-1,     
     +                  DESCB, CONE, A, IA+K+KB-1, JA+K-1, DESCA )
               CALL PYTRSM( 'Left', UPLO, 'No transpose', 'Non-unit',   
...
*
            CALL PYTRMM( 'Left', UPLO, 'No transpose', 'Non-unit', K-1, 
     +                   KB, CONE, B, IB, JB, DESCB, A, IA, JA+K-1,     
     +               DESCA )
            CALL PYHEMM( 'Right', UPLO, K-1, KB, HALF, A, IA+K-1,       
     +             JA+K-1, DESCA, B, IB, JB+K-1, DESCB, CONE, A,        
     +            IA, JA+K-1, DESCA )
            CALL PYHER2K( UPLO, 'No transpose', K-1, KB, CONE, A, IA,   
     +                  JA+K-1, DESCA, B, IB, JB+K-1, DESCB, ONE, A,    
     +                 IA, JA, DESCA )
            CALL PYHEMM( 'Right', UPLO, K-1, KB, HALF, A, IA+K-1,       
     +             JA+K-1, DESCA, B, IB, JB+K-1, DESCB, CONE, A,        
     +            IA, JA+K-1, DESCA )
            CALL PYTRMM( 'Right', UPLO, 'Conjugate transpose',          
...
*
            CALL PYTRMM( 'Right', UPLO, 'No transpose', 'Non-unit', KB, 
     +                   K-1, CONE, B, IB, JB, DESCB, A, IA+K-1, JA,    
     +                DESCA )
            CALL PYHEMM( 'Left', UPLO, KB, K-1, HALF, A, IA+K-1, JA+K-1,
     +                    DESCA, B, IB+K-1, JB, DESCB, CONE, A, IA+K-1, 
     +                   JA, DESCA )
            CALL PYHER2K( UPLO, 'Conjugate transpose', K-1, KB, CONE, A,
     +                     IA+K-1, JA, DESCA, B, IB+K-1, JB, DESCB, ONE,
     +                     A, IA, JA, DESCA )
            CALL PYHEMM( 'Left', UPLO, KB, K-1, HALF, A, IA+K-1, JA+K-1,
     +                    DESCA, B, IB+K-1, JB, DESCB, CONE, A, IA+K-1, 
     +                   JA, DESCA )
            CALL PYTRMM( 'Left', UPLO, 'Conjugate transpose',           
