...
*  =======
*
*  PEPOTRF computes the Cholesky factorization of an N-by-N real
...
*     .. External Subroutines ..
     ...
     +                  PB_TOPSET, PEPOTF2, PESYRK, PETRSM,             
     +       PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPOTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL PEPOTF2( UPLO, JB, A, IA, JA, DESCA, INFO )
         IF( INFO.NE.0 )
     $      GO TO 30
*
         IF( JB+1.LE.N ) THEN
...
*
            CALL PETRSM( 'Left', UPLO, 'Transpose', 'Non-Unit',         
     +           JB, N-JB, ONE, A, IA, JA, DESCA, A, IA, JA+JB,         
     +           DESCA )
*
*           Update the trailing matrix, A = A - U'*U
*
            CALL PESYRK( UPLO, 'Transpose', N-JB, JB, -ONE, A, IA,      
...
*           Perform unblocked Cholesky factorization on JB block
*
            CALL PEPOTF2( UPLO, JB, A, I, J, DESCA, INFO )
            IF( INFO.NE.0 ) THEN
               INFO = INFO + J - JA
               GO TO 30
            END IF
...
*              Update the trailing matrix, A = A - U'*U
*
               CALL PESYRK( UPLO, 'Transpose', N-J-JB+JA, JB,           
...
*
            CALL PESYRK( UPLO, 'No Transpose', N-JB, JB, -ONE, A, IA+JB,
...
*              Update the trailing matrix, A = A - L*L'
*
               CALL PESYRK( UPLO, 'No Transpose', N-J-JB+JA, JB, -ONE,  
