      SUBROUTINE PEPOSVX( FACT, UPLO, N, NRHS, A, IA, JA, DESCA, AF,    
...
*  =======
*
*  PEPOSVX uses the Cholesky factorization A = U**T*U or A = L*L**T to
...
*          output variable.
*
*  SR      (local input/local output) REAL(KIND=10) array,
*                                                    dimension (LLD_A)
...
*          positive.
*
*  SC      (local input/local output) REAL(KIND=10) array,
*                                              dimension (LOC(N_A))
...
*
*  X       (local input/local output) REAL(KIND=10) pointer into
*          the local memory to an array of local dimension
...
*          LWORK = MAX( PEPOCON( LWORK ), PEPORFS( LWORK ) )
*                  + LOCr( N_A ).
*          LWORK = 3*DESCA( LLD_ )
*
...
*     .. External Subroutines ..
     ...
     +       EGAMN2D, EGAMX2D, INFOG2L,                    PEPOCON, 
     +PEPOEQU, PEPORFS,                    PEPOTRF,                    
     +PEPOTRS, PELACPY, PELAQSY, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, INDXG2P, NUMROC
      REAL(KIND=10)   PELAMCH, PELANSY
      EXTERNAL           ICEIL, PELAMCH, INDXG2P, LSAME, NUMROC, PELANSY
*     ..
*     .. Intrinsic Functions ..
...
*           PEPOCON / PEPORFS calls (see comment at line 311 of the
*           upstream prologue documenting the intended formula).
*           Upstream sets LWMIN = 3*DESCA(LLD_), which is insufficient
*           and causes PEPOCON to abort with INFO=-10.
*           PEPOCON LWMIN  = 2*NPMOD + 2*NQMOD +
*                            MAX(2, MAX(NB*ICEIL(NPROW-1,NPCOL),
*                                       NQMOD + NB*ICEIL(NPCOL-1,NPROW)))
*           PEPORFS LWMIN  = 3*NPMOD
*           NPMOD/NQMOD are the unadjusted NUMROCs (PEPOCON's
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPOSVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PEPOEQU( N, A, IA, JA, DESCA, SR, SC, SCOND, AMAX,        
     +          INFEQU )
*
         IF( INFEQU.EQ.0 ) THEN
*
...
*
            CALL PELAQSY( UPLO, N, A, IA, JA, DESCA, SR, SC, SCOND,     
     +                AMAX, EQUED )
            RCEQU = LSAME( EQUED, 'Y' )
         END IF
      END IF
...
*
     ...
     +              DESCAF )
         CALL PEPOTRF( UPLO, N, AF, IAF, JAF, DESCAF, INFO )
*
...
*
      ANORM = PELANSY( '1', UPLO, N, A, IA, JA, DESCA, WORK )
*
*     Compute the reciprocal of the condition number of A.
*
      CALL PEPOCON( UPLO, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,  
...
*
     ...
     +        DESCX )
      CALL PEPOTRS( UPLO, N, NRHS, AF, IAF, JAF, DESCAF, X, IX, JX,     
     +          DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PEPORFS( UPLO, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,      
