      SUBROUTINE PEGEQR2( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEQR2 computes a QR factorization of a real distributed M-by-N
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEQR2', -INFO )
...
*
     ...
     $                   NPCOL )
            ...
            IF( MYCOL.EQ.IACOL ) THEN
               AJJ = A( I )
               CALL ELARFG( 1, AJJ, A( I ), 1, TAU( JJ ) )
               IF( N.GT.1 ) THEN
                  ALPHA = ONE - TAU( JJ )
                  CALL EGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA, 1 )
                  CALL ESCAL( NQ-JJ, ALPHA, A( I+DESCA( LLD_ ) ),       
     +                  DESCA( LLD_ ) )
               END IF
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 1, TAU( JJ ), 
     +                       1 )
               A( I ) = AJJ
            ELSE
               IF( N.GT.1 ) THEN
     ...
     +                      1, IAROW, IACOL )
         ...
         ELSE IF( MYCOL.EQ.IACOL ) THEN
            CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 1, TAU( JJ ), 1, 
     +                    IAROW, IACOL )
         END IF
*
      ELSE
...
*
               CALL PELARF( 'Left', M-J+JA, N-J+JA-1, A, I, J, DESCA, 1,
