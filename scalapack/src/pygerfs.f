      SUBROUTINE PYGERFS( TRANS, N, NRHS, A, IA, JA, DESCA, AF, IAF,    
...
*  =======
*
*  PYGERFS improves the computed solution to a system of linear
...
*          by PYGETRF. IPIV(i) -> The global row local row i
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      INTEGER            ITMAX
      PARAMETER          ( ITMAX = 5 )
      REAL(KIND=10)               ZERO, RONE, TWO, THREE
      PARAMETER          ( ZERO = 0.0E0_10, RONE = 1.0E0_10, TWO = 
...
*     .. External Subroutines ..
     ...
     +               DESCSET, INFOG2L, PYAGEMV, PYAXPY, PCHK2MAT,       
     +             PYCOPY, PYGEMV, PYGETRS, PYLACON,                    
     +PXERBLA, EGAMX2D
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                  IX, JX, DESCX, 20, 5, IDUM1, IDUM2, INFO )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGERFS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*        depending on TRANS.
*
         CALL PYCOPY( N, B, IB, JB+K, DESCB, 1, WORK( IPR ), IW, JW,    
     +             DESCW, 1 )
         CALL PYGEMV( TRANS, N, N, -ONE, A, IA, JA, DESCA, X, IX,       
...
*
   ...
   30          CONTINUE
         ...
         END IF
*
         CALL PYAGEMV( TRANS, N, N, RONE, A, IA, JA, DESCA, X, IX, JX+K,
...
*
            CALL PYGETRS( TRANS, N, 1, AF, IAF, JAF, DESCAF, IPIV,      
     +               WORK( IPR ), IW, JW, DESCW, INFO )
            CALL PYAXPY( N, ONE, WORK( IPR ), IW, JW, DESCW, 1, X, IX,  
     +                  JX+K, DESCX, 1 )
            LSTRES = S
            COUNT = COUNT + 1
            GO TO 20
...
*
               CALL PYGETRS( TRANST, N, 1, AF, IAF, JAF, DESCAF,        
...
*
   ...
   80                CONTINUE
               ...
               END IF
*
               CALL PYGETRS( TRANSN, N, 1, AF, IAF, JAF, DESCAF,        
     +                IPIV, WORK( IPR ), IW, JW, DESCW, INFO )
            END IF
            GO TO 60
         END IF
...
*           depending on TRANS.
*
            CALL PYCOPY( N, B, IB, J+K, DESCB, 1, WORK( IPR ), IW, JW,  
     +                  DESCW, 1 )
            CALL PYGEMV( TRANS, N, N, -ONE, A, IA, JA, DESCA, X,        
...
*
  ...
  120             CONTINUE
            ...
            END IF
*
            CALL PYAGEMV( TRANS, N, N, RONE, A, IA, JA, DESCA, X, IX,   
...
*
               CALL PYGETRS( TRANS, N, 1, AF, IAF, JAF, DESCAF, IPIV,   
     +                     WORK( IPR ), IW, JW, DESCW, INFO )
               CALL PYAXPY( N, ONE, WORK( IPR ), IW, JW, DESCW, 1, X,   
     +                    IX, J+K, DESCX, 1 )
               LSTRES = S
               COUNT = COUNT + 1
               GO TO 110
