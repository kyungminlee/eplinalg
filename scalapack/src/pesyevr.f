      SUBROUTINE PESYEVR( JOBZ, RANGE, UPLO, N, A, IA, JA,              
...
*     .. Array Arguments ..
      ...
      REAL(KIND=10)   A( * ), W( * ), WORK( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PESYEVR computes selected eigenvalues and, optionally, eigenvectors
...
*
*  A       (local input/workspace) 2D block cyclic REAL(KIND=10) array,
...
*          If DESCA is incorrect, PESYEVR cannot guarantee
...
*
*  PESYEVR assumes IEEE 754 standard compliant arithmetic. 
*
...
*     .. External Subroutines ..
     ...
     +     PCHK2MAT, PEELGET, PELAEVSWP, PELARED1D,                     
     +PEORMTR, PESYNTRD, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYEVR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
***********************************************************************
     ...
     + LLWORK, IINFO )
      ...
      IF (IINFO .NE. 0) THEN
         CALL PXERBLA( ICTXT, 'PESYNTRD', -IINFO )
         RETURN
      END IF
...
***********************************************************************
      ...
      ELSE IF ( INDEIG ) THEN
         ...
         IIU = IU
      ELSE IF ( VALEIG ) THEN
         CALL ELARRC('T', N, VLL, VUU, WORK( INDD2 ),      WORK( INDE2 
     ++ OFFSET ), SAFMIN, EIGCNT, IIL, IIU, INFO)
...
*
         ...
         IF ( MYIL.GT.0 ) THEN
            DOL = 1
            DOU = MYIU - MYIL + 1
            CALL ESTEGR2( JOBZ, 'I', N,  WORK( INDD2 ),                 
     +  WORK( INDE2+OFFSET ), VLL, VUU, MYIL, MYIU,                   
     +IM, W( 1 ), WORK( INDRW ), N,                    MYIU - MYIL + 1, 
...
*
         ...
         IF ( MYIL.GT.0 ) THEN
            ...
            DOU = MYIU - IIL + 1
            CALL ESTEGR2( JOBZ, 'I', N,  WORK( INDD2 ),                 
     +  WORK( INDE2+OFFSET ), VLL, VUU, IIL, IIU,                   IM, 
     +W( 1 ), WORK( INDRW ), N,                    N,                   
     +IWORK( 1 ), WORK( INDWORK ), SIZE1,                    IWORK( 
     +2*N+1 ), SIZE2, DOL, DOU,                   ZOFFSET, IINFO )
         ENDIF
         IF (IINFO .NE. 0) THEN
            CALL PXERBLA( ICTXT, 'ESTEGR2', -IINFO )
            RETURN
...
*        Part 1. compute root representations and root eigenvalues
         IF ( MYIL.GT.0 ) THEN
            DOL = MYIL - IIL + 1
            DOU = MYIU - IIL + 1
            CALL ESTEGR2A( JOBZ, 'I', N,  WORK( INDD2 ),                
     +   WORK( INDE2+OFFSET ), VLL, VUU, IIL, IIU,                   IM,
     + W( 1 ), WORK( INDRW ), N,                    N, WORK( INDWORK ), 
     +SIZE1,                    IWORK( 2*N+1 ), SIZE2, DOL,             
     +       DOU, NEEDIL, NEEDIU,                   INDERR, NSPLIT, 
     +PIVMIN, SCALE, WL, WU,                   IINFO )
         ENDIF
         IF (IINFO .NE. 0) THEN
...
*                    Copy uncertainties into communication buffer
                     CALL ECOPY(LENGTHI,WORK( IINDERR+STARTI-1 ),1,     
     +                      WORK( INDD+LENGTHI ), 1)                    
*                    send buffer
                     CALL EGESD2D( ICTXT, LENGTHI2,                     
     + 1, WORK( INDD ), LENGTHI2,                     DSTROW, DSTCOL )
                  END IF
...
*                    copy eigenvalues from communication buffer
                     CALL ECOPY( LENGTHI, WORK(INDD), 1,                
     +           W( STARTI ), 1)                    
*                    copy uncertainties (errors) from communication buffer
                     CALL ECOPY(LENGTHI,WORK(INDD+LENGTHI),1,           
...
*              Broadcast eigenvalues and errors to all processors
               CALL ECOPY(LENGTHI,W ,1, WORK( INDD ), 1)                
     + 
               CALL ECOPY(LENGTHI,WORK( IINDERR ),1,                    
     +       WORK( INDD+LENGTHI ), 1)                    
     ...
     +WORK(INDD), LENGTHI2, SRCROW, SRCCOL )
               CALL ECOPY( LENGTHI, WORK(INDD), 1, W, 1)
               CALL ECOPY(LENGTHI,WORK(INDD+LENGTHI),1,                 
     +          WORK( IINDERR ), 1)                   
            END IF
         ELSE
...
*                          send buffer
  ...
  46                 CONTINUE
     ...
     +         WORK(INDE), RLENGTHI2,                       SRCROW, 
     +SRCCOL )
*                       copy eigenvalues from communication buffer
                        CALL ECOPY( RLENGTHI, WORK(INDE), 1,            
     +               W( RSTARTI ), 1)                    
*                       copy uncertainties (errors) from communication buffer
                        CALL ECOPY(RLENGTHI,WORK(INDE+RLENGTHI),1,      
...
*      
 100     CONTINUE
         IF ( MYIL.GT.0 ) THEN
            CALL ESTEGR2B( JOBZ, N,  WORK( INDD2 ),                   
     +WORK( INDE2+OFFSET ),                    IM, W( 1 ), WORK( INDRW 
     +), N, N,                   IWORK( 1 ), WORK( INDWORK ), SIZE1,    
     +                IWORK( 2*N+1 ), SIZE2, DOL,                    
     +DOU, NEEDIL, NEEDIU, INDWLC,                   PIVMIN, SCALE, WL, 
     +WU,                   VSTART, FINISH,                    MAXCLS, 
     +NDEPTH, PARITY, ZOFFSET, IINFO )
...
*                          Copy eigenvalues into communication buffer
                           CALL ECOPY(LENGTHI,                          
     + WORK( IINDWLC+STARTI-1 ),1,                           WORK(INDD),
     + 1)                    
*                          Copy uncertainties into communication buffer
                           CALL ECOPY(LENGTHI,                          
     + WORK( IINDERR+STARTI-1 ),1,                           
...
***********************************************************************
  ...
  170    CONTINUE
*        Store NVS in IWORK(1:NPROCS+1) for PELAEVSWP
         IWORK( 1 ) = 0
         DO 180 I = 1, NPROCS
...
*           Has already been computed by PMPIM2 and stored
  ...
  180    CONTINUE
         ...
         IF ( FIRST ) THEN
            CALL PELAEVSWP(N, WORK( INDRW ), N, Z, IZ, JZ,         
     +DESCZ, IWORK( 1 ), IWORK( NPROCS+M+2 ), WORK( INDWORK ),         
     +INDRW - INDWORK )
         ELSE
            CALL PELAEVSWP(N, WORK( INDRW + N ), N, Z, IZ, JZ,         
     +DESCZ, IWORK( 1 ), IWORK( NPROCS+M+2 ), WORK( INDWORK ),         
     +INDRW - INDWORK )
         END IF
*
...
***********************************************************************
     ...
     +  WORK( INDWORK ), SIZE1, IINFO )
         END IF
         IF (IINFO.NE.0) THEN
            CALL PXERBLA( ICTXT, 'PEORMTR', -IINFO )
            RETURN
         END IF
*
