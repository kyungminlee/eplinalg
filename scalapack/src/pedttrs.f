      SUBROUTINE PEDTTRS( TRANS, N, NRHS, DL, D, DU, JA, DESCA, B, IB,  
...
*  =======
*
*  PEDTTRS solves a system of linear equations
*
...
*  stored in A(1:N,JA:JA+N-1) and AF by PEDTTRF.
...
*     .. External Subroutines ..
     ...
     +                  GLOBCHK, PEDTTRSV, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     + 'PEDTTRS, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT, 'PEDTTRS, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
      WORK_SIZE_MIN = 10*NPCOL + 4*NRHS
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -15
            CALL PXERBLA( ICTXT, 'PEDTTRS: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      PARAM_CHECK( 15, 2 ) = 1105
      PARAM_CHECK( 14, 2 ) = 1104
      PARAM_CHECK( 13, 2 ) = 1103
      PARAM_CHECK( 12, 2 ) = 1102
      PARAM_CHECK( 11, 2 ) = 1101
      PARAM_CHECK( 10, 2 ) = 10
      PARAM_CHECK( 9, 2 ) = 805
      PARAM_CHECK( 8, 2 ) = 804
      PARAM_CHECK( 7, 2 ) = 803
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEDTTRS', -INFO )
         RETURN
      END IF
*
...
*
      IF( LSAME( TRANS, 'N' ) ) THEN
*
         CALL PEDTTRSV( 'L', 'N', N, NRHS, DL( PART_OFFSET+1 ),         
     +          D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,        
     +           DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,            
     +       LWORK, INFO )
*
      ELSE
*
         CALL PEDTTRSV( 'U', 'T', N, NRHS, DL( PART_OFFSET+1 ),         
     +          D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,        
     +           DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,            
     +       LWORK, INFO )
*
...
*
      IF( ( LSAME( TRANS, 'C' ) ) .OR. ( LSAME( TRANS, 'T' ) ) ) THEN
*
         CALL PEDTTRSV( 'L', 'T', N, NRHS, DL( PART_OFFSET+1 ),         
     +          D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,        
     +           DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,            
     +       LWORK, INFO )
*
      ELSE
*
         CALL PEDTTRSV( 'U', 'N', N, NRHS, DL( PART_OFFSET+1 ),         
     +          D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,        
     +           DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,            
     +       LWORK, INFO )
*
