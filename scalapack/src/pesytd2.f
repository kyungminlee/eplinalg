      SUBROUTINE PESYTD2( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,   
     +                  LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PESYTD2 reduces a real symmetric matrix sub( A ) to symmetric
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, EAXPY,   
     +                 EGEBR2D, EGEBS2D, ELARFG,                    
     +ESYMV, ESYR2, INFOG2L, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      REAL(KIND=10)   EDOT
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYTD2', -INFO )
...
*
                  CALL ELARFG( J, A( IK+JK*LDA ), A( II+JK*LDA ), 1,    
     +                      TAUI )
                  E( JK+1 ) = A( IK+JK*LDA )
*
...
*
                     CALL ESYMV( UPLO, J, TAUI, A( II+(JJ-1)*LDA ),     
     +                       LDA, A( II+JK*LDA ), 1, ZERO,              
     +              TAU( JJ ), 1 )
*
*                    Compute  w := x - 1/2 * tau * (x'*v) * v
*
                     ALPHA = -HALF*TAUI*EDOT( J, TAU( JJ ), 1,          
     +                               A( II+JK*LDA ), 1 )
                     CALL EAXPY( J, ALPHA, A( II+JK*LDA ), 1,           
     +                 TAU( JJ ), 1 )
*
*                    Apply the transformation as a rank-2 update:
*                       A := A - v * w' - w * v'
*
                     CALL ESYR2( UPLO, J, -ONE, A( II+JK*LDA ), 1,      
...
*
   10          CONTINUE
               ...
               WORK( 2*N+1 ) = ZERO
*
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 3*N, WORK, 1 )
*
            ELSE
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 3*N, WORK, 1, 
     +                       IAROW, IACOL )
               DO 20 J = 2, N
                  JN = JJ + J - 1
                  D( JN ) = WORK( J )
...
*
                  CALL ELARFG( N-J, A( IK+1+(JK-1)*LDA ),               
...
*
                     CALL ESYMV( UPLO, N-J, TAUI, A( IK+1+JK*LDA ),     
     +                       LDA, A( IK+1+(JK-1)*LDA ), 1,              
     +              ZERO, TAU( JK ), 1 )
*
*                    Compute  w := x - 1/2 * tau * (x'*v) * v
*
                     ALPHA = -HALF*TAUI*EDOT( N-J, TAU( JK ), 1,        
     +                 A( IK+1+(JK-1)*LDA ), 1 )
                     CALL EAXPY( N-J, ALPHA, A( IK+1+(JK-1)*LDA ),      
     +                      1, TAU( JK ), 1 )
*
*                    Apply the transformation as a rank-2 update:
*                       A := A - v * w' - w * v'
*
                     CALL ESYR2( UPLO, N-J, -ONE,                       
...
*
   ...
   30          CONTINUE
               ...
               WORK( 2*N ) = ZERO
*
               CALL EGEBS2D( ICTXT, 'Columnwise', ' ', 1, 3*N-1, WORK,  
     +                           1 )
*
            ELSE
               CALL EGEBR2D( ICTXT, 'Columnwise', ' ', 1, 3*N-1, WORK,  
     +                      1, IAROW, IACOL )
               DO 40 J = 1, N - 1
                  JN = JJ + J - 1
                  D( JN ) = WORK( J )
