      SUBROUTINE PELAPV2( DIREC, ROWCOL, M, N, A, IA, JA, DESCA, IPIV,  
     +                   IP, JP, DESCIP )
      IMPLICIT NONE
*
...
*  =======
*
*  PELAPV2 applies either P (permutation matrix indicated by IPIV)
...
*
               DO 20 K = I, I+IB-1
                  IP1 = IPIV( ITMP ) - IP + IA
                  IF( IP1.NE.K )                CALL PESWAP( N, A, K, 
     +JA, DESCA, MA, A, IP1, JA,                             DESCA, MA )
                  ITMP = ITMP + 1
   20          CONTINUE
*
...
*
               DO 40 K = J, J+JB-1
                  JP1 = IPIV( ITMP ) - JP + JA
                  IF( JP1.NE.K )                CALL PESWAP( M, A, IA, 
     +K, DESCA, 1, A, IA, JP1,                             DESCA, 1 )
                  ITMP = ITMP + 1
   40          CONTINUE
*
