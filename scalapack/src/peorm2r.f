      SUBROUTINE PEORM2R( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,  
...
*  =======
*
*  PEORM2R overwrites the general real M-by-N distributed matrix
...
*        Q = H(1) H(2) . . . H(k)
*
*  as returned by PEGEQRF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          PEGEQRF in the K columns of its distributed matrix
...
*
*  TAU     (local input) REAL(KIND=10) array, dimension LOCc(JA+K-1).
*          This array contains the scalar factors TAU(j) of the
*          elementary reflectors H(j) as returned by PEGEQRF.
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORM2R', -INFO )
...
*           Apply H(j) or H(j)'
*
            CALL PEELSET2( AJJ, A, IA+J-JA, J, DESCA, ONE )
            CALL PELARF( SIDE, MI, NI, A, IA+J-JA, J, DESCA, 1, TAU, C, 
     +                   ICC, JCC, DESCC, WORK )
            CALL PEELSET( A, IA+J-JA, J, DESCA, AJJ )
*
   10    CONTINUE
*
