      SUBROUTINE PEGEHD2( N, ILO, IHI, A, IA, JA, DESCA, TAU, WORK,     
     +                LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEHD2 reduces a real general distributed matrix sub( A )
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEHD2', -INFO )
...
*        A(i+2:ihi+ia-1,j)
*
         CALL PELARFG( IHI-K, AII, I+1, J, A, MIN( I+2, N+IA-1 ), J,    
     +              DESCA, 1, TAU )
         CALL PEELSET( A, I+1, J, DESCA, ONE )
*
*        Apply H(k) to A(ia:ihi+ia-1,j+1:ihi+ja-1) from the right
*
         CALL PELARF( 'Right', IHI, IHI-K, A, I+1, J, DESCA, 1, TAU, A, 
     +                IA, J+1, DESCA, WORK )
*
*        Apply H(j) to A(i+1:ia+ihi-1,j+1:ja+n-1) from the left
*
         CALL PELARF( 'Left', IHI-K, N-K, A, I+1, J, DESCA, 1, TAU, A,  
     +               I+1, J+1, DESCA, WORK )
*
         CALL PEELSET( A, I+1, J, DESCA, AII )
   10 CONTINUE
*
      WORK( 1 ) = REAL( LWMIN , KIND=10)
*
