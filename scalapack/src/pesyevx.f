      SUBROUTINE PESYEVX( JOBZ, RANGE, UPLO, N, A, IA, JA, DESCA, VL,   
...
*     .. Scalar Arguments ..
     ...
     $                   N, NZ
      REAL(KIND=10)   ABSTOL, ORFAC, VL, VU
*     ..
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCZ( * ), ICLUSTR( * ),
     $                   IFAIL( * ), IWORK( * )
      REAL(KIND=10)   A( * ), GAP( * ), W( * ), WORK( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PESYEVX computes selected eigenvalues and, optionally, eigenvectors
...
*
*  PESYEVX assumes IEEE 754 standard compliant arithmetic.  To port
...
*
*  A       (local input/workspace) block cyclic REAL(KIND=10) array,
...
*          If DESCA( CTXT_ ) is incorrect, PESYEVX cannot guarantee
...
*          If JOBZ='V', setting ABSTOL to PELAMCH( CONTEXT, 'U') yields
...
*          set to twice the underflow threshold 2*PELAMCH('S') not zero.
...
*          insufficient space and PESYEVX is not able to detect this
...
*          PESYEVX is always able to detect insufficient space without
...
*
*  ORFAC   (global input) REAL(KIND=10)
...
*
*  Z       (local output) REAL(KIND=10) array,
*          global dimension (N, N),
...
*             PESYEVX attempts to maintain orthogonality in
...
*             is returned.  Note that when RANGE='V', PESYEVX does
*             not know how many eigenvectors are requested until
*             the eigenvalues are computed.  Therefore, when RANGE='V'
*             and as long as LWORK is large enough to allow PESYEVX to
*             compute the eigenvalues, PESYEVX will compute the
...
*                    ( NPS + 3 ) *  NPS
*
*                  ANB = PJLAENV_EP( DESCA( CTXT_), 3, 'PESYTTRD', 'L',
*                     0, 0, 0, 0)
...
*             PESTEIN will perform no better than ESTEIN on 1
*             processor.
...
*
*  GAP     (global output) REAL(KIND=10) array,
...
*                  in IFAIL.  Ensure ABSTOL=2.0*PELAMCH( 'U' )
...
*                  PESYEVX from computing all of the eigenvectors
*                  between VL and VU.  The number of eigenvectors
*                  computed is returned in NZ.
*                if (MOD(INFO/8,2).NE.0), then PESTEBZ failed to compute
*                  eigenvalues.  Ensure ABSTOL=2.0*PELAMCH( 'U' )
*                  Send e-mail to scalapack@cs.utk.edu
*
...
*  =====================================================================
*
*  Differences between PESYEVX and ESYEVX
*  ======================================
*
*  A, LDA -> A, IA, JA, DESCA
*  Z, LDZ -> Z, IZ, JZ, DESCZ
*  WORKSPACE needs are larger for PESYEVX.
...
*  PESYEVX does not promise orthogonality for eigenvectors associated
*  with tighly clustered eigenvalues.
*  PESYEVX does not reorthogonalize eigenvectors
...
*     .. Parameters ..
     ...
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      REAL(KIND=10)   ZERO, ONE, TEN, FIVE
      PARAMETER          ( ZERO = 0.0E0_10, ONE = 1.0E0_10, TEN = 
     +10.0E0_10,                    FIVE = 5.0E0_10 )
...
*     .. External Functions ..
      ...
      REAL(KIND=10)   PELAMCH, PELANSY
      EXTERNAL           LSAME, ICEIL, INDXG2P, NUMROC, PJLAENV_EP,     
     +               PELAMCH, PELANSY
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, EGEBR2D, EGEBS2D,     
     +               ELASRT, ESCAL, IGAMN2D, PCHK1MAT, PCHK2MAT,        
     +            PEELGET, PELARED1D, PELASCL, PEORMTR, PESTEBZ,        
     +            PESTEIN, PESYNTRD, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, ICHAR, INT, MAX, MIN, MOD, SQRT
*     ..
...
*           new TRD code
*
            ANB = PJLAENV_EP( ICTXT, 3, 'PESYTTRD', 'L', 0, 0, 0, 0 )
            SQNPC = INT( SQRT( REAL( NPROW*NPCOL , KIND=10) ) )
            NPS = MAX( NUMROC( N, 1, 0, 0, SQNPC ), 2*ANB )
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PESYEVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      ...
      END IF
*
      ANRM = PELANSY( 'M', UPLO, N, A, IA, JA, DESCA, WORK( INDWORK ) )
*
...
*
      IF( ISCALE.EQ.1 ) THEN
         CALL PELASCL( UPLO, ONE, SIGMA, N, N, A, IA, JA, DESCA, IINFO )
         IF( ABSTOL.GT.0 )
     $      ABSTLL = ABSTOL*SIGMA
         IF( VALEIG ) THEN
            VLL = VL*SIGMA
      ...
      END IF
*
*     Call PESYNTRD to reduce symmetric matrix to tridiagonal form.
*
      LALLWORK = LLWORK
*
      CALL PESYNTRD( UPLO, N, A, IA, JA, DESCA, WORK( INDD ),           
...
*
     ...
     $     THEN
         CALL PELARED1D( N, IA, JA, DESCA, WORK( INDD ), WORK( INDD2 ), 
     +                   WORK( INDWORK ), LLWORK )
*
         CALL PELARED1D( N, IA, JA, DESCA, WORK( INDE ), WORK( INDE2 ), 
     +                   WORK( INDWORK ), LLWORK )
         IF( .NOT.LOWER )
     $      OFFSET = 1
      ELSE
         DO 10 I = 1, N
            CALL PEELGET( 'A', ' ', WORK( INDD2+I-1 ), A, I+IA-1,       
     +              I+JA-1, DESCA )
   10    CONTINUE
         IF( LSAME( UPLO, 'U' ) ) THEN
            DO 20 I = 1, N - 1
               CALL PEELGET( 'A', ' ', WORK( INDE2+I-1 ), A, I+IA-1,    
     +                    I+JA, DESCA )
   20       CONTINUE
         ELSE
            DO 30 I = 1, N - 1
               CALL PEELGET( 'A', ' ', WORK( INDE2+I-1 ), A, I+IA,      
     +                  I+JA-1, DESCA )
   30       CONTINUE
         END IF
      END IF
*
*     Call PESTEBZ and, if eigenvectors are desired, PESTEIN.
*
      IF( WANTZ ) THEN
         ORDER = 'B'
      ELSE
...
*           NZ that we can feed to PESTEIN and PEORMTR
*
...
*     because PESTEBZ sorts the eigenvalues by block when
*     a split occurs.  So, we have to make another call to
*     PESTEBZ with a new upper limit - VUU.
*
            IF( NSPLIT.GT.1 ) THEN
               CALL ELASRT( 'I', M, W, IINFO )
               NZZ = 0
               IF( NZ.GT.0 ) THEN
*
                  ...
                  IF( VLL.GE.VUU ) THEN
                     NZZ = 0
                  ELSE
                     CALL PESTEBZ( ICTXT, RANGE, ORDER, N, VLL, VUU, IL,
...
*
            END IF
         END IF
         CALL PESTEIN( N, WORK( INDD2 ), WORK( INDE2+OFFSET ), NZ, W,   
...
*
         IF( NZ.GT.0 ) THEN
            CALL PEORMTR( 'L', UPLO, 'N', N, NZ, A, IA, JA, DESCA,      
