      REAL(KIND=10)               FUNCTION PYLANGE( NORM, M, N, A, IA, 
     +JA, DESCA,                                      WORK )
      IMPLICIT NONE
*
...
*          Specifies the value to be returned in PYLANGE as described
*          above.
*
*  M       (global input) INTEGER
*          The number of rows to be operated on i.e the number of rows
*          of the distributed submatrix sub( A ). When M = 0, PYLANGE
*          is set to zero. M >= 0.
*
*  N       (global input) INTEGER
*          The number of columns to be operated on i.e the number of
*          columns of the distributed submatrix sub( A ). When N = 0,
*          PYLANGE is set to zero. N >= 0.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YLASSQ, INFOG2L, ECOMBSSQ,     
     +               EGEBR2D, EGEBS2D, EGAMX2D, EGSUM2D,                
     +    PETREECOMB
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
         ...
         IF( NQ.GT.0 ) THEN
             DO 70 J = IOFFA, IOFFA + NQ*LDA - 1, LDA
                COLSSQ(1) = ZERO
                COLSSQ(2) = ONE
                CALL YLASSQ( MP, A( J ), 1, COLSSQ(1), COLSSQ(2) )
