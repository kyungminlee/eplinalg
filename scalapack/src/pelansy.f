      REAL(KIND=10) FUNCTION PELANSY( NORM, UPLO, N, A, IA, JA,         
     +                           DESCA, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PELANSY  returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PELANSY as described
*          above.
*
...
*          sub( A ). When N = 0, PELANSY is set to zero. N >= 0.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, EAXPY, ECOMBSSQ,               
     +     EGAMX2D, EGSUM2D, EGEBR2D,                    EGEBS2D, 
     +ELASSQ, PECOL2ROW,                    PETREECOMB
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 400 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  COLSSQ(1) = ZERO
                  COLSSQ(2) = ONE
                  CALL ELASSQ( IIA+NP-II, A( II+K ), 1,                 
     +         COLSSQ(1), COLSSQ(2) )
                  IF( MYROW.EQ.IAROW )
     $               II = II + 1
                  CALL ELASSQ( IIA+NP-II, A( II+K ), 1,                 
...
*
         CALL PETREECOMB( ICTXT, 'All', 2, SSQ, IAROW, IACOL,           
     +          ECOMBSSQ )
