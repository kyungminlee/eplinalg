      SUBROUTINE PYUNGLQ( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYUNGLQ generates an M-by-N complex distributed matrix Q denoting
...
*          returned by PYGELQF in the K rows of its distributed matrix
...
*          elementary reflectors H(i) as returned by PYGELQF.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK1MAT, PYLARFB,    
     +                PYLARFT, PYLASET, PYUNGL2, PB_TOPGET,             
     +       PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNGLQ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PYLASET( 'All', IA+M-IL, IL-IA, ZERO, ZERO, A, IL, JA,       
     +        DESCA )
*
*     Use unblocked code for the last or only block.
*
      CALL PYUNGL2( IA+M-IL, N-IL+IA, IA+K-IL, A, IL, JA+IL-IA, DESCA,  
...
*
               CALL PYLARFT( 'Forward', 'Rowwise', N-I+IA, IB, A, I, J, 
     +                       DESCA, TAU, WORK, WORK( IPW ) )
*
*              Apply H' to A(i+ib:ia+m-1,j:ja+n-1) from the right
*
               CALL PYLARFB( 'Right', 'Conjugate transpose', 'Forward', 
     +                       'Rowwise', M-I-IB+IA, N-I+IA, IB, A, I, J, 
     +                       DESCA, WORK, A, I+IB, J, DESCA,            
     +            WORK( IPW ) )
            END IF
*
*           Apply H' to columns j:ja+n-1 of current block
*
            CALL PYUNGL2( IB, N-I+IA, IB, A, I, J, DESCA, TAU, WORK,    
     +                 LWORK, IINFO )
*
*           Set columns ia:i-1 of current block to zero
*
            CALL PYLASET( 'All', IB, I-IA, ZERO, ZERO, A, I, JA, DESCA )
   10    CONTINUE
*
      END IF
*
...
*        H = H(i) H(i+1) . . . H(i+ib-1)
*
         CALL PYLARFT( 'Forward', 'Rowwise', N, IB, A, IA, JA, DESCA,   
     +               TAU, WORK, WORK( IPW ) )
*
...
*
     ...
     +          A, IA+IB, JA, DESCA, WORK( IPW ) )
*
*        Apply H' to columns ja:ja+n-1 of current block
*
         CALL PYUNGL2( IB, N, IB, A, IA, JA, DESCA, TAU, WORK, LWORK,   
     +               IINFO )
*
      END IF
*
