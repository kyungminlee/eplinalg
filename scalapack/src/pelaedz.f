      SUBROUTINE PELAEDZ( N, N1, ID, Q, IQ, JQ, LDQ, DESCQ, Z, WORK )
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCQ( * )
      REAL(KIND=10)   Q( LDQ, * ), WORK( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PELAEDZ Form the z-vector which consists of the last row of Q_1
...
*
     ...
     $              MYROW, MYCOL, IIZ1, JJZ1, IZ1ROW, IZ1COL )
      ...
      IF( ( MYROW.EQ.IZ1ROW ) .AND. ( NQ1.NE.0 ) ) THEN
         CALL ECOPY( NQ1, Q( IIZ1, JJZ1 ), LDQ, WORK, 1 )
         IF( MYROW.NE.IQROW .OR. MYCOL.NE.IQCOL )       CALL EGESD2D( 
     +ICTXT, NQ1, 1, WORK, NQ1, IQROW, IQCOL )
      END IF
*
         ...
         DO 20 I = 0, NPCOL - 1
            ...
            IF( NQ1.GT.0 ) THEN
               IF( IZ1ROW.NE.IQROW .OR. COL.NE.IQCOL ) THEN
                  IBUF = N1 + 1
                  CALL EGERV2D( ICTXT, NQ1, 1, WORK( IBUF ), NQ1,       
     +                    IZ1ROW, COL )
               ELSE
                  IBUF = 1
               END IF
               ...
               DO 10 J = 1, NBLOC
                  ZSIZ = MIN( NB, NQ1-IZ1 )
                  CALL ECOPY( ZSIZ, WORK( IBUF+IZ1 ), 1, Z( IZ ), 1 )
                  IZ1 = IZ1 + NB
                  IZ = IZ + NB*NPCOL
   10          CONTINUE
            END IF
...
*
     ...
     $              MYROW, MYCOL, IIZ2, JJZ2, IZ2ROW, IZ2COL )
      ...
      IF( ( MYROW.EQ.IZ2ROW ) .AND. ( NQ2.NE.0 ) ) THEN
         CALL ECOPY( NQ2, Q( IIZ2, JJZ2 ), LDQ, WORK, 1 )
         IF( MYROW.NE.IQROW .OR. MYCOL.NE.IQCOL )       CALL EGESD2D( 
     +ICTXT, NQ2, 1, WORK, NQ2, IQROW, IQCOL )
      END IF
*
         ...
         DO 40 I = 0, NPCOL - 1
            ...
            IF( NQ2.GT.0 ) THEN
               IF( IQROW.NE.IZ2ROW .OR. IQCOL.NE.COL ) THEN
                  IBUF = 1 + N2
                  CALL EGERV2D( ICTXT, NQ2, 1, WORK( IBUF ), NQ2,       
     +                    IZ2ROW, COL )
               ELSE
                  IBUF = 1
               END IF
               ...
               DO 30 J = 1, NBLOC
                  ZSIZ = MIN( NB, NQ2-IZ2 )
                  CALL ECOPY( ZSIZ, WORK( IBUF+IZ2 ), 1, Z( IZ ), 1 )
                  IZ2 = IZ2 + NB
                  IZ = IZ + NB*NPCOL
   30          CONTINUE
            END IF
...
*
      IF( MYROW.EQ.IQROW .AND. MYCOL.EQ.IQCOL ) THEN
         CALL EGEBS2D( ICTXT, 'All', ' ', N, 1, Z, N )
      ELSE
         CALL EGEBR2D( ICTXT, 'All', ' ', N, 1, Z, N, IQROW, IQCOL )
      END IF
*
      RETURN
*
