      SUBROUTINE PYLAUU2( UPLO, N, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLAUU2 computes the product U * U' or L' * L, where the triangular
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YYDOTC, YGEMV, YLACGV,         
     +           YESCAL, INFOG2L
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
            DO 10 NA = N-1, 1, -1
               AII = A( IDIAG )
               ICURR = IDIAG + LDA
               CALL YYDOTC( NA, DOTC, A( ICURR ), LDA, A( ICURR ), LDA )
               A( IDIAG ) = AII*AII + REAL( DOTC , KIND=10)
               CALL YLACGV( NA, A( ICURR ), LDA )
               CALL YGEMV( 'No transpose', N-NA-1, NA, ONE,             
     +         A( IOFFA+LDA ), LDA, A( ICURR ), LDA,                    
     +  CMPLX( AII , KIND=10), A( IOFFA ), 1 )
               CALL YLACGV( NA, A( ICURR ), LDA )
               IDIAG = IDIAG + LDA + 1
               IOFFA = IOFFA + LDA
   10       CONTINUE
            AII = A( IDIAG )
            CALL YESCAL( N, AII, A( IOFFA ), 1 )
*
         ELSE
*
...
*
            DO 20 NA = 1, N-1
               AII = A( IDIAG )
               ICURR = IDIAG + 1
               A(IDIAG) = AII*AII + REAL( YDOTC( N-NA, A( ICURR ), 1,   
     +                                         A( ICURR ), 1 ) , 
     +KIND=10)
               CALL YLACGV( NA-1, A( IOFFA ), LDA )
               CALL YGEMV( 'Conjugate transpose', N-NA, NA-1, ONE,      
     +                A( IOFFA+1 ), LDA, A( ICURR ), 1,                 
     +     CMPLX( AII , KIND=10), A( IOFFA ), LDA )
               CALL YLACGV( NA-1, A( IOFFA ), LDA )
               IDIAG = IDIAG + LDA + 1
               IOFFA = IOFFA + 1
   20       CONTINUE
            AII = A( IDIAG )
            CALL YESCAL( N, AII, A( IOFFA ), LDA )
*
         END IF
*
      END IF
