      SUBROUTINE PYPTTRS( UPLO, N, NRHS, D, E, JA, DESCA, B, IB, DESCB, 
...
*  =======
*
*  PYPTTRS solves a system of linear equations
*
*            A(1:N, JA:JA+N-1) * X = B(IB:IB+N-1, 1:NRHS)
*
*  where A(1:N, JA:JA+N-1) is the matrix used to produce the factors
*  stored in A(1:N,JA:JA+N-1) and AF by PYPTTRF.
...
*  A(1:N, JA:JA+N-1) = U'D *U or L*D L' as computed by PYPTTRF.
*
...
*          NRHS >= 0.
*
*  D       (local input/local output) COMPLEX(KIND=10) pointer to local
...
*
*  E       (local input/local output) COMPLEX(KIND=10) pointer to local
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,      
     + 'PYPTTRS, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*INT_ONE )) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT,       'PYPTTRS, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -14
         CALL PXERBLA( ICTXT,       'PYPTTRS: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPTTRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PYPTTRSV( 'L', 'N', N, NRHS, D( PART_OFFSET+1 ),          
     +         E( PART_OFFSET+1 ), JA_NEW, DESCA_1XP, B, IB,            
     +       DESCB_PX1, AF, LAF, WORK, LWORK, INFO )
*
      ELSE
*
         CALL PYPTTRSV( 'U', 'C', N, NRHS, D( PART_OFFSET+1 ),          
     +         E( PART_OFFSET+1 ), JA_NEW, DESCA_1XP, B, IB,            
     +       DESCB_PX1, AF, LAF, WORK, LWORK, INFO )
*
      ENDIF
...
*
        DO 10  I=PART_OFFSET+1, PART_OFFSET+ODD_SIZE
          CALL YSCAL( NRHS, CMPLX( CONE/D( I ) , KIND=10), B( I ), LLDB 
     +)
   10   CONTINUE
*
...
*
      IF( MYCOL .LT. NPCOL-1 ) THEN
        I=PART_OFFSET+ODD_SIZE+1
          CALL YSCAL( NRHS, CONE/AF( ODD_SIZE+2 ), B( I ), LLDB )
      ENDIF
*
...
*
      IF( LSAME( UPLO, 'L' ) ) THEN
*
         CALL PYPTTRSV( 'L', 'C', N, NRHS, D( PART_OFFSET+1 ),          
     +         E( PART_OFFSET+1 ), JA_NEW, DESCA_1XP, B, IB,            
     +       DESCB_PX1, AF, LAF, WORK, LWORK, INFO )
*
      ELSE
*
         CALL PYPTTRSV( 'U', 'N', N, NRHS, D( PART_OFFSET+1 ),          
     +         E( PART_OFFSET+1 ), JA_NEW, DESCA_1XP, B, IB,            
     +       DESCB_PX1, AF, LAF, WORK, LWORK, INFO )
*
      ENDIF
