      SUBROUTINE ELARRB2( N, D, LLD, IFIRST, ILAST, RTOL1,              
...
*     .. Scalar Arguments ..
      ...
      REAL(KIND=10)   LGPVMN, LGSPDM, PIVMIN,                     RTOL1,
     + RTOL2
*     ..
*     .. Array Arguments ..
      INTEGER            IWORK( * )
      REAL(KIND=10)   D( * ), LLD( * ), W( * ),                    
     +WERR( * ), WGAP( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  Given the relatively robust representation(RRR) L D L^T, ELARRB2
...
*
*  NOTE: 
*  There are very few minor differences between ELARRB from LAPACK
*  and this current subroutine ELARRB2.
...
*  using ELARRB2 is used for refinement in the construction of 
*  the representation tree, as opposed to the initial computation of the
*  eigenvalues for the root RRR which uses ELARRB. When profiling,
...
*
*  LLD     (input) REAL(KIND=10) array, dimension (N-1)
...
*
*  WGAP    (input/output) REAL(KIND=10) array, dimension (N-1)
...
*
*  LGPVMN  (input) REAL(KIND=10)
*          Logarithm of PIVMIN, precomputed.
*
*  LGSPDM  (input) REAL(KIND=10) 
...
*     .. Local Scalars ..
     ...
     $                   NEXT, NINT, OLNINT, PREV, R
      REAL(KIND=10)   BACK, CVRGD, GAP, LEFT, LGAP, MID, MNWDTH,        
...
*     .. External Functions ..
      ...
      REAL(KIND=10)   ELAMCH
      INTEGER            ELANEG2A
      EXTERNAL           EISNAN, ELAMCH,                     ELANEG2A
*
*     ..
*     .. Intrinsic Functions ..
...
*	 
         BACK = WERR( II )
 20      CONTINUE
         NEGCNT = ELANEG2A( N, WORK(INDLLD+1), LEFT, PIVMIN, R )
...
*	 
         BACK = WERR( II )
 50      CONTINUE
         NEGCNT = ELANEG2A( N, WORK(INDLLD+1),RIGHT, PIVMIN, R )
...
*
         NEGCNT = ELANEG2A( N, WORK(INDLLD+1), MID, PIVMIN, R )
...
*
*
      FUNCTION ELANEG2( N, D, LLD, SIGMA, PIVMIN, R )
*
      IMPLICIT NONE
*
      INTEGER ELANEG2
...
*     .. Local Scalars ..
      ...
      REAL(KIND=10)   DMINUS, DPLUS, GAMMA, P, S, T, TMP, XSAV
      LOGICAL SAWNAN
*     ..
*     .. External Functions ..
      LOGICAL EISNAN
...
*
*
      FUNCTION ELANEG2A( N, DLLD, SIGMA, PIVMIN, R )
*
      IMPLICIT NONE
*
      INTEGER ELANEG2A
