      SUBROUTINE PYGEQLF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGEQLF computes a QL factorization of a complex distributed M-by-N
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK1MAT, PYGEQL2,    
     +                PYLARFB, PYLARFT, PB_TOPGET, PB_TOPSET,           
     +         PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGEQLF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL PYGEQL2( M-N+J+JB-JA, JB, A, IA, J, DESCA, TAU, WORK,  
     +                   LWORK, IINFO )
*
            IF( J.GT.JA ) THEN
*
...
*              left
*
               CALL PYLARFB( 'Left', 'Conjugate transpose', 'Backward', 
...
*
      IF( MU.GT.0 .AND. NU.GT.0 )    CALL PYGEQL2( MU, NU, A, IA, JA, 
