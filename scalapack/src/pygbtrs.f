      SUBROUTINE PYGBTRS( TRANS, N, BWL, BWU, NRHS, A, JA, DESCA, IPIV, 
...
*  =======
*
*  PYGBTRS solves a system of linear equations
*
...
*  stored in A(1:N,JA:JA+N-1) and AF by PYGBTRF.
*  A(1:N, JA:JA+N-1) is an N-by-N complex
*  banded distributed
*  matrix with bandwidth BWL, BWU.
*
*  Routine PYGBTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*          PYGBTRF and this is stored in AF. If a linear system
*          is to be solved using PYGBTRS after the factorization
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,      
     + 'PYGBTRS, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.(BWL+BWU+1) )) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT,       'PYGBTRS, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -16
         CALL PXERBLA( ICTXT,       'PYGBTRS: worksize error ',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGBTRS', -INFO )
         RETURN
      END IF
*
...
*
      IF (MYCOL .LT. NPCOL-1) THEN
         CALL YGESD2D( ICTXT, BWU, NRHS, B(NB-BWU+1), LLDB,         0, 
     +MYCOL + 1)
      ENDIF
*
      IF (MYCOL .LT. NPCOL-1) THEN
...
*
      LDW = NB+BWU + 2*BW+BWU
*
      CALL YLAMOV( 'G', LM, NRHS, B(1), LLDB, WORK( WPTR ), LDW )
*
...
*
      IF (MYCOL .GT. 0) THEN
         CALL YGERV2D( ICTXT, BWU, NRHS, WORK(1), LDW,         0, 
     +MYCOL-1)
      ENDIF
*
********************************************************************
...
*
         IF( L.NE.J ) THEN
            CALL YSWAP(NRHS, WORK(L), LDW, WORK(J), LDW)
         ENDIF
*
         LPTR = BW+1 + (J-1)*LLDA + APTR
*
         CALL YGERU(LMJ,NRHS,-CONE, A(LPTR),1, WORK(J),LDW,            
     +WORK(J+1),LDW)
*
   21 CONTINUE
*
...
*        performed.
         CALL YGETRS( 'N', N-LN, NRHS, AF(BBPTR+BW*LDBB), LDBB,         
...
*
                      CALL YGERV2D(ICTXT, BM+BMN-BW, NRHS,              
     +      WORK( LN+1 ), LDW, 0, NEICOL )
*
                      BM = BM+BMN-BW
*
...
*
                ...
                ENDIF
*
                CALL YLAMOV( 'G', BM, NRHS, WORK(LN+1), LDW,            
     +    WORK(NB+BWU+BMN+1), LDW )
*
                CALL YGERV2D( ICTXT, BMN, NRHS, WORK( NB+BWU+1 ),       
     +            LDW, 0, NEICOL )
*
...
*
                   CALL YLASWP( NRHS, WORK(NB+BWU+1), LDW, 1, BW,       
     +            IPIV(LN+1), 1)
*
                   CALL YTRSM('L','L','N','U', BW, NRHS, CONE,          
     +         AF(BBPTR+BW*LDBB), LDBB, WORK(NB+BWU+1), LDW)
*
*                  Use soln just calculated to update RHS
*
                   CALL YGEMM( 'N', 'N', BM+BMN-BW, NRHS, BW,           
     +      -CONE, AF(BBPTR+BW*LDBB+BW), LDBB,                 
     +WORK(NB+BWU+1), LDW,                 CONE, WORK(NB+BWU+1+BW), LDW 
     +)
*
*                  Give answers back to partner processor
*
                   CALL YGESD2D( ICTXT, BM+BMN-BW, NRHS,                
...
*
                   CALL YLASWP( NRHS, WORK(NB+BWU+1), LDW, 1, BM+BMN,   
     +                IPIV(LN+1), 1)
*
                   CALL YTRSM('L','L','N','U', BM+BMN, NRHS, CONE,      
     +             AF(BBPTR+BW*LDBB), LDBB, WORK(NB+BWU+1), LDW)
                ENDIF
*
             ENDIF
...
*
               IF( NPACT .GT. 2 ) THEN
*
                  CALL YGESD2D( ICTXT, 2*BW, NRHS, WORK( LN+1 ),        
     +           LDW, 0, NEICOL )
*
                  CALL YGERV2D( ICTXT, BW, NRHS, WORK( LN+1 ),          
     +         LDW, 0, NEICOL )
*
               ELSE
*
...
*
               CALL YLAMOV( 'G', BW, NRHS, WORK(NB+BWU+1),              
     +  LDW, WORK(NB+BWU+BW+1), LDW )
*
               CALL YGERV2D( ICTXT, 2*BW, NRHS, WORK( LN+1 ),           
     +        LDW, 0, NEICOL )
*
               CALL YGEMM( 'N', 'N', BW, NRHS, BN,                 
     +-CONE, AF(BBPTR), LDBB,                 WORK(LN+1), LDW,          
     +       CONE, WORK(NB+BWU+BW+1), LDW )
*
*
               IF( MYCOL .GT. NPSTR ) THEN
*
                  CALL YGEMM( 'N', 'N', BW, NRHS, BW,                 
     +-CONE, AF(BBPTR+2*BW*LDBB), LDBB,                 WORK(LN+BW+1), 
     +LDW,                 CONE, WORK(NB+BWU+BW+1), LDW )
*
               ENDIF
*
               CALL YTRSM('L','U','N','N', BW, NRHS, CONE,              
     +   AF(BBPTR+BW*LDBB), LDBB, WORK(NB+BWU+BW+1), LDW)
*
*              Send new solution to neighbor
*
               CALL YGESD2D( ICTXT, BW, NRHS,                 WORK( 
     +NB+BWU+BW+1 ), LDW, 0, NEICOL )
*
*              Copy new solution into expected place
*
               CALL YLAMOV( 'G', BW, NRHS, WORK(NB+BWU+1+BW),           
...
*
               CALL YTRSM( 'L','U','N','N', BN+BNN, NRHS, CONE,         
     +          AF(BBPTR+BW*LDBB), LDBB, WORK(NB+BWU+1), LDW)
*
*              Send new solution to neighbor
*
               CALL YGESD2D( ICTXT, BW, NRHS,                 
     +WORK(NB+BWU+1), LDW, 0, NEICOL )
*
*              Shift solutions into expected positions
*
               CALL YLAMOV( 'G', BNN+BN-BW, NRHS, WORK(NB+BWU+1+BW),    
     +            LDW, WORK(LN+1), LDW )
*
*
...
*
                  DO 1064 J=1, BW
                     CALL YCOPY( NRHS, WORK(NB+BWU+J), LDW,             
     +                          WORK(LN+BW+J), LDW )
 1064             CONTINUE
*
               ENDIF
...
*
      IF( MYCOL .LT. NPCOL-1 ) THEN
*
         CALL YGESD2D( ICTXT, BW, NRHS, WORK( NB-BW+1 ),                
     +   LDW, 0, MYCOL+1 )
*
      ENDIF
*
      IF( MYCOL .GT. 0 ) THEN
*
         CALL YGERV2D( ICTXT, BW, NRHS, WORK( NB+BWU+1 ),               
     +    LDW, 0, MYCOL-1 )
*
*        Modify local right hand sides with received rhs's
*
         CALL YGEMM( 'N', 'N', LM-BM, NRHS, BW, -CONE,            AF( 1 
     +), LM, WORK( NB+BWU+1 ), LDW, CONE,            WORK( 1 ), LDW )
*
      ENDIF
*
      DO 2021 J = LN, 1, -1
...
*
         CALL YGEMV( 'T', LMJ, NRHS, -CONE, WORK( J+1), LDW,            
     +A( LPTR ), LLDA-1, CONE, WORK( J ), LDW )
*
*        Divide by diagonal element
*
         CALL YSCAL( NRHS, CONE/A( LPTR-LLDA+1 ),                WORK( 
     +J ), LDW )
 2021 CONTINUE
*
*
*
      CALL YLAMOV( 'G', ODD_SIZE, NRHS, WORK( 1 ), LDW,              B( 
     +1 ), LLDB )
*
