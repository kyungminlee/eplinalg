      SUBROUTINE PETRCON( NORM, UPLO, DIAG, N, A, IA, JA, DESCA, RCOND, 
...
*  =======
*
*  PETRCON estimates the reciprocal of the condition number of a
...
*     .. Local Scalars ..
     ...
     $                   NPMOD, NPROW, NQ, NQMOD
      REAL(KIND=10)   AINVNM, ANORM, SCALE, SMLNUM
      REAL(KIND=10)   WMAX
*     ..
*     .. Local Arrays ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PETRCON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      RCOND = ZERO
      SMLNUM = PELAMCH( ICTXT, 'Safe minimum' )*REAL( MAX( 1, N ) , 
     +KIND=10)
...
*
      ANORM = PELANTR( NORM, UPLO, DIAG, N, N, A, IA, JA, DESCA, WORK )
*
...
*
         ...
         KASE = 0
   10    CONTINUE
         CALL PELACON( N, WORK( IPV ), IV, JV, DESCV, WORK( IPX ),      
...
*
               DESCX( CSRC_ ) = IACOL
               CALL PELATRS( UPLO, 'No transpose', DIAG, NORMIN,        
...
*
               DESCX( CSRC_ ) = IACOL
               CALL PELATRS( UPLO, 'Transpose', DIAG, NORMIN,           
     +             N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,            
     +            DESCX, SCALE, WORK( IPN ), WORK( IPW ) )
               DESCX( CSRC_ ) = MYCOL
            END IF
...
*
            IF( SCALE.NE.ONE ) THEN
               CALL PEAMAX( N, WMAX, IXX, WORK( IPX ), IX, JX,          
     +          DESCX, 1 )
               IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
                  CALL PB_TOPGET( ICTXT, 'Broadcast', 'Columnwise',
     $                          CBTOP )
                  IF( MYROW.EQ.IAROW ) THEN
                     CALL EGEBS2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX,  
     +                            1 )
                  ELSE
                     CALL EGEBR2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX,  
     +                            1, IAROW, MYCOL )
                  END IF
               END IF
