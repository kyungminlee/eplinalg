      SUBROUTINE PYGERQ2( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYGERQ2 computes a RQ factorization of a complex distributed M-by-N
...
*     .. External Subroutines ..
     ...
     +          PYELSET, PYLACGV, PYLARF, PYLARFG,                    
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYGERQ2', -INFO )
...
*        A(i+m-k,ja:j+n-k-1)
*
         CALL PYLACGV( N-K+J-JA+1, A, I+M-K, JA, DESCA, DESCA( M_ ) )
         CALL PYLARFG( N-K+J-JA+1, AII, I+M-K, J+N-K, A, I+M-K, JA,     
     +             DESCA, DESCA( M_ ), TAU )
*
*        Apply H(i) to A(ia:i+m-k-1,ja:j+n-k) from the right
*
         CALL PYELSET( A, I+M-K, J+N-K, DESCA, ONE )
         CALL PYLARF( 'Right', M-K+I-IA, N-K+J-JA+1, A, M-K+I, JA,      
     +           DESCA, DESCA( M_ ), TAU, A, IA, JA, DESCA, WORK )
         CALL PYELSET( A, I+M-K, J+N-K, DESCA, AII )
         CALL PYLACGV( N-K+J-JA+1, A, I+M-K, JA, DESCA, DESCA( M_ ) )
*
   10 CONTINUE
*
