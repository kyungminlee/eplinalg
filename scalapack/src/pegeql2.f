      SUBROUTINE PEGEQL2( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEQL2 computes a QL factorization of a real distributed M-by-N
...
*
*  TAU     (local output) REAL(KIND=10) array, dimension LOCc(JA+N-1)
...
*     .. Local Scalars ..
     ...
     $                   MP, MYCOL, MYROW, NPCOL, NPROW, NQ
      REAL(KIND=10)   AJJ, ALPHA
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CHK1MAT, EGEBR2D, 
     +                   EGEBS2D, ELARFG, ESCAL, INFOG2L,               
     +     PEELSET, PELARF, PELARFG, PB_TOPGET,                    
     +PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEQL2', -INFO )
...
*
     ...
     $                    NPCOL )
         IF( MYROW.EQ.IAROW ) THEN
            IF( MYCOL.EQ.IACOL ) THEN
               ...
               AJJ = A( I )
               CALL ELARFG( 1, AJJ, A( I ), 1, TAU( JJ+NQ-1 ) )
               IF( N.GT.1 ) THEN
                  ALPHA = ONE - TAU( JJ+NQ-1 )
                  CALL EGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, ALPHA, 1 )
                  CALL ESCAL( NQ-1, ALPHA, A( II+(JJ-1)*DESCA( LLD_ ) ),
     +                         DESCA( LLD_ ) )
               END IF
     ...
     +                      1, IAROW, IACOL )
                  CALL ESCAL( NQ, ALPHA, A( II+(JJ-1)*DESCA( LLD_ ) ),  
     +                       DESCA( LLD_ ) )
               END IF
            END IF
...
*           A(ia:i+m-k-1,j+n-k)
*
            CALL PELARFG( M-K+I-IA+1, AJJ, M-K+I, N-K+J, A, IA,         
     +            N-K+J, DESCA, 1, TAU )
*
*           Apply H(j) to A(ia:i+m-k,ja:j+n-k-1) from the left
*
            CALL PEELSET( A, I+M-K, J+N-K, DESCA, ONE )
            CALL PELARF( 'Left', M-K+I-IA+1, N-K+J-JA, A, IA, N-K+J,    
     +                DESCA, 1, TAU, A, IA, JA, DESCA, WORK )
            CALL PEELSET( A, I+M-K, J+N-K, DESCA, AJJ )
*
   10    CONTINUE
*
      END IF
