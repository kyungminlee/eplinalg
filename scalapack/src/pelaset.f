...
*  =======
*
*  PELASET initializes an M-by-N distributed matrix sub( A ) denoting
...
*
     ...
     $    N.LE.( DESCA( NB_ ) - MOD( JA-1, DESCA( NB_ ) ) ) ) THEN
         CALL PELASE2( UPLO, M, N, ALPHA, BETA, A, IA, JA, DESCA )
      ELSE
*
         IF( LSAME( UPLO, 'U' ) ) THEN
            IN = MIN( ICEIL( IA, DESCA( MB_ ) ) * DESCA( MB_ ), IA+M-1 )
            CALL PELASE2( UPLO, IN-IA+1, N, ALPHA, BETA, A, IA, JA,     
     +                DESCA )
            DO 10 I = IN+1, IA+M-1, DESCA( MB_ )
               ITMP = I-IA
               IBLK = MIN( DESCA( MB_ ), M-ITMP )
               JAA = JA + ITMP
               CALL PELASE2( UPLO, IBLK, N-ITMP, ALPHA, BETA,           
     +             A, I, JAA, DESCA )
   10       CONTINUE
         ELSE IF( LSAME( UPLO, 'L' ) ) THEN
            JN = MIN( ICEIL( JA, DESCA( NB_ ) ) * DESCA( NB_ ), JA+N-1 )
            CALL PELASE2( UPLO, M, JN-JA+1, ALPHA, BETA, A, IA, JA,     
     +                DESCA )
            DO 20 J = JN+1, JA+N-1, DESCA( NB_ )
               JTMP = J-JA
               JBLK = MIN( DESCA( NB_ ), N-JTMP )
               IAA = IA + JTMP
               CALL PELASE2( UPLO, M-JTMP, JBLK, ALPHA, BETA, A, IAA,   
     +                     J, DESCA )
   20       CONTINUE
         ELSE
            IF( M.LE.N ) THEN
     ...
     $                   IA+M-1 )
               CALL PELASE2( UPLO, IN-IA+1, N, ALPHA, BETA, A, IA,      
     +                  JA, DESCA )
               DO 30 I = IN+1, IA+M-1, DESCA( MB_ )
                  ITMP = I-IA
                  IBLK = MIN( DESCA( MB_ ), M-ITMP )
                  CALL PELASE2( UPLO, IBLK, I-IA, ALPHA, ALPHA, A, I,   
     +                        JA, DESCA )
                  CALL PELASE2( UPLO, IBLK, N-I+IA, ALPHA, BETA, A, I,  
     +                         JA+I-IA, DESCA )
   30          CONTINUE
            ELSE
               JN = MIN( ICEIL( JA, DESCA( NB_ ) ) * DESCA( NB_ ),
     $                   JA+N-1 )
               CALL PELASE2( UPLO, M, JN-JA+1, ALPHA, BETA, A, IA,      
     +                  JA, DESCA )
               DO 40 J = JN+1, JA+N-1, DESCA( NB_ )
                  JTMP = J-JA
                  JBLK = MIN( DESCA( NB_ ), N-JTMP )
                  CALL PELASE2( UPLO, J-JA, JBLK, ALPHA, ALPHA, A, IA,  
     +                         J, DESCA )
                  CALL PELASE2( UPLO, M-J+JA, JBLK, ALPHA, BETA, A,     
     +                      IA+J-JA, J, DESCA )
   40          CONTINUE
            END IF
         END IF
