      SUBROUTINE PEDBTRS( TRANS, N, BWL, BWU, NRHS, A, JA, DESCA, B, IB,
...
*  =======
*
*  PEDBTRS solves a system of linear equations
*
...
*  stored in A(1:N,JA:JA+N-1) and AF by PEDBTRF.
...
*     .. External Subroutines ..
     ...
     +                  GLOBCHK, PEDBTRSV, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     + 'PEDBTRS, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*MAX( BWL, BWU ) ) ) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT, 'PEDBTRS, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -15
            CALL PXERBLA( ICTXT, 'PEDBTRS: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEDBTRS', -INFO )
         RETURN
      END IF
*
...
*
      IF( LSAME( TRANS, 'N' ) ) THEN
*
         CALL PEDBTRSV( 'L', 'N', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      ELSE
*
         CALL PEDBTRSV( 'U', 'T', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      END IF
...
*
         CALL PEDBTRSV( 'L', 'T', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      ELSE
*
         CALL PEDBTRSV( 'U', 'N', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      END IF
