      SUBROUTINE PELAHQR( WANTT, WANTZ, N, ILO, IHI, A, DESCA, WR, WI,  
...
*  =======
*
*  PELAHQR is an auxiliary routine used to find the Schur decomposition
...
*          ILO = 1). PELAHQR works primarily with the Hessenberg
...
*          = 0: successful exit
*          > 0: PELAHQR failed to compute all the eigenvalues ILO to IHI
...
*  Subroutines:
*       This routine calls:
*           PELACONSB   -> To determine where to start each iteration
...
*                        back.
*           ELAREF   -> Row/column reflector applier.  Core routine
*                        here.
...
*     .. Local Arrays ..
     ...
     $                   KP2ROW( IBLK ), KROW( IBLK ), LOCALK2( IBLK )
      REAL(KIND=10)   S1( 2*IBLK, 2*IBLK ), SMALLA( 6, 6, IBLK ),       
     +             VCOPY( 3 )
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC
...
*     .. External Subroutines ..
     ...
     +       PELABAD, PELACONSB, PELACP3, PELASMSUB,                    
     +PELAWIL, PXERBLA, ELANV2
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $              -1, -1 )
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( CONTXT, 'PELAHQR', -INFO )
         RETURN
      END IF
*
...
*
         ...
         JBLK = MAX( JBLK, 1 )
*
         CALL PELACP3( 2*JBLK, I-2*JBLK+1, A, DESCA, S1, 2*IBLK, -1, -1,
     +                  0 )
...
*
   ...
   20       CONTINUE
         ...
         ELSE
            CALL ELAHQR( .FALSE., .FALSE., 2*JBLK, 1, 2*JBLK, S1,       
...
*
c         CALL PELACONSB( A, DESCA, I, L, M, H44, H33, H43H34,
...
*
         ...
         IF( JBLK.LE.2*IBLK ) THEN
            CALL PELACP3( I-L+1, L, A, DESCA, S1, 2*IBLK, 0, 0, 0 )
            CALL ELAHQR( .FALSE., .FALSE., JBLK, 1, JBLK, S1, 2*IBLK,   
...
*
  450 CONTINUE
      CALL EGSUM2D( CONTXT, 'All', ' ', N, 1, WR, N, -1, -1 )
      CALL EGSUM2D( CONTXT, 'All', ' ', N, 1, WI, N, -1, -1 )
      RETURN
*
*     END OF PELAHQR
*
