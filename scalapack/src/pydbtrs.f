      SUBROUTINE PYDBTRS( TRANS, N, BWL, BWU, NRHS, A, JA, DESCA, B, IB,
...
*  =======
*
*  PYDBTRS solves a system of linear equations
*
...
*  stored in A(1:N,JA:JA+N-1) and AF by PYDBTRF.
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,      
     + 'PYDBTRS, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*MAX(BWL,BWU) )) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT,       'PYDBTRS, D&C alg.: NB too small',  
     +     -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -15
         CALL PXERBLA( ICTXT,       'PYDBTRS: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYDBTRS', -INFO )
         RETURN
      END IF
*
...
*
      IF( LSAME( TRANS, 'N' ) ) THEN
*
         CALL PYDBTRSV( 'L', 'N', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      ELSE
*
         CALL PYDBTRSV( 'U', 'C', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      ENDIF
...
*
         CALL PYDBTRSV( 'L', 'C', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      ELSE
*
         CALL PYDBTRSV( 'U', 'N', N, BWL, BWU, NRHS, A( PART_OFFSET+1 ),
     +                   JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,  
     +                 WORK, LWORK, INFO )
*
      ENDIF
