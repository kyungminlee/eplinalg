      SUBROUTINE PYUNGQR( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYUNGQR generates an M-by-N complex distributed matrix Q denoting
...
*          returned by PYGEQRF in the K columns of its distributed
...
*     .. External Subroutines ..
     ...
     +                PYLARFT, PYLASET, PYUNG2R, PB_TOPGET,             
     +       PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNGQR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PYLASET( 'All', JL-JA, JA+N-JL, ZERO, ZERO, A, IA, JL,       
     +        DESCA )
*
*     Use unblocked code for the last or only block.
*
      CALL PYUNG2R( M-JL+JA, JA+N-JL, JA+K-JL, A, IA+JL-JA, JL, DESCA,  
...
*
               CALL PYLARFT( 'Forward', 'Columnwise', M-I+IA, JB, A, I, 
     +                       J, DESCA, TAU, WORK, WORK( IPW ) )
*
*              Apply H to A(i:ia+m-1,j+jb:ja+n-1) from the left
*
               CALL PYLARFB( 'Left', 'No transpose', 'Forward',         
...
*
            CALL PYUNG2R( M-I+IA, JB, JB, A, I, J, DESCA, TAU, WORK,    
     +                 LWORK, IINFO )
*
*           Set rows ia:i-1 of current block to zero
*
            CALL PYLASET( 'All', I-IA, JB, ZERO, ZERO, A, IA, J, DESCA )
*
   10    CONTINUE
*
      END IF
...
*        H = H(j) H(j+1) . . . H(j+jb-1)
*
         CALL PYLARFT( 'Forward', 'Columnwise', M, JB, A, IA, JA, DESCA,
     +                  TAU, WORK, WORK( IPW ) )
*
*        Apply H to A(ia:ia+m-1,ja+jb:ja+n-1) from the left
*
         CALL PYLARFB( 'Left', 'No transpose', 'Forward', 'Columnwise', 
     +                 M, N-JB, JB, A, IA, JA, DESCA, WORK, A, IA,      
     +            JA+JB, DESCA, WORK( IPW ) )
*
*        Apply H to rows ia:ia+m-1 of current block
*
         CALL PYUNG2R( M, JB, JB, A, IA, JA, DESCA, TAU, WORK, LWORK,   
     +               IINFO )
*
      END IF
*
