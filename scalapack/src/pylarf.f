      SUBROUTINE PYLARF( SIDE, M, N, V, IV, JV, DESCV, INCV, TAU,       
     +             C, IC, JC, DESCC, WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PYLARF applies a complex elementary reflector Q to a complex M-by-N
...
*     .. External Subroutines ..
     ...
     +             YGEMV, YGERC, YGERV2D, YGESD2D,                    
     +YGSUM2D, YLASET, INFOG2L, PB_TOPGET,                    PBYTRNV
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
               IPW = MP+1
               CALL PBYTRNV( ICTXT, 'Rowwise', 'Transpose', M,          
...
*
                     IF( MP.GT.0 ) THEN
                        CALL YGEMV( 'Conjugate transpose', MP, NQ, ONE, 
     +                              C( IOFFC ), LDC, WORK, 1, ZERO,     
     +                          WORK( IPW ), 1 )
                     ELSE
                        CALL YLASET( 'All', NQ, 1, ZERO, ZERO,          
     +                      WORK( IPW ), MAX( 1, NQ ) )
                     END IF
                     CALL YGSUM2D( ICTXT, 'Columnwise', ' ', NQ, 1,     
     +                         WORK( IPW ), MAX( 1, NQ ), RDEST,        
     +                      MYCOL )
*
*                    sub( C ) := sub( C ) - v * w'
*
                     CALL YGERC( MP, NQ, -TAULOC( 1 ), WORK, 1,         
...
*
                        IF( MP.GT.0 ) THEN
                           CALL YGEMV( 'Conjugate transpose', MP, NQ,   
...
*                       sub( C ) := sub( C ) - v * w'
*
                        CALL YGERC( MP, NQ, -TAULOC( 1 ), V( IOFFV ), 1,
     +                               WORK, 1, C( IOFFC ), LDC )
                     END IF
*
                  END IF
...
*
                     IPW = MP+1
                     CALL YCOPY( MP, V( IOFFV ), 1, WORK, 1 )
                     WORK( IPW ) = TAU( JJV )
...
*                 sub( C ) := sub( C ) - v * w'
*
                  IF( IOFFC.GT.0 )                CALL YGERC( MP, NQ, 
...
*
                        IF( NQ.GT.0 ) THEN
                           CALL YGEMV( 'No transpose', MP, NQ, ONE,     
     +                             C( IOFFC ), LDC, V( IOFFV ), LDV,    
     +                              ZERO, WORK, 1 )
                        ELSE
                           CALL YLASET( 'All', MP, 1, ZERO, ZERO,       
     +                            WORK, MAX( 1, MP ) )
                        END IF
                        CALL YGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,     
...
*
                     IPW = NQ+1
                     CALL YCOPY( NQ, V( IOFFV ), LDV, WORK, 1 )
                     WORK(IPW) = TAU( IIV )
                     CALL YGESD2D( ICTXT, IPW, 1, WORK, IPW, ICROW,     
     +                         MYCOL )
*
...
*                       sub( C ) := sub( C ) - w * v'
*
                        CALL YGERC( MP, NQ, -TAULOC( 1 ), WORK( IPW ),  
...
*
               IPW = NQ+1
               CALL PBYTRNV( ICTXT, 'Columnwise', 'Transpose', N,       
...
*                    sub( C ) := sub( C ) - w * v'
*
                     CALL YGERC( MP, NQ, -TAULOC( 1 ), WORK( IPW ), 1,  
     +                          WORK, 1, C( IOFFC ), LDC )
                  END IF
*
               END IF
...
*
     ...
     +          WORK( IPW ), 1 )
                  ELSE
                     CALL YLASET( 'All', MP, 1, ZERO, ZERO, WORK( IPW ),
     +                             MAX( 1, MP ) )
                  END IF
                  CALL YGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,           
     +                WORK( IPW ), MAX( 1, MP ), RDEST,                 
