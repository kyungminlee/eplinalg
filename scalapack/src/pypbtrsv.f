      SUBROUTINE PYPBTRSV( UPLO, TRANS, N, BW, NRHS, A, JA, DESCA, B,   
...
*  =======
*
*  PYPBTRSV solves a banded triangular system of linear equations
*
...
*  Cholesky factorization code PYPBTRF
...
*
*  Routine PYPBTRF MUST be called first.
*
*  =====================================================================
*
*  Arguments
...
*          see NOTES below for full description and options.
*
*  AF      (local output) COMPLEX(KIND=10) array, dimension LAF.
...
*          COMPLEX(KIND=10) temporary workspace. This space may
...
*     .. External Subroutines ..
     ...
     +               DESC_CONVERT, GLOBCHK, PXERBLA, RESHAPE, YGEMM,    
     +                YGERV2D, YGESD2D, YLAMOV, YMATADD, YTBTRS,        
     +            YTRMM, YTRTRS
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 3 )
         CALL PXERBLA( ICTXT,      
     + 'PYPBTRSV, D&C alg.: only 1 block per proc',       -INFO )
         RETURN
      ENDIF
*
      IF((JA+N-1.GT.NB) .AND. ( NB.LT.2*BW )) THEN
         INFO = -( 8*100+4 )
         CALL PXERBLA( ICTXT,       'PYPBTRSV, D&C alg.: NB too small', 
     +      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( LWORK .LT. WORK_SIZE_MIN ) THEN
         IF( LWORK .NE. -1 ) THEN
         INFO = -15
         CALL PXERBLA( ICTXT,       'PYPBTRSV: worksize error',       
     +-INFO )
         ENDIF
         RETURN
      ENDIF
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYPBTRSV', -INFO )
         RETURN
      END IF
*
...
*
        CALL YTBTRS( UPLO, 'N', 'N', ODD_SIZE,                     BW, 
     +NRHS,                     A( OFST+1 ), LLDA,                     
...
*             then use it on RHS
*
            CALL YLAMOV( 'N', BW, NRHS,                 B( 
     +PART_OFFSET+ODD_SIZE-BW+1), LLDB,                 WORK( 1 ), BW )
*
            CALL YTRMM( 'L', 'U', 'N', 'N', BW, NRHS, -CONE,            
     +       A(( OFST+(BW+1)+(ODD_SIZE-BW)*LLDA )), LLDA-1,             
     +      WORK( 1 ), BW )
*
            CALL YMATADD( BW, NRHS, CONE, WORK( 1 ), BW,                
...
*
            CALL YGEMM( 'C', 'N', BW, NRHS, ODD_SIZE, -CONE, AF( 1 ),   
...
*
        IF( MYCOL .GT. 0) THEN
*
          CALL YGESD2D( ICTXT, BW, NRHS,                      WORK( 1 ),
     + BW,                      0, MYCOL - 1 )
*
        ENDIF
*
...
*
          CALL YGERV2D( ICTXT, BW, NRHS,                      WORK( 1 ),
     + BW,                      0, MYCOL + 1 )
*
...
*
        CALL YTRTRS( 'L', 'N', 'N', BW, NRHS, AF( ODD_SIZE*BW+MBW2+1 ), 
     +               BW, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
          GO TO 1000
        ENDIF
...
*
          CALL YGEMM( 'C', 'N', BW, NRHS, BW, -CONE,                    
     +  AF( (ODD_SIZE)*BW+1 ),                      BW,                 
...
*
          CALL YGEMM( 'N', 'N', BW, NRHS, BW, -CONE,                    
...
*       Solve with diagonal block
*
        CALL YTRTRS( 'L', 'C', 'N', BW, NRHS, AF( ODD_SIZE*BW+MBW2+1 ), 
     +               BW, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
          GO TO 1000
...
*
          CALL YGEMM( 'N', 'N', ODD_SIZE, NRHS, BW, -CONE, AF( 1 ),     
     +            ODD_SIZE, WORK( 1+BW-BW ), BW, CONE,                 
...
*           then use it on RHS
*
          CALL YLAMOV( 'N', BW, NRHS, B( PART_OFFSET+ODD_SIZE+1), LLDB, 
     +                 WORK( 1+BW-BW ), BW )
*
          CALL YTRMM( 'L', 'U', 'C', 'N', BW, NRHS, -CONE,              
     +   A(( OFST+(BW+1)+(ODD_SIZE-BW)*LLDA )), LLDA-1,                 
     +WORK( 1+BW-BW ), BW )
*
          CALL YMATADD( BW, NRHS, CONE, WORK( 1+BW-BW ), BW, CONE,      
...
*       Use main partition in each processor to solve locally
*
        CALL YTBTRS( UPLO, 'C', 'N', ODD_SIZE,                     BW, 
     +NRHS,                     A( OFST+1 ),                     LLDA,  
     +B( PART_OFFSET+1 ),                     LLDB, INFO )
*
      ENDIF
...
*
     ...
     +PART_OFFSET+ODD_SIZE-BW+1), LLDB,                 WORK( 1 ), BW )
*
            CALL YTRMM( 'L', 'L', 'C', 'N', BW, NRHS, -CONE,            
...
*
     ...
     +                 WORK( 1+BW-BW ), BW )
*
          CALL YTRMM( 'L', 'L', 'N', 'N', BW, NRHS, -CONE,              
