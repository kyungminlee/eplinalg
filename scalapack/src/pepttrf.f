      SUBROUTINE PEPTTRF( N, D, E, JA, DESCA, AF, LAF, WORK, LWORK,     
     +                INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEPTTRF computes a Cholesky factorization
...
*  subsequent calls to PEPTTRS to solve linear systems.
*
...
*     .. External Subroutines ..
     ...
     +                  EGERV2D, EGESD2D, EPTTRF, EPTTRSV, ETRRV2D,     
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT,
     + 'PEPTTRF, D&C alg.: only 1 block per proc'                  , 
     +-INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 5*100+4 )
         CALL PXERBLA( ICTXT, 'PEPTTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PEPTTRF: auxiliary storage error ',      
     +            -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -9
            CALL PXERBLA( ICTXT, 'PEPTTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPTTRF', -INFO )
         RETURN
      END IF
*
...
*
      CALL EPTTRF( ODD_SIZE, D( PART_OFFSET+1 ), E( PART_OFFSET+1 ),    
     +          INFO )
*
      IF( INFO.NE.0 ) THEN
         INFO = MYCOL + 1
...
*
            CALL EPTTRSV( 'N', ODD_SIZE, INT_ONE, D( PART_OFFSET+1 ),   
