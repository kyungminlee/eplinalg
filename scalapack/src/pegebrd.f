      SUBROUTINE PEGEBRD( M, N, A, IA, JA, DESCA, D, E, TAUQ, TAUP,     
     +                WORK, LWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PEGEBRD reduces a real general M-by-N distributed matrix
...
*     .. External Subroutines ..
     ...
     +                PEELSET, PEGEBD2, PEGEMM, PELABRD,                
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEGEBRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*        part of the matrix.
*
         CALL PELABRD( M-K+1, N-K+1, JB, A, I, J, DESCA, D, E, TAUQ,    
...
*
         CALL PEGEMM( 'No transpose', 'No transpose', M-K-JB+1,         
...
*
     ...
     $                MP )
            IF( JS.GT.0 )          CALL PEELSET( A, I+JB-1, J+JB, DESCA,
     + E( JS ) )
         ELSE
            JS = MIN( INDXG2L( J+JB-1, NB, 0, DESCA( CSRC_ ), NPCOL ),
     $                NQ )
            IF( JS.GT.0 )          CALL PEELSET( A, I+JB, J+JB-1, DESCA,
     + E( JS ) )
         END IF
*
         K = K + JB
...
*
      CALL PEGEBD2( M-K+1, N-K+1, A, IA+K-1, JA+K-1, DESCA, D, E, TAUQ, 
