      SUBROUTINE PELAED0( N, D, E, Q, IQ, JQ, DESCQ, WORK, IWORK, INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PELAED0 computes all eigenvalues and corresponding eigenvectors of a
...
*     .. External Subroutines ..
     ...
     +               EGESD2D, ESTEQR, INFOG2L, PELAED1, PXERBLA
*     ..
*     .. External Functions ..
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $   INFO = -1
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCQ( CTXT_ ), 'PELAED0', -INFO )
         RETURN
      END IF
*
      NB = DESCQ( NB_ )
...
*
     ...
     $                 MYCOL, IID, JJD, IDROW, IDCOL )
         ...
         IF( MYROW.EQ.IDROW .AND. MYCOL.EQ.IDCOL ) THEN
            IPQ = IID + ( JJD-1 )*LDQ
            CALL ESTEQR( 'I', MATSIZ, D( ID ), E( ID ), Q( IPQ ), LDQ,  
     +                  WORK, INFO )
            IF( INFO.NE.0 ) THEN
               CALL PXERBLA( DESCQ( CTXT_ ), 'ESTEQR', -INFO )
               RETURN
            END IF
            IF( MYROW.NE.IQROW .OR. MYCOL.NE.IQCOL ) THEN
               CALL EGESD2D( DESCQ( CTXT_ ), MATSIZ, 1, D( ID ), MATSIZ,
     +                        IQROW, IQCOL )
            END IF
         ELSE IF( MYROW.EQ.IQROW .AND. MYCOL.EQ.IQCOL ) THEN
            CALL EGERV2D( DESCQ( CTXT_ ), MATSIZ, 1, D( ID ), MATSIZ,   
     +                  IDROW, IDCOL )
         END IF
   50 CONTINUE
*
      IF( MYROW.EQ.IQROW .AND. MYCOL.EQ.IQCOL ) THEN
         CALL EGEBS2D( DESCQ( CTXT_ ), 'A', ' ', N, 1, D, N )
      ELSE
         CALL EGEBR2D( DESCQ( CTXT_ ), 'A', ' ', N, 1, D, N, IQROW,     
     +             IQCOL )
      END IF
*
...
*
            CALL PELAED1( MATSIZ, N1, D( ID ), ID, Q, IQ, JQ, DESCQ,    
