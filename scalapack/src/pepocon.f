      SUBROUTINE PEPOCON( UPLO, N, A, IA, JA, DESCA, ANORM, RCOND, WORK,
...
*  =======
*
*  PEPOCON estimates the reciprocal of the condition number (in the
...
*     .. Local Scalars ..
     ...
     $                   NPCOL, NPROW, NPMOD, NQ, NQMOD
      REAL(KIND=10)   AINVNM, SCALE, SL, SU, SMLNUM
      REAL(KIND=10)   WMAX
*     ..
*     .. Local Arrays ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEPOCON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PELATRS( 'Lower', 'No transpose', 'Non-unit', NORMIN,  
     +                   N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,      
     +               DESCX, SL, WORK( IPNL ), WORK( IPW ) )
            DESCX( CSRC_ ) = MYCOL
            NORMIN = 'Y'
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PELATRS( 'Lower', 'Transpose', 'Non-unit', NORMIN,     
     +                N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,         
     +            DESCX, SU, WORK( IPNU ), WORK( IPW ) )
            DESCX( CSRC_ ) = MYCOL
         END IF
