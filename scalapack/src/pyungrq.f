      SUBROUTINE PYUNGRQ( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,  
     +                   INFO )
      IMPLICIT NONE
*
...
*  =======
*
*  PYUNGRQ generates an M-by-N complex distributed matrix Q denoting
...
*     .. External Subroutines ..
     ...
     +                PYLARFT, PYLASET, PYUNGR2, PB_TOPGET,             
     +       PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PYUNGRQ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PYLASET( 'All', IN-IA+1, M-IN+IA-1, ZERO, ZERO, A, IA,       
     +        JA+N-M+IN-IA+1, DESCA )
*
*     Use unblocked code for the first or only block.
*
      CALL PYUNGR2( IN-IA+1, N-M+IN-IA+1, IN-IA-M+K+1, A, IA, JA, DESCA,
...
*
         CALL PYLARFT( 'Backward', 'Rowwise', N-M+I+IB-IA, IB, A, I, JA,
     +                  DESCA, TAU, WORK, WORK( IPW ) )
*
*        Apply H' to A(ia:i-1,ja:ja+n-m+i+ib-ia-1) from the right
*
         CALL PYLARFB( 'Right', 'Conjugate transpose', 'Backward',      
     +            'Rowwise', I-IA, N-M+I+IB-IA, IB, A, I, JA,           
     +       DESCA, WORK, A, IA, JA, DESCA, WORK( IPW ) )
*
*        Apply H' to columns ja:ja+n-m+i+ib-ia-1 of current block
*
         CALL PYUNGR2( IB, N-M+I+IB-IA, IB, A, I, JA, DESCA, TAU, WORK, 
     +                 LWORK, IINFO )
*
*        Set rows i:i+ib-1,ja+n-m+i+ib-ia:ja+n-1 of current block to
*        zero
*
         CALL PYLASET( 'All', IB, M-I-IB+IA, ZERO, ZERO, A, I,          
