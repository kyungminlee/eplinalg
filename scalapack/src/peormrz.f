      SUBROUTINE PEORMRZ( SIDE, TRANS, M, N, K, L, A, IA, JA, DESCA,    
...
*  =======
*
*  PEORMRZ overwrites the general real M-by-N distributed matrix
...
*        Q = H(1) H(2) . . . H(k)
*
*  as returned by PETZRZF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          H(i), IA <= i <= IA+K-1, as returned by PETZRZF in the
...
*          elementary reflectors H(i) as returned by PETZRZF.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PELARZB,    
     +                PELARZT, PEORMR3, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PEORMRZ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N
         END IF
         CALL PEORMR3( SIDE, TRANS, MI, NI, IB, L, A, IA, JA, DESCA,    
...
*
         CALL PELARZT( 'Backward', 'Rowwise', L, IB, A, I, JAA, DESCA,  
     +                TAU, WORK, WORK( IPW ) )
         IF( LEFT ) THEN
*
...
*        Apply H or H'
*
         CALL PELARZB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI, IB, 
...
*     mirrors PEORMRQ's structure (peormrq.f:454: "(RIGHT .AND. TRAN)
