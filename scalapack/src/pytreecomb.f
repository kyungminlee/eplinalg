      SUBROUTINE PYTREECOMB( ICTXT, SCOPE, N, MINE, RDEST0, CDEST0,     
     +                   SUBPTR )
      IMPLICIT NONE
*
...
*  =======
*
*  PYTREECOMB does a 1-tree parallel combine operation on scalars,
...
*     ..
*     .. Local Arrays ..
      COMPLEX(KIND=10)            HIS( 2 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, YGEBR2D, YGEBS2D,              
     +      YGERV2D, YGESD2D
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
            ...
            END IF
*
            CALL YGESD2D( ICTXT, N, 1, MINE, N, RMSSG, CMSSG )
*
            GO TO 20
*
         ELSE
...
*
               CALL YGERV2D( ICTXT, N, 1, HIS, N, RMSSG, CMSSG )
...
*
      IF( BCAST ) THEN
         IF( MYDIST2.EQ.0 ) THEN
            CALL YGEBS2D( ICTXT, SCOPE, ' ', N, 1, MINE, N )
         ELSE
            CALL YGEBR2D( ICTXT, SCOPE, ' ', N, 1, MINE, N,             
     +        TRDEST, TCDEST )
         END IF
      END IF
*
...
*  =======
*
*  YCOMBAMAX finds the element having max. absolute value as well
